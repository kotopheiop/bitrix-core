...
class Comment extends \Bitrix\Socialnetwork\Controller\Base
{
    public function getSourceAction(array $params = [])
    {
        ...
        if ($commentId <= 0) {
            $this->addError(
                new Error(
                    Loc::getMessage('SONET_CONTROLLER_LIVEFEED_COMMENT_COMMENT_EMPTY'),
        ...
        if ($postId <= 0) {
            $this->addError(
                new Error(
                    Loc::getMessage('SONET_CONTROLLER_LIVEFEED_COMMENT_POST_NOT_FOUND'),
        ...
        if ($commentData) {
            ...
            $this->addError(
                new Error(
                    Loc::getMessage('SONET_CONTROLLER_LIVEFEED_COMMENT_COMMENT_NOT_FOUND'),
