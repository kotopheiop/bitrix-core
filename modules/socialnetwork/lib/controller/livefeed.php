...
class Livefeed extends \Bitrix\Main\Engine\Controller
{
    ...
    public function deleteEntryAction($logId = 0)
    {
        ...
        if (!Loader::includeModule('socialnetwork')) {
            $this->addError(
                new Error('Cannot include Socialnetwork module', 'SONET_CONTROLLER_LIVEFEED_NO_SOCIALNETWORK_MODULE')
            );
            return null;
        }
    ...
    {
        $entityType = (isset($params['entityType']) && $params['entityType'] <> '' ? preg_replace(
            "/[^a-z0-9_]/i",
            '',
            $params['entityType']
        ) : false);
        ...
        $additionalParams = (isset($params['additionalParams']) && is_array(
    ...
    {
        $postEntityType = (isset($params['postEntityType']) && $params['postEntityType'] <> '' ? preg_replace(
            '/[^a-z0-9_]/i',
            '',
            $params['postEntityType']
        ) : false);
        $sourceEntityType = (isset($params['sourceEntityType']) && $params['sourceEntityType'] <> '' ? preg_replace(
            '/[^a-z0-9_]/i',
            '',
        ...
        if (in_array(
            $sourceEntityType,
            [CommentAux\CreateEntity::SOURCE_TYPE_BLOG_POST, CommentAux\CreateEntity::SOURCE_TYPE_BLOG_COMMENT]
        )) {
    ...
    public function createTaskCommentAction(array $params = []): void
    {
        ...
        $sourceEntityType = (isset($params['entityType']) && $params['entityType'] <> '' ? preg_replace(
            "/[^a-z0-9_]/i",
            '',
            $params['entityType']
        ) : false);
    ...
    public function changeFavoritesAction($logId, $value)
    {
        ...
        if ($res = \CSocNetLogFavorites::change($currentUserId, $logId)) {
            ...
            $this->addError(
                new Error(
                    (($e = $APPLICATION->getException()) ? $e->getString() : 'Cannot change log entry favorite value'),
    ...
    public function changeExpertModeAction($value)
    {
        ...
        \Bitrix\Socialnetwork\LogViewTable::set(
    ...
    public function getNextPageAction(array $params = [])
    {
        ...
        $componentResponse = new \Bitrix\Main\Engine\Response\Component(
            ...
            [],
            $this->getComponentReturnWhiteList()
        );
