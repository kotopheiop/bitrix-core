...
class UserToGroup extends Base
{
    public function joinAction(array $params = [])
    {
        ...
        $userId = (isset($params['userId']) && intval($params['userId']) > 0 ? intval(
            $params['userId']
        ...
        if (!Loader::includeModule('socialnetwork')) {
            $this->addError(
                new Error('Cannot include Socialnetwork module', 'SONET_CONTROLLER_USERTOGROUP_NO_SOCIALNETWORK_MODULE')
            );
            return null;
        }
        ...
        if ($relationFields = $res->fetch()) {
            if (in_array($relationFields['ROLE'], UserToGroupTable::getRolesMember())) {
                $this->addError(
                    new Error('User is already a member of the group', 'SONET_CONTROLLER_USERTOGROUP_ALREADY_MEMBER')
                );
                return null;
                ...
                if (!\CSocNetUserToGroup::userConfirmRequestToBeMember($userId, $relationFields['ID'], false)) {
                    $this->addError(
                        new Error(
                            (($e = $APPLICATION->getException()) ? $e->getString() : 'Cannot join the group'),
                            'SONET_CONTROLLER_USERTOGROUP_JOIN_ERROR'
                        )
                    );
                    return null;
                }
