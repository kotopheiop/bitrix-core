...
class Workgroup extends \Bitrix\Socialnetwork\Controller\Base
{
    public function getAction(array $params = [])
    {
        ...
        if ($groupId <= 0) {
            $this->addError(
                new Error(Loc::getMessage('SONET_CONTROLLER_WORKGROUP_EMPTY'), 'SONET_CONTROLLER_WORKGROUP_EMPTY')
            );
            return null;
        }
        ...
        $this->addError(
            new Error(Loc::getMessage('SONET_CONTROLLER_WORKGROUP_NOT_FOUND'), 'SONET_CONTROLLER_WORKGROUP_NOT_FOUND')
        );
        return null;
    }
    ...
    public function setFavoritesAction(array $params = [])
    {
        ...
        if ($value === false) {
            $this->addError(
                new Error('SONET_CONTROLLER_WORKGROUP_INCORRECT_VALUE', 'SONET_CONTROLLER_WORKGROUP_INCORRECT_VALUE')
            );
            return null;
        }
        ...
        if (!Loader::includeModule('socialnetwork')) {
            $this->addError(
                new Error('SONET_CONTROLLER_MODULE_NOT_INSTALLED', 'SONET_CONTROLLER_MODULE_NOT_INSTALLED')
            );
            return null;
        }
        ...
        if (!$res) {
            $this->addError(
                new Error('SONET_CONTROLLER_WORKGROUP_ACTION_FAILED', 'SONET_CONTROLLER_WORKGROUP_ACTION_FAILED')
            );
            return null;
        }
