...
class ServiceComment
{
    public static function processBlogCreateEntity($params): bool
    {
        ...
        $sourceEntityType = (isset($params['SOURCE_ENTITY_TYPE']) && in_array(
            $params['SOURCE_ENTITY_TYPE'],
            ['BLOG_POST', 'BLOG_COMMENT']
        ) ? $params['SOURCE_ENTITY_TYPE'] : false);
...
//					'LOG_DATE'
    ...
    protected static function getAuxLiveParams(array $params = []): array
    {
        ...
        if ($entityType === CommentAux\CreateEntity::ENTITY_TYPE_TASK) {
            ...
            $result['attendees'] = (!empty($entityData['USER_IDS']) && is_array(
                $entityData['USER_IDS']
            ) ? $entityData['USER_IDS'] : []);
        }
        ...
        switch ($entityType) {
            case CommentAux\CreateEntity::ENTITY_TYPE_TASK:
                if (Loader::includeModule('tasks')) {
                    $result = \CTaskNotifications::getNotificationPath(
                        ['ID' => $entityData['CREATED_BY']],
                        $entityData['ID'],
                        false
                    );
                }
