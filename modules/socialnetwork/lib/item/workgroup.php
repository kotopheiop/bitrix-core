...
class Workgroup
{
    ...
    public function getGroupUrlData($params = array())
    {
        ...
        if ($userId) {
            ...
            $serverName = (mb_strpos($groupUrl, "http://") === 0 || mb_strpos(
                $groupUrl,
                "https://"
            ) === 0 ? "" : $tmp["SERVER_NAME"]);
            $domainName = (mb_strpos($groupUrl, "http://") === 0 || mb_strpos(
                $groupUrl,
                "https://"
            ) === 0 ? "" : (isset($tmp["DOMAIN"]) && !empty($tmp["DOMAIN"]) ? "//" . $tmp["DOMAIN"] : ""));
        }
    ...
    public static function getPresets($params = array())
    {
        ...
        if (!$currentExtranetSite) {
            if (self::checkEntityOption(['open', '!extranet', '!landing'], $entityOptions)) {
                $result['project-open'] = [
                    'SORT' => $sort += 10,
                    'NAME' => ($intranetInstalled ? Loc::getMessage(
                        'SOCIALNETWORK_ITEM_WORKGROUP_TYPE_GP_OPEN'
                    ) : Loc::getMessage('SOCIALNETWORK_ITEM_WORKGROUP_TYPE_GROUP_OPEN')),
            ...
            if (self::checkEntityOption(['!open', '!extranet', '!landing'], $entityOptions)) {
                $result['project-closed'] = [
                    ...
                    ) : Loc::getMessage('SOCIALNETWORK_ITEM_WORKGROUP_TYPE_GROUP_CLOSED')),
            ...
            if (
                ...
                $result['project-closed-visible'] = [
                    ...
                    ) : Loc::getMessage('SOCIALNETWORK_ITEM_WORKGROUP_TYPE_GROUP_CLOSED_VISIBLE')),
        ...
        if (!empty($groupFieldsList)) {
            ...
            if (
                ...
                if ($userFields = $res->fetch()) {
                    $content .= ' ' . \CUser::formatName(
                            \CSite::getNameFormat(null, $groupFieldsList['SITE_ID']),
                            $userFields,
                            true
                        );
                }
    ...
    public static function setIndex($params = array())
    {
        ...
        if (!empty($content)) {
            ...
            $connection->query(
                "UPDATE " . WorkgroupTable::getTableName(
    ...
    public static function getInitiatePermOptionsList($params = array())
    {
        ...
        return array(
            UserToGroupTable::ROLE_OWNER => Loc::getMessage(
                $project ? "SOCIALNETWORK_ITEM_WORKGROUP_IP_OWNER_PROJECT" : "SOCIALNETWORK_ITEM_WORKGROUP_IP_OWNER"
            ),
            UserToGroupTable::ROLE_MODERATOR => Loc::getMessage(
                $project ? "SOCIALNETWORK_ITEM_WORKGROUP_IP_MOD_PROJECT" : "SOCIALNETWORK_ITEM_WORKGROUP_IP_MOD"
            ),
            UserToGroupTable::ROLE_USER => GetMessage(
                $project ? "SOCIALNETWORK_ITEM_WORKGROUP_IP_USER_PROJECT" : "SOCIALNETWORK_ITEM_WORKGROUP_IP_USER"
            ),
        );
    }
    ...
    public static function getByFeatureOperation(array $params = []): array
    {
        ...
        $userId = (isset($params['userId']) ? (int)$params['userId'] : (is_object(
            $USER
        ) && $USER instanceof \CUser ? $USER->getId() : 0));
