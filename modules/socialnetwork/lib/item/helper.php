...
class Helper
{
    public static function addBlogPost(
        ...
        if (
            ...
            if ($emailUserAllowed) {
                $pseudoHttpPostFields = [
                    'SPERM' => $params['SPERM'],
                    'INVITED_USER_NAME' => (!empty($params['INVITED_USER_NAME']) && is_array(
                        $params['INVITED_USER_NAME']
                    ) ? $params['INVITED_USER_NAME'] : []),
                    'INVITED_USER_LAST_NAME' => (!empty($params['INVITED_USER_NAME']) && is_array(
                        $params['INVITED_USER_LAST_NAME']
                    ) ? $params['INVITED_USER_LAST_NAME'] : []),
                    'INVITED_USER_CRM_ENTITY' => (!empty($params['INVITED_USER_CRM_ENTITY']) && is_array(
                        $params['INVITED_USER_CRM_ENTITY']
                    ) ? $params['INVITED_USER_CRM_ENTITY'] : []),
                    'INVITED_USER_CREATE_CRM_CONTACT' => (!empty($params['INVITED_USER_CREATE_CRM_CONTACT']) && is_array(
                        $params['INVITED_USER_CREATE_CRM_CONTACT']
                    ) ? $params['INVITED_USER_CREATE_CRM_CONTACT'] : []),
                ];
                $temporaryParams = [
        ...
        if ($postFields['TITLE'] == '') {
            ...
            $postFields['TITLE'] = preg_replace(
                ...
                ' ',
                \blogTextParser::killAllTags($postFields['DETAIL_TEXT'])
            );
        ...
        $inlineTagsList = \Bitrix\Socialnetwork\Util::detectTags(
            $postFields,
            ($postFields['MICRO'] === 'Y' ? ['DETAIL_TEXT'] : ['DETAIL_TEXT', 'TITLE'])
        );
        ...
        $postUrl = \CComponentEngine::makePathFromTemplate(
            htmlspecialcharsBack($pathToPost),
            [
                'post_id' => $result,
        ...
        if ($postFields['PUBLISH_STATUS'] === BLOG_PUBLISH_STATUS_PUBLISH) {
            ...
            ComponentHelper::notifyBlogPostCreated(
                [
                    ...
                    'siteId' => $siteId,
                    'postUrl' => $postUrl,
                    'socnetRights' => ($logId ? \Bitrix\Socialnetwork\Item\LogRight::get(
                        $logId
                    ) : $postFields['SOCNET_RIGHTS']),
                    'socnetRightsOld' => [],
                    'mentionListOld' => [],
    ...
    public static function updateBlogPost(
        $params = [],
        $scope = \Bitrix\Main\Engine\Controller::SCOPE_AJAX,
        &$resultFields = []
    ) {
        ...
        if (
            ...
            $updateFields['TITLE'] = preg_replace(
                ["/\n+/is" . BX_UTF_PCRE_MODIFIER, "/\s+/is" . BX_UTF_PCRE_MODIFIER],
                ' ',
                \blogTextParser::killAllTags($params['POST_MESSAGE'])
            );
        ...
        if (isset($params['IMPORTANT'])) {
            if ($params['IMPORTANT'] === 'Y') {
                ...
                if (!empty($params['IMPORTANT_DATE_END'])) {
                    $endDate = \CRestUtil::unConvertDate($params['IMPORTANT_DATE_END']);
                    if ($endDate) {
                        $updateFields['UF_IMPRTANT_DATE_END'] = \Bitrix\Main\Type\DateTime::createFromUserTime(
                            $endDate
                        );
                    }
                }
    ...
    public static function getBlogPostFields($postId)
    {
        ...
        $cacheTtl = 2592000;
        $cacheId = 'blog_post_socnet_general_' . $postId . '_' . LANGUAGE_ID . ($tzOffset <> 0 ? '_' . $tzOffset : '') . '_' . \Bitrix\Main\Context::getCurrent(
            )->getCulture()->getDateTimeFormat() . '_rest';
