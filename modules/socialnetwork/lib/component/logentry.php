...
class LogEntry extends \CBitrixComponent implements \Bitrix\Main\Engine\Contract\Controllerable, \Bitrix\Main\Errorable
{
    ...
    public static function getCommentsFullList(array $eventData, array &$params, array $options = [])
    {
        ...
        $commentEvent = (isset($options['commentEvent']) && is_array(
            $options['commentEvent']
        ) ? $options['commentEvent'] : []);
        ...
        $cacheId = 'log_comments_' . $params['LOG_ID'] . '_' . md5(
                serialize($cacheIdPartsList)
            ) . '_' . SITE_TEMPLATE_ID . '_' . SITE_ID . '_' . LANGUAGE_ID . '_' . FORMAT_DATETIME . '_' . $timeZoneOffzet . '_' . $nTopCount;
    ...
    public static function getLogCommentRecord(array $comment, array $params, array &$assets)
    {
        ...
        if (
            ...
            $dateTimeFormated = str_replace(
                array('-' . $currentYear, '/' . $currentYear, ' ' . $currentYear, '.' . $currentYear),
                '',
                $dateTimeFormated
            );
        }
        ...
        if (
            ...
            $formattedFields = call_user_func(
                [$commentEventData['CLASS_FORMAT'], $commentEventData['METHOD_FORMAT']],
                $comment,
                $params,
                false,
                $logFields
        ...
        if ($commentAuxProvider) {
            ...
            if ($message <> '') {
                $formattedFields['EVENT_FORMATTED']['FULL_MESSAGE_CUT'] = \CSocNetTextParser::closetags(
                    htmlspecialcharsback($message)
                );
            }
        }
        ...
        if (is_array($commentEventFields)) {
            ...
            if (
                ...
                $urlPreviewText = \Bitrix\Socialnetwork\ComponentHelper::getUrlPreviewContent(
                    $comment['UF']['UF_SONET_COM_URL_PRV'],
                    array(
                        'MOBILE' => 'N',
