    ...
    public function getText(): string
    {
        ...
        if (in_array($params['sourceType'], $this->getCommentTypeList())) {
            ...
            $result = Loc::getMessage(
                'SONET_COMMENTAUX_CREATEENTITY_COMMENT_' . $params['sourceType'] . (!empty($sourceData['suffix']) ? '_' . $sourceData['suffix'] : ''),
                [
            ...
            $result = Loc::getMessage(
                'SONET_COMMENTAUX_CREATEENTITY_POST_' . $params['sourceType'] . (!empty($suffix) ? '_' . $suffix : ''),
                [
                    '#ENTITY_CREATED#' => $this->getEntityCreatedMessage(),
                    '#ENTITY_NAME#' => $this->getEntityName(),
                ]
        ...
        if (
            ...
            && (!isset($options['mail']) || !$options['mail'])
                ? str_replace(
                    ...
                    $post['AUTHOR_ID'],
                    $userPage
                ) . 'blog/' . $post['ID'] . '/?commentId=' . $params['sourceId'] . '#com' . $params['sourceId']
                : ''
            );
        } else {
