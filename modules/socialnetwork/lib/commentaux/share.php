...
final class Share extends Base
{
    ...
    public function getText()
    {
            ...
            if (
                ...
                if (preg_match('/^(SG|U||UA|DR)(\d*)$/', $destinationCode, $matches)) {
                        ...
                        case 'UA':
                            if (
                                ...
                                $renderParts = new Livefeed\RenderParts\User(
                                    array_merge($options, ['skipLink' => $hiddenDestination])
                                );
                            }
                            break;
                        case 'DR':
                    ...
                    if (
                        ...
                        $entityType === 'U'
                        && (int)$entityId > 0
                            ? '[USER=' . $entityId . ']' . htmlspecialcharsback(
                                $entityDataFormatted['name']
                            ) . '[/USER]'
                            : '[URL=' . $entityDataFormatted['link'] . ']' . htmlspecialcharsback(
                                $entityDataFormatted['name']
                            ) . '[/URL]'
                        )
        ...
        $result .= Loc::getMessage(
            count($params['destinationList']) > 1 ? 'SONET_COMMENTAUX_SHARE_TEXT_1' : 'SONET_COMMENTAUX_SHARE_TEXT',
            [
