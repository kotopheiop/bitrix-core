...
class BlogPost extends Provider
{
    ...
    public function initSourceFields()
    {
        ...
        $this->setSourceTitle(
            truncateText(($post['MICRO'] === 'N' ? $post['TITLE'] : htmlspecialcharsback($post['TITLE'])), 100)
        );
    ...
    public function getPinnedDescription()
    {
        ...
        $result = truncateText(
            str_replace('&#39;', "'", htmlspecialcharsBack(\CTextParser::clearAllTags($post['DETAIL_TEXT']))),
            100
        );
        $result = preg_replace(
            '/^' . (\Bitrix\Main\Application::isUtfMode() ? "\xC2\xA0" : "\xA0") . '$/',
            '',
            $result
        );
