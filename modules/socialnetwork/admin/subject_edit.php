...
if ($ID > 0 && $socialnetworkModulePermissions >= "W") {
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('" . GetMessage(
                "SONETE_DELETE_SUBJECT_CONFIRM"
            ) . "')) window.location='/bitrix/admin/socnet_subject.php?ID=" . $ID . "&action=delete&lang=" . LANG . "&" . bitrix_sessid_get(
            ) . "#tb';",
        "WARNING" => "Y"
    );
}
...
<? CAdminMessage::ShowMessage($errorMessage); ?>
    ...
    <form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?" name="form1">
        ...
        $aTabs = array(
            array(
                "DIV" => "edit1",
                "TAB" => GetMessage("SONETE_TAB_SUBJECT"),
                "ICON" => "socialnetwork",
                "TITLE" => GetMessage("SONETE_TAB_SUBJECT_DESCR")
            )
        );
        ...
        <tr class="adm-detail-required-field">
            ...
            <td>
                ...
                while (($arSite = $rsSite->Fetch())) {
                    echo '<option value="' . htmlspecialcharsex($arSite["LID"]) . '"' . (in_array(
                            $arSite["LID"],
                            $arSubjectSites
                        ) ? ' selected' : '') . '>[' . htmlspecialcharsex(
                            $arSite["LID"]
                        ) . ']&nbsp;' . htmlspecialcharsex($arSite["NAME"]) . '</option>';
                }
