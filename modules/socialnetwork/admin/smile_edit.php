...
<form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?" name="fform" enctype="multipart/form-data">
    ...
    <tr>
        <td valign="top"><? echo GetMessage("SONET_TYPING") ?>
            :<br><small><? echo GetMessage("SONET_TYPING_NOTE") ?></small></td>
        <td valign="top">
    ...
    <tr class="adm-detail-required-field">
        <td class="adm-detail-valign-top"><? echo GetMessage("SONET_IMAGE") ?>
            :<br><small><? echo GetMessage("SONET_IMAGE_NOTE") ?></small></td>
        <td>
            <input type="file" name="IMAGE1" size="30"><?
            if (strlen($f_IMAGE) > 0) {
                ?>
                <div style="padding-top: 10px;"><img src="/bitrix/images/socialnetwork/<?
                echo ($f_SMILE_TYPE == "I") ? "icon" : "smile" ?>/<?
                echo $f_IMAGE ?>" border="0" <?
                echo (IntVal($f_IMAGE_WIDTH) > 0) ? "width=\"" . $f_IMAGE_WIDTH . "\"" : "" ?> <?
                echo (IntVal($f_IMAGE_WIDTH) > 0) ? "height=\"" . $f_IMAGE_HEIGHT . "\"" : "" ?>></div><?
            }
            ?></td>
    </tr><?
    ...
    for ($i = 0; $i < count($arSysLangs); $i++):
        ...
        <td>
            ...
            echo $arSysLangs[$i] ?>" value="<?
            echo $str_NAME ?>" size="40">
        </td>
        </tr><?
    endfor;
