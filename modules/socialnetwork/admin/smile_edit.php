...
if ($ID > 0 && $sonetPermissions == "W") {
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('" . GetMessage(
                "FSN_DELETE_SMILE_CONFIRM"
            ) . "')) window.location='/bitrix/admin/socnet_smile.php?action=delete&ID[]=" . $ID . "&lang=" . LANG . "&" . bitrix_sessid_get(
            ) . "#tb';",
        "WARNING" => "Y"
    );
}
...
<form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?" name="fform" enctype="multipart/form-data">
    ...
    $aTabs = array(
        array(
            "DIV" => "edit1",
            "TAB" => GetMessage("FSN_TAB_SMILE"),
            "ICON" => "sonet",
            "TITLE" => GetMessage("FSN_TAB_SMILE_DESCR")
        )
    );
    ...
    <tr>
        ...
        <td>
            <select name="SMILE_TYPE">
                <option value="S" <? if ($str_SMILE_TYPE == "S") {
                    echo "selected";
                } ?>><? echo GetMessage("FSE_SMILE"); ?></option>
                <option value="I" <? if ($str_SMILE_TYPE == "I") {
                    echo "selected";
                } ?>><? echo GetMessage("FSE_ICON"); ?></option>
            </select>
        </td>
    </tr>
    <tr>
        <td valign="top"><? echo GetMessage("SONET_TYPING") ?>:<br><small><? echo GetMessage(
                    "SONET_TYPING_NOTE"
                ) ?></small></td>
        <td valign="top">
    ...
    <tr class="adm-detail-required-field">
        <td class="adm-detail-valign-top"><? echo GetMessage("SONET_IMAGE") ?>:<br><small><? echo GetMessage(
                    "SONET_IMAGE_NOTE"
                ) ?></small></td>
        <td>
            ...
            if ($f_IMAGE <> '') {
                ...
                <div style="padding-top: 10px;"><img
                        src="/bitrix/images/socialnetwork/<? echo ($f_SMILE_TYPE == "I") ? "icon" : "smile" ?>/<? echo $f_IMAGE ?>"
                        border="0" <? echo (intval(
                        $f_IMAGE_WIDTH
                    ) > 0) ? "width=\"" . $f_IMAGE_WIDTH . "\"" : "" ?> <? echo (intval(
                        $f_IMAGE_WIDTH
                    ) > 0) ? "height=\"" . $f_IMAGE_HEIGHT . "\"" : "" ?>></div><?
            }
            ?></td>
    </tr><?
