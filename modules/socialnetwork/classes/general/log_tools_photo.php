...
class CSocNetLogToolsPhoto
{
    public static function OnAfterPhotoUpload($arFields, $arComponentParams, $arComponentResult)
    {
        ...
        if (is_set($arComponentParams["DETAIL_URL"]) && is_array(
                $arSiteWorkgroupsPage
            ) && $entity_type == SONET_ENTITY_GROUP) {
            foreach ($arSiteWorkgroupsPage as $groups_page) {
                if (mb_strpos($arComponentParams["DETAIL_URL"], $groups_page) === 0) {
                    $arComponentParams["DETAIL_URL"] = "#GROUPS_PATH#" . mb_substr(
                            $arComponentParams["DETAIL_URL"],
        ...
        if (
            ...
            $strSectionName = $arComponentResult["SECTION"]["PATH"][count(
                $arComponentResult["SECTION"]["PATH"]
            ) - 1]["NAME"];
        }
        ...
        if ($db_res && $res = $db_res->Fetch()) {
            ...
            $arSonetFields["TEXT_MESSAGE"] = str_replace(
                array("#TITLE#"),
...
class CSocNetPhotoCommentEvent
{
    ...
    public static function AddComment_Photo($arFields)
    {
        ...
        if ($arLogType["TYPE"] == "FORUM") {
            $arReturn = CSocNetPhotoCommentEvent::AddComment_Photo_Forum(
                $arFields,
                $arLogType["ENTITY_ID"],
                $arLogType["LOG"]
            );
        } elseif ($arLogType["TYPE"] == "BLOG") {
            $arReturn = CSocNetPhotoCommentEvent::AddComment_Photo_Blog(
                $arFields,
                $arLogType["ENTITY_ID"],
                $arLogType["LOG"]
            );
        ...
        if ($arElement) {
            ...
            if (intval($TOPIC_ID) > 0) {
                if (COption::GetOptionString("forum", "FILTER", "Y") == "Y") {
                    $arFields["TEXT_MESSAGE"] = $arFilteredText["TEXT_MESSAGE"] = CFilterUnquotableWords::Filter(
...
//			$arReturn = CSocNetPhotoCommentEvent::DeleteComment_Photo_Blog($arFields, $arLogType["ENTITY_ID"], $arLogType["LOG"]);
            ...
            if ($arRes = $dbRes->Fetch()) {
                ...
                if ($arElement = $rsElement->Fetch()) {
                    ...
                    if ($arSection = $rsSection->Fetch()) {
                        ...
                        $arLogParams["SECTION_URL"] = str_replace(
                            "#SECTION_ID#",
                            $arSection["ID"],
                            $this->arPath["SECTION_URL"]
                        );
                        ...
                        if (!$arSiteWorkgroupsPage && IsModuleInstalled(
                                "extranet"
                            ) && $entity_type == SONET_ENTITY_GROUP) {
                            $rsSite = CSite::GetList("sort", "desc", Array("ACTIVE" => "Y"));
                            while ($arSite = $rsSite->Fetch()) {
                        ...
                        if (is_set($arLogParams["SECTION_URL"]) && is_array(
                                $arSiteWorkgroupsPage
                            ) && $entity_type == SONET_ENTITY_GROUP) {
                            foreach ($arSiteWorkgroupsPage as $groups_page) {
                                if (mb_strpos($arLogParams["SECTION_URL"], $groups_page) === 0) {
                                    $arLogParams["SECTION_URL"] = "#GROUPS_PATH#" . mb_substr(
                                            $arLogParams["SECTION_URL"],
                                            mb_strlen($groups_page),
                                            mb_strlen($arLogParams["SECTION_URL"]) - mb_strlen($groups_page)
                                        );
                                }
                            }
                        }
                    ...
                    $arSonetFields = array(
                        ...
                        "TITLE_TEMPLATE" => str_replace(
                            "#AUTHOR_NAME#",
                            $sAuthorName,
                            GetMessage("SONET_PHOTOPHOTO_LOG_1")
                        ),
                ...
                if ($bSocNetLogRecordExists) {
                    ...
                    $arFieldsForSocnet = array(
                        ...
                        "MESSAGE" => $parser->convert(
                            empty($arFields["POST_MESSAGE_FILTER"]) ? $arFields["POST_MESSAGE"] : $arFields["POST_MESSAGE_FILTER"],
                            $arAllow
                        ),
                        "TEXT_MESSAGE" => $parser->convert4mail(
                            empty($arFields["POST_MESSAGE_FILTER"]) ? $arFields["POST_MESSAGE"] : $arFields["POST_MESSAGE_FILTER"]
                        ),
                        "MODULE_ID" => false,
                        "SOURCE_ID" => $ID,
                    ...
                    while ($arComment = $dbComments->GetNext()) {
                        $arFieldsForSocnet = array(
                            ...
                            "MESSAGE" => $parser->convert(
                                empty($arComment["POST_MESSAGE_FILTER"]) ? $arComment["POST_MESSAGE"] : $arComment["POST_MESSAGE_FILTER"],
                                $arAllow
                            ),
                            "TEXT_MESSAGE" => $parser->convert4mail(
                                empty($arComment["POST_MESSAGE_FILTER"]) ? $arComment["POST_MESSAGE"] : $arComment["POST_MESSAGE_FILTER"]
                    ...
                    if ($arElement) {
                        self::InheriteAlbumFollow(
                            $arElement["IBLOCK_SECTION_ID"],
                            $log_id,
                            (intval($arElement["CREATED_BY"]) > 0 ? $arElement["CREATED_BY"] : false)
                        );
                    }
                }
            }
    ...
    public static function NotifyIm($arParams)
    {
        ...
        $arTmp = CSocNetLogTools::ProcessPath(
            array("PHOTO_URL" => $arParams["URL"], "SECTION_URL" => $arParams["SECTION_URL"]),
            $arParams["PHOTO_AUTHOR_ID"]
        );
        $serverName = $arTmp["SERVER_NAME"];
        ...
        $arMessageFields["NOTIFY_MESSAGE"] = GetMessage(
            ...
            Array(
                "#photo_title#" => "<a href=\"" . $arParams["URL"] . "\" class=\"bx-notifier-item-action\">" . htmlspecialcharsbx(
                        $arParams["TITLE"]
                    ) . "</a>",
                "#album_title#" => "<a href=\"" . $arParams["SECTION_URL"] . "\" class=\"bx-notifier-item-action\">" . htmlspecialcharsbx(
                        $arParams["SECTION_NAME"]
                    ) . "</a>"
            )
        );
