...
class CAllSocNetUserToGroup
{
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if ((is_set($arFields, "ROLE") || $ACTION == "ADD") && $arFields["ROLE"] == '') {
            ...
            $APPLICATION->ThrowException(
                str_replace("#ID#", $arFields["ROLE"], GetMessage("SONET_UG_ERROR_NO_ROLE")),
                "ERROR_NO_ROLE"
            );
            return false;
        }
        ...
        if ((is_set($arFields, "INITIATED_BY_TYPE") || $ACTION == "ADD") && $arFields["INITIATED_BY_TYPE"] == '') {
            ...
            return false;
        } elseif (is_set($arFields, "INITIATED_BY_TYPE") && !in_array(
                $arFields["INITIATED_BY_TYPE"],
                $arSocNetAllowedInitiatedByType
            )) {
            $APPLICATION->ThrowException(
                str_replace("#ID#", $arFields["INITIATED_BY_TYPE"], GetMessage("SONET_UG_ERROR_NO_INITIATED_BY_TYPE")),
                "ERROR_NO_INITIATED_BY_TYPE"
            );
            return false;
        }

        if ((is_set($arFields, "INITIATED_BY_USER_ID") || $ACTION == "ADD") && intval(
                $arFields["INITIATED_BY_USER_ID"]
            ) <= 0) {
            $APPLICATION->ThrowException(
                GetMessage("SONET_UG_EMPTY_INITIATED_BY_USER_ID"),
            ...
            if (!$dbResult->Fetch()) {
                $APPLICATION->ThrowException(
                    GetMessage("SONET_UG_ERROR_NO_INITIATED_BY_USER_ID"),
            ...
            if (CModule::IncludeModule("im")) {
                ...
                if ($bSendExclude) {
                    $arMessageFields = array(
                        ...
                        "NOTIFY_TAG" => "SOCNET|INVITE_GROUP|" . intval($arUser2Group["USER_ID"]) . "|" . intval(
                                $arUser2Group["ID"]
                            ),
                        "NOTIFY_MESSAGE" => str_replace(
                            array("#NAME#"),
                            array($arUser2Group["GROUP_NAME"]),
                            GetMessage("SONET_UG_EXCLUDE_MESSAGE")
                        )
                    );
    ...
    public static function SendEvent($userGroupID, $mailTemplate = "SONET_INVITE_GROUP")
    {
        ...
        $requestsPagePath = str_replace(
            ...
            COption::GetOptionString(
                ...
                (IsModuleInstalled(
                    "intranet"
                )) ? "/company/personal/user/#USER_ID#/requests/" : "/club/user/#USER_ID#/requests/",
                $siteID
            )
        );
    ...
    /************  ACTIONS  ****************/
    /***************************************/
    public static function SendRequestToBeMember(
        $userID,
        $groupID,
        $message,
        $RequestConfirmUrl = "",
        ...
        if ($arGroup["OPENED"] == "Y") {
            ...
            if ($dbRequests) {
                ...
                $groupUrlTemplate = "#GROUPS_PATH#" . mb_substr(
                        $groupUrlTemplate,
                        mb_strlen($workgroupsPage),
                        mb_strlen($groupUrlTemplate) - mb_strlen(
                            $workgroupsPage
                        )
                    );
                ...
                while ($arRequests = $dbRequests->GetNext()) {
                    ...
                    $arMessageFields = array(
                        ...
                        "NOTIFY_TAG" => "SOCNET|REQUEST_GROUP|" . intval($userID) . "|" . $groupID . "|" . intval(
                                $ID
                            ) . "|" . $arRequests["USER_ID"],
                        "NOTIFY_SUB_TAG" => "SOCNET|REQUEST_GROUP|" . intval($userID) . "|" . $groupID . "|" . intval(
                                $ID
                            ),
                        ...
                        "NOTIFY_BUTTONS" => Array(
                            Array(
                                "TITLE" => GetMessage("SONET_UG_REQUEST_CONFIRM"),
                                "VALUE" => "Y",
                                "TYPE" => "accept"
                            ),
                    ...
                    $groupUrl = $serverName . str_replace(
                            "#group_id#",
                            $groupID,
                            COption::GetOptionString(
                                "socialnetwork",
    ...
    public static function SendRequestToJoinGroup($senderID, $userID, $groupID, $message, $bMail = true)
    {
        ...
        $arFields = array(
            ...
            "MESSAGE" => str_replace(
                ...
                Array($message, $arGroup["NAME"]),
                (empty($message) ? GetMessage("SONET_UG_INVITE_CONFIRM_TEXT_EMPTY") : GetMessage(
        ...
        if (
            ...
            $arMessageFields['NOTIFY_MESSAGE_OUT'] .= "\n\n" . GetMessage(
                ...
                ) . ": " . $requestUrl . '?INVITE_GROUP=' . $ID . '&CONFIRM=Y';
            $arMessageFields['NOTIFY_MESSAGE_OUT'] .= "\n\n" . GetMessage(
                    'SONET_UG_INVITE_REJECT'
                ) . ": " . $requestUrl . '?INVITE_GROUP=' . $ID . '&CONFIRM=N';
    ...
    public static function ConfirmRequestToBeMember(
        $userID,
        $groupID,
        $arRelationID,
        $bAutoSubscribe = true
    ) // request from a user confirmed by a moderator
    {
        global $APPLICATION, $DB, $USER;
        ...
        $bUserIsMember = ($userRole && in_array(
                $userRole,
                array(SONET_ROLES_OWNER, SONET_ROLES_MODERATOR, SONET_ROLES_USER)
            ));
        $bCanInitiate = (
            $USER->IsAdmin()
            || CSocNetUser::IsCurrentUserModuleAdmin($arGroupSites)
            || (
                ...
                && (
                    ...
                    || ($arGroup["INITIATE_PERMS"] == SONET_ROLES_MODERATOR && in_array(
                            $userRole,
                            array(
                                SONET_ROLES_OWNER,
        ...
        for ($i = 0; $i < $tmp_count; $i++) {
            ...
            if (CSocNetUserToGroup::Update($arRelation["ID"], $arFields)) {
                ...
                if (
                    ...
                    $groupSiteId = CSocNetGroup::GetDefaultSiteId($groupID, $arGroup["SITE_ID"]);
                    $workgroupsPage = COption::GetOptionString(
                        "socialnetwork",
                        "workgroups_page",
                        "/workgroups/",
                        SITE_ID
                    ...
                    $arMessageFields = array(
                        ...
                        "NOTIFY_MODULE" => "socialnetwork",
                        "NOTIFY_EVENT" => "invite_group",
                        "NOTIFY_TAG" => "SOCNET|INVITE_GROUP|" . intval($arRelation["USER_ID"]) . "|" . intval(
                                $arRelation["ID"]
                            ),
                        "NOTIFY_MESSAGE" => str_replace(
                            "#NAME#",
                    ...
                    CIMNotify::DeleteBySubTag(
                        "SOCNET|REQUEST_GROUP|" . $arRelation["USER_ID"] . "|" . $arRelation["GROUP_ID"] . "|" . $arRelation["ID"]
                    );
                    CIMNotify::Add($arMessageFields);
                }
            } else {
            ...
            if (CSocNetUserToGroup::Delete($arRelation["ID"])) {
                $arMessageFields = array(
                    ...
                    "MESSAGE" => str_replace(
                        "#NAME#",
                        $arGroup["NAME"],
                        GetMessage("SONET_UG_REJECT_MEMBER_MESSAGE_G")
                    ),
                    "=DATE_CREATE" => $DB->CurrentTimeFunction(),
    ...
    public static function UserConfirmRequestToBeMember(
        $targetUserID,
        $relationID,
        $bAutoSubscribe = true
    ) // request from group confirmed by a user
    {
        global $APPLICATION, $DB;
    ...
    public static function TransferModerator2Member($userID, $groupID, $arRelationID, $currentUserIsAdmin)
    {
        ...
        $groupUrlTemplate = "#GROUPS_PATH#" . mb_substr(
                $groupUrlTemplate,
                mb_strlen($workgroupsPage),
                mb_strlen($groupUrlTemplate) - mb_strlen($workgroupsPage)
            );
        $groupUrl = str_replace(array("#group_id#", "#GROUP_ID#"), $groupID, $groupUrlTemplate);
            ...
            if (CSocNetUserToGroup::Update($arRelation["ID"], $arFields)) {
                ...
                if ($bIMIncluded) {
                    ...
                    $arMessageFields = array(
                        ...
                        "NOTIFY_TAG" => "SOCNET|MOD_GROUP|" . intval(
                                $userID
                            ) . "|" . $groupID . "|" . $arRelation["ID"] . "|" . $arRelation["USER_ID"],
                        "NOTIFY_MESSAGE" => str_replace(
                            array("#NAME#"),
        ...
        if ($relationsToUpdateCount <= 0) {
            $APPLICATION->ThrowException(
                GetMessage("SONET_UR_ERROR_MEM2MOD_EMPTY_CORRECT_LIST"),
        ...
        if (!empty($successfulUserIdList)) {
            Integration\Im\Chat\Workgroup::setChatManagers(
                array(
                    'group_id' => $groupID,
        ...
        if (
            ...
            $APPLICATION->ThrowException(
                GetMessage("SONET_UR_ERROR_MOD2MEM_INCORRECT_PARAMS"),
                ...
                CSocNetMessages::Add(
                    array(
                        ...
                        "MESSAGE" => str_replace(
                            "#NAME#",
                            $arGroup["NAME"],
                            GetMessage("SONET_UG_UNBANMEMBER_MESSAGE")
                        ),
                        "=DATE_CREATE" => $DB->CurrentTimeFunction(),
                        "MESSAGE_TYPE" => SONET_MESSAGE_SYSTEM
                    )
        ...
        if ($userID <= 0) {
            ...
            if ($bCurrentUserIsAdmin) {
                ...
                if ($arReturn["UserIsMember"]) {
                    ...
                    $arReturn["UserCanModerateGroup"] = (in_array(
                        $arReturn["UserRole"],
                        array(SONET_ROLES_OWNER, SONET_ROLES_MODERATOR)
                    ));
    ...
    public static function NotifyImToModerators($arNotifyParams)
    {
        ...
        $arMessageFields = array(
            ...
            "NOTIFY_TAG" => "SOCNET|" . $notify_tag . "|" . intval($arNotifyParams["USER_ID"]) . "|" . intval(
                    $arNotifyParams["GROUP_ID"]
                ) . "|" . intval($arNotifyParams["RELATION_ID"]),
        );
    ...
    public static function notifyModeratorAdded($params)
    {
        ...
        $relationFields = (!empty($params['relationFields']) && is_array(
            $params['relationFields']
        ) ? $params['relationFields'] : array());
        $groupFields = (!empty($params['groupFields']) && is_array(
            $params['groupFields']
        ) ? $params['groupFields'] : array());
        $groupId = (
        ...
        $arMessageFields = array(
            ...
            "NOTIFY_TAG" => "SOCNET|MOD_GROUP|" . intval(
                    $userId
                ) . "|" . $groupId . "|" . $relationFields["ID"] . "|" . $relationFields["USER_ID"],
            "NOTIFY_MESSAGE" => str_replace(
                array("#NAME#"),
