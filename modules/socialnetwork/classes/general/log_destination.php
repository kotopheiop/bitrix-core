...
class CSocNetLogDestination
{
    ...
    public static function GetStucture($arParams = Array())
    {
        ...
        if ($bIntranetEnabled) {
            if (!(CModule::IncludeModule('extranet') && !CExtranet::IsIntranetUser())) {
                if (($iblock_id = COption::GetOptionInt('intranet', 'iblock_structure', 0)) > 0) {
                    ...
                    $cache_id = 'sonet_structure_new4_' . $iblock_id . (intval(
                            $department_id
                        ) > 0 ? "_" . $department_id : "");
        ...
        if (
            ...
            $result["department_relation"] = self::GetTreeList(
                'DR' . (intval($department_id) > 0 ? $department_id : 0),
                $result["department"],
                true
            );
            if (intval($arParams["HEAD_DEPT"]) > 0) {
                $result["department_relation_head"] = self::GetTreeList(
                    'DR' . intval($arParams["HEAD_DEPT"]),
                    $result["department"],
                    true
                );
            }
    ...
    public static function GetUsers($arParams = Array(), $bSelf = true)
    {
        ...
        $cacheTtl = 3153600;
        $cacheId = 'socnet_destination_getusers_' . md5(
                serialize($filter)
            ) . $bSelf . CSocNetUser::IsCurrentUserModuleAdmin() . ($bExtranet ? '_ex_' . $userId : '') . md5(
                serialize($avatarSize)
            );
        ...
        if ($obCache->InitCache($cacheTtl, $cacheId, $cacheDir)) {
            ...
            if (
                ...
                foreach ($arExtranetUsers as $arUserTmp) {
                    ...
                    $sName = trim(
                        CUser::FormatName(
                            empty($arParams["NAME_TEMPLATE"]) ? CSite::GetNameFormat(
                                false
                ...
                $className = (\Bitrix\Main\Loader::includeModule(
                    'intranet'
                ) ? '\Bitrix\Intranet\UserTable' : '\Bitrix\Main\UserTable');
    ...
    public static function GetGratMedalUsers($arParams = Array())
    {
        ...
        if (!isset($resultCache[$userId])) {
            ...
            while ($arUser = $dbUsers->GetNext()) {
                $sName = trim(
                    CUser::FormatName(
                        empty($arParams["NAME_TEMPLATE"]) ? CSite::GetNameFormat(false) : $arParams["NAME_TEMPLATE"],
                        $arUser
                    )
                );
    ...
    public static function searchUsers(
        ...
        if (is_array($search)) {
            ...
            $bCrmEmailUsers = (isset($arParams["CRMEMAIL_USERS"]) && ModuleManager::isModuleInstalled(
                'crm'
            ) ? $arParams["CRMEMAIL_USERS"] : false);
        ...
        $bEmailUsersAll = ($bMailEnabled && \Bitrix\Main\Config\Option::get(
                'socialnetwork',
                'email_users_all',
                'N'
            ) === 'Y');
        ...
        if ($useFulltextIndex) {
            $filter['*INDEX_SELECTOR.SEARCH_SELECTOR_CONTENT'] = \Bitrix\Main\Search\Content::prepareStringToken(
                implode(' ', $arSearchValue)
            );
        } else {
        ...
        if ($bIntranetEnabled) {
            ...
            if (
                ...
                $rsUser = \Bitrix\Main\FinderDestTable::getList(
                    array(
                        'order' => array(),
        ...
        $rsUser = null;
        if ($useFulltextIndex) {
            $rsUserFulltext = \Bitrix\Main\UserTable::getList(
                array(
                    'order' => array(
        ...
        if (
            ...
            $rsUser = \Bitrix\Main\UserTable::getList(
                array(
                    ...
                    'select' => array(
                        ...
                        new \Bitrix\Main\Entity\ExpressionField(
                            'EMAIL_OK',
                            'CASE WHEN UPPER(%s) = "' . $DB->ForSql(
                                mb_strtoupper(str_replace('%', '%%', $search))
                            ) . '" THEN 1 ELSE 0 END',
                            'EMAIL'
                        )
                    ),
            ...
            while ($arUser = $rsUser->fetch()) {
                $arUsers['U' . $arUser["ID"]] = self::formatUser(
                    $arUser,
                    array(
                        "NAME_TEMPLATE" => $nameTemplate,
                        "USE_EMAIL" => true,
            ...
            if ($group["IMAGE_ID"]) {
                ...
                if ($imageFile !== false) {
                    $arFileTmp = CFile::resizeImageGet(
                        ...
                        array(
                            "width" => (intval(
                                $params["THUMBNAIL_SIZE_WIDTH"]
                            ) > 0 ? $params["THUMBNAIL_SIZE_WIDTH"] : 100),
                            "height" => (intval(
                                $params["THUMBNAIL_SIZE_HEIGHT"]
                            ) > 0 ? $params["THUMBNAIL_SIZE_HEIGHT"] : 100)
                        ),
        ...
        if (
            ...
            if (check_email($search, true)) {
                ...
                if (
                    ...
                    while ($ar = $dbRes->Fetch()) {
                        $formatted = self::formatCrmEmailEntity(
                            $ar,
                            array(
                                'TYPE' => 'CONTACT',
                    ...
                    if ($imageFile !== false) {
                        $arFileTmp = CFile::ResizeImageGet(
                            ...
                            array(
                                ...
                                ) > 0 ? $arParams["THUMBNAIL_SIZE_WIDTH"] : 100),
                                "height" => (intval(
                                    $arParams["THUMBNAIL_SIZE_HEIGHT"]
                                ) > 0 ? $arParams["THUMBNAIL_SIZE_HEIGHT"] : 100)
                            ),
                            BX_RESIZE_IMAGE_PROPORTIONAL,
                            false
                        );
    ...
    private static function GetSocnetGroupFilteredByFeaturePerms(&$arGroups, $arFeaturePerms)
    {
        ...
        foreach ($operations as $operation) {
            $tmpOps = CSocNetFeaturesPerms::CurrentUserCanPerformOperation(
                SONET_ENTITY_GROUP,
                $arGroupsIDs,
                $feature,
                $operation
    ...
    public static function getUsersAll($arParams = [])
    {
        ...
        $arFields["IS_ONLINE"] = Array(
            "FIELD" => "IF(U.LAST_ACTIVITY_DATE > DATE_SUB(NOW(), INTERVAL " . \CUser::getSecondsForLimitOnline(
                ) . " SECOND), 'Y', 'N')"
        );
