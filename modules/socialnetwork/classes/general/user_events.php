...
class CAllSocNetUserEvents
{
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if ((is_set($arFields, "EVENT_ID") || $ACTION == "ADD") && $arFields["EVENT_ID"] == '') {
            ...
            $GLOBALS["APPLICATION"]->ThrowException(
                str_replace("#ID#", $arFields["EVENT_ID"], GetMessage("SONET_UE_ERROR_NO_EVENT_ID")),
                "ERROR_NO_EVENT_ID"
            );
            return false;
        }
        ...
        if ((is_set($arFields, "SITE_ID") || $ACTION == "ADD") && $arFields["SITE_ID"] == '') {
            ...
            if (!$dbResult->Fetch()) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["SITE_ID"], GetMessage("SONET_UE_ERROR_NO_SITE")),
                    "ERROR_NO_SITE"
                );
                return false;
            }
    ...
    public static function GetEventSite($userID, $event, $defSiteID)
    {
        ...
        $arUserEvents = array();
        if (isset($GLOBALS["SONET_USER_EVENTS_" . $userID]) && is_array(
                $GLOBALS["SONET_USER_EVENTS_" . $userID]
            ) && !in_array("SONET_USER_EVENTS_" . $userID, $_REQUEST)) {
