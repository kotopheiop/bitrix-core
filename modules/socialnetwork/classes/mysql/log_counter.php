...
class CSocNetLogCounter extends CAllSocNetLogCounter
{
    ...
    function Increment(
        ...
        $subSelect = $counter->GetSubSelect(
            $log_id,
            $entity_type,
            $entity_id,
            $event_id,
    ...
    function ClearByUser($user_id, $site_id = SITE_ID, $code = "**", $page_size = 0, $page_last_date_1 = "")
    {
        ...
        $strSQL = "
			...
			VALUES ($user_id, '" . $DB->ForSQL($site_id) . "', '" . $DB->ForSQL(
                $code
            ) . "', 0, " . $DB->CurrentTimeFunction() . ", " . (intval(
                $page_size
            ) > 0 ? $page_size : "NULL") . ", " . ($page_last_date_1 <> '' ? $DB->CharToDateFunction(
                $page_last_date_1
            ) : "NULL") . ")
			ON DUPLICATE KEY UPDATE CNT = 0, LAST_DATE = " . $DB->CurrentTimeFunction() . (intval(
                $page_size
            ) > 0 ? ", PAGE_SIZE = " . $page_size : "") . ($page_last_date_1 <> '' ? ", PAGE_LAST_DATE_1 = " . $DB->CharToDateFunction(
                    $page_last_date_1
                ) : "") . "
		";
