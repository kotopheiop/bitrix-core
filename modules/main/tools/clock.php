    ...
    public static function Show($arParams)
    {
        ...
        switch ($arParams['view']) {
            case 'label':
                ...
                <div class="bx-clock-label" onmouseover="this.className='bx-clock-label-over';"
                     onmouseout="this.className='bx-clock-label';"
                     onclick=""><? echo($arParams['initTime'] ? $initTime : 'Time'); ?></div><?
                break;
            case 'select':
                ?>
        ...
        if ($arParams['showIcon'] !== false) {
            ?><a href="javascript:void(0);" onclick="bxShowClock_<?= $jsInputId ?>()"
                 title="<?= GetMessage('BX_CLOCK_TITLE') ?>" onmouseover="this.className='bxc-icon-hover';"
                 onmouseout="this.className='';"><img id="<?= $inputId ?>_icon" src="/bitrix/images/1.gif"
                                                      class="bx-clock-icon bxc-iconkit-c"></a><?
        }
            ...
            function bxLoadClock_<?=$jsInputId?>(callback) {
                ...
                if (!window.JCClock && !window.jsUtils) {
                    return setTimeout(function () {
                        bxLoadClock_<?=$jsInputId?>(callback);
                    }, 50);
                }
                <?endif;?>
