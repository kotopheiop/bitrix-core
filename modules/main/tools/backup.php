...
{
    if ((!$backup_secret_key = CPasswordStorage::Get(
            'backup_secret_key'
        )) || $backup_secret_key != $_REQUEST['secret_key']) {
...
if ($public) {
    ...
    $arParams = array(
        ...
        'dump_site_id' => is_array(
            $ar = unserialize(COption::GetOptionString("main", "dump_site_id" . "_auto"), ['allowed_classes' => false])
        ) ? $ar : array(),
    );
    ...
    $arExpertBackupDefaultParams = array(
        ...
        'skip_mask_array' => is_array(
            $ar = unserialize(COption::GetOptionString("main", "skip_mask_array_auto"), ['allowed_classes' => false])
        ) ? $ar : array(),
...
if ($NS['step'] == 5) {
    ...
    if ($arParams['dump_integrity_check']) {
        ...
        if (!$tar->openRead($NS["arc_name"])) {
            RaiseErrorAndDie(
                GetMessage('DUMP_NO_PERMS_READ') . '<br>' . implode('<br>', $tar->err),
                510,
                $NS['arc_name']
            );
        } else {
            ...
            while ($obUpload->getPos() < $file_size && haveTime()) {
                ...
                while ($obUpload->hasRetries()) {
                    if ($res = $obUpload->Next($part, $obBucket)) {
                        ...
                        RaiseErrorAndDie(
                            'Internal Error: could not init upload for ' . $fails . ' times',
                            660,
                            $NS['arc_name']
                        );
                    }
    ...
    if (is_object($DB)) {
        ...
        foreach (GetModuleEvents("main", "OnAutoBackupError", true) as $arEvent) {
            ExecuteModuleEventEx(
                ...
                array(
                    array_merge(
                        $NS0,
                        array('ERROR' => $strError, 'ERROR_CODE' => $errCode, 'ITEM_ID' => $ITEM_ID)
                    )
                )
            );
        }
        ...
        foreach (array('ru', 'ua', 'en', 'de') as $lang) {
            \Bitrix\Main\Localization\Loc::loadLanguageFile(
                $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/main/admin/dump.php',
                $lang
            );
            $ar["LANG"][$lang] = \Bitrix\Main\Localization\Loc::getMessage(
                'DUMP_ERR_AUTO',
                array(
                    '#MESSAGE#' => $strError,
                    '#LINK#' => $link
