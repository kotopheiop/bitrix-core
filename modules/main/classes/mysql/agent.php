...
class CAgent extends CAllAgent
{
    public static function CheckAgents()
    {
        ...
        if ((defined("NO_AGENT_CHECK") && NO_AGENT_CHECK === true) || (defined(
                    "BX_CLUSTER_GROUP"
                ) && BX_CLUSTER_GROUP !== 1)) {
            return null;
        }
        ...
        if (CACHED_b_agent !== false && $CACHE_MANAGER->Read(
                CACHED_b_agent,
    ...
    public static function ExecuteAgents($str_crontab)
    {
        ...
        if ($db_result_agents->Fetch()) {
            ...
            if (CACHED_b_agent !== false) {
                $rs = $DB->Query(
                    "SELECT UNIX_TIMESTAMP(NEXT_EXEC)-UNIX_TIMESTAMP(NOW()) DATE_DIFF FROM b_agent WHERE ACTIVE = 'Y' " . $str_crontab . " ORDER BY NEXT_EXEC ASC LIMIT 1"
                );
                $ar = $rs->Fetch();
        ...
        $logFunction = (defined("BX_AGENTS_LOG_FUNCTION") && function_exists(
            BX_AGENTS_LOG_FUNCTION
        ) ? BX_AGENTS_LOG_FUNCTION : false);
