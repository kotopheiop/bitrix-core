        ...
        if (
            ...
            if (CACHED_b_lang !== false && CACHED_b_lang_domain !== false) {
                ...
                foreach ($arLang as $row) {
                    ...
                    if (array_key_exists($row["LID"], $arLangDomain)) {
                        foreach ($arLangDomain[$row["LID"]] as $dom) {
                            ...
                            if (strcasecmp(
                                    mb_substr("." . $CURR_DOMAIN, -mb_strlen("." . $dom["LD_DOMAIN"])),
                                    "." . $dom["LD_DOMAIN"]
                                ) == 0) {
                                $arJoin[] = $row + $dom;
                                $bLeft = false;
                ...
                foreach ($arJoin as $row) {
                    ...
                    if ($row["LD_LID"] != "" || strcasecmp(
                            mb_substr($cur_dir, 0, mb_strlen($row["DIR"])),
                            $row["DIR"]
                        ) == 0) {
                        $A[] = $row;
                    }
                ...
                $strSql =
                    ...
                    "	C.FORMAT_DATE, C.FORMAT_DATETIME, C.FORMAT_NAME, C.WEEK_START, C.CHARSET, C.DIRECTION " .
                    "FROM b_lang L  " .
                    "	LEFT JOIN b_lang_domain LD ON L.LID=LD.LID AND '" . $DB->ForSql(
                        "." . $CURR_DOMAIN,
                        255
                    ) . "' LIKE CONCAT('%.', LD.DOMAIN) " .
