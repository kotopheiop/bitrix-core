    ...
    <table cellspacing="0">
        <tr>
            ...
            if ($stableVersionsOnly == "N"):
                ...
                <td><?
                    echo GetMessage("SUP_BETA") ?></td>
            <? else:?>
                <td>
                    <div class="lamp-green"></div>
                </td>
                <td><?
                    echo GetMessage("SUP_RELEASE") ?></td>
            <?endif; ?>
        </tr>
        <tr>
            <?
            if (!CUpdateSystem::IsGzipInstalled()):
                ...
                <td><?
                    echo GetMessage("SUP_NO_ZLIB") ?></td>
            <? else:?>
                <td>
                    <div class="lamp-green"></div>
                </td>
                <td><?
                    echo GetMessage("SUP_ZLIB") ?></td>
            <?endif; ?>
        </tr>
        <?
        if ($DB->type == "MYSQL"):?>
            <tr>
                ...
                if ($arQueryRes = $dbQueryRes->Fetch()) {
                    ...
                    if (IntVal($arCurMySqlVer[0]) < 4
                        ...
                        <td><?
                            echo GetMessage("SUP_NO_MYSQL") ?></td>
                        <?
                    } else {
                        ?>
                        <td>
...
//*********************************
    ...
    <table cellspacing="0">
        ...
        if ($arTypesUpdates !== false
            ...
            <tr>
                <td><?
                    echo GetMessage("SUP_REGISTERED") ?></td>
                <td><?
                    echo $arTypesUpdates["CLIENT"]["@"]["NAME"] ?></td>
            </tr>
            <tr>
                <td><?= GetMessage("SUP_LICENSE_KEY") ?>:</td>
                <td><?
                    echo($MAIN_RIGHT == "W" ? CUpdateSystem::GetLicenseKey() : "XXX-XX-XXXXXXXXXXX") ?></td>
            </tr>
            <tr>
                <td><?
                    echo GetMessage("SUP_EDITION") ?></td>
                <td><?
                    echo $arTypesUpdates["CLIENT"]["@"]["LICENSE"] ?></td>
            </tr>
            <tr>
                <td><?
                    echo GetMessage("SUP_SITES") ?></td>
                <td><?
                    echo($arTypesUpdates["CLIENT"]["@"]["MAX_SITES"] > 0 ? $arTypesUpdates["CLIENT"]["@"]["MAX_SITES"] : GetMessage("SUP_CHECK_PROMT_2")) ?></td>
            </tr>
            <tr>
                <td><?
                    echo GetMessage("SUP_ACTIVE") ?></td>
                <td><?
                    echo GetMessage("SUP_ACTIVE_PERIOD", array("#DATE_TO#" => ((strlen($arTypesUpdates["CLIENT"]["@"]["DATE_TO"]) > 0) ? $arTypesUpdates["CLIENT"]["@"]["DATE_TO"] : "<i>N/A</i>"), "#DATE_FROM#" => ((strlen($arTypesUpdates["CLIENT"]["@"]["DATE_FROM"]) > 0) ? $arTypesUpdates["CLIENT"]["@"]["DATE_FROM"] : "<i>N/A</i>"))); ?></td>
            </tr>
            <?
            if (strlen($arTypesUpdates["CLIENT"]["@"]["HTTP_HOST"]) > 0):
                ...
                <tr>
                    <td><?
                        echo GetMessage("SUP_SERVER") ?></td>
                    <td><?
                        echo $arTypesUpdates["CLIENT"]["@"]["HTTP_HOST"] ?></td>
                </tr>
            <?endif ?>
            <?
        } else {
            ...
            <tr>
                <td><?
                    echo GetMessage("SUP_SERVER") ?></td>
                <td><?
                    echo(($s = COption::GetOptionString("main", "update_site")) == "" ? GetMessage("SUP_NO_SERVER") : $s) ?></td>
            </tr>
            <?
        }
        ?>
...
//*********************************
    if ($arTypesUpdates !== false
        ...
        <form method="POST" action="sysupdate.php" name="activate_key_form" id="activate_key_form"
            ...
            <tr class="heading">
                <td colspan="2"><?
                    echo GetMessage("SUP_REG_INFO") ?></td>
            </tr>
            ...
            <tr class="heading">
                <td colspan="2"><?
                    echo GetMessage("SUP_REG_INFO_USER") ?></td>
            </tr>
            <tr>
                <td colspan="2">
                    <?
                    ...
                    <input name="GENERATE_USER" id="GENERATE_USER" type="radio"
                           onclick="ActivateEnableDisableUser(true)" value="Y"<?
                    if ($GENERATE_USER != "N") echo " checked" ?>><label
                            for="GENERATE_USER"><?= GetMessage("SUP_SUBA_UI_CREATE") ?></label><br/>
                    <input name="GENERATE_USER" id="GENERATE_USER_NO" type="radio"
                           onclick="ActivateEnableDisableUser(false)" value="N"<?
                    if ($GENERATE_USER == "N") echo " checked" ?>><label for="GENERATE_USER_NO"><?
                        echo GetMessage("SUP_SUBA_UI_EXIST"); ?></label>
                    <?
                    echo EndNote() ?>
                </td>
            </tr>
            ...
            ?>
            <input <?
            if ($MAIN_RIGHT < "W") echo "disabled" ?> type="submit" value="<?= GetMessage("SUP_ACTIVATE_DO") ?>">
            <?
            $tabControl->End();
            ?>
        </form>
...
//*********************************
    ...
    if (!$bLockUpdateSystemKernel
        ...
        <form method="POST" action="sysupdate.php">
            ...
            <input <?
            if ($MAIN_RIGHT < "W") echo "disabled" ?> type="submit" value="<?= GetMessage("SUP_NO_KEY_ENTER_DO") ?>">
...
//*****************************
        ...
        <form method="POST" action="sysupdate.php" name="license_form">
            <input name="agree_license" type="checkbox" value="Y" id="agree_license_id"
                   onclick="document.license_form.SUP_LICENSE_ENTER_DON.disabled=<?
                   if ($MAIN_RIGHT < "W"):?>true<? else:?>!this.checked<?endif; ?>">
...
//*****************************
    if (!$bLockUpdateSystemKernel) {
        ...
        <form method="POST" action="sysupdate.php">
            ...
            if (isset($arTypesUpdates["CLIENT"]["@"]["SUBSCR_EMAIL"]) && $arTypesUpdates["CLIENT"]["@"]["SUBSCR_EMAIL"] <> ""):?>
                <input <?
                if ($MAIN_RIGHT < "W") echo "disabled" ?> type="submit"
                                                          value="<?= GetMessage("SUP_SUBSCR_ALREADY_CHANGE_U") ?>">
                <input <?
                if ($MAIN_RIGHT < "W") echo "disabled" ?> type="submit" name="SUBSCR_DELETE"
            ...
            <td style="padding:0px;">
                <form method="post" action="sysupdate.php" onsubmit="return ValidateForm(this);">
                    <input type="submit" name="load_button" id="load_button"
                           value="<?= GetMessage("SUP_STEP2_LOAD_BTN") ?>" <?
                    if (!isset($arModulesUpdates["MODULES"]) || !isset($arModulesUpdates["MODULES"]["#"]["MODULE"]) || !is_array($arModulesUpdates["MODULES"]["#"]["MODULE"]) || !count($arModulesUpdates["MODULES"]["#"]["MODULE"]) > 0) echo "disabled"; ?>>
...
if ($TYPE == "UPDATE" && ($STEP == 2 || $STEP == 3) && $MAIN_RIGHT == "W") {
    ...
    <iframe ID="updates_list" name="updates_list" src="sys_update_list.php?lang=<?
    echo LANG ?>" width="100%" height="400"></iframe>
    <!--</div>-->
    ...
    if ($arLangsUpdates
        ...
        <form method="post" action="sysupdate.php"
            ...
            if ($strLangsHTML2 <> "") {
                ?>
                <p<?
                if ($strLangsHTML1 <> "") echo ' class="update-dots"' ?>><?= GetMessage("SUP_LSTEP2_OTHER") ?>:</p>
                <?
    ...
    if ($arHelpUpdates
        ...
        <form method="post" action="sysupdate.php"
            ...
            if ($strHelpsHTML2 <> "") {
                ?>
                <p<?
                if ($strHelpsHTML1 <> "") echo ' class="update-dots"' ?>><?= GetMessage("SUP_HSTEP2_OTHER") ?>:</p>
                <?
