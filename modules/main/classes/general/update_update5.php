		...
		<td class="content">
			...
			<table cellspacing="0" width="100%">
				<tr>
					...
					<td align="right"><a class="button ' . ($aOpt[$id] == "off" ? "button-down" : "button-up") . '" title="' . ($aOpt[$id] == "off" ? GetMessage(
                "SUP_EXPAND"
            ) : GetMessage("SUP_COLLAPSE")) . '" href="javascript:void(0);" onclick="ToggleInfo(\'' . $id . '\', this)"></a></td>
				</tr>
			</table>
			</div>
            ...
            for ($i = 0; $i < count($arEmails); $i++) {
                ...
                if (strlen($arEmails[$i]) > 0) {
                    if (CUpdateSystem::CheckEMail($arEmails[$i])) {
                        ...
                        $strErrorMessage .= "[PSUUE02] " . GetMessage(
                                "SUP_ERROR_BAD_MAIL_U",
                                array("#EMAIL#" => $arEmails[$i])
                            ) . ".<br>";
                    }
                }
            }
...
if ($TYPE == "UPDATE" && $STEP == 3 && $MAIN_RIGHT == "W") {
    ...
    if (!isset($FAS) || $FAS != "Y") {
        ...
        if (!file_exists($archiveFileName) || !is_file($archiveFileName)) {
            $strErrorMessage .= "[PU3021] " . GetMessage(
                    "SUP_NO_TEMP_FILE",
                    array("#FILE#" => $archiveFileName)
                ) . ".<br>";
        }
    }
    ...
    <table cellspacing="0">
        ...
        <tr>
            ...
            if (!CUpdateSystem::IsGzipInstalled()):
                $sWarning .= GetMessage(
                        "SUP_ZLIB_RECOM",
                        array("#ZLIB_URL#" => "http://www.php.net/manual/en/ref.zlib.php")
                    ) . "<br>";
                ?>
                <td>
                    <div class="lamp-yellow"></div>
            ...
            <tr>
                ...
                if ($arQueryRes = $dbQueryRes->Fetch()) {
                    ...
                    if (IntVal($arCurMySqlVer[0]) < 4
                        ...
                        || IntVal($arCurMySqlVer[0]) == 4 && IntVal($arCurMySqlVer[1]) == 0 && IntVal(
                            $arCurMySqlVer[2]
                        ) < 18) {
    ...
    <table cellspacing="0">
        ...
        if ($arTypesUpdates !== false
            ...
            <tr>
                ...
                <td><? echo($arTypesUpdates["CLIENT"]["@"]["MAX_SITES"] > 0 ? $arTypesUpdates["CLIENT"]["@"]["MAX_SITES"] : GetMessage(
                        "SUP_CHECK_PROMT_2"
                    )) ?></td>
            </tr>
            <tr>
                ...
                <td><? echo GetMessage(
                        ...
                        array(
                            "#DATE_TO#" => ((strlen(
                                ...
                                ) > 0) ? $arTypesUpdates["CLIENT"]["@"]["DATE_TO"] : "<i>N/A</i>"),
                            "#DATE_FROM#" => ((strlen(
                                    $arTypesUpdates["CLIENT"]["@"]["DATE_FROM"]
                                ) > 0) ? $arTypesUpdates["CLIENT"]["@"]["DATE_FROM"] : "<i>N/A</i>")
                        )
                    ); ?></td>
            </tr>
            <?
            ...
            <tr>
                <td><? echo GetMessage("SUP_SERVER") ?></td>
                <td><? echo(($s = COption::GetOptionString("main", "update_site")) == "" ? GetMessage(
                        "SUP_NO_SERVER"
                    ) : $s) ?></td>
            </tr>
            <?
...
//*********************************
    ...
    if ($bLicenceNotActive) {
        CAdminMessage::ShowMessage(
            array(
                "MESSAGE" => GetMessage("SUP_UPDATES_TITLE"),
                "TYPE" => "OK",
                "DETAILS" => GetMessage("SUP_UPDATES"),
                "HTML" => true
            )
        );
    }
    ...
    if ($sWarning <> "") {
        CAdminMessage::ShowMessage(
            array("MESSAGE" => GetMessage("SUP_WARNING"), "TYPE" => "OK", "DETAILS" => $sWarning, "HTML" => true)
        );
    }
...
//*********************************
    if ($arTypesUpdates !== false
        ...
        <form method="POST" action="sysupdate.php" name="activate_key_form" id="activate_key_form"
            ...
            <tr>
                <td colspan="2">
                    <div id="errorDiv" style="width:30%;"></div>
                </td>
            </tr>
            <tr class="heading">
            ...
            <tr>
                ...
                <td width="50%" nowrap>
                    <div id="id_activate_name_error"></div>
                    <input type="text" id="id_activate_name" name="NAME"
                           value="<?= htmlspecialcharsEx($_POST["NAME"]) ?>" size="40"></td>
            </tr>
            <tr>
                ...
                <td width="50%">
                    ...
                    <input type="text" id="SITE_URL" name="SITE_URL"
                           value="<?= htmlspecialcharsEx($_POST["SITE_URL"]) ?>" size="40"></td>
            </tr>
            <tr>
                <td width="50%"><span class="required">*</span><?= GetMessage("SUP_SUBA_RI_PHONE") ?>:</td>
                <td width="50%">
                    <div id="PHONE_error"></div>
                    <input type="text" id="PHONE" name="PHONE" value="<?= htmlspecialcharsEx($_POST["PHONE"]) ?>"
                           size="40"></td>
            </tr>
            <tr>
                ...
                <td width="50%">
                    ...
                    <input type="text" id="EMAIL" name="EMAIL" value="<?= htmlspecialcharsEx($_POST["EMAIL"]) ?>"
                           size="40"></td>
            </tr>
            <tr>
                ...
                <td width="50%">
                    ...
                    <input type="text" id="CONTACT_PERSON" name="CONTACT_PERSON"
                           value="<?= htmlspecialcharsEx($_POST["CONTACT_PERSON"]) ?>" size="40"></td>
            </tr>
            <tr>
                ...
                <td width="50%">
                    ...
                    <input type="text" id="CONTACT_EMAIL" name="CONTACT_EMAIL"
                           value="<?= htmlspecialcharsEx($_POST["CONTACT_EMAIL"]) ?>" size="40"></td>
            </tr>
            <tr>
                <td width="50%"><span class="required">*</span><?= GetMessage("SUP_SUBA_RI_CONTACT_PHONE") ?>:</td>
                <td width="50%">
                    <div id="CONTACT_PHONE_error"></div>
                    <input type="text" id="CONTACT_PHONE" name="CONTACT_PHONE"
                           value="<?= htmlspecialcharsEx($_POST["CONTACT_PHONE"]) ?>" size="40"></td>
            </tr>
            <tr>
                <td width="50%"><?= GetMessage("SUP_SUBA_RI_CONTACT") ?>:</td>
                <td width="50%"><input type="text" name="CONTACT_INFO"
                                       value="<?= htmlspecialcharsEx($_POST["CONTACT_INFO"]) ?>" size="40"></td>
            </tr>
            ...
            <tr>
                <td colspan="2">
                    ...
                    <input name="GENERATE_USER" id="GENERATE_USER" type="radio"
                           onclick="ActivateEnableDisableUser(true)"
                           value="Y"<? if ($GENERATE_USER != "N") echo " checked" ?>><label
                            for="GENERATE_USER"><?= GetMessage("SUP_SUBA_UI_CREATE") ?></label><br/>
                    <input name="GENERATE_USER" id="GENERATE_USER_NO" type="radio"
                           onclick="ActivateEnableDisableUser(false)"
                           value="N"<? if ($GENERATE_USER == "N") echo " checked" ?>><label
                            for="GENERATE_USER_NO"><? echo GetMessage("SUP_SUBA_UI_EXIST"); ?></label>
                    <? echo EndNote() ?>
                </td>
            </tr>
                ...
                <td colspan="2">
                    <div id="new-user">
                        <table width="100%" border="0">
                            <tr id="tr_USER_NAME">
                                <td width="50%" class="field-name" style="padding: 3px;"><span
                                            class="required">*</span><?= GetMessage("SUP_SUBA__UI_NAME") ?>:
                                </td>
                                <td width="50%" style="padding: 3px;">
                                    <div id="USER_NAME_error"></div>
                                    <input type="text" id="USER_NAME" name="USER_NAME"
                                           value="<?= htmlspecialcharsEx($_POST["USER_NAME"]) ?>" size="40"></td>
                            </tr>
                            <tr id="tr_USER_LAST_NAME">
                                <td width="50%" class="field-name" style="padding: 3px;"><span
                                            class="required">*</span><?= GetMessage("SUP_SUBA_UI_LASTNAME") ?>:
                                </td>
                                <td width="50%" style="padding: 3px;">
                                    <div id="USER_LAST_NAME_error"></div>
                                    <input type="text" id="USER_LAST_NAME" name="USER_LAST_NAME"
                                           value="<?= htmlspecialcharsEx($_POST["USER_LAST_NAME"]) ?>" size="40"></td>
                            </tr>
                            <tr id="tr_USER_LOGIN">
                                <td width="50%" class="field-name" style="padding: 3px;"><span
                                            class="required">*</span><?= GetMessage("SUP_SUBA_UI_LOGIN") ?>:
                                </td>
                                <td width="50%" style="padding: 3px;">
                                    <div id="USER_LOGIN_error"></div>
                                    <input type="text" id="USER_LOGIN_activate" name="USER_LOGIN_A"
                                           value="<?= htmlspecialcharsEx($_POST["USER_LOGIN_A"]) ?>" size="40"></td>
                            </tr>
                            <tr id="tr_USER_PASSWORD">
                                <td width="50%" class="field-name" style="padding: 3px;"><span
                                            class="required">*</span><?= GetMessage("SUP_SUBA_UI_PASSWORD") ?>:
                                </td>
                                <td width="50%" style="padding: 3px;">
                                    <div id="USER_PASSWORD_error"></div>
                                    <input type="password" id="USER_PASSWORD" name="USER_PASSWORD" value="" size="40"
                                           autocomplete="off"></td>
                            </tr>
                            <tr id="tr_USER_PASSWORD_CONFIRM">
                                <td width="50%" class="field-name" style="padding: 3px;"><span
                                            class="required">*</span><?= GetMessage("SUP_SUBA_UI_PASSWORD_CONF") ?>:
                                </td>
                                <td width="50%" style="padding: 3px;">
                                    <div id="USER_PASSWORD_CONFIRM_error"></div>
                                    <input type="password" id="USER_PASSWORD_CONFIRM" name="USER_PASSWORD_CONFIRM"
                                           value="" size="40"></td>
                            </tr>
                            <tr id="tr_USER_EMAIL">
                                <td width="50%" class="field-name" style="padding: 3px;"><span class="required">*</span>E-mail:
                                </td>
                                <td width="50%" style="padding: 3px;">
                                    <div id="USER_EMAIL_error"></div>
                                    <input type="text" id="USER_EMAIL" name="USER_EMAIL"
                                           value="<?= htmlspecialcharsEx($_POST["USER_EMAIL"]) ?>" size="40"></td>
                            </tr>
                        </table>
                    </div>
                    <div id="exist-user" style="display:none;">
                        <table width="100%" border="0">
                            <tr>
                                ...
                                <td width="50%" style="padding: 3px;">
                                    <div id="USER_LOGIN_EXIST_error"></div>
                                    <input id="USER_LOGIN" name="USER_LOGIN" maxlength="50"
                                           value="<?= htmlspecialcharsEx($_POST["USER_LOGIN"]) ?>" size="40"
                                           type="text"></td>
                            </tr>
                        </table>
                    </div>
            ...
            $tabControl->Buttons();
            ?>
            <input <? if ($MAIN_RIGHT < "W") echo "disabled" ?> type="submit"
                                                                value="<?= GetMessage("SUP_ACTIVATE_DO") ?>">
            <?
            $tabControl->End();
            ?>
        </form>
    ...
    if (!$bLockUpdateSystemKernel
        ...
        if (defined("DEMO") && DEMO == "Y") {
            $sMess = GetMessage(
                "SUP_NO_KEY_PROMT",
                array(
                    "#URL#" => "http://" . (LANGUAGE_ID == "ru" ? "www.bitrixsoft.ru" : "www.bitrixsoft.com") . "/bsm_register.php?dt=" . UrlEncode(
                            $SiteExpireDate
                        )
                )
            );
            $sLink = '<a href="http://' . (LANGUAGE_ID == "ru" ? "www.bitrixsoft.ru" : "www.bitrixsoft.com") . '/bsm_register.php?dt=' . UrlEncode(
                    $SiteExpireDate
                ) . '" target="_blank">' . GetMessage("SUP_NO_KEY_ACT") . '</a>';
        ...
        if (defined("DEMO") && DEMO == "Y") {
            ...
            $sMess = GetMessage(
                "SUP_NO_KEY_PROMT_SRC",
                array(
                    "#URL_SET#" => "/bitrix/admin/settings.php",
                    "#URL#" => "http://www.bitrixsoft." . (LANGUAGE_ID == "ru" ? "ru" : "com") . "/support/"
                )
            );
        }
        ob_start();
        ...
        <form method="POST" action="sysupdate.php">
            <input type="text" name="NEW_LICENSE_KEY"
                   value="<?= (($MAIN_RIGHT < "W") ? "XXXXXXX" : htmlspecialchars($strLicenseKey_tmp)) ?>" size="30">
            <input <? if ($MAIN_RIGHT < "W") echo "disabled" ?> type="submit"
...
//*********************************
    if ($arTypesUpdates !== false && isset($arTypesUpdates["UPDATE_SYSTEM"])) {
        $sLink = '<a href="sysupdate.php?TYPE=UPDUPD&amp;STEP=1&amp;lang=' . LANGUAGE_ID . '">' . GetMessage(
                "SUP_UPD_UPD_ACT"
            ) . '</a>';
...
//*********************************
        if ($arTypesUpdates !== false
            ...
            $sLink = '<a href="sysupdate.php?TYPE=REGVER&amp;STEP=1&amp;lang=' . LANGUAGE_ID . '">' . GetMessage(
                    "SUP_REG_ACT"
                ) . '</a>';
...
//*********************************
        if ($arTypesUpdates !== false
            ...
            $sMess = GetMessage(
                "SUP_MUPD_PROMT",
                array(
                    "#NUM#" => $arTypesUpdates["MODULES"]["@"]["COUNT"],
                    "#END#" => CUpdateSystem::NumberEndings($arTypesUpdates["MODULES"]["@"]["COUNT"])
                )
            );
            $sLink = '<a href="sysupdate.php?STEP=2&amp;TYPE=UPDATE&amp;lang=' . LANGUAGE_ID . '">' . GetMessage(
                    "SUP_MUPD_ACT"
                ) . '</a>';
...
//*********************************
        if ($arTypesUpdates !== false
            ...
            if (!isset($arTypesUpdates["MODULES"]) || IntVal($arTypesUpdates["MODULES"]["@"]["COUNT"]) <= 0) {
                $sLink = '<a href="sysupdate.php?TYPE=LOADSRC&amp;STEP=1&amp;lang=' . LANGUAGE_ID . '">' . GetMessage(
                        "SUP_SRC_ACT"
                    ) . '</a>';
            }
        ...
        if ($arTypesUpdates !== false
            ...
            if (IntVal($arTypesUpdates["LANGS"]["@"]["COUNT"]) > 0) {
                $sMess .= GetMessage(
                    "SUP_LUPD_PROMT",
                    array(
                        "#END#" => CUpdateSystem::NumberEndings(
                            $arTypesUpdates["LANGS"]["@"]["COUNT"]
                        ),
                        "#NUM#" => $arTypesUpdates["LANGS"]["@"]["COUNT"]
                    )
                );
            }
            ...
            $sLink = '<a href="sysupdate.php?STEP=2&amp;TYPE=LANGS&amp;lang=' . LANGUAGE_ID . '">' . GetMessage(
                    "SUP_LUPD_ACT"
                ) . '</a>';
...
//*********************************
        if ($arTypesUpdates !== false
            ...
            if (IntVal($arTypesUpdates["HELPS"]["@"]["COUNT"]) > 0) {
                $sMess .= GetMessage(
                    ...
                    array(
                        ...
                        "#NUM#" => $arTypesUpdates["HELPS"]["@"]["COUNT"]
                    )
                );
            }
            ...
            $sLink = '<a href="sysupdate.php?STEP=2&amp;TYPE=HELP&amp;lang=' . LANGUAGE_ID . '">' . GetMessage(
                    "SUP_HUPD_ACT"
                ) . '</a>';
...
//*********************************
        if ($arTypesUpdates !== false
            ...
            if ($arTypesUpdates["CLIENT"]["@"]["MAX_SITES"] > 0) {
                $sMess .= GetMessage(
                    ...
                    array(
                        ...
                        "#NUM#" => $arTypesUpdates["CLIENT"]["@"]["MAX_SITES"]
                    )
                );
            } else {
            ...
            $sLink = '<a href="sysupdate.php?TYPE=ADD_SITE&amp;STEP=1&amp;lang=' . LANGUAGE_ID . '">' . GetMessage(
                    "SUP_CHECK_ACT"
                ) . '</a>';
            echo CUpdateOutput::ShowBlock($sMess, $sLink, "icon-licence");
        }
...
//*****************************
        ...
        $sMess = GetMessage(
                "SUP_LICENSE_PROMT"
            ) . '<br><br><iframe name="license_text" src="' . $license_text_file . '" style="width:500px; height:250px; display:block;"></iframe>';

        ob_start();
        ?>
        <form method="POST" action="sysupdate.php" name="license_form">
            <input name="agree_license" type="checkbox" value="Y" id="agree_license_id"
                   onclick="document.license_form.SUP_LICENSE_ENTER_DON.disabled=<? if ($MAIN_RIGHT < "W"):?>true<? else:?>!this.checked<?endif; ?>">
            <label for="agree_license_id"><?= GetMessage("LICENSE_AGREE_PROMT") ?></label>
            <br>
            <input disabled id="SUP_LICENSE_ENTER_DO" name="SUP_LICENSE_ENTER_DON" type="submit"
                   value="<?= GetMessage("SUP_LICENSE_ENTER_DO") ?>">
...
//*****************************
    if (!$bLockUpdateSystemKernel) {
        ...
        $sLink = '<a href="sysupdate.php?TYPE=TURN_STABILITY&amp;STEP=1&amp;lang=' . LANGUAGE_ID . '">' . ($stableVersionsOnly == "N" ? GetMessage(
                "SUP_STABLE_TURN_ON"
            ) : GetMessage("SUP_STABLE_TURN_OFF")) . '</a>';
...
//*****************************
    if (!$bLockUpdateSystemKernel) {
        ...
        <form method="POST" action="sysupdate.php">
            ...
            <? if (isset($arTypesUpdates["CLIENT"]["@"]["SUBSCR_EMAIL"]) && $arTypesUpdates["CLIENT"]["@"]["SUBSCR_EMAIL"] <> ""):?>
                <input <? if ($MAIN_RIGHT < "W") echo "disabled" ?> type="submit" value="<?= GetMessage(
                    "SUP_SUBSCR_ALREADY_CHANGE_U"
                ) ?>">
                <input <? if ($MAIN_RIGHT < "W") echo "disabled" ?> type="submit" name="SUBSCR_DELETE"
                                                                    value="<?= GetMessage(
...
if ($TYPE == "UPDATE" && ($STEP == 2 || $STEP == 3) && $MAIN_RIGHT == "W") {
    ...
    <iframe ID="updates_list" name="updates_list" src="sys_update_list.php?lang=<? echo LANG ?>" width="100%"
            height="400"></iframe>
    <!--</div>-->
    ...
    <script language="javascript">
        ...
        oTotalSizeInfo.innerHTML = "<?= GetMessage(
            "SUP_TOTAL_UPDS"
...
if ($TYPE == "LANGS" && $STEP == 2 && $MAIN_RIGHT == "W") {
    ...
    if ($arLangsUpdates
        ...
        ?>
        <form method="post" action="sysupdate.php"
              onsubmit="document.getElementById('load_button').disabled=true; return true;">
            <?
            ...
            ?>
            <input type="submit" name="load_button" id="load_button" value="<?= GetMessage("SUP_LSTEP2_LOAD_BTN") ?>"
                   disabled>
            <input type="hidden" name="lang" value="<?= LANGUAGE_ID ?>">
            <input type="hidden" name="STEP" value="3">
            <input type="hidden" name="UNIID" value="<? echo md5(time()); ?>">
...
if ($TYPE == "HELP" && $STEP == 2 && $MAIN_RIGHT == "W") {
    ...
    if ($arHelpUpdates
        ...
        <form method="post" action="sysupdate.php"
            ...
            ?>
            <input type="submit" name="load_button" id="load_button" value="<?= GetMessage("SUP_HSTEP2_LOAD_BTN") ?>"
                   disabled>
            <input type="hidden" name="lang" value="<?= LANGUAGE_ID ?>">
            <input type="hidden" name="STEP" value="3">
            <input type="hidden" name="UNIID" value="<? echo md5(time()); ?>">
            ...
            if (array_key_exists($arHelpUpdates["HELPS"]["#"]["HELP"][$i]["@"]["ID"], $arErrorHelp)) {
                ...
                $sMess .=
                    '<p>' . GetMessage(
                        "SUP_HSTEP3_UPD_ERR",
                        array(
                            "#DATE#" => Date($strPHPDateFormat, $dDate),
                            "#HELP#" => $arHelpUpdates["HELPS"]["#"]["HELP"][$i]["@"]["NAME"] . " (" . $arHelpUpdates["HELPS"]["#"]["HELP"][$i]["@"]["ID"] . ")"
                        )
                    ) . '<br>' .
                ...
                $sMess .= '<p>' . GetMessage(
                        "SUP_HSTEP3_UPD_NONE",
                        array("#HELP#" => $arHelpUpdates["HELPS"]["#"]["HELP"][$i]["@"]["NAME"] . " (" . $arHelpUpdates["HELPS"]["#"]["HELP"][$i]["@"]["ID"] . ")")
                    ) . '</p>';
            }
        }
