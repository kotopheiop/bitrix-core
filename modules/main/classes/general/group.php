...
class CAllGroup
{
    ...
    public static function GetList($by = 'c_sort', $order = 'asc', $arFilter = [], $SHOW_USERS_AMOUNT = "N")
    {
            ...
            foreach ($arFilter as $key => $val) {
                ...
                switch ($key) {
                    case "ID":
                        ...
                        break;
                    case "TIMESTAMP_1":
                        $arSqlSearch[] = "G.TIMESTAMP_X >= FROM_UNIXTIME('" . MkDateTime(
                                FmtDate($val, "D.M.Y"),
                                "d.m.Y"
                            ) . "')";
                        break;
        ...
        if ($SHOW_USERS_AMOUNT == "Y") {
            ...
            $str_TABLE = "LEFT JOIN b_user_group U ON (U.GROUP_ID=G.ID AND ((U.DATE_ACTIVE_FROM IS NULL) OR (U.DATE_ACTIVE_FROM <= " . $DB->CurrentTimeFunction(
                ) . ")) AND ((U.DATE_ACTIVE_TO IS NULL) OR (U.DATE_ACTIVE_TO >= " . $DB->CurrentTimeFunction() . ")))";
        }
    ...
    public static function GetListEx(
        ...
        $arFields = array(
            ...
            "USER_USER_ID" => array(
                "FIELD" => "UG.USER_ID",
                "TYPE" => "int",
                "FROM" => "INNER JOIN b_user_group UG ON (G.ID = UG.GROUP_ID)"
            ),
            "USER_GROUP_ID" => array(
                "FIELD" => "UG.GROUP_ID",
                "TYPE" => "string",
            ...
            "USER_DATE_ACTIVE_FROM" => array(
                "FIELD" => "UG.DATE_ACTIVE_FROM",
                "TYPE" => "datetime",
                "FROM" => "INNER JOIN b_user_group UG ON (G.ID = UG.GROUP_ID)"
            ),
            "USER_DATE_ACTIVE_TO" => array(
                "FIELD" => "UG.DATE_ACTIVE_TO",
                "TYPE" => "datetime",
                "FROM" => "INNER JOIN b_user_group UG ON (G.ID = UG.GROUP_ID)"
            )
        );
    ...
    public static function GetByID($ID, $SHOW_USERS_AMOUNT = "N")
    {
        ...
        $strSql = "SELECT G.ID, G.ACTIVE, G.C_SORT, G.ANONYMOUS, G.NAME, G.STRING_ID, G.DESCRIPTION, " . $DB->DateToCharFunction(
                "G.TIMESTAMP_X"
            ) . " as TIMESTAMP_X ";
        ...
        if ($SHOW_USERS_AMOUNT == "Y") {
            $strSql .= "LEFT JOIN b_user_group U ON (U.GROUP_ID=G.ID AND ((U.DATE_ACTIVE_FROM IS NULL) OR (U.DATE_ACTIVE_FROM <= " . $DB->CurrentTimeFunction(
                ) . ")) AND ((U.DATE_ACTIVE_TO IS NULL) OR (U.DATE_ACTIVE_TO >= " . $DB->CurrentTimeFunction() . "))) ";
        }
                ...
                foreach ($arFields["USER_ID"] as $arUser) {
                    if ($arUser["DATE_ACTIVE_FROM"] <> '' && !CheckDateTime($arUser["DATE_ACTIVE_FROM"])) {
                        $error = str_replace(
                            "#USER_ID#",
                            $arUser["USER_ID"],
                            GetMessage("WRONG_USER_DATE_ACTIVE_FROM")
                        );
    ...
    public function Update($ID, $arFields)
    {
            ...
            if (COption::GetOptionString("main", "event_log_group_policy", "N") === "Y") {
                ...
                if (!empty($aDiff)) {
                    CEventLog::Log(
                        ...
                        "main",
                        $ID,
                        print_r($aPrevPolicy, true) . " => " . print_r($aNewPolicy, true)
                    );
                }
            }
        ...
        if (is_set($arFields, "USER_ID") && is_array($arFields["USER_ID"])) {
            ...
            if ($log) {
                ...
                $aPrevUsers = array();
                $res = $DB->Query(
                    "SELECT USER_ID FROM b_user_group WHERE GROUP_ID=" . $ID . ($ID == "1" ? " AND USER_ID<>1" : "")
                );
    ...
    public static function GetSubordinateGroups($grId)
    {
        ...
        if (!empty($groupFilter)) {
            if (CACHED_b_group_subordinate === false) {
                $z = $DB->Query(
                    "SELECT AR_SUBGROUP_ID FROM b_group_subordinate WHERE ID in (" . implode(", ", $groupFilter) . ")"
                );
                while ($zr = $z->Fetch()) {
        ...
        if (COption::GetOptionString("main", "event_log_module_access", "N") === "Y") {
            ...
            if (!empty($aDiff)) {
                CEventLog::Log(
                    ...
                    "main",
                    $ID,
                    "(" . implode(", ", $arOldTasks) . ") => (" . implode(", ", $aNewTasks) . ")"
                );
            }
        }
    ...
    public static function SetTasksForModule($module_id, $arGroupTask)
    {
        ...
        if (COption::GetOptionString("main", "event_log_module_access", "N") === "Y") {
            ...
            if (!empty($arIds)) {
                $rsTask = $DB->Query(
                    "SELECT GROUP_ID, TASK_ID FROM b_group_task WHERE TASK_ID IN (" . implode(",", $arIds) . ")"
                );
                while ($arTask = $rsTask->Fetch()) {
            ...
            foreach ($arOldTasks as $gr_id => $task_id) {
                if ($task_id <> $arGroupTask[$gr_id]['ID']) {
                    CEventLog::Log(
                        ...
                        "main",
                        $gr_id,
                        $module_id . ": (" . $task_id . ") => (" . $arGroupTask[$gr_id]['ID'] . ")"
                    );
                }
            }
    ...
    public static function GetModulePermission($group_id, $module_id)
    {
        ...
        $strSql = "SELECT T.ID, GT.TASK_ID FROM b_task T LEFT JOIN b_group_task GT ON T.ID=GT.TASK_ID AND GT.GROUP_ID=" . intval(
                $group_id
            ) . " WHERE T.MODULE_ID='" . $DB->ForSql($module_id) . "'";
    ...
    public static function SetModulePermission($group_id, $module_id, $permission)
    {
        ...
        if (intval($permission) <= 0 && $permission != false) {
            $strSql = "SELECT T.ID FROM b_task T WHERE T.MODULE_ID='" . $DB->ForSql(
                    $module_id
                ) . "' AND NAME='" . $DB->ForSql($permission) . "'";
        ...
        if (intval($permission) > 0 || $permission === false) {
            ...
            if (!empty($arIds)) {
                $strSql = "DELETE FROM b_group_task WHERE GROUP_ID=" . intval(
                        $group_id
                    ) . " AND TASK_ID IN (" . implode(",", $arIds) . ")";
                $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
            }
