...
class CAllTask
{
    ...
    public static function Delete($ID, $protect = true)
    {
        ...
        if (!$protect) {
            ...
            $DB->Query(
                "DELETE FROM b_task_operation WHERE TASK_ID=" . $ID,
                false,
                "FILE: " . __FILE__ . "<br> LINE: " . __LINE__
            );
        }
    ...
    public static function GetList($arOrder = array('MODULE_ID' => 'asc', 'LETTER' => 'asc'), $arFilter = array())
    {
        ...
        foreach ($arOrder as $by => $order) {
            if (isset($arFields[strtoupper($by)])) {
                $strOrderBy .= $arFields[strtoupper($by)]["FIELD_NAME"] . ' ' . (strtolower(
                        $order
                    ) == 'desc' ? 'desc' : 'asc') . ',';
            }
        }
        ...
        if (!empty($aDiff)) {
            if (COption::GetOptionString("main", "event_log_task", "N") === "Y") {
                CEventLog::Log(
                    ...
                    "main",
                    $ID,
                    "(" . implode(", ", $aPrevOp) . ") => (" . implode(", ", $aNewOp) . ")"
                );
            }
    ...
    public static function GetTasksInModules($mode = false, $module_id = false, $binding = false)
    {
        ...
        if ($mode) {
            while ($r = $z->Fetch()) {
                ...
                $arr[$r['MODULE_ID']]['reference'][] = '[' . ($r['LETTER'] ? $r['LETTER'] : '..') . '] ' . static::GetLangTitle(
                        $r['NAME'],
                        $r['MODULE_ID']
                    );
            }
