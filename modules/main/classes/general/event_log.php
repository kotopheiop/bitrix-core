...
class CEventLog
{
    ...
    public static function Add($arFields)
    {
        ...
        $arFields = array(
            ...
            "GUEST_ID" => ($session->isStarted() && $session->has(
                "SESS_GUEST_ID"
            ) && $session["SESS_GUEST_ID"] > 0 ? $session["SESS_GUEST_ID"] : false),
    ...
    public static function CleanUpAgent()
    {
        ...
        if ($cleanup_days > 0) {
            ...
            $DB->Query(
                "DELETE FROM b_event_log WHERE TIMESTAMP_X <= " . $DB->CharToDateFunction(
    ...
    public static function GetList($arOrder = Array("ID" => "DESC"), $arFilter = array(), $arNavParams = false)
    {
        ...
        if (is_array($arNavParams)) {
            ...
            return $DB->Query(
                "SELECT L.*, " . $DB->DateToCharFunction("L.TIMESTAMP_X") . " as TIMESTAMP_X" . $strSql,
                false,
                $err_mess . __LINE__
            );
        }
...
class CEventMain
{
    ...
    {
        $arFilter = array();
        if (COption::GetOptionString("main", "event_log_register", "N") === "Y" || COption::GetOptionString(
                "main",
                ...
                "event_log_user_edit",
                "N"
            ) === "Y" || COption::GetOptionString("main", "event_log_user_groups", "N") === "Y") {
    ...
    public static function GetEventInfo($row, $arParams)
    {
        ...
        if ($arUser = $rsUser->GetNext()) {
            $userURL = SITE_DIR . CComponentEngine::MakePathFromTemplate(
                    $arParams['USER_PATH'],
                    array(
                        "user_id" => $row['ITEM_ID'],
                        "SITE_ID" => ""
