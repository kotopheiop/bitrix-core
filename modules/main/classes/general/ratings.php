...
class CAllRatings
{
    ...
    public static function Update($ID, $arFields)
    {
        ...
        if ($arFields['NEW_CALC'] === 'Y') {
            $DB->Query(
                "UPDATE b_rating_results SET PREVIOUS_VALUE = 0 WHERE RATING_ID=" . $ID . " and ENTITY_TYPE_ID='" . $DB->ForSql(
                    $arFields["ENTITY_ID"]
                ) . "'",
                false,
                $err_mess . __LINE__
    ...
    public static function GetRatingVoteResultCache($entityTypeId, $entityId, $user_id = 0)
    {
        ...
        if ($CACHE_MANAGER->Read(
            ...
            $cache_id = "b_rvg_" . $entityTypeId . $bucket,
            "b_rating_voting"
        )) {
    ...
    public static function GetRatingResult($ID, $entityId)
    {
        ...
        if (!empty($arToSelect)) {
            ...
            while ($arRes = $res->Fetch()) {
                ...
                $arRes['ROUND_CURRENT_VALUE'] = round($arRes['CURRENT_VALUE']) == 0 ? 0 : round(
                    $arRes['CURRENT_VALUE']
                );
                $arRes['ROUND_PREVIOUS_VALUE'] = round($arRes['PREVIOUS_VALUE']) == 0 ? 0 : round(
                    $arRes['CURRENT_VALUE']
                );
                $arRes['CURRENT_POSITION'] = $arRes['CURRENT_POSITION'] > 0 ? $arRes['CURRENT_POSITION'] : GetMessage(
                    'RATING_NO_POSITION'
                );
    ...
    public static function AddRatingVote($arParam)
    {
        ...
        if ($arParam['ENTITY_TYPE_ID'] === 'USER' && isset(
                \Bitrix\Main\Application::getInstance()->getSession()['RATING_VOTE_COUNT']
            )) {
            if (\Bitrix\Main\Application::getInstance()->getSession(
                )['RATING_VOTE_COUNT'] >= \Bitrix\Main\Application::getInstance()->getSession(
        ...
        $rowAffected = $DB->Update(
            "b_rating_voting",
            $arFields,
            "WHERE ENTITY_TYPE_ID='" . $DB->ForSql(
                $arParam['ENTITY_TYPE_ID']
            ) . "' AND ENTITY_ID='" . (int)$arParam['ENTITY_ID'] . "'",
            $err_mess . __LINE__
        );
        if ($rowAffected > 0) {
            $rsRV = $DB->Query(
                "SELECT ID, TOTAL_POSITIVE_VOTES FROM b_rating_voting WHERE ENTITY_TYPE_ID='" . $DB->ForSql(
                    $arParam['ENTITY_TYPE_ID']
                ) . "' AND ENTITY_ID='" . (int)$arParam['ENTITY_ID'] . "'",
                false,
                $err_mess . __LINE__
            ...
            if ($votePlus) {
                $rsRVR = $DB->Query(
                    "SELECT TOTAL_VOTES FROM b_rating_voting_reaction WHERE ENTITY_TYPE_ID='" . $DB->ForSql(
                        $arParam['ENTITY_TYPE_ID']
                    ) . "' AND ENTITY_ID='" . (int)$arParam['ENTITY_ID'] . "'",
                    false,
                    $err_mess . __LINE__
                ...
                $DB->Query(
                    "INSERT INTO b_rating_voting_reaction (ENTITY_TYPE_ID, ENTITY_ID, REACTION, TOTAL_VOTES) " .
                    "VALUES ('" . $DB->ForSql(
                        $arParam['ENTITY_TYPE_ID']
                    ) . "', " . (int)$arParam['ENTITY_ID'] . ", '" . $DB->ForSql($arParam['REACTION']) . "', 1)" .
        ...
        $rsRVR = $DB->Query(
            "SELECT REACTION, TOTAL_VOTES FROM b_rating_voting_reaction WHERE ENTITY_TYPE_ID='" . $DB->ForSql(
                $arParam['ENTITY_TYPE_ID']
            ) . "' AND ENTITY_ID='" . (int)$arParam['ENTITY_ID'] . "'",
            false,
            $err_mess . __LINE__
    ...
    public static function ChangeRatingVote($arParam)
    {
        ...
        if ($arVote = $res->Fetch()) {
            ...
            if (!empty($arVote['REACTION_OLD'])) {
                $DB->Query(
                    "UPDATE b_rating_voting_reaction SET TOTAL_VOTES = TOTAL_VOTES - 1 WHERE ENTITY_TYPE_ID = '" . $DB->ForSql(
                        $arParam['ENTITY_TYPE_ID']
                    ) . "' AND ENTITY_ID = '" . (int)$arParam['ENTITY_ID'] . "' AND REACTION = '" . $DB->ForSql(
                        $arVote['REACTION_OLD']
                    ) . "'",
                    false,
                    $err_mess . __LINE__
            ...
            $ID = $DB->Update(
                "b_rating_vote",
                $arFields,
                "WHERE RATING_VOTING_ID=" . (int)$arParam['RATING_VOTING_ID'] . " AND USER_ID=" . (int)$arParam['USER_ID'],
                $err_mess . __LINE__
            );
            if (!$ID) {
                return false;
        ...
        if ($arVote = $res->Fetch()) {
            ...
            if ($votePlus) {
                $DB->Query(
                    ...
                    "VALUES ('" . $DB->ForSql(
                        $arParam['ENTITY_TYPE_ID']
                    ) . "', " . (int)$arParam['ENTITY_ID'] . ", '" . $DB->ForSql($arVote['REACTION']) . "', 0)" .
                    "ON DUPLICATE KEY UPDATE TOTAL_VOTES = TOTAL_VOTES - 1",
                    false,
                    $err_mess . __LINE__
                );
    ...
    public static function UpdateRatingUserBonus($arParam)
    {
        ...
        $rows = $DB->Update(
            "b_rating_user",
            $arFields,
            "WHERE RATING_ID = " . $arParam['RATING_ID'] . " AND ENTITY_ID = " . $arParam['ENTITY_ID']
        );
        if ($rows == 0) {
            $rsRB = $DB->Query(
                "SELECT * FROM b_rating_user WHERE RATING_ID = " . $arParam['RATING_ID'] . " AND ENTITY_ID = " . $arParam['ENTITY_ID'],
                false,
                $err_mess . __LINE__
            );
        ...
        if (CACHED_b_rating_vote !== false) {
            ...
            $CACHE_MANAGER->Clean(
                "b_rvu_" . $arParam['RATING_ID'] . (int)($arParam['ENTITY_ID'] / $bucket_size),
                "b_rating_user"
            );
        }
        return true;
...
//				($bplus ? " AND RV.VALUE > 0 ": " and RV.VALUE < 0 ")
    ...
    public static function getRatingVoteList($arParam)
    {
        ...
        if ($arParam['LIST_LIMIT'] != 0 && ceil($cnt / (int)$arParam['LIST_LIMIT']) >= (int)$arParam['LIST_PAGE']) {
            ...
            $res->NavQuery(
                $sqlStr,
                $cnt,
                Array('iNumPage' => (int)$arParam['LIST_PAGE'], 'nPageSize' => (int)$arParam['LIST_LIMIT'])
            );
    ...
    public static function getUserWeight($userId = 0)
    {
        ...
        $res = \Bitrix\Main\Application::getConnection()->query(
            'SELECT MAX(VOTES) AS VOTES FROM b_rating_subordinate WHERE RATING_ID = ' . $ratingId . ' AND ENTITY_ID = ' . $userId
        );
        if ($record = $res->fetch()) {
    ...
    public static function deleteRatingVoting(array $params = [])
    {
        ...
        $DB->query(
            "DELETE FROM b_rating_vote WHERE ENTITY_TYPE_ID='" . $DB->forSql(
                $entityTypeId
            ) . "' AND ENTITY_ID=" . $entityId,
            true
        );
        $DB->query(
            "DELETE FROM b_rating_voting WHERE ENTITY_TYPE_ID='" . $DB->forSql(
                $entityTypeId
            ) . "' AND ENTITY_ID=" . $entityId,
            true
        );
        $DB->query(
            "DELETE FROM b_rating_voting_reaction WHERE ENTITY_TYPE_ID='" . $DB->forSql(
                $entityTypeId
            ) . "' AND ENTITY_ID=" . $entityId,
            true
        );
