...
class CAllFavorites extends CDBResult
{
    ...
    public static function Delete($ID)
    {
        ...
        return ($DB->Query(
            "DELETE FROM b_favorite WHERE ID='" . intval($ID) . "'",
            false,
            "File: " . __FILE__ . "<br>Line: " . __LINE__
        ));
    }
    ...
    public static function OnLanguageDelete($language_id)
    {
        global $DB;
        return ($DB->Query(
            "DELETE FROM b_favorite WHERE LANGUAGE_ID='" . $DB->ForSQL($language_id, 2) . "'",
            false,
            "File: " . __FILE__ . "<br>Line: " . __LINE__
        ));
    }
...
class CBXFavAdmMenu
{
    ...
    private function Init()
    {
        ...
        while ($arFav = $dbFav->GetNext()) {
            if ($arFav["COMMON"] == "Y" && $arFav["MODULE_ID"] <> "" && $APPLICATION->GetGroupRight(
                    $arFav["MODULE_ID"]
                ) < "R") {
                continue;
            }
    ...
    private function CheckFilterActivity($currentUrl, $menuUrl, $activeSectUrl)
    {
        ...
        if (CBXFavUrls::GetPresetId($currentUrl) && CBXFavUrls::GetFilterId($menuUrl) == CBXFavUrls::GetPresetId(
                $currentUrl
            )) {
            return true;
        }
    ...
    public function GenerateMenuHTML($id = 0)
    {
        ...
        if (empty($menuItems)) {
            ...
            echo '<script type="text/javascript" bxrunfirst="true">BX.adminFav.setLastId(' . intval(
                    $id
                ) . ');</script>';
...
<div id="adm-favorites-cap-hint-block" class="adm-favorites-cap-hint-block">
	...
	<div class="adm-favorites-cap-hint-text">
        ...
        if (!$IsMenuEmpty) {
            $retHtml .= '
	<a class="adm-favorites-cap-remove" href="javascript:void(0);" onclick="BX.adminFav.closeHint(this);">' . GetMessage(
