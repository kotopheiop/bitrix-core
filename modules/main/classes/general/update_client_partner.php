...
class CUpdateClientPartner
{
    public static function RegisterModules(&$strError, $lang = false, $stableVersionsOnly = false)
    {
        ...
        CUpdateClientPartner::AddMessage2Log(
            "TIME RegisterModules(request) " . Round(CUpdateClientPartner::__GetMicroTime() - $stime, 3) . " sec"
        );
        ...
        if ($strError_tmp == '') {
            ...
            if (!file_exists($updatesDirFull . "/update_info.xml") || !is_file($updatesDirFull . "/update_info.xml")) {
                $strError_tmp .= "[URV05] " . str_replace(
                        "#FILE#",
                        $updatesDirFull . "/update_info.xml",
        ...
        if ($strError_tmp == '') {
            if (isset($arRes["DATA"]["#"]["ERROR"]) && is_array($arRes["DATA"]["#"]["ERROR"]) && count(
                    $arRes["DATA"]["#"]["ERROR"]
                ) > 0) {
            ...
            if ($handle) {
                while (false !== ($dir = readdir($handle))) {
                    ...
                    if (file_exists($_SERVER["DOCUMENT_ROOT"] . US_SHARED_KERNEL_PATH . "/modules/" . $dir)) {
                        $strError_tmp1 = "";
                        CUpdateClientPartner::__CopyDirFiles(
                            $updatesDirFull . "/" . $dir,
                            $_SERVER["DOCUMENT_ROOT"] . US_SHARED_KERNEL_PATH . "/modules/" . $dir,
                            $strError_tmp1,
                            false
                        );
                        if ($strError_tmp1 <> '') {
    ...
    public static function loadModule4Wizard($moduleId, $lang = false)
    {
        ...
        if (isset($arStepUpdateInfo["DATA"]["#"]["NOUPDATES"])) {
            CUpdateClientPartner::ClearUpdateFolder(
                $_SERVER["DOCUMENT_ROOT"] . "/bitrix/updates/" . $temporaryUpdatesDir
            );
    ...
    public static function LoadModuleNoDemand($moduleId, &$strError, $stableVersionsOnly = "Y", $lang = false)
    {
        ...
        if ($strError_tmp == '') {
            ...
            CUpdateClientPartner::AddMessage2Log(
                "TIME LoadModuleNoDemand(request) " . Round(CUpdateClientPartner::__GetMicroTime() - $stime, 3) . " sec"
            );
        }

        if ($strError_tmp == '') {
            if (!($fp1 = fopen($_SERVER["DOCUMENT_ROOT"] . "/bitrix/updates/update_archive.gz", "wb"))) {
                $strError_tmp = "[GNSU03] " . str_replace(
                        "#FILE#",
                        $_SERVER["DOCUMENT_ROOT"] . "/bitrix/updates",
                        GetMessage("SUPP_RV_ER_TEMP_FILE")
                    ) . ". ";
    ...
    public static function SearchModulesEx($arOrder, $arFilter, $searchPage, $lang, &$strError)
    {
        ...
        $strQuery = CUpdateClientPartner::__CollectRequestData(
            ...
            array(
                ...
                "search_category" => (is_array($arFilter["CATEGORY"]) ? implode(
                    ",",
                    $arFilter["CATEGORY"]
                ) : $arFilter["CATEGORY"]),
        ...
        CUpdateClientPartner::AddMessage2Log(
            "TIME SearchModulesEx(request) " . Round(CUpdateClientPartner::__GetMicroTime() - $stime, 3) . " sec"
        );
    ...
    public static function SearchModules($searchModule, $lang)
    {
        ...
        CUpdateClientPartner::AddMessage2Log(
            "TIME SearchModules(request) " . Round(CUpdateClientPartner::__GetMicroTime() - $stime, 3) . " sec"
        );
    ...
    public static function GetNextStepUpdates(
        ...
        if ($strError_tmp == '') {
            ...
            CUpdateClientPartner::AddMessage2Log(
                "TIME GetNextStepUpdates(request) " . Round(CUpdateClientPartner::__GetMicroTime() - $stime, 3) . " sec"
            );
        }
    ...
    public static function UnGzipArchive(&$updatesDir, &$strError, $bDelArch = true)
    {
        ...
        CUpdateClientPartner::AddMessage2Log(
            "TIME UnGzipArchive " . Round(CUpdateClientPartner::__GetMicroTime() - $stime, 3) . " sec"
        );
        ...
        if ($handle = @opendir($updatesDirFull)) {
            while (($file = readdir($handle)) !== false) {
                ...
                if (is_dir($updatesDirFull . "/" . $file)) {
                    ...
                    $strRealPath = $_SERVER["DOCUMENT_ROOT"] . US_SHARED_KERNEL_PATH . "/modules/" . mb_substr(
                            $updatesDir . "/" . $file,
                            mb_strpos($updatesDir . "/" . $file, "/")
                        );
    ...
    public static function __CollectRequestData(
        &$strError,
        $lang = false,
        $stableVersionsOnly = "Y",
        $arRequestedModules = array(),
...
//				$strResult .= "&m_".$key."=".urlencode($value);
            ...
            if (method_exists("CHTMLPagesCache", "IsOn") && method_exists(
                    "CHTMLPagesCache",
                    "IsCompositeEnabled"
                ) && CHTMLPagesCache::IsOn() && CHTMLPagesCache::IsCompositeEnabled()) {
    ...
    public static function GetCurrentModules(&$strError)
    {
        ...
        if (file_exists($_SERVER["DOCUMENT_ROOT"] . US_SHARED_KERNEL_PATH . "/modules/main/classes/general/version.php")
            && is_file(
                $_SERVER["DOCUMENT_ROOT"] . US_SHARED_KERNEL_PATH . "/modules/main/classes/general/version.php"
            )) {
            $p = file_get_contents(
                $_SERVER["DOCUMENT_ROOT"] . US_SHARED_KERNEL_PATH . "/modules/main/classes/general/version.php"
            );
            ...
            $arClientModules["main"] = array(
                "VERSION" => $arVers[1],
                "IS_DEMO" => ((defined("DEMO") && DEMO == "Y") ? "Y" : "N")
            );
        }
                ...
                if (is_dir($_SERVER["DOCUMENT_ROOT"] . US_SHARED_KERNEL_PATH . "/modules/" . $dir)
                    ...
                    if (file_exists($module_dir . "/install/index.php")) {
                        ...
                        if (!isset($arInfo["VERSION"]) || $arInfo["VERSION"] == '') {
                            ...
                            if ($arInfo["ACTIVE"] == "Y") {
                                $arClientModules[$dir] = array(
                                    "VERSION" => $arInfo["VERSION"],
                                    "IS_DEMO" => $arInfo["IS_DEMO"]
                                );
                            }
                        }
                    } else {
    ...
    public static function GetUpdatesList(
        ...
        CUpdateClientPartner::AddMessage2Log(
            "TIME GetUpdatesList(request) " . Round(CUpdateClientPartner::__GetMicroTime() - $stime, 3) . " sec"
        );
    ...
    public static function LoadModulesUpdates(
        ...
        if (count($arUpdateDescription) <= 0 || $errorMessage <> '') {
            $arUpdateDescription = array();
    ...
    private static function getSocketError($errstr, $errno, $addrParams)
    {
        ...
        CUpdateClientPartner::AddMessage2Log(
            "Error connecting 2 " . $addrParams["SOCKET_IP"] . ": [" . $errno . "] " . $errstr . "",
            "ERRCONN1"
        );
    ...
    private static function loadFileBx(
        $requestedUrl,
        $realSize,
        $outputFilename,
        $timeout,
        ...
        for ($i = 0, $cnt = count($replyHeaderArray); $i < $cnt; $i++) {
            if (mb_strpos($replyHeaderArray[$i], "Content-Length") !== false) {
                $pos = mb_strpos($replyHeaderArray[$i], ":");
                $contentLength = intval(
                    trim(mb_substr($replyHeaderArray[$i], $pos + 1, mb_strlen($replyHeaderArray[$i]) - $pos + 1))
                );
            }
        }
        ...
        CUpdateClientPartner::AddMessage2Log(
            "Time - " . (CUpdateClientPartner::__GetMicroTime() - $startTime) . " sec",
            "DOWNLOAD"
        );
    ...
    private static function loadFile($requestedUrl, $realSize, $outputFilename, $timeout, &$errorMessage)
    {
        ...
        if (preg_match("#([A-Z]{4})/([0-9.]{3}) ([0-9]{3})#", $replyHeaderArray[0], $regs)) {
            ...
            $replymsg = mb_substr(
                $replyHeaderArray[0],
                mb_strpos($replyHeaderArray[0], $replycode) + mb_strlen($replycode) + 1,
                mb_strlen($replyHeaderArray[0]) - mb_strpos($replyHeaderArray[0], $replycode) + 1
            );
        }
        ...
        for ($i = 1; $i < count($replyHeaderArray); $i++) {
            if (mb_strpos($replyHeaderArray[$i], "Content-Range") !== false) {
                $replyContentRange = trim(
                    mb_substr(
                        $replyHeaderArray[$i],
                        mb_strpos($replyHeaderArray[$i], ":") + 1,
                        mb_strlen($replyHeaderArray[$i]) - mb_strpos($replyHeaderArray[$i], ":") + 1
                    )
                );
            } elseif (mb_strpos($replyHeaderArray[$i], "Content-Length") !== false) {
                $replyContentLength = doubleval(
                    Trim(
                        mb_substr(
                            $replyHeaderArray[$i],
                            mb_strpos($replyHeaderArray[$i], ":") + 1,
    ...
    public static function UpdateStepModules($updatesDir, &$strError, $bSaveUpdaters = false)
    {
                        ...
                        while (false !== ($dir = readdir($handle))) {
                            if (mb_substr($dir, 0, 7) == "updater") {
                                ...
                                if ($bSaveUpdaters) {
                                    CUpdateClientPartner::__CopyDirFiles(
                                        $updateDirFrom . "/" . $dir,
                                        $_SERVER["DOCUMENT_ROOT"] . US_SAVE_UPDATERS_DIR . "/" . $arModules[$i] . "/" . $dir,
                                        $strError_tmp1,
                                        false
                                    );
                                }
                    ...
                    for ($i1 = 0, $n = count($arUpdaters); $i1 < $n; $i1++) {
                        if ($arUpdaters[$i1][2] == "N") {
                            ...
                            CUpdateClientPartner::__RunUpdaterScript(
                                $updateDirFrom . $arUpdaters[$i1][0],
                                $strError_tmp2,
                                "/bitrix/updates/" . $updatesDir . "/" . $arModules[$i],
                                $arModules[$i]
                            );
                            if ($strError_tmp2 <> '') {
                                $strError_tmp1 .=
        ...
        CUpdateClientPartner::AddMessage2Log(
            "TIME UpdateStepModules " . Round(CUpdateClientPartner::__GetMicroTime() - $stime, 3) . " sec"
        );
    ...
    public static function ActivateCoupon($coupon, &$strError, $lang = false, $stableVersionsOnly = "Y")
    {
        ...
        if ($strError_tmp == '') {
			...
			*/
            ...
            CUpdateClientPartner::AddMessage2Log(
                "TIME ActivateCoupon(request) " . Round(CUpdateClientPartner::__GetMicroTime() - $stime, 3) . " sec"
            );
        }
    ...
    public static function __RunUpdaterScript($path, &$strError, $updateDirFrom, $moduleID)
    {
        ...
        if (!isset($GLOBALS["UPDATE_STRONG_UPDATE_CHECK"])
            ...
            $GLOBALS["UPDATE_STRONG_UPDATE_CHECK"] = ((US_CALL_TYPE != "DB") ? COption::GetOptionString(
                "main",
                "strong_update_check",
                "Y"
            ) : "Y");
...
//CUpdateClientPartner::AddMessage2Log($strRequest, "!!!!!");
            ...
            if ($bChunked) {
                ...
                $strChunkSize = "";
                $i = 0;
                while ($i < mb_strlen($line) && in_array(
                        $line[$i],
                        array(
                            "0",
                            "1",
    ...
    public static function __ParseServerData(&$strServerOutput, &$arRes, &$strError)
    {
        ...
        if ($strError_tmp == '') {
            if (mb_substr(
                    ...
                    0,
                    mb_strlen("<DATA>")
                ) != "<DATA>" && CUpdateClientPartner::__IsGzipInstalled()) {
...
//			$arRes = CUpdateClientPartner::xmlize($strServerOutput);
    ...
    public static function __CheckDirPath($path, $bPermission = true)
    {
        ...
        while (mb_strlen($path) > 1 && $path[mb_strlen($path) - 1] == "/") //������� / � �����, ���� ����
        {
...
//			return $arModuleVersion;
        ...
        if ($result["IS_DEMO"] == "Y" && IsModuleInstalled($module_code) && CModule::IncludeModuleEx(
                $module_code
            ) == MODULE_DEMO_EXPIRED) {
            $result["ACTIVE"] = "N";
        }
