    ...
    public function RunSQLBatch($filepath)
    {
        ...
        foreach ($connection->parseSqlBatch($contents) as $strSql) {
            if (!$this->Query($strSql, true)) {
                $arErr[] = "<hr><pre>Query:\n" . $strSql . "\n\nError:\n<font color=red>" . $this->GetErrorMessage(
                    ) . "</font></pre>";
            }
        }
...
abstract class CAllDBResult
{
    ...
    public static function GetNavParams($nPageSize = 0, $bShowAll = true, $iNumPage = false)
    {
        ...
        if (intval($PAGEN) <= 0) {
            if (CPageOption::GetOptionString("main", "nav_page_in_session", "Y") == "Y" && intval(
                    \Bitrix\Main\Application::getInstance()->getSession()[$SESS_PAGEN]
                ) > 0) {
        ...
        //Show all records
        $SHOW_ALL = ($bShowAll ? (isset($SHOWALL) ? ($SHOWALL == 1) : (CPageOption::GetOptionString(
                "main",
                "nav_page_in_session",
                "Y"
            ) == "Y" && \Bitrix\Main\Application::getInstance()->getSession()[$SESS_ALL] == 1)) : false);
