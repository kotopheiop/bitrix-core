...
class CAllEventMessage
{
    ...
    public function CheckFields($arFields, $ID = false)
    {
        ...
        if (is_set($arFields, "EVENT_NAME")) {
            $r = CEventType::GetListEx(
                array(),
                array("EVENT_NAME" => $arFields["EVENT_NAME"]),
                array("type" => "none")
            );
            if (!$r->Fetch()) {
            ...
            foreach ($arFilter as $key => $val) {
                ...
                switch ($key) {
                    ...
                    case "TYPE_ID":
                        ...
                        break;
                    case "TIMESTAMP_1":
                        $arSqlSearch[] = "M.TIMESTAMP_X>=TO_DATE('" . FmtDate(
                                $val,
                                "D.M.Y"
        ...
        if ($action == "ADD") {
            ...
            if (empty($aMsg)) {
                $db_res = CEventType::GetList(
                    array("LID" => $arFields["LID"], "EVENT_NAME" => $arFields["EVENT_NAME"])
                );
