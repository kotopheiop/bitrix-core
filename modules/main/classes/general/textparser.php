...
class CTextParser
{
    ...
    public function getAnchorSchemes()
    {
        if ($this->anchorSchemes === null) {
            static $schemes = null;
            if ($schemes === null) {
                $schemes = \Bitrix\Main\Config\Option::get(
                    "main",
        ...
        foreach ($this->smiles as $row) {
            if ($row["TYPING"] <> '' && $row["IMAGE"] <> '') {
                ...
                $description = preg_quote(
                    htmlspecialcharsbx(str_replace(array("\x5C"), array("&#092;"), $row["DESCRIPTION"]), ENT_QUOTES),
                    "/"
                );
    ...
    public function convertText($text)
    {
        ...
        $text = preg_replace(
            array("#([?&;])PHPSESSID=([0-9a-zA-Z]{32})#is", "/\\x{00A0}/" . BX_UTF_PCRE_MODIFIER),
            array("\\1PHPSESSID1=", " "),
            $text
        );
        ...
        if (($this->allow["ANCHOR"] ?? '') == "Y") {
            ...
            if (mb_strpos($text, "&lt;") !== false) {
                $patt[] = "/(?<=\\&lt\\;)((((" . $this->getAnchorSchemes(
                    ) . "):\\/\\/)|www\\.)[._:a-z0-9@-].*?)(?=\\&gt\\;)/is" . BX_UTF_PCRE_MODIFIER;
            }

            $word_separator = str_replace("?", "", $this->wordSeparator);
            if (self::strpos($text, "(") !== false) {
                $patt[] = "/(?<=\\()(?<!\\[nomodify\\]|<nomodify>)((((" . $this->getAnchorSchemes(
                    ) . "):\\/\\/)|www\\.)[._:a-z0-9@-].*?)(?=\\))/is" . BX_UTF_PCRE_MODIFIER;
            }
            if (self::strpos($text, "�") !== false) {
                $patt[] = "/(?<=[" . self::chr("�") . "])(?<!\\[nomodify\\]|<nomodify>)((((" . $this->getAnchorSchemes(
                    ) . "):\\/\\/)|www\\.)[._:a-z0-9@-].*?)(?=[" . self::chr("�") . "])/is" . BX_UTF_PCRE_MODIFIER;
            }
            if (self::strpos($text, "�") !== false) {
                $patt[] = "/(?<=[" . self::chr("�") . "])(?<!\\[nomodify\\]|<nomodify>)((((" . $this->getAnchorSchemes(
                    ) . "):\\/\\/)|www\\.)[._:a-z0-9@-].*?)(?=[" . self::chr("�") . "])/is" . BX_UTF_PCRE_MODIFIER;
            ...
            $patt[] = "/(?<=^|[" . $word_separator . "]|\\s)(?<!\\[nomodify\\]|<nomodify>)((((" . $this->getAnchorSchemes(
                ) . "):\\/\\/)|www\\.)[._:a-z0-9@-].*?)(?=[\\s'\"{}\\[\\]]|&quot;|\$)/is" . BX_UTF_PCRE_MODIFIER;
        ...
        foreach ($res as $tag => $val) {
            ...
            switch ($tag) {
                ...
                case "LIST":
                    while (preg_match(
                        "/\\[list\\s*=\\s*(1|a)\\s*\\](.+?)\\[\\/list\\]/is" . BX_UTF_PCRE_MODIFIER,
                        $text
                    )) {
                        $text = preg_replace(
                            array(
        ...
        if (isset($this->allow["USERFIELDS"]) && is_array($this->allow["USERFIELDS"])) {
            foreach ($this->allow["USERFIELDS"] as $userField) {
                ...
                if (!empty($userField["TAG"]) && array_key_exists("VALUE", $userField) && !empty($userField["VALUE"]) &&
                    ...
                    $userField["VALUE"] = (is_array(
                        $userField["VALUE"]
                    ) ? $userField["VALUE"] : array($userField["VALUE"]));
                    $this->userField = $userField;
                    $text = preg_replace_callback(
                        "/\\[(" . (is_array($userField["TAG"]) ? implode(
                            "|",
                            $userField["TAG"]
                        ) : $userField["TAG"]) . ")\\s*=\\s*([a-z0-9]+)([^\\]]*)\\]/is" . BX_UTF_PCRE_MODIFIER,
    ...
    public function convertVideo($matches)
    {
        ...
        preg_match(
            "/type\\=(YOUTUBE|RUTUBE|VIMEO|VK|FACEBOOK|INSTAGRAM)/is" . BX_UTF_PCRE_MODIFIER,
            $params,
            $provider
        );
    ...
    protected function convert_video($arParams)
    {
        ...
        if ($this->type == "mail") {
            ...
            if (\Bitrix\Main\UrlPreview\UrlPreview::isHostTrusted($uri) || $uri->getHost(
                ...
                if ($this->bMobile) {
                    ?>
                    <iframe class="bx-mobile-video-frame" src="<?= $pathEncoded ?>" allowfullscreen="" frameborder="0"
                            height="100%" width="100%" style="max-width: 600px; min-height: 300px;"></iframe><?
                } else {
                    ?>
                    <iframe src="<?= $pathEncoded ?>" allowfullscreen="" frameborder="0"
                            height="<?= intval($arParams["HEIGHT"]) ?>" width="<?= intval($arParams["WIDTH"]) ?>"
    ...
    public function convert_emoticon(
        $code = "",
        $image = "",
        $description = "",
        $width = "",
        ...
        $html = '<img src="' . htmlspecialcharsbx(
                $this->serverName
            ) . $this->pathToSmile . $image . '" border="0" data-code="' . $code . '" data-definition="' . $imageDefinition . '" alt="' . $code . '"' . ' style="' . ($width > 0 ? 'width:' . $width . 'px;' : '') . ($height > 0 ? 'height:' . $height . 'px;' : '') . '"' . ' title="' . $description . '" class="bx-smile" />';
        $cacheKey = md5($html);
    ...
    public function convertCode($matches)
    {
        ...
        return $this->defended_tags(
            $this->convert_open_tag('code') . "<pre>" . $text . "</pre>" . $this->convert_close_tag('code'),
            'replace'
        );
    }
    ...
    public function convert_spoiler_tag($text, $title = "")
    {
        ...
        if ($this->type === 'mail') {
            return "<dl><dt>" . ($title ?: Loc::getMessage(
                    "MAIN_TEXTPARSER_SPOILER"
                )) . "</dt><dd>" . htmlspecialcharsbx($text) . "</dd></dl>";
        }
    ...
    public function convert_font_attr($attr, $value = "", $text = "")
    {
        ...
        if ($attr == "size") {
            ...
            ) ? $this->arFontSize[$value] . '%' : $this->arFontSize[$value]);
            ...
            switch ($type) {
                ...
                case 'email':
                    ...
                    if (
                        ...
                        $pathToUser .= (mb_strpos(
                                $pathToUser,
                                '?'
                            ) === false ? '?' : '&') . 'entityType=' . $this->pathToUserEntityType . '&entityId=' . intval(
                                $this->pathToUserEntityId
                            );
                    }
                    break;
                default:
    ...
    protected function render_user($fields)
    {
        ...
        return '<a class="blog-p-user-name' . $classAdditional . '" href="' . CComponentEngine::MakePathFromTemplate(
                $pathToUser,
                array("user_id" => $userId)
            ) . '" bx-tooltip-user-id="' . (!$this->bMobile ? $userId : '') . '"' . (!empty($fields['TOOLTIP_PARAMS']) ? ' bx-tooltip-params="' . htmlspecialcharsbx(
                    $fields['TOOLTIP_PARAMS']
                ) . '"' : '') . '>' . $userName . '</a>';
    }
    ...
    public function detectTags($text)
    {
        ...
        $result = array_map(
            function ($tag) {
                return CTextParser::cleanTag($tag);
            },
            $result
        );
    ...
    public function convert_anchor_tag($url, $text, $pref = "")
    {
        ...
        if (mb_substr($url, 0, 1) != "/" && !preg_match(
                "/^(" . $this->getAnchorSchemes() . ")\\:/i" . BX_UTF_PCRE_MODIFIER,
                $url
            )) {
            $url = "http://" . $url;
        }
        ...
        if ($bShortUrl &&
            ...
            preg_match(
                "/^(" . $this->getAnchorSchemes() . "):\\/\\/(\\S+)$/i" . BX_UTF_PCRE_MODIFIER,
                $text,
                $matches
            )) {
            $uri_type = $matches[1];
    ...
    public function pre_convert_anchor_tag($url, $text = "", $str = "")
    {
        if (mb_stripos($str, "[url") !== 0) {
            ...
            if ($text <> '') {
                ...
                $text = preg_replace(
                    "/(?<=^|[" . $word_separator . "]|\\s)(?<!\\[nomodify\\]|<nomodify>)((" . $this->getAnchorSchemes(
                    ) . "):\\/\\/[._:a-z0-9@-].*?)(?=[\\s'\"{}\\[\\]]|&quot;|\$)/is" . BX_UTF_PCRE_MODIFIER,
                    "\\1",
                    $text
                );
    ...
    public function post_convert_anchor_tag($str)
    {
        if (!empty($this->defended_urls)) {
            return str_replace(
                array_reverse(array_values($this->defended_urls)),
                array_reverse(array_keys($this->defended_urls)),
                $str
            );
        } else {
            return $str;
