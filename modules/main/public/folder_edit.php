...
$arInheritProperties = $APPLICATION->GetDirPropertyList(
    Array($site, $path)
); //All properties for folder. Includes properties from root folders
...
<? if (IsModuleInstalled("fileman")): ?>
    <p><a href="/bitrix/admin/fileman_folder.php?lang=<?= urlencode($lang) ?>&site=<?= urlencode(
            $site
        ) ?>&path=<?= urlencode($path) ?>&back_url=<?= urlencode($back_url) ?>"><?= GetMessage(
                "FOLDER_EDIT_IN_ADMIN_SECTION"
            ) ?></a></p>
<? endif ?>
...
?>
    ...
    <table class="bx-width100" id="bx_folder_properties">
        ...
        <tr>
            ...
            <td><input type="text" style="width:90%;" name="sSectionName"
                       value="<?= htmlspecialcharsEx($sSectionName) ?>"></td>
        </tr>
        ...
        <? foreach ($arDirProperties as $propertyCode => $propertyValue): ?>
            ...
            <tr id="bx_user_property_<?= $propertyIndex ?>">
                <td class="bx-popup-label bx-width30"><?= htmlspecialcharsEx(ToUpper($propertyCode)) ?><input
                            type="hidden" name="PROPERTY[<?= $propertyIndex ?>][CODE]"
