                ...
                <table border="0" cellpadding="0" cellspacing="0" class="bx-width100 internal" class="menu-table">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            ?>
                            <span onclick="if (!GLOBAL_bDisableActions) {currentLink = '<?= $i ?>'; OpenFileBrowserWindFile_<?= $i ?>();}"
                                  class="rowcontrol folder" title="<?= GetMessage('MENU_EDIT_TOOLTIP_FD') ?>"></span>
                        </td>
                        <td><span onclick="menuMoveUp(<?= $i ?>)" class="rowcontrol up"
                                  style="visibility: <?= ($i == 1 ? 'hidden' : 'visible') ?>"
                                  title="<?= GetMessage('MENU_EDIT_TOOLTIP_UP') ?>"></span></td>
                        <td><span onclick="menuMoveDown(<?= $i ?>)" class="rowcontrol down"
                                  style="visibility: <?= ($i == count($aMenuLinksTmp) ? 'hidden' : 'visible') ?>"
                                  title="<?= GetMessage('MENU_EDIT_TOOLTIP_DOWN') ?>"></span></td>
                        <td><span onclick="menuDelete(<?= $i ?>)" class="rowcontrol delete"
                                  title="<?= GetMessage('MENU_EDIT_TOOLTIP_DELETE') ?>"></span></td>
                    </tr>
                    </tbody>
                </table>
            </div></div><? endfor ?>
    </div>
<? if (!$only_edit): ?><br/><input type="button" onClick="menuAdd()"
                                   value="<? echo GetMessage("MENU_EDIT_ADD_ITEM") ?>" /><? endif; ?>
    ...
    <script type="text/javascript">
        ...
        function setLink(filename, path, site) {
            ...
            GetForm()['link_' + currentLink].value = ((path == '' || path == '/') ? '/' : path + '/') + filename;
        ...
        function menuDelete(i) {
            ...
            var obInput = <?echo $obJSPopup->jsPopup?>.
            GetForm()['del_' + i];
        ...
        function menuAdd() {
            var obCounter = <?echo $obJSPopup->jsPopup?>.
            GetForm().itemcnt;
            var nums = parseInt(obCounter.value);
            obCounter.value = ++nums;

            var obPlacement = BX.create('DIV', {
                props: {
                    className: 'bx-menu-placement',
                    id: 'bx_menu_placement_' + nums
                }
            });
