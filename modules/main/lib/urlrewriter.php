        ...
        foreach ($arChildren as $child) {
            if ($child->isDirectory()) {
                ...
                if ($maxExecutionTime <= 0 || $ns["FLG"] == ''
                    ...
                    if (static::recursiveReindex(
                            $rootPath,
                            mb_substr($child->getPath(), mb_strlen($rootPath)),
                            $arSites,
                            $maxExecutionTime,
                            $ns
                        ) === false) {
        ...
        if (is_array($searchMasksCache)) {
            ...
            $inc = str_replace(
                ...
                str_replace(
                    "*",
                    ".*?",
                    str_replace("?", ".", str_replace(".", "\\.", str_replace("\\", "/", $inc)))
                )
            );
            ...
            $exc = str_replace(
                ...
                str_replace(
                    "*",
                    ".*?",
                    str_replace("?", ".", str_replace(".", "\\.", str_replace("\\", "/", $exc)))
                )
            );
...
class UrlRewriterRuleMaker
{
    ...
    public function process($sefRule)
    {
        ...
        $this->condition = "#^" . preg_replace_callback(
                "/(#[a-zA-Z0-9_]+#)/",
