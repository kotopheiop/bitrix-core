...
abstract class MysqlCommonConnection extends Connection
{
    ...
    public function createIndex($tableName, $indexName, $columnNames, $columnLengths = null, $indexType = null)
    {
        ...
        if ($indexType !== null
            && in_array(
                mb_strtoupper($indexType),
                [static::INDEX_UNIQUE, static::INDEX_FULLTEXT, static::INDEX_SPATIAL],
                true
            )
        ) {
        ...
        $sql = 'CREATE ' . $indexTypeSql . ' INDEX ' . $sqlHelper->quote($indexName) . ' ON ' . $sqlHelper->quote(
                $tableName
            )
            . ' (' . join(', ', $columnNames) . ')';
    ...
    {
        $this->query(
            'RENAME TABLE ' . $this->getSqlHelper()->quote($currentName) . ' TO ' . $this->getSqlHelper()->quote(
                $newName
            )
        );
    }
