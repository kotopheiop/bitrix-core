    ...
    public function copy($path, array $file)
    {
        ...
        if (mb_substr($newFile, -mb_strlen($file['tmp_name'])) == $file['tmp_name']) {
            ...
            if (!$directory->create()) {
                ...
                if (!((!file_exists($newFile) || @unlink($newFile)) && File::http()->download(
                        $file['tmp_url'],
                        $newFile
                    ) !== false)) {
                ...
                if (!file_exists($file['tmp_name'])) {
                    ...
                    if (!file_exists($file['tmp_name'])) {
                        ...
                        if (!((!file_exists($newFile) || @unlink($newFile)) && move_uploaded_file(
                                $file['tmp_name'],
                                $newFile
                            ))) {
...
class CloudStorage extends Storage implements Storable
{
    ...
    private function findBucket($file)
    {
        $bucket = \CCloudStorage::findBucketForFile(
            array('FILE_SIZE' => $file['size'], 'MODULE_ID' => $this->moduleId),
            $file["name"]
        );
    ...
    protected function moveIntoCloud(\CCloudStorageBucket $bucket, $path, $file)
    {
        ...
        if (!isset(\Bitrix\Main\Application::getInstance()->getSession()["upload_tmp"][$path])) {
            $relativePath = \Bitrix\Main\Application::getInstance()->getSession(
            )["upload_tmp"][$path] = \CCloudTempFile::GetDirectoryName($bucket, 12) . $filename;
        } else {
