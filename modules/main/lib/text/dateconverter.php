...
class DateConverter
{
    ...
    public static function decode($text, $limit = 0)
    {
        ...
        $timeOfStartDate = str_pad(intval($workTimeStart[0]), 2, "0", STR_PAD_LEFT) . ':' . str_pad(
                intval($workTimeStart[1]),
                2,
                "0",
                STR_PAD_LEFT
            );
        ...
        $timeOfEndDate = str_pad(intval($workTimeEnd[0]), 2, "0", STR_PAD_LEFT) . ':' . str_pad(
                intval($workTimeEnd[1]),
                2,
                "0",
                STR_PAD_LEFT
            );
                    ...
                    case $pattern[1]:
                        ...
                        if (is_array($metrics[1])) {
                            foreach ($metrics[1] as $key => $metric) {
                                if ($metric['TYPE'] == 'WEEK') {
                                    $position = defined("BX_UTF") ? mb_strlen(
                                        substr($text, 0, $matchPattern[1])
                                    ) : $matchPattern[1];
                                    $matchWord = self::getMatchWord($originalText, $position);

                                    if ($matchPattern[1] < $metric['POSITION'] && $matchPattern[1] + mb_strlen(
                                            $matchWord
                                        ) >= $metric['POSITION'] + $metric['COUNT']) {
                                        unset($metrics[1][$key]);
                ...
                if (in_array($matchType, Array('BEFORE', 'AFTER')) && isset($metrics[1])) {
                    foreach ($metrics[1] as $key => $metric) {
                        if (in_array($metric['TYPE'], Array('DAYAFTERTOMORROW', 'DAYBEFOREYESTERDAY'))) {
                            ...
                            if (in_array($metric['TYPE'], Array('TOMORROW', 'YESTERDAY'))) {
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            if ($patternOriginal[$i] != 'bxt2dmetricskip') {
                $pattern[$i] = str_replace(
                    $defaultPregExceptionSearch,
                    $defaultPregExceptionReplace,
                    $patternOriginal[$i]
                );
                $pattern[$i] = str_replace(
                    Array('yyyy', 'yy', 'mm', 'dd'),
        ...
        if ($countOfMetrics == 1 && (isset($metrics[3]) || isset($metrics[5]))) {
            ...
            if ($countOfMetrics == 1) {
                ...
                if ($countOfMetrics == 2 && isset($metrics[3])) {
                    $useDefault = !isset($metrics[5]);
                } else {
                    if ($countOfMetrics == 2 && isset($metrics[5]) && (isset($metrics[1]) || isset($metrics[2]) || isset($metrics[4]))) {
                        if (isset($metrics[1])) {
                            foreach ($metrics[1] as $metric) {
                                ...
                                $result[] = new DateConverterResult(
                                    $progressDate,
                                    $matchParams,
                                    DateConverterResult::TYPE_RELATIVE,
                                    $metric,
                                    $metrics[5]
                                );
                            }
                            ...
                            if (isset($metrics[2])) {
                                foreach ($metrics[2] as $metric) {
                                    ...
                                    $result[] = new DateConverterResult(
                                        $progressDate,
                                        $matchParams,
                                        DateConverterResult::TYPE_DAYOFWEEK,
                                        $metric,
                                        $metrics[5]
                                    );
                                }
                                ...
                                if (isset($metrics[4])) {
                                    foreach ($metrics[4] as $metric) {
                                        ...
                                        $result[] = new DateConverterResult(
                                            $progressDate,
                                            $matchParams,
                                            DateConverterResult::TYPE_CALENDAR,
                                            $metric,
                                            $metrics[5]
                                        );
                                    }
                        ...
                        if ($countOfMetrics == 2 && isset($metrics[1]) && (isset($metrics[2]) || isset($metrics[4]))) {
                            if (isset($metrics[2])) {
                                foreach ($metrics[2] as $metric) {
                                    $array = array();
        ...
        if ($useDefault) {
            ...
            if (isset($metrics[4]) && count($metrics[4])) {
                ...
                if (isset($metrics[2]) && count($metrics[2])) {
                    foreach ($metrics[2] as $metric) {
                    ...
                    if (isset($metrics[1]) && count($metrics[1])) {
                        foreach ($metrics[1] as $metric) {
                        ...
                        if (isset($metrics[5]) && count($metrics[5])) {
                            foreach ($metrics[5] as $metric) {
                                ...
                                $result[] = new DateConverterResult(
                                    $progressDate,
                                    $matchParams,
                                    DateConverterResult::TYPE_PARTOFDAY,
                                    $metric,
                                    $modificators
                                );
                            }
                        } else {
                            if (isset($metrics[3]) && count($metrics[3])) {
                                foreach ($metrics[3] as $metric) {
                                    ...
                                    $result[] = new DateConverterResult(
                                        $progressDate,
                                        $matchParams,
                                        DateConverterResult::TYPE_MODIFIER,
                                        $metric
                                    );
                                }
                            }
     ...
     *
     */
    private static function createDateUsingMetrics(
        $type,
        $metric,
        &$metricModificator = array(),
        \Bitrix\Main\Type\DateTime $date = null
    ) {
        ...
        switch ($type) {
            case 1:
                ...
                foreach ($metricModificator as $currentModificator) {
                    if ($metric['TYPE'] == 'WEEK') {
                        if (in_array($currentModificator['TYPE'], Array('BEFORE', 'PREVIOUS'))) {
                            ...
                            if (in_array($currentModificator['TYPE'], Array('AFTER', 'NEXT'))) {
                                $metricDate->add('1 WEEK');
                            } else {
                                if ($currentModificator['TYPE'] == 'MIDDLEOF') {
                                    $metricDate = self::getDateOfDayOfCurrentWeek('WEDNESDAY', $metricDate);
                                    $modificator = true;
                                } else {
                                    if ($currentModificator['TYPE'] == 'STARTOF') {
                                        $metricDate = self::getDateOfDayOfCurrentWeek('MONDAY', $metricDate);
                                        $modificator = true;
                                    } else {
                                        if ($currentModificator['TYPE'] == 'ENDOF') {
                        ...
                        if ($metric['TYPE'] == 'WEEKEND') {
                            if (in_array($currentModificator['TYPE'], Array('BEFORE'))) {
                                ...
                                if (in_array($currentModificator['TYPE'], Array('PREVIOUS'))) {
                                    $metricDate->add('-1 WEEK');
                                    $metricDate = self::getDateOfDayOfCurrentWeek('SATURDAY', $metricDate);
                                    $modificator = true;
                                } else {
                                    if (in_array($currentModificator['TYPE'], Array('NEXT'))) {
                                        $metricDate->add('1 WEEK');
                                        $metricDate = self::getDateOfDayOfCurrentWeek('SATURDAY', $metricDate);
                                        $modificator = true;
                                    } else {
                                        if (in_array($currentModificator['TYPE'], Array('AFTER'))) {
                                            ...
                                            if ($currentModificator['TYPE'] == 'MIDDLEOF') {
                                                ...
                                                if ($currentModificator['TYPE'] == 'STARTOF') {
                                                    $metricDate = self::getDateOfDayOfCurrentWeek(
                                                        'SATURDAY',
                                                        $metricDate
                                                    );
                            ...
                            if ($metric['TYPE'] == 'MONTH') {
                                if (in_array($currentModificator['TYPE'], Array('BEFORE', 'PREVIOUS'))) {
                                    $metricDate->add('-1 MONTH');
                                    $numberOfMonth = $metricDate->format('t');
                                    $metricDate = new \Bitrix\Main\Type\DateTime(
                                        $metricDate->format(
                                            'Y-m'
                                        ) . '-' . $numberOfMonth . ' ' . $timeOfEndDate . ':00', 'Y-m-d H:i:s'
                                    );
                                    $modificator = true;
                                } else {
                                    if (in_array($currentModificator['TYPE'], Array('AFTER', 'NEXT'))) {
                                        ...
                                        if ($currentModificator['TYPE'] == 'MIDDLEOF') {
                                            ...
                                            $metricDate = new \Bitrix\Main\Type\DateTime(
                                                $metricDate->format(
                                                    'Y-m'
                                                ) . '-' . $numberOfHalfMonth . ' ' . $timeOfEndDate . ':00',
                                                'Y-m-d H:i:s'
                                            );
                                            $modificator = true;
                                        } else {
                                            if ($currentModificator['TYPE'] == 'STARTOF') {
                                                $metricDate = new \Bitrix\Main\Type\DateTime(
                                                    $metricDate->format('Y-m') . '-01 ' . $timeOfStartDate . ':00',
                                                    'Y-m-d H:i:s'
                                                );
                                                $modificator = true;
                                            } else {
                                                if ($currentModificator['TYPE'] == 'ENDOF') {
                                                    ...
                                                    $metricDate = new \Bitrix\Main\Type\DateTime(
                                                        $metricDate->format(
                                                            'Y-m'
                                                        ) . '-' . $numberOfMonth . ' ' . $timeOfEndDate . ':00',
                                                        'Y-m-d H:i:s'
                                                    );
                                                    $modificator = true;
                                                }
                    ...
                    if ($metric['TYPE'] == 'WEEK') {
                        ...
                        if ($metric['TYPE'] == 'WEEKEND') {
                            if (ToLower($metricDate->format('l')) == 'saturday') {
                                ...
                                if (ToLower($metricDate->format('l')) != 'sunday') {
                            ...
                            if ($metric['TYPE'] == 'MONTH') {
                                ...
                                if ($numberOfMonth > 15) {
                                    ...
                                    if ($numberOfMonth >= 10 && $numberOfMonth <= 15) {
                ...
                $result = new \Bitrix\Main\Type\DateTime(
                    $metricDate->format('Y-m-d') . ' ' . $metricTime . ':00',
                    'Y-m-d H:i:s'
                );
            ...
            case 2:
                ...
                foreach ($metricModificator as $currentModificator) {
                    ...
                    if ($currentModificator['TYPE'] == 'BEFORE') {
                        ...
                        if ($currentModificator['TYPE'] == 'AFTER') {
                            $metricDate->add('1 DAY');
                        } else {
                            if ($currentModificator['TYPE'] == 'THIS') {
                                $metricDate->add('LAST ' . $metric['TYPE']);
                            } else {
                                if (in_array($currentModificator['TYPE'], Array('NEXT', 'PREVIOUS'))) {
                                    ...
                                    if ($currentModificator['TYPE'] == 'MIDDLEOF') {
                                        ...
                                        if ($currentModificator['TYPE'] == 'STARTOF') {
                                            ...
                                            if (in_array(
                                                $currentModificator['TYPE'],
                                                Array('MORNING', 'LUNCH', 'EVENING', 'HHMM')
                                            )) {
                                                $metricTime = $currentModificator['VALUE'];
                                            }
                                        }
    ...
    private static function checkModifierPosition($metrics, $metricModifier)
    {
        ...
        while ($while && $maxWhile > 0) {
            ...
            foreach ($metricModifier as $key => $currentModificator) {
                $diffResult = abs(
                    $newMetrics['POSITION'] - ($currentModificator['POSITION'] + $currentModificator['COUNT'])
                );
                if ($diffResult < 5) {
                    $while = true;
                    ...
                    $diffResult = abs(
                        $newMetrics['POSITION'] + $newMetrics['COUNT'] - ($currentModificator['POSITION'])
                    );
                    if ($diffResult < 5) {
                        $while = true;
                        $newMetrics = $currentModificator;
