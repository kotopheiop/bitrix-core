...
class Vk extends OpenGraph
{
    public function handle(HtmlDocument $document)
    {
        ...
        if ($document->getExtraField('VIDEO') <> '' && $document->getExtraField(
                'VIDEO_TYPE'
            ...
            if (!empty($swfQuery)) {
                ...
                if (isset($swfParams['oid']) && isset($swfParams['vid']) && isset($swfParams['embed_hash'])) {
                    ...
                    $embedUri->addParams(
                        array('oid' => $swfParams['oid'], 'id' => $swfParams['vid'], 'hash' => $swfParams['embed_hash'])
                    );
                    if ($document->getExtraField('VIDEO_WIDTH') && $document->getExtraField(
                            'VIDEO_WIDTH'
                    ...
                    if ($document->getExtraField('VIDEO_HEIGHT') && $document->getExtraField(
                            'VIDEO_HEIGHT'
                    ...
                    $iframe = '<iframe src="' . $embedUri->getLocator(
                        ) . '" allowfullscreen="" width="' . $width . '" height="' . $height . '" frameborder="0"></iframe>';
