...
class UserIndexTable extends Main\Entity\DataManager
{
    ...
    public static function merge(array $data)
    {
        ...
        if (isset($updateData['SEARCH_USER_CONTENT'])) {
            ...
            $updateData['SEARCH_USER_CONTENT'] = new \Bitrix\Main\DB\SqlExpression(
                "IF(SHA1(SEARCH_USER_CONTENT) = '{$encryptedValue}', SEARCH_USER_CONTENT, '{$value}')"
            );
        }
        ...
        if (isset($updateData['SEARCH_DEPARTMENT_CONTENT'])) {
            ...
            $updateData['SEARCH_DEPARTMENT_CONTENT'] = new \Bitrix\Main\DB\SqlExpression(
                "IF(SHA1(SEARCH_DEPARTMENT_CONTENT) = '{$encryptedValue}', SEARCH_DEPARTMENT_CONTENT, '{$value}')"
            );
        }
        ...
        if (isset($updateData['SEARCH_ADMIN_CONTENT'])) {
            ...
            $updateData['SEARCH_ADMIN_CONTENT'] = new \Bitrix\Main\DB\SqlExpression(
                "IF(SHA1(SEARCH_ADMIN_CONTENT) = '{$encryptedValue}', SEARCH_ADMIN_CONTENT, '{$value}')"
            );
        }
