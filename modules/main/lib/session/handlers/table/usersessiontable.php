...
class UserSessionTable extends Entity\DataManager
{
    ...
    public static function lock($id, $timeout = 60)
    {
        ...
        if ($connection instanceof MysqlCommonConnection) {
            ...
            trigger_error(
                sprintf('SessionTable::lock not supported for connection of type "%s"', get_class($connection)),
                E_USER_WARNING
            );
        }
    ...
    public static function unlock($id)
    {
        ...
        if ($connection instanceof MysqlCommonConnection) {
            ...
            trigger_error(
                sprintf('SessionTable::unlock not supported for connection of type "%s"', get_class($connection)),
                E_USER_WARNING
            );
        }
