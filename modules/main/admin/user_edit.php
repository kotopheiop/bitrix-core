...
if ($ID <> 1 || $COPY_ID > 0):
    ...
    <tr>
        ...
        <td>
            ...
            <? else:?>
                <input type="checkbox" <?
                if ($str_ACTIVE == "Y") echo " checked" ?> disabled>
...
?>
    ...
    if ($ID <= 0 || $COPY_ID > 0): ?> class="adm-detail-required-field"<?endif ?>>
        <td><?
            echo GetMessage('NEW_PASSWORD_REQ') ?>:<sup><span class="required">1</span></sup></td>
        <td><input type="password" name="NEW_PASSWORD" size="30" maxlength="255"
                   value="<? echo htmlspecialcharsbx($NEW_PASSWORD) ?>" autocomplete="off"
    ...
    if ($ID <= 0 || $COPY_ID > 0): ?> class="adm-detail-required-field"<?endif ?>>
        ...
        <td><input type="password" name="NEW_PASSWORD_CONFIRM" size="30" maxlength="255"
                   value="<? echo htmlspecialcharsbx($NEW_PASSWORD_CONFIRM) ?>" autocomplete="off"></td>
    </tr>
<?
    ...
    if (!empty($arAuthList)):
        ...
        <tr>
            ...
            <td>
                ...
                <select id="bx_EXTERNAL_AUTH_ID" name="EXTERNAL_AUTH_ID"<?
                if (!$canSelfEdit) echo " disabled" ?> onchange="BXAuthSwitch(this.value)">
                    <option value=""><?
                        echo GetMessage("MAIN_USERED_AUTH_INT") ?></option>
                    <?
                    foreach ($arAuthList as $arExtAuth):?>
                        <option value="<?= $arExtAuth['ID'] ?>"<?
                        if ($str_EXTERNAL_AUTH_ID == $arExtAuth['ID']) echo ' selected'; ?>><?= $arExtAuth['NAME'] ?></option>
                    <?endforeach; ?>
                </select>
            </td>
        </tr>
                    ...
                    <tr>
                        ...
                        <td class="align-left">
                            <label for="GROUP_ID_ACT_ID_<?= $ind ?>"><?= htmlspecialcharsbx($arGroups["NAME"]) ?> [<a
                                        ...
                                        title="<?= GetMessage("MAIN_VIEW_GROUP") ?>"><?
...
?>
    <tr>
        <td><?
            echo $tabControl->GetCustomLabelHTML() ?></td>
        <td><?
            echo SelectBoxFromArray("PERSONAL_COUNTRY", GetCountryArray(), $str_PERSONAL_COUNTRY, GetMessage("USER_DONT_KNOW")); ?></td>
    </tr>
<?
...
?>
    <tr>
        <td><?
            echo $tabControl->GetCustomLabelHTML() ?></td>
        <td><?
            echo SelectBoxFromArray("WORK_COUNTRY", GetCountryArray(), $str_WORK_COUNTRY, GetMessage("USER_DONT_KNOW")); ?></td>
    </tr>
<?
...
if (($editable && $ID != $USER->GetID()) || $USER->IsAdmin()):
    ...
    ?>
    <tr>
        <td align="center" colspan="2"><textarea name="ADMIN_NOTES" cols="50" rows="10" style="width:100%;"><?
                echo $str_ADMIN_NOTES ?></textarea></td>
    </tr>
    <?
...
if (!defined('BX_PUBLIC_MODE') || BX_PUBLIC_MODE != 1): ?>
    ...
    echo GetMessage("RATING_BONUS_NOTICE") ?><br>
    <?
    echo EndNote(); ?>
<?endif; ?>
