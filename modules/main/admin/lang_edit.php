...
<form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?" name="form1">
    ...
    <? if ($_REQUEST["COPY_ID"] <> ''): ?><input type="hidden" name="COPY_ID"
                                                 value="<? echo HtmlFilter::encode($_REQUEST["COPY_ID"]) ?>"><? endif ?>
    <?
    $tabControl->Begin();
    ...
    <tr>
        ...
        <td><input type="checkbox" name="ACTIVE" id="active"
                   value="Y"<? if ($language["ACTIVE"] == "Y") echo " checked" ?>></td>
    </tr>
    <tr class="adm-detail-required-field">
        ...
        <td>
            ...
            <select name="CULTURE_ID" onchange="BXSetCulture()" id="bx_culture_select">
                ...
                foreach ($cultures as $cult):
                    ...
                    if ($cult["ID"] == $language["CULTURE_ID"]) echo " selected" ?>><?= HtmlFilter::encode($cult["NAME"]) ?></option>
                <?
                endforeach;
                ?>
            </select>
