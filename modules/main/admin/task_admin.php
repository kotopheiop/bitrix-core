...
while ($arRes = $rsData->NavNext(true, "f_")) {
    $row =& $lAdmin->AddRow(
        $f_ID,
        $arRes,
        "task_edit.php?lang=" . LANGUAGE_ID . "&ID=" . $f_ID,
        GetMessage("MAIN_EDIT_TITLE")
    );
    $row->AddViewField(
        "ID",
        "<a href='task_edit.php?lang=" . LANGUAGE_ID . "&ID=" . $f_ID . "' title='" . GetMessage(
            "MAIN_EDIT_TITLE"
        ) . "'>" . $f_ID . "</a>"
    );
    ...
    $arActions[] = array(
        "ICON" => "edit",
        "TEXT" => (($sys) ? GetMessage("MENU_VIEW") : GetMessage("MAIN_ADMIN_MENU_EDIT")),
        "DEFAULT" => true,
        "ACTION" => $lAdmin->ActionRedirect("task_edit.php?ID=" . $f_ID)
    );
    $arActions[] = array(
        "ICON" => "copy",
        "TEXT" => GetMessage("MAIN_ADMIN_MENU_COPY"),
        "ACTION" => $lAdmin->ActionRedirect("task_edit.php?COPY_ID=" . $f_ID)
    );
    ...
    if ($USER->CanDoOperation('edit_tasks') && (!$sys)) {
        ...
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . GetMessage('CONFIRM_DEL_TASK') . "')) " . $lAdmin->ActionDoGroup(
                    $f_ID,
                    "delete"
                )
        );
...
?>
    <form name="find_form" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?">
        ...
        <tr>
            ...
            <td nowrap><input type="text" name="find_letter" value="<? echo htmlspecialcharsbx($find_letter) ?>"
                              size="10"></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td nowrap>
                ...
                echo SelectBoxFromArray(
                    "find_binding",
                    array("reference" => $arRef, "reference_id" => $arRefId),
                    htmlspecialcharsbx($find_binding)
                );
                ?>
            </td>
        </tr>
        ...
        $oFilter->Buttons(
            array(
                "table_id" => htmlspecialcharsbx($sTableID),
