...
$dbResultList = CSmileSet::getList(
    Array(
...
$lAdmin->AddHeaders(
    array(
        ...
        array(
            "id" => "SMILE_COUNT",
            "content" => GetMessage("SMILE_SMILE_COUNT"),
            "sort" => "SMILE_COUNT",
            "default" => true
        ),
    )
...
while ($arForum = $dbResultList->NavNext(true, "f_")) {
    ...
    $row->AddViewField(
        ...
        '<a title="' . GetMessage(
            "SMILE_EDIT_DESCR"
        ) . '" href="' . "smile.php?SET_ID=" . $f_ID . "&lang=" . LANG . "&" . GetFilterParams(
            "filter_"
        ) . '">' . ($f_NAME <> '' ? $f_NAME : GetMessage(
            'SMILE_SET_NAME',
            Array('#ID#' => $f_ID)
        )) . '</a>'
    );
        ...
        $arActions = Array(
            array(
                ...
                "ACTION" => $lAdmin->ActionRedirect(
                    "smile_set_edit.php?GALLERY_ID=" . $arFilter['PARENT_ID'] . "&ID=" . $f_ID . "&lang=" . LANG . "&" . GetFilterParams(
                        "filter_"
                    ) . ""
                ),
                "DEFAULT" => true
