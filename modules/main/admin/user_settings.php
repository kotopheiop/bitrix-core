...
<form method="POST" name="form1" action="<? echo $APPLICATION->GetCurPage() ?>">
    ...
    <tr>
        ...
        <td width="60%"><input type="checkbox" name="context_menu"
                               value="Y"<? if ($aUserOpt["context_menu"] == "Y") echo " checked" ?>
                               onclick="this.form.context_ctrl[0].disabled = this.form.context_ctrl[1].disabled = !this.checked">
        </td>
    </tr>
    <tr>
        <td class="adm-detail-valign-top"><? echo GetMessage("user_sett_context_ctrl") ?></td>
        <td>
            <input type="radio" name="context_ctrl" id="context_ctrl_N"
                   value="N"<? if ($aUserOpt["context_ctrl"] <> "Y") echo " checked" ?><? if ($aUserOpt["context_menu"] <> "Y") echo " disabled" ?>><label
                    for="context_ctrl_N"><? echo GetMessage("user_sett_context_ctrl_val1") ?></label><br>
            <input type="radio" name="context_ctrl" id="context_ctrl_Y"
                   value="Y"<? if ($aUserOpt["context_ctrl"] == "Y") echo " checked" ?><? if ($aUserOpt["context_menu"] <> "Y") echo " disabled" ?>><label
                    for="context_ctrl_Y"><? echo GetMessage("user_sett_context_ctrl_val2") ?></label><br>
        </td>
    </tr>
    <tr>
        <td><? echo GetMessage("user_sett_autosave") ?></td>
        <td><input type="checkbox" name="autosave"
                   value="Y"<? if ($aUserOpt["autosave"] == "Y") echo " checked=\"checked\"" ?> /></td>
    </tr>
    <tr class="heading">
    ...
    <tr>
        ...
        <td><input type="checkbox" name="panel_dynamic_mode"
                   value="Y"<? if ($aUserOpt["panel_dynamic_mode"] == "Y") echo " checked" ?>></td>
    </tr>
    <tr>
        <td><? echo GetMessage("MAIN_OPTION_PAGE_EDIT_ENABLE") ?></td>
        <td><input type="checkbox" name="page_edit_control_enable"
                   value="Y"<? if ($aUserOpt["page_edit_control_enable"] != "N") echo " checked" ?>></td>
    </tr>
    <tr class="heading">
    ...
    <tr>
        ...
        <td><input type="checkbox" name="start_menu_preload"
                   value="Y"<? if ($aUserOpt["start_menu_preload"] == "Y") echo " checked" ?>></td>
    </tr>
    <tr>
        <td><? echo GetMessage("user_sett_titles") ?></td>
        <td><input type="checkbox" name="start_menu_title"
                   value="Y"<? if ($aUserOpt["start_menu_title"] == "Y") echo " checked" ?>></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            ...
            ?>
            <input type="text" name="sound_login" value="<? echo htmlspecialcharsbx($aUserOpt["sound_login"]) ?>"
                   size="40">
            <input type="button" value="..." title="<? echo GetMessage("user_sett_sounds_button_title") ?>"
                   onclick="OpenFileBrowserWindFile()">
        </td>
    </tr>
    ...
    $tabControl->Buttons();
    ?>
    <input<? if (!$editable) echo " disabled" ?> type="submit" name="apply"
                                                 value="<? echo GetMessage("admin_lib_edit_apply") ?>"
                                                 title="<? echo GetMessage("admin_lib_edit_apply_title") ?>"
                                                 class="adm-btn-save">
    <?
    $tabControl->End();
