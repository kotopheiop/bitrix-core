...
$aTabs = array(
    array(
        "DIV" => "edit1",
        "TAB" => GetMessage("MAIN_TEMPLATE_LOAD_TITLE"),
        "ICON" => "template_load",
        "TITLE" => GetMessage("MAIN_TEMPLATE_LOAD_TITLE")
    ),
);
...
<form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?" name="bform2" enctype="multipart/form-data">
    ...
    <input <? if (!$isAdmin) echo "disabled" ?> type="submit" name="import"
                                                value="<? echo GetMessage("MAIN_TEMPLATE_LOAD_SUBMIT") ?>"
                                                class="adm-btn-save">
    <?
    $tabControl->End();
    ?>
