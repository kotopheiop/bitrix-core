...
if ($_SERVER["REQUEST_METHOD"] == "POST" && ($edit_php || $lpa) && check_bitrix_sessid()) {
    ...
    if ($lpa) {
        ...
        if ($l > 0) {
            ...
            for ($n = 0; $n < $l; $n++) {
                ...
                if ($usrc == unifyPHPfragment('$APPLICATION->ShowPanel()') ||
                    ...
                    ) == $comp2_begin) // component 2.0 or another predefined fragment
                {
                    continue;
                }
                $arPHPscripts[] = $src;
        ...
        while (preg_match('/#PHP\d{4}#/i', $CONTENT, $res_php, PREG_OFFSET_CAPTURE)) {
            ...
            $php_fr_num = intval(
                    mb_substr($CONTENT, $php_begin + 4, 4)
                ) - 1; // Number of PHP fragment from #PHPXXXX# conctruction
        ...
        if ($body_pos > 0) {
            $content_ = mb_substr($CONTENT, 0, $body_pos + mb_strlen($body)) . $sp . mb_substr(
                    $CONTENT,
                    $body_pos + mb_strlen(
                        $body
                    )
        ...
        if ($wa_pos > 0) {
            $content__ = $s1 . mb_substr($content_, 0, $wa_pos + mb_strlen($wa)) . $s2 . mb_substr(
                    $content_,
                    $wa_pos + mb_strlen(
                        $wa
                    )
    ...
    if ($USER->CanDoFileOperation(
            'fm_view_file',
            array(SITE_ID, BX_PERSONAL_ROOT . "/templates/" . $ID)
        ) && $ID <> '') {
        CopyDirFiles(
            $_SERVER["DOCUMENT_ROOT"] . BX_PERSONAL_ROOT . "/templates/" . $ID,
            $_SERVER["DOCUMENT_ROOT"] . BX_PERSONAL_ROOT . "/tmp/templates/__bx_preview",
            false,
            true
        );
    }
    ...
    if ($CONTENT <> '') {
        ...
        if (!$APPLICATION->SaveFileContent(
            $_SERVER["DOCUMENT_ROOT"] . BX_PERSONAL_ROOT . "/tmp/templates/__bx_preview/header.php",
            $header
        )) {
        ...
        if (!$APPLICATION->SaveFileContent(
            $_SERVER["DOCUMENT_ROOT"] . BX_PERSONAL_ROOT . "/tmp/templates/__bx_preview/footer.php",
            $footer
        )) {
    ...
    if (!$APPLICATION->SaveFileContent(
        $_SERVER["DOCUMENT_ROOT"] . BX_PERSONAL_ROOT . "/tmp/templates/__bx_preview/styles.css",
        $STYLES
    )) {
    ...
    if (!$APPLICATION->SaveFileContent(
        $_SERVER["DOCUMENT_ROOT"] . BX_PERSONAL_ROOT . "/tmp/templates/__bx_preview/template_styles.css",
        $TEMPLATE_STYLES
    )) {
