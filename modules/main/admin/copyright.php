...
">
	...
	">
            ...
            echo Loc::getMessage(
                "main_copyright_bitrix_license",
                [
                    "#PRODUCT#" => $bitrixCopyright->getProductName(),
                    "#URL#" => '<a href="' . HtmlFilter::encode(
                            $bitrixCopyright->getLicenceUrl()
                        ) . '" target="_blank">',
                    "#/URL#" => '</a>',
                ]
            ) ?>
                ...
                <tr>
                    <td><?
                        if ($software->getProductUrl()) {
                            echo '<a href="' . HtmlFilter::encode(
                                    $software->getProductUrl()
                                ) . '" target="_blank">' . HtmlFilter::encode($software->getProductName()) . '</a>';
                        } else {
                    ...
                    <td><?
                        if ($software->getCopyrightUrl()) {
                            echo '<a href="' . HtmlFilter::encode(
                                    $software->getCopyrightUrl()
                                ) . '" target="_blank">' . HtmlFilter::encode($software->getCopyright()) . '</a>';
                        } else {
                    ...
                    <td><?
                        if ($software->getLicenceUrl()) {
                            echo '<a href="' . HtmlFilter::encode(
                                    $software->getLicenceUrl()
                                ) . '" target="_blank">' . HtmlFilter::encode($software->getLicence()) . '</a>';
                        } else {
                            if (($text = $software->getLicenceText())) {
                                echo '<div id="bx_licence_' . $i . '" style="display: none;"><div style="font-family: monospace; font-size: small; max-height: 500px;">' . nl2br(
                                        HtmlFilter::encode($text)
                                    ) . '</div></div>';
                                echo '<a href="javascript:void(0);" onclick="BX.UI.Dialogs.MessageBox.alert(BX(\'bx_licence_' . $i . '\').innerHTML, \'' . CUtil::JSEscape(
                                        Loc::getMessage("main_copyright_popup_title")
                                    ) . '\');">' . HtmlFilter::encode($software->getLicence()) . '</a>';
                            } else {
