...
$strBXError = '';
$bBitrixCloud = function_exists('openssl_encrypt') && CModule::IncludeModule('bitrixcloud') && CModule::IncludeModule(
        'clouds'
    );
...
if ($_REQUEST['action']) {
    ...
    if ($_REQUEST['action'] == 'download') {
        ...
        if ($url) {
            echo 'window.prompt("' . GetMessage("MAIN_DUMP_USE_THIS_LINK") . ' restore.php", "' . htmlspecialcharsbx(
                    $url
                ) . '");' . "\n";
        }
        echo '</script>';
                        ...
                        while (file_exists(DOCUMENT_ROOT . $path . '/' . $ID)) {
                            if (!rename(DOCUMENT_ROOT . $path . '/' . $ID, DOCUMENT_ROOT . $path . '/' . $new_name)) {
                                $lAdmin->AddGroupError(
                                    GetMessage("MAIN_DUMP_ERR_FILE_RENAME") . htmlspecialcharsbx($ID),
                                    $ID
                                );
                                break;
                            }
        ...
        foreach ($backup->listFiles() as $ar) {
            $arTmpFiles[] = array(
                ...
                'DATE' => preg_match(
                    ...
                    $ar['FILE_NAME'],
                    $r
                ) ? strtotime("{$r[1]}-{$r[2]}-{$r[3]} {$r[4]}:{$r[5]}:{$r[6]}") : '',
                'BUCKET_ID' => -1,
...
$lAdmin->AddHeaders(
    array(
        ...
        array(
            "id" => "DATE",
            "content" => GetMessage('MAIN_DUMP_FILE_TIMESTAMP'),
            "sort" => "timestamp",
            "default" => true
        )
    )
...
while ($f = $rsDirContent->NavNext(true, "f_")) {
    ...
    if (defined('DUMP_DEBUG_MODE')) {
        $arActions[] = array(
            ...
            "ACTION" => "if(k=prompt('" . CUtil::JSEscape(
                    GetMessage("MAIN_DUMP_SEND_FILE_CLOUD")
                ) . "?')) document.location=\"/bitrix/admin/dump.php?f_id=" . urlencode(
                    $f['NAME']
                ) . "&action=cloud_send&dump_bucket_id=-1&" . bitrix_sessid_get() . '&dump_encrypt_key="+k;'
        );
        $arActions[] = array(
            "ICON" => "archive",
            ...
            "ACTION" =>
                strpos($f['NAME'], '.enc.') ?
                    "if(k=prompt('" . CUtil::JSEscape(
                        GetMessage("INTEGRITY_CHECK")
                    ) . "?')) document.location=\"/bitrix/admin/dump.php?f_id=" . urlencode(
                        $f['NAME']
                    ) . "&action=check_archive&" . bitrix_sessid_get() . '&dump_encrypt_key="+k;'
                    :
                    "if(confirm('" . CUtil::JSEscape(
                        GetMessage("INTEGRITY_CHECK")
                    ) . "?')) document.location=\"/bitrix/admin/dump.php?f_id=" . urlencode(
                        $f['NAME']
                    ) . "&action=check_archive&" . bitrix_sessid_get() . '";'
        );
    }
    ...
    if (!preg_match('#\.sql$#i', $f['NAME'])) {
        ...
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . CUtil::JSEscape(
                    GetMessage("MAIN_RIGHT_CONFIRM_EXECUTE")
                ) . "')) AjaxSend('/bitrix/admin/dump_list.php?action=restore&f_id=" . $f['NAME'] . "&BUCKET_ID=" . $BUCKET_ID . "&" . bitrix_sessid_get(
                ) . "')"
        );
        ...
        if ($BUCKET_ID == 0) {
            if ($arWriteBucket) {
                ...
                foreach ($arWriteBucket as $arBucket) {
                    $arActions[] = array(
                        "ICON" => "clouds",
                        "TEXT" => GetMessage("MAIN_DUMP_SEND_CLOUD") . ' "' . htmlspecialcharsbx(
                                $arBucket['BUCKET']
                            ) . '"',
                        "ACTION" => "if(confirm('" . CUtil::JSEscape(
                                GetMessage("MAIN_DUMP_SEND_FILE_CLOUD")
                            ) . "?')) " . $lAdmin->ActionRedirect(
                                "/bitrix/admin/dump.php?f_id=" . urlencode(
                                    $f['NAME']
                                ) . "&action=cloud_send&dump_bucket_id=" . $arBucket['ID'] . "&" . bitrix_sessid_get()
                            )
                    );
                }
            }
            ...
            $arActions[] = array(
                ...
                "ACTION" => "if(name=prompt('" . CUtil::JSEscape(
                        GetMessage("MAIN_DUMP_ARC_NAME_W_O_EXT")
                    ) . "','" . htmlspecialcharsbx(
                        $arName['name']
                    ) . "')) tbl_dump.GetAdminList('/bitrix/admin/dump_list.php?ID=" . urlencode(
                        $f['NAME']
                    ) . "&action=rename&lang=" . LANGUAGE_ID . "&" . bitrix_sessid_get(
    ...
    if ($BUCKET_ID > -1) {
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . CUtil::JSEscape(
                    GetMessage('MAIN_DUMP_ALERT_DELETE')
                ) . "')) " . $lAdmin->ActionDoGroup($BUCKET_ID . '_' . $f['NAME'], "delete")
        );
    }
...
echo BeginNote();
echo GetMessage(
    "MAIN_DUMP_HEADER_MSG1",
    array('#EXPORT#' => 'https://www.1c-bitrix.ru/download/files/scripts/restore.php')
);
echo EndNote();
