...
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="list-table">
    ...
    foreach ($arModules as $info) :
        ...
        <tr>
            <td><b><?
                    echo htmlspecialcharsex($info["MODULE_NAME"]) ?></b> <?
                ...
                <br><?
                echo $info["MODULE_DESCRIPTION"] ?></td>
            <td ondblclick="<?
            ...
            <td nowrap><?
                echo CDatabase::FormatDate($info["MODULE_VERSION_DATE"], "YYYY-MM-DD HH:MI:SS", CLang::GetDateFormat("SHORT")); ?></td>
            <td nowrap><?
                if ($info["IsInstalled"]):?><?
                    echo GetMessage("MOD_INSTALLED") ?><? else:?><span class="required"><?
                    echo GetMessage("MOD_NOT_INSTALLED") ?></span><?endif ?></td>
            <td>
                <form action="<?
                echo $APPLICATION->GetCurPage() ?>" method="GET" id="form_for_<?
                echo htmlspecialcharsbx($info["MODULE_ID"]) ?>">
                    <input type="hidden" name="action" value="" id="action_for_<?
                    echo htmlspecialcharsbx($info["MODULE_ID"]) ?>">
                    ...
                    if ($info["IsInstalled"]):?>
                        <input <?
                        if (!$isAdmin || in_array($info["MODULE_ID"], array("fileman", "intranet", "ui")) || $info["MODULE_ID"] == "rest" && IsModuleInstalled('intranet')) echo "disabled" ?>
                                type="submit" name="uninstall" value="<?
                        echo GetMessage("MOD_DELETE") ?>">
                    <? else:?>
                        <input <?
                        if (!$isAdmin) echo "disabled" ?> type="submit" class="adm-btn-green" name="install" value="<?
                        echo GetMessage("MOD_INSTALL_BUTTON") ?>">
                    <?endif ?>
                </form>
            </td>
        </tr>
