...
$rsData = CSiteTemplate::GetList(
    array($by => $order),
    array('TYPE' => ($isEditingMessageThemePage ? 'mail' : '')),
    array("ID", "NAME", "DESCRIPTION", "SCREENSHOT", "SORT")
);
...
$lAdmin->AddHeaders(
    array(
        ...
        array(
            "id" => "DESCRIPTION",
            "content" => GetMessage('MAIN_T_ADMIN_DESCRIPTION'),
            "sort" => "description",
            "default" => true
        ),
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    $row =& $lAdmin->AddRow(
        $f_ID,
        $arRes,
        ($isEditingMessageThemePage ? "message_theme_edit.php" : "template_edit.php") . "?ID=" . $u_ID,
        GetMessage("MAIN_EDIT_TITLE")
    );

    $row->AddViewField(
        "SCREENSHOT",
        ($f_SCREENSHOT <> '' ? CFile::Show2Images(
            ($f_PREVIEW <> '' ? $f_PREVIEW : $f_SCREENSHOT),
            $f_SCREENSHOT,
            130,
            100,
            "border=0"
    ...
    $row->AddViewField(
        "ID",
        '<a href="' . ($isEditingMessageThemePage ? "message_theme_edit.php" : "template_edit.php") . '?lang=' . LANGUAGE_ID . '&amp;ID=' . $u_ID . '" title="' . GetMessage(
            "MAIN_EDIT_TITLE"
        ) . '">' . $f_ID . '</a>'
    );
    ...
    $arActions[] = array(
        "ICON" => "edit",
        "TEXT" => ($USER->CanDoOperation('edit_other_settings') || $USER->CanDoOperation(
            'lpa_template_edit'
        ) ? GetMessage("MAIN_ADMIN_MENU_EDIT") : GetMessage("MAIN_ADMIN_MENU_VIEW")),
    ...
    if ($edit_php) {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionDoGroup($u_ID, "copy")
        );
        $arActions[] = array(
            "ICON" => "export",
            "TEXT" => GetMessage("MAIN_ADMIN_LIST_EXPORT"),
        ...
        if ($edit_php) {
            ...
            $arActions[] = array(
                "ICON" => "delete",
                "TEXT" => GetMessage("MAIN_T_ADMIN_DEL"),
                "ACTION" => "if(confirm('" . GetMessage('MAIN_T_ADMIN_DEL_CONF') . "')) " . $lAdmin->ActionDoGroup(
                        $u_ID,
                        "delete"
                    )
            );
...
$APPLICATION->SetTitle(
    ($isEditingMessageThemePage ? GetMessage("MAIN_T_ADMIN_TITLE_THEME") : GetMessage("MAIN_T_ADMIN_TITLE"))
);
