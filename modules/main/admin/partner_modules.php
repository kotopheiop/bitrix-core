                        ...
                        if (defined(str_replace(".", "_", $info->MODULE_ID) . "_DEMO")) {
                            ...
                            if ($info->IsInstalled()) {
                                if (CModule::IncludeModuleEx($info->MODULE_ID) != MODULE_DEMO_EXPIRED) {
                                    $arModules[$dir]["DEMO_DATE"] = ConvertTimeStamp(
                                        $GLOBALS["SiteExpireDate_" . str_replace(".", "_", $info->MODULE_ID)],
                                        "SHORT"
                                    );
                                } else {
...
if (LANGUAGE_ID == "ru") {
    ...
    $linkToBuyUpdate = "https://marketplace.1c-bitrix.ru" . "/tobasket.php?ID=#CODE#&lckey=" . md5(
...
if (!empty($arUpdateList["MODULE"])) {
    foreach ($arUpdateList["MODULE"] as $k => $v) {
        if (!array_key_exists($v["@"]["ID"], $arClientModules)) {
            ...
            $modules[$v["@"]["ID"]] = Array(
                "VERSION" => (isset($v["#"]["VERSION"]) ? $v["#"]["VERSION"][count(
                    $v["#"]["VERSION"]
                ) - 1]["@"]["ID"] : ""),
                "FREE_MODULE" => $v["@"]["FREE_MODULE"],
                "DATE_FROM" => $v["@"]["DATE_FROM"],
...
echo "<h2>" . GetMessage("MOD_SMP_AV_MOD") . "</h2><p>" . GetMessage("MOD_SMP_AV_MOD_TEXT1") . "<br />" . GetMessage(
        "MOD_SMP_AV_MOD_TEXT2"
    ) . "</p>";
...
while ($info = $rsData->Fetch()) {
    ...
    $name = "<b><a href=\"https://marketplace.1c-bitrix.ru/" . htmlspecialcharsbx(
            $info["MODULE_ID"]
        ) . "\" target=\"_blank\">" . htmlspecialcharsbx($info["MODULE_NAME"]) . "</a></b> (" . htmlspecialcharsbx(
            $info["MODULE_ID"]
        ) . ")";
    if ($info["DEMO"] == "Y") {
    ...
    $row->AddViewField(
        "PARTNER",
        (($info["MODULE_PARTNER"] <> '') ? " " . str_replace(
                array("#NAME#", "#URI#"),
        ...
        if ($info["DEMO"] == "Y") {
            if ($linkToBuy) {
                if ($info["DEMO_END"] == "Y") {
                    $row->AddViewField(
                        "DATE_TO",
                        "<span class=\"required\">" . GetMessage(
                            "MOD_DEMO_END"
                        ) . "</span><br /><a href=\"" . str_replace(
                            "#CODE#",
                            $info["MODULE_ID"],
                            $linkToBuy
                        ) . "\" target=\"_blank\">" . GetMessage("MOD_UPDATE_BUY_DEMO") . "</a>"
                    );
                } else {
                    $row->AddViewField(
                        "DATE_TO",
                        $info["DEMO_DATE"] . "<br /><a href=\"" . str_replace(
                            "#CODE#",
                            $info["MODULE_ID"],
                            $linkToBuy
                        ) . "\" target=\"_blank\">" . GetMessage("MOD_UPDATE_BUY_DEMO") . "</a>"
            ...
            if ($modules[$info["MODULE_ID"]]["UPDATE_END"] == "Y") {
                if ($linkToBuy && !empty($modules[$info["MODULE_ID"]]["VERSION"])) {
                    $row->AddViewField(
                        "DATE_TO",
                        "<span style=\"color:red;\">" . $modules[$info["MODULE_ID"]]["DATE_TO"] . "</span><br /><a href=\"" . str_replace(
                            "#CODE#",
                            $info["MODULE_ID"],
                            $linkToBuyUpdate
                        ) . "\" target=\"_blank\">" . GetMessage("MOD_UPDATE_BUY") . "</a>"
                    );
                } else {
                    $row->AddViewField(
                        "DATE_TO",
                        "<span style=\"color:red;\">" . $modules[$info["MODULE_ID"]]["DATE_TO"] . "</span>"
                    );
                }
            } else {
    ...
    if (!empty($modules[$info["MODULE_ID"]]["VERSION"])) {
        $status .= "<br /><a href=\"/bitrix/admin/update_system_partner.php?tabControl_active_tab=tab2&addmodule=" . $info["MODULE_ID"] . "\" style=\"color:green;\">" . GetMessage(
                "MOD_SMP_NEW_UPDATES"
            ) . "</a>";
    }
    ...
    if (!empty($modules[$info["MODULE_ID"]]) && !empty($modules[$info["MODULE_ID"]]["VERSION"])) {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect(
                "/bitrix/admin/update_system_partner.php?tabControl_active_tab=tab2&addmodule=" . $info["MODULE_ID"]
            ),
        );
    }
    ...
    if ($info["IsInstalled"]) {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect(
                $APPLICATION->GetCurPage() . "?id=" . htmlspecialcharsbx(
                    $info["MODULE_ID"]
                ) . "&lang=" . LANG . "&uninstall=Y&" . bitrix_sessid_get()
            ),
        );
    } else {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect(
                $APPLICATION->GetCurPage() . "?id=" . htmlspecialcharsbx(
                    $info["MODULE_ID"]
                ) . "&lang=" . LANG . "&install=Y&" . bitrix_sessid_get()
            ),
        );
        $arActions[] = array(
            "ICON" => "delete",
            ...
            "ACTION" => "if(confirm('" . GetMessageJS(
                    'MOD_CLEAR_CONFIRM',
                    Array("#NAME#" => htmlspecialcharsbx($info["MODULE_NAME"]))
                ) . "')) " . $lAdmin->ActionRedirect(
                    $APPLICATION->GetCurPage() . "?id=" . htmlspecialcharsbx(
                        $info["MODULE_ID"]
                    ) . "&lang=" . LANG . "&clear=Y&" . bitrix_sessid_get()
                ),
        );
    }
...
echo "<h2>" . GetMessage("MOD_SMP_BUY_MOD") . "</h2><p>" . GetMessage("MOD_SMP_BUY_MOD_TEXT1") . "<br />" . GetMessage(
        "MOD_SMP_BUY_MOD_TEXT2"
    ) . "</p>";
...
while ($info = $rsData->Fetch()) {
    ...
    $row->AddViewField(
        "NAME",
        "<b><a href=\"http://marketplace.1c-bitrix.ru/" . htmlspecialcharsbx(
            $info["ID"]
        ) . "\" target=\"_blank\">" . htmlspecialcharsbx(
            $info["NAME"]
        ) . "</a></b> (" . htmlspecialcharsbx($info["ID"]) . ")<br />" . htmlspecialcharsbx(
            $info["DESCRIPTION"]
        )
    );
    ...
    if ($info["UPDATE_END"] == "Y") {
        if ($linkToBuy) {
            if ($info["DATE_TO"] <> '') {
                $row->AddViewField(
                    "DATE_TO",
                    "<span style=\"color:red;\">" . $info["DATE_TO"] . "</span><br /><a href=\"" . str_replace(
                        "#CODE#",
                        $info["ID"],
                        $linkToBuyUpdate
                    ) . "\" target=\"_blank\">" . GetMessage("MOD_UPDATE_BUY") . "</a>"
...
if ($mod <> '' && $resultMod == "OK") {
    ...
    CAdminMessage::ShowNote(
        GetMessage("MOD_SMP_UNINSTALLED", Array("#MODULE_NAME#" => $arModules[$mod]["MODULE_NAME"]))
    );
...
if ($errorMessage <> '') {
    CAdminMessage::ShowMessage(
        Array("DETAILS" => $errorMessageFull, "TYPE" => "ERROR", "MESSAGE" => $errorMessage, "HTML" => true)
    );
}
if ($resultMod == "OPAD") {
...
if (!empty($_SESSION["MP_MOD_DELETED"]) && in_array(LANGUAGE_ID, array("ru", "ua"))) {
    ...
    <form action="" method="POST">
        ...
        <?= GetMessage(
            "MOD_SMP_DELETE_OPINION",
            array(
                "#ID#" => $_SESSION["MP_MOD_DELETED"]["ID"],
                "#NAME#" => htmlspecialcharsbx($_SESSION["MP_MOD_DELETED"]["NAME"])
            )
        ) ?>
        <p><?= GetMessage("MOD_SMP_DELETE_OPINION_REASON") ?></p>
        <input type="radio" name="reason" value="1" id="r1">&nbsp;<label for="r1"><?= GetMessage(
                "MOD_SMP_DELETE_OPINION_REASON1"
            ) ?></label><br/>
        <input type="radio" name="reason" value="2" id="r2">&nbsp;<label for="r2"><?= GetMessage(
                "MOD_SMP_DELETE_OPINION_REASON2"
            ) ?></label><br/>
        <input type="radio" name="reason" value="3" id="r3">&nbsp;<label for="r3"><?= GetMessage(
                "MOD_SMP_DELETE_OPINION_REASON3"
            ) ?></label><br/>
        <input type="radio" name="reason" value="4" id="r4">&nbsp;<label for="r4"><?= GetMessage(
                "MOD_SMP_DELETE_OPINION_REASON4"
            ) ?></label><br/>
        <input type="radio" name="reason" value="5" id="r5">&nbsp;<label for="r5"><?= GetMessage(
                "MOD_SMP_DELETE_OPINION_REASON5"
            ) ?></label><br/>
        <input type="radio" name="reason" value="6" id="r6" checked>&nbsp;<label for="r6"><?= GetMessage(
                "MOD_SMP_DELETE_OPINION_REASON6"
            ) ?></label><br/><br/>
