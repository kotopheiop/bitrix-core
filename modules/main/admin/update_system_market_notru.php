...
if ($_REQUEST["action"] == "load" && $_REQUEST["id"] <> '' && check_bitrix_sessid()) {
    ...
    if (!CUpdateClientPartner::LoadModuleNoDemand($_REQUEST["id"], $errorMessage, "Y", false)) {
        ...
        LocalRedirect(
            "/bitrix/admin/module_admin.php?lang=" . LANG . "&id=" . urlencode(
                $_REQUEST["id"]
            ) . "&" . bitrix_sessid_get() . "&install=Y"
        );
    }
}
...
$arModules = CUpdateClientPartner::SearchModulesEx(
    array($by => $order),
    $arFilter,
    (intval($_REQUEST["PAGEN_1"]) > 0 ? intval($_REQUEST["PAGEN_1"]) : 1),
    LANG,
    $errorMessage
);
if ($errorMessage <> '') {
...
$lAdmin->AddHeaders(
    array(
        ...
        array(
            "id" => "DATE_UPDATE",
            "content" => GetMessage("USMP_H_DATE_UPDATE"),
            "sort" => "DATE_UPDATE",
            "default" => true
        ),
        array(
            "id" => "DATE_CREATE",
            "content" => GetMessage("USMP_H_DATE_CREATE"),
            "sort" => "DATE_CREATE",
            "default" => false
        ),
...
while ($arResultItem = $dbResultList->Fetch()) {
    ...
    $row->AddField(
        "ID",
        '<a href="update_system_market_detail.php?id=' . $arResultItem["ID"] . '&' . GetFilterParams(
            "filter_"
        ) . '" title="' . GetMessage("USMP_VIEW") . '">' . $arResultItem["ID"] . '</a>'
    );
    ...
    $row->AddField(
        "LOADED",
        array_key_exists($arResultItem["ID"], $arCurrentModules) ? GetMessage("USMP_YES") : GetMessage(
            "USMP_NO"
        )
    );
    ...
    $arActions[] = array(
        ...
        "ACTION" => $lAdmin->ActionRedirect(
            'update_system_market_detail.php?id=' . $arResultItem["ID"] . '&' . GetFilterParams("filter_") . ''
        ),
        "DEFAULT" => true
    );
    if (!array_key_exists($arResultItem["ID"], $arCurrentModules)) {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect(
                'update_system_market.php?id=' . $arResultItem["ID"] . '&action=load&' . bitrix_sessid_get()
            ),
            "DEFAULT" => false
        );
    }
