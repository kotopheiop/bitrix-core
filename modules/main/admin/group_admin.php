            ...
            case "delete":
                if ($ID > 2) {
                    ...
                    $lAdmin->AddGroupError(
                        GetMessage("MAIN_ERROR_GROUP") . $ID . GetMessage("MAIN_ERROR_GROUP_DELETE")
                    );
                }
                break;
            case "activate":
...
$lAdmin->AddHeaders(
    array(
        ...
        array(
            "id" => "C_SORT",
            "content" => GetMessage("MAIN_C_SORT"),
            "sort" => "c_sort",
            "default" => true,
            "align" => "right"
        ),
        ...
        array(
            "id" => "DESCRIPTION",
            "content" => GetMessage("MAIN_DESCRIPTION"),
            "sort" => "description",
            "default" => false
        ),
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    $row =& $lAdmin->AddRow(
        $f_ID,
        $arRes,
        "group_edit.php?lang=" . LANGUAGE_ID . "&ID=" . $f_ID,
        GetMessage("MAIN_EDIT_TITLE")
    );
    $row->AddViewField(
        "ID",
        "<a href='group_edit.php?lang=" . LANGUAGE_ID . "&ID=" . $f_ID . "' title='" . GetMessage(
            "MAIN_EDIT_TITLE"
        ) . "'>" . $f_ID . "</a>"
    );
    ...
    if ($f_ID != 2) {
        $row->AddViewField(
            "USERS",
            "<a href='user_admin.php?lang=" . LANGUAGE_ID . "&GROUPS_ID[]=" . $f_ID . "&apply_filter=Y' title='" . GetMessage(
                "USERS_OF_GROUP"
            ) . "'>" . $f_USERS . "</a>"
        );
    }
    ...
    if (intval($f_ID) > 2 && $USER->CanDoOperation('edit_groups')) {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect("group_edit.php?ID=" . $f_ID)
        );
        $arActions[] = array(
            "ICON" => "copy",
            "TEXT" => GetMessage("MAIN_ADMIN_MENU_COPY"),
            "ACTION" => $lAdmin->ActionRedirect("group_edit.php?COPY_ID=" . $f_ID)
        );
        $arActions[] = array("SEPARATOR" => true);
        $arActions[] = array(
            "ICON" => "delete",
            "TEXT" => GetMessage("MAIN_ADMIN_MENU_DELETE"),
            "ACTION" => "if(confirm('" . GetMessage('CONFIRM_DEL_GROUP') . "')) " . $lAdmin->ActionDoGroup(
                    $f_ID,
                    "delete"
                )
        );
...
?>
    <form name="find_form" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?">
        ...
        <tr>
            ...
            <td nowrap><input type="text" name="find_name" value="<? echo htmlspecialcharsbx($find_name) ?>"
                              size="47"><?= ShowFilterLogicHelp() ?></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><input type="text" name="find_users_1" size="10" value="<? echo htmlspecialcharsbx($find_users_1) ?>"
                       placeholder="<? echo GetMessage("group_admin_flt_from") ?>">&nbsp;<input type="text"
                                                                                                name="find_users_2"
                                                                                                size="10"
                                                                                                value="<? echo htmlspecialcharsbx(
                                                                                                    $find_users_2
                                                                                                ) ?>"
                                                                                                placeholder="<? echo GetMessage(
                                                                                                    "group_admin_flt_to"
                                                                                                ) ?>"></td>
        </tr>
        <?
