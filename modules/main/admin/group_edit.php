...
<form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?" name="form1">
    ...
    if ($ID > 0 && $ID != 2 && $COPY_ID <= 0) {
        ...
        if ($arGroupTmp = $dbGroupTmp->Fetch()) {
            ...
            <tr>
                <td><?
                    echo GetMessage('MAIN_TOTAL_USERS') ?></td>
                <td><a href="user_admin.php?lang=<?= LANG ?>&GROUPS_ID[]=<?= $ID ?>&apply_filter=Y"
                       title="<?= GetMessage("MAIN_VIEW_USER_GROUPS") ?>"><?= intval($arGroupTmp["USERS"]) ?></a></td>
            </tr>
            <?
        }
    }
    ...
    <? if ($USER_COUNT <= $USER_COUNT_MAX && $ID != 2): ?>
        ...
        <tr>
            <td colspan="2" align="center">
                ...
                <input type="hidden" name="USER_ID_NUMBER" value="<?= $ind ?>"></td>
        </tr>
    <? endif ?>
    <? $tabControl->BeginNextTab(); ?>
    <script>
        ...
        <tr valign="top">
            ...
            <td>
                ...
                <input type="checkbox" name="gp_<?= $key ?>_parent" OnClick="gpChangeParent('<?= $key ?>'); gpSync();"
                       id="id_gp_<?= $key ?>_parent" value="Y"<?
                if ($curValParent) echo "checked"; ?>><label
                ...
                switch ($arControl[0]) {
                    case "checkbox":
                        ...
                        <input type="checkbox" onclick="gpSync();" id="gp_<?= $key ?>" name="gp_<?= $key ?>"
                               value="<?= htmlspecialcharsbx($arControl[1]) ?>" <?
                        if ($curVal === $arControl[1]) echo "checked" ?> <?
                        if ($curValParent) echo "disabled"; ?>>
                        <?
                        break;
                    default:
                        ?>
                        <input type="text" onchange="gpSync();" name="gp_<?= $key ?>"
                               value="<?= htmlspecialcharsbx($curVal) ?>" size="<?
                        echo($arControl[1] > 0 ? $arControl[1] : "30") ?>" <?
                        if ($curValParent) echo "disabled"; ?>>
                    <?
                }
                ?>
            ...
            <td width="50%">
                <select id="subordinate_groups" name="subordinate_groups[]" multiple size="6">
                    ...
                    while ($arRes = $rsData->Fetch()) {
                        ...
                        ?>
                        <option value="<?= $arRes['ID'] ?>"<?
                        echo($bSel ? ' selected' : '') ?>><? echo htmlspecialcharsbx($arRes['NAME']) . ' [' . $arRes['ID'] . ']' ?></option><?
                    }
                    ?>
                </select>
                <script>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            if (isset($arTasksModules[$MID])) {
                                ...
                                <tr><?
                                    ...
                                    if (
                                        ...
                                        $use_padding = true;
                                        ?>
                                        <td style="padding: 3px;"><input type="hidden" name="SITES_<?= $moduleName ?>[]"
                                                                         value=""><?
                                    ...
                                    ?>
                                    <td <?
                                        if ($use_padding): ?>style="padding: 3px;"<?
                                    endif;
                                    ?>><?
                            ...
                            if (
                                ...
                                ?><a href="javascript:void(0)" onclick="settingsAddRights(this)"
                                     class="bx-action-href"><?
                                echo GetMessage("RIGHTS_ADD") ?></a><?
                            }
                            ?></td>
                    </tr>
                <?
