...
<form name="fd1" action="<? echo $APPLICATION->GetCurPage() ?>?lang=<?= LANGUAGE_ID ?>" method="GET">
    ...
    if ($bBitrixCloud) {
        ?>
        <tr>
            <td class="adm-detail-valign-top" width="40%"><?= GetMessage('DUMP_MAIN_BITRIX_CLOUD_DESC') ?><span
    ...
    if (count($arSitePath) > 1) {
        ?>
        <tr>
            <td class="adm-detail-valign-top"><?= GetMessage("DUMP_MAIN_SITE") ?><span
    ...
    ?>
    <tr>
        <td colspan=2 align=center><input type="checkbox" name="dump_expert" onclick="CheckExpert()" id="dump_expert">
            <label for="dump_expert"><?= GetMessage("DUMP_MAIN_ENABLE_EXPERT") ?></label></td>
    </tr>
    ...
    $editTab->Buttons();
    ?>
    <input type="button" id="start_button" class="adm-btn-save" value="<?= GetMessage("MAIN_DUMP_FILE_DUMP_BUTTON") ?>"
           OnClick="StartDump();">
    <input type="button" id="stop_button" value="<?= GetMessage("MAIN_DUMP_FILE_STOP_BUTTON") ?>"
           OnClick="EndDump(true)" disabled>
    <?
    $editTab->End();
    ?>
</form>
