...
if ($queryType == "M") {
    ...
    $loadResult = CUpdateClient::LoadModulesUpdates(
        $errorMessage,
        $arUpdateDescription,
        LANG,
        $stableVersionsOnly,
    ...
    if ($errorMessage <> '') {
        ...
        for ($i = 0, $cnt = count($arUpdateDescription["DATA"]["#"]["ITEM"]); $i < $cnt; $i++) {
            ...
            if ($arUpdateDescription["DATA"]["#"]["ITEM"][$i]["@"]["DESCR"] <> '') {
                $strModuleDescr = "<br>" . htmlspecialcharsback(
                        $arUpdateDescription["DATA"]["#"]["ITEM"][$i]["@"]["DESCR"]
                    );
            ...
            CUpdateClient::AddMessage2Log(
                "Updated: " . $arUpdateDescription["DATA"]["#"]["ITEM"][$i]["@"]["NAME"] . (($arUpdateDescription["DATA"]["#"]["ITEM"][$i]["@"]["VALUE"] != "0") ? " (" . $arUpdateDescription["DATA"]["#"]["ITEM"][$i]["@"]["VALUE"] . ")" : "") . $strModuleDescr,
                "UPD_SUCCESS"
            );
    ...
    $loadResult = CUpdateClient::LoadLangsUpdates(
        $errorMessage,
        $arUpdateDescription,
        LANG,
        $stableVersionsOnly,
                    ...
                    for ($i = 0, $cnt = count($arStepUpdateInfo["DATA"]["#"]["ITEM"]); $i < $cnt; $i++) {
                        if (isset($arStepUpdateInfo["DATA"]["#"]["ITEM"][$i]["@"]["ID"])
                            ...
                            if (class_exists('\Bitrix\Main\Localization\CultureTable')) {
                                $culture = \Bitrix\Main\Localization\CultureTable::getRow(
                                    array(
                                        'filter' => array(
                                            ...
                                            "=CHARSET" => (defined(
                                                'BX_UTF'
                                            ) ? "utf-8" : $arStepUpdateInfo["DATA"]["#"]["ITEM"][$i]["@"]["ENC"]),
                                ...
                                if ($culture) {
                                    ...
                                    $addResult = \Bitrix\Main\Localization\CultureTable::add(
                                        array(
