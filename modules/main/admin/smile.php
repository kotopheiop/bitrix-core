...
while ($arForum = $dbResultList->NavNext(true, "f_")) {
    ...
    $row->AddField(
        "TYPE",
        ($f_TYPE == CSmile::TYPE_ICON ? GetMessage("SMILE_TYPE_ICON") : GetMessage("SMILE_TYPE_SMILE"))
    );
    ...
    $row->AddViewField(
        ...
        '<a title="' . GetMessage(
            "SMILE_EDIT_DESCR"
        ) . '" href="' . "smile_set_edit.php?ID=" . $f_SET_ID . "&lang=" . LANG . "&" . GetFilterParams(
            "filter_"
        ) . '">' . ($f_SET_NAME <> '' ? $f_SET_NAME : GetMessage(
            'SMILE_SET_NO_NAME',
            Array('#ID#' => $f_SET_ID)
        )) . '</a>'
    );
    $row->AddViewField(
        ...
        '<a title="' . GetMessage(
            "SMILE_EDIT_DESCR"
        ) . '" href="' . "smile_edit.php?ID=" . $f_ID . "&lang=" . LANG . "&" . GetFilterParams(
            "filter_"
        ) . '">' . ($f_NAME <> '' ? $f_NAME : GetMessage('SMILE_NO_NAME')) . '</a>'
    );

    $row->AddField("TYPING", $f_TYPING);
    $row->AddField(
        "ICON",
        "<img src=\"" . ($f_TYPE == CSmile::TYPE_ICON ? CSmile::PATH_TO_ICON : CSmile::PATH_TO_SMILE) . $f_SET_ID . "/" . $f_IMAGE . "\" border=\"0\" " . ((intval(
                $f_IMAGE_WIDTH
            ) > 0) ? "width=\"" . $f_IMAGE_WIDTH . "\"" : "") . " " . ((intval(
                $f_IMAGE_HEIGHT
            ) > 0) ? "height=\"" . $f_IMAGE_HEIGHT . "\"" : "") . ">"
    );
    $row->AddField(
        "IMAGE",
        ($f_TYPE == CSmile::TYPE_ICON ? CSmile::PATH_TO_ICON : CSmile::PATH_TO_SMILE) . $f_SET_ID . "/" . $f_IMAGE
    );
    ...
    $arActions = Array(
        array(
            "ICON" => "edit",
            "TEXT" => GetMessage("SMILE_EDIT_DESCR"),
            "ACTION" => $lAdmin->ActionRedirect(
                "smile_edit.php?ID=" . $f_ID . "&lang=" . LANG . "&" . GetFilterParams("filter_") . ""
            ),
            "DEFAULT" => true
        ),
        ...
        array(
            "ICON" => "delete",
            "TEXT" => GetMessage("SMILE_DELETE_DESCR"),
            "ACTION" => "if(confirm('" . GetMessage('SMILE_DEL_CONF') . "')) " . $lAdmin->ActionDoGroup(
                    $f_ID,
                    "delete",
                    "SET_ID=" . $arFilter['SET_ID']
                )
        )
    );
...
if (in_array($arSmileSet["STRING_ID"], Array('bitrix_main'))) {
    CAdminMessage::ShowMessage(
        array(
            ...
            "DETAILS" => GetMessage(
                "SMILE_BITRIX_SET_WARNING_DESC",
                Array(
                    '#LINK_START#' => '<a href="smile_set_edit.php?lang=' . LANG . '&GALLERY_ID=' . $arSmileSet['PARENT_ID'] . '">',
                    '#LINK_END#' => '</a>'
                )
            ),
            "HTML" => true,
