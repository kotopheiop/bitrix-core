...
<form method="POST" action="<?= $APPLICATION->GetCurPage() ?>" name="form1">
    ...
    foreach ($arParams["LANGUAGE"] as $idLang => $arLang):
        ...
        <tr class="heading">
            <td colspan="2">
                ...
                <input type="checkbox" id="box_<?= $idLang ?>"
                       name="<?= $idLang ?>" <?= ($arParams["DATA"][$idLang]["ID"] <> '' && $_REQUEST[$idLang] <> "N" || $_REQUEST[$idLang] == "Y" || $arParams["EVENT_NAME"] == '' ? " checked" : "") ?>
                       value="Y">
        ...
        <tr>
            ...
            <td>
                ...
                <input type="hidden" name="FIELDS[<?= $arLang["ID"] ?>][ID]"
                       value="<?= htmlspecialcharsbx($arParams["DATA"][$arLang["ID"]]["ID"]) ?>">
            </td>
        </tr>
    <? endif; ?>
        <tr>
        ...
        <tr>
            ...
            <td>
                <input type="hidden" name="FIELDS_OLD[<?= $arLang["ID"] ?>][NAME]"
                       value="<?= htmlspecialcharsbx($arParams["DATA_OLD"][$arLang["ID"]]["NAME"]) ?>">
                <input type="text" name="FIELDS[<?= $arLang["ID"] ?>][NAME]"
                       value="<?= htmlspecialcharsbx($arParams["DATA"][$arLang["ID"]]["NAME"]) ?>" style="width:100%;">
            </td>
        </tr>
        <tr>
    ...
    if ($arParams["ACTION"] == "UPDATE" && $arParams["DATA"]["EVENT_TYPE"] == EventTypeTable::TYPE_EMAIL):
        ...
        <tr>
            <td colspan="2">
                <a href="message_edit.php?lang=<?= LANGUAGE_ID ?>&amp;EVENT_NAME=<?= urlencode($arParams["EVENT_NAME"]) ?>"><?
