    ...
    foreach ($FIELDS as $ID => $arFields) {
        ...
        if (!CBXShortUri::Update($ID, $arFields)) {
            $lAdmin->AddUpdateError(
                GetMessage("SU_AF_SAVE_ERROR") . $ID . ": " . implode("\n ", CBXShortUri::GetErrors()),
                $ID
            );
            $DB->Rollback();
        }
...
?>
    ...
    <form name="find_form" method="get" action="<? echo $APPLICATION->GetCurPage(); ?>">
        ...
        <tr>
            ...
            <td>
                <input type="text" size="47" name="find_uri"
                       value="<? echo htmlspecialcharsbx($find_uri) ?>">&nbsp;<?= ShowFilterLogicHelp() ?>
            </td>
        </tr>
        <tr>
            <td><? echo GetMessage("SU_AF_F_SHORT_URI") ?>:</td>
            <td><input type="text" name="find_short_uri" size="47"
                       value="<? echo htmlspecialcharsbx($find_short_uri) ?>"></td>
        </tr>
        <tr>
            <td><? echo GetMessage("SU_AF_F_MODIFIED") ?>:</td>
            <td><? echo CalendarPeriod(
                    "find_modified_1",
                    htmlspecialcharsbx($find_modified_1),
                    "find_modified_2",
                    htmlspecialcharsbx($find_modified_2),
                    "find_form",
                    "Y"
                ) ?></td>
        </tr>
        <tr>
            ...
            <td><? echo CalendarPeriod(
                    "find_last_used_1",
                    htmlspecialcharsbx($find_last_used_1),
                    "find_last_used_2",
                    htmlspecialcharsbx($find_last_used_2),
                    "find_form",
                    "Y"
                ) ?></td>
        </tr>
