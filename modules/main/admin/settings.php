...
?>
    ...
    <form action="">
        <select name="mid"
                onchange="window.location='settings.php?lang=<?= LANGUAGE_ID . ($_REQUEST["mid_menu"] <> "" ? "&amp;mid_menu=1" : "") ?>&amp;mid='+this[this.selectedIndex].value;">
            <? foreach ($arModules as $k => $m): ?>
                <option value="<? echo htmlspecialcharsbx(
                    $k
                ) ?>"<? if ($mid == $k) echo " selected" ?>><? echo htmlspecialcharsbx($m["NAME"]) ?></option>
            <?endforeach; ?>
        </select>
    </form>
    <br/>
...
function renderInput($Option, $arControllerOption, $fieldName, $val)
{
    ...
    if ($type[0] == "checkbox"):
        ...
        echo ' disabled title="' . GetMessage("MAIN_ADMIN_SET_CONTROLLER_ALT") . '"';
                 ...
                 value="Y"<? if ($val == "Y") {
        echo " checked";
    } ?><?= $disabled ?><? if ($type[2] <> '') echo " " . $type[2] ?>><?
    elseif ($type[0] == "text" || $type[0] == "password"):
        ...
        echo ' disabled title="' . GetMessage("MAIN_ADMIN_SET_CONTROLLER_ALT") . '"';
         ...
         value="<? echo htmlspecialcharsbx($val) ?>" name="<?= htmlspecialcharsbx(
        $fieldName
    ) ?>"<?= $disabled ?><?= ($type[0] == "password" || $type["noautocomplete"] ? ' autocomplete="new-password"' : '') ?>><?
    elseif ($type[0] == "selectbox"):
        $arr = $type[1];
        if (!is_array($arr)) {
            $arr = array();
        }
        ?><select name="<?= htmlspecialcharsbx($fieldName) ?>" <? if (isset($arControllerOption[$Option[0]])) {
        echo ' disabled title="' . GetMessage("MAIN_ADMIN_SET_CONTROLLER_ALT") . '"';
    } ?> <?= $disabled ?>><?
    ...
    elseif ($type[0] == "multiselectbox"):
        ...
        foreach ($arr as $key => $v):
            ?>
            <option value="<? echo $key ?>"<? if (in_array(
            $key,
            $arr_val
    ...
    elseif ($type[0] == "textarea"):
        ...
        echo ' disabled title="' . GetMessage("MAIN_ADMIN_SET_CONTROLLER_ALT") . '"';
    } ?> rows="<? echo $type[1] ?>" cols="<? echo $type[2] ?>" name="<?= htmlspecialcharsbx(
        $fieldName
    ) ?>"<?= $disabled ?>><? echo htmlspecialcharsbx($val) ?></textarea><?
