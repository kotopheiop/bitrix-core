...
if (!$USER->CanDoOperation('edit_php')) {
            ...
            if (count($arTables) > 0) {
                ...
                if (!$bCheckOK) {
                    ...
                    } elseif ($op == "check") //otherwise step optimize
                    {
                        CAdminMessage::ShowMessage(
                            array(
                                "MESSAGE" => htmlspecialcharsbx($arTable["Name"]) . " - " . GetMessage(
                                        "RDB_OPTIMIZE_OPTIMIZE"
                                    ),
                                "TYPE" => "PROGRESS",
                                "PROGRESS_VALUE" => $tables_count - count($arTables),
                                "PROGRESS_TOTAL" => $tables_count,
    ...
    if ($DB->type == "MYSQL") {
        if ($_REQUEST["check_tables"] == "Y" && check_bitrix_sessid()) {
            ...
            <table cellspacing="0" cellpadding="0" border="0" class="list-table" width="0%">
                ...
                while ($arResult = $result->Fetch()) {
                    ...
                    if (
                        ...
                        || (isset($arResult["Type"]) && (mb_strtoupper($arResult["Type"]) == "MYISAM" || mb_strtoupper(
                                    $arResult["Type"]
                                ) == "INNODB"))
                        || (isset($arResult["Engine"]) && (mb_strtoupper(
                                    $arResult["Engine"]
                                ) == "MYISAM" || mb_strtoupper($arResult["Engine"]) == "INNODB"))
                    ) {
                        echo "<td>";
                ...
                ?>
                <tr class="head">
                    <td colspan="5"><? echo "<b>" . GetMessage("RDB_EXEC_TIME") . " </b>" . round(
                                (microtime(true) - $start),
                                5
                            ) . GetMessage("RDB_SEC"); ?></td>
                </tr>
            </table>
            <?
            ...
            <input type="button" name="opt_start" id="opt_start" value="<? echo GetMessage("RDB_OPTIMIZE_BTN_START") ?>"
                   OnClick="Optimize('start|');">
            <input type="button" name="opt_pause" id="opt_pause" value="<? echo GetMessage("RDB_OPTIMIZE_BTN_PAUSE") ?>"
                   OnClick="Pause(true);" disabled>
            <input type="button" name="opt_continue" id="opt_continue"
                   value="<? echo GetMessage("RDB_OPTIMIZE_BTN_CONTINUE") ?>" OnClick="Pause(false);" disabled>
            <script>
                var pause = false;
                ...
                function Optimize(table_name) {
                    if (pause) {
                        ...
                        var url = 'repair_db.php?lang=<?echo LANGUAGE_ID?>&<?echo bitrix_sessid_get(
