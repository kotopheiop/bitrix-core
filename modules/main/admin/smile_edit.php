...
<form method="POST" action="<?= $APPLICATION->GetCurPageParam() ?>" name="smile_edit" enctype="multipart/form-data">
    ...
    <tr>
        <td valign="top"><?= GetMessage("SMILE_TYPING") ?>:<br><small><?= GetMessage("SMILE_TYPING_NOTE") ?></small>
        </td>
        <td valign="top">
    ...
    <? if (!empty($arSmile["IMAGE"])): ?>
        <tr>
            <td>
                <?= GetMessage("SMILE_IMAGE") ?>:
            </td>
            <td>
                <div style="margin-top: 5px">
                    <img src="<?= ($arSmile["TYPE"] == CSmile::TYPE_ICON ? CSmile::PATH_TO_ICON : CSmile::PATH_TO_SMILE) . $arSmile["SET_ID"] . "/" . $arSmile["IMAGE"] ?>"
