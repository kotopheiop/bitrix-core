    ...
    <div id="whole_form">
        ...
        <form name="form1" action="<? echo $APPLICATION->GetCurPage() ?>?lang=<?= LANG ?>" method="POST">
            ...
            for ($i = 1; $i <= $query_count - ($remove ? 1 : 0); $i++) {
                ...
                <tr valign="top">
                    <td width="100%" colspan="2">
                        <textarea cols="60" name="query<?
                        echo $i ?>" id="query<?
                        echo $i ?>" rows="15" wrap="OFF" style="width:100%;"><?
                            echo htmlspecialcharsbx($query); ?></textarea><br/>
                        <?
            ...
            <input<? if (!$isAdmin) echo " disabled" ?> type="button" accesskey="x" name="execute"
                                                        value="<? echo GetMessage("php_cmd_button") ?>"
                                                        onclick="return __FPHPSubmit();" class="adm-btn-save">
            <input type="button" value="<? echo GetMessage("php_cmd_button_clear") ?>"
                   onclick="this.form.reset(); __FPHPClear();">
