            ...
            foreach ($arrTemplateDir as $template_dir) {
                ...
                if ($handle) {
                    while (false !== ($file_name = readdir($handle))) {
                        if (is_file(
                                $_SERVER["DOCUMENT_ROOT"] . $path . $file_name
                            ) && $file_name != "." && $file_name != "..") {
                ...
                foreach ($arTemplate as $k => $val) {
                    ...
                    if ($bW) {
                        if (count($arSeparators) > 0) {
                            $arTemplateTemp[md5(uniqid(rand(), true))] = array(
                                "NAME" => "----------------------------",
                                "SEPARATOR" => "Y"
                            );
                        }
                        ...
                        if ($handle_mod = @opendir($_SERVER["DOCUMENT_ROOT"] . $path_mod)) {
                            while (($file_templ = readdir($handle_mod)) !== false) {
                                ...
                                if (is_dir($_SERVER["DOCUMENT_ROOT"] . $path_mod . "/" . $file_templ)) {
                                    ...
                                    $iSort = 500;
                                    if (file_exists(
                                        $_SERVER["DOCUMENT_ROOT"] . $path_mod . "/" . $file_templ . "/.description.php"
                                    )) {
                                        if (file_exists(
                                            ($fname = $_SERVER["DOCUMENT_ROOT"] . $path_mod . "/lang/" . LangSubst(
                                                    LANGUAGE_ID
                                                ) . "/" . $module_name . "/.description.php")
                                        )) {
                                            __IncludeLang($fname);
                                        }
                                        if (LANGUAGE_ID <> "ru" && file_exists(
                                                ($fname = $_SERVER["DOCUMENT_ROOT"] . $path_mod . "/lang/" . LANGUAGE_ID . "/" . $module_name . "/.description.php")
                                            )) {
                                            __IncludeLang($fname);
                                        }
...
function ParsePath($path, $bLast = false, $url = false, $param = "", $bLogical = false)
{
    ...
    $arPath = array();
    if ($bLast || $path <> '' || $site <> '') {
        $html_path = '<a href="' . $url . '?lang=' . LANG . '&' . $addUrl . '">' . GetMessage(
                "MAIN_ROOT_FOLDER"
            ) . '</a>/';
    } else {
    ...
    foreach ($arDirPath as $i => $pathPart) {
        ...
        if ($i == $pathLast && (!$bLast || !$io->DirectoryExists($DOC_ROOT . $full_path))) {
            ...
            $html_path .= "<a href=\"" . $url . "?lang=" . LANG . '&' . $addUrl . "&path=" . UrlEncode(
                    $full_path
                ) . ($site ? "&site=" . $site : "") . ($param <> "" ? "&" . $param : "") . "\">" . $sSectionName . "</a>/";
            if (!$arSite || !$bLogical || ($bLogical && rtrim($arSite["DIR"], "/") != rtrim($full_path, "/"))) {
                $arPath[] = array(
                    "LINK" => $url . "?lang=" . LANG . "&" . $addUrl . "&path=" . UrlEncode(
                            $full_path
                        ) . ($site ? "&site=" . $site : "") . ($param <> "" ? "&" . $param : ""),
                    "TITLE" => $sSectionName
                );
            }
        }
...
function CompareFiles($f1, $f2, $sort = array())
{
    ...
    if (mb_strtolower($order) == "desc") {
        if ($by == "size") {
            return intval($f1["SIZE"]) < intval($f2["SIZE"]);
        }
        if ($by == "timestamp") {
            return intval($f1["TIMESTAMP"]) < intval($f2["TIMESTAMP"]);
        }
...
function GetDirList(
    ...
    foreach ($arChildren as $child) {
        ...
        $arPerm = $APPLICATION->GetFileAccessPermission(
        ...
        if (mb_substr(
                $arFile["ABS_PATH"],
                0,
                mb_strlen(BX_ROOT . "/modules")
            ) == BX_ROOT . "/modules" && !$USER->CanDoOperation('edit_php') && !$task_mode) {
            continue;
        }
...
function SetPrologTitle($prolog, $title)
{
    if (preg_match(
        ...
        if ($p === false) {
            ...
            $prolog = mb_substr($prolog, 0, $p + 1) . ";\n\$APPLICATION->SetTitle(\"" . EscapePHPString(
                    $title
                ) . "\")" . mb_substr($prolog, $p + 1);
        }
    }
    return $prolog;
}
    ...
    if (preg_match(
        ...
        if ($property_val == '') {
            ...
            $prolog = str_replace(
                ...
                $regs[1] . $regs[2] . EscapePHPString($property_val, $regs[2]) . $regs[4] . $regs[5],
                $prolog
            );
        }
    } else {
        if ($property_val <> '') {
            ...
            if ($p !== false) {
                $p = mb_strpos(mb_substr($prolog, $p), ")") + $p;
                $prolog = mb_substr($prolog, 0, $p + 1) . ";\n\$APPLICATION->SetPageProperty(\"" . EscapePHPString(
                        $property_key
                    ) . "\", \"" . EscapePHPString($property_val) . "\")" . mb_substr($prolog, $p + 1);
            }
        }
    }
    return $prolog;
