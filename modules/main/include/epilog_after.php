...
if (\Bitrix\Main\ModuleManager::isModuleInstalled('translate')) {
    if (isset($_GET["show_lang_files"]) || isset(
            \Bitrix\Main\Application::getInstance()->getSession()["SHOW_LANG_FILES"]
        )) {
...
if (!defined('PUBLIC_AJAX_MODE') && (($_REQUEST["mode"] ?? '') != 'excel')) {
    ...
    $bShowStat = ($DB->ShowSqlStat && ($canEditPHP || \Bitrix\Main\Application::getInstance()->getKernelSession(
            )["SHOW_SQL_STAT"] == "Y"));
    $bShowCacheStat = (\Bitrix\Main\Data\Cache::getShowCacheStat(
        ) && ($canEditPHP || \Bitrix\Main\Application::getInstance()->getKernelSession()["SHOW_CACHE_STAT"] == "Y"));
