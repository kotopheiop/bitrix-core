...
function SelectBox($strBoxName, $a, $strDetText = "", $strSelectedVal = "", $field1 = "class=\"typeselect\"")
{
    ...
    while (($ar = $a->Fetch())) {
        ...
        $strReturnBox = $strReturnBox . "value=\"" . htmlspecialcharsbx($reference_id) . "\">" . htmlspecialcharsbx(
                $reference
            ) . "</option>";
    }
...
function SelectBoxMFromArray(
    ...
    foreach ($reference_id as $key => $value) {
        ...
        $strReturnBox .= "<option value=\"" . htmlspecialcharsbx($value) . "\" " . $sel . ">" . htmlspecialcharsbx(
                $reference[$key]
            ) . "</option>";
    }
...
function Calendar($sFieldName, $sFormName = "skform", $sFromName = "", $sToName = "")
{
    ...
    if (!$bCalendarCode) {
        ...
        $func =
            ...
            "	}\n" .
            "	window.open('/bitrix/tools/calendar.php?lang=" . LANGUAGE_ID . (defined(
                "ADMIN_SECTION"
            ) && ADMIN_SECTION === true ? "&admin_section=Y" : "&admin_section=N") . "&'+params+'&date='+escape(dateVal)+'&initdate='+escape(dateVal),'','scrollbars=no,resizable=yes,width='+width+',height='+height+',left='+left+',top='+top);\n" .
            "}\n" .
            "//-->\n" .
            "</script>\n";
    }
    return $func . "<a href=\"javascript:void(0);\" onclick=\"window.Calendar('name=" . urlencode(
            $sFieldName
        ) . "&amp;from=" . urlencode($sFromName) . "&amp;to=" . urlencode($sToName) . "&amp;form=" . urlencode(
            $sFormName
        ) . "', document['" . $sFormName . "']['" . $sFieldName . "'].value);\" title=\"" . GetMessage(
            "TOOLS_CALENDAR"
        ) . "\"><img src=\"" . BX_ROOT . "/images/icons/calendar.gif\" alt=\"" . GetMessage(
            "TOOLS_CALENDAR"
...
function CalendarDate($sFromName, $sFromVal, $sFormName = "skform", $size = "10", $param = "class=\"typeinput\"")
{
    ...
    return '<input type="text" name="' . $sFromName . '" id="' . $sFromName . '" size="' . $size . '" value="' . htmlspecialcharsbx(
            $sFromVal
...
function CalendarPeriod(
    ...
    if (defined("ADMIN_SECTION") && ADMIN_SECTION == true) {
        return CAdminCalendar::CalendarPeriod(
            $sFromName,
            $sToName,
            $sFromVal,
            $sToVal,
...
";
        ...
        <script type="text/javascript">
            ...
            if (!is_array($arr["reference_id"])) {
                $arr["reference_id"] = array();
            }
            $arr["reference"][] = $i . " " . GetMessage("TOOLS_DN");
            $arr["reference_id"][] = $i;
            }
            ?></script><?
        $str .= SelectBoxFromArray(
            $sname,
            $arr,
            $value,
            "&nbsp;",
            "onchange=\"" . $sFromName . "_SetDate()\" " . $field_select
        );
        $str .= "&nbsp;";
    }
    $str .=
        '<input ' . $ds . ' ' . $field_input . ' type="text" name="' . $sFromName . '" id="' . $sFromName . '" size="' . $size . '" value="' . htmlspecialcharsbx(
            $sFromVal
        ) . '" /> ' . "\n" .
        Calendar($sFromName, $sFormName, $sFromName, $sToName) . ' ... ' . "\n" .
        '<input ' . $field_input . ' type="text" name="' . $sToName . '" id="' . $sToName . '" size="' . $size . '" value="' . htmlspecialcharsbx(
            $sToVal
        ) . '" /> ' . "\n" .
...
function GetNumMonth($month)
{
    ...
    if ($month) {
        for ($i = 1; $i <= 12; $i++) {
            if (strcasecmp($MESS['MONTH_' . $i . '_S'], $month) === 0 || strcasecmp(
                    $MESS['MON_' . $i],
                    $month
                ) === 0 || strcasecmp($MESS['MONTH_' . $i], $month) === 0) {
                return $i;
            }
        }
    }
...
function ParseDateTime($datetime, $format = false)
{
    ...
    if (preg_match_all("/(DD|MI|MMMM|MM|M|YYYY|HH|H|SS|TT|T|GG|G)/i", $format, $fm_args)) {
        $dt_args = array();
        if (preg_match_all(
            "~([^:\\\\/\\s.,0-9-]+|[^:\\\\/\\s.,a-z-]+)~i" . BX_UTF_PCRE_MODIFIER,
            $datetime,
            $dt_args
        )) {
            $arrResult = array();
            foreach ($fm_args[0] as $i => $v) {
                if (isset($dt_args[0][$i])) {
                    ...
                    } elseif (($dt_args[0][$i] == "am" || $dt_args[0][$i] == "pm") && array_search(
                            "T",
                            $fm_args[0]
                        ) !== false) {
...
function convertTimeToMilitary($strTime, $fromFormat = 'H:MI T', $toFormat = 'HH:MI')
{
    ...
    $ts = mktime(
        $arParsedDate['HH'],
        $arParsedDate['MI'],
        (isset($arParsedDate['SS']) ? $arParsedDate['SS'] : 0),
        3,
        7,
        2012
    );
...
function FormatDate($format = "", $timestamp = false, $now = false)
{
    ...
    if ($timestamp === false) {
        ...
        if ($timestamp instanceof \Bitrix\Main\Type\Date) {
    ...
    if ($now === false) {
        ...
        if ($now instanceof \Bitrix\Main\Type\Date) {
...
function PrepareTxtForEmail($text, $lang = false, $convert_url_tag = true, $convert_image_tag = true)
{
    ...
    $text = preg_replace_callback(
        "#\\[code(\\s+[^\\]]*\\]|\\])(.+?)\\[/code(\\s+[^\\]]*\\]|\\])#is",
        array($helper, "convertCodeTagForEmail"),
        $text
    );
...
function convert_code_tag_for_html_after(
    $text = "",
    $code_table_class,
    $code_head_class,
    $code_body_class,
...
function convert_open_quote_tag($quote_table_class, $quote_head_class, $quote_body_class)
{
    global $QUOTE_OPENED;
    $QUOTE_OPENED++;
    return "<table class='$quote_table_class' width='95%' border='0' cellpadding='3' cellspacing='1'><tr><td class='" . $quote_head_class . "'>" . GetMessage(
            "MAIN_QUOTE"
...
function convert_to_href(
    ...
    $s = "<a class=\"" . $link_class . "\" href=\"" . delete_special_symbols(
            $url
        ) . "\"" . $target . ">" . $url . "</a>";
...
{
    $s = stripslashes($s);
    $s = "<a class=\"" . $link_class . "\" href=\"mailto:" . delete_special_symbols($s) . "\" title=\"" . GetMessage(
            "MAIN_MAILTO"
        ) . "\">" . $s . "</a>";
    return $s;
}
...
)
{
    ...
    if ($CODE_ENABLED == "Y") {
        ...
        $str = preg_replace_callback(
            "#\\[code(\\s+[^\\]]*\\]|\\])(.+?)\\[/code(\\s+[^\\]]*\\]|\\])#is",
            array($helper, "convertCodeTagForHtmlBefore"),
            $str
        );
    }
    ...
    if ($bMakeUrls) {
        ...
        $str = preg_replace_callback(
            ...
            array("CConvertorsPregReplaceHelper", "extractUrl"),
            $str
        );
        $str = preg_replace(
            "#(([=_\\.'0-9a-z+~\x01-\x08-]+)@[_0-9a-z\x01-\x08-.]+\\.[a-z]{2,10})#is",
            chr(3) . "\\1" . chr(3),
            $str
        );
        //replace back to @
    ...
    if ($CODE_ENABLED == "Y") {
        ...
        $str = preg_replace_callback(
            "#\\[code\\](.*?)\\[/code\\]#is",
            array($helper, "convertCodeTagForHtmlAfter"),
            $str
        );
...
function GetScriptFileExt()
{
    ...
    $script_files = COption::GetOptionString(
        "fileman",
        "~script_files",
...
function SendError($error)
{
    if (defined('ERROR_EMAIL') && ERROR_EMAIL <> '') {
        ...
        ) && ERROR_EMAIL_REPLY_TO <> '' ? ERROR_EMAIL_REPLY_TO : 'admin@bitrix.ru');
        bxmail(
            ERROR_EMAIL,
        ...
        if ($sText <> '') {
            ...
            if ($fp = @fopen(LOG_FILENAME, "ab")) {
                if (flock($fp, LOCK_EX)) {
                    ...
                    $arBacktrace = Bitrix\Main\Diag\Helper::getBackTrace(
                        $traceDepth,
                        ($bShowArgs ? null : DEBUG_BACKTRACE_IGNORE_ARGS)
                    );
                    $strFunctionStack = "";
                    $strFilesStack = "";
...
function FindUserID(
    ...
    if ($APPLICATION->GetGroupRight("main") >= "R") {
        $strReturn = "
<input type=\"text\" name=\"" . $tag_name . "\" id=\"" . $tag_name . "\" value=\"" . htmlspecialcharsbx(
                $tag_value
            ) . "\" size=\"" . $tag_size . "\" maxlength=\"" . $tag_maxlength . "\" class=\"" . $tag_class . "\">
	...
	if (!!DV_" . $tag_name_x . ")
	{
		if (tv" . $tag_name_x . "!=document." . $form_name . "['" . $tag_name . "'].value)
		{
			...
			if (tv" . $tag_name_x . "!='')
			{
				...
				BX(\"hiddenframe" . $tag_name . "\").src='get_user.php?ID=' + tv" . $tag_name_x . "+'&strName=" . $tag_name . "&lang=" . LANG . (defined(
                "ADMIN_SECTION"
...
function GetWhoisLink($ip, $class = '')
{
    ...
    return '<a href="' . $URL . '"' . ($class <> '' ? ' class="' . $class . '"' : '') . ' target="_blank" title="' . GetMessage(
            "WHOIS_SERVICE"
        ) . '">' . htmlspecialcharsbx($ip) . '</a>';
}
...
function minimumPHPVersion($vercheck)
{
    ...
    if ((intval($curver[0]) < intval($minver[0])) || ((intval($curver[0]) == intval($minver[0])) && (intval(
                    $curver[1]
                ) < intval($minver[1]))) || ((intval($curver[0]) == intval($minver[0])) && (intval(
                    $curver[1]
                ) == intval($minver[1])) && (intval($curver[2]) < intval($minver[2])))) {
        return false;
    } else {
        return true;
    }
...
function Help($module = "", $anchor = "", $help_file = "")
{
    ...
    echo "<a href=\"javascript:Help('" . urlencode(
            $help_file
        ) . "','" . $module . "','" . $anchor . "')\" title='" . GetMessage(
            "TOOLS_HELP"
        ) . "'><img src='" . BX_ROOT . "/images/main/show_help.gif' width='16' height='16' border='0' alt='" . GetMessage(
            "TOOLS_HELP"
...
function IncludeAJAX()
{
    ...
    $APPLICATION->AddHeadString(
        '<script type="text/javascript">var ajaxMessages = {wait:"' . CUtil::JSEscape(
            GetMessage('AJAX_WAIT')
        ) . '"}</script>',
        true
    );
...
class CUtil
{
        ...
        if ($bSkipNative) {
            ...
            if (mb_substr($data, 0, 1) == '{') // object
            {
                ...
                foreach ($arTokens as $token) {
                    ...
                    $arResult[CUtil::JsObjectToPhp($arTokenData[0], true)] = CUtil::JsObjectToPhp(
                        $arTokenData[1],
                        true
                    );
                }
            } elseif (mb_substr($data, 0, 1) == '[') // array
            {
                ...
                if (count($arCommaPos) > 0) {
                    $prev_index = 0;
                    foreach ($arCommaPos as $pos) {
                        $arResult[] = CUtil::JsObjectToPhp(
                            mb_substr($token, $prev_index, $pos - $prev_index - 1),
                            true
                        );
                        $prev_index = $pos;
                    }
    ...
    public static function ConvertToLangCharset($string)
    {
        ...
        if (defined("BX_UTF") && !$bUTF) {
            ...
            $fromCP = "UTF-8";
            $toCP = (defined("LANG_CHARSET") ? LANG_CHARSET : (defined(
                "BX_DEFAULT_CHARSET"
            ) ? BX_DEFAULT_CHARSET : "Windows-1251"));
        }
    ...
    public static function translit($str, $lang, $params = array())
    {
        ...
        if (!isset($search[$lang])) {
            $mess = IncludeModuleLangFile(
                $_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/main/js_core_translit.php",
                $lang,
                true
            );
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if (preg_match("/[a-zA-Z0-9]/" . BX_UTF_PCRE_MODIFIER, $chr) || mb_strpos(
                    $params["safe_chars"],
                    $chr
                ) !== false) {
                $chr_new = $chr;
...
class CHTTP
{
    ...
    public static function isPathTraversalUri($uri)
    {
        ...
        $uri = trim($uri);
        return preg_match(
            "#(?:/|2f|^|\\\\|5c)(?:(?:%0*(25)*2e)|\\.){2,}(?:/|%0*(25)*2f|\\\\|%0*(25)*5c|$)#i",
            $uri
        ) ? true : false;
    }
}
    ...
    if (mb_strlen($number) >= 10) {
        if (mb_substr($number, 0, 2) == '80' || mb_substr($number, 0, 2) == '81' || mb_substr($number, 0, 2) == '82') {
            ...
            if (mb_substr($number, 0, 2) == '00') {
                ...
                if (mb_substr($number, 0, 3) == '011') {
                    $number = mb_substr($number, 3);
                } else {
                    if (mb_substr($number, 0, 1) == '8') {
