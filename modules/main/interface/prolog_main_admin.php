                    ...
                    <div class="adm-menu-wrapper<?= $bOptMenuMinimized ? ' adm-main-menu-close' : '' ?>"
                        ...
                        <div class="adm-main-menu">
                            ...
                            foreach ($adminMenu->aGlobalMenu as $menu):
                                ...
                                if ($menu['url']):
                                    ?>
                                    <a href="<?= htmlspecialcharsbx($menu["url"]) ?>"
                                       class="adm-default <?= $menuClass ?>" onclick="BX.adminMenu.GlobalMenuClick('<?
                                    echo $menu["menu_id"] ?>'); return false;" onfocus="this.blur();" id="global_menu_<?
                                    echo $menu["menu_id"] ?>">
                                        <div class="adm-main-menu-item-icon"></div>
                                        <div class="adm-main-menu-item-text"><?
                                            echo htmlspecialcharsbx($menu["text"]) ?></div>
                                        <div class="adm-main-menu-hover"></div>
                                    </a>
                                <?
                                else:
                                    ?>
                                    <span class="adm-default <?= $menuClass ?>"
                                          onclick="BX.adminMenu.GlobalMenuClick('<?
                                          echo $menu["menu_id"] ?>'); return false;" id="global_menu_<?
                                    echo $menu["menu_id"] ?>">
							<div class="adm-main-menu-item-icon"></div>
							<div class="adm-main-menu-item-text"><?
                                echo htmlspecialcharsbx($menu["text"]) ?></div>
                                ...
                                <div class="adm-global-submenu<?= ($subMenuDisplay == "block" ? " adm-global-submenu-active" : "") ?>"
                                    ...
                                    <div class="adm-submenu-items-wrap">
                                        <div class="adm-submenu-items-stretch-wrap"
                                             onscroll="BX.adminMenu.itemsStretchScroll()">
        ...
        <td class="adm-workarea-wrap <?= defined('BX_ADMIN_SECTION_404') && BX_ADMIN_SECTION_404 == 'Y' ? 'adm-404-error' : 'adm-workarea-wrap-top' ?>">
            <div class="adm-workarea adm-workarea-page" id="adm-workarea">
				...
				<?= $currentFavId ? GetMessage("MAIN_PR_ADMIN_FAV_DEL") : GetMessage("MAIN_PR_ADMIN_FAV_ADD") ?>"></a>
                        <?endif; ?>
                        <a id="navchain-link" href="<?
                        echo htmlspecialcharsbx($_SERVER["REQUEST_URI"]) ?>" title="
			<?
                ...
                if ($USER->IsAuthorized()):
                    ...
                    if (isset($bxProductConfig["saas"])):
                        ...
                        ?>
                        <span class="required"><?
                            echo GetMessage("TRIAL_ATTENTION") ?></span>
                        <?
                        echo GetMessage("TRIAL_ATTENTION_TEXT1_" . $vendor) ?>
                        <?
                    if ($daysToExpire >= 0):?>
                        <?
                        echo GetMessage("TRIAL_ATTENTION_TEXT2") ?> <span class="required"><b><?
                            echo $daysToExpire ?></b></span> <?
                        echo GetMessage("TRIAL_ATTENTION_TEXT3") ?>.
                    <?
                    else:?>
                        <?
                    ...
                    ?>
                        <style>
                            #menu-popup-prolong-popup .popup-window-hr {
                                display: none;
                            }

                            #menu-popup-prolong-popup .menu-popup .menu-popup-item {
                                min-width: 100px;
                                margin-top: 7px;
                            }

                            #menu-popup-prolong-popup .menu-popup-item:hover {
                            ...
                            function showProlongMenu(bindElement) {
                                BX.PopupMenu.show("prolong-popup", bindElement, [
                                        ...
                                        },
                                        {
                                            ...
                                            onclick: function () {
                                                prolongRemind('<?=AddToTimeStamp(array("DD" => 7));?>', this)
                                            }
                                        },
                                        {
                                            text: '<?=GetMessageJS("prolog_main_support_menu3")?>',
                                            onclick: function () {
                                                prolongRemind('<?=AddToTimeStamp(array("DD" => 14));?>', this)
                                            }
                                        },
                                        {
                                            text: '<?=GetMessageJS("prolog_main_support_menu4")?>',
                                            onclick: function () {
                                                prolongRemind('<?=AddToTimeStamp(array("MM" => 1));?>', this)
                                            }
                                        }
                                    ],
                                    {
