...
<div style="overflow: auto; width:100%; height: 200px; background-color:#F8F9FC; border: 1px solid #E7EAF5"><?
    ...
    >
        ...
        foreach ($v as $vname => $vvalue):
            ...
            ?>
            <input type="hidden" name="<? echo htmlspecialcharsbx($vname) ?>"
                   value="<? echo htmlspecialcharsbx($vvalue) ?>">
        <? endforeach ?>
        <input type="text" size="100" class="typeinput" name="srchlngfil"
               value="<?= htmlspecialcharsbx($_REQUEST["srchlngfil"]) ?>">
