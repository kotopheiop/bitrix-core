    ...
    if (count($arUserOptions) > 0):
        ...
        <div class="adm-submenu-items-block"><?
            foreach ($arUserOptions as $DESKTOP_ID => $arUserOption):
                ...
                ?><a href="/bitrix/admin/?dt_page=<?= $DESKTOP_ID ?>"
                     class="adm-submenu-item<?= $desktop_className ? ' ' . $desktop_className : '' ?>">
                <div class="adm-submenu-item-icon"></div>
                <div class="adm-submenu-item-text"><?= $arUserOption["NAME"] <> '' ? htmlspecialcharsbx(
                        $arUserOption["NAME"]
                    ) : str_replace('#NUM#', $DESKTOP_ID + 1, GetMessage('DESKTOP_DEFAULT_NAME')); ?></div>
                </a><?
            endforeach;
            ?>
            <div class="adm-submenu-add-desktop" onclick="BX.adminPanel.addDesktop();">
                <span class="adm-submenu-add-desktop-icon"></span><span
                        class="adm-submenu-add-desktop-text"><?= GetMessage('DESKTOP_ADD') ?></span>
            </div>
        </div>
