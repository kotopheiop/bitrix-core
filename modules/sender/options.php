...
if ($POST_RIGHT >= "R") :
    ...
    $arAllOptions = array(
        ...
        array(
            "auto_method",
            GetMessage("opt_method"),
            array(
                "selectbox",
                array("agent" => GetMessage("opt_method_agent"), "cron" => GetMessage("opt_method_cron"))
            )
        ),
        ...
        array(
            "reiterate_method",
            GetMessage("opt_reiterate_method"),
            array(
                "selectbox",
                array("agent" => GetMessage("opt_method_agent"), "cron" => GetMessage("opt_method_cron"))
            )
        ...
        array(
            "thread_type",
            GetMessage("opt_threads_type"),
            array(
                "selectbox",
                array
                (
                    "Single" => GetMessage("opt_method_single_thread"),
                    "Ten" => GetMessage("opt_method_ten_threads")
                )
            )
        ),
        ...
        array(
            "link_protocol",
            GetMessage("opt_link_protocol"),
            array("selectbox", array("" => "http", "https" => "https"))
        ),
            ...
            <tr>
                ...
                <td width="60%">
                    ...
                    elseif ($type[0] == "selectbox"):
                        ...
                        $alertWarning = '';
                        if (in_array($Option[0], array('auto_method', 'reiterate_method')) && !CheckVersion(
                                SM_VERSION,
                                '15.0.9'
                            )) {
                            $alertWarning = 'onchange="if(this.value==\'cron\')alert(\'' . GetMessage(
                                    'opt_sender_cron_support'
                                ) . SM_VERSION . '.\');"';
                        }
                        ...
                        for ($j = 0; $j < $arr_keys_count; $j++):
                            ?>
                            <option value="<? echo $arr_keys[$j] ?>"<? if ($val == $arr_keys[$j]) echo " selected" ?>><? echo htmlspecialcharsbx(
                            $arr[$arr_keys[$j]]
                        ) ?></option><?
                        endfor;
                        ?></select><?
