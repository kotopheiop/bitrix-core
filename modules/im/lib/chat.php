...
class Chat
{
    ...
    public static function getRelation($chatId, $params = [])
    {
        ...
        if (!empty($skipUserTypes)) {
            ...
            if (count($skipUserTypes) === 1) {
                $skipUserTypesSql = "AND (U.EXTERNAL_AUTH_ID != '" . $connection->getSqlHelper()->forSql(
                        $skipUserTypes[0]
                    ) . "' OR U.EXTERNAL_AUTH_ID IS NULL)";
            } else {
                $skipUserTypes = array_map(
                ...
                $skipUserTypesSql = "AND (U.EXTERNAL_AUTH_ID NOT IN ('" . implode(
                        "','",
                        $skipUserTypes
                    ) . "') OR U.EXTERNAL_AUTH_ID IS NULL)";
            }
        ...
        if (isset($params['REAL_COUNTERS']) && $params['REAL_COUNTERS'] != 'N' || $skipUnmodifiedRecords) {
            if (is_array($params['REAL_COUNTERS']) && isset($params['REAL_COUNTERS']['LAST_ID'])) {
                $sqlSelectCounter = "R.COUNTER PREVIOUS_COUNTER, (
					SELECT COUNT(1) FROM b_im_message M WHERE M.CHAT_ID = R.CHAT_ID AND M.ID > " . intval(
                        $params['REAL_COUNTERS']['LAST_ID']
                    ) . "
				) COUNTER";
            } else {
                $customCounter = true;
        ...
        while ($row = $query->fetch()) {
            if ($customCounter) {
                if (isset($counters[$row['LAST_ID']])) {
                    ...
                    if ($row['LAST_ID'] < $customMinId) {
                        $row['COUNTER'] = count($counters);
                    } else {
                        if ($row['LAST_ID'] > $customMaxId) {
                            $row['COUNTER'] = 0;
                        }
                    }
                }
    ...
    public static function getMessageCount($chatId, $userId = null)
    {
        ...
        if (!$relationData || $relationData['START_ID'] == 0) {
            $counter = \Bitrix\Im\Model\MessageTable::getList(
                array(
    ...
    public static function getMessages($chatId, $userId = null, $options = Array())
    {
        ...
        while ($message = $orm->fetch()) {
            ...
            if ($message['AUTHOR_ID'] && !isset($users[$message['AUTHOR_ID']])) {
                ...
                $user['last_activity_date'] = $message['USER_LAST_ACTIVITY_DATE'] ? date(
                    'c',
                    $message['USER_LAST_ACTIVITY_DATE']->getTimestamp()
                ) : false;
                $user['desktop_last_date'] = $message['USER_DESKTOP_LAST_DATE'] ? date(
                    'c',
                    $message['USER_DESKTOP_LAST_DATE']->getTimestamp()
                ) : false;
                $user['mobile_last_date'] = $message['USER_MOBILE_LAST_DATE'] ? date(
                    'c',
                    $message['USER_MOBILE_LAST_DATE']->getTimestamp()
                ) : false;
        ...
        if ($startFromUnread) {
            ...
            $additionalMessages = self::getMessages(
                $chatId,
                $userId,
                [
                    'LIMIT' => $limit,
        ...
        if ($options['JSON']) {
            foreach ($result['MESSAGES'] as $key => $value) {
                ...
                if (isset($value['PARAMS']['CHAT_LAST_DATE']) && $value['PARAMS']['CHAT_LAST_DATE'] instanceof \Bitrix\Main\Type\DateTime) {
                    $result['MESSAGES'][$key]['PARAMS']['CHAT_LAST_DATE'] = date(
                        'c',
                        $value['PARAMS']['CHAT_LAST_DATE']->getTimestamp()
                    );
                }
        ...
        if (isset($params['FILTER']['ID'])) {
            ...
            if (isset($params['FILTER']['SEARCH'])) {
                ...
                if (Model\ChatIndexTable::getEntity()->fullTextIndexEnabled('SEARCH_CONTENT')) {
                    ...
                    if (\Bitrix\Main\Search\Content::canUseFulltextSearch(
                        $find,
