        ...
        if ($state === Call::STATE_INVITING && $prevState === Call::STATE_NEW) {
            ...
            if ($state === Call::STATE_FINISHED) {
                ...
                $userIds = array_values(
                    array_filter(
                        $this->call->getUsers(),
                        function ($userId) use ($initiatorId) {
                ...
                if (count($userIds) == 1) {
                    ...
                    if ($otherUserState == CallUser::STATE_DECLINED) {
                        $message = Loc::getMessage(
                            "IM_CALL_INTEGRATION_CHAT_CALL_USER_DECLINED_" . $otherUser->getGender(),
                            [
                                '#NAME#' => $otherUser->getFullName(false)
                            ]
                        );
                    } else {
                        if ($otherUserState == CallUser::STATE_BUSY) {
                            $message = Loc::getMessage(
                            ...
                            if ($otherUserState == CallUser::STATE_UNAVAILABLE || $otherUserState == CallUser::STATE_CALLING) {
                                $message = Loc::getMessage(
                                    "IM_CALL_INTEGRATION_CHAT_CALL_MISSED_" . $initiator->getGender(),
                                    [
