...
class TextareaIcon
{
    ...
    public static function register(array $fields)
    {
        ...
        if (isset($fields['IFRAME'])) {
            ...
            $iframeWidth = isset($fields['IFRAME_WIDTH']) && intval($fields['IFRAME_WIDTH']) > 250 ? intval(
                $fields['IFRAME_WIDTH']
            ) : $iframeWidth;
            $iframeHeight = isset($fields['IFRAME_HEIGHT']) && intval($fields['IFRAME_HEIGHT']) > 50 ? intval(
                $fields['IFRAME_HEIGHT']
            ) : $iframeHeight;
        } else {
            if (!$jscommand) {
                return false;
        ...
        foreach ($icons as $cmd) {
            if ($botId) {
                ...
                if ($appId) {
                    ...
                    if ($moduleId == $cmd['MODULE_ID'] && $icon == $cmd['ICON_CODE']) {
                        return $cmd['ID'];
                    }
                }
            }
        ...
        $result = \Bitrix\Im\Model\TextareaIconTable::add(
            Array(
                'HASH' => $hash,
                'BOT_ID' => $botId,
                'MODULE_ID' => $moduleId,
        ...
        if ($moduleId == 'rest') {
            foreach ($langSet as $lang) {
                ...
                try {
                    \Bitrix\Im\Model\TextareaIconLangTable::add(
                        array(
                            'ICON_ID' => $iconId,
            ...
            if ($loadRestLang) {
                ...
                foreach ($result as $iconId => $commandData) {
                    if (isset($langSet[$iconId][$lang])) {
                        ...
                        if (isset($langSet[$iconId][$langAlter])) {
