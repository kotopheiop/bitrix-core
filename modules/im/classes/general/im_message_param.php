...
class CIMMessageParam
{
                ...
                if (empty($v1)) {
                    ...
                    foreach ($v1 as $v2) {
                        if (is_array($v2)) {
                            ...
                            if (is_object(
                                    $v2
                                ) && ($v2 instanceof \Bitrix\Im\Bot\Keyboard || $v2 instanceof \Bitrix\Im\Bot\ContextMenu)) {
                                $value = $v2->getJson();
                                if ($value <> '') {
                                    $key = md5($name . $value);
                                    $arToInsert[$key] = array(
    ...
    public static function UpdateTimestamp($messageId, $chatId = 0)
    {
        ...
        $orm = IM\Model\MessageParamTable::getList(
            array(
                'select' => array('ID'),
        ...
        if ($tsParam = $orm->fetch()) {
            ...
            IM\Model\MessageParamTable::add(
                array('MESSAGE_ID' => $messageId, 'PARAM_NAME' => 'TS', 'PARAM_VALUE' => $timestamp)
            );
        }
    ...
    public static function SendPull($messageId, $sendExtraParams = true)
    {
        ...
        if ($messageData['MESSAGE_TYPE'] == IM_MESSAGE_OPEN || $messageData['MESSAGE_TYPE'] == IM_MESSAGE_OPEN_LINE) {
            CPullWatch::AddToStack(
                'IM_PUBLIC_' . $messageData['CHAT_ID'],
                Array(
                    'module_id' => 'im',
                    'command' => 'messageParamsUpdate',
                    'params' => $arPullMessage,
    ...
    public static function PrepareValues($values, $withDefault = false)
    {
        ...
        foreach ($values as $key => $value) {
            if (in_array(
                ...
                if (in_array($key, Array('KEYBOARD_UID'))) {
                    ...
                    if (in_array(
                        ...
                        if (in_array($key, Array('CHAT_LAST_DATE'))) {
                            if (is_object($value) && $value instanceof \Bitrix\Main\Type\DateTime) {
                                ...
                                if (is_object($value[0]) && $value[0] instanceof \Bitrix\Main\Type\DateTime) {
                                    ...
                                    $arValues[$key] = \Bitrix\Main\Type\DateTime::createFromTimestamp(
                                        intval($value[0])
                                    );
                                }
                            }
                            ...
                            if ($key == 'DATE_TEXT') {
                                ...
                                if ($key == 'CHAT_USER' || $key == 'DATE_TS' || $key == 'FILE_ID' || $key == 'LIKE' || $key == 'FAVORITE' || $key == 'KEYBOARD_ACTION' || $key == 'URL_ID' || $key == 'LINK_ACTIVE' || $key == 'USERS') {
                                    if (is_array($value) && !empty($value)) {
                                        foreach ($value as $k => $v) {
                                            ...
                                            if ($key == 'ATTACH') {
                                                ...
                                                if (
                                                    ...
                                                    if ($key == 'CONNECTOR_MID') {
                                                        ...
                                                        if ($key == 'NAME') {
                                                            $arValues[$key] = isset($value[0]) ? htmlspecialcharsbx(
                                                                $value[0]
                                                            ) : $arDefault[$key];
                                                        } else {
                                                            if ($key == 'USER_ID') {
                                                                $arValues[$key] = isset($value[0]) ? intval(
                                                                    $value[0]
                                                                ) : $arDefault[$key];
                                                            } else {
                                                                if ($key == 'AVATAR') {
                                                                    if (isset($value)) {
                                                                        $arValues[$key] = CIMChat::GetAvatarImage(
                                                                            $value[0],
                                                                            200,
                                                                            false
                                                                        );
...
class CIMMessageParamAttach
{
    ...
    public function AddUser($params)
    {
        ...
        if (isset($params['NETWORK_ID'])) {
            $add['NETWORK_ID'] = htmlspecialcharsbx(mb_substr($params['NETWORK_ID'], 0, 1)) . intval(
                    mb_substr($params['NETWORK_ID'], 1)
                );
        } else {
            if (isset($params['USER_ID']) && intval($params['USER_ID']) > 0) {
                $add['USER_ID'] = intval($params['USER_ID']);
            } else {
                if (isset($params['CHAT_ID']) && intval($params['CHAT_ID']) > 0) {
                    $add['CHAT_ID'] = intval($params['CHAT_ID']);
                    $add['AVATAR_TYPE'] = 'CHAT';
                } else {
                    if (isset($params['BOT_ID']) && intval($params['BOT_ID']) > 0) {
    ...
    public function AddLink($params)
    {
        ...
        if (isset($params['NETWORK_ID']) && isset($params['NAME'])) {
            $result['NETWORK_ID'] = htmlspecialcharsbx(mb_substr($params['NETWORK_ID'], 0, 1)) . intval(
                    mb_substr($params['NETWORK_ID'], 1)
                );
        } else {
            if (isset($params['USER_ID']) && intval($params['USER_ID']) > 0 && isset($params['NAME'])) {
                $result['USER_ID'] = intval($params['USER_ID']);
            } else {
                if (isset($params['CHAT_ID']) && intval($params['CHAT_ID']) > 0 && isset($params['NAME'])) {
                    $result['CHAT_ID'] = intval($params['CHAT_ID']);
                } else {
                    if (!isset($params['LINK']) || isset($params['LINK']) && !preg_match(
                            '#^(?:/|https?://)#',
                            $params['LINK']
                        )) {
                        return false;
    ...
    public function AddRichLink($params)
    {
        ...
        if (isset($params['HTML'])) {
            ...
            if (isset($params['PREVIEW']) && preg_match('#^(?:/|https?://)#', $params['PREVIEW'])) {
                ...
                if (isset($params['EXTRA_IMAGE']) && preg_match('#^(?:/|https?://)#', $params['EXTRA_IMAGE'])) {
    ...
    public function AddGrid($params)
    {
        ...
        foreach ($params as $grid) {
            ...
            if (isset($grid['DISPLAY']) && in_array(
                    $grid['DISPLAY'],
                    Array('BLOCK', 'LINE', 'CARD', 'ROW', 'COLUMN', 'TABLE')
                )) {
    ...
    public function AddImages($params)
    {
        ...
        foreach ($params as $images) {
            ...
            if (!isset($images['LINK']) || isset($images['LINK']) && !preg_match(
                    '#^(?:/|https?://)#',
                    $images['LINK']
                )) {
                continue;
        ...
        foreach ($blocks as $data) {
            if (isset($data['USER'])) {
                ...
                if (isset($data['LINK'])) {
                    ...
                    if (isset($data['RICH_LINK'])) {
                        if (is_array($data['RICH_LINK']) && !\Bitrix\Main\Type\Collection::isAssociative(
                                $data['RICH_LINK']
                            )) {
                        ...
                        if (isset($data['MESSAGE'])) {
                            ...
                            if (isset($data['GRID'])) {
                                ...
                                if (isset($data['IMAGE'])) {
                                    if (is_array($data['IMAGE']) && \Bitrix\Main\Type\Collection::isAssociative(
                                            $data['IMAGE']
                                        )) {
                                    ...
                                    if (isset($data['FILE'])) {
                                        if (is_array($data['FILE']) && \Bitrix\Main\Type\Collection::isAssociative(
                                                $data['FILE']
                                            )) {
    ...
    public static function GetTextForIndex($attach): array
    {
        ...
        array_walk_recursive(
            $attach,
            function ($item, $key) use (&$textNodes) {
        ...
        if ($linkParam['TYPE'] == UrlPreview\UrlMetadataTable::TYPE_STATIC) {
            ...
            if (intval($linkParam['IMAGE_ID']) > 0) {
                ...
                if ($linkParam['IMAGE'] <> '') {
                    ...
                    if (!empty($linkParam['EXTRA']['IMAGES'])) {
            ...
            if ($linkParam['TYPE'] == UrlPreview\UrlMetadataTable::TYPE_DYNAMIC) {
