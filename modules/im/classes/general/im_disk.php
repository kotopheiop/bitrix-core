...
class CIMDisk
{
    ...
    public static function GetStorage()
    {
        ...
        if (!$storageModel) {
            ...
            $storageModel = $driver->addStorageIfNotExist(
                $data,
                array(
                    array(
                        'ACCESS_CODE' => 'AU',
    ...
    public static function UploadFileRegister($chatId, $files, $text = '', $linesSilentMode = false)
    {
        ...
        foreach ($files as $fileId => $fileData) {
            ...
            $newFile = $folderModel->addBlankFile(
                Array(
                    'NAME' => $fileData['name'],
                    'SIZE' => $fileData['size'],
    ...
    public static function UploadFileFromDisk($chatId, $files, $text = '', $options = [], $robot = false)
    {
        ...
        foreach ($files as $fileId) {
            if (mb_substr($fileId, 0, 6) == 'upload') {
                ...
                $newFile = self::SaveFromLocalDisk(
                    $chatId,
                    mb_substr($fileId, 4),
                    $makeSymlink,
                    $skipUserCheck ? 0 : $userId
                );
            }
                        ...
                        if ($chat['ENTITY_TYPE'] == 'LINES') {
                            ...
                            if ($chat['ENTITY_TYPE'] != 'LIVECHAT') {
                                foreach ($chatRelation as $relation) {
                                    ...
                                    \Bitrix\Disk\Driver::getInstance()->getRecentlyUsedManager()->push(
                                        $relation['USER_ID'],
                                        $newFile
                                    );
                                }
            ...
            if ($chat['ENTITY_TYPE'] == 'LIVECHAT' && CModule::IncludeModule('imopenlines')) {
                ...
                if ($session->load(
                    ...
                    if ($session->isNowCreated()) {
                        \Bitrix\ImOpenLines\Connector::saveCustomData(
                            $session->getData('CHAT_ID'),
                            $_SESSION['LIVECHAT']['CUSTOM_DATA']
                        );
    ...
    public static function GetMaxFileId($chatId)
    {
        ...
        $result = \Bitrix\Disk\Internals\ObjectTable::getList(
            array(
                'select' => array('MAX_ID'),
                'filter' => array(
                    '=PARENT_ID' => $folderModel->getId(),
    ...
    public static function GetFileParams($chatId, $fileModel)
    {
        ...
        if (\Bitrix\Disk\TypeFile::isImage($fileModel->getName())) {
            ...
            if (\Bitrix\Disk\TypeFile::isVideo($fileModel->getName())) {
                ...
                if (\Bitrix\Disk\TypeFile::isAudio($fileModel->getName())) {
                    $contentType = 'audio';
                }
            }
        }
        ...
        try {
            $viewerType = \Bitrix\Disk\Ui\FileAttributes::buildByFileId(
                $fileModel->getFileId(),
                $fileData['urlDownload']
            )
    ...
    public static function GetFolderModel($chatId)
    {
        ...
        if (!$folderId) {
            ...
            $folderModel = $storageModel->addFolder(
                array('NAME' => 'chat' . $chatId, 'CREATED_BY' => self::GetUserId()),
                $accessCodes,
                true
            );
            if ($folderModel) {
    ...
    public static function GetLocalDiskFolderPath()
    {
        ...
        return \Bitrix\Disk\Driver::getInstance()->getUrlManager()->getUrlFocusController(
            'openFolderList',
            array(
    ...
    public static function GetLocalDiskFilePath($fileId = 0)
    {
        ...
        return \Bitrix\Disk\Driver::getInstance()->getUrlManager()->getUrlFocusController(
            'showObjectInGrid',
            array('objectId' => $fileId ? $fileId : '_FILE_ID_')
        );
    }
    ...
    public static function GetPublicPath($type, \Bitrix\Disk\File $fileModel, $checkContentVersion = true)
    {
        ...
        if ($type == self::PATH_TYPE_SHOW) {
            ...
            if ($type == self::PATH_TYPE_PREVIEW) {
                if (!($isImage || $isVideo)) {
                    return $result;
                }
                ...
                $result = $urlManager->create(
                    ...
                    [
                        ...
                        'width' => 640,
                        'height' => 640,
                        'signature' => \Bitrix\Disk\Security\ParameterSigner::getImageSignature(
                            $fileModel->getId(),
                            640,
                            640
                        ),
                ...
                if ($type == self::PATH_TYPE_DOWNLOAD) {
                    $result = $urlManager->create(
                        'disk.api.file.download',
                        [
                            'humanRE' => 1,
    ...
    public static function RemoveTmpFileAgent()
    {
        ...
        $fileModels = \Bitrix\Disk\File::getModelList(
            Array(
                'filter' => Array(
                    'GLOBAL_CONTENT_VERSION' => 1,
