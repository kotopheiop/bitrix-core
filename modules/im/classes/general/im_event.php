...
class CIMEvent
{
            ...
            if (mb_substr($params['LOGIN'], 0, mb_strlen(IM\Bot::LOGIN_START)) == IM\Bot::LOGIN_START) {
                ...
                if ($orm->fetch()) {
                    $GLOBALS["APPLICATION"]->ThrowException(
                        GetMessage("IM_EVENT_ERROR_CHANGE_PASSWORD_FOR_BOT"),
    ...
    public static function OnAddRatingVote($id, $arParams)
    {
        ...
        if (!empty($contentId['ENTITY_TYPE'])) {
            ...
            if (
                ...
                $arParams["ENTITY_MESSAGE"] = trim(
                    strip_tags(str_replace(array("\r\n", "\n", "\r"), ' ', $description))
                );
                $arParams["ENTITY_BODY"] = preg_replace(
                    ...
                    if ($bExtranetInstalled) {
                        ...
                        while ($arSite = $dbSite->Fetch()) {
                            $arSites[$arSite["ID"]] = array(
                                ...
                                "SERVER_NAME" => (trim(
                                    $arSite["SERVER_NAME"]
                                ) <> '' ? $arSite["SERVER_NAME"] : COption::GetOptionString(
                                    "main",
                                    "server_name",
                                    $_SERVER["HTTP_HOST"]
                                ))
                                ...
                                if (
                                    ...
                                    if ($followValue != "N") {
                                        ...
                                        $arMessageFields = array(
                                            ...
                                            "NOTIFY_SUB_TAG" => $ratingMentionNotifyTag . '|' . intval(
                                                    $mentioned_user_id
                                                ),
                                            "NOTIFY_MESSAGE" => self::GetMessageRatingVote($arParams),
                                            "NOTIFY_MESSAGE_OUT" => self::GetMessageRatingVote($arParams, true)
    ...
    public static function OnCancelRatingVote($id, $arParams)
    {
        CIMNotify::DeleteByTag(
            "RATING|" . $arParams['ENTITY_TYPE_ID'] . "|" . $arParams['ENTITY_ID'],
            $arParams['USER_ID']
        );
    }
    ...
    public static function GetMessageRatingVote($arParams, $bForMail = false)
    {
        ...
        if (
            ...
            $arParams["ENTITY_MESSAGE"] = $stripped . (mb_strlen($stripped) != mb_strlen(
                    $arParams["ENTITY_MESSAGE"]
                ) ? '...' : '');
        } else {
            $stripped = $CCTP->strip_words($arParams["ENTITY_TITLE"], 199);
            $arParams["ENTITY_TITLE"] = $stripped . (mb_strlen($stripped) != mb_strlen(
                    $arParams["ENTITY_TITLE"]
                ) ? '...' : '');
        }
        ...
        if ($bForMail) {
            if ($arParams['ENTITY_TYPE_ID'] == 'BLOG_POST') {
                $message = str_replace(
                    ...
                    $arParams["ENTITY_TITLE"],
                    GetMessage(
                        'IM_EVENT_RATING_BLOG_POST' . ($arParams['MENTION'] ? '_MENTION' : '') . $like
                    )
                );
            } elseif ($arParams['ENTITY_TYPE_ID'] == 'BLOG_COMMENT') {
                $message = str_replace(
                    Array('#TITLE#', '#A_START#', '#A_END#'),
                    Array($arParams["ENTITY_MESSAGE"], '', ''),
                    GetMessage(
                        'IM_EVENT_RATING_COMMENT' . ($arParams['MENTION'] ? '_MENTION' : '') . $like
                    )
                );
                ...
                $message = str_replace(
                    '#LINK#',
                    $arParams["ENTITY_TITLE"],
                    GetMessage('IM_EVENT_RATING_FORUM_TOPIC' . $like)
                );
                ...
                if (isset($arParams["ENTITY_BODY"])) {
                    $message = str_replace(
                        Array('#TITLE#', '#A_START#', '#A_END#'),
                        array($arParams["ENTITY_BODY"], '', ''),
                        GetMessage('IM_EVENT_RATING_PHOTO_ALBUM' . $like)
                    );
                } elseif (is_numeric($arParams["ENTITY_TITLE"])) {
                    $message = str_replace(
                        Array('#A_START#', '#A_END#'),
                        Array('', ''),
                        GetMessage('IM_EVENT_RATING_PHOTO_ALBUM1' . $like)
                    );
                } else {
                    $message = str_replace(
                        Array('#TITLE#', '#A_START#', '#A_END#'),
                        Array($arParams["ENTITY_TITLE"], '', ''),
                        GetMessage('IM_EVENT_RATING_PHOTO_ALBUM' . $like)
                    );
                }
            } elseif (
                ...
                $message = str_replace(
                    Array('#TITLE#', '#A_START#', '#A_END#'),
                    Array($arParams["ENTITY_TITLE"], '', ''),
                    GetMessage('IM_EVENT_RATING_FILE' . $like)
                );
            } elseif (
                $arParams['ENTITY_TYPE_ID'] == 'IBLOCK_ELEMENT'
                && $arParams['ENTITY_PARAM'] == 'wiki'
                ...
                $message = str_replace(
                    Array('#TITLE#', '#A_START#', '#A_END#'),
                    Array($arParams["ENTITY_TITLE"], '', ''),
                    GetMessage('IM_EVENT_RATING_WIKI' . $like)
                );
            } elseif (
                $arParams['ENTITY_TYPE_ID'] == 'IBLOCK_ELEMENT'
                && $arParams['ENTITY_PARAM'] == 'photos'
                ...
                if (isset($arParams["ENTITY_BODY"])) {
                    $message = str_replace(
                        Array('#TITLE#', '#A_START#', '#A_END#'),
                        Array($arParams["ENTITY_BODY"], '', ''),
                        GetMessage('IM_EVENT_RATING_PHOTO' . $like)
                    );
                } elseif (is_numeric($arParams["ENTITY_TITLE"])) {
                    $message = str_replace(
                        Array('#A_START#', '#A_END#'),
                        Array('', ''),
                        GetMessage('IM_EVENT_RATING_PHOTO1' . $like)
                    );
                } else {
                    $message = str_replace(
                        Array('#TITLE#', '#A_START#', '#A_END#'),
            ...
            if ($arParams['ENTITY_TYPE_ID'] == 'BLOG_POST') {
                $message = str_replace(
                    '#LINK#',
                    '<a href="' . $arParams['ENTITY_LINK'] . '" class="bx-notifier-item-action">' . $arParams["ENTITY_TITLE"] . '</a>',
                    GetMessage(
                        'IM_EVENT_RATING_BLOG_POST' . ($arParams['MENTION'] ? '_MENTION' : '') . $like
                    )
                );
                ...
                $message = str_replace(
                    '#LINK#',
                    $arParams['ENTITY_LINK'] <> '' ? '<a href="' . $arParams['ENTITY_LINK'] . '" class="bx-notifier-item-action">' . $arParams["ENTITY_TITLE"] . '</a>' : '<i>' . $arParams["ENTITY_TITLE"] . '</i>',
                    GetMessage('IM_EVENT_RATING_ELSE' . $like)
                );
            }
            ...
            if ($intranetInstalled) {
                $message .= "\n" . str_replace(
                        "#REACTION#",
                        \CRatingsComponentsMain::getRatingLikeMessage(
                            !empty($arParams['REACTION']) ? $arParams['REACTION'] : ''
                        ),
                        Bitrix\Main\Localization\Loc::getMessage("IM_EVENT_RATING_REACTION")
                    );
            }
        }
    ...
    public static function GetMessageRatingEntityURL(
        $url,
        $user_id = false,
        $arSites = false,
        $intranet_site_id = false,
            ...
            if (
                ...
                if ($bExtranetUser) {
                    ...
                    if (mb_substr(
                            $link,
                            0,
                            mb_strlen($arSites[$extranet_site_id]['DIR'])
                        ) == $arSites[$extranet_site_id]['DIR']) {
                        $link = mb_substr($link, mb_strlen($arSites[$extranet_site_id]['DIR']));
                    }

                    $SiteServerName = $arSites[$extranet_site_id]['SERVER_NAME'] . $arSites[$extranet_site_id]['DIR'] . ltrim(
                            $link,
                            "/"
                        );
                } else {
                    ...
                    if (mb_substr(
                            $link,
                            0,
                            mb_strlen($arSites[$intranet_site_id]['DIR'])
                        ) == $arSites[$intranet_site_id]['DIR']) {
                        $link = mb_substr($link, mb_strlen($arSites[$intranet_site_id]['DIR']));
                    }

                    $SiteServerName = $arSites[$intranet_site_id]['SERVER_NAME'] . $arSites[$intranet_site_id]['DIR'] . ltrim(
                            $link,
                            "/"
                        );
                }
                ...
                $SiteServerName = (defined(
                    'SITE_SERVER_NAME'
                ) && SITE_SERVER_NAME <> '' ? SITE_SERVER_NAME : COption::GetOptionString(
                    "main",
                    "server_name",
                    $_SERVER['SERVER_NAME']
                ));
    ...
    private static function GetMessageRatingLogCommentURL(
        $arComment,
        $user_id = false,
        $arSites = false,
        $intranet_site_id = false,
        ...
        if (
            ...
            $url = (in_array(
                $arComment["ENTITY_TYPE"],
                array("CRMLEAD", "CRMCONTACT", "CRMCOMPANY", "CRMDEAL", "CRMACTIVITY")
            ) ? $arSites[$user_site_id]["DIR"] . "crm/stream?log_id=#log_id#" : COption::GetOptionString(
                "socialnetwork",
                "log_entry_page",
                $arSites[$user_site_id]["DIR"] . "company/personal/log/#log_id#/",
                $user_site_id
            ));
            ...
            $url .= (mb_strpos(
                    $url,
                    "?"
                ) !== false ? "&" : "?") . "commentId=" . $arComment["ID"] . "#com" . $arComment["ID"];
            $url = (CMain::IsHTTPS() ? "https" : "http") . "://" . $arSites[$user_site_id]['SERVER_NAME'] . $url;
        } else {
            $url = (in_array(
                $arComment["ENTITY_TYPE"],
                array("CRMLEAD", "CRMCONTACT", "CRMCOMPANY", "CRMDEAL", "CRMACTIVITY")
            ) ? SITE_DIR . "crm/stream?log_id=#log_id#" : COption::GetOptionString(
                "socialnetwork",
                "log_entry_page",
                SITE_DIR . "company/personal/log/#log_id#/",
                SITE_ID
            ));
            $url = str_replace("#log_id#", $arComment["LOG_ID"], $url);
            $url .= (mb_strpos(
