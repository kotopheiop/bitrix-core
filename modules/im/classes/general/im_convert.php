    ...
    public static function RecentList()
    {
        ...
        if (!empty($arRecent)) {
            if (isset($arRecent[IM_MESSAGE_PRIVATE]) && !empty($arRecent[IM_MESSAGE_PRIVATE])) {
                ...
                $arMessages = $CIMMessage->GetLastSendMessage(
                    Array(
                ...
                if (!empty($arUsers)) {
                    $strSql = "SELECT ITEM_ID FROM b_im_recent WHERE USER_ID = " . $USER->GetId(
                        ) . " AND ITEM_TYPE = '" . IM_MESSAGE_PRIVATE . "' AND ITEM_ID IN (" . implode(
                            ',',
                            $arUsers
                        ) . ")";
            ...
            if (!empty($massageType)) {
                ...
                $arMessagesGroup = $CIMChat->GetLastSendMessage(
                    Array(
