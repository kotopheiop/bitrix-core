...
$aTabs = array(
    array(
        "DIV" => "edit1",
        "TAB" => Loc::getMessage("IM_TAB_SETTINGS"),
        "ICON" => "im_path",
        "TITLE" => Loc::getMessage("IM_TAB_TITLE_SETTINGS"),
    ),
);
...
if ($_POST['Update'] . $_GET['RestoreDefaults'] <> '' && check_bitrix_sessid() && $MOD_RIGHT >= 'W') {
    if ($_GET['RestoreDefaults'] <> '') {
        ...
        $arSettings = CIMSettings::checkValues(
            CIMSettings::SETTINGS,
            Array(
...
<form method="post"
            ...
            if ($arSettingsDefault['panelPositionVertical'] == 'top' && $arSettingsDefault['panelPositionHorizontal'] == 'center') {
                ...
                if ($arSettingsDefault['panelPositionVertical'] == 'bottom' && $arSettingsDefault['panelPositionHorizontal'] == 'left') {
                    ...
                    if ($arSettingsDefault['panelPositionVertical'] == 'bottom' && $arSettingsDefault['panelPositionHorizontal'] == 'right') {
                        ...
                        if ($arSettingsDefault['panelPositionVertical'] == 'bottom' && $arSettingsDefault['panelPositionHorizontal'] == 'center') {
    ...
    $arReference = Array(
        ...
        'select3' => Array(
            Loc::getMessage('IM_SELECT_1_2'),
            Loc::getMessage('IM_SELECT_2_2'),
            Loc::getMessage('IM_SELECT_2_3')
        ),
        'sendByEnter' => Array("Enter", "Ctrl+Enter"),
        'location' => Array(
            Loc::getMessage('IM_PANEL_LOCATION_TL'),
            Loc::getMessage('IM_PANEL_LOCATION_TR'),
            Loc::getMessage('IM_PANEL_LOCATION_TC'),
            Loc::getMessage('IM_PANEL_LOCATION_BL'),
            Loc::getMessage('IM_PANEL_LOCATION_BR'),
            Loc::getMessage('IM_PANEL_LOCATION_BC')
        ),
        'birthday' => Array(
            Loc::getMessage('IM_CONTACT_LIST_BIRTHDAY_ALL'),
            Loc::getMessage('IM_CONTACT_LIST_BIRTHDAY_DEPARTMENT'),
            Loc::getMessage('IM_CONTACT_LIST_BIRTHDAY_NONE')
        ),
    );
    ?>
    <? if (IsModuleInstalled('voximplant')): ?>
        <tr>
            ...
            <td class="adm-detail-content-cell-r" width="60%"><input type="checkbox"
                                                                     name="CALL_SERVER_ENABLED" <?= (COption::GetOptionString(
                    "im",
                    'call_server_enabled'
                ) ? 'checked="checked"' : '') ?>></td>
        </tr>
    ...
    <tr id="video_group_2" <? if (!$selfVideoServer): ?>style="display: none"<? endif; ?>>
        ...
        <td class="adm-detail-content-cell-r"><input type="input" size="40" value="<?= htmlspecialcharsbx(
                COption::GetOptionString("im", "turn_server")
            ) ?>" name="TURN_SERVER"></td>
    </tr>
    <tr id="video_group_3" <? if (!$selfVideoServer): ?>style="display: none"<? endif; ?>>
        ...
        <td class="adm-detail-content-cell-r"><input type="input" size="40" value="<?= htmlspecialcharsbx(
                COption::GetOptionString("im", "turn_server_firefox")
    ...
    <tr id="video_group_4" <? if (!$selfVideoServer): ?>style="display: none"<? endif; ?>>
        ...
        <td class="adm-detail-content-cell-r"><input type="input" size="20" value="<?= htmlspecialcharsbx(
                COption::GetOptionString("im", "turn_server_login")
    ...
    <tr id="video_group_5" <? if (!$selfVideoServer): ?>style="display: none"<? endif; ?> >
        ...
        <td class="adm-detail-content-cell-r"><input type="input" size="20" value="<?= htmlspecialcharsbx(
                COption::GetOptionString("im", "turn_server_password")
    ...
    <tr>
        ...
        <td class="adm-detail-content-cell-r" width="60%"><?= SelectBoxFromArray(
                "SEND_BY_ENTER",
                array(
                    'reference_id' => $arReferenceId['sendByEnter'],
                    'reference' => $arReference['sendByEnter']
                ),
                $arSettingsDefault['sendByEnter'] ? 'Y' : 'N'
            ); ?></td>
    </tr>
    <? /*<tr>
    ...
    <tr>
        ...
        <td class="adm-detail-content-cell-r" width="60%"><?= SelectBoxFromArray(
                "PANEL_LOCATION",
                array(
                    'reference_id' => $arReferenceId['location'],
                    'reference' => $arReference['location']
                ),
                $arSettingsDefault['location']
            ); ?></td>
    </tr>
    <? if (!IsModuleInstalled('intranet')): ?>
        <tr>
            ...
            <td class="adm-detail-content-cell-r" width="60%"><?= SelectBoxFromArray(
                    "PRIVACY_MESSAGE",
                    array(
                        'reference_id' => $arReferenceId['select'],
                        'reference' => $arReference['select1']
                    ),
                    $arSettingsDefault['privacyMessage']
                ); ?></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="adm-detail-content-cell-r" width="60%"><?= SelectBoxFromArray(
                    ...
                    array(
                        'reference_id' => $arReferenceId['select'],
                        'reference' => $arReference['select2']
                    ),
                    $arSettingsDefault['privacyChat']
                ); ?></td>
        </tr>
        <tr>
            ...
            <td class="adm-detail-content-cell-r" width="60%"><?= SelectBoxFromArray(
                    ...
                    $arSettingsDefault['privacySearch']
                ); ?></td>
        </tr>
        <? if (IsModuleInstalled('b24network')): ?>
            <tr>
                ...
                <td class="adm-detail-content-cell-r" width="60%"><?= SelectBoxFromArray(
                        "PRIVACY_PROFILE",
                        array(
                            'reference_id' => $arReferenceId['select3'],
                            'reference' => $arReference['select3']
                        ),
                        $arSettingsDefault['privacyProfile']
                    ); ?></td>
            </tr>
        <? endif; ?>
    <? endif; ?>
    <tr>
        ...
        <td class="adm-detail-content-cell-r" width="60%"><?= SelectBoxFromArray(
                ...
                array(
                    ...
                    'reference' => Array(
                        Loc::getMessage(
                            'IM_START_CHAT_MESSAGE_FIRST'
                        ),
                        Loc::getMessage(
                            'IM_START_CHAT_MESSAGE_LAST'
                        )
                    )
                ),
    ...
    <? if (IsModuleInstalled('intranet')): ?>
        <tr>
            ...
            <td class="adm-detail-content-cell-r" width="60%"><?= SelectBoxFromArray(
                    ...
                    array(
                        'reference_id' => $arReferenceId['birthday'],
                        'reference' => $arReference['birthday']
                    ),
                    COption::GetOptionString(
                        "im",
                        'contact_list_birthday'
    ...
    <tr>
        ...
        <td class="adm-detail-content-cell-r" width="60%"><input type="checkbox"
                                                                 name="COLOR_ENABLE" <?= (COption::GetOptionString(
                "im",
                'color_enable'
            ) ? 'checked="checked"' : '') ?>></td>
    </tr>
    <tr>
        ...
        <td class="adm-detail-content-cell-r" width="60%"><input type="checkbox"
                                                                 name="OPEN_CHAT_ENABLE" <?= (COption::GetOptionString(
                "im",
                'open_chat_enable'
            ) ? 'checked="checked"' : '') ?>></td>
    </tr>
    <? if (IsModuleInstalled('intranet')): ?>
        <tr>
            ...
            <td class="adm-detail-content-cell-r" width="60%"><input type="checkbox"
                                                                     name="CONTACT_LIST_LOAD" <?= (COption::GetOptionString(
                    "im",
                    'contact_list_load'
                ) ? 'checked="checked"' : '') ?>></td>
        </tr>
        <tr>
            ...
            <td class="adm-detail-content-cell-r" width="60%"><input type="checkbox"
                                                                     name="GENERAL_CHAT_MESSAGE_JOIN" <?= (COption::GetOptionString(
                    "im",
        ...
        <tr>
            ...
            <td class="adm-detail-content-cell-r" width="60%"><input type="checkbox"
                                                                     name="GENERAL_CHAT_MESSAGE_LEAVE" <?= (COption::GetOptionString(
                    "im",
                ...
                <table width="75%" align="center">
                    ...
                    foreach ($arDefValues as $key => $value) {
                        if ($key == "user_name_template") {
                            ?>
                            <tr>
                                <td align="right" valign="middle" width="50%"><?= Loc::getMessage(
                                        "IM_OPTIONS_NAME_TEMPLATE"
                                    ); ?>:
                                </td>
                                <td>
                            ...
                            ?>
                            <tr>
                                <td align="right"><?php echo Loc::getMessage("IM_OPTIONS_" . mb_strtoupper($key)) ?>:
                                </td>
                                <td><input type="text" size="40" value="<?= htmlspecialcharsbx(
                                        COption::GetOptionString("im", $key, $value, $site["LID"])
