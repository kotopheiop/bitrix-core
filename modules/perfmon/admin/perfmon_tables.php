        ...
        switch ($_REQUEST['action']) {
            case "convert":
                if ($to != mb_strtoupper($arStatus["Engine"])) {
                    if ($to == "MYISAM") {
                        $res = $DB->Query(
                            "alter table " . CPerfomanceTable::escapeTable($table_name) . " ENGINE = MyISAM",
                            false
                        );
                    } elseif ($to == "INNODB") {
                        $res = $DB->Query(
                            "alter table " . CPerfomanceTable::escapeTable($table_name) . " ENGINE = InnoDB",
                            false
                        );
                    } else {
                        $res = true;
                ...
                foreach ($arFields as $columnName => $columnInfo) {
                    ...
                    if ($objectSettings) {
                        ...
                        $fields[$columnName] = "\t\t\t"
                            ...
                            . ($columnInfo["orm_type"] === "enum"
                                ? $offset . "\t\t->configureValues([" . implode(
                                    ", ",
...
while ($arRes = $rsData->GetNext()) {
    ...
    $row->AddViewField(
        "TABLE_NAME",
        '<a class="table_name" data-table-name="' . $arRes["TABLE_NAME"] . '" href="perfmon_table.php?lang=' . LANGUAGE_ID . '&amp;table_name=' . urlencode(
            $arRes["TABLE_NAME"]
        ) . '">' . $arRes["TABLE_NAME"] . '</a>'
    );
...
if ($DB->type == "MYSQL") {
    ...
    foreach ($arEngines as $id => $ar) {
        $arGroupActions["convert_to_" . $id] = GetMessage(
            "PERFMON_TABLES_ACTION_CONVERT",
            array("#ENGINE_TYPE#" => $ar["NAME"])
        );
    }
        ...
        foreach ($arLastTables as $i => $table_name) {
            if ($DB->TableExists($table_name)) {
                $arLastTables[$i] = array(
                    "NAME" => '<a href="perfmon_table.php?lang=' . LANGUAGE_ID . '&amp;table_name=' . urlencode(
                            $table_name
                        ) . '">' . $table_name . '</a>',
                );
            } else {
