...
class CPerfomanceMeasure
{
    ...
    {
        $res = array();
        $file_name = $_SERVER["DOCUMENT_ROOT"] . "/" . COption::GetOptionString(
                "main",
                "upload_dir",
                "/upload/"
            ) . "/perfmon#i#.php";
        $content = "<?\$s='" . str_repeat("x", 1024) . "';?><?/*" . str_repeat("y", 1024) . "*/?><?\$r='" . str_repeat(
                "z",
                1024
            ) . "';?>";
    ...
    public static function GetDBMark($type)
    {
        ...
        switch ($type) {
            case "read":
                ...
                break;
            case "update":
                $strSql = "update b_perf_test set REFERENCE_ID = ID+1, NAME = '" . str_repeat(
                        "y",
                        200
                    ) . "' WHERE ID = #i#";
                $bFetch = false;
        ...
        if (function_exists('accelerator_get_status')) {
            ...
            array_unshift(
                ...
                array(
                    ...
                    "IS_OK" => $is_ok,
                    "VALUE" => $is_ok ? GetMessage("PERFMON_MEASURE_UP_AND_RUNNING") : GetMessage(
                        "PERFMON_MEASURE_DISABLED"
                    ),
                    "RECOMMENDATION" => "",
                )
            );
            ...
            "memory_abs" => array(
                array(
                    ...
                    "RECOMMENDATION" => GetMessage(
                        "PERFMON_MEASURE_EQUAL_OR_GREATER_THAN_REC",
                        array("#value#" => "40")
                    )
                ),
            ),
        ...
        $result = array(
            "enabled" => array(
                ...
                array(
                    ...
                    "RECOMMENDATION" => GetMessage(
                        "PERFMON_MEASURE_ZERO_OR_GREATER_THAN_REC",
                        array("#value#" => "20000")
                    ),
            ...
            "memory_pct" => array(
                array(
                    "PARAMETER" => 'apc.shm_size (' . GetMessage(
                            "PERFMON_MEASURE_CURRENT_VALUE",
                            array("#value#" => ini_get('apc.shm_size'))
                        ) . ')',
...
class CPerfAccelWinCache extends CPerfAccel
{
    ...
    function GetParams()
    {
        return array(
            ...
            "memory_pct" => array(
                array(
                    "PARAMETER" => 'wincache.ocachesize (' . GetMessage(
                            "PERFMON_MEASURE_CURRENT_VALUE",
                            array(
                                "#value#" => ini_get(
                                    'wincache.ocachesize'
        ...
        if (extension_loaded('Zend OPcache')) {
            ...
            array_unshift(
                ...
                array(
                    ...
                    "RECOMMENDATION" => GetMessage(
                        "PERFMON_MEASURE_EQUAL_OR_GREATER_THAN_REC",
                        array("#value#" => $rec_accelerated_files)
                    ),
                )
            );
