...
if ($RIGHT >= "R") :
    ...
    $arAllOptions = array(
        ...
        array(
            "large_cache_log",
            GetMessage("PERFMON_OPTIONS_LARGE_CACHE_LOG"),
            array("checkbox"),
            GetMessage("PERFMON_OPTIONS_LARGE_CACHE_NOTE")
        ),
        ...
        array(
            "slow_sql_log",
            GetMessage("PERFMON_OPTIONS_SLOW_SQL_LOG"),
            array("checkbox"),
            GetMessage("PERFMON_OPTIONS_SLOW_SQL_NOTE")
        ),
    ...
    $aTabs = array(
        ...
        array(
            "DIV" => "edit3",
            "TAB" => GetMessage("PERFMON_TAB_ORM"),
            "ICON" => "perfmon_settings",
            "TITLE" => GetMessage("PERFMON_TAB_TITLE_ORM")
        ),
        array(
            "DIV" => "edit2",
    ...
    <form method="post"
        ...
        foreach ($ormOptions as $option) {
            if (is_string($option)) {
                ?>
                <tr class="heading">
                    <td colspan="2"><?= htmlspecialcharsbx($option); ?></td>
                </tr>
                <?
                continue;
            }
        ...
        function slow_sql_log_check() {
            ...
            if (activeCheckbox) {
                ...
                if (
                    ...
                    jsSelectUtils.addNewOption(activeCheckbox, '3600', '<?echo GetMessageJS(
                        "PERFMON_OPT_INTERVAL_3600_SEC"
                    )?>');
                    jsSelectUtils.addNewOption(activeCheckbox, '14400', '<?echo GetMessageJS(
                        "PERFMON_OPT_INTERVAL_4_HOURS"
                    )?>');
                    jsSelectUtils.addNewOption(activeCheckbox, '28800', '<?echo GetMessageJS(
                        "PERFMON_OPT_INTERVAL_8_HOURS"
                    )?>');
                    jsSelectUtils.addNewOption(activeCheckbox, '86400', '<?echo GetMessageJS(
                        "PERFMON_OPT_INTERVAL_24_HOURS"
                    )?>');
                    jsSelectUtils.addNewOption(activeCheckbox, '604800', '<?echo GetMessageJS(
                        "PERFMON_OPT_INTERVAL_7_DAYS"
                    )?>');
                } else {
                    jsSelectUtils.addNewOption(activeCheckbox, '60', '<?echo GetMessageJS(
                        "PERFMON_OPT_INTERVAL_60_SEC"
                    )?>');
                    jsSelectUtils.addNewOption(activeCheckbox, '300', '<?echo GetMessageJS(
                        "PERFMON_OPT_INTERVAL_300_SEC"
                    )?>');
                    jsSelectUtils.addNewOption(activeCheckbox, '600', '<?echo GetMessageJS(
                        "PERFMON_OPT_INTERVAL_600_SEC"
                    )?>');
                    jsSelectUtils.addNewOption(activeCheckbox, '1800', '<?echo GetMessageJS(
                        "PERFMON_OPT_INTERVAL_1800_SEC"
                    )?>');
                    jsSelectUtils.addNewOption(activeCheckbox, '3600', '<?echo GetMessageJS(
                        "PERFMON_OPT_INTERVAL_3600_SEC"
                    )?>');
