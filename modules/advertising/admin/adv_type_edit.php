...
<form name="form1" method="POST" action="<? echo $APPLICATION->GetCurPage() ?>">
    ...
    <tr valign="top">
        <td><?
            if ($isEditMode) :
                ?><span style="font-weight: bold;"><?= GetMessage("AD_SID") ?><br><?= GetMessage("AD_SID_ALT") ?>
                </span><?
            else :
        ...
        <td><?
            if ($isEditMode) :
                ?><input maxlength="255" type="text" name="SID" size="20" value="<?
            echo (strlen($strError) > 0) ? $original_SID : $str_SID ?>"><?
            else :
                echo $str_SID;
            endif;
            ?></td>
