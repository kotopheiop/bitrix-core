...
<form name="form1" method="POST" action="<? echo $APPLICATION->GetCurPage() ?>">
                    ...
                    while (list($sid, $arrS) = each($arrSites)):
                        ...
                        <div class="adm-list-item">
                            ...
                            <div class="adm-list-label"><?
                                echo '[<a href="/bitrix/admin/site_edit.php?LID=' . urlencode($sid) . '&lang=' . LANGUAGE_ID . '" title="' . GetMessage("AD_SITE_ALT") . '">' . htmlspecialcharsex($sid) . '</a>]&nbsp;<label for="site_' . htmlspecialcharsbx($sid) . '">' . htmlspecialcharsex($arrS["NAME"]) ?></label></div>
                        </div>
                    <?endwhile; ?>
                </div>
            <?
                    ...
                    while ($arType = $rsType->GetNext()) {
                        ...
                        <div class="adm-list-item">
                            <div class="adm-list-control">
                                <input <?
                                if ($ID > 0 && in_array($arType["SID"], $arrTYPE)) echo "checked" ?>
                                        type="checkbox"
                                        name="arrTYPE[]"
                                        value="<?= $arType["SID"] ?>"
                                        id="arType_<?= $i ?>">
    ...
    <? else: ?>
        <tr valign="top">
            <td><?= GetMessage("AD_NOT_SHOW_PAGES"); ?></td>
            <td><textarea name="NOT_SHOW_PAGE" cols="45" rows="6"
                          wrap="OFF"><?= $str_NOT_SHOW_PAGE ?></textarea><br><?= GetMessage("AD_NOT_SHOW_PAGES_ALT") ?>
            </td>
        </tr>
    <? endif; ?>
    ...
    <? if ($isAdmin || ($isDemo && !$isOwner)): ?>
        ...
        <? if ($isEditMode) : ?>
            <tr>
                <td colspan="2" align="center"><textarea style="width:85%" name="ADMIN_COMMENTS" rows="7"
