...
$lAdmin->AddHeaders(
    array(
        ...
        array(
            "id" => "DATE_CREATE",
            "content" => GetMessage("ad_list_created"),
            "title" => GetMessage("ad_list_created_title"),
            "sort" => "s_date_create"
        ),
        array(
            "id" => "DATE_MODIFY",
            "content" => GetMessage("ad_list_modified"),
            "title" => GetMessage("ad_list_modified_title"),
            "sort" => "s_date_modify"
        ),
        array(
            "id" => "VISITOR_COUNT",
        ...
        array(
            "id" => "SHOW_COUNT_LAST_SHOW",
            "content" => GetMessage("AD_DATE_LAST_SHOW"),
            "sort" => "s_date_last_show"
        ),
        array(
            "id" => "CLICK_COUNT",
        ...
        array(
            "id" => "CLICK_COUNT_LAST_CLICK",
            "content" => GetMessage("AD_DATE_LAST_CLICK"),
            "sort" => "s_date_last_click"
        ),
        array("id" => "CTR", "content" => "CTR (%)", "sort" => "s_ctr", "align" => "right"),
        array("id" => "FIRST_SHOW", "content" => GetMessage("AD_FIRST_SHOW_DATE"), "sort" => "s_firstd_c"),
        array(
            "id" => "UNIFORM_COEF",
            "content" => GetMessage("AD_UNIFORM_COEF_VIEW"),
            "sort" => "s_uniform_c",
            "align" => "right"
        ),
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    $row =& $lAdmin->AddRow(
        $f_ID,
        $arRes,
        "adv_banner_edit.php?ID=" . $f_ID . "&CONTRACT_ID=" . $f_CONTRACT_ID . "&lang=" . LANGUAGE_ID,
        GetMessage("ADV_EDIT_TITLE")
    );
    $row->AddViewField(
        "ID",
        '<a href="adv_banner_edit.php?lang=' . LANGUAGE_ID . '&amp;ID=' . $f_ID . '&amp;CONTRACT_ID=' . $f_CONTRACT_ID . '" title="' . GetMessage(
            "ADV_EDIT_TITLE"
        ) . '">' . $f_ID . '</a>'
    );
    ...
    if (is_array($arrSITE)) {
        foreach ($arrSITE as $sid) {
            if (in_array($sid, $arrContractSite[$f_CONTRACT_ID])) {
                if ($isAdmin) {
                    $sites .= htmlspecialcharsbx(
                            $arrSites[$sid]["NAME"]
                        ) . ' [<a href="/bitrix/admin/site_edit.php?LID=' . htmlspecialcharsbx(
                            $sid
                        ) . '&amp;lang=' . LANGUAGE_ID . '" title="' . GetMessage(
                            "ADV_SITE_VIEW"
                        ) . '">' . htmlspecialcharsbx($sid) . '</a>]<br>';
                } else {
    ...
    if ($canAddbanner) {
        ...
        $row->AddViewField(
            "TYPE_SID",
            "[<a href='adv_type_edit.php?SID=" . $f_TYPE_SID . "&lang=" . LANGUAGE_ID . "&action=view' title='" . GetMessage(
                "ADV_TYPE_VIEW"
            ) . "'>" . $f_TYPE_SID . "</a>] " . $f_TYPE_NAME
        );
        $row->AddViewField(
            "CONTRACT_ID",
            "[<a href='adv_contract_edit.php?ID=" . $f_CONTRACT_ID . "&lang=" . LANGUAGE_ID . "&action=view' title='" . GetMessage(
                "ADV_CONTRACT_VIEW"
            ) . "'>" . $f_CONTRACT_ID . "</a>] " . $f_CONTRACT_NAME
        );
    }
    ...
    if (isset($arRes["FLYUNIFORM"]) && $arRes["FLYUNIFORM"] == "Y") {
        ...
        if ($rot && $tim) {
            $arProgress = $rot / $tim;
            $f_UNIFORM_COEF = round($rot * 100) . "%&nbsp;/&nbsp;" . round($tim * 100) . "%&nbsp;=&nbsp;" . round(
                    $arProgress,
                    3
                );
        }
    ...
    if ($isDemo || $canAddbanner) {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect("adv_banner_edit.php?ID=" . $f_ID . "&CONTRACT_ID=" . $f_CONTRACT_ID),
            "DEFAULT" => true
        );
    }
    ...
    $arActions[] = array(
        ...
        "ACTION" => $lAdmin->ActionRedirect(
            "adv_banner_edit.php?ID=" . $f_ID . "&CONTRACT_ID=" . $f_CONTRACT_ID . "&action=view"
        )
    );

    $arActions[] = array(
        "ICON" => "adv_graph",
        "TEXT" => GetMessage("AD_BANNER_STATISTICS_VIEW"),
        "TITLE" => GetMessage("AD_BANNER_STATISTICS_VIEW_TITLE"),
        "ACTION" => $lAdmin->ActionRedirect(
            "adv_banner_graph.php?find_banner_id[]=" . $f_ID . "&find_what_show[]=ctr&set_filter=Y"
        )
    );
    ...
    if ($isDemo || $canAddbanner) {
        ...
        if ($f_ACTIVE == 'Y') {
            $arActions[] = array(
                "ICON" => "deactivate",
                "TEXT" => GetMessage("MAIN_ADMIN_LIST_DEACTIVATE_UC"),
                "ACTION" => $lAdmin->ActionDoGroup($f_ID, "deactivate")
            );
        } else {
            $arActions[] = array(
                "ICON" => "activate",
                "TEXT" => GetMessage("MAIN_ADMIN_LIST_ACTIVATE_UC"),
        ...
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . GetMessage('AD_DELETE_BANNER_CONFIRM') . "')) " . $lAdmin->ActionDoGroup(
                    $f_ID,
                    "delete"
                )
        );
...
<form name="form1" action="<?= $APPLICATION->GetCurPage() ?>">
    ...
    <tr>
        ...
        <td>
            ...
            <select name="find_type_f">
                ...
                <option value="name"<? if ($find_type_f == "name") echo " selected" ?>><?= GetMessage(
                        'AD_F_NAME'
                    ) ?></option>
                <option value="code"<? if ($find_type_f == "code") echo " selected" ?>><?= GetMessage(
                        'AD_F_CODE'
                    ) ?></option>
            </select>
        </td>
    ...
    <tr>
        ...
        <td><input type="text" name="find_ctr_1" size="10"
                   value="<?= htmlspecialcharsbx($find_ctr_1) ?>"><? echo "&nbsp;" . GetMessage("AD_TILL") . "&nbsp;" ?>
            <input type="text" name="find_ctr_2" size="10" value="<?= htmlspecialcharsbx($find_ctr_2) ?>"></td>
    </tr>
    <tr>
        <td valign="top"><?= GetMessage("AD_F_GROUP") ?>:</td>
        <td><input type="text" name="find_group" size="47"
                   value="<? echo htmlspecialcharsbx($find_group) ?>"><?= InputType(
                "checkbox",
                "find_group_exact_match",
                "Y",
                $find_group_exact_match,
    ...
    <tr>
        ...
        <td><input type="text" name="find_contract" size="47"
                   value="<? echo htmlspecialcharsbx($find_contract) ?>"><?= InputType(
                "checkbox",
                "find_contract_exact_match",
                "Y",
                $find_contract_exact_match,
    ...
    <tr>
        ...
        <td><input type="text" name="find_type" size="47"
                   value="<? echo htmlspecialcharsbx($find_type) ?>"><?= InputType(
                "checkbox",
                "find_type_exact_match",
                "Y",
                $find_type_exact_match,
    ...
    <tr>
        ...
        <td><input type="text" name="find_code" size="47"
                   value="<? echo htmlspecialcharsbx($find_code) ?>"><?= InputType(
                "checkbox",
                "find_code_exact_match",
                "Y",
                $find_code_exact_match,
    ...
    <tr>
        ...
        <td><input type="text" name="find_comments" size="47"
                   value="<? echo htmlspecialcharsbx($find_comments) ?>"><?= InputType(
                "checkbox",
                "find_comments_exact_match",
                "Y",
                $find_comments_exact_match,
