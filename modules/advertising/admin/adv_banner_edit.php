...
<? if (strlen($strError) > 0)
    ...
    <form name="bx_adv_edit_form" method="POST" action="<?= $APPLICATION->GetCurPage() ?>"
        ...
        <tr>
            <td><?= GetMessage("AD_TYPE") ?><? if ($isEditMode): ?><span class="required"><sup>1</sup></span><? endif ?>
            </td>
            <td><?
                if ($isEditMode) :
                ...
                <select onchange='setType(this);' name="AD_TYPE">
                    ...
                    <? foreach ($arTemplates as $k => $template): ?>
                        <option id="AD_TYPE_TEMPLATE[<?= $k ?>]" value="template"
                                data-name="<?= $template['NAME'] ?>" <? if ((($ID && $str_AD_TYPE == 'template') || (isset($AD_TYPE) && ($AD_TYPE == 'template'))) && ($template['NAME'] == $templateName)): ?> selected<? endif; ?>><?= $template['DESCRIPTION'] ?></option>
                    <? endforeach; ?>
                </select>
        ...
        <? if ($isEditMode || intval($str_IMAGE_ID) > 0): ?>
            ...
            <tr id="eFlashTrans" style="display: none;">
                ...
                <td>
                    <select id="FLASH_TRANSPARENT" name="FLASH_TRANSPARENT">
                        <option value="transparent"<? if ($str_FLASH_TRANSPARENT == 'transparent'): ?> selected="selected"<? endif; ?>>
                            transparent
                        </option>
                        <option value="opaque"<? if ($str_FLASH_TRANSPARENT == 'opaque'): ?> selected="selected"<? endif; ?>>
                            opaque
                        </option>
                        <option value="window"<? if ($str_FLASH_TRANSPARENT == 'window'): ?> selected="selected"<? endif; ?>>
                            window
                        </option>
                    </select>
                </td>
            ...
            <tr valign="top" id="eUrlTarget" style="display: none;">
                <td><?= GetMessage("AD_URL_TARGET"); ?><? if ($isEditMode): ?><span
                            class="required"><sup>1</sup></span><? endif ?></td>
                <td><?
                    $ref = array(
            ...
            <tr valign="top" id="eImageAlt" style="display: none;">
                <td><?= GetMessage("AD_IMAGE_ALT") ?><? if ($isEditMode): ?><span
                            class="required"><sup>1</sup></span><? endif ?></td>
                <td><?
                    if ($isEditMode) :
                ...
                <table width="95%" cellspacing="0" border="0" cellpadding="0">
                    <? if ($isEditMode):
                        ...
                        <? else: ?>
                            ...
                            <tr>
                                <td align="center"><textarea style="width:100%" rows="30" name="CODE"
                                                             onfocus="t=this"><?= $str_CODE ?></textarea></td>
                            </tr>
                        <? endif;
                    else: ?>
                    ...
                    <? if ($isEditMode): ?>
                        <tr>
                            <td><?= GetMessage("AD_HTML_ALT") ?>&nbsp;<a href="javascript:PutRandom('#RANDOM1#')"
                                                                         title="<?= GetMessage("AD_INS_TEMPL") ?>">#RANDOM1#</a>,
        ...
        <tr id="eAddTemplateBanner" style="display:none;">
            ...
            <? else: ?>
                <td colspan="2" style="text-align: center;"><input type="button"
                                                                   onclick='window.oBXBannerTemplate.addNewTBanner();return false;'
                                                                   class="adm-btn-save"
                                                                   value="<?= GetMessage("AD_ADD_SLIDE") ?>"
                                                                   title="<?= GetMessage("AD_ADD_SLIDE") ?>"></td>
            <? endif ?>
        </tr>
        <tr class="heading">
        ...
        <? if (COption::GetOptionString('advertising', 'DONT_FIX_BANNER_SHOWS') == "Y"): ?>
            <tr>
                <td colspan="2"><?= BeginNote() ?><?= GetMessage("AD_EDIT_NOT_FIX") ?><?= EndNote() ?></td>
            </tr>
        <? endif ?>
        <? endif ?>

        <tr valign="top">
            <td width="40%"><label for="FLYUNIFORM"><?= GetMessage("AD_UNIFORM") ?></label><span
        ...
        <? else: ?>
            <tr>
                ...
                <td><input type="text" name="MAX_VISITOR_COUNT" id="MAX_VISITOR_COUNT" size="10"
                           value="<?= $str_MAX_VISITOR_COUNT ?>"<?= $disableFixShow ?>><?
                    if ($ID > 0) :
                        ?>&nbsp;<?= GetMessage("AD_VISITORS") ?>&nbsp;<b><?= $str_VISITOR_COUNT ?></b>&nbsp;&nbsp;<?
                        if ($isAdmin || ($isDemo && !$isOwner) || $isManager) {
                            echo '<label for="RESET_VISITOR_COUNT">' . GetMessage("AD_RESET_COUNTER") . "</label>";
                            ?>&nbsp;<input type="checkbox" name="RESET_VISITOR_COUNT" value="Y"
                                           id="RESET_VISITOR_COUNT"<?= $disableFixShow ?>><?
                        }
                    endif;
                    ?></td>
            </tr>
        ...
        <? else: ?>
            <tr>
                ...
                <td><input type="text" name="MAX_SHOW_COUNT" id="MAX_SHOW_COUNT" size="10"
                           value="<?= $str_MAX_SHOW_COUNT ?>"<?= $disableFixShow ?>><?
                    if ($ID > 0) :
                        ?>&nbsp;<?= GetMessage("AD_SHOWN") ?>&nbsp;<b><?= $str_SHOW_COUNT ?></b>&nbsp;&nbsp;<?
                        if ($isAdmin || ($isDemo && !$isOwner) || $isManager) {
                            echo '<label for="RESET_SHOW_COUNT">' . GetMessage("AD_RESET_COUNTER") . '</label>';
                            ?>&nbsp;<input type="checkbox" name="RESET_SHOW_COUNT" value="Y"
                                           id="RESET_SHOW_COUNT"<?= $disableFixShow ?>><?
                        }
                    endif;
                    ?></td>
            </tr>
    ...
    <script>
        ...
        changeType('<?=$str_AD_TYPE?>', {
            params: <?=CUtil::PHPToJsObject($arPropsTemplate)?>,
            val: <?=CUtil::PHPToJsObject($arCurVal)?>});
    </script>
<?
