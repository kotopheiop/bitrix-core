...
$man = false;
if ((!isset($_SESSION["SESS_ADMIN"]["AD_STAT_CONTRACT_DIAGRAM"]) || empty($_SESSION["SESS_ADMIN"]["AD_STAT_CONTRACT_DIAGRAM"])) && $find_date1 == '' && $find_date2 == '' && !is_array(
        $find_contract_id
...
if ((!is_set($find_contract_id) && !is_set($find_what_show)) || (!is_set($find_what_show) && is_set(
            $find_contract_id
        )) || (is_set($find_what_show) && !is_set($find_contract_id))) {
                ...
                foreach ($arShow as $ctype) {
                    ...
                    if (${"sum_" . mb_strtolower($counter_type)} > 0) {
                        ...
                        $aTabs[] = array(
                            ...
                            "TITLE" => GetMessage("AD_CONTRACT_DIAGRAM_TITLE")
                        );
                    }
                }
                                            ...
                                            foreach ($arrLegend as $keyL => $arrS):
                                                if ($arrS["COUNTER_TYPE"] == "DETAIL" && $arrS["TYPE"] == $diagram_type):
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td valign="center">
                                                            <div style="background-color: <?= "#" . $color ?>"><img
                                                        ...
                                                        <td nowrap><? echo "[<a href='/bitrix/admin/adv_contract_edit.php?find_id=" . $arrS["ID"] . "&lang=" . LANGUAGE_ID . "&action=view' title='" . GetMessage(
                                                                    "AD_CONTRACT_VIEW"
                                                                ) . "'>" . $arrS["ID"] . "</a>] " . $arrS["NAME"]; ?></td>
                                                    </tr>
                                                <?
                                                endif;
                                            endforeach;
