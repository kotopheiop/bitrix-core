    ...
    <form name="form1" method="POST" action="" enctype="multipart/form-data">
        ...
        if (isset($vote)) {
            if (strlen($vote->get("TIMESTAMP_X")) > 0 && $vote->get("TIMESTAMP_X") != "00.00.0000 00:00:00") {
                ...
                <td><?= $vote->get("TIMESTAMP_X") ?></td></tr><?
            }
        }
        ?>
        <tr>
            <td><?= GetMessage("VOTE_ACTIVE_TITLE") ?></td>
            <td><input type="hidden" name="ACTIVE" value="N"/><input type="checkbox" name="ACTIVE" id="ACTIVE"
                                                                     value="Y" <?= ($fields["ACTIVE"] == "Y" ? " checked" : "") ?> />
        ...
        if ($fields["AUTHOR_ID"] > 0 && ($arAuthor = CUser::GetByID($fields["AUTHOR_ID"])->Fetch())) {
            ...
            <tr>
                ...
                <td>
                    <a href="/bitrix/admin/user_edit.php?ID=<?= $fields["AUTHOR_ID"] ?>&lang=<?= LANG ?>">
                        [<?= $fields["AUTHOR_ID"] ?>] <?= $arAuthor["NAME"] ?></a>
                </td>
            </tr>
            <?
        }
        ...
        <tr>
            ...
            <td width="60%"><?
                ...
                if (IsModuleInstalled("im") && IsModuleInstalled("search")) {
                    $ref["reference_id"][] = "I";
                    $ref["reference"][] = GetMessage("VOTE_NOTIFY_IM");
                } else {
                    $fields["NOTIFY"] = ($fields["NOTIFY"] == "I" ? "N" : $fields["NOTIFY"]);
                }
                $ref["reference_id"][] = "Y";
                $ref["reference"][] = GetMessage("VOTE_NOTIFY_EMAIL");
                $ref["reference_id"][] = "N";
                $ref["reference"][] = GetMessage("VOTE_NOTIFY_N");
        ...
        <tr>
            ...
            <td><select name="CHANNEL_ID"><?
                    foreach ($channels as $res):
                        ?>
                        <option
                        value="<?= $res["ID"] ?>" <?= ($fields["CHANNEL_ID"] == $res["ID"] ? " selected" : "") ?><?
...
//********************
            ...
            <tr>
                <td>event1:</td>
                <td><input type="text" id="event1" name="EVENT1" size="15"
                           value="<?= $fields["EVENT1"] ?>" <?= $fields["EVENTS_disabled"] ?> /></td>
            </tr>
            <tr>
                <td>event2:</td>
                <td><input type="text" id="event2" name="EVENT2" size="15"
                           value="<?= $fields["EVENT2"] ?>" <?= $fields["EVENTS_disabled"] ?> /></td>
            </tr>
            <tr>
                <td>event3:</td>
                <td><input type="text" id="event3" name="EVENT3" size="15"
                           value="<?= $fields["EVENT3"] ?>" <?= $fields["EVENTS_disabled"] ?> /></td>
            </tr>
        <?
        endif;
        if ($old_module_version == "Y"):
            ...
            <tr>
                ...
                <td><?= SelectBoxFromArray("TEMPLATE", GetTemplateList(), $fields["TEMPLATE"]);
                    ?>&nbsp;[&nbsp;<a title="<?
                    echo GetMessage("VOTE_CHOOSE_TITLE") ?>"
                                      href="vote_preview.php?lang=<?= LANGUAGE_ID ?>&VOTE_ID=<?= $ID ?>"
                                      class="tablebodylink"><?= GetMessage("VOTE_CHOOSE") ?></a>&nbsp;]
                </td>
            </tr>
            <tr>
                <td><?= GetMessage("VOTE_RESULT_TEMPLATE") ?></td>
                <td><?
                    echo SelectBoxFromArray("RESULT_TEMPLATE", GetTemplateList("RV"), $fields["RESULT_TEMPLATE"]);
                    ?>&nbsp;[&nbsp;<a title="<?
                    echo GetMessage("VOTE_CHOOSE_RESULT_TITLE") ?>"
                                      href="vote_results.php?lang=<?= LANGUAGE_ID ?>&VOTE_ID=<?= $ID ?>"
                                      class="tablebodylink"><?= GetMessage("VOTE_CHOOSE") ?></a>&nbsp;]
                </td>
            </tr>
        <?
        ...
        ?>
        <tr class="heading">
            <td colspan="2"><?= \Bitrix\Main\Localization\Loc::getMessage("VOTE_LIMITS") ?></td>
        </tr>
        <? if (IsModuleInstalled('statistic')) { ?>
            <tr>
                <td><input type="checkbox" id="UNIQUE_TYPE_SESSION" name="UNIQUE_TYPE[]"
                           value="<?= EventLimits::BY_SESSION ?>" <?= ($uniqType & EventLimits::BY_SESSION) ? " checked" : "" ?> />
                </td>
                <td><label for="UNIQUE_TYPE_SESSION"><?= GetMessage("VOTE_UNIQUE_SESSION") ?></label></td>
            </tr>
        <? } ?>
        <tr>
            <td><input type="checkbox" id="UNIQUE_TYPE_COOKIE" name="UNIQUE_TYPE[]"
                       value="<?= EventLimits::BY_COOKIE ?>" <?= ($uniqType & EventLimits::BY_COOKIE) ? " checked" : "" ?> />
            </td>
            <td><label for="UNIQUE_TYPE_COOKIE"><?= GetMessage("VOTE_UNIQUE_COOKIE_ONLY") ?></label></td>
        </tr>
        <tr>
            <td><input type="checkbox" id="UNIQUE_TYPE_IP" name="UNIQUE_TYPE[]"
                       value="<?= EventLimits::BY_IP ?>" <?= ($uniqType & EventLimits::BY_IP) ? " checked" : "" ?> />
            </td>
        ...
        <tr>
            <td><input type="checkbox" id="UNIQUE_TYPE_USER_ID" name="UNIQUE_TYPE[]"
                       value="<?= EventLimits::BY_USER_ID ?>" <?= ($uniqType & EventLimits::BY_USER_ID) ? " checked" : "" ?> />
            </td>
        ...
        <tr>
            <td>&nbsp;</td>
            <td>
                <input type="checkbox" id="UNIQUE_TYPE_USER_ID_NEW" name="UNIQUE_TYPE[]"
                       value="<?= EventLimits::BY_USER_DATE_REGISTER ?>" <?= ($uniqType & EventLimits::BY_USER_DATE_REGISTER) ? " checked" : "" ?> />&nbsp;
                <label for="UNIQUE_TYPE_USER_ID_NEW"><?= GetMessage("VOTE_UNIQUE_ID_NEW_MSG") ?></label><br/>
            </td>
        </tr>
        <tr class="heading">
            <td colspan="2"><?= Loc::getMessage("VOTE_ADDITIONAL") ?></td>
        </tr>
        <tr>
            <td><input type="checkbox" id="OPTION_ALLOW_REVOTE" name="OPTIONS[]"
                       value="<?= Option::ALLOW_REVOTE ?>" <?= ($fields["OPTIONS"] & Option::ALLOW_REVOTE) ? " checked" : "" ?> />
            </td>
            <td><label for="OPTION_ALLOW_REVOTE"><?= Loc::getMessage("VOTE_ALLOW_CHANGE_BALLOT") ?></td>
        </tr>
        <tr style="display:none;">
            <td><input type="checkbox" id="OPTION_HIDE_RESULT" name="OPTIONS[]"
                       value="<?= Option::HIDE_RESULT ?>" <?= ($fields["OPTIONS"] & Option::HIDE_RESULT) ? " checked" : "" ?> />
            </td>
