...
$lAdmin->AddHeaders(
    array(
        ...
        array(
            "id" => "TIMESTAMP_X",
            "content" => GetMessage("VOTE_TIMESTAMP"),
            "sort" => "s_timestamp",
            "default" => true
        ),
        ...
        array(
            "id" => "SYMBOLIC_NAME",
            "content" => GetMessage("VOTE_SID"),
            "sort" => "s_symbolic_name",
            "default" => true
        ),
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    if (is_array($arrSITE)) {
        foreach ($arrSITE as $sid) {
            $str .= '<a title="' . GetMessage(
                    "VOTE_SITE_EDIT"
                ) . '" href="/bitrix/admin/site_edit.php?LID=' . $sid . '&lang=' . LANGUAGE_ID . '">' . $sid . '</a>, ';
        }
    };
    ...
    if ($VOTE_RIGHT == "W") {
        ...
        $row->AddViewField(
            "TITLE",
            '<a href="vote_channel_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $f_ID . '" title="' . GetMessage(
                "VOTE_EDIT_TITLE"
            ) . '">' . $f_TITLE . '</a>'
        );
    } else {
    ...
    $row->AddViewField(
        ...
        '<a title="' . GetMessage(
            "VOTE_OPEN_VOTES"
        ) . '" href="vote_list.php?lang=' . LANGUAGE_ID . '&find_channel=' . $f_ID . '&set_filter=Y">' . $f_VOTES . '</a>&nbsp;[<a title="' . GetMessage(
            "VOTE_ADD_VOTE"
        ) . '" href="vote_edit.php?CHANNEL_ID=' . $f_ID . '&lang=' . LANGUAGE_ID . '">+</a>]'
    );
    ...
    $arActions[] = array(
        ...
        "ACTION" => $lAdmin->ActionRedirect("vote_channel_edit.php?ID=" . $f_ID)
    );
    if ($f_ID != '1' && $VOTE_RIGHT == "W") {
        ...
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . GetMessage(
                    "VOTE_CONFIRM_DEL_CHANNEL"
                ) . "')) window.location='vote_channel_list.php?lang=" . LANGUAGE_ID . "&action=delete&ID=$f_ID&" . bitrix_sessid_get(
                ) . "'"
        );
    }
...
<form name="find_form" method="GET" action="<?= $APPLICATION->GetCurPage() ?>?">
    ...
    <tr valign="top">
        <td><?= GetMessage("VOTE_F_SITE") ?><br><img src="/bitrix/images/vote/mouse.gif" width="44" height="21" border=0
                                                     alt=""></td>
        <td><?
            $ref = array();
            $ref_id = array();
    ...
    <tr>
        ...
        <td nowrap><input type="text" name="find_sid" value="<? echo htmlspecialcharsbx($find_sid) ?>"
                          size="47"><?= InputType(
                "checkbox",
                "find_sid_exact_match",
                "Y",
