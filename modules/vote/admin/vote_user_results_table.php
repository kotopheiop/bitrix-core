...
if ($VOTE_RIGHT == "W" && $request->getRequestMethod() == "GET" && ($request->getQuery(
            "save"
        ) <> '' || $request->getQuery("apply") <> '') && check_bitrix_sessid()) {
...
<form name="form1" action="" method="get">
    ...
    <tr>
        ...
        <td> [<a class="tablebodylink" href="vote_edit.php?lang=<?= LANGUAGE_ID ?>&ID=<?= $arVote["ID"] ?>"
        ...
        <td><?
            if ($event["AUTH_USER_ID"] > 0) :
                ?>[<a class="tablebodylink" title="<?= GetMessage("VOTE_EDIT_USER") ?>"
                      href="user_edit.php?lang=<?= LANGUAGE_ID ?>&ID=<?= $event["AUTH_USER_ID"] ?>"><?= $event["AUTH_USER_ID"] ?></a>] (<?= $event["LOGIN"] ?>) <?= $event["AUTH_USER_NAME"] ?> [<?
                if (CModule::IncludeModule("statistic")) :
                    ?><a class="tablebodylink"
                         href="guest_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $event["STAT_GUEST_ID"] ?>&set_filter=Y"><?= $event["STAT_GUEST_ID"] ?></a><?
                else :
    ...
    <tr>
        ...
        <td><?
            if (CModule::IncludeModule("statistic")) :
                ?><a class="tablebodylink"
                     href="session_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $event["STAT_SESSION_ID"] ?>&set_filter=Y" ><?= $event["STAT_SESSION_ID"] ?></a><?
            else:
    ...
    $tabControl->Buttons(
        array(
            "disabled" => ($VOTE_RIGHT < "W"),
            "back_url" => "vote_user_votes_table.php?lang=" . LANGUAGE_ID . "&VOTE_ID=" . $VOTE_ID
        )
    );
    $tabControl->End();
    ?>
...
<tr>
    ...
    <td>
        <div class="lamp-<?= $arVote["LAMP"] ?>"></div>
    </td>
</tr>
<tr>
                                    ...
                                    switch ($arAnswer["FIELD_TYPE"]) :
                                        case 0:
                                            ...
                                            ?><input type="radio" name="<?= $field_name ?>"
                                                     value="<?= $arAnswer["ID"] ?>" <?= $checked ?>><font class="text">
                                            &nbsp;<?= $arAnswer["MESSAGE"] ?></font><?
                                            break;
                                        case 1:
                                        ...
                                        case 4:
                                            ...
                                            ?><? if (trim($arAnswer["MESSAGE"]) <> ''):?><font
                                                class="text"><?= $arAnswer["MESSAGE"] ?></font><br><?endif ?><input
