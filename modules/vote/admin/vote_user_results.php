...
if (!($event = CVoteEvent::GetByID($EVENT_ID))) {
    ...
    ?><a href="vote_user_list.php?lang=<?= LANGUAGE_ID ?>" class="navchain"><?= GetMessage("VOTE_USER_LIST") ?></a><font
            class="navchain">&nbsp;&raquo&nbsp;</font><a href="vote_user_votes.php?lang=<?= LANGUAGE_ID ?>"
                                                         class="navchain"><?= GetMessage("VOTE_RESULTS_LIST") ?></a><?
    ...
    <form name="form1" action="" method="get">
        ...
        <tr>
            ...
            <td><?
                if ($str_AUTH_USER_ID > 0) :
                    ?>[<a class="tablebodylink" title="<?= GetMessage("VOTE_EDIT_USER") ?>"
                          href="user_edit.php?lang=<?= LANGUAGE_ID ?>&ID=<?= $str_AUTH_USER_ID ?>"><?
                    echo $str_AUTH_USER_ID ?></a>] (<?= $str_LOGIN ?>) <?= $str_AUTH_USER_NAME ?> [<?
                    if (CModule::IncludeModule("statistic")) :
                        ?><a class="tablebodylink"
                             href="guest_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $str_STAT_GUEST_ID ?>&set_filter=Y"><?= $str_STAT_GUEST_ID ?></a><?
                    else :
        ...
        <tr>
            ...
            <td><?
                if (CModule::IncludeModule("statistic")) :
                    ?><a class="tablebodylink"
                         href="session_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $str_STAT_SESSION_ID ?>&set_filter=Y" ><?= $str_STAT_SESSION_ID ?></a><?
                else:
    ...
    <table cellspacing=0 cellpadding=0 width="100%">
        <tr>
            <td width="100%"><?
                ...
                ?><font class="h2"><b><?
                        echo $arVote["TITLE"]; ?></b></font><br><img src="/bitrix/images/1.gif" width="1" height="6"
                                                                     border=0 alt=""><?
                endif;
                ?><font class="smalltext"><?
                    if ($arVote["DATE_START"]):
                        ?><br><?= GetMessage("VOTE_START_DATE") ?>&nbsp;<?
                        echo $arVote["DATE_START"] ?><?
                    endif;
                    if ($arVote["DATE_END"] && $arVote["DATE_END"] != "31.12.2030 23:59:59"):
                        ?><br><?= GetMessage("VOTE_END_DATE") ?>&nbsp;<?
                        echo $arVote["DATE_END"] ?><?
                    endif;
                                            ...
                                            <td colspan=2><?
                                                switch ($arAnswer["FIELD_TYPE"]) :
                                                    ...
                                                    case 5:
                                                        ...
                                                        ?><font class="text"><?
                                                        if (strlen(trim($arAnswer["MESSAGE"])) > 0) echo $arAnswer["MESSAGE"] . "<br>" ?></font>
                                                        <textarea
