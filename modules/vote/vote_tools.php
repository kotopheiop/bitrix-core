...
function GetVoteDataByID(
    ...
    $GLOBALS["VOTE_CACHE_VOTING"][$VOTE_ID] = (is_array(
        $GLOBALS["VOTE_CACHE_VOTING"][$VOTE_ID]
    ...
    if (empty($GLOBALS["VOTE_CACHE_VOTING"][$VOTE_ID])) {
        ...
        if (!empty($arQuestions)) {
            ...
            while ($res = $db_res->GetNext()) {
                ...
                {
                    case 2:
                        $arDropDown[$res["QUESTION_ID"]] = (is_array(
                            $arDropDown[$res["QUESTION_ID"]]
                        ) ? $arDropDown[$res["QUESTION_ID"]] :
                        ...
                        break;
                    case 3:
                        $arMultiSelect[$res["QUESTION_ID"]] = (is_array(
                            $arMultiSelect[$res["QUESTION_ID"]]
                        ) ? $arMultiSelect[$res["QUESTION_ID"]] :
                            array("reference" => array(), "reference_id" => array(), "~reference" => array()));
            ...
            $event_id = intval(
                $arAddParams["bRestoreVotedData"] == "Y" && !!$_SESSION["VOTE"]["VOTES"][$VOTE_ID] ?
                    $_SESSION["VOTE"]["VOTES"][$VOTE_ID] : 0
            );
            if ($event_id > 0) {
                $db_res = CVoteEvent::GetUserAnswerStat(
                    $VOTE_ID,
                    array("bGetMemoStat" => "N", "bGetEventResults" => $event_id)
                );
                if ($db_res && ($res = $db_res->Fetch())) {
                    do {
                        if (isset($arQuestions[$res["QUESTION_ID"]]) && is_array(
                                $arQuestions[$res["QUESTION_ID"]]["ANSWERS"][$res["ANSWER_ID"]]
                            ) && is_array($res)) {
                            ...
                            if ($event_id > 0 && !empty($res["RESTORED_ANSWER_ID"])) {
                                switch ($arQuestions[$res["QUESTION_ID"]]["ANSWERS"][$res["ANSWER_ID"]]["FIELD_TYPE"]):
                                    ...
                                    case 3: // multiselect list
                                        ...
                                        $_REQUEST[$fieldName] = (is_array(
                                            $_REQUEST[$fieldName]
                                        ) ? $_REQUEST[$fieldName] : array());
...
{
    $z = CVoteChannel::GetList(
        '',
        '',
        array("SID" => $GROUP_SID, "SID_EXACT_MATCH" => "Y", "SITE" => $site_id, "ACTIVE" => "Y")
    );
    if ($zr = $z->Fetch()) {
...
function ShowCurrentVoteResults($GROUP_SID, $site_id = SITE_ID)
{
    ...
    if (intval($CURRENT_VOTE_ID) > 0) {
        ShowVoteResults($CURRENT_VOTE_ID);
    }
}
...
function ShowVote($VOTE_ID, $template1 = "")
{
    ...
    if ($USER_GROUP_PERMISSION > 2) {
        $USER_GROUP_PERMISSION = 0;
    }

    $VOTE_ID = GetVoteDataByID(
        $VOTE_ID,
        $arChannel,
        $arVote,
        $arQuestions,
        ...
        if (intval($perm) >= 2) {
            ...
            if ($APPLICATION->GetShowIncludeAreas()) {
                ...
                if (CModule::IncludeModule("fileman")) {
                    $arIcons[] = Array(
                        "URL" => "/bitrix/admin/fileman_file_edit.php?lang=" . LANGUAGE_ID . "&site=" . SITE_ID . "&full_src=Y&path=" . urlencode(
                                $path . $template
                            ),
...
function fill_arc($start, $end, $color)
{
    ...
    imageline(
        ...
        $centerX,
        $centerY,
        $centerX + cos(deg2rad($start)) * $radius,
        $centerY + sin(deg2rad($start)) * $radius,
        $color
    );
    imageline(
        $im,
        $centerX,
        $centerY,
        $centerX + cos(deg2rad($end)) * $radius,
        $centerY + sin(deg2rad($end)) * $radius,
        $color
    );
...
{
    if (mb_substr($start_color, 0, 1) == "#") {
        $start_color = mb_substr($start_color, 1, 6);
    }
    if (mb_substr($end_color, 0, 1) == "#") {
        $end_color = mb_substr($end_color, 1, 6);
    }
    if (mb_substr($current_color, 0, 1) == "#") {
    ...
    if ($current_color == '') {
        ...
        if ($dec < hexdec($start_color)) {
            $dec = $start_color;
        } elseif ($dec > hexdec($end_color)) {
            $dec = $end_color;
