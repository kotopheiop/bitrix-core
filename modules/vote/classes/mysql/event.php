        ...
        foreach ($arFilter as $key => $val) {
            ...
            switch ($key) {
                ...
                case "AUTH_USER_ID":
                    ...
                    if (!empty($val)) {
                        $str = ($strNegative == "Y" ? " " . $key . " IS NULL OR NOT " : "") . "(" . $key . " " . $strOperation . " " . intval(
                                $val
                            ) . ")";
                        if ($strOperation == "IN") {
                            $val = array_unique(
                            ...
                            if (!empty($val)) {
                                $str = ($strNegative == "Y" ? " NOT " : "") . "(" . $key . " IN (" . implode(
                                        ",",
                                        $val
                                    ) . "))";
                            }
                ...
                case "VISIBLE":
                    if (empty($val)) {
                        ...
                        $arSqlSearch[] = ($strNegative == "Y" ? " VE." . $key . " IS NULL OR NOT " : "") . "(VE." . $key . " " . $strOperation . " '" . $DB->ForSql(
                                $val
                            ) . "' )";
                    }
                    break;
        ...
        if (isset($arFilter["bGetVoters"])) {
            ...
            if ($db_res && ($res = $db_res->Fetch())) {
                $strSql = "SELECT VEQ.QUESTION_ID, VEA.ANSWER_ID, VU.AUTH_USER_ID, COUNT(DISTINCT VEA.ID) as COUNTER, \n\t" .
                    ...
                    "INNER JOIN b_vote_event_answer VEA ON (VEA.EVENT_QUESTION_ID = VEQ.ID) \n\t" .
                    "LEFT JOIN b_vote_user VU ON (VU.ID = VE.VOTE_USER_ID)\n\t" .
                    "LEFT JOIN b_rating_user RV ON (RV.ENTITY_ID = VU.AUTH_USER_ID AND RV.RATING_ID = " . intval(
                    ...
                    " ORDER BY " . (IsModuleInstalled(
                        "intranet"
                    ) ? "RV.VOTE_WEIGHT DESC, `RANK` DESC" : "`RANK` DESC, RV.VOTE_WEIGHT DESC") . ", VU.AUTH_USER_ID ASC";
