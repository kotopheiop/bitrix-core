...
class Basket
{
    ...
    public static function addProduct(array $product, array $basketFields = [], array $options = [])
    {
        ...
        if ($productId <= 0) {
            $result->addError(
                new Main\Error(Main\Localization\Loc::getMessage('BX_CATALOG_PRODUCT_BASKET_ERR_NO_PRODUCT'))
            );
            return $result;
        }
     ...
     * @throws Main\ObjectNotFoundException
     */
    public static function addProductToBasketWithPermissions(
        Sale\BasketBase $basket,
        array $fields,
        array $context,
        $options = true
    ...
    private static function add(Sale\BasketBase $basket, array $fields, array $context, array $options = [])
    {
        ...
        if ($module == 'catalog') {
            ...
            if ($productFields['TYPE'] == Catalog\ProductTable::TYPE_OFFER) {
                ...
                if (empty($skuInfo)) {
                    $result->addError(
                        new Main\Error(Loc::getMessage('BX_CATALOG_PRODUCT_BASKET_ERR_PRODUCT_BAD_TYPE'))
                    );
                    return $result;
                } else {
            ...
            if ($productFields['TYPE'] == Catalog\ProductTable::TYPE_SET) {
                ...
                if (!empty($itemIds)) {
                    $result->addError(
                        new Main\Error(Loc::getMessage('BX_CATALOG_PRODUCT_BASKET_ERR_NO_PRODUCT_SET_ITEMS'))
                    );
                    return $result;
                }
            }
