...
class DiscountManager
{
    ...
    public static function calculateApplyCoupons(array $couponsList, array $basket, array $params): array
    {
        ...
        foreach ($filteredBasket as $basketCode => $basketItem) {
            ...
            $itemsDiscountResult = \CCatalogDiscount::applyDiscountList(
                $basketItem['PRICE'],
                $basketItem['CURRENCY'],
                $itemDiscounts
            );
    ...
    protected static function loadFromDatabase(int $id, array $discount)
    {
        ...
        if (empty($discount['MODULES'])) {
            ...
            $moduleIterator = $conn->query(
                'select MODULE_ID from ' . $helper->quote('b_catalog_discount_module') . ' where ' . $helper->quote(
                    'DISCOUNT_ID'
                ) . ' = ' . $id
            );
    ...
    protected static function prepareEntity($entityList)
    {
        ...
        if (!empty($entityList['catalog'])) {
            ...
            if (!empty($entityList['catalog']['ELEMENT_PROPERTY']) && is_array(
                    $entityList['catalog']['ELEMENT_PROPERTY']
                )) {
     ...
     * @return void
     */
    protected static function fillEmptyProperties(
    ...
    protected static function fillProperties(
        array &$productData,
        array $productIds,
        array $iblockData,
        array $entityData
    ...
    protected static function createSaleAction(&$discount, $params): void
    {
        ...
        $action = '\Bitrix\Catalog\Discount\DiscountManager::applyDiscount(' . $params['BASKET_ITEM'] . ', ' . var_export(
                $data,
                true
            ) . ');';
