    ...
    for ($i = 0, $intCount = count($arChld); $i < $intCount; $i++) {
        ...
        if (!$res) {
            $strImportErrorMessage .= str_replace(
                    ...
                    str_replace(
                        "#NAME#",
                        "[" . $GROUP_ID . "] \"" . $GROUP_NAME . "\" (" . $GROUP_XML_ID . ")",
                        GetMessage("CICML_ERROR_ADD_SECTION")
                    )
                ) . ".<br>";
            $STT_GROUP_ERROR++;
        } else {
...
if ('' === $strImportErrorMessage) {
    ...
    if (true) {
        $xml_content = $GLOBALS['APPLICATION']->ConvertCharset(
            $xml_content,
            $CONVERT_UTF8 == "Y" ? "utf-8" : "windows-1251",
            LANG_CHARSET
        );
...
if ('' === $strImportErrorMessage) {
    ...
    for ($i = 0, $intOFCount = count($offerlists); $i < $intOFCount; $i++) {
        ...
        for ($j = 0, $intPropCount = count($props); $j < $intPropCount; $j++) {
            $arPriceType[$props[0]->GetAttribute($nameUTF['CatalogID'])] = $props[0]->GetAttribute(
                $nameUTF['PropertyId']
            );
            break;
        }
    ...
    for ($i = 0, $intCatCount = count($catalogs); $i < $intCatCount; $i++) {
        ...
        if (!$res) {
            $strImportErrorMessage .= str_replace(
                    ...
                    str_replace(
                        "#NAME#",
                        "[" . $IBLOCK_ID . "] \"" . $IBLOCK_NAME . "\" (" . $IBLOCK_XML_ID . ")",
                        GetMessage("CICML_ERROR_ADDING_CATALOG")
                    )
                ) . ".<br>";
            $STT_CATALOG_ERROR++;
        } else {
            ...
            for ($j = 0, $intPropCount = count($props); $j < $intPropCount; $j++) {
                ...
                $res = CIBlock::GetProperties(
                    $IBLOCK_ID,
                    array(),
                    Array("IBLOCK_ID" => $IBLOCK_ID, "XML_ID" => $PROP_XML_ID)
                );
                $bNewRecord_tmp = false;
                if ($res_arr = $res->Fetch()) {
                    $PROP_ID = $res_arr["ID"];
                ...
                if (!$res) {
                    $strImportErrorMessage .= str_replace(
                            ...
                            str_replace(
                                "#NAME#",
                                "[" . $PROP_ID . "] \"" . $PROP_NAME . "\" (" . $PROP_XML_ID . ")",
                                GetMessage("CICML_ERROR_ADD_PROPS")
                            )
                        ) . ".<br>";
                    $STT_PROP_ERROR++;
                } else {
            ...
            for ($j = 0, $intProdCount = count($products); $j < $intProdCount; $j++) {
                ...
                if (!$res) {
                    $strImportErrorMessage .= str_replace(
                            ...
                            str_replace(
                                "#NAME#",
                                "[" . $PRODUCT_ID . "] \"" . $PRODUCT_NAME . "\" (" . $PRODUCT_XML_ID . ")",
                                GetMessage("CICML_ERROR_ADDING_PRODUCT")
                            )
                        ) . ".<br>";
                    $STT_PRODUCT_ERROR++;
                } else {
...
//	$arProductsTmp = $arCatalogsParams[$IBLOCK_XML_ID]["arProducts"];
    ...
    for ($i = 0, $intOFListCount = count($offerlists); $i < $intOFListCount; $i++) {
        ...
        for ($j = 0, $intOtOFCount = count($offers); $j < $intOtOFCount; $j++) {
            ...
            if (!isset($arProducts[$PRODUCT_XML_ID])) {
                ...
                $db_res_tmp = CIBlockElement::GetList(
                    array(),
                    Array(
                        "XML_ID" => $PRODUCT_XML_ID,
                        "IBLOCK_ID" => $arIBlockCacheTmp[$IBLOCK_XML_ID]
                    )
                );
                if ($ar_res_tmp = $db_res_tmp->Fetch()) {
    ...
    $strImportOKMessage .= str_replace(
            "#TIME#",
            RoundEx(getmicrotime() - $startImportExecTime, 2),
            GetMessage("CICML_LOAD_TIME")
        ) . "<br>";
    $strImportOKMessage .= str_replace(
            "#NUM#",
            ($STT_CATALOG_UPDATE + $STT_CATALOG_ADD),
            GetMessage("CICML_LOAD_CATALOG")
        ) . " ";
    $strImportOKMessage .= str_replace(
            "#NUM_UPD#",
            $STT_CATALOG_UPDATE,
    ...
    $strImportOKMessage .= str_replace(
            "#NUM#",
            ($STT_GROUP_UPDATE + $STT_GROUP_ADD),
            GetMessage("CICML_LOAD_GROUP")
        ) . " ";
    $strImportOKMessage .= str_replace(
            "#NUM_UPD#",
            $STT_GROUP_UPDATE,
    ...
    $strImportOKMessage .= str_replace(
            "#NUM#",
            ($STT_PROP_UPDATE + $STT_PROP_ADD),
            GetMessage("CICML_LOAD_PROPS")
        ) . " ";
    $strImportOKMessage .= str_replace(
            "#NUM_UPD#",
            $STT_PROP_UPDATE,
    ...
    $strImportOKMessage .= str_replace(
            "#NUM#",
            ($STT_PRODUCT_UPDATE + $STT_PRODUCT_ADD),
            GetMessage("CICML_LOAD_PROD")
        ) . " ";
    $strImportOKMessage .= str_replace(
            "#NUM_UPD#",
            $STT_PRODUCT_UPDATE,
