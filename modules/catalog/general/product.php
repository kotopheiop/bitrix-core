...
class CAllCatalogProduct
{
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if ((is_set(
                    $arFields,
                    "QUANTITY_TRACE"
                ) || $ACTION == "ADD") && ($arFields["QUANTITY_TRACE"] != "Y" && $arFields["QUANTITY_TRACE"] != "N")) {
            $arFields["QUANTITY_TRACE"] = "D";
        }
        if ((is_set(
                    $arFields,
                    "CAN_BUY_ZERO"
                ) || $ACTION == "ADD") && ($arFields["CAN_BUY_ZERO"] != "Y" && $arFields["CAN_BUY_ZERO"] != "N")) {
        ...
        if ((is_set(
                    $arFields,
                    "PRICE_TYPE"
                ) || $ACTION == "ADD") && ($arFields["PRICE_TYPE"] != "R") && ($arFields["PRICE_TYPE"] != "T")) {
        ...
        if ((is_set(
                    $arFields,
                    "RECUR_SCHEME_TYPE"
                ) || $ACTION == "ADD") && ($arFields["RECUR_SCHEME_TYPE"] == '' || !in_array(
                    $arFields["RECUR_SCHEME_TYPE"],
                    Catalog\ProductTable::getPaymentPeriods(false)
                ))) {
            $arFields["RECUR_SCHEME_TYPE"] = self::TIME_PERIOD_DAY;
        }

        if ((is_set($arFields, "RECUR_SCHEME_LENGTH") || $ACTION == "ADD") && (intval(
                    $arFields["RECUR_SCHEME_LENGTH"]
                ) <= 0)) {
            ...
            if ($existPurchasingPrice) {
                ...
                if ($purchasingPrice !== false) {
                    ...
                    if ($purchasingCurrency === false) {
                        $arMsg[] = array(
                            'id' => 'PURCHASING_CURRENCY',
                            'text' => Loc::getMessage('BT_MOD_CATALOG_PROD_ERR_COST_CURRENCY')
                        );
                        $boolResult = false;
                    }
                }
                ...
                if (
                    ...
                    if (
                        ...
                        if ($needCalculateAvailable && !empty($needFields)) {
                            $product = $productIterator = Catalog\ProductTable::getList(
                                array(
                                    'select' => $needFields,
                                    'filter' => array('=ID' => $ID)
                                )
                    ...
                    if (!empty($offerList[$ID])) {
                        $skuAvailable = false;
                        $offerIterator = Catalog\ProductTable::getList(
                            array(
...
//		$boolDiscountVat = ('N' != COption::GetOptionString('catalog', 'discount_vat', 'Y'));
    ...
    public static function GetOptimalPriceList(
        array $products,
        $arUserGroups = array(),
        $renewal = "N",
        $priceList = array(),
        ...
        foreach ($priceList as $priceData) {
                ...
                if ($lastBasketCode !== false) {
                    foreach (GetModuleEvents('catalog', 'OnGetOptimalPriceResult', true) as $arEvent) {
                        if (ExecuteModuleEventEx(
                                $arEvent,
                                array(&$resultList[$lastProductId][$lastBasketCode])
                            ) === false) {
                            continue;
                        }
                    }
            ...
            if ($isNeedDiscounts) {
                ...
                $discountResult = \CCatalogDiscount::applyDiscountList(
                    $currentPrice,
                    $resultCurrency,
                    $discountList[$priceData['PRODUCT_ID']]
                );
                if ($discountResult === false) {
                ...
                foreach ($arDiscSave as $strDiscountKey => $arOneDiscount) {
                    ...
                    switch ($arOneDiscount['VALUE_TYPE']) {
                        case CCatalogDiscountSave::TYPE_PERCENT:
                            $dblPriceTmp = round(
                                $dblCurrentPrice * (1 - $arOneDiscount['VALUE'] / 100.0),
                                CATALOG_VALUE_PRECISION
                            );
                            break;
    ...
    private static function getPossibleSalePrice(
        $intProductID,
        array $priceData,
        $quantity,
        $siteID,
    ...
    private static function normalizeFields(array &$fields)
    {
        ...
        if (isset($fields['QUANTITY_RESERVED']) && is_string(
