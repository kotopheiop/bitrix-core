...
class CCatalogAdmin
{
    ...
    public static function get_other_elements_menu($IBLOCK_TYPE_ID, $IBLOCK_ID, $arSection, &$more_url)
    {
        ...
        $urlSectionAdminPage = CIBlock::GetAdminSectionListLink(
            $IBLOCK_ID,
            array('catalog' => null, "skip_public" => true)
        );
        ...
        $more_url[] = CIBlock::GetAdminSectionEditLink(
            ...
            array(
                'catalog' => null,
                "find_section_section" => $arSection["ID"]
            )
        );
        $more_url[] = CIBlock::GetAdminSectionEditLink(
            $IBLOCK_ID,
    ...
    public static function get_sections_menu(
        $IBLOCK_TYPE_ID,
        $IBLOCK_ID,
        $DEPTH_LEVEL,
        $SECTION_ID,
        ...
        $baseUrlSectionAdminPage = CIBlock::GetAdminSectionListLink(
            $IBLOCK_ID,
            array("catalog" => null, "skip_public" => true)
        );
        ...
        while ($arSection = $rsSections->Fetch()) {
            ...
            if ($limit > 0 && $intCount >= $limit) {
                if (empty($arOtherSectionTmp)) {
                    ...
                    $arOtherSectionTmp = array(
                        ...
                        "more_url" => array(
                            ...
                            CIBlock::GetAdminElementListLink(
                                $IBLOCK_ID,
                                array("find_section_section" => $arSection["ID"])
                            ),
                    ...
                    CCatalogAdmin::get_other_elements_menu(
                        $IBLOCK_TYPE_ID,
                        $IBLOCK_ID,
                        $arSection,
                        $arOtherSectionTmp["more_url"]
                    );
                } else {
                    $arOtherSectionTmp['more_url'][] = $baseUrlSectionAdminPage . "&find_section_section=" . $arSection["ID"] . "&SECTION_ID=" . $arSection["ID"];
                    $arOtherSectionTmp['more_url'][] = CIBlock::GetAdminElementEditLink(
                        $IBLOCK_ID,
                        0,
                        array("find_section_section" => $arSection["ID"])
                    );
                    $arOtherSectionTmp['more_url'][] = CIBlock::GetAdminSectionEditLink(
                        $IBLOCK_ID,
                        0,
                        array('catalog' => null)
                    );
                ...
                if (isset($arSectionsChain[$arSection["ID"]])) {
                    $arSectionTmp["items"] = CCatalogAdmin::get_sections_menu(
                        $IBLOCK_TYPE_ID,
                        $IBLOCK_ID,
                        $DEPTH_LEVEL + 1,
                        $arSection["ID"],
    ...
    public static function OnBuildGlobalMenu(
        /** @noinspection PhpUnusedParameterInspection */ &$aGlobalMenu,
        &$aModuleMenu
    ) {
        ...
        while ($arIBlock = $rsIBlocks->Fetch()) {
            ...
            $arItems[] = [
                ...
                "url" => ($mixedList
                    ? $url . "?IBLOCK_ID=" . $arIBlock["ID"] . "&type=" . urlencode(
                        $arIBlock["IBLOCK_TYPE_ID"]
                    ) . '&lang=' . LANGUAGE_ID . '&find_section_section=0&SECTION_ID=0&apply_filter=Y'
                    : $url . "?IBLOCK_ID=" . $arIBlock["ID"] . "&type=" . urlencode(
                        $arIBlock["IBLOCK_TYPE_ID"]
                    ) . '&lang=' . LANGUAGE_ID . '&find_section_section=-1'
                ),
                "more_url" => [
                    $url . "?IBLOCK_ID=" . $arIBlock["ID"] . "&type=" . urlencode(
                        $arIBlock["IBLOCK_TYPE_ID"]
            ...
            if ($publicMenu) {
                $text = ($totalCount > 1 ? htmlspecialcharsEx($arIBlock["NAME"]) : Loc::getMessage(
                    "CAT_MENU_ROOT_TITLE"
                ));
            } else {
            ...
            $aMenu["items"][] = array(
                ...
                "module_id" => "catalog",
                "items" => $arItems,
                "url" => $url . "?lang=" . LANGUAGE_ID . "&IBLOCK_ID=" . $arIBlock["ID"] . "&type=" . urlencode(
                        $arIBlock["IBLOCK_TYPE_ID"]
    ...
    public static function OnBuildSaleMenu(
        /** @noinspection PhpUnusedParameterInspection */ &$arGlobalMenu,
        &$arModuleMenu
    ) {
        global $adminMenu;
    ...
    protected static function OnBuildSaleMenuItem(&$arMenu)
    {
        ...
        foreach ($arMenu as &$arMenuItem) {
            ...
            if (!isset($arMenuItem['items_id']) || !is_string($arMenuItem['items_id']) || !in_array(
                    $arMenuItem['items_id'],
                    $arMenuID
                )) {
                continue;
    ...
    protected static function OnBuildSaleSettingsMenu(&$arItems)
    {
        ...
        $showExtra = (Catalog\Config\Feature::isMultiPriceTypesEnabled(
            ) && (self::$catalogRead || self::$catalogExtra));
            ...
            while ($arProfile = $rsProfiles->Fetch()) {
                ...
                if ($arProfile['DEFAULT_PROFILE'] == 'Y') {
                    $arProfileList[] = array(
                        ...
                        "title" => Loc::getMessage("CAM_EXPORT_DESCR_EXPORT") . " &quot;" . htmlspecialcharsbx(
                                $strName
                            ) . "&quot;",
                    ...
                    $arProfileList[] = array(
                        ...
                        "title" => Loc::getMessage("CAM_EXPORT_DESCR_EDIT") . " &quot;" . htmlspecialcharsbx(
                                $strName
                            ) . "&quot;",
            ...
            while ($arProfile = $rsProfiles->Fetch()) {
                ...
                if ($arProfile['DEFAULT_PROFILE'] == 'Y') {
                    $arProfileList[] = array(
                        ...
                        "title" => Loc::getMessage("CAM_IMPORT_DESCR_IMPORT") . " &quot;" . htmlspecialcharsbx(
                                $strName
                            ) . "&quot;",
                    ...
                    $arProfileList[] = array(
                        ...
                        "title" => Loc::getMessage("CAM_IMPORT_DESCR_EDIT") . " &quot;" . htmlspecialcharsbx(
                                $strName
                            ) . "&quot;",
