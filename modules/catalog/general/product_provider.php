...
class CCatalogProductProvider implements IBXSaleProductProvider
{
    ...
    public static function GetProductData($arParams)
    {
        ...
        $arParams['CURRENCY'] = (isset($arParams['CURRENCY']) ? Currency\CurrencyManager::checkCurrencyID(
            $arParams['CURRENCY']
        ) : false);
        ...
        if (empty(self::$catalogList[$arProduct['IBLOCK_ID']]) || !is_array(
        ...
        if ($arParams["CHECK_PRICE"] == "Y") {
            ...
            if ($arParams['CHECK_COUPONS'] == 'Y') {
                if ($useSaleDiscountOnly) {
                    ...
                    $arCoupons = DiscountCouponsManager::getForApply(
                        array('MODULE_ID' => 'catalog'),
                        $productHash,
                        true
                    );
                }
            ...
            if (empty($arPrice['RESULT_PRICE']) || !is_array($arPrice['RESULT_PRICE'])) {
                $arPrice['RESULT_PRICE'] = CCatalogDiscount::calculateDiscountList(
                    $arPrice['PRICE'],
                    $arParams['CURRENCY'],
                    $arDiscountList,
                    true
...
//			"AVAILABLE_QUANTITY" => $arCatalogProduct["QUANTITY"],
    ...
    public static function ReserveProduct($arParams)
    {
        ...
        if (empty($arProduct)) {
            $APPLICATION->ThrowException(
                Loc::getMessage("RSRV_ID_NOT_FOUND", array("#PRODUCT_ID#" => $arParams["PRODUCT_ID"])),
                "ID_NOT_FOUND"
            );
            $arRes["RESULT"] = false;
            return $arRes;
        ...
        if (
            ...
            $APPLICATION->ThrowException(
                Loc::getMessage("RSRV_SKU_FOUND", array("#PRODUCT_ID#" => $arParams["PRODUCT_ID"])),
                "SKU_FOUND"
            );
            $arRes["RESULT"] = false;
            return $arRes;
        ...
        if ($arRes["RESULT"]) {
            ...
            $APPLICATION->ThrowException(
                Loc::getMessage("RSRV_UNKNOWN_ERROR", self::GetProductCatalogInfo($arParams["PRODUCT_ID"])),
    ...
    public static function DeductProduct($arParams)
    {
                    ...
                    if ($useStoreControl) {
                        if ($isOrderConverted != 'N' && empty($arParams["STORE_DATA"]) && $basketItem) {
                            if (static::canProductAutoShip($basketItem)) {
                                $arParams["STORE_DATA"] = static::getProductStoreData(
                                    $basketItem,
                                    $arParams["QUANTITY"]
                                );
                            }
                                            ...
                                            if (strval(trim($barcodeValue)) == "") {
                                                $APPLICATION->ThrowException(
                                                    Loc::getMessage(
                                                        ...
                                                        array_merge(
                                                            self::GetProductCatalogInfo($arParams["PRODUCT_ID"]),
                                                            array("#STORE_ID#" => $arRecord['STORE_ID'])
                                                        )
                                                    ),
                                                    "DDCT_DEDUCTION_MULTI_BARCODE_EMPTY"
                                                );
                                ...
                                if ($arProp = $rsProps->Fetch()) {
                                    if ($arProp["AMOUNT"] < $arRecord["QUANTITY"]) {
                                        ...
                                        if ($res) {
                                            ...
                                            //deleting barcodes
                                            if (isset($arRecord["BARCODE"]) && is_array($arRecord["BARCODE"]) && count(
                                                    $arRecord["BARCODE"]
                                                ) > 0) {
                                                foreach ($arRecord["BARCODE"] as $barcodeId => $barcodeValue) {
                ...
                if ($arParams["EMULATE"] == "Y" || $arProduct["QUANTITY_TRACE"] == "N") {
                    ...
                    if ($useStoreControl) {
                        if ($isOrderConverted != 'N' && empty($arParams["STORE_DATA"]) && $basketItem) {
                            ...
                            if (empty($arParams["STORE_DATA"])) {
                                $arParams["STORE_DATA"] = static::getProductOneStoreData(
                                    $basketItem,
                                    $arParams["QUANTITY"]
                                );
                            }
                                ...
                                if ($arProp = $rsProps->Fetch()) {
                                    ...
                                    if ($res) {
                                        ...
                                        if (isset($arRecord["BARCODE"])) {
                                            if (!empty($arRecord["BARCODE"]) && is_array($arRecord["BARCODE"])) {
                                                foreach ($arRecord["BARCODE"] as $barcodeValue) {
                                                    if (strval(trim($barcodeValue)) == '' || (strval(
                                                                trim($barcodeValue)
                                                            ) != '' && !$barcodeMulti)) {
                                                        continue;
                                                    }
        ...
        if (!$arRes["RESULT"]) {
            $APPLICATION->ThrowException(
                Loc::getMessage("DDCT_UNKNOWN_ERROR", self::GetProductCatalogInfo($arParams["PRODUCT_ID"])),
     ...
     */
    public static function tryShipmentProduct(
        ...
        $arProduct = Catalog\ProductTable::getList(
            array(
        ...
        if (empty($arProduct)) {
            $result->addError(
                new \Bitrix\Sale\ResultError(
                    Loc::getMessage("DDCT_DEDUCTION_PRODUCT_NOT_FOUND_ERROR", self::GetProductCatalogInfo($productId)),
                    "DDCT_DEDUCTION_PRODUCT_NOT_FOUND_ERROR"
                )
            );
            return $result;
        }
                    ...
                    if (intval($storeId) == -1) {
                        ...
                        if ($arProp = $rsProps->Fetch()) {
                            if ($arProp["AMOUNT"] < $basketStore["QUANTITY"]) {
                                ...
                                //check barcodes
                                if (isset($basketStore["BARCODE"]) && is_array($basketStore["BARCODE"]) && count(
                                        $basketStore["BARCODE"]
                                    ) > 0) {
                                                ...
                                                $result->addError(
                                                    new \Bitrix\Sale\ResultError(
                                                        Loc::getMessage(
                                                            ...
                                                            array_merge(
                                                                self::GetProductCatalogInfo($productId),
                                                                array("#STORE_ID#" => $basketStore['STORE_ID'])
                                                            )
                                                        ),
                                                        "DDCT_DEDUCTION_MULTI_BARCODE_EMPTY"
                                                    )
...
//				$arRes["RESULT"] = true;
    ...
    public static function GetSetItems($productID, $intType, $arProducInfo = array())
    {
        ...
        if (!isset($proxyCatalogProductSet[$proxyCatalogProductSetKey])) {
            $proxyCatalogProductSet[$proxyCatalogProductSetKey] = CCatalogProductSet::getAllSetsByProduct(
                $productID,
                $intType
            );
        }
        ...
        if (is_array($arSets)) {
            foreach ($arSets as $k => $arSet) {
                foreach ($arSet["ITEMS"] as $k1 => $item) {
                    ...
                    if (!empty($proxyCatalogSkuData[$item["ITEM_ID"]]) && is_array(
                    ...
                    if (!empty($arParentSku)) {
                        ...
                        $proxyProductPropertyKey = $item["ITEM_ID"] . "_" . $arParentSku["IBLOCK_ID"] . "_" . md5(
                                join('|', $arPropsSku)
                            );
                    ...
                    foreach ($arSets as $k => $arSet) {
                        foreach ($arSet["ITEMS"] as $k1 => $item) {
                            if ($item["ITEM_ID"] == $arProduct["ID"]) {
                                ...
                                if (!empty($proxyCatalogSkuData[$item["ITEM_ID"]]) && mb_strpos(
                                        $arProduct["XML_ID"],
                                        '#'
                                    ) === false) {
                                    $arParentSku = $proxyCatalogSkuData[$item["ITEM_ID"]];
                                    if (!empty($proxyParentData[$arParentSku['ID']]) && is_array(
                                            $proxyParentData[$arParentSku['ID']]
                                        )) {
                                        $parentData = $proxyParentData[$arParentSku['ID']];
                                    } else {
                                        $parentIterator = \Bitrix\Iblock\ElementTable::getList(
                                            array(
                                ...
                                $arSets["$k"]["ITEMS"][$k1]["PROPS"] = array_merge(
                                    $arSets["$k"]["ITEMS"][$k1]["PROPS"],
                                    $arProps
                                );
                            }
                        }
    ...
    protected static function isNeedClearPublicCache(
        $currentQuantity,
        $newQuantity,
        $quantityTrace,
        $canBuyZero,
    ...
    protected static function clearPublicCache($productID, $productInfo = array())
    {
        ...
        $iblockID = (int)(isset($productInfo['IBLOCK_ID']) ? $productInfo['IBLOCK_ID'] : CIBlockElement::GetIBlockByID(
            $productID
        ));
        if ($iblockID <= 0) {
            return;
    ...
    protected static function getPriceTitle($priceType)
    {
        ...
        if (!isset(self::$priceTitleCache[$priceType])) {
            ...
            $group = Catalog\GroupTable::getList(
                array(
    ...
    protected static function canProductAutoShip(\Bitrix\Sale\BasketItem $basketItem)
    {
        ...
        if ($productStore = static::GetProductStores(
            array(
    ...
    protected static function getStoreIds(array $params)
    {
        ...
        if (empty($storeIds)) {
            ...
            if (isset($params['SITE_ID']) && $params['SITE_ID'] != '') {
                ...
                $subFilter->logic('or')->where('SITE_ID', '=', $params['SITE_ID'])->where(
                    'SITE_ID',
                    '=',
                    ''
                )->whereNull('SITE_ID');
