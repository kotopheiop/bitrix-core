...
class CAllCatalogDiscount
{
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if ($ACTION == 'ADD') {
            ...
            if (!isset($arFields['CONDITIONS'])) {
                ...
                $arMsg[] = array(
                    'id' => 'CONDITIONS',
                    'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_EMPTY_CONDITIONS')
                );
            }
        ...
        if ($ACTION == 'UPDATE') {
            ...
            if ($ID <= 0) {
                ...
                $arMsg[] = array(
                    'id' => 'ID',
                    'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_BAD_ID', array('#ID#', $ID))
                );
            } else {
                ...
                if (empty($arFields['CONDITIONS'])) {
                    ...
                    if (!is_array($arFields['CONDITIONS'])) {
                        if (!CheckSerializedData($arFields['CONDITIONS'])) {
                            ...
                            $arMsg[] = array(
                                'id' => 'CONDITIONS',
                                'text' => Loc::getMessage("BT_MOD_CATALOG_DISC_ERR_BAD_CONDITIONS")
                            );
                        } else {
                            $arFields['CONDITIONS'] = unserialize(
                                $arFields['CONDITIONS'],
                    ...
                    if ($boolCond) {
                        ...
                        if ($DB->type == 'MYSQL') {
                            if (64000 < CUtil::BinStrlen($arFields['UNPACK']) || 64000 < CUtil::BinStrlen(
                                    $arFields['CONDITIONS']
                                )) {
                                $boolResult = false;
                                $arMsg[] = array(
                                    'id' => 'CONDITIONS',
                                    'text' => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_CONDITIONS_TOO_LONG')
                                );
    ...
    public static function Update($ID, $arFields)
    {
        ...
        if ($boolUpdateRestrictions) {
            if (!CCatalogDiscount::__UpdateSubdiscount(
                $ID,
                $mxRows,
    ...
    public static function GetDiscountByPrice(
        $productPriceID,
        $arUserGroups = array(),
        $renewal = "N",
        $siteID = false,
    ...
    public static function GetDiscountByProduct(
        $productID = 0,
        $arUserGroups = array(),
        $renewal = "N",
    ...
    private static function getReformattedDiscounts(
    ...
    private static function getSaleDiscountsByProduct(
        array $product,
        $siteId,
        array $userGroups,
        array $priceRow,
    ...
    public static function GetDiscount(
        ...
        if (empty($arCatalogGroups)) {
            $catalogGroupIterator = CCatalogGroup::GetGroupsList(
                array(
        ...
        if (self::$useSaleDiscount && Loader::includeModule('sale')) {
            ...
            if (!empty($arDiscountIDs)) {
                ...
                if ($arDiscountCoupons === false) {
                    ...
                    if (self::$existCouponsManager && Loader::includeModule('sale')) {
                        $orderCoupons = DiscountCouponsManager::getOrderedCoupons(
                            true,
                            array('COUPON' => $arDiscountCoupons)
                        );
                    }
                }
                if ($arDiscountCoupons === false) {
                ...
                if (!isset(self::$arCacheDiscountResult[$strCacheKey])) {
                    ...
                    foreach ($discountRows as $pageIds) {
                        ...
                        CTimeZone::Disable();
                        $iterator = Catalog\DiscountTable::getList(
                            array(
                                'select' => $select,
            ...
            if (!$boolGetIDS) {
                $arDiscSave = CCatalogDiscountSave::GetDiscount(
                    array(
                        'USER_ID' => 0,
    ...
    public static function GetDiscountForProduct($arProduct, $arParams = false)
    {
        ...
        if (self::$useSaleDiscount && Loader::includeModule('sale')) {
            ...
            if (isset($arParams['DISCOUNT_FIELDS']) && !empty($arParams['DISCOUNT_FIELDS']) && is_array(
        ...
        if (!empty($productDiscountList)) {
            if (self::$useSaleDiscount && Loader::includeModule('sale')) {
                ...
                foreach ($productDiscountList as &$priority) {
                    $currentPrice = self::calculatePriorityLevel(
                        $price,
                        $currentPrice,
                        $currency,
                        $priority,
            ...
            switch ($discount['VALUE_TYPE']) {
                case self::TYPE_FIX:
                    if ($discount['CURRENCY'] == $currency) {
                        ...
                        $currentDiscount = CCurrencyRates::ConvertCurrency(
                            $discount['VALUE'],
                            $discount['CURRENCY'],
                            $currency
                        );
                ...
                case self::TYPE_PERCENT:
                    ...
                    if ($discount['MAX_DISCOUNT'] > 0) {
                        if ($discount['CURRENCY'] == $currency) {
                            ...
                            $maxDiscount = CCurrencyRates::ConvertCurrency(
                    ...
                    if (isset($arParams['PRODUCT']) && $arParams['PRODUCT'] == 'Y') {
                        ...
                        if (!empty($arProduct['DATE_ACTIVE_FROM'])) {
                            ...
                            if ($intStackTimestamp . '!' != $arProduct['DATE_ACTIVE_FROM'] . '!') {
                                $arProduct['DATE_ACTIVE_FROM'] = (int)MakeTimeStamp(
                        ...
                        if (!empty($arProduct['DATE_ACTIVE_TO'])) {
                            ...
                            if ($intStackTimestamp . '!' != $arProduct['DATE_ACTIVE_TO'] . '!') {
                                $arProduct['DATE_ACTIVE_TO'] = (int)MakeTimeStamp(
                        ...
                        if (!empty($arProduct['DATE_CREATE'])) {
                            ...
                            if ($intStackTimestamp . '!' != $arProduct['DATE_CREATE'] . '!') {
                                $arProduct['DATE_CREATE'] = (int)MakeTimeStamp(
                        ...
                        if (!empty($arProduct['TIMESTAMP_X'])) {
                            ...
                            if ($intStackTimestamp . '!' != $arProduct['TIMESTAMP_X'] . '!') {
                                $arProduct['TIMESTAMP_X'] = (int)MakeTimeStamp(
                        ...
                        case 'Date':
                            ...
                            if ('' != $arOneProp['VALUE']) {
                                ...
                                if ($intStackTimestamp . '!' != $arOneProp['VALUE'] . '!') {
                                    $arOneProp['VALUE'] = (int)MakeTimeStamp(
                                            $arOneProp['VALUE'],
                                            $propertyFormat
                                        ) - $intTimeOffset;
                                } else {
    ...
    protected static function __ParseArrays(&$arFields)
    {
        ...
        if (!self::__CheckOneEntity($arFields, 'GROUP_IDS')) {
            $arMsg[] = array(
                'id' => 'GROUP_IDS',
                "text" => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_PARSE_USER_GROUP')
            );
            $boolResult = false;
        }
        if (!self::__CheckOneEntity($arFields, 'CATALOG_GROUP_IDS')) {
            $arMsg[] = array(
                'id' => 'CATALOG_GROUP_IDS',
                "text" => Loc::getMessage('BT_MOD_CATALOG_DISC_ERR_PARSE_PRICE_TYPE')
            );
            $boolResult = false;
        }
    ...
    protected static function __GetOldOneEntity(&$arFields, &$arCondList, $strEntityID, $strCondID)
    {
        if (is_array($arCondList) && !empty($arCondList)) {
            ...
            if (isset($arCondList[$strCondID]) && !empty($arCondList[$strCondID]) && is_array(
                    $arCondList[$strCondID]
                )) {
                if (isset($arCondList[$strCondID]['VALUES']) && !empty($arCondList[$strCondID]['VALUES']) && is_array(
                        $arCondList[$strCondID]['VALUES']
                    )) {
                    $arCheck = array();
        ...
        if (!empty($arParams) && is_array($arParams)) {
            if (!empty($arParams['ENTITY_ID']) && !empty($arParams['TABLE_ID']) && !empty($arParams['FIELD_ID'])) {
                if (isset($arFields[$arParams['ENTITY_ID']])) {
                    if ($boolUpdate) {
                        $DB->Query(
                            "DELETE FROM " . $arParams['TABLE_ID'] . " WHERE DISCOUNT_ID = " . $intID,
                            false,
    ...
    protected static function primaryDiscountFilter(
        $price,
        $currency,
        &$discountList,
        &$priceDiscountList,
    ...
    protected static function initDiscountSettings()
    {
        ...
        if (self::$getPercentFromBasePrice === null) {
            ...
            self::$getPercentFromBasePrice = (string)Option::get(
                    $moduleID,
