    ...
    if ($STEP == 1) {
        ...
        <tr>
            ...
            <td width="60%"><?
                if ($intCountSelected) {
                    foreach ($V as &$oneKey) {
                        ?><input type="hidden" value="<? echo intval($oneKey); ?>" name="V[]"
                                 id="oldV<? echo intval($oneKey); ?>"><?
                    }
                }
                ?>
                <div id="tree"></div>
        ...
        <tr>
            ...
            <td width="60%">
                <b><? echo htmlspecialcharsbx(COption::GetOptionString("catalog", "export_default_path", "/bitrix/catalog_export/")); ?></b><input
                        type="text" name="SETUP_FILE_NAME" value="<?
                echo (strlen($SETUP_FILE_NAME) > 0) ? htmlspecialcharsbx($SETUP_FILE_NAME) : "ebay_product_feed_" . mktime() . ".xml" ?>"
                        size="50"/>
            </td>
        </tr>
        <?
