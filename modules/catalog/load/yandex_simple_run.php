...
if ($strExportErrorMessage == '') {
    if (!$fp = @fopen($_SERVER["DOCUMENT_ROOT"] . $SETUP_FILE_NAME, "wb")) {
        $strExportErrorMessage .= str_replace(
                '#FILE#',
                $_SERVER["DOCUMENT_ROOT"] . $SETUP_FILE_NAME,
                GetMessage('CET_YAND_RUN_ERR_SETUP_FILE_OPEN_WRITING')
            ) . "\n";
    } else {
        if (!@fwrite(
            $fp,
            ...
            $strExportErrorMessage .= str_replace(
                    '#FILE#',
                    $_SERVER["DOCUMENT_ROOT"] . $SETUP_FILE_NAME,
                    GetMessage('CET_YAND_RUN_ERR_SETUP_FILE_WRITE')
                ) . "\n";
            @fclose($fp);
        } else {
...
if ($strExportErrorMessage == '') {
    ...
    fwrite(
        $fp,
        "<name>" . $APPLICATION->ConvertCharset(
            htmlspecialcharsbx(COption::GetOptionString("main", "site_name", "")),
            LANG_CHARSET,
            'windows-1251'
        ) . "</name>\n"
    );
    fwrite(
        $fp,
        "<company>" . $APPLICATION->ConvertCharset(
            htmlspecialcharsbx(COption::GetOptionString("main", "site_name", "")),
            LANG_CHARSET,
            'windows-1251'
        ) . "</company>\n"
    ...
    fwrite(
        $fp,
        "<url>" . $usedProtocol . htmlspecialcharsbx(
            $SETUP_SERVER_NAME <> '' ? $SETUP_SERVER_NAME : COption::GetOptionString("main", "server_name", "")
        ) . "</url>\n"
    );
    ...
    if (is_array($YANDEX_EXPORT)) {
        ...
        foreach ($YANDEX_EXPORT as $ykey => $yvalue) {
            ...
            while ($arAcc = $db_acc->Fetch()) {
                ...
                $strTmpCat .= '<category id="' . $arAcc['ID'] . '"' . ($arAcc['IBLOCK_SECTION_ID'] > 0 ? ' parentId="' . $arAcc['IBLOCK_SECTION_ID'] . '"' : '') . '>' . yandex_text2xml(
                        $arAcc['NAME'],
                        true
                    ) . '</category>' . "\n";
            ...
            while ($arAcc = $res->GetNext()) {
                ...
                $strTmpOff .= "<url>" . $usedProtocol . $arAcc['SERVER_NAME'] . htmlspecialcharsbx(
                        $arAcc["~DETAIL_PAGE_URL"]
                    ) . (mb_strstr(
                        $arAcc['DETAIL_PAGE_URL'],
                        '?'
                    ) === false ? '?' : '&amp;') . "r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?></url>\n";
                ...
                if (intval($arAcc["DETAIL_PICTURE"]) > 0 || intval($arAcc["PREVIEW_PICTURE"]) > 0) {
                    ...
                    if (is_array($arPictInfo)) {
                        if (mb_substr($arPictInfo["SRC"], 0, 1) == "/") {
                            $strFile = $usedProtocol . $arAcc['SERVER_NAME'] . CHTTP::urnEncode(
                                    $arPictInfo["SRC"],
                                    'utf-8'
                                );
                        } else {
            ...
            if ($boolNeedRootSection) {
                ...
                $strTmpCat .= '<category id="' . $maxSections[$yvalue] . '">' . yandex_text2xml(
                        GetMessage('YANDEX_ROOT_DIRECTORY_EXT', array('#NAME#' => $iblockName)),
                        true
                    ) . "</category>\n";
                unset($iblockName);
            }
