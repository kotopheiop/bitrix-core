            ...
            switch ($iblockProperty['PROPERTY_TYPE']) {
                                ...
                                if ($intValue > 0) {
                                    if ($ar_file = CFile::GetFileArray($intValue)) {
                                        if (mb_substr($ar_file["SRC"], 0, 1) == "/") {
                                            $strFile = $options['PROTOCOL'] . $options['SITE_NAME'] . CHTTP::urnEncode(
                                                    $ar_file['SRC'],
                                                    'utf-8'
                                                );
                                        } else {
                ...
                default:
                    if ($bParam && $iblockProperty['WITH_DESCRIPTION'] == 'Y') {
                        ...
                        $value = is_array($arProperty['~VALUE']) ? implode(
                            ', ',
                            $arProperty['~VALUE']
                        ) : $arProperty['~VALUE'];
                    }
    ...
    function yandexPrepareItems(array &$list, array $parents, array $options)
    {
        ...
        foreach (array_keys($list) as $index) {
            ...
            if ($row['DETAIL_PICTURE'] > 0 || $row['PREVIEW_PICTURE'] > 0) {
                $pictureFile = CFile::GetFileArray(
                    $row['DETAIL_PICTURE'] > 0 ? $row['DETAIL_PICTURE'] : $row['PREVIEW_PICTURE']
                );
                if (!empty($pictureFile)) {
                    if (strncmp($pictureFile['SRC'], '/', 1) == 0) {
                        $picturePath = $options['PROTOCOL'] . $options['SITE_NAME'] . CHTTP::urnEncode(
                                $pictureFile['SRC'],
                                'utf-8'
                            );
                    } else {
    ...
    if (!empty($arOffers['IBLOCK_ID'])) {
        ...
        if (($arOfferIBlock = $rsOfferIBlocks->Fetch())) {
            ...
            if ($arOfferIBlock['DETAIL_PAGE_URL'] == '#PRODUCT_URL#') {
                ...
                $offerUrlConfig['USE_DOMAIN'] = !(preg_match(
                    "/^(http|https):\\/\\//i",
                    $arOfferIBlock['DETAIL_PAGE_URL']
                ));
                $offerUrlConfig['REFERRER_SEPARATOR'] = (mb_strpos(
                    $arOfferIBlock['DETAIL_PAGE_URL'],
                    '?'
                ) === false ? '?' : '&amp;');
            }
    ...
    if (empty($arRunErrors)) {
        ...
        if (is_array($XML_DATA['CURRENCY'])) {
            foreach ($XML_DATA['CURRENCY'] as $CURRENCY => $arCurData) {
                if (in_array($CURRENCY, $arCurrencyAllowed)) {
                    $strTmp .= '<currency id="' . $CURRENCY . '"'
                        . ' rate="' . ($arCurData['rate'] == 'SITE' ? CCurrencyRates::ConvertCurrency(
                            1,
                            $CURRENCY,
                            $RUR
                        ) : $arCurData['rate']) . '"'
            ...
            while ($currency = $currencyIterator->fetch()) {
                $strTmp .= '<currency id="' . $currency['CURRENCY'] . '" rate="' . (CCurrencyRates::ConvertCurrency(
                        1,
                        $currency['CURRENCY'],
                        $RUR
                    )) . '" />' . "\n";
        ...
        if (!$bAllSections) {
            for ($i = 0, $intSectionsCount = count($arSections); $i < $intSectionsCount; $i++) {
                $sectionIterator = CIBlockSection::GetNavChain(
                    $IBLOCK_ID,
                    $arSections[$i],
                    array(
                        'ID',
        ...
        foreach ($arAvailGroups as $value) {
            $strTmpCat .= '<category id="' . $value['ID'] . '"' . ($value['IBLOCK_SECTION_ID'] > 0 ? ' parentId="' . $value['IBLOCK_SECTION_ID'] . '"' : '') . '>' . yandex_text2xml(
                    $value['NAME'],
                    $itemOptions
                ) . '</category>' . "\n";
        }
        ...
        if (!empty($items)) {
            ...
            if ($needDiscountCache) {
                ...
                \CCatalogDiscount::SetDiscountProductCache(
                    $itemIdsList,
                    array('IBLOCK_ID' => $IBLOCK_ID, 'GET_BY_ID' => 'Y')
                );
            }
            ...
            foreach ($itemIdsList as $id) {
                ...
                if ($row['TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS'])) {
                    ...
                    foreach ($row['OFFERS'] as $offer) {
                        ...
                        $itemsContent .= "<url>" . ($offerUrlConfig['USE_DOMAIN'] ? $usedProtocol . $site['SERVER_NAME'] : '') . htmlspecialcharsbx(
                        ...
                        foreach ($arYandexFields as $key) {
                            switch ($key) {
                                case 'name':
                                    ...
                                    $itemsContent .= "<name>" . yandex_text2xml(
                                            $offer['NAME'],
                                            $itemOptions
                                        ) . "</name>\n";
                                    break;
                                ...
                                case 'title':
                                    if (!$fieldsExist || !isset($fields[$key])) {
                                        if (
                                            ...
                                            $itemsContent .= "<" . $key . ">" . yandex_text2xml(
                                                    $offer['NAME'],
                                                    $itemOptions
                                                ) . "</" . $key . ">\n";
                                        }
                    ...
                    $itemsContent .= "<url>" . ($itemUrlConfig['USE_DOMAIN'] ? $usedProtocol . $site['SERVER_NAME'] : '') . htmlspecialcharsbx(
                            $row['DETAIL_PAGE_URL']
                        ) . $referer . "</url>\n";
                    unset($referer);
...
if (empty($arRunErrors)) {
    ...
    if ($finalExport) {
        ...
        if ($boolNeedRootSection) {
            $content .= '<category id="' . $intMaxSectionID . '">' . yandex_text2xml(
                    GetMessage('YANDEX_ROOT_DIRECTORY'),
                    $itemOptions
                ) . '</category>' . "\n";
        }
        $content .= "</categories>\n";
            ...
            if (file_exists($_SERVER["DOCUMENT_ROOT"] . $SETUP_FILE_NAME)) {
                if (!unlink($_SERVER["DOCUMENT_ROOT"] . $SETUP_FILE_NAME)) {
                    $arRunErrors[] = str_replace(
                        '#FILE#',
                        $SETUP_FILE_NAME,
                        GetMessage('BX_CATALOG_EXPORT_YANDEX_ERR_UNLINK_FILE')
                    );
                    $process = false;
                }
            }
