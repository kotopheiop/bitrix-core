...
if (
    ...
    <form name="catalog_reindex_form" action="<? echo $APPLICATION->GetCurPage(); ?>" method="GET"><?
        ...
        $tabControl->Buttons();
        ?>
        <input type="button" id="catalog_reindex_start_button"
               value="<? echo Loc::getMessage('BX_CATALOG_REINDEX_UPDATE_BTN') ?>">
        <input type="button" id="catalog_reindex_stop_button"
               value="<? echo Loc::getMessage('BX_CATALOG_REINDEX_STOP_BTN') ?>" disabled>
        <?
        $tabControl->End();
        ?></form>
