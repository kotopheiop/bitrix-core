...
?>
    <form enctype="multipart/form-data" method="POST" action="<?= $actionUrl ?>" id="form_b_catalog_store_docs"
          name="form_b_catalog_store_docs">
        ...
        <div class="adm-detail-content-item-block">
            ...
            <textarea cols="120" rows="4" class="typearea"
                      name="CAT_DOC_COMMENTARY" <?= $isDisable ?> wrap="virtual"><?= $str_COMMENTARY ?></textarea>
        </div>
        <?
        $tabControl->Buttons(
            array(
        ...
        if ($adminSidePanelHelper->isSidePanelFrame()) {
            if (!$bReadOnly && !$isDocumentConduct) {
                ...
                <input type="button" class="adm-btn-save" name="save_and_conduct" value="<?
                echo GetMessage("CAT_DOC_ADD_CONDUCT") ?>">
                <input type="button" class="adm-btn" name="save_document" value="<?
                echo GetMessage("CAT_DOC_SAVE") ?>">
                <?
            } elseif ($isDocumentConduct) {
                ?>
                <span class="hor-spacer"></span>
                <input type="button" class="adm-btn" name="cancellation" value="<?
                echo GetMessage("CAT_DOC_CANCELLATION") ?>">
                <?
            }
            ?>
            <input type="button" class="adm-btn" name="dontsave" value="<?
            echo GetMessage("CAT_DOC_CANCEL") ?>">
            <?
        } else {
            if (!$bReadOnly && !$isDocumentConduct) {
                ?>
                <span style="display:inline-block; width:20px; height: 22px;"></span>
                <input type="submit" class="adm-btn-save" name="save_and_conduct" value="<?
                echo GetMessage("CAT_DOC_ADD_CONDUCT") ?>">
                <input type="submit" class="adm-btn" name="save_document" value="<?
                echo GetMessage("CAT_DOC_SAVE") ?>">
                <?
            } elseif ($isDocumentConduct) {
                ?>
            ...
            ?>
            <input type="submit" class="adm-btn" name="dontsave" id="dontsave" value="<?
            echo GetMessage("CAT_DOC_CANCEL") ?>">
            <?
        }
                        ...
                        children: [
                            ...
                            BX.create('input', {
                                props: {
                                    ...
                                    value: '<?=GetMessageJS('CAT_DOC_ADD')?>' /*disabled: (maxId >= BX('CAT_DOC_AMOUNT_'+id).value)*/
                                },
                                style: {
                                    marginLeft: '5px'
                                },
                                        ...
                                        if (BX("BARCODE_INPUT_" + id).value.replace(/^\s+|\s+$/g, '') !== '' && !<?=intval($bReadOnly)?>) {
                                            ...
                                            BX('BARCODE_DIV_' + id).appendChild(BX.create('DIV', {
                                                ...
                                                children: [
                                                    ...
                                                    BX.create('a', {
                                                        props: {
                                                            className: 'split-delete-item',
                                                            tabIndex: '-1',
                                                            href: 'javascript:void(0);',
                                                            id: "BARCODE_DELETE[" + id + "][" + counter + "]"
                                                        },
                                                        events: {
                                                            click: function () {
                                                                if (!<?=intval($bReadOnly)?>) {
