...
while ($arDiscount = $dbResultList->Fetch()) {
    ...
    $row->AddViewField(
        "ID",
        '<a href="/bitrix/admin/cat_discount_coupon_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arDiscount["ID"] . '">' . $arDiscount["ID"] . '</a>'
    );
    ...
    $arActions[] = array(
        ...
        "ACTION" => $lAdmin->ActionRedirect(
            "/bitrix/admin/cat_discount_coupon_edit.php?ID=" . $arDiscount['ID'] . "&lang=" . LANGUAGE_ID . GetFilterParams(
                "filter_",
                false
            ) . ""
        ),
    ...
    <form name="find_form" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?">
            ...
            <td>
                <select name="filter_discount_id">
                    ...
                    while ($arDiscountList = $dbDiscountList->Fetch()) {
                        ?>
                        <option value="<? echo $arDiscountList["ID"] ?>"<? if ($filter_discount_id == $arDiscountList["ID"]) {
                            echo " selected";
                        } ?>><? echo "[" . $arDiscountList["ID"] . "] " . htmlspecialcharsbx(
                                $arDiscountList["NAME"] . " (" . $arDiscountList["SITE_ID"] . ")"
                            ) ?></option><?
                    }
        ...
        <tr>
            ...
            <td>
                <select name="filter_active">
                    ...
                    <option value="Y"<? if ($filter_active == "Y") echo " selected" ?>><? echo htmlspecialcharsex(
                            GetMessage("DSC_CPN_YES")
                        ) ?></option>
                    <option value="N"<? if ($filter_active == "N") echo " selected" ?>><? echo htmlspecialcharsex(
                            GetMessage("DSC_CPN_NO")
                        ) ?></option>
                </select>
            </td>
        </tr>
            ...
            <td>
                <select name="filter_one_time">
                    ...
                    foreach ($arCouponType as $strType => $strName) {
                        ?>
                        <option value="<? echo $strType; ?>"<? if ($filter_one_time == $strType) {
                            echo " selected";
                        } ?>><? echo htmlspecialcharsex($strName); ?></option><?
                    }
                    ?></select>
            </td>
        </tr>
        <tr>
            ...
            <td><? echo CalendarPeriod(
                    "filter_apply_time_start",
                    htmlspecialcharsex($filter_apply_time_start),
                    "filter_apply_time_end",
                    htmlspecialcharsex($filter_apply_time_end),
                    "find_form"
                ) ?></td>
        </tr>
        <tr>
