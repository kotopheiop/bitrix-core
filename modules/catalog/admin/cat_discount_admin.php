...
if (!empty($filter_date_active_from)) {
    $arFilter["!>ACTIVE_FROM"] = $filter_date_active_from;
}
if (!empty($filter_date_active_to)) {
    $arFilter["!<ACTIVE_TO"] = $filter_date_active_to;
}
...
if (!empty($filter_renewal)) {
    $arFilter["RENEWAL"] = $filter_renewal;
}
...
while ($arDiscount = $dbResultList->Fetch()) {
    ...
    $row->AddViewField(
        "ID",
        '<a href="/bitrix/admin/cat_discount_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arDiscount["ID"] . '">' . $arDiscount["ID"] . '</a>'
    );
    ...
    if ($arSelectFieldsMap['VALUE']) {
        ...
        if ($arDiscount["VALUE_TYPE"] == CCatalogDiscount::TYPE_PERCENT) {
            ...
            $strDiscountValue = '= ' . CCurrencyLang::CurrencyFormat(
                    $arDiscount["VALUE"],
                    $arDiscount["CURRENCY"],
                    true
                );
    ...
    if ($arSelectFieldsMap['MAX_DISCOUNT']) {
        $row->AddViewField(
            "MAX_DISCOUNT",
            (0 < $arDiscount['MAX_DISCOUNT'] ? CCurrencyLang::CurrencyFormat(
                $arDiscount['MAX_DISCOUNT'],
                $arDiscount["CURRENCY"],
                true
            ) : GetMessage('DSC_MAX_DISCOUNT_UNLIM'))
        );
    }
    ...
    $arActions[] = array(
        ...
        "ACTION" => $lAdmin->ActionRedirect(
            "/bitrix/admin/cat_discount_edit.php?ID=" . $arDiscount['ID'] . "&lang=" . LANGUAGE_ID . GetFilterParams(
                "filter_",
                false
            ) . ""
        ),
    ...
    if (!$bReadOnly) {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect(
                "/bitrix/admin/cat_discount_edit.php?ID=" . $arDiscount['ID'] . '&action=copy&lang=' . LANGUAGE_ID
            )
        );
...
if ($arSelectFieldsMap['CREATED_BY'] || $arSelectFieldsMap['MODIFIED_BY']) {
    if (!empty($arUserID)) {
        ...
        while ($oneUser = $userIterator->fetch()) {
            ...
            if ($canViewUserList) {
                $arUserList[$oneUser['ID']] = '<a href="/bitrix/admin/user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $oneUser['ID'] . '">' . CUser::FormatName(
                        $nameFormat,
                        $oneUser
                    ) . '</a>';
            } else {
...
?>
    <form name="find_form" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?">
            ...
            <td><?
                ...
                ?><select name="filter_site_id[]" multiple size="<?= $siteSize; ?>"><?
                    foreach ($siteList as $row) {
                        ?>
                        <option value="<?= $row['LID']; ?>"<?= (in_array(
                            $row['LID'],
                            $filterSite
                        ) ? ' selected' : ''); ?>>[<?= $row['LID']; ?>]&nbsp;<?= htmlspecialcharsbx(
                            $row['NAME']
                        ); ?></option><?
                    }
                    unset($row);
        ...
        <tr>
            ...
            <td>
                <input type="text" name="filter_name" size="50"
                       value="<?= htmlspecialcharsbx($filter_name); ?>">&nbsp;<?= ShowFilterLogicHelp() ?>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <select name="filter_renewal">
                    <option value=""><?= htmlspecialcharsbx("(" . GetMessage("DSC_ALL") . ")"); ?></option>
                    <option value="Y"<? if ($filter_renewal == "Y") echo " selected" ?>><?= htmlspecialcharsbx(
                            GetMessage("DSC_YES")
                        ); ?></option>
                    <option value="N"<? if ($filter_renewal == "N") echo " selected" ?>><?= htmlspecialcharsbx(
                            GetMessage("DSC_NO")
                        ); ?></option>
                </select>
            </td>
        ...
        <tr>
            ...
            <td>
                <input type="text" name="filter_value_start" value="<?= htmlspecialcharsbx($filter_value_start); ?>"
                       size="15">
                ...
                <input type="text" name="filter_value_end" value="<?= htmlspecialcharsbx($filter_value_end); ?>"
                       size="15">
            </td>
        </tr>
        <tr>
            ...
            <td>
                <select name="filter_use_coupons">
                    <option value=""><?= htmlspecialcharsbx("(" . GetMessage("DSC_ALL") . ")"); ?></option>
                    <option value="Y"<? if ($filter_use_coupons == "Y") echo " selected" ?>><?= htmlspecialcharsbx(
                            GetMessage("DSC_YES")
                        ); ?></option>
                    <option value="N"<? if ($filter_use_coupons == "N") echo " selected" ?>><?= htmlspecialcharsbx(
                            GetMessage("DSC_NO")
                        ); ?></option>
                </select>
            </td>
