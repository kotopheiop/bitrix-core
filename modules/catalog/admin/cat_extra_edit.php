...
CAdminMessage::ShowMessage($errorMessage); ?>
    <form method="POST" action="<?= $actionUrl ?>" name="form1">
        ...
        <tr class="adm-detail-required-field">
            ...
            <td width="60%">
                <input type="text" name="PERCENTAGE" size="10" maxlength="20"
                       value="<?= htmlspecialcharsbx($str_PERCENTAGE); ?>"/>%
            </td>
        </tr>
        <?
        if ($ID > 0) {
            ...
            <tr>
                <td width="40%"><?
                    echo GetMessage("CEEN_RECALC") ?>:
                </td>
                <td width="60%">
                    <input type="hidden" name="RECALCULATE" value="N"/>
                    <input type="checkbox" name="RECALCULATE" value="Y"<?
                    if ($str_RECALCULATE == "Y") echo " checked" ?> />
                </td>
            </tr>
            <?
        }
