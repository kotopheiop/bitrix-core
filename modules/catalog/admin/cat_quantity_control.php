...
if (check_bitrix_sessid()) {
    ...
    if ($_REQUEST['action'] == "clearQuantity") {
        if (!$useStoreControl) {
            $dbElements = CCatalogProduct::GetList(
                array(),
                array("ELEMENT_IBLOCK_ID" => $iblockId, "!QUANTITY" => 0),
                false,
                false,
                array("ID")
            );
        ...
        $dbElements = CCatalogProduct::GetList(
            array(),
            array("ELEMENT_IBLOCK_ID" => $iblockId, "!QUANTITY_RESERVED" => 0),
            false,
            false,
            array("ID", "QUANTITY_RESERVED", "QUANTITY")
        );
        while ($arElements = $dbElements->Fetch()) {
        ...
        if ($storeId > 0 || $storeId == -1) {
            ...
            $dbElements = CCatalogProduct::GetList(
                array(),
                array("ELEMENT_IBLOCK_ID" => $iblockId),
                false,
                false,
                array("ID", "QUANTITY")
            );
            ...
            if ($storeId === -1) {
                $dbStoreElements = CCatalogStoreProduct::GetList(
                    array(),
                    array("PRODUCT_ID" => array_keys($arElementsId)),
                    false,
                    false,
                    array("ID", "AMOUNT", "PRODUCT_ID")
                );
            } else {
                $dbStoreElements = CCatalogStoreProduct::GetList(
                    array(),
                    array(
                        "STORE_ID" => $storeId,
                        "PRODUCT_ID" => array_keys($arElementsId)
                    ),
                    false,
                    false,
                    array("ID", "AMOUNT", "PRODUCT_ID")
            ...
            while ($arStoreElements = $dbStoreElements->Fetch()) {
                ...
                if ($useStoreControl) {
                    ...
                    CCatalogProduct::Update(
                        $arStoreElements["PRODUCT_ID"],
                        array("QUANTITY" => $arElementsId[$arStoreElements["PRODUCT_ID"]])
                    );
                }
            }
