            ...
            if ('' == $strWarning) {
                ...
                while ($arCatGroups = $dbCatGroups->Fetch()) {
                    ...
                    for ($i = 0; $i < $intBasePriceCount; $i++) {
                        ${"CAT_PRICE_" . $arCatGroups["ID"] . "_" . $arCatalogBasePrices[$i]["IND"]} = str_replace(
                            [' ', ','],
                            ['', '.'],
                            ${"CAT_PRICE_" . $arCatGroups["ID"] . "_" . $arCatalogBasePrices[$i]["IND"]}
                        );
                        $arCatalogPrice_tmp[$i] = array(
                            ...
                            "CURRENCY" => trim(
                                ${"CAT_CURRENCY_" . $arCatGroups["ID"] . "_" . $arCatalogBasePrices[$i]["IND"]}
                            ),
                ...
                if (isset($_REQUEST["AR_BARCODE_ID"]) && ($barcodeMultiply == 'Y')) {
                    ...
                    unset($row);
                } elseif (isset($_REQUEST["AR_BARCODE_ID"]) && is_array(
                        $_REQUEST["AR_BARCODE_ID"]
                    ) && $barcodeMultiply != 'Y') {
                    ...
                    foreach ($_REQUEST["AR_BARCODE_ID"] as $barcodeId) {
                        ...
                        if (!isset($_REQUEST["CAT_BARCODE_$barcodeId"]) || trim(
                                $_REQUEST["CAT_BARCODE_$barcodeId"]
                            ) == '') {
