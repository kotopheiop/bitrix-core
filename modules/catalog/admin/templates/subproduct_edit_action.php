    ...
    if (0 < $IBLOCK_ID && 0 < $ID) {
        ...
        if (CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $PRODUCT_ID, "element_edit_price")) {
            IncludeModuleLangFile(
                $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/catalog/templates/product_edit_action.php'
            );
            ...
            if ('' == $strWarning) {
                ...
                while ($arCatGroups = $dbCatGroups->Fetch()) {
                    ...
                    for ($i = 0; $i < $intBasePriceCount; $i++) {
                        ${"SUBCAT_PRICE_" . $arCatGroups["ID"] . "_" . $arCatalogBasePrices[$i]["IND"]} = str_replace(
                            [' ', ','],
                            ['', '.'],
                            ${"SUBCAT_PRICE_" . $arCatGroups["ID"] . "_" . $arCatalogBasePrices[$i]["IND"]}
                        );
                        $arCatalogPrice_tmp[$i] = array(
                            ...
                            "CURRENCY" => trim(
                                ${"SUBCAT_CURRENCY_" . $arCatGroups["ID"] . "_" . $arCatalogBasePrices[$i]["IND"]}
                            ),
                        ...
                        if ($arCatalogPrice_tmp[$i]["EXTRA_ID"] > 0) {
                            if (0 < doubleval($arCatalogBasePrices[$i]["PRICE"])) {
                                ...
                                $arCatalogPrice_tmp[$i]["PRICE"] = roundEx(
                                    $arCatalogBasePrices[$i]["PRICE"] * (1 + (float)$arCatalogExtra["PERCENTAGE"] / 100),
                    ...
                    if (!empty($row) && $row['BARCODE_MULTI'] == 'N') {
                        ...
                        if (is_object($dbAmount) && ($arAmount = $dbAmount->Fetch())) {
                            $dbBarCode = CCatalogStoreBarCode::GetList(
                                ...
                                false,
                                false,
                                array("ID", "BARCODE", "PRODUCT_ID", "STORE_ID")
                            );
                            while ($arBarCode = $dbBarCode->Fetch()) {
                                $arBarCodeResult = $arBarCode;
                                $countBarCode++;
                            }
                            if ((!empty($arBarCodeResult)) && ($countBarCode == 1) && (intval(
                                        $arBarCodeResult["STORE_ID"]
                                    ) == 0)) {
                            ...
                            if ($arAmount["SUM"] != $countBarCode) {
                                $strWarning .= GetMessage(
                                    "C2IT_ERROR_USE_MULTIBARCODE",
                                    array("#COUNT#" => ($arAmount["SUM"] - $countBarCode))
                                );
                                $barcodeMultiply = 'N';
                    ...
                    if (count($arId) == 1) {
                        if (isset($_REQUEST['SUBCAT_BARCODE_EDIT']) && 'Y' == $_REQUEST['SUBCAT_BARCODE_EDIT']) {
                            if ('' != trim($_REQUEST["SUBCAT_BARCODE"])) {
                                if (!CCatalogStoreBarCode::Update(
                                    $arId[0],
                                    array(
                                        "MODIFIED_BY" => $userId,
                                        "STORE_ID" => 0,
                                        'BARCODE' => trim($_REQUEST["SUBCAT_BARCODE"])
                                    )
                                )) {
                            ...
                            if (!CCatalogStoreBarCode::Update(
                                $arId[0],
                                array("MODIFIED_BY" => $userId, "STORE_ID" => 0)
                            )) {
                                $strWarning .= GetMessage("C2IT_ERROR_SAVE_BARCODE");
                            }
                        }
                            ...
                            if ($arCatalogPrice && $arCatalogPrice["PRODUCT_ID"] == $PRODUCT_ID) {
                                ...
                                if (!CPrice::Update($arCatalogBasePrices[$i]["ID"], $arCatalogFields)) {
                                    $strWarning .= str_replace(
                                            "#ID#",
                                            $arCatalogBasePrices[$i]["ID"],
                                            GetMessage("C2IT_ERROR_PRPARAMS")
                                        ) . "<br>";
                                }
                            } else {
                                ...
                                if (!$ID_tmp) {
                                    $strWarning .= str_replace(
                                            "#PRICE#",
                                            $arCatalogFields["PRICE"],
                                            GetMessage("C2IT_ERROR_SAVEPRICE")
                                        ) . "<br>";
                                }
                            }
                        } else {
                        ...
                        if (isset($arCurProductGroups[$arGroup["ID"]])) {
                            if (isset(${"SUBCAT_USER_GROUP_ID_" . $arGroup["ID"]}) && ${"SUBCAT_USER_GROUP_ID_" . $arGroup["ID"]} == "Y") {
                                if ((int)(${"SUBCAT_ACCESS_LENGTH_" . $arGroup["ID"]}) != (int)($arCurProductGroups[$arGroup["ID"]]["ACCESS_LENGTH"])
                                    ...
                                    CCatalogProductGroups::Update(
                                        $arCurProductGroups[$arGroup["ID"]]["ID"],
                                        $arCatalogFields
                                    );
                                }
                            } else {
