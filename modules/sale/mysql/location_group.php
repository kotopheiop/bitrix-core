...
class CSaleLocationGroup extends CAllSaleLocationGroup
{
    public static function GetList($arOrder = Array("NAME" => "ASC"), $arFilter = Array(), $strLang = LANGUAGE_ID)
    {
                ...
                case "LOCATION":
                    ...
                    if (CSaleLocation::isLocationProMigrated()) {
                        try {
                            ...
                            $arSqlSearch[] = "	LG.ID " . ($bInvert ? 'not' : '') . " in (" . $class::getConnectedEntitiesQuery(
                                    intval($val),
                                    'id',
                        ...
                        $arSqlSearch[] = "LG.ID = L2LG.LOCATION_GROUP_ID AND L2LG.LOCATION_GROUP_ID " . ($bInvert ? "<>" : "=") . " " . intval(
                                $val
                            ) . " ";
        ...
        $strSql =
            ...
            "	" . $strSqlSearchFrom . ") " .
            "	LEFT JOIN b_sale_location_group_lang LGL ON (LG.ID = LGL.LOCATION_GROUP_ID AND LGL.LID = '" . $DB->ForSql(
                $strLang,
                2
            ) . "') " .
            "WHERE 1 = 1 " .
        ...
        foreach ($arOrder as $by => $order) {
            ...
            if ($by == "ID") {
                $arSqlOrder[] = " LG.ID " . $order . " ";
            } elseif ($by == "NAME") {
                $arSqlOrder[] = " LGL.NAME " . $order . " ";
            } else {
    ...
    public static function Add($arFields)
    {
        ...
        if (CSaleLocation::isLocationProMigrated()) {
            ...
            for ($i = 0; $i < $countFieldLoc; $i++) {
                ...
                if ($strSqlHeadLength + $strSqlLen + mb_strlen(
                        $tmpSql
