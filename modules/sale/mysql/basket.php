...
class CSaleBasket extends CAllSaleBasket
{
    ...
    public static function GetList(
        ...
        if ($isOrderConverted != 'N') {
            $result = \Bitrix\Sale\Compatible\BasketCompatibility::getList(
                $arOrder,
                $arFilter,
                $arGroupBy,
                $arNavStartParams,
        ...
        $arFields = array(
            ...
            "ORDER_ALLOW_DELIVERY" => array(
                ...
                "TYPE" => "string",
                "FROM" => "LEFT JOIN b_sale_order O ON (O.ID = B.ORDER_ID)"
            ),
            ...
            "USER_ID" => array(
                "FIELD" => "F.USER_ID",
                "TYPE" => "string",
                "FROM" => "LEFT JOIN b_sale_fuser F ON (F.ID = B.FUSER_ID)"
            ),
    ...
    public static function Add($arFields)
    {
        ...
        if (!$bFound) {
            ...
            if ($isOrderConverted == 'N') {
                ...
                $strSql = "INSERT INTO b_sale_basket(" . $arInsert[0] . ", DATE_INSERT, DATE_UPDATE) VALUES(" . $arInsert[1] . ", " . $DB->GetNowFunction(
                    ) . ", " . $DB->GetNowFunction() . ")";
                    ...
                    if (!isset($arFields["MANUAL_SET_ITEMS_INSERTION"])) // set items will be added separately (from admin form data)
                    {
                        ...
                        if ($productProvider = CSaleBasket::GetProductProvider($arFields)) {
                            if (method_exists($productProvider, "GetSetItems")) {
                                $arSets = $productProvider::GetSetItems(
                                    $arFields["PRODUCT_ID"],
    ...
    public static function Delete($ID)
    {
        ...
        if (0 < intval($arBasket["ORDER_ID"])) {
            CSaleOrderChange::AddRecord(
                $arBasket["ORDER_ID"],
                "BASKET_REMOVED",
                array("PRODUCT_ID" => $arBasket["PRODUCT_ID"], "NAME" => $arBasket["NAME"])
            );
        }
    ...
    public static function GetLeave(
        ...
        $arFields = array(
            ...
            "NAME_SEARCH" => array(
                ...
                "TYPE" => "string",
                "FROM" => "LEFT JOIN b_user U ON (U.ID = F.USER_ID)"
            ),
            "USER_NAME" => array(
                "FIELD" => "U.NAME",
                "TYPE" => "string",
            ...
            "USER_GROUP_ID" => array(
                "FIELD" => "UG.GROUP_ID",
                "TYPE" => "int",
                "FROM" => "LEFT JOIN b_user_group UG ON (UG.USER_ID = F.USER_ID)"
            ),
        );
        ...
        if (!empty($arFilter)) {
            foreach ($arFilter as $k => $v) {
                if (mb_strpos($k, "QUANTITY_ALL") !== false || mb_strpos($k, "PRICE_ALL") !== false || mb_strpos(
                        $k,
                        "PR_COUNT"
                    ) !== false) {
                    $arFilterH[$k] = $v;
...
class CSaleUser extends CAllSaleUser
{
    ...
    public static function GetBuyersList(
        ...
        $arFields = array(
            ...
            "ORDER_COUNT" => array(
                "FIELD" => "(SELECT COUNT(O3.PRICE) FROM b_sale_order O3 WHERE O3.USER_ID=F.USER_ID AND O3.CURRENCY = '" . $DB->ForSQL(
                        $CURRENCY
                    ) . "' AND O3.PAYED = 'Y' AND O3.LID = '" . $DB->ForSQL($LID) . "' )",
                "TYPE" => "double"
            ),
            "ORDER_SUM" => array(
                "FIELD" => "(SELECT SUM(O3.PRICE) FROM b_sale_order O3 WHERE O3.USER_ID=F.USER_ID AND O3.CURRENCY = '" . $DB->ForSQL(
                        $CURRENCY
                    ) . "' AND O3.PAYED = 'Y' AND O3.LID = '" . $DB->ForSQL($LID) . "' )",
                "TYPE" => "double"
            ),
            "CURRENCY" => array(
                "FIELD" => "O1.CURRENCY",
                "TYPE" => "string",
                "FROM" => "LEFT JOIN b_sale_order O1 ON (O1.USER_ID=U.ID AND O1.CURRENCY = '" . $DB->ForSQL(
                        $CURRENCY
                    ) . "' AND O1.LID = '" . $DB->ForSQL($LID) . "' AND O1.PAYED = 'Y')"
            ),
            "LAST_ORDER_DATE" => array(
                "FIELD" => "(SELECT MAX(O2.DATE_INSERT) FROM b_sale_order O2 WHERE (O2.USER_ID=F.USER_ID))",
                "TYPE" => "datetime"
            ),
            "NAME_SEARCH" => array(
                "FIELD" => "U.NAME, U.LAST_NAME, U.SECOND_NAME, U.EMAIL, U.LOGIN, U.ID",
                "TYPE" => "string",
                "FROM" => "INNER JOIN b_user U ON (U.ID = F.USER_ID)"
            ),
            "USER_ID" => array(
                "FIELD" => "F.USER_ID",
                "TYPE" => "int",
        ...
        if (!empty($arFilter)) {
            foreach ($arFilter as $k => $v) {
                if (mb_strpos($k, "ORDER_SUM") !== false || mb_strpos($k, "ORDER_COUNT") !== false || mb_strpos(
                        $k,
                        "LAST_ORDER_DATE"
                    ) !== false) {
                    $arFilterH[$k] = $v;
