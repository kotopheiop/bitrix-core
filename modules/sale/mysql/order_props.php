...
class CSaleOrderProps extends CAllSaleOrderProps
{
    function GetList(
        ...
        if (isset($arFilter["RELATED"]) && !is_array($arFilter["RELATED"]) && intval(
                $arFilter["RELATED"]
            ) == 0) // filter all not related to anything
        {
        ...
        $arFields = array(
            ...
            "GROUP_ID" => array(
                "FIELD" => "PG.ID",
                "TYPE" => "int",
                "FROM" => "LEFT JOIN b_sale_order_props_group PG ON (P.PROPS_GROUP_ID = PG.ID)"
            ),
            "GROUP_PERSON_TYPE_ID" => array(
                "FIELD" => "PG.PERSON_TYPE_ID",
                "TYPE" => "int",
                "FROM" => "LEFT JOIN b_sale_order_props_group PG ON (P.PROPS_GROUP_ID = PG.ID)"
            ),
            "GROUP_NAME" => array(
            ...
            "PERSON_TYPE_LID" => array(
                "FIELD" => "SPT.LID",
                "TYPE" => "string",
                "FROM" => "LEFT JOIN b_sale_person_type SPT ON (P.PERSON_TYPE_ID = SPT.ID)"
            ),
            ...
            "PERSON_TYPE_SORT" => array(
                "FIELD" => "SPT.SORT",
                "TYPE" => "int",
                "FROM" => "LEFT JOIN b_sale_person_type SPT ON (P.PERSON_TYPE_ID = SPT.ID)"
            ),
            "PERSON_TYPE_ACTIVE" => array(
                "FIELD" => "SPT.ACTIVE",
                "TYPE" => "char",
                "FROM" => "LEFT JOIN b_sale_person_type SPT ON (P.PERSON_TYPE_ID = SPT.ID)"
            ),
            ...
            "PAYSYSTEM_ID" => array(
                "FIELD" => "SOP.PROPERTY_ID",
                "TYPE" => "int",
                "FROM" => "LEFT JOIN b_sale_order_props_relation SOP ON P.ID = SOP.PROPERTY_ID",
                "WHERE" => array("CSaleOrderProps", "PrepareRelation4Where")
            ),
            "DELIVERY_ID" => array(
                "FIELD" => "SOD.PROPERTY_ID",
                "TYPE" => "string",
                "FROM" => "LEFT JOIN b_sale_order_props_relation SOD ON P.ID = SOD.PROPERTY_ID",
                "WHERE" => array("CSaleOrderProps", "PrepareRelation4Where")
            )
        );
        // <-- FIELDS
        ...
        if (isset($arFilter["RELATED"])) {
            if (!is_array($arFilter["RELATED"]) && intval($arFilter["RELATED"]) == 0) {
                ...
                if (isset($arFilter["PAYSYSTEM_ID"]) && intval($arFilter["PAYSYSTEM_ID"]) > 0) {
                    $strSqlRelatedWhere .= "(SOP.ENTITY_TYPE = 'P' AND SOP.ENTITY_ID = " . $DB->ForSql(
                            $arFilter["PAYSYSTEM_ID"]
                        ) . ")";
                }
                ...
                if (isset($arFilter["DELIVERY_ID"]) && strlen($arFilter["DELIVERY_ID"]) > 0) {
                    ...
                    $strSqlRelatedWhere .= "(SOD.ENTITY_TYPE = 'D' AND SOD.ENTITY_ID = '" . $DB->ForSql(
                            $arFilter["DELIVERY_ID"]
                        ) . "')";
                }
