...
class CSaleTaxRate extends CAllSaleTaxRate
{
    ...
    public static function GetList($arOrder = array("APPLY_ORDER" => "ASC"), $arFilter = array())
    {
        ...
        for ($i = 0; $i < $countFilteKey; $i++) {
            ...
            switch (ToUpper($key)) {
                ...
                case "TAX_ID":
                    ...
                    break;
                case "PERSON_TYPE_ID":
                    $arSqlSearch[] = " (TR.PERSON_TYPE_ID " . ($bInvert ? "<>" : "=") . " " . intval(
                            $val
                ...
                case "LOCATION":
                    ...
                    if (CSaleLocation::isLocationProMigrated()) {
                        try {
                            ...
                            $arSqlSearch[] = "	TR.ID in (" . $class::getConnectedEntitiesQuery(
                                    intval($val),
                                    'id',
        ...
        $strSql =
            ...
            "	TR.IS_PERCENT, TR.IS_IN_PRICE, TR.APPLY_ORDER, " . $DB->DateToCharFunction(
                "TR.TIMESTAMP_X",
                "FULL"
            ) . " as TIMESTAMP_X, " .
