...
if ($params['BILL_HEADER_SHOW'] == 'Y') {
    ...
    if ($params["SELLER_COMPANY_PHONE"]) {
        $text = CSalePdf::prepareToPdf(
            Loc::getMessage('SALE_HPS_BILL_SELLER_COMPANY_PHONE', array('#PHONE#' => $params["SELLER_COMPANY_PHONE"]))
        );
        $textWidth = $width - $logoWidth;
        while ($pdf->GetStringWidth($text)) {
            list($string, $text) = $pdf->splitString($text, $textWidth);
...
if ($params['BILL_HEADER']) {
    ...
    $billNo_tmp = CSalePdf::prepareToPdf(
        $params['BILL_HEADER'] . ' ' . Loc::getMessage(
            'SALE_HPS_BILL_SELLER_TITLE',
            array(
                '#PAYMENT_NUM#' => $params["ACCOUNT_NUMBER"],
                '#PAYMENT_DATE#' => $params["PAYMENT_DATE_INSERT"]
            )
        )
    );
    ...
    if ($params["BUYER_PERSON_COMPANY_NAME"]) {
        $pdf->Write(
            ...
            CSalePdf::prepareToPdf(
                Loc::getMessage(
                    ...
                    array('#BUYER_NAME#' => $params["BUYER_PERSON_COMPANY_NAME"])
                )
            )
        );
        if ($params["BUYER_PERSON_COMPANY_INN"]) {
            $pdf->Write(
                ...
                CSalePdf::prepareToPdf(
                    Loc::getMessage(
                        'SALE_HPS_BILL_BUYER_PERSON_INN',
                        array('#INN#' => $params["BUYER_PERSON_COMPANY_INN"])
                    )
                )
            );
        }
    ...
    if ($params['DELIVERY_PRICE'] > 0) {
        ...
        foreach ($arCols as $columnId => $col) {
            ...
            switch ($columnId) {
                ...
                case 'VAT_RATE':
                    $data = CSalePdf::prepareToPdf(
                        roundEx($params['DELIVERY_VAT_RATE'] * 100, SALE_VALUE_PRECISION) . "%"
                    );
                    break;
                case 'SUM':
                    $data = CSalePdf::prepareToPdf(
    ...
    if ($params['BILL_TOTAL_SHOW'] == 'Y') {
        ...
        if ($params['SUM'] - $sum > $eps) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
                SaleFormatCurrency($sum, $params['CURRENCY'], true)
            );
        }
                ...
                $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                    sprintf(
                        ...
                        ($vat <= 0 && $tax["IS_PERCENT"] == "Y") ? sprintf(
                            ' (%s%%)',
                            roundEx($tax["VALUE"], SALE_VALUE_PRECISION)
                        ) : ""
                    )
                );
                $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
        ...
        if (!$params['TAXES']) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILL_TOTAL_VAT_RATE')
            );
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILL_TOTAL_VAT_RATE_NO')
            );
        }
