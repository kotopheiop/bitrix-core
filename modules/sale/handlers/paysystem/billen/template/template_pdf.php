        ...
        if ($basketItem['PROPS']) {
            foreach ($basketItem['PROPS'] as $basketItemProperty) {
                ...
                $arProps[$n][] = CSalePdf::prepareToPdf(
                    sprintf("%s: %s", $basketItemProperty["NAME"], $basketItemProperty["VALUE"])
                );
            }
        }
        ...
        if (!empty($vats)) {
            ...
            foreach ($vats as $vatRate => $vatSum) {
                ...
                $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                    sprintf("Tax (%s%%):", roundEx($vatRate * 100, SALE_VALUE_PRECISION))
                );
                $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
                    SaleFormatCurrency($vatSum, $params['CURRENCY'], false)
                );
            }
        } else {
            if ($params['TAXES']) {
                ...
                $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
                    SaleFormatCurrency(0, $params['CURRENCY'], false)
                );
            }
        }
