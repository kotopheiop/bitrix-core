...
class WooppayHandler extends Sale\PaySystem\ServiceHandler implements Sale\PaySystem\IRefund
{
    ...
    private function send(string $method, string $url, array $headers, array $params = []): Sale\PaySystem\ServiceResult
    {
        ...
        if ($response === false) {
            ...
            $result->addError(
                new Main\Error(Main\Localization\Loc::getMessage('SALE_HPS_WOOPPAY_ERROR_EMPTY_RESPONSE'))
            );
    ...
    private function verifyResponse(string $response, int $httpStatus): Sale\PaySystem\ServiceResult
    {
        ...
        if (!$responseData) {
            $result->addError(
                new Main\Error(Main\Localization\Loc::getMessage('SALE_HPS_WOOPPAY_ERROR_DECODE_RESPONSE'))
            );
        }

        if ($httpStatus === self::HTTP_CODE_UNAUTHORIZED) {
            $result->addError(
                new Main\Error(Main\Localization\Loc::getMessage('SALE_HPS_WOOPPAY_ERROR_STATUS_UNAUTHORIZED'))
            );
            ...
            $result->addError(
                new Main\Error(Main\Localization\Loc::getMessage('SALE_HPS_WOOPPAY_ERROR_STATUS_UNPROCESSABLE_ENTITY'))
            );
            ...
            $result->addError(
                new Main\Error(Main\Localization\Loc::getMessage('SALE_HPS_WOOPPAY_ERROR_STATUS_INTERNAL_SERVER_ERROR'))
            );
