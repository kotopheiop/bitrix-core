...
class PlatonHandler extends PaySystem\ServiceHandler implements PaySystem\IRefund
{
    ...
    public function processRequest(Payment $payment, Request $request)
    {
        $result = new PaySystem\ServiceResult();
        Logger::addDebugInfo(
            __CLASS__ . ': request payload: ' . Json::encode($request->getValues(), JSON_UNESCAPED_UNICODE)
        );
    ...
    public function refund(Payment $payment, $refundableSum)
    {
        ...
        if ($cardNumber) {
            ...
            Logger::addDebugInfo(
                __CLASS__ . ': refund payload: ' . Json::encode($responseData, JSON_UNESCAPED_UNICODE)
            );
            switch ($responseData['result']) {
                case self::REFUND_STATUS_ACCEPTED:
                    $newDescription = $payment->getField('PS_STATUS_DESCRIPTION') . ' ' . Loc::getMessage(
     ...
     * @return string
     */
    private function getCallbackSignature(
        string $email,
        string $password,
