...
<div style="margin: 0pt; padding: <?= join(
    ...
    <? if ($params['BILLKZ_HEADER_SHOW'] == 'Y'): ?>
        ...
        <table class="acc" width="100%">
            ...
            <tr class="nbb">
                <td><b>
                        <nobr><?= Loc::getMessage('SALE_HPS_BILLKZ_BENEFICIARY') ?>:</nobr>
                    </b></td>
                <td colspan="2" style="text-align: center;"><b>
                        <nobr><?= Loc::getMessage('SALE_HPS_BILLKZ_IIK') ?></nobr>
                    </b></td>
                <td style="text-align: center;"><b>
                        <nobr><?= Loc::getMessage('SALE_HPS_BILLKZ_KBE') ?></nobr>
                    </b></td>
            </tr>
            <tr class="ntb nbb">
                <td><b><?= htmlspecialcharsbx($params["SELLER_COMPANY_NAME"]) ?></b></td>
                <td colspan="2" style="text-align: center;"><?= $sellerRs ? htmlspecialcharsbx(
                        $sellerRs
                    ) : '&nbsp;' ?></td>
                <td style="text-align: center;"><?= $params["SELLER_COMPANY_KBE"] ? htmlspecialcharsbx(
                        $params["SELLER_COMPANY_KBE"]
                    ) : '&nbsp;' ?></td>
            </tr>
            <tr class="ntb">
                <td><?= $params["SELLER_COMPANY_BIN"] ? Loc::getMessage(
                            'SALE_HPS_BILLKZ_BIN'
                        ) . ': ' . htmlspecialcharsbx($params["SELLER_COMPANY_BIN"]) : '&nbsp;' ?></td>
                <td colspan="2">&nbsp;</td>
                <td>&nbsp;</td>
            </tr>
            <tr class="ntb nbb">
                <td>
                    <nobr><?= Loc::getMessage('SALE_HPS_BILLKZ_BENEFICIARY_BANK') ?>:</nobr>
                </td>
                <td style="text-align: center;"><b>
                        <nobr><?= Loc::getMessage('SALE_HPS_BILLKZ_SELLER_BANK_BIK') ?></nobr>
                    </b></td>
                <td colspan="2" style="text-align: center;"><b>
                        <nobr><?= Loc::getMessage('SALE_HPS_BILLKZ_PAYMENT_PC') ?></nobr>
                    </b></td>
            </tr>
            <tr class="ntb">
                <td><?= $sellerBank ? htmlspecialcharsbx($sellerBank) : '&nbsp;' ?></td>
                <td style="text-align: center;"><?= $params["SELLER_COMPANY_BANK_BIC"] ? htmlspecialcharsbx(
                        $params["SELLER_COMPANY_BANK_BIC"]
                    ) : '&nbsp;' ?></td>
                <td colspan="2" style="text-align: center;"><?= $params["PAYMENT_PC"] ? htmlspecialcharsbx(
                        $params["PAYMENT_PC"]
                    ) : '&nbsp;' ?></td>
            </tr>
        ...
        <? if ($params['BILLKZ_HEADER']): ?>
            ...
            <tr>
                <td style="font-size: 1.6em; font-weight: bold;">
                    <nobr><?= htmlspecialcharsbx($params['BILLKZ_HEADER']); ?> <?= Loc::getMessage(
                            'SALE_HPS_BILLKZ_SELLER_TITLE',
                            array(
                                '#PAYMENT_NUM#' => htmlspecialcharsbx($params["ACCOUNT_NUMBER"]),
                                '#PAYMENT_DATE#' => htmlspecialcharsbx($dateValue)
                            )
                        ); ?></nobr>
                </td>
            </tr>
    ...
    <table width="100%" style="margin: 16pt 0;">
    ...
    if ($params['BILLKZ_PAYER_SHOW'] == 'Y') {
        if ($params["BUYER_PERSON_COMPANY_BIN"] || $params["BUYER_PERSON_COMPANY_IIN"]) {
            ...
            if ($params["BUYER_PERSON_COMPANY_BIN"]) {
                ...
                if ($params["BUYER_PERSON_COMPANY_IIN"]) {
    ...
    <table class="selbuy" width="100%">
        ...
        <? if ($params['BILLKZ_PAYER_SHOW'] == 'Y'): ?>
            <tr>
                <td colspan="2" style="padding: 0; line-height: 7pt;">&nbsp;</td>
            </tr>
            <tr>
        ...
        foreach ($arCols as $columnId => $caption) {
            ...
            switch ($columnId) {
                ...
                case 'MEASURE':
                    ...
                    ) : Loc::getMessage('SALE_HPS_BILLKZ_BASKET_MEASURE_DEFAULT');
                    break;
                case 'PRICE':
                ...
                case 'SUM':
                    $data = SaleFormatCurrency(
                        $basketItemPrice * $basketItem['QUANTITY'],
                        $basketItem['CURRENCY'],
                        true
                    );
                    break;
    ...
    if ($params['BILLKZ_TOTAL_SHOW'] == 'Y') {
        ...
        if ($params['SUM'] - $sum > $eps) {
            ...
            $cells[$n][$arColumnKeys[$columnCount - 2]] = htmlspecialcharsbx(
                Loc::getMessage('SALE_HPS_BILLKZ_SUBTOTAL')
            );
        ...
        if ($params['TAXES']) {
            foreach ($params['TAXES'] as $tax) {
                ...
                $cells[$n][$arColumnKeys[$columnCount - 1]] = SaleFormatCurrency(
                    $tax["VALUE_MONEY"],
                    $params['CURRENCY'],
                    true
                );
        ...
        if (!$params['TAXES']) {
            ...
            $cells[$n][$arColumnKeys[$columnCount - 2]] = htmlspecialcharsbx(
                Loc::getMessage('SALE_HPS_BILLKZ_TOTAL_VAT_RATE')
            );
            $cells[$n][$arColumnKeys[$columnCount - 1]] = htmlspecialcharsbx(
                Loc::getMessage('SALE_HPS_BILLKZ_TOTAL_VAT_RATE_NO')
            );
        }
