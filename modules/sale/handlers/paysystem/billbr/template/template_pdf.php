...
if ($params["BUYER_PERSON_COMPANY_NAME"]) {
    $pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_FOR', null, $billLang)));
}
...
if ($params['BILLBR_PAYER_SHOW'] === 'Y') {
    ...
    $title = CSalePdf::prepareToPdf(
        $params['BILLBR_HEADER'] . ' ' . Loc::getMessage('SALE_HPS_BILLBR_NUMBER', null, $billLang) . ': '
    );
...
if ($params['BILLBR_PAYER_SHOW'] === 'Y') {
    ...
    $title = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_DATE_INSERT', null, $billLang) . ': ');
    $titleWidth = $pdf->GetStringWidth($title);
    $pdf->SetX($pdf->GetX() - $invoiceInfoWidth - $titleWidth - 6);
    $pdf->Write(15, $title, 0, 0, 'R');
}
...
if ($params["DATE_PAY_BEFORE"]) {
    ...
    $title = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_DATE_PAY_BEFORE', null, $billLang) . ': ');
    $titleWidth = $pdf->GetStringWidth($title);
    $pdf->SetX($pdf->GetX() - $invoiceInfoWidth - $titleWidth - 6);
    $pdf->Write(15, $title, 0, 0, 'R');
}
    ...
    if ($params['BILLBR_TOTAL_SHOW'] == 'Y') {
        ...
        if ($params['SUM'] - $sum > $eps) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLBR_SUB_TOTAL', null, $billLang) . ":"
            );
        ...
        if ($params['SUM_PAID']) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLBR_SUM_PAID', null, $billLang) . ":"
            );
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
        ...
        if ($params['DISCOUNT_PRICE']) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLBR_DISCOUNT', null, $billLang) . ":"
            );
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
        ...
        $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
            Loc::getMessage('SALE_HPS_BILLBR_TOTAL', null, $billLang) . ":"
        );
        $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
...
if ($params["BILLBR_COMMENT1"] || $params["BILLBR_COMMENT2"]) {
    $pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_COMMENT', null, $billLang)));
    $pdf->Ln();
...
if ($bankAccNo && $bankRouteNo && $bankSwift) {
    ...
    $pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_DETAIL', null, $billLang)));
    $pdf->Ln();
    ...
    if ($bank || $bankAddr || $bankPhone) {
        $bankDetails .= CSalePdf::prepareToPdf(
            Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_2', null, $billLang) . ": "
        );
        if ($bank) {
    ...
    $bankDetails .= CSalePdf::prepareToPdf(
        sprintf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_ROUTE_NO', null, $billLang) . ": %s\n", $bankRouteNo)
    );
    $bankDetails .= CSalePdf::prepareToPdf(
        sprintf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_SWIFT', null, $billLang) . ": %s\n", $bankSwift)
    );
