...
if ($params["SELLER_COMPANY_PHONE"]) {
    ...
    $text = CSalePdf::prepareToPdf(
        sprintf(
            Loc::getMessage('SALE_HPS_BILLLA_COMPANY_PHONE', null, $billLang) . ".: %s",
...
if ($params["BUYER_PERSON_COMPANY_NAME"]) {
    $pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_FOR', null, $billLang)));
}
...
if ($params['BILLLA_PAYER_SHOW'] === 'Y') {
    ...
    $title = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_DATE_INSERT', null, $billLang) . ': ');
    $titleWidth = $pdf->GetStringWidth($title);
    $pdf->SetX($pdf->GetX() - $invoiceInfoWidth - $titleWidth - 6);
    $pdf->Write(15, $title, 0, 0, 'R');
}
...
if ($params["DATE_PAY_BEFORE"]) {
    ...
    $title = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_DATE_PAY_BEFORE', null, $billLang) . ': ');
    $titleWidth = $pdf->GetStringWidth($title);
    $pdf->SetX($pdf->GetX() - $invoiceInfoWidth - $titleWidth - 6);
    $pdf->Write(15, $title, 0, 0, 'R');
}
...
if ($params['BASKET_ITEMS']) {
    ...
    foreach ($params['BASKET_ITEMS'] as $basketItem) {
        ...
        if ($productName == "OrderDelivery") {
            $productName = Loc::getMessage('SALE_HPS_BILLLA_DELIVERY', null, $billLang);
        } else {
            if ($productName == "OrderDiscount") {
                $productName = Loc::getMessage('SALE_HPS_BILLLA_DISCOUNT', null, $billLang);
            }
        }
    ...
    if ($params['DELIVERY_PRICE'] > 0) {
        $sDeliveryItem = Loc::getMessage('SALE_HPS_BILLLA_DELIVERY', null, $billLang);
    ...
    if ($params['BILLLA_TOTAL_SHOW'] == 'Y') {
        ...
        if ($params['SUM'] - $sum > $eps) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLLA_SUB_TOTAL', null, $billLang) . ":"
            );
        ...
        if (!empty($vats)) {
            ...
            foreach ($vats as $vatRate => $vatSum) {
                ...
                $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                    sprintf(
                        Loc::getMessage('SALE_HPS_BILLLA_TAX', null, $billLang) . " (%s%%):",
        ...
        if ($params['SUM_PAID']) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLLA_SUM_PAID', null, $billLang) . ":"
            );
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
        ...
        if ($params['DISCOUNT_PRICE']) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLLA_DISCOUNT', null, $billLang) . ":"
            );
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
        ...
        $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
            Loc::getMessage('SALE_HPS_BILLLA_TOTAL', null, $billLang) . ":"
        );
        $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
...
if ($params["BILLLA_COMMENT1"] || $params["BILLLA_COMMENT2"]) {
    $pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_COMMENT', null, $billLang)));
    $pdf->Ln();
...
if ($bankAccNo && $bankRouteNo && $bankSwift) {
    ...
    $pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLLA_COMPANY_BANK_DETAIL', null, $billLang)));
    $pdf->Ln();
    ...
    if ($params["SELLER_COMPANY_NAME"]) {
        $bankDetails .= CSalePdf::prepareToPdf(
            sprintf(
                Loc::getMessage('SALE_HPS_BILLLA_COMPANY_NAME', null, $billLang) . ": %s\n",
    ...
    $bankDetails .= CSalePdf::prepareToPdf(
        sprintf("# " . Loc::getMessage('SALE_HPS_BILLLA_COMPANY_BANK', null, $billLang) . ": %s\n", $bankAccNo)
    );
    ...
    if ($bank || $bankAddr || $bankPhone) {
        $bankDetails .= CSalePdf::prepareToPdf(
            Loc::getMessage('SALE_HPS_BILLLA_COMPANY_BANK_2', null, $billLang) . ": "
        );
        if ($bank) {
    ...
    $bankDetails .= CSalePdf::prepareToPdf(
        sprintf(Loc::getMessage('SALE_HPS_BILLLA_COMPANY_BANK_ROUTE_NO', null, $billLang) . ": %s\n", $bankRouteNo)
    );
    $bankDetails .= CSalePdf::prepareToPdf(
        sprintf(Loc::getMessage('SALE_HPS_BILLLA_COMPANY_BANK_SWIFT', null, $billLang) . ": %s\n", $bankSwift)
    );
