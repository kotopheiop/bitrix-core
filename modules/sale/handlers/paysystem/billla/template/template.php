...
<html>
<head>
    <title><?= Loc::getMessage('SALE_HPS_BILLLA_TITLE', null, $billLang) ?></title>
...
<div style="margin: 0pt; padding: <?= join(
    ...
    <? if ($params['BILLLA_HEADER']): ?>
        <div style="text-align: center; font-size: 2em"><b><?= htmlspecialcharsbx($params['BILLLA_HEADER']); ?></b>
        </div>
    ...
    <table width="100%">
        <tr>
            <? if ($params["BUYER_PERSON_COMPANY_NAME"]) { ?>
                <td>
                    <b><?= Loc::getMessage('SALE_HPS_BILLLA_FOR', null, $billLang) ?></b><br>
            ...
            <td align="right">
                <? if ($params['BILLLA_PAYER_SHOW'] === 'Y'): ?>
                    <table class="inv">
                        ...
                        <tr align="right">
                            <td><b><?= Loc::getMessage('SALE_HPS_BILLLA_DATE_INSERT', null, $billLang) ?>:&nbsp;</b>
                            </td>
                            <td><?= htmlspecialcharsbx($params["DATE_INSERT"]); ?></td>
                        </tr>
                        <? if ($params["DATE_PAY_BEFORE"]) { ?>
                            <tr align="right">
                                <td><b><?= Loc::getMessage('SALE_HPS_BILLLA_DATE_PAY_BEFORE', null, $billLang) ?>
                                        :&nbsp;</b></td>
                                <td><?= (
            ...
            foreach ($arCols as $columnId => $col) {
                ...
                switch ($columnId) {
                    ...
                    case 'MEASURE':
                        ...
                        ) : Loc::getMessage('SALE_HPS_BILLLA_MEASURE', null, $billLang);
                        break;
                    case 'PRICE':
    ...
    <? if ($params["BILLLA_COMMENT1"] || $params["BILLLA_COMMENT2"]) { ?>
        <b><?= Loc::getMessage('SALE_HPS_BILLLA_COMMENT', null, $billLang) ?></b>
        <br>
        ...
        <tr>
            <td style="width: 50%; ">
                ...
                <? if ($bankAccNo && $bankRouteNo && $bankSwift) { ?>
                    ...
                    <b><?= Loc::getMessage('SALE_HPS_BILLLA_COMPANY_BANK_DETAIL', null, $billLang) ?></b>
                    <br>

                    <? if ($params["SELLER_COMPANY_NAME"]) { ?>
                        <?= Loc::getMessage('SALE_HPS_BILLLA_COMPANY_NAME', null, $billLang) ?>: <?= htmlspecialcharsbx(
                            $params["SELLER_COMPANY_NAME"]
                        ); ?>
                        <br>
                    <? } ?>

                    # <?= Loc::getMessage('SALE_HPS_BILLLA_COMPANY_BANK', null, $billLang) ?>: <?= htmlspecialcharsbx(
                        $bankAccNo
                    ); ?>
                    <br>
                    ...
                    <? if ($bank || $bankAddr || $bankPhone) { ?>
                        <?= Loc::getMessage(
                            'SALE_HPS_BILLLA_COMPANY_BANK_2',
                            null,
                            $billLang
                    ...
                    <?= Loc::getMessage(
                    'SALE_HPS_BILLLA_COMPANY_BANK_ROUTE_NO',
                    null,
                    $billLang
                    ...
                    <?= Loc::getMessage(
                    'SALE_HPS_BILLLA_COMPANY_BANK_SWIFT',
                    null,
                    $billLang
