...
<div class="paysystem-yandex mb-4" id="paysystem-assest">
    <form id="paysystem-assest-form" action="<?= $params['URL']; ?>" METHOD="POST" target="_blank">
        ...
        <input type="hidden" name="CardPayment"
               value="<?= ((int)$params['ASSIST_PAYMENT_CardPayment'] == 1) ? 1 : 0; ?>">
        <input type="hidden" name="YMPayment" value="<?= ((int)$params['ASSIST_PAYMENT_YMPayment'] == 1) ? 1 : 0; ?>">
        <input type="hidden" name="QIWIPayment"
               value="<?= ((int)$params['ASSIST_PAYMENT_QIWIPayment'] == 1) ? 1 : 0; ?>">
        <input type="hidden" name="WMPayment"
               value="<?= ((int)$params['ASSIST_PAYMENT_WebMoneyPayment'] == 1) ? 1 : 0; ?>">
