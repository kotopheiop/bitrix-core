...
if ($params['BILLUA_HEADER']) {
    $pdf->Write(
        15,
        CSalePdf::prepareToPdf($params['BILLUA_HEADER']) . CSalePdf::prepareToPdf(
            Loc::getMessage(
                'SALE_HPS_BILLUA_TITLE',
                array(
                    '#PAYMENT_NUMBER#' => htmlspecialcharsbx($params["ACCOUNT_NUMBER"]),
                    '#PAYMENT_DATE#' => $params["DATE_INSERT"]
                ),
                $billLang
            )
        )
...
if ($params['BILLUA_SELLER_SHOW'] == 'Y') {
    ...
    $pdf->MultiCell(
        ...
        CSalePdf::prepareToPdf(
            sprintf(
                Loc::getMessage('SALE_HPS_BILLUA_SELLER_COMPANY_RS', null, $billLang) . ' %s, ' . Loc::getMessage(
                    'SALE_HPS_BILLUA_SELLER_COMPANY_BANK',
                    null,
                    $billLang
                ) . ' %s, ' . Loc::getMessage('SALE_HPS_BILLUA_SELLER_COMPANY_MFO', null, $billLang) . ' %s',
    ...
    if ($params["SELLER_COMPANY_PHONE"]) {
        $string .= ', ' . sprintf(
                Loc::getMessage('SALE_HPS_BILLUA_SELLER_COMPANY_PHONE', null, $billLang) . ': %s',
...
if ($params['BILLUA_PAYER_SHOW'] === 'Y') {
    $pdf->Cell(
        $title_width,
        15,
        CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLUA_BUYER', null, $billLang) . ': ')
    );
    ...
    if ($buyerPhone || $buyerFax) {
        ...
        if ($buyerPhone) {
            $pdf->Write(
                ...
                CSalePdf::prepareToPdf(
                    sprintf(
                        Loc::getMessage('SALE_HPS_BILLUA_BUYER_PHONE', null, $billLang) . ': %s',
                        $buyerPhone
                    )
                )
            );
        ...
        if ($buyerFax) {
            $pdf->Write(
                15,
                CSalePdf::prepareToPdf(
                    sprintf(Loc::getMessage('SALE_HPS_BILLUA_BUYER_FAX', null, $billLang) . ': %s', $buyerFax)
                )
            );
        }
...
if ($params['BASKET_ITEMS']) {
    ...
    foreach ($params['BASKET_ITEMS'] as $basketItem) {
        ...
        foreach ($arCols as $columnId => $col) {
            ...
            switch ($columnId) {
                ...
                case 'SUM':
                    $data = CSalePdf::prepareToPdf(
                        SaleFormatCurrency($basketItemPrice * $basketItem['QUANTITY'], $basketItem['CURRENCY'], true)
                    );
                    $arCols[$columnId]['IS_DIGIT'] = true;
                    break;
                default:
    ...
    if ($params['BILLUA_TOTAL_SHOW'] === 'Y') {
        ...
        if ($params['SUM_PAID']) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLUA_PAYMENT_PAID', null, $billLang) . ":"
            );
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
                SaleFormatCurrency($params['DELIVERY_PRICE'], $params['CURRENCY'], true)
            );
        ...
        if ($params['DISCOUNT_PRICE']) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLUA_DISCOUNT', null, $billLang) . ":"
            );
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
        ...
        $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
            Loc::getMessage('SALE_HPS_BILLUA_SUM', null, $billLang) . ':'
        );
        $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
...
if ($params['BILLUA_TOTAL_SHOW'] === 'Y') {
    ...
    if ($vat > 0) {
        ...
        $pdf->Write(
            15,
            CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLUA_BASKET_ITEMS_WITHOUT_TAX', null, $billLang))
        );
    }
    $pdf->Ln();
    $pdf->Ln();
...
if ($params['BILLUA_FOOTER_SHOW'] == 'Y') {
    ...
    if ($params["SELLER_COMPANY_ACCOUNTANT_POSITION"]) {
        ...
        $pdf->Write(
            15,
            CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLUA_ACC_POSITION', null, $billLang) . ': ')
        );
