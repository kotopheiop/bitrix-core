...
<div class="mb-4">
    <form id="pay" name="pay" method="POST" action="<?= $params['URL'] ?>">
        ...
        <input type="hidden" name="LMI_PAYMENT_DESC" value="<?= str_replace(
            array('#PAYMENT_ID#', '#DATE_INSERT#'),
            array($params['PAYMENT_ID'], $params['PAYMENT_DATE_INSERT']),
            Loc::getMessage('SALE_HPS_PAYMASTER_TEMPLATE_DESC_PAYMENT')
        ) ?>">
        ...
        <input type="hidden" name="LMI_PAYER_PHONE_NUMBER"
        ...
        <input type="submit" class="btn btn-lg btn-success pl-4 pr-4" style="border-radius: 32px;"
               value="<?= Loc::getMessage('SALE_HPS_PAYMASTER_TEMPLATE_BUTTON_PAID'); ?>">
    </form>
</div>