...
if ($params["SELLER_COMPANY_PHONE"]) {
    ...
    $text = CSalePdf::prepareToPdf(
        sprintf(
            Loc::getMessage('SALE_HPS_BILLFR_COMPANY_PHONE', null, $billLang) . ".: %s",
...
if ($params["BUYER_PERSON_COMPANY_NAME"]) {
    $pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLFR_FOR', null, $billLang)));
}
...
if ($params['BILLFR_PAYER_SHOW'] === 'Y') {
    ...
    $title = CSalePdf::prepareToPdf(
        $params['BILLFR_HEADER'] . ' ' . Loc::getMessage('SALE_HPS_BILLFR_NUMBER', null, $billLang) . ': '
    );
...
if ($params['BILLFR_PAYER_SHOW'] === 'Y') {
    ...
    $title = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLFR_DATE_INSERT', null, $billLang) . ': ');
    $titleWidth = $pdf->GetStringWidth($title);
    $pdf->SetX($pdf->GetX() - $invoiceInfoWidth - $titleWidth - 6);
    $pdf->Write(15, $title, 0, 0, 'R');
}
...
if ($params["DATE_PAY_BEFORE"]) {
    ...
    $title = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLFR_DATE_PAY_BEFORE', null, $billLang) . ': ');
    $titleWidth = $pdf->GetStringWidth($title);
    $pdf->SetX($pdf->GetX() - $invoiceInfoWidth - $titleWidth - 6);
    $pdf->Write(15, $title, 0, 0, 'R');
}
    ...
    foreach ($params['BASKET_ITEMS'] as $basketItem) {
        ...
        foreach ($arCols as $columnId => $col) {
            ...
            switch ($columnId) {
                ...
                case 'SUM':
                    $data = CSalePdf::prepareToPdf(
                        SaleFormatCurrency($vatLessPrice * $basketItem['QUANTITY'], $basketItem['CURRENCY'], false)
                    );
                    $arCols[$columnId]['IS_DIGIT'] = true;
                    break;
                default :
        ...
        if ($basketItem['PROPS']) {
            ...
            foreach ($basketItem['PROPS'] as $basketPropertyItem) {
                ...
                $arProps[$n][] = CSalePdf::prepareToPdf(
    ...
    if ($params['DELIVERY_PRICE'] > 0) {
        ...
        foreach ($arCols as $columnId => $col) {
            ...
            switch ($columnId) {
                ...
                case 'PRICE':
                    $data = CSalePdf::prepareToPdf(
                        SaleFormatCurrency($params['DELIVERY_PRICE'] / (1 + $vat), $params['CURRENCY'], false)
                    );
                    break;
                case 'VAT_RATE':
    ...
    if ($params['BILLFR_TOTAL_SHOW'] == 'Y') {
        ...
        if ($params['SUM'] - $sum > $eps) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLFR_SUB_TOTAL', null, $billLang) . ":"
            );
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
                SaleFormatCurrency($sum, $params['CURRENCY'], false)
            );
        }
        ...
        if (!empty($vats)) {
            ...
            foreach ($vats as $vatRate => $vatSum) {
                ...
                $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                    sprintf(
                        Loc::getMessage('SALE_HPS_BILLFR_TAX', null, $billLang) . " (%s%%):",
            ...
            if ($params['TAXES']) {
                foreach ($params['TAXES'] as $tax) {
                    ...
                    $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
                        SaleFormatCurrency($tax["VALUE_MONEY"], $params["CURRENCY"], false)
                    );
                }
            }
        }
        ...
        if ($params['SUM_PAID']) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLFR_SUM_PAID', null, $billLang) . ":"
            );
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
                SaleFormatCurrency($params['SUM_PAID'], $params['CURRENCY'], false)
            );
        }
        ...
        if ($params['DISCOUNT_PRICE']) {
            ...
            $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
                Loc::getMessage('SALE_HPS_BILLFR_DISCOUNT', null, $billLang) . ":"
            );
            $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
                SaleFormatCurrency($params['DISCOUNT_PRICE'], $params['CURRENCY'], false)
            );
        }
        ...
        $arCells[$n][$arColumnKeys[$columnCount - 2]] = CSalePdf::prepareToPdf(
            Loc::getMessage('SALE_HPS_BILLFR_TOTAL', null, $billLang) . ":"
        );
        $arCells[$n][$arColumnKeys[$columnCount - 1]] = CSalePdf::prepareToPdf(
...
if ($params["BILLFR_COMMENT1"] || $params["BILLFR_COMMENT2"]) {
    $pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLFR_COMMENT', null, $billLang)));
    $pdf->Ln();
...
if ($bankAccNo && $bankRouteNo && $bankSwift) {
    ...
    $pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLFR_COMPANY_BANK_DETAIL', null, $billLang)));
    $pdf->Ln();
    ...
    if ($params["SELLER_COMPANY_NAME"]) {
        $bankDetails .= CSalePdf::prepareToPdf(
            sprintf(
                Loc::getMessage('SALE_HPS_BILLFR_COMPANY_NAME', null, $billLang) . ": %s\n",
    ...
    $bankDetails .= CSalePdf::prepareToPdf(
        sprintf(
            Loc::getMessage('SALE_HPS_BILLFR_COMPANY_BANK', null, $billLang) . " " . Loc::getMessage(
                'SALE_HPS_BILLFR_NUMBER',
                null,
                $billLang
            ) . ": %s\n",
            $bankAccNo
    ...
    if ($bank || $bankAddr || $bankPhone) {
        $bankDetails .= CSalePdf::prepareToPdf(
            Loc::getMessage('SALE_HPS_BILLFR_COMPANY_BANK_2', null, $billLang) . ": "
        );
        if ($bank) {
    ...
    $bankDetails .= CSalePdf::prepareToPdf(
        sprintf(Loc::getMessage('SALE_HPS_BILLFR_COMPANY_BANK_ROUTE_NO', null, $billLang) . ": %s\n", $bankRouteNo)
    );
    $bankDetails .= CSalePdf::prepareToPdf(
        sprintf(Loc::getMessage('SALE_HPS_BILLFR_COMPANY_BANK_SWIFT', null, $billLang) . ": %s\n", $bankSwift)
    );
...
$trFileName = CUtil::translit($fileName, 'fr', array('max_len' => 1024, 'safe_chars' => '.', 'replace_space' => '-'));
