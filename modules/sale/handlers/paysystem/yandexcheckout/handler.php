    ...
    PaySystem\IRecurring
{
     ...
     * @throws Main\SystemException
     */
    private function createYandexPayment(
        Payment $payment,
        Request $request,
    ...
    public function processRequest(Payment $payment, Request $request)
    {
        ...
        if ($data !== false) {
            ...
            if ($response['status'] === static::PAYMENT_STATUS_SUCCEEDED) {
                ...
                $error = Localization\Loc::getMessage(
                        'SALE_HPS_YANDEX_CHECKOUT_ERROR_STATUS'
                    ) . ': ' . $response['status'];
                $result->addError(PaySystem\Error::create($error));
            ...
            $result->addError(
                PaySystem\Error::create(Localization\Loc::getMessage('SALE_HPS_YANDEX_CHECKOUT_ERROR_QUERY'))
            );
        }
    ...
    {
        PaySystem\Logger::addDebugInfo(
            __CLASS__ . ': yandexSum=' . PriceMaths::roundPrecision(
                $paymentData['amount']['value']
            ) . "; paymentSum=" . PriceMaths::roundPrecision($payment->getSum())
        );

        return PriceMaths::roundPrecision($paymentData['amount']['value']) === PriceMaths::roundPrecision(
                $payment->getSum()
            );
    }
