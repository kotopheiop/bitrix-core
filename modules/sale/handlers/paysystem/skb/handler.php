    ...
    implements PaySystem\IRefund
{
    ...
    private function updatePassword(Payment $payment, string $password): PaySystem\ServiceResult
    {
        ...
        $oldMapping = BusinessValue::getMapping(
            'SKB_PASSWORD',
    ...
    private function processSkbPaymentStatus(
        Payment $payment,
        PaySystem\ServiceResult $paymentStatusResult
        ...
        if (!empty($skbPayment['status'])) {
            ...
            if (!empty($skbPayment['trxId'])) {
                $additionalPsStatusDescription = Loc::getMessage(
    ...
    private function makeQuery($url, array $params = [], array $headers = []): PaySystem\ServiceResult
    {
        ...
        PaySystem\Logger::addDebugInfo(
            __CLASS__ . ': response data: ' . Main\Text\Encoding::convertEncoding($response, "UTF-8", LANG_CHARSET)
        );
    ...
    private function verifyResponse($response): PaySystem\ServiceResult
    {
        ...
        if (isset($responseData['errCode']) && !\in_array(
