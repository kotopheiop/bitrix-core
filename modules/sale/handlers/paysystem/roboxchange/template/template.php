...
<form action="<?= $params['URL'] ?>" method="post" class="mb-4">
    ...
    <p><?= Loc::getMessage("SALE_HPS_ROBOXCHANGE_TEMPL_TO_PAY"); ?>
        <strong><?= SaleFormatCurrency($params['PAYMENT_SHOULD_PAY'], $params["PAYMENT_CURRENCY"]) ?></strong></p>
    ...
    <input type="submit" name="submit" class="btn btn-primary pl-4 pr-4"
           value="<?= Loc::getMessage("SALE_HPS_ROBOXCHANGE_TEMPL_BUTTON") ?>">
</form>