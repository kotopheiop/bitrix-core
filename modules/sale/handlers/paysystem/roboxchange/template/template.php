...
<div class="mb-4">
    ...
    <p><?= Loc::getMessage(
            'SALE_HANDLERS_PAY_SYSTEM_TEMPLATE_ROBOXCHANGE_CHECKOUT_SUM',
            [
    ...
    <div class="d-flex align-items-center mb-3">
        <div class="col-auto pl-0">
            <form action="<?= $params['URL'] ?>" method="post" class="mb-4" style="display: inline-block">
                <input type="hidden" name="MerchantLogin"
                       value="<?= htmlspecialcharsbx($params['ROBOXCHANGE_SHOPLOGIN']); ?>">
                <input type="hidden" name="OutSum" value="<?= htmlspecialcharsbx($params['SUM']); ?>">
                <?php if (!empty($params['OUT_SUM_CURRENCY'])): ?>
                    <input type="hidden" name="OutSumCurrency"
                           value="<?= htmlspecialcharsbx($params['OUT_SUM_CURRENCY']); ?>">
                <?php endif; ?>
                <input type="hidden" name="InvId" value="<?= htmlspecialcharsbx($params['PAYMENT_ID']); ?>">
                <input type="hidden" name="Description"
                       value="<?= htmlspecialcharsbx($params['ROBOXCHANGE_ORDERDESCR']); ?>">
                ...
                <input type="submit" name="submit" class="btn btn-lg btn-success pl-4 pr-4" style="border-radius: 32px;"
                       value="<?= Loc::getMessage(
                           "SALE_HANDLERS_PAY_SYSTEM_TEMPLATE_ROBOXCHANGE_CHECKOUT_BUTTON_PAID"
                       ) ?>">
            </form>
        </div>
    </div>
