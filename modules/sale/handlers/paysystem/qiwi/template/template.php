...
<div class="mb-4">
    <? if (!preg_match('/^\+7\d{10}$/', $params['BUYER_PERSON_PHONE'])): ?>
        ...
        <form action="<?= POST_FORM_ACTION_URI ?>" method="post" class="form-inline">
            ...
            <input type="submit" class="btn btn-primary pl-4 pr-4" name="SET_NEW_PHONE"
                   value="<?= Loc::getMessage("SALE_HPS_QIWI_SEND_PHONE") ?>"/>
        </form>
    <? else: ?>
        <form action="<?= $params['URL'] ?>" method="post">
            ...
            <input type="hidden" name="comm"
                   value="<?= htmlspecialcharsbx(Loc::getMessage("SALE_HPS_QIWI_COMMENT", array("#ID#" => $params['PAYMENT_ID']))) ?>"/>
