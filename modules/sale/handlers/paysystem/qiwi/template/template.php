    ...
    <form action="<?= $params['URL'] ?>" method="post">
        <p>
            ...
            <?php if (Loader::includeModule("currency")): ?>
                <strong><?= CCurrencyLang::CurrencyFormat(
                        $params['PAYMENT_SHOULD_PAY'],
                        $params['PAYMENT_CURRENCY'],
                        true
                    ); ?></strong>
            <?php else: ?>
                <strong><?= htmlspecialcharsbx($params['SHOULD_PAY']); ?> <?= htmlspecialcharsbx(
                        $params['CURRENCY']
                    ) ?></strong>
            <?php endif; ?>
        </p>
        ...
        <input type="hidden" name="comm" value="<?= htmlspecialcharsbx(
            Loc::getMessage("SALE_HPS_QIWI_COMMENT", array("#ID#" => $params['PAYMENT_ID']))
        ) ?>"/>
        ...
        <input type="submit" class="btn btn-lg btn-success pl-4 pr-4" style="border-radius: 32px;"
               value="<?= Loc::getMessage("SALE_HPS_QIWI_DO_BILL"); ?>"/>
    </form>
</div>
