...
$sUrl = "/";
$sVars = "check_bill:issuer_email=" . $issuer_email . "&invoice_number=&issuer_id=" . $ORDER_ID . "&hash=" . md5(
    ...
    if ($aResult[0] == "OK") {
        ...
        if (intval($aFields["issuer_id"]) == $ORDER_ID) {
            ...
            foreach ($aFields as $key => $val) {
                $str_PS_RESPONSE_FORMATTED .= '<tr valign="top" class="tablebody"><td><font class="tablebodytext">' . htmlspecialcharsbx(
                        $key
                    ) . '</font></td><td><font class="tablebodytext">' . htmlspecialcharsbx(
                        $val
                    ) . '</font></td></tr>' . "\n";
            }
