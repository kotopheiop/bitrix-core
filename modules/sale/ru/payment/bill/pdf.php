...
if (CSalePaySystemAction::GetParamValue('PATH_TO_LOGO', false)) {
    ...
    $pdf->Image(
        CSalePaySystemAction::GetParamValue('PATH_TO_LOGO', false),
        $pdf->GetX(),
        $pdf->GetY(),
        -$imgDpi,
        -$imgDpi
...
if (CSalePaySystemAction::GetParamValue("SELLER_ADDRESS", false)) {
    ...
    $pdf->MultiCell(
        0,
        15,
        CSalePdf::prepareToPdf(CSalePaySystemAction::GetParamValue("SELLER_ADDRESS", false)),
        0,
        'L'
    );
}
...
if (CSalePaySystemAction::GetParamValue("SELLER_PHONE", false)) {
    ...
    $pdf->Write(
        15,
        CSalePdf::prepareToPdf(sprintf("���.: %s", CSalePaySystemAction::GetParamValue("SELLER_PHONE", false)))
    );
    $pdf->Ln();
}
...
$pdf->Cell(
    ...
    (CSalePaySystemAction::GetParamValue("SELLER_INN", false))
        ? CSalePdf::prepareToPdf(sprintf("��� %s", CSalePaySystemAction::GetParamValue("SELLER_INN", false)))
        : ''
);
$x1 = $pdf->GetX();
$pdf->Cell(
    ...
    (CSalePaySystemAction::GetParamValue("SELLER_KPP", false))
        ? CSalePdf::prepareToPdf(sprintf("��� %s", CSalePaySystemAction::GetParamValue("SELLER_KPP", false)))
        : ''
);
$x2 = $pdf->GetX();
$pdf->Cell(50, 18);
...
$pdf->Cell(50, 18, CSalePdf::prepareToPdf('��. �'));
...
if (CSalePaySystemAction::GetParamValue("BUYER_NAME", false)) {
    ...
    if (CSalePaySystemAction::GetParamValue("BUYER_INN", false)) {
        $pdf->Write(
            15,
            CSalePdf::prepareToPdf(sprintf(" ��� %s", CSalePaySystemAction::GetParamValue("BUYER_INN", false)))
        );
    }
    if (CSalePaySystemAction::GetParamValue("BUYER_ADDRESS", false)) {
        $pdf->Write(
            15,
            CSalePdf::prepareToPdf(sprintf(", %s", CSalePaySystemAction::GetParamValue("BUYER_ADDRESS", false)))
        );
    }
    if (CSalePaySystemAction::GetParamValue("BUYER_PHONE", false)) {
        $pdf->Write(
            15,
            CSalePdf::prepareToPdf(sprintf(", %s", CSalePaySystemAction::GetParamValue("BUYER_PHONE", false)))
        );
    }
    if (CSalePaySystemAction::GetParamValue("BUYER_FAX", false)) {
        $pdf->Write(
            15,
            CSalePdf::prepareToPdf(sprintf(", %s", CSalePaySystemAction::GetParamValue("BUYER_FAX", false)))
        );
    }
    if (CSalePaySystemAction::GetParamValue("BUYER_PAYER_NAME", false)) {
        $pdf->Write(
            15,
            CSalePdf::prepareToPdf(
                sprintf(", %s", CSalePaySystemAction::GetParamValue("BUYER_PAYER_NAME", false))
            )
        );
    }
    $pdf->Ln();
...
if ($arBasket = $dbBasket->Fetch()) {
    ...
    $arColsCaption = array(
        ...
        CSalePdf::prepareToPdf('���-��'),
        CSalePdf::prepareToPdf('��.'),
        CSalePdf::prepareToPdf('����, ' . $currency),
        CSalePdf::prepareToPdf('������ ���'),
        CSalePdf::prepareToPdf('�����, ' . $currency)
    );
    $arCells = array();
    ...
    do {
        ...
        $arCells[++$n] = array(
            ...
            CSalePdf::prepareToPdf($arBasket["MEASURE_NAME"] ? $arBasket["MEASURE_NAME"] : '��.'),
...
if (CSalePaySystemAction::GetParamValue("SELLER_DIR_POS", false)) {
    ...
    if (!$blank && CSalePaySystemAction::GetParamValue('SELLER_DIR_SIGN', false)) {
        list($signHeight, $signWidth) = $pdf->GetImageSize(
