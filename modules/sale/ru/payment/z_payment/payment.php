...
<form id="pay" name="pay" method="POST" action="https://z-payment.ru/merchant.php">
    ...
    <input type="hidden" name="LMI_PAYMENT_AMOUNT" value="<?= htmlspecialcharsbx(
        CCurrencyRates::ConvertCurrency(
            $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["SHOULD_PAY"],
            $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["CURRENCY"],
            CSalePaySystemAction::GetParamValue("ZP_CODE_RUR")
        )
    ) ?>">
    <input type="hidden" name="LMI_PAYMENT_DESC"
           value="����� <?= intval($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["ID"]) ?> �� <?= htmlspecialcharsbx(
