                    ...
                    <td>
                        ...
                        switch ($arShownFieldsParams[$i]["KEY"]) {
                            ...
                            case "PERSON_TYPE":
                                ...
                                break;
                            case "PAYED":
                                echo (($arOrder["PAYED"] == "Y") ? GetMessage("SEXE_YES") : GetMessage(
                                        "SEXE_NO"
                                    )) . "</td><td>";
                                echo $arOrder["DATE_PAYED"];
                                break;
                            case "CANCELED":
                                echo (($arOrder["CANCELED"] == "Y") ? GetMessage("SEXE_YES") : GetMessage(
                                        "SEXE_NO"
                                    )) . " ";
                            ...
                            case "STATUS":
                                if (!isset($LOCAL_STATUS_CACHE[$arOrder["STATUS_ID"]])
                                    ...
                                    if ($arStatus = CSaleStatus::GetByID($arOrder["STATUS_ID"])) {
                                        $LOCAL_STATUS_CACHE[$arOrder["STATUS_ID"]] = htmlspecialcharsEx(
                                            $arStatus["NAME"]
                                        );
                                    }
                                }
                            ...
                            case "PRICE_DELIVERY":
                                ...
                                break;
                            case "ALLOW_DELIVERY":
                                echo (($arOrder["ALLOW_DELIVERY"] == "Y") ? GetMessage("SEXE_YES") : GetMessage(
                                        "SEXE_NO"
                                    )) . " ";
                                ...
                                if (intval($arOrder["PAY_SYSTEM_ID"]) > 0) {
                                    if (!isset($LOCAL_PAY_SYSTEM_CACHE[$arOrder["PAY_SYSTEM_ID"]])
                                        ...
                                        if ($arPaySys = CSalePaySystem::GetByID($arOrder["PAY_SYSTEM_ID"])) {
                                            $LOCAL_PAY_SYSTEM_CACHE[$arOrder["PAY_SYSTEM_ID"]] = htmlspecialcharsEx(
                                                $arPaySys["NAME"]
                                            );
                                        }
                                    }
                                ...
                                if (intval($arOrder["DELIVERY_ID"]) > 0) {
                                    if (!isset($LOCAL_DELIVERY_CACHE[$arOrder["DELIVERY_ID"]])
                                        ...
                                        if ($arDelivery = CSaleDelivery::GetByID($arOrder["DELIVERY_ID"])) {
                                            $LOCAL_DELIVERY_CACHE[$arOrder["DELIVERY_ID"]] = htmlspecialcharsEx(
                                                $arDelivery["NAME"]
                                            );
                                        }
                                    }
