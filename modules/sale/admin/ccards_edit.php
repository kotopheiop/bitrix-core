...
<? if (strlen($errorMessage) > 0)
    ...
    <form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?" name="fccards_edit">
            ...
            <td>
                <select name="PAY_SYSTEM_ACTION_ID">
                    ...
                    while ($arPaySysActions = $dbPaySysActions->Fetch()) {
                        ?>
                        <option value="<?= $arPaySysActions["ID"] ?>"<?
                        if (IntVal($str_PAY_SYSTEM_ACTION_ID) == IntVal($arPaySysActions["ID"])) echo " selected"; ?>><?= htmlspecialcharsEx($arPaySysActions["NAME"] . " [" . $arPaySysActions["PS_NAME"] . " / " . $arPaySysActions["PT_NAME"] . "]") ?></option><?
                    }
                    ?>
                </select>
            </td>
            ...
            <td>
                <select name="CARD_EXP_MONTH">
                    ...
                    for ($i = 1; $i <= 12; $i++) {
                        ?>
                        <option value="<?= $i ?>"<?
                        if (IntVal($str_CARD_EXP_MONTH) == $i) echo " selected"; ?>><?= ((strlen($i) < 2) ? "0" . $i : $i) ?></option><?
                    }
                    ?>
                </select>
                <select name="CARD_EXP_YEAR">
                    ...
                    for ($i = 2005; $i <= 2100; $i++) {
                        ?>
                        <option value="<?= $i ?>"<?
                        if (IntVal($str_CARD_EXP_YEAR) == $i) echo " selected"; ?>><?= $i ?></option><?
                    }
                    ?>
                </select>
            </td>
        ...
        if ($str_LAST_STATUS == "Y" || $str_LAST_STATUS == "N") {
            ...
            <tr>
                <td><?
                    echo GetMessage("SCE_STATUS_CODE") ?></td>
                <td>
            ...
            <tr>
                <td><?
                    echo GetMessage("SCE_DESCRIPTION") ?></td>
                <td>
            ...
            <tr>
                <td><?
                    echo GetMessage("SCE_MESSAGE") ?></td>
                <td>
