...
if (isConnected() === false) {
    ...
    <div class="crm-admin-wrap">
        ...
        <div class="crm-admin-set" id="id_new_crm_reg_form">
            ...
            echo $APPLICATION->GetCurPage() ?>?" name="form1_do_create_link">
                ...
                <table class="crm-admin-set-content-table" cellspacing="0">
                    <tr>
                        <td class="crm-admin-set-left"><span
                                    class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_SETTINGS_ADDRES') ?>
                        </td>
                        <td class="crm-admin-set-right">
                            <select class="crm-admin-set-select" name="CONFIG_EXTERNAL_SERVICE_SCHEME"
                                    title="<?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_SETTINGS_ADDRES') ?>">
                                <option value="https"<?= (($instance->getSchemeServiceUrl() == "https") ? " selected" : "") ?>>
                                    https
                                </option>
                                <option value="http"<?= (($instance->getSchemeServiceUrl() == "http") ? " selected" : "") ?>>
                                    http
                                </option>
                            </select><span class="crm-admin-set-text">&nbsp;://&nbsp;</span><input type="text"
                                                                                                   class="crm-admin-set-input"
                                                                                                   name="CONFIG_EXTERNAL_SERVICE_URL"
                                                                                                   value="<?= htmlspecialcharsbx($instance->getServiceUrl()) ?>"/>
                        </td>
                    </tr>
                    <tr>
                        <td class="crm-admin-set-left">
                            <nobr>
                                <span class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_SETTINGS_ACCESS_TOKEN') ?>
                            </nobr>
                        </td>
                        <td class="crm-admin-set-right"><input class="crm-admin-set-input" type="text"
                                                               name="CONFIG_REFRESH_TOKEN"
                                                               value="<?= htmlspecialcharsbx($instance->getRefreshToken()) ?>"/>
                        </td>
                    </tr>
                </table>
                <div class="crm-admin-set-button">
                    <a class="adm-btn adm-btn-green"
                       href='javascript:document.forms["form1_do_create_link"].submit();'><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_APPLY') ?></a>&nbsp;&nbsp;
                    <a class="adm-btn"
                       href="javascript:location.href='<?= $APPLICATION->GetCurPage() ?>'"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_CANCEL') ?></a>
                </div>
                <?= bitrix_sessid_post(); ?>
    ...
    ?>
    <form method="POST" action="<?
    echo $APPLICATION->GetCurPage() ?>?" name="sale_synchronizer_settings">
        <?
        ...
        <tr class="heading">
            <td colspan="2"
                align="center"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_CONNECTED_SETTINGS') ?></td>
        </tr>
        <tr>
            <td align="right" width="40%"><label
                        for="use-"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_CONNECTED_ACTIVE') ?>
            </td>
            <td align="left"><input type="checkbox" <?= $instance->isActive() ? 'checked' : '' ?> name="IS_ACTIVE"
                                    value="Y"></td>
        </tr>
        <tr>
            <td align="right" width="40%"><label for="use-"><span
                            class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_CONNECTED_ADDRES') ?>
            </td>
            <td align="left">
                <select name="CONFIG_EXTERNAL_SERVICE_SCHEME">
                    ...
                    <option value="http"<?= (($instance->getSchemeServiceUrl() == "http") ? " selected" : "") ?>>http
                    </option>
                </select><span>&nbsp;://&nbsp;</span><input type="text" name="CONFIG_EXTERNAL_SERVICE_URL"
                                                            value="<?= $instance->getServiceUrl() ?>"/>
            </td>
        </tr>
        <tr>
            <td align="right" width="40%"><label for="use-"><span
                            class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_ACCESS_TOKEN') ?>
            </td>
            <td align="left"><input name="CONFIG_REFRESH_TOKEN" type='text' size=80
                                    value='<?= $instance->getRefreshToken() ?>'/></td>
        </tr>

        <tr class="heading">
            <td colspan="2"
                align="center"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_GENERAL_SETTINGS') ?></td>
        </tr>
        <tr>
            <td align="right" width="40%"><label for="use-"><span
                            class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_SITE_DEFAULT') ?>
            </td>
            <td align="left"><select name="DEFAULT_SITE_ID"><?
                    foreach ($arSites

                    as $id => $name): ?>
                    <option <?= $instance->getDefaultSiteId() == $id ? 'selected' : '' ?>
                            value="<?= htmlspecialcharsbx($id) ?>"><?= htmlspecialcharsbx($name) ?></option><?
                endforeach;
                ?></td>
        </tr>
        <tr>
            <td align="right" width="40%"><label for="use-"><span
                            class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_PS_DEFAULT') ?>
            </td>
            <td align="left"><select name="DEFAULT_PAY_SYSTEM_ID"><?
                    foreach ($arPaySystemsWithoutInner

                    as $id => $name): ?>
                    <option <?= $instance->getDefaultPaySystemId() == $id ? 'selected' : '' ?>
                            value="<?= htmlspecialcharsbx($id) ?>"><?= htmlspecialcharsbx($name) ?></option><?
                endforeach;
                ?></td>
        </tr>
        <tr>
            <td align="right" width="40%"><label for="use-"><span
                            class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_DS_DEFAULT') ?>
            </td>
            <td align="left"><select name="DEFAULT_DELIVERY_SYSTEM_ID"><?
                    foreach ($shipmentServices

                    as $id => $name): ?>
                    <option <?= $instance->getDefaultDeliverySystemId() == $id ? 'selected' : '' ?>
                            value="<?= htmlspecialcharsbx($id) ?>"><?= htmlspecialcharsbx($name) ?></option><?
                endforeach;
                ?></td>
        </tr>
        <tr>
            <td align="right" width="40%"><label for="use-"><span
                            class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_PT_DEFAULT') ?>
            </td>
            <td align="left"><select name="DEFAULT_PERSON_TYPE">
                    <option></option><?
                    foreach ($internal['PERSON_TYPE'] as $row): ?>
                    <option <?= $instance->getDefaultPersonTypeId() == $row['ID'] ? 'selected' : '' ?>
                            value="<?= $row['ID'] ?>"><?= '[' . $row['ID'] . '] ' . htmlspecialcharsbx($row['NAME']) ?></option><?
                endforeach;
                ?></td>
        </tr>
        <tr>
            <td align="right" width="40%"><label for="use-"><span
                            class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_ORDER_STATUS_DEFAULT') ?>
            </td>
            <td align="left"><select name="DEFAULT_ORDER_STATUS">
                    <option></option><?
                    foreach ($internal['ORDER_STATUSES'] as $row): ?>
                    <option <?= $instance->getDefaultOrderStatusId() == $row['ID'] ? 'selected' : '' ?>
                            value="<?= $row['ID'] ?>"><?= '[' . $row['ID'] . '] ' . htmlspecialcharsbx($row['NAME']) ?></option><?
                endforeach;
                ?></td>
        </tr>
        <tr>
            <td align="right" width="40%"><label for="use-"><span
                            class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_DELIVERY_STATUS_DEFAULT') ?>
            </td>
            <td align="left"><select name="DEFAULT_DELIVERY_STATUS">
                    <option></option><?
                    foreach ($internal['DELIVERY_STATUSES'] as $row): ?>
                    <option <?= $instance->getDefaultDeliveryStatusId() == $row['ID'] ? 'selected' : '' ?>
                            value="<?= $row['ID'] ?>"><?= '[' . $row['ID'] . '] ' . htmlspecialcharsbx($row['NAME']) ?></option><?
                endforeach;
                ?></td>
        </tr>
        <tr>
            <td align="right" width="40%"><label for="use-"><span
                            class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_CATALOGS') ?>
            </td>
            <td align="left"><select name="" multiple size="5" disabled><?
                    foreach ($catalogList as $catalog) {
                        ?>
                        <option selected><?= '[' . $catalog['IBLOCK_ID'] . '] ' . htmlspecialcharsbx($catalog['CATALOG_CATALOG_IBLOCK_IBLOCK_NAME']) ?></option><?
                    } ?></select></td>
        </tr>

        <tr>
            <td align="right" width="40%"><label
                        for="use-"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_MARKED') ?></td>
            <td align="left"><input type="checkbox" <?= $instance->isMarked() ? 'checked' : '' ?> name="IS_MARKED"
                                    value="Y"></td>
        </tr>
        ...
        if (empty($errorOAuth) && empty($errorConnection)):?>
            ...
            <tr class="heading">
                <td colspan="2"
                    align="center"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_TITLE') ?></td>
            </tr>
            <tr>
                <td align="right" width="40%"><label for="use-"><span
                                class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_ACTIVE'); ?>
                    </label></td>
                <td align="left"><?= $external['SYNCHRONIZER']['IS_ACTIVE'] == 'Y' ? '<font color="green">' . \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_ACTIVE_Y') . '</font>' : '<font color="red">' . \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_ACTIVE_N') . '</font>' ?></td>
            </tr>
            <tr>
                <td align="right" width="40%"><label for="use-"><span
                                class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_SITE_DEFAULT'); ?>
                    </label></td>
                <td align="left"><?= '[' . $external['SYNCHRONIZER']['SITE']['ID'] . '] ' . htmlspecialcharsbx($external['SYNCHRONIZER']['SITE']['NAME']); ?></td>
            </tr>
            <tr>
                <td align="right" width="40%"><label for="use-"><span
                                class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_PS_DEFAULT'); ?>
                    </label></td>
                <td align="left"><?= '[' . $external['SYNCHRONIZER']['PAY_SYSTEM']['ID'] . '] ' . htmlspecialcharsbx($external['SYNCHRONIZER']['PAY_SYSTEM']['NAME']); ?></td>
            </tr>
            <tr>
                <td align="right" width="40%"><label for="use-"><span
                                class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_DS_DEFAULT'); ?>
                    </label></td>
                <td align="left"><?= '[' . $external['SYNCHRONIZER']['DELIVERY_SYSTEM']['ID'] . '] ' . htmlspecialcharsbx($external['SYNCHRONIZER']['DELIVERY_SYSTEM']['NAME']); ?></td>
            </tr>
            <tr>
                <td align="right" width="40%"><label for="use-"><span
                                class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_PT_DEFAULT'); ?>
                    </label></td>
                <td align="left"><?= '[' . $external['SYNCHRONIZER']['PERSON_TYPE']['ID'] . '] ' . htmlspecialcharsbx($external['SYNCHRONIZER']['PERSON_TYPE']['NAME']); ?></td>
            </tr>
            <tr>
                <td align="right" width="40%"><label for="use-"><span
                                class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_ORDER_STATUS_DEFAULT'); ?>
                    </label></td>
                <td align="left"><?= '[' . $external['SYNCHRONIZER']['ORDER_STATUS']['ID'] . '] ' . htmlspecialcharsbx($external['SYNCHRONIZER']['ORDER_STATUS']['NAME']); ?></td>
            </tr>
            <tr>
                <td align="right" width="40%"><label for="use-"><span
                                class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_DELIVERY_STATUS_DEFAULT'); ?>
                    </label></td>
                <td align="left"><?= '[' . $external['SYNCHRONIZER']['DELIVERY_STATUS']['ID'] . '] ' . htmlspecialcharsbx($external['SYNCHRONIZER']['DELIVERY_STATUS']['NAME']); ?></td>
            </tr>
            <tr>
                <td align="right" width="40%"><label for="use-"><span
                                class="required">*</span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_EXTERNAL_SETTINGS_CATALOGS'); ?>
                    </label></td>
                <td align="left"><select name="" multiple size="5" disabled><?
                        foreach ($external['SYNCHRONIZER']['CATALOGS'] as $tradeCatalog):?>
                            <option selected><?= '[' . $tradeCatalog['ID'] . '] ' . htmlspecialcharsbx($tradeCatalog['NAME']) ?></option><?endforeach; ?>
                    </select></td>
            </tr>

            <tr>
                <td colspan="2" align="center"><?
                    echo BeginNote('align="center"'); ?><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_NOTE') ?><?
                    echo EndNote(); ?></td>
            </tr>

            <tr class="heading">
                <td colspan="2"
                    align="center"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_MATCHING_PT') ?></td>
            </tr>
            <tr><?
                showSelect($internal['PERSON_TYPE'], $external['PERSON_TYPE'], 'PERSON_TYPE') ?></tr>
            <tr class="heading">
                <td colspan="2"
                    align="center"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_MATCHING_PS') ?></td>
            </tr>
            <tr><?
                showSelect($internal['PAY_SYSTEMS'], $external['PAY_SYSTEMS'], 'PAY_SYSTEMS') ?></tr>
            <tr class="heading">
                <td colspan="2"
                    align="center"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_MATCHING_SD') ?></td>
            </tr>
            <tr><?
                showSelect($internal['DELIVERY_SYSTEMS'], $external['DELIVERY_SYSTEMS'], 'DELIVERY_SYSTEMS') ?></tr>
            <tr class="heading">
                <td colspan="2"
                    align="center"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_MATCHING_P') ?></td>
            </tr>
            <tr>
                <td colspan="2" align="center"><?
                    echo BeginNote('align="center"'); ?><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_NOTE1') ?><?
                    echo EndNote(); ?></td>
            </tr>
            ...
            <tr class="heading">
                <td colspan="2"
                    align="center"><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_MATCHING1') ?></td>
            </tr>
            <tr>
                <td colspan="2">
                    <?
            ...
            <tr class="heading">
                <td colspan="2" align="center"><span
                            class="required"></span><?= \Bitrix\Main\Localization\Loc::getMessage('SALE_SYNCHRONIZER_MATCHING2') ?>
                </td>
            </tr>
            <tr><?
                showSelect($internal['SITES'], $external['TRADE_PLATFORMS'], 'SITES') ?></tr>
        <?endif;
...
function showSelect($internal, $external, $name)
{
    foreach ($internal as $internalFields):?>
        <tr>
            <td align="right" width="40%"><label for="use-<?= $internalFields["ID"] ?>">[<?= $internalFields['ID'] ?>
                    ] <?= htmlspecialcharsbx($internalFields['NAME']) ?>:</label></td>
            <!--<td align="left">[<?= $internalFields['ID'] ?>] <?= htmlspecialcharsbx($internalFields['NAME']) ?></td>-->
            <td align="left"><select name="<?= htmlspecialcharsbx($name . '_' . $internalFields['ID']) ?>">
                    <option></option><?
                    foreach ($external as $externalFields) {
                        ?>
                        <option <?= $externalFields['XML_ID'] <> '' ? 'value="' . htmlspecialcharsbx($externalFields['XML_ID']) . '"' : '' ?> <?= $externalFields['XML_ID'] <> '' && $externalFields['XML_ID'] == $internalFields['XML_ID'] ? 'selected' : '' ?> >
                        [<?= htmlspecialcharsbx($externalFields['ID']) ?>
                        ] <?= htmlspecialcharsbx($externalFields['NAME']) ?></option><?
                    }
                    ?></select></td>
        </tr>
    <?endforeach;
