...
namespace Bitrix\Sale\PaySystem\AdminPage\PaySystemCashbox {
    ...
    if (
        ...
        if ($request->get('CAN_PRINT_CHECK_SELF') === 'Y') {
            ...
            $onDisabledFiscalizationResult = Sale\PaySystem\Cashbox\EventHandler::onDisabledFiscalization(
                $service,
                $kkmId
            );
                ...
                <td width="60%" class="adm-detail-content-cell-r">
                    <select name="CASHBOX[KKM_ID]" id="KKM_ID" onchange="BX.Sale.PaySystemCashbox.reloadSettings()">
                        ...
                        foreach ($supportedKkmModels as $supportedKkm) {
                            ...
                            echo '<option value="' . $supportedKkm . '" ' . $selected . '>' . htmlspecialcharsbx(
                                    $supportedKkm
                                ) . '</option>';
                        }
                        ?>
        ...
        <tr>
            <td width="40%" class="adm-detail-content-cell-l">
                <label for="CAN_PRINT_CHECK_SELF"><?= Loc::getMessage(
                        'SALE_PAY_SYSTEM_CASHBOX_FISCALIZATION'
                    ) ?></label>
            </td>
            <td width="60%" class="adm-detail-content-cell-r">
                <input class="adm-designed-checkbox" type="checkbox" name="CAN_PRINT_CHECK_SELF"
                       id="CAN_PRINT_CHECK_SELF" value="Y"
                       onchange="BX.Sale.PaySystemCashbox.toggleCashboxSetting(this)" <?= ($fiscalizationEnable ? ' checked' : '') ?>>
        ...
        <tr>
            ...
            <td width="60%" class="adm-detail-content-cell-r">
                ...
                <script>
                    BX.hint_replace(BX('hint_EMAIL'), '<?= \CUtil::JSEscape(
                        Loc::getMessage('SALE_PAY_SYSTEM_EMAIL_HINT')
                    ) ?>');
                </script>
            </td>
        </tr>
        ...
        ?>
        <tbody id="sale-cashbox-settings-container"
               data="pay-system-cashbox-settings-container" <?= ($fiscalizationEnable ? '' : "style='display:none;'") ?>>
        <?= $cashboxSettings ?>
        </tbody>
        <?php
    }
