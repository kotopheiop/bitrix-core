...
$arHeaders = array(
    ...
    array(
        "id" => "ACCOUNT_NUMBER",
        "content" => Loc::getMessage("SOA_ACCOUNT_NUMBER"),
        "sort" => "ACCOUNT_NUMBER",
        "default" => true
    ),
    array(
        "id" => "DATE_INSERT",
        "content" => Loc::getMessage("SI_DATE_INSERT_ORDER"),
        "sort" => "DATE_INSERT",
        "default" => true
    ),
    ...
    array(
        "id" => "DATE_ARCHIVED",
        "content" => Loc::getMessage("SI_DATE_ARCHIVED"),
        "sort" => "DATE_ARCHIVED",
        "default" => true
    ),
...
$orderIterator = \Bitrix\Sale\Internals\OrderArchiveTable::getList(
    array(
        'filter' => $arFilterTmp,
...
if (!empty($orderList) && is_array($orderList)) {
    ...
    foreach ($orderList as $id => $arOrder) {
        ...
        $row->AddField(
            "ORDER_ID",
            str_replace('##ID##', Loc::getMessage("SO_ORDER_ID_PREF") . $arOrder["ORDER_ID"], $rowTmp)
        );
        ...
        if (in_array("PERSON_TYPE", $arVisibleColumns)) {
            if (!isset($LOCAL_PERSON_TYPE_CACHE[$arOrder["PERSON_TYPE_ID"]])
                ...
                if (count($personTypeList)) {
                    $LOCAL_PERSON_TYPE_CACHE[$arOrder["PERSON_TYPE_ID"]] = htmlspecialcharsbx(
                        $personTypeList[$arOrder["PERSON_TYPE_ID"]]["NAME"]
                    );
                }
            }
            $fieldValue = "[";
        ...
        $fieldValue = "";
        if (in_array("PAYED", $arVisibleColumns)) {
            $fieldValue .= '<span id="payed_' . $arOrder['ID'] . '">' . (($arOrder["DEDUCTED"] == "Y") ? Loc::getMessage(
                    "SO_YES"
                ) : Loc::getMessage("SO_NO")) . "</span>";
        }
        ...
        $row->AddField(
            ...
            '<span style="white-space:nowrap;">' . htmlspecialcharsbx(
                SaleFormatCurrency($arOrder["PRICE"], $arOrder["CURRENCY"])
            ) . '</span>'
        );
        ...
        if ($bNeedBasket && is_array($basketItems)) {
            ...
            foreach ($basketItems as $arItem) {
                ...
                if ($arItem["PRICE"] <> '') {
                    $fieldPrice .= "<nobr>" . htmlspecialcharsbx(
                            SaleFormatCurrency($arItem["PRICE"], $arItem["CURRENCY"])
                        ) . "</nobr>";
                } else {
                    $fieldPrice .= "<br />";
                }
        ...
        $idTmp = '<table>
						<tr>
							<td valign="top"></td>
							<td><a href="/bitrix/admin/sale_order_archive_view.php?ID=##ID##"><b>' . Loc::getMessage(
                "SO_ORDER_ID_PREF"
            ) . '##ID##</b></a></td>
						</tr>
					</table>';
        ...
        if (in_array($arOrder["STATUS_ID"], $allowedStatusesUpdate)) {
            $arActions[] = array(
                "ICON" => "copy",
                "TEXT" => Loc::getMessage("SOA_RESTORE_ORDER"),
                "ACTION" => $lAdmin->ActionRedirect(
                    "sale_order_create.php?restoreID=" . $arOrder['ID'] . "&lang=" . LANGUAGE_ID . "&" . 'SITE_ID=' . $arOrder['LID'] . '&' . bitrix_sessid_get(
        ...
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect(
                "sale_order_archive_view.php?ID=" . $arOrder['ID'] . "&lang=" . LANGUAGE_ID . "&" . 'SITE_ID=' . $arOrder['LID'] . '&' . bitrix_sessid_get(
                )
            ),
            "DEFAULT" => true
        );
        if ($saleModulePermissions == "W") {
            $arActions[] = array(
                ...
                "ACTION" => $lAdmin->ActionDoGroup($arOrder['ID'], 'delete')
            );
        }
    ...
    <form name="find_form" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?">
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="filter_order_id_from" OnChange="filter_id_from_Change()"
                       value="<? echo ((int)($filter_order_id_from) > 0) ? (int)($filter_order_id_from) : "" ?>"
                       size="10">
                <? echo Loc::getMessage("SALE_F_TO"); ?>
                <input type="text" name="filter_order_id_to"
                       value="<? echo ((int)($filter_order_id_to) > 0) ? (int)($filter_order_id_to) : "" ?>" size="10">
            </td>
        </tr>
        <tr>
            <td><? echo Loc::getMessage("SALE_F_ACCOUNT_NUMBER"); ?>:</td>
            <td>
                <input type="text" name="filter_account_number"
                       value="<? echo htmlspecialcharsbx($filter_account_number) ?>" size="10">
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="filter_price_from"
                       value="<?= ((float)($filter_price_from) > 0) ? (float)($filter_price_from) : "" ?>" size="3">

                <? echo Loc::getMessage("SOA_F_PRICE_TO"); ?>
                <input type="text" name="filter_price_to"
                       value="<?= ((float)($filter_price_to) > 0) ? (float)($filter_price_to) : "" ?>" size="3">
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="filter_id_from" OnChange="filter_id_from_Change()"
                       value="<? echo ((int)($filter_id_from) > 0) ? (int)($filter_id_from) : "" ?>" size="10">
                <? echo Loc::getMessage("SALE_F_TO"); ?>
                <input type="text" name="filter_id_to"
                       value="<? echo ((int)($filter_id_to) > 0) ? (int)($filter_id_to) : "" ?>" size="10">
            </td>
        </tr>
        <tr>
