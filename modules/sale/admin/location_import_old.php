...
?>
    <tr>
        <td colspan="2"><input type="submit"
                               onclick="WizardWindow.Open('bitrix:sale.locations', '<?= bitrix_sessid() ?>');"
                               class="adm-btn-save" value="<?= GetMessage('LOCA_LOADING_WIZARD') ?>"></td>
    </tr>
<?
$tabControl->EndTab();
?>
...
?>
    <form method="POST" action="sale_location_import.php" enctype="multipart/form-data">
        ...
        <?= bitrix_sessid_post() ?>
        <tr>
            <td colspan="2"><input onclick="return confirm('<?= GetMessage('LOCATION_CLEAR_CONFIRM'); ?>');"
                                   type="submit" name="delete_all" value="<?= GetMessage('LOCATION_CLEAR_BTN') ?>"></td>
        </tr>
    </form>
<?
$tabControl->EndTab();
