...
?>
    <form method="post" action="<?= $APPLICATION->GetCurPage() ?>?lang=<?= LANGUAGE_ID ?>" name="ymform">
        ...
        ?>
            ...
            <tr>
                ...
                <td>
                    <select name="YMSETTINGS[AUTH_TYPE]">
                        <option value="HEADER"<?= isset($siteSetts["AUTH_TYPE"]) && $siteSetts["AUTH_TYPE"] == "HEADER" ? " selected" : "" ?>>
                            HEADER
                        </option>
                        <option value="URL"<?= isset($siteSetts["AUTH_TYPE"]) && $siteSetts["AUTH_TYPE"] == "URL" ? " selected" : "" ?>>
                            URL
                        </option>
                    </select>
            ...
            <tr>
                <td class="adm-detail-valign-top"><?
                    echo GetMessage("SALE_YM_OUTLETS") ?>:
                </td>
                <td id="OUTLETS_IDS_<?= htmlspecialcharsbx($SITE_ID) ?>"><?
                    if (isset($siteSetts["OUTLETS_IDS"]) && is_array($siteSetts["OUTLETS_IDS"])) {
                        foreach ($siteSetts["OUTLETS_IDS"] as $outletId) {
                            ?><input type="text" name="YMSETTINGS[OUTLETS_IDS][]" size="10"
                                     value="<?= htmlspecialcharsbx($outletId) ?>"><br><?
                        }
                    }
                    ?>
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="YMSETTINGS[PERIOD_FROM]" size="10" maxlength="10"
                           value="<?= isset($siteSetts["PERIOD_FROM"]) ? intval($siteSetts["PERIOD_FROM"]) : "7" ?>">
                    &nbsp;-&nbsp;
                    <input type="text" name="YMSETTINGS[PERIOD_TO]" size="10" maxlength="10"
                           value="<?= isset($siteSetts["PERIOD_TO"]) ? intval($siteSetts["PERIOD_TO"]) : "21" ?>">
        ...
        <?
        foreach ($arDeliveryList

        as $deliveryId => $deliveryName):
                            ...
                            <td>
                                <table style="margin-left: 40px;">
                                    ...
                                    foreach (\CSaleYMHandler::getExistPaymentMethods() as $methodIdx => $method):?>
                                        <tr>
                                            <td><?= GetMessage("SALE_YM_DLV_PS_" . $method) ?></td>
                                            <td><input type="checkbox" class="adm-sale-dlv-ps-methods"
                                                       name="YMSETTINGS[DLV_PS][<?= $deliveryId ?>][<?= $methodIdx ?>]"
                                                       value="Y"<?= $siteSetts['DLV_PS'][$deliveryId][$methodIdx] && $siteSetts['DLV_PS'][$deliveryId][$methodIdx] == 'N' ? '' : ' checked' ?>>
                                            </td>
                                        </tr>
                                    <?endforeach; ?>
                                </table>
        ...
        $tabControl->BeginNextTab(); ?>
            ...
            <tr class="heading">
                <td colspan="2"><?= GetMessage("SALE_YM_STATUS_IN") ?></td>
            </tr>
            <tr>
        ...
        ?>
            <tr class="heading">
                <td colspan="2"><?= GetMessage("SALE_YM_STATUS_OUT") ?></td>
            </tr>
        <?
        foreach ($statuses
        ...
        <?
        foreach ($requiredOrderProperties
