...
while ($paySystem = $dbRes->fetch()) {
    if (!empty($personTypeId)) {
        ...
        while ($restriction = $dbRestriction->fetch()) {
            if (!CSalePaySystemAction::checkRestriction(
                $restriction,
...
//		$editUrl = "/crm/configs/ps/edit/".$arCCard["ID"]."/";
    ...
    if ($restriction = $dbRestriction->fetch()) {
        $ptRes = \Bitrix\Sale\PersonTypeTable::getList(
            array('select' => array('NAME'), 'filter' => array('ID' => $restriction['PARAMS']['PERSON_TYPE_ID']))
        );
    ...
    if ($restriction = $dbRestriction->fetch()) {
        $siteRes = \Bitrix\Main\SiteTable::getList(
            array(
                ...
                'filter' => array('LID' => $restriction['PARAMS']['SITE_ID'], 'LANGUAGE_ID' => $context->getLanguage())
            )
        );
    ...
    if ($saleModulePermissions >= "W") {
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . CUtil::JSEscape(
                    GetMessage('SALE_CONFIRM_DEL_MESSAGE')
                ) . "')) " . $lAdmin->ActionDoGroup($arCCard["ID"], "delete"),
        );
    }
