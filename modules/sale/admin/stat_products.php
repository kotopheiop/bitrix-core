...
while ($arBasket = $dbBasket->Fetch()) {
    ...
    if (intval($arBasket["ORDER_ID"]) > 0) {
        ...
        $arResult[$key]["PRICE_PRODUCT"][$arBasket["CURRENCY"]] += roundEx(
            $arBasket["PRICE"] * $arBasket["QUANTITY"],
            SALE_VALUE_PRECISION
        );
        $arResult[$key]["PRICE_ORDER"][$arBasket["CURRENCY"]] += roundEx(
            $arBasket["PRICE"] * $arBasket["QUANTITY"],
            SALE_VALUE_PRECISION
        );
    } else {
    ...
    $arResult[$key]["PRICE"][$arBasket["CURRENCY"]] += roundEx(
        $arBasket["PRICE"] * $arBasket["QUANTITY"],
        SALE_VALUE_PRECISION
    );
    ...
    if ($arBasket["ORDER_PAYED"] == "Y") {
        $arResult[$key]["PAYED"]++;
        $arResult[$key]["PRICE_PRODUCT_PAYED"][$arBasket["CURRENCY"]] += roundEx(
            $arBasket["PRICE"] * $arBasket["QUANTITY"],
            SALE_VALUE_PRECISION
        );
        $arResult[$key]["PRICE_ORDER_PAYED"][$arBasket["CURRENCY"]] += roundEx(
            $arBasket["PRICE"] * $arBasket["QUANTITY"],
            SALE_VALUE_PRECISION
        );
    }
    ...
    if ($arBasket["ORDER_ALLOW_DELIVERY"] == "Y") {
        ...
        $arResult[$key]["PRICE_PRODUCT_ALLOW_DELIVERY"][$arBasket["CURRENCY"]] += roundEx(
            $arBasket["PRICE"] * $arBasket["QUANTITY"],
            SALE_VALUE_PRECISION
        );
        $arResult[$key]["PRICE_ORDER_ALLOW_DELIVERY"][$arBasket["CURRENCY"]] += roundEx(
            $arBasket["PRICE"] * $arBasket["QUANTITY"],
            SALE_VALUE_PRECISION
        );
    }
...
$arPrices = Array(
    ...
    "PRICE_ORDER",
    "PRICE_ORDER_PAYED",
    "PRICE_ORDER_ALLOW_DELIVERY",
    "PRICE_PRODUCT",
    "PRICE_PRODUCT_PAYED",
    "PRICE_PRODUCT_ALLOW_DELIVERY",
    "PRICE_MIN",
    "PRICE_MAX"
);
...
$arHeaders = array(
    ...
    array(
        "id" => "QUANTITY",
        "content" => GetMessage("SALE_QUANTITY"),
        "sort" => "QUANTITY",
        "default" => true,
        "align" => "right"
    ),
    ...
    array(
        "id" => "ORDER_QUANTITY",
        "content" => GetMessage("SALE_ORDER_QUANTITY"),
        "sort" => "ORDER_QUANTITY",
        "default" => true,
        "align" => "right"
    ),
    array(
        "id" => "ORDER_COUNT",
        "content" => GetMessage("SALE_ORDER_COUNT"),
        "sort" => "ORDER_COUNT",
        "default" => true,
        "align" => "right"
    ),
    ...
    array(
        "id" => "BASKET_QUANTITY",
        "content" => GetMessage("SALE_BASKET_QUANTITY"),
        "sort" => "BASKET_QUANTITY",
        "default" => true,
        "align" => "right"
    ),
);
...
while ($arCur = $dbCur->Fetch()) {
    ...
    if (in_array($arCur["CURRENCY"], $arCurUsed)) {
        foreach ($arPrices as $v) {
            $arHeaders[] = array(
                "id" => $v . "_" . $arCur["CURRENCY"],
                "content" => GetMessage("SALE_" . $v, Array("#CURRENCY#" => htmlspecialcharsEx($arCur["FULL_NAME"]))),
                "sort" => $v . "_" . $arCur["CURRENCY"],
                "default" => true,
                "align" => "right"
            );
        }
...
while ($arResult = $dbResult->GetNext()) {
    ...
    if (intval($arResult["ORDER_COUNT"]) > 0) {
        ...
        $row->AddViewField(
            "ALLOW_DELIVERY_PROC",
            roundEx($arResult["ALLOW_DELIVERY"] * 100 / $arResult["ORDER_COUNT"], 0)
        );
    } else {
    ...
    foreach ($arCurUsed as $currency) {
        foreach ($arPrices as $price) {
            $row->AddViewField(
                $price . "_" . $currency,
                \Bitrix\Sale\PriceMaths::roundByFormatCurrency($arResult[$price][$currency], $currency)
            );
        }
    }
}
