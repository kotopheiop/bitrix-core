...
?>
    <form method="POST"
          action="<? echo $APPLICATION->GetCurPage() ?>?ID=<?= htmlspecialcharsbx($statusId) ?>&lang=<?= LANGUAGE_ID ?>"
          name="fform">
        ...
        <tr class="adm-detail-required-field">
            ...
            <td width="60%">
                <? if ($statusId): ?>
                    <b><?= $statusId ?></b>
                <? else: ?>
                    <input type="text" name="NEW_ID" value="<?= htmlspecialcharsbx($status['ID']) ?>" size="4"
                           maxlength="2">
                <? endif ?>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <input type="checkbox"
                       name="NOTIFY"<? if (!($statusId && $status['NOTIFY'] == 'N')): ?> checked="checked"<? endif ?>
                       onclick="(
				function (box)
				{
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="NEW_COLOR" id="new_color" value="<?= htmlspecialcharsbx($status['COLOR']) ?>"
                       size="4" maxlength="7" style="float:left; margin-right: 5px">
                <script>
        ...
        <? foreach ($languages as $languageId => $languageName): ?>
            ...
            <tr class="adm-detail-required-field">
                ...
                <td><input type="text" name="NAME_<?= htmlspecialcharsbx($languageId) ?>"
                           value="<?= htmlspecialcharsbx($translations[$languageId]['NAME']) ?>" size="30"></td>
            </tr>
            <tr>
                <td valign="top"><?= $statusLangFields['DESCRIPTION']->getTitle() ?>:</td>
                <td>
                    <textarea name="DESCRIPTION_<?= htmlspecialcharsbx($languageId); ?>" cols="35"
                              rows="3"><?= htmlspecialcharsbx($translations[$languageId]['DESCRIPTION']) ?></textarea>
                </td>
            </tr>
        <? endforeach ?>
        <tr class="heading">
        ...
        <tr>
            ...
            <td>
                <a href="/bitrix/admin/task_admin.php?lang=<?= LANGUAGE_ID ?>&set_filter=Y&find_module_id=sale&find_binding=status"
                   target="_blank">
