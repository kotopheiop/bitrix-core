...
<? CAdminMessage::ShowMessage($errorsList); ?>
    <form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>" name="form1" onSubmit='return prepareData()'
        ...
        <tr>
            ...
            <td width="50%">
                <input type="checkbox" name="USE_DIFF_SITES_SETTINGS"
                       id="USE_DIFF_SITES_SETTINGS"<?= $bSites ? " checked=\"checked\"" : "" ?>
            ...
            <td colspan="2">
                ...
                <table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table"
                    ...
                    <tr>
                        ...
                        <td><input type="checkbox" name="HANDLER[ACTIVE]"
                                   value="Y" <?= ($arDeliveryInfo[$SITE_ID]["ACTIVE"] == "Y" ? "checked=\"checked\"" : "") ?> />
                        </td>
                    </tr>
                    <tr>
                        <td class="field-name"><?= GetMessage('SALE_DH_HANDLER_SORT') ?></td>
                        <td><input type="text" name="HANDLER[SORT]"
                                   value="<?= intval($arDeliveryInfo[$SITE_ID]["SORT"]) ?>" size="3"/></td>
                    </tr>
                    <tr>
                        <td class="field-name"><?= GetMessage('SALE_DH_HANDLER_NAME') ?></td>
                        <td><input type="text" name="HANDLER[NAME]"
                                   value="<?= htmlspecialcharsbx($arDeliveryInfo[$SITE_ID]["NAME"]) ?>"/></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="HANDLER[TAX_RATE]"
                                   value="<?= doubleval($arDeliveryInfo[$SITE_ID]["TAX_RATE"]) ?>" size="3"/></td>
                    </tr>
                    <tr>
                ...
                foreach ($arDeliveryInfo[$SITE_ID]["CONFIG"]["CONFIG_GROUPS"] as $group => $arConfigGroup) {
                    ...
                    ?>
                    <table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table"
                           id="params_<?= htmlspecialcharsbx($group) ?>_table"><?
                ...
                foreach ($arDeliveryInfo[$SITE_ID]["PROFILES"] as $profileId => $arProfile) {
                    if (!isset($arProfile["TABBED"])) {
                        ...
                        ?>
                        <table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table"
                               id="params_<?= htmlspecialcharsbx($profileId) ?>_table"><?
...
function printProfileInfo($SID, $profileId, $arProfile, $baseCurrency)
{
    ...
    <tr>
        ...
        <td valign="top" width="60%"><input type="checkbox"
                                            name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][ACTIVE]"
                                            value="Y"
                                            <? if ($arProfile['ACTIVE'] == 'Y'): ?>checked="checked"<?endif ?> /></td>
    </tr>
    <tr>
        <td align="right"><?= GetMessage('SALE_DH_PROFILE_TITLE') ?></td>
        <td valign="top" width="60%"><input type="text"
                                            name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][TITLE]"
                                            value="<?= htmlspecialcharsbx($arProfile["TITLE"]) ?>" size="25"/></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td valign="top" width="60%"><input type="text"
                                            name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][TAX_RATE]"
                                            value="<?= floatval($arProfile["TAX_RATE"]) ?>" size="3"/></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td valign="top" width="60%">
            <input type="text" name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][RESTRICTIONS_WEIGHT][0]"
                   value="<?= number_format($arProfile['RESTRICTIONS_WEIGHT'][0] / $weight_koef, 2, '.', '') ?>"
                   size="8"/>&nbsp;
            <input type="text" name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][RESTRICTIONS_WEIGHT][1]"
                   value="<?= number_format($arProfile['RESTRICTIONS_WEIGHT'][1] / $weight_koef, 2, '.', '') ?>"
                   size="8"/>
        </td>
    </tr>
    <tr>
        ...
        <td valign="top" width="60%">
            <input type="text" name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][RESTRICTIONS_SUM][1]"
                   value="<?= number_format(floatval($arProfile['RESTRICTIONS_SUM'][1]), 2, '.', '') ?>" size="8"/>&nbsp;
            <input type="text" name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][RESTRICTIONS_SUM][2]"
                   value="<?= number_format(floatval($arProfile['RESTRICTIONS_SUM'][2]), 2, '.', '') ?>" size="8"/>&nbsp;
            <input type="hidden" name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][RESTRICTIONS_SUM][0]"
                   value="<?= $baseCurrency ?>">
        </td>
    </tr>
    <tr class="heading">
    ...
    <tr>
        ...
        <td valign="top" width="60%">
            <input type="text"
                   name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][RESTRICTIONS_DIMENSIONS][0]"
                   value="<?= intval($arProfile['RESTRICTIONS_DIMENSIONS'][0]) ?>" size="8"/>&nbsp;
            <input type="text"
                   name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][RESTRICTIONS_DIMENSIONS][1]"
                   value="<?= intval($arProfile['RESTRICTIONS_DIMENSIONS'][1]) ?>" size="8"/>&nbsp;
            <input type="text"
                   name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][RESTRICTIONS_DIMENSIONS][2]"
                   value="<?= intval($arProfile['RESTRICTIONS_DIMENSIONS'][2]) ?>" size="8"/>
        </td>
    </tr>
    <tr>
        <td class="field-name"><?= GetMessage('SDEN_PACKAGE_MAX_SIZE') ?>:</td>
        <td valign="top" width="60%">
            <input type="text" name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][RESTRICTIONS_MAX_SIZE]"
                   value="<?= intval($arProfile['RESTRICTIONS_MAX_SIZE']) ?>" size="8"/>
        </td>
    </tr>
    <tr>
        ...
        <td valign="top" width="60%">
            <input type="text"
                   name="HANDLER[PROFILES][<?= htmlspecialcharsbx($profileId) ?>][RESTRICTIONS_DIMENSIONS_SUM]"
                   value="<?= intval($arProfile['RESTRICTIONS_DIMENSIONS_SUM']) ?>" size="8"/>
        </td>
    </tr>
    <?
}
