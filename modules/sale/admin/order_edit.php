    ...
    if (!$disabledRefresh) {
        $actionMenu[] = array(
            ...
            "ONCLICK" => "if(confirm('" . GetMessageJS(
                    "SOE_ORDER_REFRESH_CONFIRM"
                ) . "')) BX.Sale.Admin.OrderEditPage.onRefreshOrderDataAndSave();"
        );
    }
...
$aTabs = array(
    ...
    array(
        "DIV" => "tab_analysis",
        "TAB" => Loc::getMessage("SALE_TAB_ANALYSIS"),
        "TITLE" => Loc::getMessage("SALE_TAB_ANALYSIS")
    )
);
...
?>
    <form method="POST" action="<?= $url ?>" name="sale_order_edit_form" id="sale_order_edit_form"
                    ...
                    foreach ($blocksOrder as $blockCode) {
                        ...
                        switch (BlockType::resolveId($blockCode)) {
                            ...
                            case BlockType::BASKET:
                                ...
                                echo '<div style="display: none;">' . $orderBasket->settingsDialog->getHtml(
                                    ) . '</div>';
                                break;
                            default:
    ...
    <script type="text/javascript">
        BX.ready(function () {
            BX.Sale.Admin.OrderAjaxer.sendRequest(
                BX.merge(
                    BX.Sale.Admin.OrderEditPage.ajaxRequests.getOrderTails("<?=$order->getId(
                    )?>", "edit", "<?=$basketPrefix?>"),
                    <?=\CUtil::PhpToJSObject(
                        $link
                            ->create()
