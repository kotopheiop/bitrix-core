...
while ($arRes = $rsData->NavNext(true, "f_")):
    ...
    $arActions[] = array(
        ...
        "ACTION" => $lAdmin->ActionRedirect(
            "userfield_edit.php?ID=" . $f_ID . "&back_url=%2Fbitrix%2Fadmin%2Fsale_order_uf_props.php?lang=" . LANG
        )
    );
    $arActions[] = array(
        "ICON" => "delete",
        "TEXT" => GetMessage("MAIN_DELETE"),
        "ACTION" => "if(confirm('" . GetMessage('USERTYPE_DELETE_CONF') . "')) " . $lAdmin->ActionDoGroup(
                $f_ID,
                "delete"
            )
    );
...
?>
    <form name="find_form" method="get" action="<? echo $APPLICATION->GetCurPage(); ?>">
        ...
        <tr>
            ...
            <td><input type="text" name="find_field_name" size="47"
                       value="<? echo htmlspecialchars($find_field_name) ?>"></td>
        </tr>
        <tr>
