    ...
    <td>
        <select name="GROUPS[]" multiple size="5">
            ...
            foreach ($userGroupList as $userGroupData) {
                ?>
                <option value="<?= $userGroupData["ID"] ?>"<?
                if (in_array($userGroupData["ID"], $currentGroups)) echo " selected"; ?>><?= htmlspecialcharsEx($userGroupData["NAME"]) ?></option><?
            }
            ?>
        </select>
    </td>
    ...
    <td>
        <select name="RESPONSIBLE_GROUPS[]" multiple size="5">
            ...
            foreach ($userGroupList as $userGroupData) {
                ?>
                <option value="<?= $userGroupData["ID"] ?>"<?
                if (in_array($userGroupData["ID"], $currentResponsibleGroups)) echo " selected"; ?>><?= htmlspecialcharsEx($userGroupData["NAME"]) ?></option><?
            }
            ?>
        </select>
    </td>
...
<script language="JavaScript">
    ...
    for (i in inputList) {
        if (inputList.hasOwnProperty(i))
            BX.bind(inputList[i], 'change', function () {
                isFormChange = true
            });
    }
    ...
    for (i in selectList) {
        if (selectList.hasOwnProperty(i))
            BX.bind(selectList[i], 'change', function () {
                isFormChange = true
            });
    }
