...
if (!isset($settings[$SITE_ID])) {
    LocalRedirect(
        "/bitrix/admin/sale_ebay_general.php?lang=" . LANG . "&SITE_ID=" . $SITE_ID . "&back_url=" . urlencode(
...
?>
    <form method="post" action="<?= $APPLICATION->GetCurPage() ?>?lang=<?= LANGUAGE_ID ?>"
        ...
        ?>
        <tr class="heading">
            <td colspan="2"><?= Loc::getMessage("SALE_EBAY_DEFAULT") ?></td>
        </tr>
        <tr>
            <td width="40%"><span class="adm-required-field"><?= Loc::getMessage(
                        "SALE_EBAY_TAB_POLICY_RETURN"
                    ) ?>:</span></td>
            <td width="60%">
                <? $hiddenPolicyFields = ""; ?>
                    ...
                    <? if ($policy): ?>
                        <? foreach (
                            ...
                            <option value="<?= htmlspecialcharsbx(
                                $policyId
                            ) ?>"<?= isset($siteSettings["POLICY"]["RETURN"]["DEFAULT"]) && $siteSettings["POLICY"]["RETURN"]["DEFAULT"] == $policyId ? " selected" : "" ?>><?= $policyName ?></option>
                ...
                <select name="EBAY_SETTINGS[POLICY][SHIPPING][DEFAULT]">
                    <? if ($policy): ?>
                        <? foreach (
                            ...
                            <option value="<?= htmlspecialcharsbx(
                                $policyId
                            ) ?>"<?= isset($siteSettings["POLICY"]["SHIPPING"]["DEFAULT"]) && $siteSettings["POLICY"]["SHIPPING"]["DEFAULT"] == $policyId ? " selected" : "" ?>><?= $policyName ?></option>
                ...
                <select name="EBAY_SETTINGS[POLICY][PAYMENT][DEFAULT]">
                    <? if ($policy): ?>
                        <? foreach (
                            ...
                            <option value="<?= htmlspecialcharsbx(
                                $policyId
                            ) ?>"<?= isset($siteSettings["POLICY"]["PAYMENT"]["DEFAULT"]) && $siteSettings["POLICY"]["PAYMENT"]["DEFAULT"] == $policyId ? " selected" : "" ?>><?= $policyName ?></option>
                    ...
                    <td width="60%">
                        <select name="EBAY_SETTINGS[MAPS][SHIPMENT][<?= $service ?>]">
                            ...
                            <? foreach ($arDeliveryList as $deliveryId => $deliveryName):?>
                                <option value="<?= $deliveryId ?>"<?= (isset($siteSettings["MAPS"]["SHIPMENT"][$service]) && $siteSettings["MAPS"]["SHIPMENT"][$service] == $deliveryId ? " selected" : "") ?>><?= htmlspecialcharsbx(
                                        $deliveryName
                                    ) ?></option>
                            <?endforeach; ?>
                        </select>
