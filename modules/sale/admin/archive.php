...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["archive"] == "Y" && check_bitrix_sessid()) {
    ...
    <form method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?lang=<? echo htmlspecialcharsbx(LANG) ?>"
          id="archiveForm">
        <?
        $tabControl->Begin();
        ...
        <tr>
            ...
            <td><input type="text" name="archive_count_execution" id="archive_count_execution" size="3"
                       value="<? echo $countExecutionOrders; ?>"> <? echo Loc::getMessage("ARCHIVE_STEP_ORDER") ?></td>
        </tr>
        <tr>
        ...
        if ($saleModulePermissions >= "W") {
            ?>
            <tr>
                <td valign="top"><label for="archive_blocked_order_accept"><?
                        echo Loc::getMessage("ARCHIVE_BLOCKED_ORDER_ACCEPT") ?>:</label></td>
                <td>
                    <input type="checkbox" name="archive_blocked_order_accept" id="archive_blocked_order_accept"
                           value="Y" <?
                    if (Option::get("sale", "archive_blocked_order") === "Y") echo "checked" ?>>
                </td>
            </tr>
            <?
            $shopList = array();
                ...
                <tr valign="top">
                    ...
                    <td>
                        <select name="archive_site[]" id="archive_site" multiple
                                size="<?= (count($shopList) < 5) ? count($shopList) : 5 ?>">
                            <?
        ...
        $tabControl->Buttons();
        ?>
        <input type="button" id="start_button" value="<? echo Loc::getMessage("ARCHIVE_ARCHIVE_BUTTON") ?>"
               OnClick="StartReindex();" class="adm-btn-save">
        <input type="button" id="stop_button" value="<?= Loc::getMessage("ARCHIVE_STOP") ?>" OnClick="StopArchive();"
               disabled>
        <input type="button" id="continue_button" value="<?= Loc::getMessage("ARCHIVE_CONTINUE") ?>"
               OnClick="ContinueArchive();" disabled>
        <?
        $tabControl->End();
        ?>
    </form>
