...
?>
    <form method="POST" action="<?= $actionUrl ?>" name="form1">
        ...
        <tr class="adm-detail-required-field">
            ...
            <td width="60%">
                <? if ($ID > 0): ?>
                    ...
                    [<a title="<? echo GetMessage("SAE_USER_PROFILE") ?>"
                        href="<?= $profileUrl ?>"><?= $str_USER_ID ?></a>] (<?= $str_USER_LOGIN ?>) <?= $str_USER_NAME ?> <?= $str_USER_LAST_NAME ?>
                <? else: ?>
        ...
        <tr class="adm-detail-required-field">
            ...
            <td>
                <input type="text" name="CURRENT_BUDGET" size="10" maxlength="20"
                       value="<?= roundEx($str_CURRENT_BUDGET, SALE_VALUE_PRECISION) ?>">
                <?
                if ($ID > 0) {
                    ?>
        ...
        <? if ($ID > 0 && $str_LOCKED == "Y"): ?>
            <tr>
                <td><?
                    echo GetMessage("SAE_UNLOCK") ?></td>
                <td>
                    <input type="checkbox" name="UNLOCK" value="Y"<?
                    if ($str_LOCKED != "Y") echo " disabled" ?>>
                    <?
                    if ($str_LOCKED == "Y")
        ...
        <tr>
            <td valign="top"><? echo GetMessage("SAE_OSN") ?><br><small><? echo GetMessage("SAE_OSN_NOTE") ?></small>
            </td>
            <td valign="top">
