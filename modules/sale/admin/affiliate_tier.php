...
while ($arAffiliateTier = $dbResultList->NavNext(true, "f_")) {
    $row =& $lAdmin->AddRow(
        $f_ID,
        $arAffiliateTier,
        "sale_affiliate_tier_edit.php?ID=" . $f_ID . "&lang=" . LANG . GetFilterParams("filter_"),
        GetMessage("SAT1_UPDATE_TIER")
    );
    ...
    if ($saleModulePermissions >= "W") {
        ...
        $arActions[] = array(
            "ICON" => "delete",
            "TEXT" => GetMessage("SAT1_DELETE"),
            "ACTION" => "if(confirm('" . GetMessage("SAT1_DELETE_CONF") . "')) " . $lAdmin->ActionDoGroup(
                    $f_ID,
                    "delete"
                )
        );
