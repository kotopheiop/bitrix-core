...
if (($ids = $lAdmin->GroupAction()) && $saleModulePermissions >= "U" && check_bitrix_sessid()) {
    foreach ($ids as $id) {
        ...
        switch ($_REQUEST['action']) {
            case 'delete':
                ...
                if ($res->isSuccess()) {
                    $lAdmin->AddActionSuccessMessage(
                        Loc::getMessage('SALE_DELIVERY_REQ_LIST_DELETE', array('#ID#' => $id))
                    );
                } else {
            ...
            default:
                $lAdmin->AddGroupError(
                    Loc::getMessage('SALE_DELIVERY_REQ_LIST_UNKNOWN_ACTION', array('#ACTION#' => $_REQUEST['action'])),
                    $id
                );
                break;
        }
...
if (intval($find_id) > 0) {
    $filter["=ID"] = intval($find_id);
}
if (!empty($find_delivery_id) && is_array($find_delivery_id)) {
    $filter["=DELIVERY_ID"] = $find_delivery_id;
}
//if(strlen(trim($find_status)) > 0) $filter["=STATUS"] = trim($find_status);
if (trim($find_external_id) <> '') {
    $filter["=EXTERNAL_ID"] = trim($find_external_id);
}
...
if (strval(trim($find_external_date_insert_to)) != '') {
    if ($arDate = ParseDateTime($find_external_date_insert_to, CSite::GetDateFormat("FULL", SITE_ID))) {
        ...
        $find_external_date_insert_to = date(
...
while ($row = $res->fetch()) {
    ...
    if (!isset($deliveryList[$handlingDeliveryServiceId])) {
        ...
        if ($handlingDelivery) {
            $deliveryList[$handlingDeliveryServiceId] = $handlingDelivery->getNameWithParent(
                ) . ' [' . $handlingDeliveryServiceId . ']';
        }
    }
}
...
$aHeaders = array(
    ...
    array(
        "id" => "DATE",
        "content" => Loc::getMessage('SALE_DELIVERY_REQ_LIST_F_DATE_INSERT'),
        "sort" => "DATE",
        "default" => false
    ),
    array(
        "id" => "DELIVERY_ID",
        "content" => Loc::getMessage('SALE_DELIVERY_REQ_LIST_F_DELIVERY_ID'),
        "default" => true
    ),
//	array("id"=>"STATUS", "content"=>Loc::getMessage('SALE_DELIVERY_REQ_LIST_F_STATUS'), "default"=>true),
    array(
        "id" => "EXTERNAL_ID",
        "content" => Loc::getMessage('SALE_DELIVERY_REQ_LIST_F_EXTERNAL_ID'),
        "default" => true
    ),
    array(
        "id" => "ORDERS_NUMBER",
        "content" => Loc::getMessage('SALE_DELIVERY_REQ_LIST_F_ORDERS_NUMBER'),
        "default" => false
    ),
    array(
        "id" => "SHIPMENTS_NUMBER",
        "content" => Loc::getMessage('SALE_DELIVERY_REQ_LIST_F_SHIPMENTS_NUMBER'),
        "default" => false
    )
);
...
while ($fields = $resRequestList->fetch()) {
    ...
    $row->AddViewField(
        'DELIVERY_ID',
        '<a href="/bitrix/admin/sale_delivery_service_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $fields['DELIVERY_ID'] . '&PARENT_ID=' . $delivery->getParentId(
        ) . '">' . htmlspecialcharsbx($deliveryServiceName) . '</a>'
    );
...
//	$row->AddViewField("STATUS", $fields['STATUS']);
    ...
    if (in_array("ORDERS_NUMBER", $arVisibleColumns) || in_array("SHIPMENTS_NUMBER", $arVisibleColumns)) {
        ...
        $reqRes = Requests\ShipmentTable::getList(
            array(
                'filter' => array(
        ...
        while ($req = $reqRes->fetch()) {
            ...
            $shipmentNumbers .= Requests\Helper::getShipmentEditLink(
                $req['SHIPMENT_ID'],
                $req['SHIPMENT_ID'],
                $req['ORDER_ID']
            );
    ...
    if ($saleModulePermissions >= "U") {
        ...
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect(
                "sale_delivery_request_view.php?lang=" . LANGUAGE_ID . "&ID=" . $fields['ID']
            )
        );
        ...
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . Loc::getMessage(
                    'SALE_DELIVERY_REQ_LIST_DEL_CONFIRM'
                ) . "')) " . $lAdmin->ActionDoGroup($fields['ID'], "delete")
        );
...
?>
    <form name="form1" method="POST" action="<?= $APPLICATION->GetCurPage() ?>">
        ...
        <tr>
            ...
            <td><input type="text" name="find_id" size="40"
                       value="<?= htmlspecialcharsbx($find_name) ?>"><?= ShowFilterLogicHelp() ?></td>
        </tr>
        <tr>
            ...
            <td>
                <select name="find_delivery_id[]" multiple size="3" class="adm-select-multiple">
                    <? foreach ($deliveryList as $deliveryId => $deliveryName): ?>
                        <option value="<?= $deliveryId ?>"<?= (is_array($find_delivery_id) && in_array(
                            $deliveryId,
                            $find_delivery_id
                        ) ? ' selected' : '') ?>><?= htmlspecialcharsbx($deliveryName) ?></option>
                    <?endforeach; ?>
                </select>
            </td>
        </tr>
	...
	<tr>
		...
		<td><input type="text" name="find_status" size="40" value="<?= htmlspecialcharsbx(
            $find_status
...
-->
        <tr>
            ...
            <td><input type="text" name="find_external_id" size="40"
                       value="<?= htmlspecialcharsbx($find_external_id) ?>"><?= ShowFilterLogicHelp() ?></td>
        </tr>
        <tr>
