...
$lAdmin->AddHeaders(
    array(
        array(
            "id" => "EXTERNAL_ID",
            "content" => GetMessage("SALE_REFUND_HANDLERS_LIST_EXTERNAL_ID"),
            "default" => true
        ),
...
while ($arCCard = $dbRes->NavNext(false)) {
    $row =& $lAdmin->AddRow(
        $arCCard["EXTERNAL_ID"],
        $arCCard,
        "sale_ps_handler_refund_edit.php?" . $arCCard["LINK_PARAMS"] . "&handler=" . ToLower(
            $arCCard["HANDLER"]
        ) . "&lang=" . LANGUAGE_ID
    );
    ...
    $row->AddField(
        "EXTERNAL_ID",
        "<a href=\"sale_ps_handler_refund_edit.php?" . $arCCard["LINK_PARAMS"] . '&handler=' . ToLower(
            $arCCard["HANDLER"]
        ) . "&lang=" . LANGUAGE_ID . "\">" . $arCCard["EXTERNAL_ID"] . "</a>"
    );
