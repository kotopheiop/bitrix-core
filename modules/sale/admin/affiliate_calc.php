...
<head>
    <title><?
        echo GetMessage("SAC_STEP_TITLE") ?></title>
</head>
<body>
<?
...
echo LANG; ?>&<?
echo $urlParams ?>"><?
    echo GetMessage("SAC_STEP_AUTO_HINT2") ?></a><br>
...
<form method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?" name="form1">
    ...
    <tr>
        ...
        <td width="60%" valign="top">
            <? echo CalendarPeriod("DATE_CALC_FROM", $DATE_CALC_FROM, "DATE_CALC_TO", $DATE_CALC_TO, "form1", "N") ?>
            <br>
            <small><? echo GetMessage("SAC_CALC_PERIOD_HINT") ?><br><? echo GetMessage("SAC_CALC_PERIOD_HINT1") ?>
            </small>
        </td>
    </tr>
    <tr>
        ...
        <td width="60%" valign="top">
            ...
            <br>
            <small><? echo GetMessage("SAC_PLAN_PERIOD_HINT3") ?><br><? echo GetMessage("SAC_PLAN_PERIOD_HINT4") ?>
            </small>
        </td>
    </tr>
    <tr>
        ...
        <td width="60%" valign="top">
            ...
            foreach ($arPossibleActions as $key => $value) {
                ?>
                <input type="radio" name="SUM_TODO" id="ID_SUM_TODO_<?= $key ?>" value="<?= $key ?>"<?
                if ($SUM_TODO == $key || StrLen($SUM_TODO) <= 0 && $key == "U") echo " checked"; ?>>
    ...
    <input<?= ($saleModulePermissions >= "W") ? "" : " disabled" ?> type="submit" name="apply"
                                                                    value="<? echo GetMessage("SAC_DO_CALC") ?>"
                                                                    title="<? echo GetMessage("SAC_DO_CALC_DESCR") ?>"
                                                                    class="adm-btn-save">
    <input<?= ($saleModulePermissions >= "W") ? "" : " disabled" ?> type="reset" name="dontsave"
                                                                    value="<? echo GetMessage("SAC_RESET") ?>"
                                                                    title="<? echo GetMessage("SAC_RESET") ?>">
