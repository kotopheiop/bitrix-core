...
if ($salePermissions >= "R") :
    ...
    $arAllOptions = array(
        ...
        array(
            ...
            GetMessage("SALE_1C_EXPORT_ALLOW_DELIVERY_ORDERS"),
            "",
            Array("checkbox")
        ),
        ...
        while ($arPersonType = $dbPersonType->GetNext()) {
            ...
            if (intval($arExportProfile[$arPersonType["ID"]]) > 0) {
                $res = CSaleExport::Update(
                    $arExportProfile[$arPersonType["ID"]],
                    Array(
                        "PERSON_TYPE_ID" => $arPersonType["ID"],
                        "VARS" => serialize($arParams)
                    )
                );
            } else {
    ...
    <script>
        ...
        var arUserFieldsNameList = new Array("<?=GetMessage("SPS_USER_ID")?>", "<?=GetMessage(
            ...
            "SPS_USER_FAX"
        )?>", "<?=GetMessage("SPS_USER_PHONE")?>", "<?=GetMessage("SPS_USER_PHONE_MOBILE")?>", "<?=GetMessage(
            "SPS_USER_ADDRESS"
        )?>", "<?=GetMessage("SPS_USER_POST")?>", "<?=GetMessage("SPS_USER_CITY")?>", "<?=GetMessage(
            "SPS_USER_STATE"
        )?>", "<?=GetMessage("SPS_USER_ZIP")?>", "<?=GetMessage("SPS_USER_COUNTRY")?>", "<?=GetMessage(
            "SPS_USER_COMPANY"
        )?>", "<?=GetMessage("SPS_USER_DEPT")?>", "<?=GetMessage("SPS_USER_DOL")?>", "<?=GetMessage(
            "SPS_USER_COM_WEB"
        )?>", "<?=GetMessage("SPS_USER_COM_PHONE")?>", "<?=GetMessage("SPS_USER_COM_FAX")?>", "<?=GetMessage(
            "SPS_USER_COM_ADDRESS"
        )?>", "<?=GetMessage("SPS_USER_COM_POST")?>", "<?=GetMessage("SPS_USER_COM_CITY")?>", "<?=GetMessage(
            "SPS_USER_COM_STATE"
        )?>", "<?=GetMessage("SPS_USER_COM_ZIP")?>", "<?=GetMessage("SPS_USER_COM_COUNTRY")?>", "<?=GetMessage(
            "SPS_USER_COM_NOTES"
        )?>");
        ...
        var arOrderFieldsNameList = new Array("<?=GetMessage("SPS_ORDER_ID")?>", "<?=GetMessage(
            ...
            "SPS_ORDER_PS"
        )?>", "<?=GetMessage("SPS_ORDER_PS_NAME")?>", "<?=GetMessage("SPS_ORDER_DELIV")?>", "<?=GetMessage(
            "SPS_ORDER_DELIV_NAME"
        )?>", "<?=GetMessage("SPS_ORDER_TAX")?>", "<?=GetMessage("SPS_ORDER_TRACKING_NUMBER")?>", "<?=GetMessage(
            "SPS_ORDER_PAY_VOUCHER_NUM"
        )?>", "<?=GetMessage("SPS_ORDER_PAY_VOUCHER_DATE")?>", "<?=GetMessage("SPS_ORDER_SUM_PAID")?>", "<?=GetMessage(
            "SPS_ORDER_DELIVERY_DOC_NUM"
        )?>", "<?=GetMessage("SPS_ORDER_DELIVERY_DOC_DATE")?>", "<?=GetMessage("SPS_ORDER_COMMENTS")?>", "<?=GetMessage(
            "SPS_ORDER_USER_DESCRIPTION"
        )?>");
        ...
        function ActionFileChange(ind, type) {
            ...
            window.frames["hidden_action_frame_" + ind].location.replace('/bitrix/admin/sale_options_get.php?lang=<?= htmlspecialcharsbx(
                $lang
            ) ?>&type=' + type + '&divInd=' + ind);
        }
        ...
        function AddRekv(ind, j) {
            ...
            BX.bind(BX('TYPE_REKV_n' + j + '_' + ind), 'change', function () {
                PropertyTypeChange('REKV_n' + j, ind);
            });
        ...
        <td colspan="2">
            ...
            while ($arPersonType = $dbPersonType->GetNext()) {
                $aTabs1[] = Array(
                    ...
                    "TAB" => $arPersonType["NAME"] . " (" . htmlspecialcharsbx(
                            implode(", ", $arPersonType["LIDS"])
                        ) . ")",
                    "TITLE" => $arPersonType["NAME"] . " (" . htmlspecialcharsbx(
                            implode(", ", $arPersonType["LIDS"])
                        ) . ")"
                );
                ...
                <script>
                    ...
                    $i++;
                    ?>
                    arPropFieldsList[<?= $arPersonType["ID"] ?>][<?= $i ?>] = '<?= CUtil::JSEscape(
                        $arOrderProps["ID"]
                    ) ?>';
                    arPropFieldsNameList[<?= $arPersonType["ID"] ?>][<?= $i ?>] = '<?= CUtil::JSEscape(
                        $arOrderProps["NAME"]
                    ) ?>';
                    <?
                    ...
                    ?>
                    arPropFieldsList[<?= $arPersonType["ID"] ?>][<?= $i ?>] = '<?= CUtil::JSEscape(
                        $arOrderProps["ID"] . "_COUNTRY"
                    ) ?>';
                    arPropFieldsNameList[<?= $arPersonType["ID"] ?>][<?= $i ?>] = '<?= CUtil::JSEscape(
                        $arOrderProps["NAME"] . " (" . GetMessage("SPS_JCOUNTRY") . ")"
                    ) ?>';
                    <?
                    $i++;
                    ?>
                    arPropFieldsList[<?= $arPersonType["ID"] ?>][<?= $i ?>] = '<?= CUtil::JSEscape(
                        $arOrderProps["ID"] . "_REGION"
                    ) ?>';
                    arPropFieldsNameList[<?= $arPersonType["ID"] ?>][<?= $i ?>] = '<?= CUtil::JSEscape(
                        $arOrderProps["NAME"] . " (" . GetMessage("SPS_JREGION") . ")"
                    ) ?>';
                    <?
                    ...
                    ?>
                    arPropFieldsList[<?= $arPersonType["ID"] ?>][<?= $i ?>] = '<?= CUtil::JSEscape(
                        $arOrderProps["ID"] . "_CITY"
                    ) ?>';
                    arPropFieldsNameList[<?= $arPersonType["ID"] ?>][<?= $i ?>] = '<?= CUtil::JSEscape(
                        $arOrderProps["NAME"] . " (" . GetMessage("SPS_JCITY") . ")"
                    ) ?>';
                    <?
                    }
                    }
            ...
            foreach ($personType as $val) {
                ...
                <table cellspacing="5" cellpadding="0" border="0" width="0%">
                    <tr>
                        ...
                        <td><select name="person_type_1c_<?= $val["ID"] ?>" id="person_type_1c_<?= $val["ID"] ?>"
                                    onchange="ActionFileChange(<?= CUtil::JSEscape($val["ID"]) ?>, this.value)">
                ...
                <iframe style="width:0px; height:0px; border: 0px" name="hidden_action_frame_<?= $val["ID"] ?>" src=""
                        width="0" height="0"></iframe>
                <input type="hidden" name="export_fields_<?= $val["ID"] ?>" id="export_fields_<?= $val["ID"] ?>"
                       value="">
                <script language="JavaScript">
                    <!--
                    BX.ready(function () {
