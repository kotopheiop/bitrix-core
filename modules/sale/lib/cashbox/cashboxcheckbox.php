...
class CashboxCheckbox extends Cashbox implements IPrintImmediately, ICheckable
{
    ...
    public function printImmediately(Check $check)
    {
        ...
        $printResult = $this->sendRequestWithAuthorization(
    ...
    private function sendRequestWithAuthorization(
        string $method,
        string $url,
        array $headersData = [],
        array $bodyData = [],
    ...
    private function openShift(): Result
    {
        ...
        $openShiftResult = $this->sendRequestWithAuthorization(
            self::HTTP_METHOD_POST,
            $url,
            $requestHeaders,
            $requestBody
        ...
        switch ($response['http_code']) {
            case self::HTTP_RESPONSE_CODE_202:
                ...
                if (!$openShiftSuccess) {
                    $openShiftResult->addError(
                        new Main\Error(Loc::getMessage('SALE_CASHBOX_CHECKBOX_SHIFT_OPEN_ERROR'))
                    );
                    return $openShiftResult;
                }
            ...
            case self::HTTP_RESPONSE_CODE_400:
                ...
                if ($currentShift['status'] && $currentShift['status'] === static::SHIFT_STATUS_OPENED) {
                    $openShiftResult->addWarning(
                        new ResultWarning(Loc::getMessage('SALE_CASHBOX_CHECKBOX_SHIFT_ALREADY_OPENED'))
                    );
                    return $openShiftResult;
                }
    ...
    private function closeShift(): Result
    {
        ...
        $closeShiftResult = $this->sendRequestWithAuthorization(
            self::HTTP_METHOD_POST,
            $url,
            $requestHeaders,
            $requestBody
