...
class CashboxRobokassa extends CashboxPaySystem
{
    ...
    protected function processPrintResult(Sale\Result $result): Sale\Result
    {
        ...
        switch ($resultCode) {
            ...
            case 1:
                $processPrintResult->addError(
                    new Main\Error(Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_PRINT_ERROR_FORMAT'))
                );
                break;
            ...
            case 2:
                $processPrintResult->addError(
                    new Main\Error(
                        Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_PRINT_ERROR_NOT_ENOUGH_MONEY')
                    )
                );
                break;
            ...
            default:
                $processPrintResult->addError(
                    new Main\Error(Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_PRINT_ERROR_INTERNAL'))
                );
                break;
        }
    ...
    protected function processCheckResult(Sale\Result $result): Sale\Result
    {
        ...
        switch ($resultCode) {
            case 1:
                $processCheckResult->addError(
                    new Main\Error(Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_PRINT_ERROR_WAIT'))
                );
                break;
            ...
            case 3:
                $processCheckResult->addError(
                    new Main\Error(Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_STATUS_ERROR_REGISTER'))
                );
                break;

            default:
                $processCheckResult->addError(
                    new Main\Error(Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_STATUS_ERROR_PROCESSING'))
                );
                break;
        }
    ...
    protected function onAfterProcessCheck(Sale\Result $result, Sale\Payment $payment): Sale\Result
    {
        ...
        if ($checkList) {
            ...
            foreach ($statuses as $key => $status) {
                ...
                switch ($resultCode) {
                    case 1:
                        $onAfterProcessCheckResult->addError(
                            new Main\Error(Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_PRINT_ERROR_WAIT'))
                        );
                        break;
                    ...
                    case 3:
                        $onAfterProcessCheckResult->addError(
                            new Main\Error(
                                Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_STATUS_ERROR_REGISTER')
                            )
                        );
                        break;
                    ...
                    default:
                        $onAfterProcessCheckResult->addError(
                            new Main\Error(
                                Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_STATUS_ERROR_PROCESSING')
                            )
                        );
                        break;
                }
            ...
            $onAfterProcessCheckResult->addError(
                new Main\Error(Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_CHECK_NOT_FOUND'))
            );
        }
    ...
    protected function send(string $url, Sale\Payment $payment, array $fields): Sale\Result
    {
        ...
        if ($response === false) {
            $result->addError(
                new Main\Error(Main\Localization\Loc::getMessage('SALE_CASHBOX_ROBOKASSA_ERROR_EMPTY_RESPONSE'))
            );
    ...
    public static function getSupportedKkmModels()
    {
        ...
        while ($paySystemItem = $paySystemIterator->fetch()) {
            ...
            if (
                ...
                $supportedKkmModels[] = Sale\BusinessValue::getValuesByCode(
                    $paySystemService->getConsumerName(),
                    $paySystemCodeForKkm
                );
            }
        }
