...
abstract class Check extends AbstractCheck
{
            ...
            foreach ($entityList as $entity) {
                ...
                if (static::getSupportedRelatedEntityType() === self::SUPPORTED_ENTITY_TYPE_PAYMENT
                    ...
                    throw new Main\NotSupportedException(
                        static::getType() . ' is not supported payment as related entity'
                    );
                }
                ...
                if (static::getSupportedRelatedEntityType() === self::SUPPORTED_ENTITY_TYPE_SHIPMENT
                    ...
                    throw new Main\NotSupportedException(
                        static::getType() . ' is not supported shipment as related entity'
                    );
                }
            }
        }
        ...
        while ($entity = $dbRes->fetch()) {
            if ($order === null) {
                if ($entity['ENTITY_TYPE'] === CheckRelatedEntitiesTable::ENTITY_TYPE_PAYMENT) {
                    ...
                    $dbResPayment = $paymentClassName::getList(
                        array(
                    ...
                    $dbResShipment = $shipmentClassName::getList(
                        array(
                            'select' => array('ORDER_ID'),
                            'filter' => array('ID' => $entity['ENTITY_ID'])
                        )
            ...
            if ($entity['ENTITY_TYPE'] === CheckRelatedEntitiesTable::ENTITY_TYPE_PAYMENT) {
                ...
                $this->relatedEntities[$entity['ENTITY_CHECK_TYPE']][] = $paymentCollection->getItemById(
                    $entity['ENTITY_ID']
                );
            } elseif ($entity['ENTITY_TYPE'] === CheckRelatedEntitiesTable::ENTITY_TYPE_SHIPMENT) {
                $shipmentCollection = $order->getShipmentCollection();
                $this->relatedEntities[$entity['ENTITY_CHECK_TYPE']][] = $shipmentCollection->getItemById(
                    $entity['ENTITY_ID']
                );
            }
        }
    ...
    private function extractDataFromEntitiesInternal(array $entities)
    {
        ...
        foreach ($entities as $entity) {
            ...
            if ($entity instanceof Payment) {
                ...
                foreach ($entity->getPayableItemCollection()->getBasketItems() as $payableItem) {
                    ...
                    $item['SUM'] = PriceMaths::roundPrecision(
                        $basketItem->getPriceWithVat() * $payableItem->getQuantity()
                    );
                ...
                foreach ($sellableItems as $shipmentItem) {
                    ...
                    if ($this->needPrintMarkingCode($basketItem)) {
                        ...
                        $item['SUM'] = PriceMaths::roundPrecision(
                            $basketItem->getPriceWithVat() * $shipmentItem->getQuantity()
                        );
        ...
        if ($order !== null) {
            ...
            if (!$result['BUYER']) {
                $result['BUYER']['EMAIL'] = Main\Config\Option::get(
                    "main",
                    "email_from",
    ...
    public function validate()
    {
        ...
        if (!isset($data['PRODUCTS'])) {
            $result->addError(
                new Main\Error(Main\Localization\Loc::getMessage('SALE_CASHBOX_CHECK_ERROR_NO_PRODUCTS'))
            );
        } else {
            $errors = [];
        ...
        if (!isset($data['BUYER']) || !$data['BUYER']) {
            $result->addError(
                new Main\Error(Main\Localization\Loc::getMessage('SALE_CASHBOX_CHECK_ERROR_NO_BUYER_INFO'))
            );
        }
