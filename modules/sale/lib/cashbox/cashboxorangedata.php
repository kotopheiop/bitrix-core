    ...
    implements IPrintImmediately, ICheckable, ITestConnection, ICorrection
{
    ...
    {
        $url = $this->getUrl();
        $url .= '/documents/' . $this->getValueFromSettings('SERVICE', 'INN') . '/status/' . $check->getField(
                'EXTERNAL_UUID'
            );
    ...
    protected function checkInternal($url)
    {
        ...
        if ($response === false) {
            return $result->addError(
                new Errors\Error(Localization\Loc::getMessage('SALE_CASHBOX_ORANGE_DATA_ERROR_CHECK_CHECK'))
            );
        }
        ...
        $settings['INTERACTION'] = [
            ...
            'ITEMS' => [
                'MODE_HANDLER' => [
                    ...
                    'OPTIONS' => [
                        static::HANDLER_MODE_ACTIVE => Localization\Loc::getMessage(
    ...
    {
        $url = $this->getUrl();
        $url .= '/corrections/' . $this->getValueFromSettings('SERVICE', 'INN') . '/status/' . $check->getField(
                'EXTERNAL_UUID'
            );
