    ...
    private static function getBasketProduct(Sale\BasketItemBase $basketItem)
    {
        ...
        if ($arBasketItem['CUSTOM_PRICE'] !== 'Y') {
            ...
            if ($arBasketItem['DISCOUNT_PRICE_PERCENT'] === null) {
                ...
                $arBasketItem['DISCOUNT_PRICE_PERCENT'] = PriceMaths::roundPrecision(
                    $arBasketItem['DISCOUNT_PRICE_PERCENT']
                );
            }
        }
        ...
        $arBasketItem['SUM_BASE'] = PriceMaths::roundPrecision(
            $basketItem->getBasePrice() * $basketItem->getQuantity()
        );

        $arBasketItem['SUM_DISCOUNT_DIFF'] = PriceMaths::roundPrecision(
            $arBasketItem['SUM_BASE'] - $arBasketItem['SUM']
        );
