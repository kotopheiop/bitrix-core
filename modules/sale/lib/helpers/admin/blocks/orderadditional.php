...
class OrderAdditional
{
    public static function getEdit($collection, $formName, $formPrefix, array $post = array())
    {
        ...
        if (isset($data['EMP_RESPONSIBLE']) && !empty($data['EMP_RESPONSIBLE'])) {
            $blockEmpResponsible = '
                ...
                ) . ': <span style="color: #66878F" id="order_additional_info_date_responsible">' . $data['DATE_RESPONSIBLE'] . '</span>  <a href="" id="order_additional_info_emp_responsible">' . htmlspecialcharsbx(
                    $data['EMP_RESPONSIBLE']
                ) . '</a></div>
					</td>
				</tr>
			';
        ...
        $data['COMPANIES'] = Manager::getListWithRestrictions(
            $collection,
            ...
            if ($saleModulePermissions == "P") {
                ...
                if (count($userCompanyList) == 1) {
                    ...
                    if (count($data['COMPANIES']) == 1) {
                        $company = reset(
                            $data['COMPANIES']
                        );// [$userCompanyId]["NAME"]." [".$data['COMPANIES'][$userCompanyId]["ID"]."]";
		...
		<div class="adm-bus-moreInfo_part2">
			<table class="adm-detail-content-table edit-table" border="0" width="100%" cellpadding="0" cellspacing="0">
				<tbody>
					<tr>
						<td class="adm-detail-content-cell-l vat" width="40%">' . Loc::getMessage(
                'SALE_ORDER_ADDITIONAL_INFO_MANAGER_COMMENT'
            ) . ':</td>
						<td class="adm-detail-content-cell-r">
							<div>
    ...
    public static function getView($collection, $formName)
    {
			...
			<table class="adm-detail-content-table edit-table" border="0" width="100%" cellpadding="0" cellspacing="0">
				<tbody>
                    ...
                    'SALE_ORDER_ADDITIONAL_INFO_COMPANY'
						...
						<td class="adm-detail-content-cell-r">' . htmlspecialcharsbx(
        ...
        return '
			...
			<table class="adm-detail-content-table edit-table" border="0" width="100%" cellpadding="0" cellspacing="0">
				<tbody>
					<tr>
						<td class="adm-detail-content-cell-l' . ($orderLocked ? '' : ' vat') . '" width="40%">' . Loc::getMessage(
                'SALE_ORDER_ADDITIONAL_INFO_MANAGER_COMMENT'
            ) . ':</td>
						<td class="adm-detail-content-cell-r">' . ($orderLocked ? '' : '<a href="javascript:void(0);" style="text-decoration: none; border-bottom: 1px dashed" onClick="BX.Sale.Admin.OrderAdditionalInfo.showCommentsDialog(\'' . $collection->getField(
                    'ID'
                ) . '\', BX(\'sale-adm-comments-view\'))">' . Loc::getMessage(
                    'SALE_ORDER_ADDITIONAL_INFO_COMMENT_TITLE'
                ) . '</a>') .
            '<p id="sale-adm-comments-view" style="color:gray; max-width:800px; overflow:auto;">' . ($data['COMMENTS'] <> '' ? nl2br(
                htmlspecialcharsbx($data['COMMENTS'])
            ) : '') . '</p>
						</td>
					</tr>
    ...
    {
        return '<a href="/bitrix/admin/user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $data["RESPONSIBLE_ID"] . '" id="order_additional_info_responsible">' . htmlspecialcharsbx(
                $data['RESPONSIBLE']
            ) . '</a>';
    }
    ...
    public static function getScripts()
    {
        ...
        return '<script type="text/javascript">' .
            'BX.message({
				SALE_ORDER_ADDITIONAL_INFO_COMMENT_EDIT: "' . \CUtil::jsEscape(
                Loc::getMessage("SALE_ORDER_ADDITIONAL_INFO_COMMENT_EDIT")
            ) . '",
				SALE_ORDER_ADDITIONAL_INFO_COMMENT_SAVE: "' . \CUtil::jsEscape(
                Loc::getMessage("SALE_ORDER_ADDITIONAL_INFO_COMMENT_SAVE")
            ) . '",
				SALE_ORDER_ADDITIONAL_INFO_NO_COMMENT: "' . \CUtil::jsEscape(
                Loc::getMessage("SALE_ORDER_ADDITIONAL_INFO_NO_COMMENT")
            ) . '"
			})' .
            '</script>';
    }
    ...
    protected static function prepareData($collection)
    {
        ...
        if (is_null($collection)) {
            ...
            if (intval($collection->getField('EMP_RESPONSIBLE_ID')) > 0) {
                $data['EMP_RESPONSIBLE'] = \Bitrix\Sale\Helpers\Admin\OrderEdit::getUserName(
                    $collection->getField('EMP_RESPONSIBLE_ID')
                );
            }
        ...
        if (intval($collection->getField('RESPONSIBLE_ID')) > 0) {
            $data['RESPONSIBLE'] = \Bitrix\Sale\Helpers\Admin\OrderEdit::getUserName(
                $collection->getField('RESPONSIBLE_ID')
            );
