...
class OrderMarker
{
    ...
    protected static function getViewList($orderId, $entityId = null)
    {
        ...
        while ($data = $res->fetch()) {
            if ($data['ENTITY_TYPE'] == EntityMarker::ENTITY_TYPE_SHIPMENT) {
                $markerListHtml .= static::getShipmentBlockHtml(
                    $data['ORDER_ID'],
                    $data['ID'],
                    $data['MESSAGE'],
                    $data['ENTITY_ID'],
                ...
                $markerListHtml .= static::getPaymentBlockHtml(
                    $data['ORDER_ID'],
                    $data['ID'],
                    $data['MESSAGE'],
                    $data['ENTITY_ID'],
                ...
                $markerListHtml .= static::getOrderBlockHtml(
                    $data['ORDER_ID'],
                    $data['ID'],
                    $data['MESSAGE'],
                    $data['TYPE']
        ...
        if (strval($text) === '') {
            ...
            $result = '
				<div class="adm-bus-orderproblem" id="sale-adm-shipment-problem-block-' . $id . '">
                    ...
                    "SALE_ORDER_MARKER_BLOCK_SHIPMENT_PROBLEM",
                    array(
                        '#ENTITY_ID#' => $entityId
                    )
                ) . ':</td>
            ...
            if ($type == EntityMarker::ENTITY_MARKED_TYPE_AUTO) {
                $result .= '<br/><div><span class="adm-btn adm-btn-green" onclick="BX.Sale.Admin.OrderEditPage.onMarkerFixErrorClick(\'' . $id . '\', \'' . $orderId . '\', \'sale-adm-shipment-problem-block-' . $id . '\', \'' . $entityId . '\', ' . ($forEntity ? 'true' : 'false') . ');">' . Loc::getMessage(
                        "SALE_ORDER_MARKER_FIX"
                    ) . '</span></div>';
            }
            ...
            $result .= '
                    ...
                    "SALE_ORDER_MARKER_CLOSE"
                ) . '" onclick="BX.Sale.Admin.OrderEditPage.onMarkerCloseClick(\'' . $id . '\', \'' . $orderId . '\', \'sale-adm-shipment-problem-block-' . $id . '\', \'' . $entityId . '\', ' . ($forEntity ? 'true' : 'false') . ');"></span>
					</div>
				</div>';
        }
    ...
    protected static function getPaymentBlockHtml($orderId, $id, $text, $entityId, $type = null, $forEntity = false)
    {
        if (strval($text) === '') {
				...
				<div class="adm-bus-orderproblem" id="sale-adm-payment-problem-block-' . $id . '">
					<div class="adm-bus-orderproblem-container">
						<table>
							<tr>
								<td class="adm-bus-orderproblem-title">' . Loc::getMessage(
                    "SALE_ORDER_MARKER_BLOCK_PAYMENT_PROBLEM",
                    array(
                        '#ENTITY_ID#' => $entityId
                    )
                ) . ':</td>
            ...
            if ($type == EntityMarker::ENTITY_MARKED_TYPE_AUTO) {
                $result .= '<br/><div><span class="adm-btn adm-btn-green" onclick="BX.Sale.Admin.OrderEditPage.onMarkerFixErrorClick(\'' . $id . '\', \'' . $orderId . '\', \'sale-adm-payment-problem-block-' . $id . '\', \'' . $entityId . '\', ' . ($forEntity ? 'true' : 'false') . ');">' . Loc::getMessage(
                        "SALE_ORDER_MARKER_FIX"
                    ) . '</span></div>';
            }
            ...
            $result .= '
						<span class="adm-bus-orderproblem-close" title="' . Loc::getMessage(
                    "SALE_ORDER_MARKER_CLOSE"
                ) . '" onclick="BX.Sale.Admin.OrderEditPage.onMarkerCloseClick(\'' . $id . '\', \'' . $orderId . '\', \'sale-adm-payment-problem-block-' . $id . '\', \'' . $entityId . '\', ' . ($forEntity ? 'true' : 'false') . ');"></span>
					</div>
				</div>';
        }
    ...
    protected static function getOrderBlockHtml($orderId, $id, $text, $type = null)
    {
        if (strval($text) === '') {
				...
				<div class="adm-bus-orderproblem" id="sale-adm-order-problem-block-' . $id . '">
					<div class="adm-bus-orderproblem-container">
						<table>
							<tr>
								<td class="adm-bus-orderproblem-title">' . Loc::getMessage(
                    "SALE_ORDER_MARKER_BLOCK_ORDER_PROBLEM"
                ) . ':</td>
								<td class="adm-bus-orderproblem-text">' . $text . '<br/>
								<div class="adm-bus-orderproblem-separator"></div>
								';
            if ($type == EntityMarker::ENTITY_MARKED_TYPE_AUTO) {
                $result .= '<br/><div><span class="adm-btn adm-btn-green" onclick="BX.Sale.Admin.OrderEditPage.onMarkerFixErrorClick(\'' . $id . '\', \'' . $orderId . '\', \'sale-adm-order-problem-block-' . $id . '\');">' . Loc::getMessage(
                        "SALE_ORDER_MARKER_FIX"
                    ) . '</span></div>';
            }
            $result .= '</td>
            ...
            $result .= '
						<span class="adm-bus-orderproblem-close" title="' . Loc::getMessage(
                    "SALE_ORDER_MARKER_CLOSE"
                ) . '" onclick="BX.Sale.Admin.OrderEditPage.onMarkerCloseClick(\'' . $id . '\', \'' . $orderId . '\', \'sale-adm-order-problem-block-' . $id . '\');"></span>
					</div>
				</div>';
        }
