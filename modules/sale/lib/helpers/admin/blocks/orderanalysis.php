...
class OrderAnalysis
{
    ...
    public static function getView(Order $order, OrderBasket $orderBasket, $selectPayment = null, $selectId = null)
    {
        ...
        foreach ($order->getPaymentCollection() as $payment) {
            ...
            if ($saleModulePermissions == "P") {
                ...
                $isPaymentAllowCompany = ($isAllowCompany || in_array(
                        $payment->getField('COMPANY_ID'),
                        $userCompanyList
                    ));
                $documentAllowList['PAYMENT'][$payment->getId(
                )] = ($isPaymentUserResponsible || $isPaymentAllowCompany);
            }
        }
        ...
        foreach ($order->getShipmentCollection() as $shipment) {
            if (!$shipment->isSystem()) {
                ...
                if ($saleModulePermissions == "P") {
                    $isShipmentUserResponsible = ($isUserResponsible || $shipment->getField(
                            'RESPONSIBLE_ID'
                        ) == $USER->GetID());
                    $isShipmentAllowCompany = ($isAllowCompany || in_array(
                            $shipment->getField('COMPANY_ID'),
                            $userCompanyList
                        ));
                    $documentAllowList['SHIPMENT'][$shipment->getId(
                    )] = ($isShipmentUserResponsible || $isShipmentAllowCompany);
                }
            }
        }
                ...
                <? foreach ($items as $item):
                    ...
                    if (is_array($item['SKU_PROPS'])) {
                        foreach ($item['SKU_PROPS'] as $skuProp) {
                            $properties .= '<tr>';
                            $properties .= '<td style="text-align: left; padding-left: 0;">' . htmlspecialcharsbx(
                                    $skuProp['NAME']
                                ) . ' : ' . '</td>';
                            ...
                            if (isset($skuProp['VALUE']['PICT']) && $skuProp['VALUE']['PICT']) {
                                ...
                                $properties .= '<td><span>' . htmlspecialcharsbx(
                                        $skuProp['VALUE']['NAME']
                                    ) . '</span></td>';
                            }
                    ...
                    <tr class="bdb-line">
                        ...
                        <td class="tac" style="text-align: right !important;"><?= $quantity . ' ' . htmlspecialcharsEx(
                                $item['MEASURE_TEXT']
                            ) ?></td>
                        <td class="tac"
                            style="text-align: right !important;;"><?= $shippedQuantity . ' ' . htmlspecialcharsEx(
                                $item['MEASURE_TEXT']
                            ) ?></td>
                        <td class="tac"
                            style="text-align: right !important;;"><?= ($quantity - $shippedQuantity) . ' ' . htmlspecialcharsEx(
                                $item['MEASURE_TEXT']
                            ) ?></td>
                    </tr>
                <?endforeach ?>
                <tr>
                    <td colspan="8" style="padding: 16px; background: #f7fafa; text-align: right;"
                        class="fwb"><?= Loc::getMessage('SALE_OANALYSIS_ITEMS_QUANTITY') . ': ' . count($items) ?></td>
                </tr>
                </tbody>
            </table>
                ...
                <div class="adm-bus-orderdocs-threelist-container">
                    <div class="adm-bus-orderdocs-threelist-block-top<?= $selectPayment === null ? ' adm-bus-orderdocs-threelist-block-children-open' : '' ?>">
                        ...
                        <div class="adm-bus-orderdocs-threelist-block-content">
                            ...
                            <? self::renderBottomBlocks(
                                $order->getField('DATE_INSERT'),
                                $order->getField('RESPONSIBLE_ID')
                            ) ?>
                        </div>
                        <div class="clb"></div>
                    </div>
                    <? foreach ($documents as $document): $isPayment = $document instanceof Payment;
                            ...
                            <div class="adm-bus-orderdocs-threelist-block-content">
                                <div class="adm-bus-orderdocs-threelist-block-title">
                                    <? if ($isPayment):
                                        ...
                                        ?>
                                        <? if ($document->isPaid()):?>
                                        <span class="adm-bus-orderdocs-docstatus adm-bus-orderdocs-docstatus-paid"><?= Loc::getMessage(
                                                'SALE_OANALYSIS_PAYMENT_PAID'
                                            ) ?></span>
                                    <? elseif ($document->isReturn()):?>
                                        <span class="adm-bus-orderdocs-docstatus"><?= Loc::getMessage(
                                                'SALE_OANALYSIS_PAYMENT_RETURN'
                                            ) ?></span>
                                    <?endif ?>
                                        <? if ($isAllowCompany): ?>
                                    ...
                                    <?
                                        ...
                                        <span class="adm-bus-orderdocs-docstatus adm-bus-orderdocs-docstatus-shippingallowed"><?= Loc::getMessage(
                                                'SALE_OANALYSIS_SHIPMENT_SHIPPED'
                                            ) ?></span>
                                    <? elseif ($document->isCanceled()):?>
                                        <span class="adm-bus-orderdocs-docstatus adm-bus-orderdocs-docstatus-canceled"><?= Loc::getMessage(
                                                'SALE_OANALYSIS_SHIPMENT_CANCELED'
                                            ) ?></span>
                                    <? elseif ($document->isAllowDelivery()):?>
                                        <span class="adm-bus-orderdocs-docstatus adm-bus-orderdocs-docstatus-shippingallowed"><?= Loc::getMessage(
                                                'SALE_OANALYSIS_SHIPMENT_ALLOWED'
                                            ) ?></span>
                                    <?endif ?>
                                        <? if ($isAllowCompany): ?>
                                ...
                                <? self::renderBottomBlocks(
                                    $document->getField($isPayment ? 'DATE_BILL' : 'DATE_INSERT'),
                                    $document->getField('RESPONSIBLE_ID')
                                ) ?>
                            </div>
                            <div class="clb"></div>
                        </div>
            ...
            <div class="adm-bus-orderdocs-threelist-block-children" style="padding-left: 60px;">
                ...
                <div class="adm-bus-orderdocs-threelist-block-content">
                    ...
                    <div class="adm-bus-orderdocs-threelist-block-date-block">
                        <?= Loc::getMessage('SALE_OANALYSIS_CREATED_AT') ?>: <span
                                class="adm-bus-orderdocs-threelist-block-date"><?= $request['REQUEST_DATE'] ?></span>
                    </div>
                </div>
                <div class="clb"></div>
            </div>
    ...
    private static function renderBottomBlocks($creationDate, $userId)
    {
        ...
        <div class="adm-bus-orderdocs-threelist-block-date-block">
            <?= Loc::getMessage('SALE_OANALYSIS_CREATED_AT') ?>: <span
                    class="adm-bus-orderdocs-threelist-block-date"><?= $creationDate ?></span>
        </div>
        <? if ($userName) :?>
    ...
    {
        return '<a class="fwb" href="' . htmlspecialcharsbx($item['EDIT_PAGE_URL']) . '">' . htmlspecialcharsEx(
                $item['NAME']
            ) . '</a>';
    }
    ...
    protected static function renderPaymentEditLink($data)
    {
        ...
        return '<a href="' . $url . '" class="adm-bus-orderdocs-threelist-block-title-link">' . Loc::getMessage(
                'SALE_OANALYSIS_PAYMENT_TITLE',
                array(
    ...
    protected static function renderShipmentEditLink($data)
    {
        ...
        return '<a href="' . $url . '" class="adm-bus-orderdocs-threelist-block-title-link' . ($shipment->isCanceled(
            ) ? "adm-bus-orderdocs-threelist-block-title-link-canceled" : "") . '">' .
            Loc::getMessage(
                'SALE_OANALYSIS_SHIPMENT_TITLE',
                array(
    ...
    {
        return '<a class="adm-bus-orderdocs-threelist-block-responsible-name" href="/bitrix/admin/user_edit.php?ID=' . $data['RESPONSIBLE_ID'] . '">' . htmlspecialcharsbx(
                $data['RESPONSIBLE']
            ) . '</a>';
    }
