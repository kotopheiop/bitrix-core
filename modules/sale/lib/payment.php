...
class Payment extends Internals\CollectableEntity implements IBusinessValueProvider, \IEntityMarker
{
    ...
    public function delete()
    {
        ...
        if ($this->isPaid()) {
            $result->addError(
                new ResultError(Loc::getMessage('SALE_PAYMENT_DELETE_EXIST_PAID'), 'SALE_PAYMENT_DELETE_EXIST_PAID')
            );
            return $result;
        }
    ...
    private function callEventOnBeforeEntityDeleted()
    {
        ...
        $event = new Main\Event(
            'sale', "OnBeforeSalePaymentEntityDeleted", [
            'ENTITY' => $this,
    ...
    private function callEventOnEntityDeleted()
    {
        ...
        /** @var Main\Event $event */
        $event = new Main\Event(
            'sale', "OnSalePaymentEntityDeleted", [
            'ENTITY' => $this,
            'VALUES' => $this->fields->getOriginalValues(),
        ]
        );
    ...
    private function callEventOnBeforeEntitySaved()
    {
        ...
        $event = new Main\Event(
            'sale', 'OnBeforeSalePaymentEntitySaved', [
            'ENTITY' => $this,
            'VALUES' => $this->fields->getOriginalValues()
        ]
        );
    ...
    public function verify()
    {
        ...
        if ($this->getPaymentSystemId() <= 0) {
            $result->addError(
                new ResultError(
                    Loc::getMessage("SALE_PAYMENT_PAYMENT_SERVICE_EMPTY"),
