        ...
        if ($this->eventName) {
            ...
            if ($eventsList = $eventManager->findEventHandlers('sale', 'OnBefore' . $this->eventName . 'SetField')) {
                ...
                $event = new Main\Event(
                    'sale', 'OnBefore' . $this->eventName . 'SetField', array(
                    'ENTITY' => $this,
                    'NAME' => $name,
                    'VALUE' => $value,
                )
                        ...
                        if ($eventResult->getType() == Main\EventResult::SUCCESS) {
                            ...
                            $errorMsg = new ResultError(
                                Main\Localization\Loc::getMessage(
                                    'SALE_EVENT_ON_BEFORE_' . mb_strtoupper($this->eventName) . '_SET_FIELD_ERROR'
                                ), 'SALE_EVENT_ON_BEFORE_' . mb_strtoupper($this->eventName) . '_SET_FIELD_ERROR'
                            );
        ...
        if ($oldValue != $value || ($oldValue === null && $value !== null)) {
            ...
            if ($this->eventName) {
                if ($eventsList = $eventManager->findEventHandlers('sale', 'On' . $this->eventName . 'SetField')) {
                    $event = new Main\Event(
                        'sale', 'On' . $this->eventName . 'SetField', array(
                        'ENTITY' => $this,
                        'NAME' => $name,
                        'VALUE' => $value,
                        'OLD_VALUE' => $oldValue,
                ...
                if ($event->getResults()) {
                    ...
                    foreach ($event->getResults() as $eventResult) {
                        if ($eventResult->getType() == Main\EventResult::SUCCESS) {
                            ...
                            $errorMsg = new ResultError(
                                Main\Localization\Loc::getMessage(
                                    'SALE_EVENT_ON_BEFORE_' . mb_strtoupper($this->eventName) . '_SET_FIELDS_ERROR'
                                ), 'SALE_EVENT_ON_BEFORE_' . mb_strtoupper($this->eventName) . '_SET_FIELDS_ERROR'
                            );
