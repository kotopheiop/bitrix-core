...
class DiscountTable extends Main\Entity\DataManager
{
    ...
    public static function getMap()
    {
        return array(
            ...
            'DISCOUNT_TYPE' => new Main\Entity\StringField(
                'DISCOUNT_TYPE', array(
                'default_value' => 'P',
            ...
            'LAST_LEVEL_DISCOUNT' => new Main\Entity\BooleanField(
                'LAST_LEVEL_DISCOUNT', array(
                'values' => array('N', 'Y'),
                'default_value' => 'N',
            )
            ),
            'VERSION' => new Main\Entity\EnumField(
                'VERSION', array(
            ...
            'CONDITIONS_LIST' => new Main\Entity\TextField(
            ...
            'ACTIONS_LIST' => new Main\Entity\TextField(
                'ACTIONS_LIST', array(
                'serialized' => true,
                'column_name' => 'ACTIONS',
            ...
            'PREDICTIONS_LIST' => new Main\Entity\TextField(
            ...
            'EXECUTE_MODULE' => new Main\Entity\StringField(
            ...
            'EXECUTE_MODE' => new Main\Entity\IntegerField(
                'EXECUTE_MODE', array(
                'default_value' => self::EXECUTE_MODE_GENERAL,
            )
            ),
            'HAS_INDEX' => new Main\Entity\BooleanField(
                'HAS_INDEX', array(
                'values' => array('N', 'Y'),
                'default_value' => 'N',
            )
            ),
            'PRESET_ID' => new Main\Entity\StringField(
                'PRESET_ID', array(
                'validation' => array(__CLASS__, 'validatePresetId'),
            )
            ),
            'SHORT_DESCRIPTION_STRUCTURE' => new Main\Entity\TextField(
                'SHORT_DESCRIPTION_STRUCTURE', array(
                'serialized' => true,
                'column_name' => 'SHORT_DESCRIPTION',
            )
            ),
            'SHORT_DESCRIPTION' => new Main\Entity\ExpressionField(
                'SHORT_DESCRIPTION',
                '%s',
    ...
    public static function setAllUseCoupons($use)
    {
        ...
        $helper = $conn->getSqlHelper();
        $conn->queryExecute(
            'update ' . $helper->quote(self::getTableName()) . ' set ' . $helper->quote(
                'USE_COUPONS'
            ) . ' = \'' . $use . '\''
        );
        unset($helper, $conn);
    ...
    protected static function copyOldFields(&$result, $data)
    {
        if (!isset($data['CONDITIONS_LIST']) && isset($data['CONDITIONS'])) {
            $result['CONDITIONS_LIST'] = (is_array($data['CONDITIONS']) ? $data['CONDITIONS'] : unserialize(
                $data['CONDITIONS'],
        ...
        if (!isset($data['ACTIONS_LIST']) && isset($data['ACTIONS'])) {
            $result['ACTIONS_LIST'] = (is_array($data['ACTIONS']) ? $data['ACTIONS'] : unserialize(
                $data['ACTIONS'],
                ['allowed_classes' => false]
            ));
        }
