    ...
    public function __construct($siteId)
    {
        ...
        if (empty($settings[$siteId]["API"]["SITE_ID"])) {
            throw new SystemException(
                Loc::getMessage('SALE_EBAY_ENTITY_SETTINGS_EMPTY', array('#SITE_ID#' => $siteId))
            );
        }

        if (empty($settings[$siteId]["API"]["SITE_ID"])) {
            throw new ArgumentNullException(
                Loc::getMessage('SALE_EBAY_ENTITY_TOKEN_EMPTY', array('#SITE_ID#' => $siteId))
            );
        }

        $this->ebaySiteId = $settings[$siteId]["API"]["SITE_ID"];
        $this->authToken = $settings[$siteId]["API"]["AUTH_TOKEN"];
        $this->apiCaller = new Caller(
            array(
