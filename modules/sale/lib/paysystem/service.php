...
class Service
{
    ...
    public function processRequest(Request $request)
    {
        ...
        Logger::addDebugInfo(
            get_class($this->handler) . " ProcessRequest. paySystemId=" . $this->getField(
                "ID"
            ) . ", request=" . ($debugInfo ? $debugInfo : "empty")
        );
        ...
        if (empty($paymentId)) {
            ...
            Logger::addError(
                get_class($this->handler) . '. ProcessRequest: ' . Loc::getMessage(
        ...
        if ($serviceResult->isSuccess()) {
            ...
            if ($operationType == ServiceResult::MONEY_COMING) {
                ...
                if ($operationType == ServiceResult::MONEY_LEAVING) {
                    $status = 'N';
                }
            }
    ...
    public function confirm(Payment $payment, $sum = 0)
    {
        if ($this->isBlockable()) {
            if ($this->handler instanceof IPartialHold) {
                ...
                if ($sum > 0) {
                    throw new SystemException(
                        Loc::getMessage('SALE_PS_SERVICE_ERROR_PARTIAL_CONFIRM_IS_NOT_SUPPORTED')
                    );
                }
            }
    ...
    public function check(Payment $payment)
    {
        ...
        if ($this->isCheckable()) {
            ...
            if (!$order->isCanceled()) {
                ...
                $result->addError(
                    new EntityError(
                        Loc::getMessage('SALE_PS_SERVICE_ORDER_CANCELED', array('#ORDER_ID#' => $order->getId()))
                    )
                );
            }
        }
