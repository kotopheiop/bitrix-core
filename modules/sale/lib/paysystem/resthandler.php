...
class RestHandler extends PaySystem\ServiceHandler
{
    ...
    private function getDefaultTemplate(Payment $payment): string
    {
        ...
        $template = '<form action="' . htmlspecialcharsbx(
                $formSettings['ACTION_URI']
            ) . '" method="' . htmlspecialcharsbx($formSettings['METHOD']) . '" name="rest-handler-form">';
        ...
        $template .= '<input name="button" value="' . Loc::getMessage(
    ...
    private function getTemplateFromParams(Payment $payment, array $params): string
    {
        ...
        foreach ($params as $key => $value) {
            $template .= '<input type="hidden" name="' . htmlspecialcharsbx($key) . '" value="' . htmlspecialcharsbx(
                    $businessValueParams[$value]
                ) . '">';
        }
