...
final class Router
{
    ...
    protected function checkToken($accessToken, $refreshToken)
    {
        ...
        try {
            ...
            return $r->addError(
                new Error(Loc::getMessage("SALE_ROUTER_INTERNAL_SERVER_ERROR", ["#ERROR#" => $exception->getMessage()]))
            );
        }
        ...
        if (isset($response["error"])) {
            return $r->addError(
                new Error(
                    Loc::getMessage(
                        "SALE_ROUTER_INTERNAL_SERVER_ERROR_AUHORIZATION",
    ...
    protected function getBySourceEntity(
        $sourceEntityTypeId = '',
        $sourceEntityId = '',
    ...
    protected function getByDestinationEntity(
        $sourceEntityTypeId = '',
        $sourceEntityId = '',
        $destinationEntityTypeId = '',
        $destinationEntityId = ''
    ...
    {
        $request['PLACEMENT_OPTIONS'] = isset($request['PLACEMENT_OPTIONS']) ? json_decode(
            $request['PLACEMENT_OPTIONS'],
            true
        ) : [];
