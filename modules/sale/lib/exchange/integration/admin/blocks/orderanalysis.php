...
class OrderAnalysis extends \Bitrix\Sale\Helpers\Admin\Blocks\OrderAnalysis
{
    ...
    {
        return '<a class="adm-bus-orderdocs-threelist-block-responsible-name" href="/bitrix/admin/user_edit.php?ID=' . $data['RESPONSIBLE_ID'] . '" target="_blank">' . htmlspecialcharsbx(
                $data['RESPONSIBLE']
            ) . '</a>';
    }
    ...
    {
        return '<a class="fwb" href="' . htmlspecialcharsbx(
                $item['EDIT_PAGE_URL']
            ) . '" target="_blank">' . htmlspecialcharsEx($item['NAME']) . '</a>';
    }
}