...
class Automatic extends Base
{
        ...
        if (isset($this->config["MAIN"]["SID"]) && $this->config["MAIN"]["SID"] <> '') {
            ...
            if ($this->handlerInitParams == false) {
                throw new SystemException(
                    "Can't get delivery services init params. Delivery id: " . $this->id . ", sid: " . $this->sid
                );
            }
                ...
                "MAIN" => array(
                    ...
                    "ITEMS" => array(
                        "SID" => array(
                            ...
                            "OPTIONS" => $handlers,
                            "ONCHANGE" => "var data=" . \CUtil::PhpToJSObject(
                                    $jsData
    ...
    public function prepareFieldsForSaving(array $fields)
    {
        ...
        if (isset($this->handlerInitParams["DBSETSETTINGS"]) && is_callable(
                $this->handlerInitParams["DBSETSETTINGS"]
            )) {
    ...
    {
        if (!isset($oldConfig["CONFIG_GROUPS"]) || !is_array(
                $oldConfig["CONFIG_GROUPS"]
            ) || !isset($oldConfig["CONFIG"]) || !is_array($oldConfig["CONFIG"])) {
            return array();
        }
        ...
        foreach ($arPathList as $basePath) {
            if (file_exists($_SERVER["DOCUMENT_ROOT"] . $basePath) && is_dir($_SERVER["DOCUMENT_ROOT"] . $basePath)) {
                ...
                while (($filename = readdir($handle)) !== false) {
                    ...
                    if (!is_dir($_SERVER["DOCUMENT_ROOT"] . $basePath . "/" . $filename) && mb_substr(
                            $filename,
                            0,
                            9
                        ) == "delivery_") {
    ...
    public function calculateProfile($profileId, array $profileConfig, \Bitrix\Sale\Shipment $shipment)
    {
        static $result = array();
        $oldOrder = self::convertNewOrderToOld($shipment);
        $hitCacheId = $this->id . '_' . $profileId . '_' . md5(serialize($profileConfig)) . '_' . md5(
                serialize($oldOrder)
            );
    ...
    public function getAdminMessage()
    {
        ...
        if (isset($this->handlerInitParams["GET_ADMIN_MESSAGE"]) && is_callable(
                $this->handlerInitParams["GET_ADMIN_MESSAGE"]
            )) {
    ...
    public function execAdminAction()
    {
        ...
        if (isset($this->handlerInitParams["EXEC_ADMIN_ACTION"]) && is_callable(
                $this->handlerInitParams["EXEC_ADMIN_ACTION"]
            )) {
    ...
    public function getAdditionalInfoShipmentEdit(Shipment $shipment)
    {
        ...
        if (isset($this->handlerInitParams["GET_ADD_INFO_SHIPMENT_EDIT"]) && is_callable(
    ...
    public function processAdditionalInfoShipmentEdit(Shipment $shipment, array $requestData)
    {
        ...
        if (isset($this->handlerInitParams["PROCESS_ADD_INFO_SHIPMENT_EDIT"]) && is_callable(
    ...
    public function getAdditionalInfoShipmentView(Shipment $shipment)
    {
        ...
        if (isset($this->handlerInitParams["GET_ADD_INFO_SHIPMENT_VIEW"]) && is_callable(
