...
abstract class OrderBase extends Internals\Entity
{
     ...
     *
     */
    public function onPropertyValueCollectionModify(
        $action,
    ...
    public function refreshData()
    {
        ...
        $this->calculateType = ($this->getId(
        ) > 0 ? static::SALE_ORDER_CALC_TYPE_REFRESH : static::SALE_ORDER_CALC_TYPE_NEW);
        ...
        if (!empty($oldEntityValues)) {
            ...
            if ($eventsList = $eventManager->findEventHandlers('sale', 'OnSaleOrderEntitySaved')) {
                ...
                $event = new Main\Event(
                    'sale', 'OnSaleOrderEntitySaved', array(
                    'ENTITY' => $this,
    ...
    protected function callEventOnSaleOrderSaved()
    {
        ...
        if ($eventsList = $eventManager->findEventHandlers('sale', EventActions::EVENT_ON_ORDER_SAVED)) {
            $event = new Main\Event(
                'sale', EventActions::EVENT_ON_ORDER_SAVED, array(
                'ENTITY' => $this,
                'IS_NEW' => $this->isNew,
    ...
    protected function callEventOnBeforeOrderSaved()
    {
        ...
        if ($eventsList = $eventManager->findEventHandlers('sale', EventActions::EVENT_ON_ORDER_BEFORE_SAVED)) {
            ...
            $event = new Main\Event(
                'sale', EventActions::EVENT_ON_ORDER_BEFORE_SAVED, array(
                'ENTITY' => $this,
                'VALUES' => $oldEntityValues
            )
            );
    ...
    protected function onFieldModify($name, $oldValue, $value)
    {
        ...
        if ($name == "PRICE") {
            ...
            $event = new Main\Event(
                'sale', EventActions::EVENT_ON_BEFORE_ORDER_CANCELED, array(
                'ENTITY' => $this
            )
            );
            $event->send();
            ...
            $event = new Main\Event(
                'sale', EventActions::EVENT_ON_BEFORE_ORDER_STATUS_CHANGE, array(
                'ENTITY' => $this,
                'VALUE' => $value,
                'OLD_VALUE' => $oldValue,
            )
            ...
            Internals\EventsPool::addEvent(
                $this->getInternalId(),
                EventActions::EVENT_ON_ORDER_STATUS_CHANGE,
                array(
                    'ENTITY' => $this,
                    'VALUE' => $value,
                    'OLD_VALUE' => $oldValue,
            ...
            Internals\EventsPool::addEvent(
                $this->getInternalId(),
                EventActions::EVENT_ON_ORDER_STATUS_CHANGE_SEND_MAIL,
                array(
                    'ENTITY' => $this,
                    'VALUE' => $value,
                    'OLD_VALUE' => $oldValue,
            ...
            if ($this->isStatusChangedOnPay($value, $oldValue)) {
                Internals\EventsPool::addEvent(
                    $this->getInternalId(),
                    EventActions::EVENT_ON_ORDER_STATUS_ALLOW_PAY_CHANGE,
                    array(
                        'ENTITY' => $this,
                        'VALUE' => $value,
                        'OLD_VALUE' => $oldValue,
    ...
    public function doFinalAction($hasMeaningfulField = false)
    {
        ...
        if ($basket) {
            ...
            if (!empty($eventsList)) {
                $event = new Main\Event(
                    'sale', 'OnBeforeSaleOrderFinalAction', array(
                    'ENTITY' => $this,
...
//				return $result;
            ...
            if ($r->isSuccess() && ($discountData = $r->getData()) && !empty($discountData) && is_array(
                    $discountData
                )) {
                /** @var Result $r */
