...
class CAllSaleTaxRate
{
    ...
    public static function CheckFields($ACTION, &$arFields)
    {
        ...
        if ($arFields["IS_PERCENT"] == "Y") {
            ...
            return false;
        } elseif (!is_set($arFields, "IS_PERCENT") && (is_set(
                    $arFields,
                    "CURRENCY"
                ) && $arFields["CURRENCY"] !== false)) {
            return false;
        ...
        if (is_set($arFields, "TAX_ID")) {
            if (!($arTax = CSaleTax::GetByID($arFields["TAX_ID"]))) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["TAX_ID"], GetMessage("SKGTR_NO_TAX")),
                    "ERROR_NO_TAX"
                );
                return false;
            }
        ...
        if (is_set($arFields, "PERSON_TYPE_ID") && $arFields["PERSON_TYPE_ID"] !== false) {
            if (!\Bitrix\Sale\Internals\PersonTypeTable::getRowById($arFields["PERSON_TYPE_ID"])) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["PERSON_TYPE_ID"], GetMessage("SKGTR_NO_PERS_TYPE")),
    ...
    public static function Update($ID, $arFields, $arOptions = array())
    {
        ...
        $strSql = "UPDATE b_sale_tax_rate SET TIMESTAMP_X = " . $DB->GetNowFunction(
    ...
    public static function GetByID($ID)
    {
        ...
        $strSql =
            "SELECT ID, TAX_ID, PERSON_TYPE_ID, VALUE, CURRENCY, IS_PERCENT, IS_IN_PRICE, APPLY_ORDER, " . $DB->DateToCharFunction(
                "TIMESTAMP_X",
                "FULL"
