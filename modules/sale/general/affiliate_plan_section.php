...
class CAllSaleAffiliatePlanSection
{
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
            ...
            if ($ID <= 0) {
                ...
                if (!$arPlanSection) {
                    $GLOBALS["APPLICATION"]->ThrowException(
                        str_replace("#ID#", $ID, GetMessage("SCGAPS1_NO_RECORD")),
                        "NO_PLAN_SECTION"
                    );
                    return false;
                }
        ...
        if ($ACTION == "ADD") {
            ...
            if ($arFields["RATE_TYPE"] == "F" && (!is_set(
                        $arFields,
                        "RATE_CURRENCY"
    ...
    public static function DeleteByPlan($planID, $arSectionIDs)
    {
        ...
        return $DB->Query(
            "DELETE FROM b_sale_affiliate_plan_section WHERE PLAN_ID = " . $planID . " AND ID NOT IN (" . $strSectionIDs . ")",
            true
        );
    }
    ...
    public static function GetByID($ID)
    {
        ...
        if (isset($GLOBALS["SALE_AFFILIATE_PLAN_SECTION"]["SALE_AFFILIATE_PLAN_SECTION_CACHE_" . $ID]) && is_array(
                $GLOBALS["SALE_AFFILIATE_PLAN_SECTION"]["SALE_AFFILIATE_PLAN_SECTION_CACHE_" . $ID]
            )) {
