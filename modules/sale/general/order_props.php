...
class CSaleOrderProps
{
    ...
    public static function DoProcessOrder(
        ...
        if ($deliveryId <> '') {
            ...
            $relationFilter[] = [
                ...
                '=Bitrix\Sale\Internals\OrderPropsRelationTable:lPROPERTY.ENTITY_ID' => \CSaleDelivery::getIdByCode(
                    $deliveryId
                ),
            ];
        }
        ...
        while ($arOrderProp = $dbRes->fetch()) {
                ...
                if ($arOrderProp["IS_EMAIL"] == "Y") {
                    ...
                    if (!check_email($curVal)) {
                        $arWarnings[] = array(
                            ...
                            "TEXT" => str_replace(
                                array("#EMAIL#", "#NAME#"),
                                array(htmlspecialcharsbx($curVal), htmlspecialcharsbx($arOrderProp["NAME"])),
                                GetMessage("SALE_GOPE_WRONG_EMAIL")
                            )
                        );
                    }
                }
            ...
            if ($bErrorField) {
                $arWarnings[] = array(
                    ...
                    "TEXT" => str_replace(
                        "#NAME#",
                        htmlspecialcharsbx($arOrderProp["NAME"]),
                        GetMessage("SALE_GOPE_FIELD_EMPTY")
                    )
                );
                $bErrorField = false;
            }
     ...
     * @param array $arErrors
     */
    public static function DoSaveOrderProps(
        $orderId,
        $personTypeId,
        $arOrderProps,
        &$arErrors,
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if (is_set($arFields, "IS_LOCATION") && is_set(
                $arFields,
                "TYPE"
            ) && $arFields["IS_LOCATION"] == "Y" && $arFields["TYPE"] != "LOCATION") {
            $APPLICATION->ThrowException(Loc::getMessage("SKGOP_WRONG_PROP_TYPE"), "ERROR_WRONG_TYPE1");
            return false;
        }
        if (is_set($arFields, "IS_LOCATION4TAX") && is_set(
                $arFields,
                "TYPE"
            ) && $arFields["IS_LOCATION4TAX"] == "Y" && $arFields["TYPE"] != "LOCATION") {
        ...
        if (is_set($arFields, "PERSON_TYPE_ID")) {
            if (!($arPersonType = CSalePersonType::GetByID($arFields["PERSON_TYPE_ID"]))) {
                $APPLICATION->ThrowException(
                    str_replace("#ID#", $arFields["PERSON_TYPE_ID"], Loc::getMessage("SKGOP_NO_PERS_TYPE")),
    ...
    public static function UpdateOrderPropsRelations($ID, $arEntityIDs, $entityType)
    {
        ...
        $DB->Query(
            "DELETE FROM b_sale_order_props_relation WHERE PROPERTY_ID = '" . $DB->ForSql(
                $ID
...
final class CSaleOrderPropsAdapter implements FetchAdapter
{
    ...
    function __construct(OrderQuery $query, array $select)
    {
        ...
        if (!$query->aggregated()) {
            ...
            $query->registerRuntimeField(
                'PROPERTY_ID',
                new Entity\ExpressionField('PROPERTY_ID', 'DISTINCT(%s)', 'ID')
            );
            $sel = $query->getSelect();
            ...
            case 'LOCATION':
                ...
                if (intval($property['DEFAULT_VALUE'])) {
                    $res = \Bitrix\Sale\Location\LocationTable::getList(
                        array('filter' => array('=ID' => intval($property['DEFAULT_VALUE'])), 'select' => array('CODE'))
                    )->fetch();
