...
class CAllSaleUserCards
{
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if ((is_set($arFields, "PAY_SYSTEM_ACTION_ID") || $ACTION == "ADD") && intval(
                $arFields["PAY_SYSTEM_ACTION_ID"]
            ) <= 0) {
        ...
        if ((is_set($arFields, "CARD_EXP_MONTH") || $ACTION == "ADD") && (intval(
                    $arFields["CARD_EXP_MONTH"]
                ) <= 0 || intval($arFields["CARD_EXP_MONTH"]) > 12)) {
            $GLOBALS["APPLICATION"]->ThrowException("Empty card expiration month field", "EMPTY_CARD_EXP_MONTH");
            return false;
        }
        if ((is_set($arFields, "CARD_EXP_YEAR") || $ACTION == "ADD") && (intval(
                    $arFields["CARD_EXP_YEAR"]
                ) <= 2000 || intval($arFields["CARD_EXP_YEAR"]) > 2100)) {
        ...
        if (is_set($arFields, "USER_ID")) {
            ...
            if (!$dbUser->Fetch()) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["USER_ID"], GetMessage("SKGUC_NO_USER")),
                    "ERROR_NO_USER_ID"
                );
                return false;
            }
        ...
        if (is_set($arFields, "PAY_SYSTEM_ACTION_ID")) {
            if (!($arPaySysAction = CSalePaySystemAction::GetByID($arFields["PAY_SYSTEM_ACTION_ID"]))) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["PAY_SYSTEM_ACTION_ID"], GetMessage("SKGUC_NO_PS")),
    ...
    public static function WithdrawByID($sum, $currency, $ID, $orderID = 0)
    {
        ...
        if (!$arUserCard) {
            $GLOBALS["APPLICATION"]->ThrowException(
                str_replace("#ID#", $ID, GetMessage("SKGUC_NO_RECID")),
                "NO_RECORD"
            );
            return false;
        }
    ...
    public static function Withdraw($sum, $currency, $arUserCard, $orderID = 0)
    {
        ...
        if (DoubleVal($arUserCard["SUM_MAX"]) > 0) {
            $maxSum = roundEx(
                CCurrencyRates::ConvertCurrency($arUserCard["SUM_MAX"], $arUserCard["SUM_CURRENCY"], $currency),
            ...
            if ($maxSum < $sum) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace(
                        "#SUM1#",
                        SaleFormatCurrency($arUserCard["SUM_MAX"], $arUserCard["SUM_CURRENCY"]),
                        str_replace("#SUM2#", SaleFormatCurrency($sum, $currency), GetMessage("SKGUC_CROSS_BOUND"))
                    ),
                    "MAX_SUM_LIMIT"
                );
                return false;
        ...
        if (!$arPSAction) {
            $GLOBALS["APPLICATION"]->ThrowException(
                str_replace("#ID#", $arUserCard["PAY_SYSTEM_ACTION_ID"], GetMessage("SKGUC_NO_ACTION")),
        ...
        if (!file_exists($psActionPath)) {
            $GLOBALS["APPLICATION"]->ThrowException(
                str_replace("#FILE#", $arPSAction["ACTION_FILE"], GetMessage("SKGUC_NO_PATH")),
                "NO_PS_PATH"
            );
            return false;
        }
        ...
        if (!file_exists($psActionPath . "/action.php")) {
            $GLOBALS["APPLICATION"]->ThrowException(
                str_replace("#FILE#", $psActionPath . "/action.php", GetMessage("SKGUC_NO_SCRIPT")),
                "NO_PS_SCRIPT"
            );
            return false;
        }
        ...
        if (DoubleVal($arUserCard["SUM_MIN"]) > 0) {
            $minSum = roundEx(
                CCurrencyRates::ConvertCurrency($arUserCard["SUM_MIN"], $arUserCard["SUM_CURRENCY"], $currency),
                SALE_VALUE_PRECISION
            );
            if ($minSum > $sum) {
                $INPUT_SUM = $minSum;
        ...
        if ($orderID > 0) {
            if ($arOrder = CSaleOrder::GetByID($orderID)) {
                ...
                $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["SHOULD_PAY"] = DoubleVal($arOrder["PRICE"]) - DoubleVal(
                        $arOrder["SUM_PAID"]
                    );
        ...
        if ($OUTPUT_STATUS == "Y") {
            ...
            if ($OUTPUT_CURRENCY != $currency) {
                $OUTPUT_SUM = roundEx(
                    CCurrencyRates::ConvertCurrency($OUTPUT_SUM, $OUTPUT_CURRENCY, $currency),
                    SALE_VALUE_PRECISION
                );
            }
