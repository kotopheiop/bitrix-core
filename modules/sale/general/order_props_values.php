...
class CAllSaleOrderPropsValue
{
    ...
    public static function GetOrderRelatedProps($ORDER_ID, $arFilter = array())
    {
        ...
        return self::GetList(
            ...
            array(
                'ORDER_ID' => $ORDER_ID,
                'PAYSYSTEM_ID' => $arFilter['PAYSYSTEM_ID'],
                'DELIVERY_ID' => $arFilter['DELIVERY_ID']
            ),
            false,
            false,
            array(
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if (is_set($arFields, "ORDER_ID")) {
            if (!($arOrder = CSaleOrder::GetByID($arFields["ORDER_ID"]))) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["ORDER_ID"], GetMessage("SKGOPV_NO_ORDER_ID")),
                    "ERROR_NO_ORDER"
                );
                return false;
            }
        ...
        if (is_set($arFields, "ORDER_PROPS_ID")) {
            if (!($arOrder = CSaleOrderProps::GetByID($arFields["ORDER_PROPS_ID"]))) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["ORDER_PROPS_ID"], GetMessage("SKGOPV_NO_PROP_ID")),
                    "ERROR_NO_PROPERY"
                );
                return false;
            }
            ...
            if (is_set($arFields, "ORDER_ID")) {
                ...
                if ($arP = $dbP->Fetch()) {
                    $GLOBALS["APPLICATION"]->ThrowException(
                        GetMessage(
                            "SKGOPV_DUPLICATE_PROP_ID",
                            Array("#ID#" => $arFields["ORDER_PROPS_ID"], "#ORDER_ID#" => $arFields["ORDER_ID"])
                        ),
...
//		}
        ...
        if (!empty($arFields['ORDER_PROPS_ID']) && intval(
                $arFields['ORDER_PROPS_ID']
            ) > 0 && !empty($arFields['VALUE'])) {
            if ($value = self::correctValueToMultiple($arFields['ORDER_PROPS_ID'], $arFields['VALUE'])) {
                $arFields['VALUE'] = $value;
            }
        }

        return Internals\OrderPropsValueTable::add(
            array_intersect_key($arFields, CSaleOrderPropsValueAdapter::$allFields)
        )->getId();
    }
...
//		}
        ...
        return Internals\OrderPropsValueTable::update(
            $ID,
            array_intersect_key(
                $arFields,
                CSaleOrderPropsValueAdapter::$allFields
            )
        )->getId();
    }
    ...
    private static function correctValueToMultiple($id, $value)
    {
        ...
        $res = Internals\OrderPropsTable::getList(
            array(
