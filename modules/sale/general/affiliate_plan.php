...
class CAllSaleAffiliatePlan
{
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
            ...
            if ($ID <= 0) {
                ...
                if (!$arPlan) {
                    $GLOBALS["APPLICATION"]->ThrowException(
                        str_replace("#ID#", $ID, GetMessage("SCGAP1_NO_PLAN")),
                        "NO_PLAN"
                    );
                    return false;
                }
        ...
        if ($ACTION == "ADD") {
            ...
            if ($arFields["BASE_RATE_TYPE"] == "F" && (!is_set(
                        $arFields,
                        "BASE_RATE_CURRENCY"
                    ) || $arFields["BASE_RATE_CURRENCY"] == '')) {
    ...
    public static function Delete($ID)
    {
        ...
        if (intval($cnt) > 0) {
            $GLOBALS["APPLICATION"]->ThrowException(
                str_replace("#ID#", $ID, GetMessage("SCGAP1_AFF_EXISTS")),
                "NOT_EMPTY_PLAN"
            );
            return false;
        }
    ...
    public static function GetByID($ID)
    {
        ...
        if (isset($GLOBALS["SALE_AFFILIATE_PLAN"]["SALE_AFFILIATE_PLAN_CACHE_" . $ID]) && is_array(
                $GLOBALS["SALE_AFFILIATE_PLAN"]["SALE_AFFILIATE_PLAN_CACHE_" . $ID]
            )) {
    ...
    public static function CheckAffiliatePlanFunc($affiliatePlan)
    {
        if (is_array($affiliatePlan)) {
            ...
            if (!$arAffiliatePlan) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $affiliatePlanID, GetMessage("SCGAP1_NO_PLAN")),
                    "NO_AFFILIATE_PLAN"
                );
                return false;
            }
