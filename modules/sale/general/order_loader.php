...
class CSaleOrderLoader
{
    ...
    public function setMarkedOrder(\Bitrix\Sale\Order $order)
    {
        ...
        $newReasonMarked = ($oldReasonMarked == '' ? '' : $oldReasonMarked . $this->delimiter_reason) . GetMessage(
                "CC_BSC1_ERROR_EXCHANGE_1C_ORDER_DEDUCTED"
            ) . $this->marked_code;
            ...
            if (
                ...
                $date = new Bitrix\Main\Type\Date(
                    CDatabase::FormatDate(
                        str_replace("T", " ", $arDocument["TRAITS"][GetMessage("CC_BSC1_1C_PAYED_DATE")]),
                        "YYYY-MM-DD HH:MI:SS",
    ...
    {
        if ($arDocument["VERSION_1C"] != $order->getField('VERSION_1C') || ($order->getField(
                    'VERSION_1C'
                ) == '' || $arDocument["VERSION_1C"] == '')) {
            if ($arDocument["TRAITS"][GetMessage("CC_BSC1_1C_PAYED_DATE")] <> '') {
                if (
                    ($paymentCompatible1CId = $this->getPaymentCompatible1CByOrder($order))
    ...
    {
        return $_SERVER["DOCUMENT_ROOT"] . "/" . COption::GetOptionString(
                "main",
                "upload_dir",
                "upload"
            ) . "/1c_exchange/";
        ...
        if (!$order instanceof Bitrix\Sale\Order) {
            return new Bitrix\Main\EventResult(
                ...
                new Bitrix\Sale\ResultError(
                    Bitrix\Main\Localization\Loc::getMessage('SALE_EVENT_COMPATIBILITY_ORDER_SAVE_WRONG_ORDER'),
    ...
    function getDocumentId($type, $code, $xmlId1CDocument)
    {
        ...
        if (($code == '' || $result['ID'] === false) && $xmlId1CDocument <> '')//try to search document from 1C // !==false fix for accountNumberPrefix
        {
            switch ($type) {
                case 'Shipment':
                    if ($shimpentCode1C = \Bitrix\Sale\Internals\ShipmentTable::getList(
                        array(
                    ...
                    break;
                case 'Payment':
                    if ($paymentCode1C = \Bitrix\Sale\Internals\PaymentTable::getList(
                        array(
                            'select' => array('ID', 'ID_1C', 'ORDER_ID'),
                            'filter' => array('ID_1C' => $xmlId1CDocument),
                            'order' => array('ID' => 'DESC')
                    ...
                    break;
                case 'Order':
                    if ($OrderCode1C = \Bitrix\Sale\Internals\OrderTable::getList(
                        array(
        ...
        if ($orderCode <> '') {
            if (is_numeric($orderCode)) {
                if (!$orderByID = \Bitrix\Sale\Internals\OrderTable::getById($orderCode)->fetch()) {
                    if (!$orderBy1CID = \Bitrix\Sale\Internals\OrderTable::getList(
                        array(
                            'select' => array('ID'),
                        ...
                        if (!$orderByAccountNumber = \Bitrix\Sale\Internals\OrderTable::getList(
                            array(
                                'select' => array('ID'),
    ...
    function updateOrderWithoutShipmentsPayments(array $arOrder)
    {
        ...
        $key = $eventManager->addEventHandler(
            'sale',
            'OnSaleOrderBeforeSaved',
            Array($this, "onBeforeUpdateOrderWithoutShipmentsPayments")
        );
        ...
        if ($orderInfo = $dbOrder->Fetch()) {
            if ($arOrder["VERSION_1C"] != $orderInfo["VERSION_1C"] || ($orderInfo["VERSION_1C"] == '' || $arOrder["VERSION_1C"] == '')) // skip update if the same version
            {
                ...
                if (
                    ...
                    if ($arOrderTax = $dbOrderTax->Fetch()) {
                        $bTaxFound = true;
                        if (intval($arOrderTax["VALUE_MONEY"]) != intval($arOrder["TAX"]["VALUE_MONEY"]) || intval(
                                $arOrderTax["VALUE"]
                            ) != intval(
                                $arOrder["TAX"]["VALUE"]
                            ) || ($arOrderTax["IS_IN_PRICE"] != $arOrder["TAX"]["IS_IN_PRICE"])) {
                    ...
                    foreach ($basket as $basketItem) {
                        ...
                        foreach ($arOrder["items"] as $k => $items) {
                            if ($this->existsBasketItem($basketItem, $items)) {
                                ...
                                if ($basketItem->getField("QUANTITY") != $items[$basketItem->getField(
                                        "PRODUCT_XML_ID"
                                    )]["QUANTITY"]) {
                                    $arFields["QUANTITY"] = $items[$basketItem->getField("PRODUCT_XML_ID")]["QUANTITY"];
                                }
                                if ($basketItem->getField("PRICE") != $items[$basketItem->getField(
                                        "PRODUCT_XML_ID"
                                    )]["PRICE"]) {
                                    $arFields["PRICE"] = $items[$basketItem->getField("PRODUCT_XML_ID")]["PRICE"];
                                }
                                if ($basketItem->getField("VAT_RATE") != $items[$basketItem->getField(
                                        "PRODUCT_XML_ID"
                                    )]["VAT_RATE"]) {
                                ...
                                $arFields["DISCOUNT_PRICE"] = $items[$basketItem->getField(
                                    "PRODUCT_XML_ID"
                                )]["DISCOUNT_PRICE"];
                                ...
                                if (count($arFields) > 0) {
                                    ...
                                    $arShoppingCart[] = array(
                                        ...
                                        "PROPS" => $this->getBasketProperty(
                                            $items[$basketItem->getField("PRODUCT_XML_ID")]
                                        )
                                    );
                                }
                        ...
                        if (!$foundedBasketItem) {
                            if ($arOrder["CANCELED"] != "true" && $arOrder["TRAITS"][GetMessage(
                                    "CC_BSC1_CANCEL"
                                )] != "true" && $orderInfo["CANCELED"] == "N") {
                                $bNeedUpdate = true;
                            }
                        }
                    }
                            ...
                            foreach ($items as $itemID => $item) {
                                if ($item["CHECKED"] != "Y") {
                                    if ($item["TYPE"] == GetMessage("CC_BSC1_ITEM")) {
                                        if ($arBasketFields = $this->prepareProduct4Basket(
                                            $itemID,
                                            $item,
                                            $orderId,
                                            $orderInfo
                    ...
                    if ($bNeedUpdate) {
                        $arErrors = array();
                        if (!CSaleBasket::DoSaveOrderBasket(
                            $orderId,
                            $orderInfo["LID"],
                            $orderInfo["USER_ID"],
                            $arShoppingCart,
            ...
            if (\Bitrix\Main\Config\Option::get(
                    ...
                    ""
                ) && $arOrder["TRAITS"][GetMessage("CC_BSC1_1C_STATUS_ID")] <> '') {
            ...
            $this->strErrorDocument .= "\n" . GetMessage(
                    "CC_BSC1_ORDER_ERROR_4",
                    Array('#XML_1C_DOCUMENT_ID#' => $this->getXMLDocumentID())
            ...
            if ($this->documentMustHaveProducts($arDocument)) {
                ...
                if (intval($arDocument["PERSON_TYPE_ID"]) > 0) {
                    ...
                    if (intval($arDocument["USER_ID"]) > 0) {
                        ...
                        $arAditFields["DATE_INSERT"] = CDatabase::FormatDate(
                            $arDocument["DATE"] . " " . $arDocument["TIME"],
                            "YYYY-MM-DD HH:MI:SS",
                            CLang::GetDateFormat("FULL", LANG)
                        );
                        ...
                        foreach ($arDocument["items"] as $k => $items) {
                            foreach ($items as $productID => $item) {
                                if ($item['TYPE'] <> GetMessage("CC_BSC1_SERVICE")) {
                                    $orderFields["BASKET_ITEMS"][$k] = $this->prepareProduct4Basket(
                                        $productID,
                                        $item,
                                        false,
                                        $orderFields
                        ...
                        $this->strErrorDocument .= "\n" . GetMessage(
                            ...
                            ) . GetMessage("CC_BSC1_ORDER_USER_PROBLEM", Array("#ID#" => $arDocument["ID_1C"]));
                        if (!empty($arErrors)) {
                    ...
                    $this->strErrorDocument .= "\n" . GetMessage(
                        ...
                        ) . GetMessage("CC_BSC1_ORDER_PERSON_TYPE_PROBLEM", Array("#ID#" => $arDocument["ID_1C"]));
                }
            } else {
    ...
    function Paid(\Bitrix\Sale\Payment $payment, $arDocument)
    {
        ...
        if (intval($payment->getId()) > 0) {
            if ($arDocument["TRAITS"][GetMessage("CC_BSC1_1C_PAYED")] == "true") {
                if ($arDocument["CANCELED"] !== "true" && $arDocument["TRAITS"][GetMessage(
                        "CC_BSC1_CANCEL"
                    )] !== "true") {
                    if (!$payment->isPaid()) {
                        ...
                        if (!$r->isSuccess()) {
                            $this->strErrorDocument .= "\n" . GetMessage(
                                    "CC_BSC1_ORDER_ERROR_4",
                                    Array('#XML_1C_DOCUMENT_ID#' => $arDocument['XML_1C_DOCUMENT_ID'])
    ...
    function Ship(\Bitrix\Sale\Shipment $shipment, $arDocument)
    {
        ...
        if (mb_strlen($arDocument["DATE"]) > 1) {
            ...
            if (isset($arDocument["TRAITS"][GetMessage("CC_BSC1_1C_TRACKING_NUMBER")]) && strval(
                    $arDocument["TRAITS"][GetMessage("CC_BSC1_1C_TRACKING_NUMBER")]
                ) != '') {
    ...
    function saveOrder($arDocument, $orderFields, $arAditFields)
    {
        ...
        if ($arOrder["ID"] = CSaleOrder::DoSaveOrder($orderFields, $arAditFields, 0, $arErrors)) {
            ...
            if (intval($arOrder["USER_PROFILE_ID"]) > 0) {
                ...
                $dbUPV = CSaleOrderUserPropsValue::GetList(
                    array(),
                    array("USER_PROPS_ID" => $arOrder["USER_PROFILE_ID"])
                );
                while ($arUPV = $dbUPV->Fetch()) {
                    $arOrder["AGENT"]["PROFILE_PROPS_VALUE"][$arUPV["ORDER_PROPS_ID"]] = array(
                        "ID" => $arUPV["ID"],
                        "VALUE" => $arUPV["VALUE"]
                    );
                }
            ...
            if (intval($arOrder["USER_PROFILE_ID"]) <= 0 || (intval(
                        $arOrder["USER_PROFILE_ID"]
                    ) > 0 && $arOrder["USER_PROFILE_VERSION"] != $arOrder["AGENT"]["VERSION"])) {
                        ...
                        if (intval($arOrder["USER_PROFILE_ID"]) > 0) {
                            ...
                            if (empty($arOrder["AGENT"]["PROFILE_PROPS_VALUE"][$arOrderProperties["ID"]])) {
                                ...
                                CSaleOrderUserPropsValue::Update(
                                    $arOrder["AGENT"]["PROFILE_PROPS_VALUE"][$arOrderProperties["ID"]]["ID"],
                                    $arFields
                                );
                            }
                        }
            ...
            $this->strErrorDocument .= "\n" . GetMessage(
                    "CC_BSC1_ORDER_ADD_PROBLEM",
                    Array("#ID#" => $arDocument["XML_1C_DOCUMENT_ID"])
                );
    ...
    function addProduct($arDocument, $documentBasketItems, $documentShipmentItems, \Bitrix\Sale\Shipment $shipment)
    {
        ...
        foreach ($documentBasketItems as $item) {
            foreach ($item as $itemID => $arItem) {
                $arBasketFields = $this->prepareProduct4Basket(
                    $itemID,
                    $arItem,
                    $order->getId(),
                    array(
    ...
    function updatePaymentFromDocument(array $arDocument, \Bitrix\Sale\Payment $payment)
    {
        ...
        if ($arDocument["VERSION_1C"] != $payment->getField('VERSION_1C') || ($payment->getField(
                    'VERSION_1C'
                ) == '' || $arDocument["VERSION_1C"] == '')) // skip update if the same version
        {
    ...
    function addPaymentFromDocumentByOrder(array $arDocument, \Bitrix\Sale\Order $order)
    {
        ...
        if ($paySystem = \Bitrix\Sale\PaySystem\Manager::getObjectById($paymentServiceId)) {
            ...
            $this->strErrorDocument .= "\n " . GetMessage(
                    "CC_BSC1_PAYMENT_ERROR_7",
                    Array('#XML_1C_DOCUMENT_ID#' => $this->getXMLDocumentID())
                ) . GetMessage("CC_BSC1_ORDER_NOT_FOUND");
        }
    ...
    function addOrderWithoutShipmentsPayments(array $arDocument, $addOrderField = array())
    {
        ...
        $key = $eventManager->addEventHandler(
            'sale',
            'OnSaleOrderBeforeSaved',
            Array($this, "onBeforeSaveOrderWithoutShipmentsPayments")
        );
                ...
                foreach ($arDocument['items'] as $k => $item) {
                    if (
                        ...
                        if ($externalQuantity < $shipmentItemQuantity) {
                            ...
                            } elseif (\Bitrix\Main\Config\Option::get(
                                    "sale",
                                    "1C_IMPORT_UPDATE_BASKET_QUANTITY",
                                    'Y'
                                ) == 'Y') {
                                $this->updateBasketNeedExternalQuantity(
                                    $externalQuantity,
            ...
            foreach ($basket as $basketItem) {
                foreach ($arDocument['items'] as $item) {
                    if (!$this->existsBasketItem($basketItem, $item)) {
                        $shipmentItems = $shipment->getShipmentItemCollection()->getItemByBasketCode(
                            $basketItem->getId()
                        );
                        if (!is_null($shipmentItems)) {
                            $shipmentItems->delete();
                            $this->logMessage(
                                "BasketItem: " . "[" . $shipmentItems->getId() . "]" . $shipmentItems->getField('NAME')
                            );
                        }
                    }
                }
    ...
    function updateShipmentNeedQuantity(
        $externalQuantity,
        ...
        if (empty($availableQuantityByProducts['SYSTEM'][$basketItem->getField('PRODUCT_XML_ID')])) {
            ...
            $systemShipmentQuantity = intval(
                $availableQuantityByProducts['SYSTEM'][$basketItem->getField('PRODUCT_XML_ID')]
            );
        }
        ...
        if ($systemShipmentQuantity >= $needQuantity) {
            ...
            $this->updateShipmentNeedQuantityFromBasketItems(
                $needQuantityWithoutSystem,
                $needQuantity,
                $basketItem,
                $shipment
            );
    ...
    function updateShipmentNeedQuantityFromBasketItems(
        $needQuantityWithoutSystem,
        $needQuantity,
        \Bitrix\Sale\BasketItem $basketItem,
        \Bitrix\Sale\Shipment $shipment
        ...
        foreach ($order->getShipmentCollection() as $shipmentAny) {
            ...
            $shipmentItemAny = $shipmentAny->getShipmentItemCollection()->getItemByBasketCode(
    ...
    function updateShipmentItemQuantity(
        $needQuantity,
        \Bitrix\Sale\BasketItem $basketItem,
        \Bitrix\Sale\Shipment $shipment
    ) {
    ...
    function setShipmentItemQuantity(
        $needQuantity,
        \Bitrix\Sale\BasketItem $basketItem,
        \Bitrix\Sale\Shipment $shipment
    ) {
    ...
    function updateShipmentNeedQuantityFromSystem(
        $Quantity,
        \Bitrix\Sale\BasketItem $basketItem,
        \Bitrix\Sale\Shipment $shipment
    ) {
    ...
    function updateBasketNeedExternalQuantity(
        $externalQuantity,
        $availableQuantityByProducts,
        \Bitrix\Sale\BasketItem $basketItem,
        \Bitrix\Sale\Shipment $shipment
    ...
    {
        if ($this->getVersion1C() != $shipment->getField('VERSION_1C') || ($shipment->getField(
                    'VERSION_1C'
                ) == '' || $this->getVersion1C() == '')) {
            return true;
        } else {
    ...
    function setExportInfo()
    {
        if (empty($this->arExportInfo)) {
            ...
            while ($arExport = $dbExport->Fetch()) {
                $this->arExportInfo[$arExport["PERSON_TYPE_ID"]] = unserialize(
                    $arExport["VARS"],
        ...
        return doubleval(
            str_replace(
                ...
                str_replace(
                    ",",
                    ".",
                    (!empty($_SESSION['versionSchema']) ? $_SESSION['versionSchema'] : self::DEFUALT_VERSION)
                )
            )
        );
    }
    ...
    function getAvailableQuentityProduct(\Bitrix\Sale\BasketItem $basketItem, \Bitrix\Sale\Shipment $shipment)
    {
        ...
        foreach ($order->getShipmentCollection() as $shipment) {
            if ($this->checkConditionForShipmentByBasket($basketItem, $shipment)) {
                $sipmentItem = $shipment->getShipmentItemCollection()->getItemByBasketCode(
                    $basketItem->getBasketCode()
                );
                if ($sipmentItem !== null) {
    ...
    function getUserByProperty($arOrder)
    {
        if (!empty($arOrder["AGENT"]) && $arOrder["AGENT"]["ID"] <> '') {
            ...
            $arOrder["USER_ID"] = 0;
            $arErrors = array();
            $dbUProp = CSaleOrderUserProps::GetList(
                array(),
                array("XML_ID" => $arOrder["AGENT"]["ID"]),
                false,
                false,
                array(
                    "ID",
            ...
            if ($arUProp = $dbUProp->Fetch()) {
                ...
                if ($arOrder["AGENT"]["ID"] <> '') {
                    ...
                    if (intval($arAI[0]) > 0) {
                        ...
                        if ($arU = $dbUser->Fetch()) {
                            if (htmlspecialcharsback(
                                    mb_substr(
                                        htmlspecialcharsbx(
                                            $arU["ID"] . "#" . $arU["LOGIN"] . "#" . $arU["LAST_NAME"] . " " . $arU["NAME"] . " " . $arU["SECOND_NAME"]
                                        ),
                                        0,
                                        80
                                    )
                ...
                if (intval($arOrder["USER_ID"]) <= 0) {
                    ...
                    $arOrder["USER_ID"] = CSaleUser::DoAutoRegisterUser(
                        $arUser["EMAIL"],
                        $arUser["NAME"],
                        $this->getSiteId(),
                        $arErrors,
    ...
    protected function nodeHandlerPartialVersion($arDocument)
    {
        ...
        if (Sale\Configuration::useStoreControl() || \Bitrix\Main\Config\Option::get(
                'catalog',
                'enable_reservation',
                'N'
            ) == 'Y') {
                        ...
                        switch ($arDocument['OPERATION_TYPE']) {
                            case 'order_operation':
                                ...
                                if ($arDocument["XML_1C_DOCUMENT_ID"] <> '') {
                                    ...
                                    $this->logMessage(
                                        "Document.XML_1C_DOCUMENT_ID: " . $arDocument['XML_1C_DOCUMENT_ID']
                                    );
                                    $this->logMessage("Document.VERSION_1C: " . $arDocument['VERSION_1C']);
                                            ...
                                            if (!$order->isCanceled()) {
                                                if ($order->getField("STATUS_ID") != "F") {
                                                    if ($arDocument['CANCELED'] == "true") {
                                                        ...
                                                        if ($arDocument["ID"] <> '' && ($payment = $paymentCollection->getItemById(
                                                                $arDocument["ID"]
                                                            ))) {
                                                            ...
                                                            if (!$deletePayment->isSuccess()) {
                                                                $this->strErrorDocument .= "\n" . GetMessage(
                                                                        ...
                                                                        $deletePayment->getErrors()
                                                                    )->getMessage();
                                                            }
                                                        } else {
                                                            $this->strErrorDocument .= "\n" . GetMessage(
                                                                    "CC_BSC1_PAYMENT_ERROR_9",
                                                                    Array(
                                                                        '#ORDER_ID#' => $arDocument['ORDER_ID'],
                                                                        '#XML_1C_DOCUMENT_ID#' => $arDocument['XML_1C_DOCUMENT_ID']
                                                                    )
                                                                );
                                                        }
                                                    } else {
                                                        if ($arDocument["ID"] <> '') {
                                                            ...
                                                            if ($payment = $paymentCollection->getItemById(
                                                                $arDocument["ID"]
                                                            )) {
                                                                $this->beforePaidCompatible1C($order);
                                                                ...
                                                                if ($this->strErrorDocument == '') {
                                                                    ...
                                                                    if ($this->strErrorDocument == '') {
                                                                        ...
                                                                        $payment->setField(
                                                                            'ID_1C',
                                                                            $this->getXMLDocumentID()
                                                                        );
                                                                    }
                                                                }
                                                            } else {
                                                                $this->strErrorDocument .= "\n" . GetMessage(
                                                                        "CC_BSC1_PAYMENT_ERROR_3",
                                                                        Array(
                                                                            "#ID#" => $arDocument["ID"],
                                                                            '#ORDER_ID#' => $arDocument['ORDER_ID'],
                                                                            '#XML_1C_DOCUMENT_ID#' => $arDocument['XML_1C_DOCUMENT_ID']
                                                                ...
                                                                "Y"
                                                            ) == 'Y') // create new payment (ofline 1C))
                                                        {
                                                            $this->beforePaidCompatible1C($order);

                                                            $payment = $this->addPaymentFromDocumentByOrder(
                                                                $arDocument,
                                                                $order
                                                            );
                                ...
                                if ($arDocument["XML_1C_DOCUMENT_ID"] <> '') {
                                    ...
                                    if ($arDocument['ORDER_ID'] !== false) {
                                        ...
                                        if ($order = $orderClass::load($arDocument['ORDER_ID'])) {
                                            if ($order->getField("STATUS_ID") != "F") {
                                                if ($arDocument["CANCELED"] == "true") {
                                                    if ($arDocument["ID"] <> '' && ($shipment = $order->getShipmentCollection(
                                                        )->getItemById($arDocument['ID']))) {
                                                        ...
                                                        if (!$deleteShipment->isSuccess()) {
                                                            $this->strErrorDocument .= "\n" . GetMessage(
                                                                    ...
                                                                    $deleteShipment->getErrors()
                                                                )->getMessage();
                                                        }
                                                    } else {
                                                        $this->strErrorDocument .= "\n" . GetMessage(
                                                                "CC_BSC1_SHIPMENT_ERROR_16",
                                                                Array(
                                                                    '#ORDER_ID#' => $arDocument['ORDER_ID'],
                                                                    '#XML_1C_DOCUMENT_ID#' => $arDocument['XML_1C_DOCUMENT_ID']
                                                                )
                                                    ...
                                                    if ($arDocument["ID"] <> '') {
                                                        if ($shipment = $order->getShipmentCollection()->getItemById(
                                                            $arDocument['ID']
                                                        )) {
                                                            /** @var Bitrix\Sale\Shipment $shipment */
                                                            if (!$shipment->isSystem()) {
                                                                if (!$shipment->isShipped()) {
                                                                    $this->deleteShipmentItemsByDocument(
                                                                        $arDocument,
                                                                        $shipment
                                                                    );

                                                                    $this->updateShipmentQuantityFromDocument(
                                                                        $arDocument,
                                                                        $shipment
                                                                    );
                                                        ...
                                                        $shipment = $this->addShipmentFromDocumentByOrder(
                                                            $arDocument,
                                                            $order
                                                        );
                                            ...
                                            "Y"
                                        ) == 'Y') // create new shipment (ofline 1C))
                                    {
                                        if ($arDocument["CANCELED"] != "true") {
                                            /** @var Bitrix\Sale\Order $order */
                                            $arOrder = $this->addOrderWithoutShipmentsPayments($arDocument);
                                    ...
                                    $this->strErrorDocument .= "\n" . GetMessage(
                                            "CC_BSC1_SHIPMENT_ERROR_6",
                                            Array("#ID#" => $arDocument["ID"])
                                        );
                                }
    ...
    function nodeHandler(CDataXML $dataXml, CXMLFileStream $fileStream)
    {
        ...
        if ($importer instanceof Sale\Exchange\ImportOneCBase) {
            ...
            if (!empty($arAgentInfo["AGENT"])) {
                ...
                $dbUProp = CSaleOrderUserProps::GetList(
                    array(),
                    array("XML_ID" => $arAgentInfo["AGENT"]["ID"]),
                    false,
                    false,
                    array(
                        "ID",
                ...
                if ($arUProp = $dbUProp->Fetch()) {
                    ...
                    if (!empty($user)) {
                        ...
                        $arAgentInfo["USER_ID"] = CSaleUser::DoAutoRegisterUser(
                            $arUser["EMAIL"],
                            $arUser["NAME"],
                            $this->arParams["SITE_NEW_ORDERS"],
                            $arErrors,
                            array(
                    ...
                    if (intval($arAgentInfo["USER_ID"]) > 0) {
                        ...
                        $this->strError .= "\n" . GetMessage(
                                "CC_BSC1_AGENT_USER_PROBLEM",
                                Array("#ID#" => $arAgentInfo["AGENT"]["ID"])
                            );
                        if (!empty($arErrors)) {
                            foreach ($arErrors as $v) {
                                $this->strError .= "\n" . $v["TEXT"];
                ...
                if ($mode) {
                    ...
                    if (empty($arExportInfo)) {
                        ...
                        while ($arExport = $dbExport->Fetch()) {
                            $arExportInfo[$arExport["PERSON_TYPE_ID"]] = unserialize(
                                $arExport["VARS"],
                                ['allowed_classes' => false]
                            );
                        }
                    ...
                    if (intval($arAgentInfo["PERSON_TYPE_ID"]) > 0) {
                            ...
                            if ($curVal <> '') {
                                ...
                                if (intval($arAgentInfo["PROFILE_ID"]) > 0) {
                                    ...
                                    if (empty($arAgentInfo["PROFILE_PROPS_VALUE"][$arOrderProperties["ID"]])) {
                                        ...
                                        CSaleOrderUserPropsValue::Update(
                                            $arAgentInfo["PROFILE_PROPS_VALUE"][$arOrderProperties["ID"]]["ID"],
                                            $arFields
                                        );
                                    }
                                }
                        ...
                        $this->strError .= "\n" . GetMessage(
                                "CC_BSC1_AGENT_PERSON_TYPE_PROBLEM",
                                Array("#ID#" => $arAgentInfo["AGENT"]["ID"])
                            );
                    }
                }
    ...
    function collectDocumentInfo($value)
    {
        ...
        if (in_array(
            ...
            if (is_array(
                    $value["#"][GetMessage("CC_BSC1_REK_VALUES")][0]["#"][GetMessage("CC_BSC1_REK_VALUE")]
                ) && !empty($value["#"][GetMessage("CC_BSC1_REK_VALUES")][0]["#"][GetMessage("CC_BSC1_REK_VALUE")])) {
                foreach (
                    $value["#"][GetMessage("CC_BSC1_REK_VALUES")][0]["#"][GetMessage(
                        "CC_BSC1_REK_VALUE"
                    )] as $val
                ) {
                    $arOrder["TRAITS"][$val["#"][GetMessage("CC_BSC1_NAME")][0]["#"]] = $val["#"][GetMessage(
                        "CC_BSC1_VALUE"
                    )][0]["#"];
                }
            }
            ...
            $arOrder["items"] = array();
            $basketItems = array();
            if (is_array($value["#"][GetMessage("CC_BSC1_ITEMS")][0]["#"]) && is_array(
                    $value["#"][GetMessage("CC_BSC1_ITEMS")][0]["#"][GetMessage("CC_BSC1_ITEM")]
                )) {
                foreach ($value["#"][GetMessage("CC_BSC1_ITEMS")][0]["#"][GetMessage("CC_BSC1_ITEM")] as $val) {
                    ...
                    if (is_array($val)) {
                        ...
                        if (doubleval($quantity) > 0) {
                            ...
                            if (isset($val[GetMessage("CC_BSC1_DISCOUNTS")]) && $val[GetMessage(
                                    "CC_BSC1_DISCOUNTS"
                                )][0]["#"][GetMessage("CC_BSC1_DISCOUNT")][0]["#"][GetMessage(
                                    "CC_BSC1_SUMM"
                                )][0]['#'] <> '') {
                                if ($priceone != $price) {
                            ...
                            if (is_array($val[GetMessage("CC_BSC1_ITEM_UNIT")]) && is_array(
                                    $val[GetMessage("CC_BSC1_ITEM_UNIT")][0]["#"]
                                )) {
                                $basketItems["MEASURE_CODE"] = $val[GetMessage("CC_BSC1_ITEM_UNIT")][0]["#"][GetMessage(
                                    "CC_BSC1_ITEM_UNIT_CODE"
                                )][0]["#"];
                                $basketItems["MEASURE_NAME"] = $val[GetMessage("CC_BSC1_ITEM_UNIT")][0]["#"][GetMessage(
                                    "CC_BSC1_ITEM_UNIT_NAME"
                                )][0]["#"];
                            }

                            if (is_array(
                                $val[GetMessage("CC_BSC1_REK_VALUES")][0]["#"][GetMessage("CC_BSC1_REK_VALUE")]
                            )) {
                                foreach (
                                    $val[GetMessage("CC_BSC1_REK_VALUES")][0]["#"][GetMessage(
                                        "CC_BSC1_REK_VALUE"
                                    )] as $val1
                                ) {
                                    if ($val1["#"][GetMessage("CC_BSC1_NAME")][0]["#"] == GetMessage(
                                            "CC_BSC1_ITEM_TYPE"
                                        )) {
                                        $basketItems["TYPE"] = $val1["#"][GetMessage("CC_BSC1_VALUE")][0]["#"];
                                    } elseif (mb_strpos(
                                            $val1["#"][GetMessage("CC_BSC1_NAME")][0]["#"],
                                            GetMessage("CC_BSC1_PROP_BASKET") . "#"
                                        ) === 0) {
                                        $markerPosition = mb_strpos(
                                            $val1["#"][GetMessage("CC_BSC1_NAME")][0]["#"],
                                            GetMessage("CC_BSC1_PROP_BASKET") . "#"
                                        );
                                        $idBasketProperty = mb_substr(
                                            $val1["#"][GetMessage("CC_BSC1_NAME")][0]["#"],
                                            $markerPosition + mb_strlen(
                                                GetMessage("CC_BSC1_PROP_BASKET") . "#"
                                            )
                                        );
                                        $basketItems["ATTRIBUTES"][$idBasketProperty] = $val1["#"][GetMessage(
                                            "CC_BSC1_VALUE"
                                        )][0]["#"];
                                    }
                                }
                            ...
                            if ($value["#"][GetMessage("CC_BSC1_TAXES")][0]["#"][GetMessage(
                                    "CC_BSC1_TAX"
                                )][0]["#"][GetMessage("CC_BSC1_NAME")][0]["#"] <> '') {
                                $taxValueTmp = $val[GetMessage("CC_BSC1_TAXES")][0]["#"][GetMessage(
                                    "CC_BSC1_TAX"
                                )][0]["#"][GetMessage("CC_BSC1_TAX_VALUE")][0]["#"];
                                $basketItems["VAT_RATE"] = $taxValueTmp / 100;

                                if (intval($taxValueTmp) > intval($taxValue)) {
                                    $taxName = $val[GetMessage("CC_BSC1_TAXES")][0]["#"][GetMessage(
                                        "CC_BSC1_TAX"
                                    )][0]["#"][GetMessage("CC_BSC1_NAME")][0]["#"];
            ...
            if (intval($taxValue) > 0) {
                $price = $this->ToFloat(
                    $value["#"][GetMessage("CC_BSC1_TAXES")][0]["#"][GetMessage("CC_BSC1_TAX")][0]["#"][GetMessage(
                        "CC_BSC1_SUMM"
                    )][0]["#"]
                );
                $arOrder["TAX"] = Array(
                    "NAME" => $taxName,
                    "VALUE" => $taxValue,
                    "IS_IN_PRICE" => ($value["#"][GetMessage("CC_BSC1_TAXES")][0]["#"][GetMessage(
                        "CC_BSC1_TAX"
                    )][0]["#"][GetMessage("CC_BSC1_IN_PRICE")][0]["#"] == "true" ? "Y" : "N"),
                    "VALUE_MONEY" => $price,
                );
            }
            ...
            if ($bNeedFull) {
                ...
                if (!empty($value["#"][GetMessage("SALE_EXPORT_CONTRAGENTS")][0]["#"])) {
                    $arAgentInfo = $this->collectAgentInfo(
                        $value["#"][GetMessage("SALE_EXPORT_CONTRAGENTS")][0]["#"][GetMessage(
                            "SALE_EXPORT_CONTRAGENT"
                        )][0]["#"]
                    );
                    ...
                    if ($arOrder["TRAITS"][GetMessage("SALE_EXPORT_DELIVERY_ADDRESS")] <> '') {
                        if (!empty($arOrder["AGENT"]["REGISTRATION_ADDRESS"])) {
                            $arOrder["AGENT"]["REGISTRATION_ADDRESS"]["PRESENTATION"] = $arOrder["TRAITS"][GetMessage(
                                "SALE_EXPORT_DELIVERY_ADDRESS"
                            )];
                        }
                        if (!empty($arOrder["AGENT"]["ADDRESS"])) {
                            $arOrder["AGENT"]["ADDRESS"]["PRESENTATION"] = $arOrder["TRAITS"][GetMessage(
                                "SALE_EXPORT_DELIVERY_ADDRESS"
                            )];
                        }
                    }
    ...
    function collectAgentInfo($data = array())
    {
        ...
        foreach ($schema as $k => $v) {
            if (is_array($v)) {
                if (isset($data[GetMessage("SALE_EXPORT_" . $k)]) && !empty(
                    $data[GetMessage(
                        "SALE_EXPORT_" . $k
                    )][0]["#"]
                    )) {
                    ...
                    foreach ($v as $kk => $vv) {
                        if (is_array($vv)) {
                            if (isset($adr[GetMessage("SALE_EXPORT_" . $kk)]) && !empty(
                                $adr[GetMessage(
                                    "SALE_EXPORT_" . $kk
                                )][0]["#"]
                                ) > 0) {
                                foreach ($vv as $vvv) {
                                    foreach ($adr[GetMessage("SALE_EXPORT_" . $kk)] as $val) {
                                        if ($val["#"][GetMessage("SALE_EXPORT_TYPE")][0]["#"] == GetMessage(
                                                "SALE_EXPORT_" . $vvv
                                            )
                                            && $val["#"][GetMessage("SALE_EXPORT_VALUE")][0]["#"] <> ''
                                        ) {
                                            $result["AGENT"][$kk][$vvv] = $val["#"][GetMessage(
                                                "SALE_EXPORT_VALUE"
                                            )][0]["#"];
                                        } elseif (empty(
                                            $val["#"][GetMessage(
                                            ...
                                            ) && $val["#"][GetMessage("SALE_EXPORT_RELATION")][0]["#"] == GetMessage(
                                                "SALE_EXPORT_CONTACT_PERSON"
                                            )) {
                                            $result["AGENT"]["CONTACT"][$vvv] = $val["#"][GetMessage(
                                                "SALE_EXPORT_ITEM_NAME"
                                            )][0]["#"];
                                        }
                                    }
                                }
                            ...
                            if (isset($adr[GetMessage("SALE_EXPORT_" . $vv)]) && $adr[GetMessage(
                                    "SALE_EXPORT_" . $vv
                                )][0]["#"] <> '') {
                                ...
                                if (!empty($adr[GetMessage("SALE_EXPORT_ADDRESS_FIELD")])) {
                                    foreach ($adr[GetMessage("SALE_EXPORT_ADDRESS_FIELD")] as $val) {
                                        if ($val["#"][GetMessage("SALE_EXPORT_TYPE")][0]["#"] == GetMessage(
                                            ...
                                            $result["AGENT"][$k][$vv] = $val["#"][GetMessage(
                                                "SALE_EXPORT_VALUE"
                                            )][0]["#"];
                                        }
                                    }
                ...
                if (isset($data[GetMessage("SALE_EXPORT_" . $v)]) && $data[GetMessage(
                        "SALE_EXPORT_" . $v
                    )][0]["#"] <> '') {
    ...
    function oldSaveOrder($arOrder, $option = array())
    {
        ...
        {
            $dbOrder = $parentEntity::GetList(
                array("ID" => "DESC"),
                array("ID_1C" => $arOrder["ID_1C"]),
                false,
                false,
                array("ID", "ID_1C")
            );
        ...
        if ($arOrder["ID"] <> '') // exists site order
        {
            ...
            if ($orderInfo = $dbOrder->Fetch()) {
                    ...
                    if ($orderInfo["PAYED"] != "Y" && $orderInfo["ALLOW_DELIVERY"] != "Y" && $orderInfo["STATUS_ID"] != "F") {
                                ...
                                if ($arItem["CHECKED"] != "Y") {
                                    if ($arItem["TYPE"] == GetMessage("CC_BSC1_ITEM")) {
                                        ...
                                        if ($orderInfo['CURRENCY'] == $currencyTo) {
                                            ...
                                            if ($arBasketFields = $this->prepareProduct4Basket(
                                                ...
                                                if ($isInvoice) {
                                                    $basketItem = $basket->createItem(
                                                        $arBasketFields['MODULE'],
                                                        $arBasketFields['PRODUCT_ID']
                                                    );
                                            ...
                                            $this->strError .= "\r\n " . GetMessage(
                                                    "CC_BSC1_ORDER_ERROR_5",
                                                    Array(
                                                        '#XML_1C_DOCUMENT_ID#' => $arOrder['ID'],
                                                        '#CURRENCY_FROM#' => $orderInfo['CURRENCY'],
                        ...
                        if ($bNeedUpdate) {
                            if ($isInvoice) {
                                ...
                                $arErrors = array();
                                if (!$basketEntity::DoSaveOrderBasket(
                                    $orderId,
                                    $orderInfo["LID"],
                                    $orderInfo["USER_ID"],
                                    $arShoppingCart,
                                    ...
                                    if (is_object($e)) {
                                        $this->strError .= "\r\n " . GetMessage(
                                                "CC_BSC1_ORDER_ERROR_3",
                                                Array('#XML_1C_DOCUMENT_ID#' => $arOrder["ID"])
                                            ) . $e->GetString();
                                    }
                                }
                            }
                ...
                if ($arOrder['CANCELED'] == "true" || $arOrder["TRAITS"][GetMessage(
                        "CC_BSC1_CANCELED"
                    )] == "true" || $arOrder["TRAITS"][GetMessage("CC_BSC1_CANCEL")] == "true") {
                    if ($orderInfo["CANCELED"] == "N") {
                    ...
                    if (mb_strlen($arOrder["TRAITS"][GetMessage("CC_BSC1_1C_PAYED_DATE")]) > 1) {
                        ...
                        $arAditFields["PAY_VOUCHER_DATE"] = CDatabase::FormatDate(
                            str_replace("T", " ", $arOrder["TRAITS"][GetMessage("CC_BSC1_1C_PAYED_DATE")]),
                            "YYYY-MM-DD HH:MI:SS",
                            CLang::GetDateFormat("FULL", LANG)
                        );
                    ...
                    if (mb_strlen($arOrder["TRAITS"][GetMessage("CC_BSC1_1C_DELIVERY_DATE")]) > 1) {
                        ...
                        $arAditFields["DATE_ALLOW_DELIVERY"] = CDatabase::FormatDate(
                            str_replace("T", " ", $arOrder["TRAITS"][GetMessage("CC_BSC1_1C_DELIVERY_DATE")]),
                            "YYYY-MM-DD HH:MI:SS",
                            CLang::GetDateFormat("FULL", LANG)
                        );
                        ...
                        if ($this->arParams["FINAL_STATUS_ON_DELIVERY"] <> '' && $orderInfo["STATUS_ID"] != "F" && $orderInfo["STATUS_ID"] != $this->arParams["FINAL_STATUS_ON_DELIVERY"]) {
                            static::setStatus(
                                $orderInfo["ID"],
                                $this->arParams["FINAL_STATUS_ON_DELIVERY"],
                                $isInvoice
                            );
                        }
                        if ($arOrder["TRAITS"][GetMessage("CC_BSC1_1C_DELIVERY_NUM")] <> '') {
                            $arAditFields["DELIVERY_DOC_NUM"] = $arOrder["TRAITS"][GetMessage(
                ...
                if ($this->arParams["CHANGE_STATUS_FROM_1C"] && $arOrder["TRAITS"][GetMessage(
        ...
        } elseif ($this->arParams["IMPORT_NEW_ORDERS"] == "Y") // create new order (ofline 1C)
        {
            if (!empty($arOrder["AGENT"]) && $arOrder["AGENT"]["ID"] <> '') {
                ...
                if ($arUProp = $dbUProp->Fetch()) {
                    ...
                    $dbUPropValue = CSaleOrderUserPropsValue::GetList(
                        array(),
                        array("USER_PROPS_ID" => $arUProp["ID"])
                    );
                    while ($arUPropValue = $dbUPropValue->Fetch()) {
                        $arOrder["USER_PROPS"][$arUPropValue["ORDER_PROPS_ID"]] = $arUPropValue["VALUE"];
                    }
                ...
                if (intval($arOrder["PERSON_TYPE_ID"]) > 0) {
                    ...
                    foreach ($arAgent as $k => $v) {
                        if (empty($v) ||
                            (
                                (empty($v["VALUE"]) || $v["TYPE"] != "PROPERTY") &&
                                (empty($arOrder["USER_PROPS"])
                                    || (is_array($v) && is_string(
                                            $v["VALUE"]
                                        ) && empty($arOrder["USER_PROPS"][$v["VALUE"]]))
                                )
                            )
                        ) {
                            unset($arAgent[$k]);
                    ...
                    if (intval($arOrder["USER_ID"]) > 0) {
                        ...
                        $arAditFields = array(
                            ...
                            "UPDATED_1C" => "Y",
                            "DATE_INSERT" => CDatabase::FormatDate(
                                $arOrder["DATE"] . " " . $arOrder["TIME"],
                                "YYYY-MM-DD HH:MI:SS",
                                CLang::GetDateFormat("FULL", LANG)
                            ),
                        );
                        ...
                        foreach ($arOrder["items"] as $productID => $val) {
                            $orderFields["BASKET_ITEMS"][] = $this->prepareProduct4Basket(
                                $productID,
                                $val,
                                false,
                                $orderFields
                    ...
                    $this->strError .= "\n" . GetMessage(
                            "CC_BSC1_ORDER_PERSON_TYPE_PROBLEM",
                            Array("#ID#" => $arOrder["ID_1C"])
                        );
                }
            } else {
