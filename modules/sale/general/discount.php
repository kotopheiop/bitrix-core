...
class CAllSaleDiscount
{
    ...
    public static function DoProcessOrder(&$arOrder, $arOptions, &$arErrors)
    {
        ...
        if ($isOrderConverted != 'N') {
            ...
            if ($adminSection) {
                ...
                if (isset($arOrder['SITE_ID'])) {
                    ...
                    if (!isset($modeParams['CURRENCY'])) {
                        $modeParams['CURRENCY'] = Sale\Internals\SiteCurrencyTable::getSiteCurrency(
                            $modeParams['SITE_ID']
                        );
                    }
                }
                ...
                if (!empty($basketIdList)) {
                    $iterator = Sale\Internals\BasketTable::getList(
                        array(
            ...
            if (!empty($modeParams)) {
                Sale\Discount\Actions::setUseMode(
                    ...
                    array(
                        'USE_BASE_PRICE' => \Bitrix\Main\Config\Option::get(
                            'sale',
        ...
        if ($checkIds) {
            $groupDiscountIterator = Sale\Internals\DiscountGroupTable::getList(
                array(
        ...
        if (!empty($arIDS)) {
            ...
            $couponList = Sale\DiscountCouponsManager::getForApply(
                array('MODULE_ID' => 'sale', 'DISCOUNT_ID' => $arIDS),
                array(),
                true
            );
            ...
            foreach ($arOrder['BASKET_ITEMS'] as &$arOneItem) {
                if (
                    array_key_exists(
                        ...
                        $arOneItem
                    ) && empty($arOneItem['PRODUCT_PROVIDER_CLASS'])
            ...
            if (!empty($couponList)) {
                $iterator = Sale\Internals\DiscountCouponTable::getList(
                    array(
                    ...
                    if ($isOrderConverted != 'N') {
                        ...
                        if (!empty($discountResult['DELIVERY']) || !empty($discountResult['BASKET'])) {
                            if ($discount['USE_COUPONS'] == 'Y' && !empty($discount['DISCOUNT_COUPON'])) {
                                ...
                                $couponApply = Sale\DiscountCouponsManager::setApply(
        ...
        foreach (array_keys($arOrder['BASKET_ITEMS']) as $index) {
            ...
            if (!CSaleBasketHelper::isSetItem($arShoppingCartItem)) {
                ...
                if (!$customPrice) {
                    $arShoppingCartItem['DISCOUNT_PRICE'] = Sale\PriceMaths::roundPrecision(
                ...
                $arShoppingCartItem["PRICE_FORMATED"] = CCurrencyLang::CurrencyFormat(
                ...
                $arShoppingCartItem["DISCOUNT_PRICE_PERCENT_FORMATED"] = roundEx(
                        $arShoppingCartItem["DISCOUNT_PRICE_PERCENT"],
                        0
                    ) . "%";
    ...
    public static function CheckFields($ACTION, &$arFields)
    {
        ...
        if ((is_set(
                    $arFields,
                    "DISCOUNT_TYPE"
                ) || $ACTION == "ADD") && $arFields["DISCOUNT_TYPE"] != self::OLD_DSC_TYPE_PERCENT) {
        ...
        if ((is_set($arFields, "ACTIVE_FROM") || $ACTION == "ADD") && (!$DB->IsDate(
        ...
        if ((is_set($arFields, "ACTIVE_TO") || $ACTION == "ADD") && (!$DB->IsDate(
                $arFields["ACTIVE_TO"],
                false,
                LANGUAGE_ID,
                "FULL"
        ...
        if ($updateData) {
            ...
            if (!empty($selectFields)) {
                ...
                $discountInfo = Sale\Internals\DiscountTable::getList(
                    array(
            ...
            if ($useConditions) {
                if (!isset($arFields['CONDITIONS']) || empty($arFields['CONDITIONS'])) {
                    $APPLICATION->ThrowException(
                        Loc::getMessage('BT_MOD_SALE_DISC_ERR_EMPTY_CONDITIONS'),
                        'CONDITIONS'
                    );
                    return false;
                } else {
        ...
        if (array_key_exists('USER_GROUPS', $arFields) || $ACTION == "ADD") {
            ...
            if (empty($arFields['USER_GROUPS']) || !is_array($arFields['USER_GROUPS'])) {
                $APPLICATION->ThrowException(
                    Loc::getMessage("BT_MOD_SALE_DISC_ERR_USER_GROUPS_ABSENT_SHORT"),
                    "USER_GROUPS"
                );
                return false;
            }
            ...
            if ('ADD' == $strAction || $boolUpdate) {
                if (!array_key_exists('LID', $arFields)) {
                    ...
                    if ('' == $arFields['LID']) {
                        ...
                        $arMsg[] = array(
                            'id' => 'LID',
                            'text' => Loc::getMessage('BT_MOD_SALE_ERR_DSC_SITE_ID_ABSENT')
                        );
                    } else {
                        ...
                        if (!$arSite = $rsSites->Fetch()) {
                            ...
                            $arMsg[] = array(
                                'id' => 'LID',
                                'text' => Loc::getMessage('SKGD_NO_SITE', array('#ID#' => $arFields['LID']))
                            );
                        } else {
                ...
                if (!array_key_exists('CURRENCY', $arFields)) {
                    ...
                    $arMsg[] = array(
                        'id' => 'CURRENCY',
                        'text' => Loc::getMessage('BT_MOD_SALE_ERR_DSC_CURRENCY_ABSENT')
                    );
                } else {
                    ...
                    if ('' == $arFields['CURRENCY']) {
                        ...
                        if (!($arCurrency = CCurrency::GetByID($arFields["CURRENCY"]))) {
                            ...
                            $arMsg[] = array(
                                'id' => 'CURRENCY',
                                'text' => Loc::getMessage('SKGD_NO_CURRENCY', array('#ID#' => $arFields['CURRENCY']))
                            );
                        }
                    }
                }
                ...
                if (!array_key_exists("DISCOUNT_TYPE", $arFields)) {
                    ...
                    $arMsg[] = array(
                        'id' => 'DISCOUNT_TYPE',
                        'text' => Loc::getMessage('BT_MOD_SALE_ERR_DSC_TYPE_ABSENT')
                    );
                } else {
                    ...
                    if (CSaleDiscount::OLD_DSC_TYPE_PERCENT != $arFields["DISCOUNT_TYPE"] && CSaleDiscount::OLD_DSC_TYPE_FIX != $arFields["DISCOUNT_TYPE"]) {
                        ...
                        $arMsg[] = array(
                            'id' => 'DISCOUNT_TYPE',
                            'text' => Loc::getMessage('BT_MOD_SALE_ERR_DSC_TYPE_BAD')
                        );
                    }
                }
                ...
                if (!array_key_exists('DISCOUNT_VALUE', $arFields)) {
                    ...
                    $arMsg[] = array(
                        'id' => 'DISCOUNT_VALUE',
                        'text' => Loc::getMessage('BT_MOD_SALE_ERR_DSC_VALUE_ABSENT')
                    );
                } else {
                    ...
                    if (0 >= $arFields['DISCOUNT_VALUE']) {
                        ...
                        $arMsg[] = array(
                            'id' => 'DISCOUNT_VALUE',
                            'text' => Loc::getMessage('BT_MOD_SALE_ERR_DSC_VALUE_BAD')
                        );
                    }
                }
                ...
                if ($boolResult) {
                    ...
                    if (array_key_exists('PRICE_FROM', $arFields)) {
                        ...
                        if (0 < $arFields["PRICE_FROM"]) {
                            $dblValue = roundEx(
                                ($boolCurrency ? $arFields['PRICE_FROM'] : CCurrencyRates::ConvertCurrency(
                                    $arFields['PRICE_FROM'],
                                    $arFields['CURRENCY'],
                                    $strSiteCurrency
                                )),
                    ...
                    if (array_key_exists('PRICE_TO', $arFields)) {
                        ...
                        if (0 < $arFields["PRICE_TO"]) {
                            $dblValue = roundEx(
                                ($boolCurrency ? $arFields['PRICE_TO'] : CCurrencyRates::ConvertCurrency(
                                    $arFields['PRICE_TO'],
                                    $arFields['CURRENCY'],
                                    $strSiteCurrency
                                )),
                    ...
                    if (self::OLD_DSC_TYPE_PERCENT == $arFields['DISCOUNT_TYPE']) {
                        ...
                        $dblValue = roundEx(
                            ($boolCurrency ? $arFields['DISCOUNT_VALUE'] : CCurrencyRates::ConvertCurrency(
                                $arFields['DISCOUNT_VALUE'],
                                $arFields['CURRENCY'],
                                $strSiteCurrency
                            )),
