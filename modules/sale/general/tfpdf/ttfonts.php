...
class TTFontFile
{
    ...
    function calcChecksum($data)
    {
        if (mb_strlen($data, '8bit') % 4) {
            $data .= str_repeat("\0", (4 - (mb_strlen($data, '8bit') % 4)));
        }
        $hi = 0x0000;
        $lo = 0x0000;
        for ($i = 0; $i < mb_strlen($data, '8bit'); $i += 4) {
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    function makeSubset($file, &$subset)
    {
        ...
        foreach ($subsetglyphs AS $originalGlyphIdx => $uni) {
            ...
            if ($glyphLen > 0) {
                $up = unpack("n", mb_substr($data, 0, 2, '8bit'));
            }
            ...
            if ($glyphLen > 2 && ($up[1] & (1 << 15))) {    // If number of contours <= -1 i.e. composiste glyph
                ...
                while ($flags & GF_MORE) {
                    ...
                    $up = unpack("n", mb_substr($data, $pos_in_glyph, 2, '8bit'));
                    $flags = $up[1];
                    $up = unpack("n", mb_substr($data, $pos_in_glyph + 2, 2, '8bit'));
                    $glyphIdx = $up[1];
    ...
    function endTTFile(&$stm)
    {
        ...
        foreach ($tables AS $tag => $data) {
            ...
            $stm .= pack("NN", $offset, mb_strlen($data, '8bit'));
            $paddedLength = (mb_strlen($data, '8bit') + 3) & ~3;
        ...
        foreach ($tables AS $tag => $data) {
            $data .= "\0\0\0";
            $stm .= mb_substr($data, 0, (mb_strlen($data, '8bit') & ~3), '8bit');
        }
