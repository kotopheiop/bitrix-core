...
class CAllSaleOrderPropsGroup
{
        ...
        if (is_set($arFields, "PERSON_TYPE_ID")) {
            if (!($arPersonType = CSalePersonType::GetByID($arFields["PERSON_TYPE_ID"]))) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["PERSON_TYPE_ID"], GetMessage("SKGOPG_NO_PERS_TYPE")),
    ...
    public static function Delete($ID)
    {
        ...
        while ($arOrderProps = $db_orderProps->Fetch()) {
            ...
            $DB->Query(
                "UPDATE b_sale_order_props_value SET ORDER_PROPS_ID = NULL WHERE ORDER_PROPS_ID = " . $arOrderProps["ID"] . "",
                true
            );
