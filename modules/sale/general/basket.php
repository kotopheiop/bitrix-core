...
class CAllSaleBasket
{
        ...
        while ($arItemsBasket = $rsItemsBasket->Fetch()) {
            ...
            if (isset($arSubscribeProd[$LID]) && $arSubscribeProd[$LID]["use"] == "Y") {
                ...
                if ($productProvider = CSaleBasket::GetProductProvider($arItemsBasket)) {
                    $arCallback = $productProvider::GetProductData(
                        array(
                            "PRODUCT_ID" => $ID,
                            "QUANTITY" => 1,
                            "RENEWAL" => "N",
    ...
    public static function DoGetUserShoppingCart(
        ...
        foreach ($shoppingCart as $itemIndex => $arShoppingCartItem) {
            if ((array_key_exists("CALLBACK_FUNC", $arShoppingCartItem) && !empty($arShoppingCartItem["CALLBACK_FUNC"]))
                ...
                if ($productProvider = CSaleBasket::GetProductProvider($arShoppingCartItem)) {
                    ...
                    $providerParams = array(
                        ...
                        "CHECK_COUPONS" => ('Y' == $arShoppingCartItem['CAN_BUY'] && (!array_key_exists(
                                'DELAY',
                                $arShoppingCartItem
                            ) || 'Y' != $arShoppingCartItem['DELAY']) ? 'Y' : 'N'),
            ...
            if ($arShoppingCartItem["CAN_BUY"] == "Y") {
                ...
                $arShoppingCartItem["DIMENSIONS"] = unserialize(
                    $arShoppingCartItem["DIMENSIONS"],
                    ['allowed_classes' => false]
                );
                $arShoppingCartItem["VAT_RATE"] = floatval($arShoppingCartItem["VAT_RATE"]);
                $arShoppingCartItem["DISCOUNT_PRICE"] = roundEx(
                    $arShoppingCartItem["DISCOUNT_PRICE"],
                    SALE_VALUE_PRECISION
                );

                if ($arShoppingCartItem["VAT_RATE"] > 0) {
                    $arShoppingCartItem["VAT_VALUE"] = \Bitrix\Sale\PriceMaths::roundPrecision(
                        ($arShoppingCartItem["PRICE"] / ($arShoppingCartItem["VAT_RATE"] + 1)) * $arShoppingCartItem["VAT_RATE"]
                    );
                }
     ...
     * @return bool
     */
    public static function DoChangeProductQuantity(
        $arBasketItem,
        $deltaQuantity,
        ...
        if ($productProvider = CSaleBasket::GetProductProvider($arBasketItem)) {
            ...
            {
                $quantityPreviouslyLeftToReserve = ($arBasketItem["RESERVED"] == "Y") ? floatval(
                    $arBasketItem["RESERVE_QUANTITY"]
                ) : 0;
                ...
                $arRes = CSaleBasket::ReserveBasketProduct(
                    $arBasketItem["ID"],
                    $deltaQuantity + $quantityPreviouslyLeftToReserve,
                    $isOrderDeducted
                );
                if (array_key_exists("ERROR", $arRes)) {
                    CSaleOrder::SetMark(
                        $arAdditionalParams["ORDER_ID"],
                        Loc::getMessage(
                            "SKGB_RESERVE_ERROR",
                            array("#MESSAGE#" => $arRes["ERROR"]["MESSAGE"])
                        )
                    );
                    return false;
                }
                ...
                $arDeductResult = CSaleBasket::DeductBasketProduct(
                    $arBasketItem["ID"],
                    $deltaQuantity,
                ...
                if ($isOrderReserved && !$isOrderDeducted) // we need to reserve product
                {
     ...
     * @return bool
     */
    public static function DoSaveOrderBasket(
        $orderId,
        $siteId,
        $userId,
        &$arShoppingCart,
        ...
        foreach ($arShoppingCart as &$arItem) {
            ...
            if (array_key_exists("ID", $arItem) && (int)$arItem["ID"] > 0) {
                ...
                if (defined("SALE_DEBUG") && SALE_DEBUG) {
                    CSaleHelper::WriteToLog(
                        "DoSaveOrderBasket - Product #" . $arItem["ID"] . " already in the basket",
                        array(),
                        "DSOB3"
                    );
                }
                ...
                if (CSaleBasketHelper::isSetItem(
                    $arItem
                ...
                if ($isOrderConverted != 'N') {
                    ...
                    CSaleBasket::Update(
                        $arItem["ID"],
                        array("ORDER_ID" => $orderId, "IGNORE_CALLBACK_FUNC" => "Y") + $arItem
                    );
                }
            } else // new product in the basket
            {
                ...
                {
                    $oldSetParentId = -1;
                    if (CSaleBasketHelper::isSetParent($arItem) && array_key_exists(
                    ...
                    if (CSaleBasketHelper::isSetItem($arItem) && array_key_exists(
                            $arItem["SET_PARENT_ID"],
                            $arTmpSetParentId
                        )) {
        ...
        if (!array_key_exists('IGNORE_CALLBACK_FUNC', $arFields) || 'Y' != $arFields['IGNORE_CALLBACK_FUNC']) {
            if ((array_key_exists("CALLBACK_FUNC", $arFields) && !empty($arFields["CALLBACK_FUNC"]))
                || (array_key_exists(
                        "PRODUCT_PROVIDER_CLASS",
                        $arFields
                    ) && !empty($arFields["PRODUCT_PROVIDER_CLASS"]))
            ) {
                /** @var $productProvider IBXSaleProductProvider */
...
//						$productProvider::getProviderData();
        ...
        if (is_set($arFields, "ORDER_ID")) {
            ...
            if (!isset($orderList[$arFields["ORDER_ID"]])) {
                $APPLICATION->ThrowException(
                    str_replace("#ID#", $arFields["ORDER_ID"], Loc::getMessage("SKGB_NO_ORDER")),
                    "ORDER_ID"
                );
                return false;
            }
        ...
        if (is_set($arFields, 'CURRENCY')) {
            ...
            if (empty($arFields['CURRENCY'])) {
                $APPLICATION->ThrowException(
                    str_replace("#ID#", $arFields["CURRENCY"], Loc::getMessage("SKGB_NO_CURRENCY")),
                    "CURRENCY"
                );
                return false;
            } else {
        ...
        if (is_set($arFields, "LID")) {
            ...
            if (!$dbSite->Fetch()) {
                $APPLICATION->ThrowException(
                    str_replace("#ID#", $arFields["LID"], Loc::getMessage("SKGB_NO_SITE")),
                    "LID"
                );
                return false;
            }
        ...
        if (!empty($arFields['LID']) && !empty($arFields['CURRENCY'])) {
            ...
            if ($siteCurrency != $arFields['CURRENCY']) {
                $arFields["PRICE"] = roundEx(
                    CCurrencyRates::ConvertCurrency($arFields["PRICE"], $arFields["CURRENCY"], $siteCurrency),
                    SALE_VALUE_PRECISION
                );
                if (is_set($arFields, "DISCOUNT_PRICE")) {
                    $arFields["DISCOUNT_PRICE"] = roundEx(
    ...
    public static function _Update($ID, &$arFields)
    {
        ...
        if (!empty($strUpdate)) {
            ...
            $strSql = "update b_sale_basket set " . $strUpdate . ", DATE_UPDATE = " . $DB->GetNowFunction(
                ) . " where ID = " . $ID;
        ...
        if ($updateHistory) {
            CSaleOrderChange::AddRecordsByFields(
                $arFields["ORDER_ID"],
                $arOldFields,
                $arFields,
                array('PROPS'),
        ...
        if (is_set($arFields, "QUANTITY") && floatval($arFields["QUANTITY"]) <= 0) {
            ...
            if ($isOrderConverted != 'N') {
                ...
                if (is_set(
                    $arFields,
                    "QUANTITY"
                )) // if quantity updated and is set parent item, update all set items' quantity
                {
                    $arBasket = $arFields;
                    $oldQuantity = false;
                            ...
                            while ($arItem = $dbSetItems->Fetch()) {
                                ...
                                CSaleBasket::Update(
                                    ...
                                    array(
                                        ...
                                        'SET_PARENT_ID' => (int)$arItem['SET_PARENT_ID'],
                                        'TYPE' => (int)$arItem['TYPE']
                                    )
                                );
                            }
    ...
    public static function ReReadPrice(
        $callbackFunc = "",
        $module = "",
        $productID = 0,
        $quantity = 0,
        ...
        if (CSaleBasket::GetProductProvider(array("MODULE" => $module, "PRODUCT_PROVIDER_CLASS" => $productProvider))) {
            return $productProvider::GetProductData(
                array(
    ...
    public static function OnOrderProduct(
        $callbackFunc = "",
        $module = "",
        $productID = 0,
        $quantity = 0,
    ...
    public static function UpdatePrice(
        ...
        $providerName = CSaleBasket::GetProductProvider(
            array("MODULE" => $module, "PRODUCT_PROVIDER_CLASS" => $productProvider)
        );
        if ($providerName) {
            ...
            if ($arBasket["DELAY"] == "N" && $arBasket["CAN_BUY"] == "Y") {
                if (!empty($arBasket["ORDER_CALLBACK_FUNC"]) || !empty($arBasket["PRODUCT_PROVIDER_CLASS"])) {
                    ...
                    if (!empty($arFields) && is_array($arFields)) {
                        ...
                        $removeCoupon = DiscountCouponsManager::deleteApplyByProduct(
                            array(
            ...
            while ($arBasket = $dbBasketList->Fetch()) {
                if ($arBasket["PAY_CALLBACK_FUNC"] <> '' || $arBasket["PRODUCT_PROVIDER_CLASS"] <> '') {
                    if ($bPaid) {
                        /** @var $productProvider IBXSaleProductProvider */
                        if ($productProvider = CSaleBasket::GetProductProvider($arBasket)) {
                            $arFields = $productProvider::DeliverProduct(
                                array(
                                    "PRODUCT_ID" => $arBasket["PRODUCT_ID"],
                        ...
                        if ($arFields && is_array($arFields) && count($arFields) > 0) {
                            $arFields["ORDER_ID"] = $orderID;
                            $arFields["REMAINING_ATTEMPTS"] = (Defined(
                        ...
                        /** @var $productProvider IBXSaleProductProvider */
                        if ($productProvider = CSaleBasket::GetProductProvider($arBasket)) {
                            $productProvider::DeliverProduct(
                                array(
                                    "PRODUCT_ID" => $arBasket["PRODUCT_ID"],
                                    "USER_ID" => $arOrder["USER_ID"],
                                    "PAID" => $bPaid,
    ...
    public static function OrderReservation($orderID, $bUndoReservation = false)
    {
        ...
        if ($arOrder) {
            ...
            while ($arBasket = $dbBasketList->Fetch()) {
                if ($bUndoReservation && $arBasket["RESERVED"] == "N" && COption::GetOptionString(
                        "catalog",
                        "enable_reservation"
                    ) != "N") {
                    continue;
                ...
                if ($productProvider = CSaleBasket::GetProductProvider($arBasket)) {
                    ...
                    if ($arOrder["DEDUCTED"] == "Y") // order already deducted, don't reserve it
                    {
                        ...
                        if (defined("SALE_DEBUG") && SALE_DEBUG) {
                            CSaleHelper::WriteToLog(
                                "Order already deducted. Product won't be reserved.",
                                array(),
                                "OR5"
                            );
                        }
                    } else {
                        $res = $productProvider::ReserveProduct(
                            array(
                                "PRODUCT_ID" => $arBasket["PRODUCT_ID"],
                                "QUANTITY_ADD" => $arBasket["QUANTITY"],
                                "UNDO_RESERVATION" => ($bUndoReservation) ? "Y" : "N",
                    ...
                    if ($res["RESULT"]) {
                        ...
                        if (defined("SALE_DEBUG") && SALE_DEBUG) {
                            CSaleHelper::WriteToLog(
                                "Product #" . $arBasket["PRODUCT_ID"] . " reserved successfully",
                                array("arUpdateFields" => $arUpdateFields),
                                "OR4"
                            );
                        }
                        ...
                        if (defined("SALE_DEBUG") && SALE_DEBUG) {
                            CSaleHelper::WriteToLog(
                                "Product #" . $arBasket["PRODUCT_ID"] . " reservation error",
                                array(),
                                "OR4"
                            );
                        }
            ...
            if ($productProvider = CSaleBasket::GetProductProvider($arBasket)) {
                ...
                if ($res["RESULT"]) {
                    ...
                    if (isset($res["QUANTITY_NOT_RESERVED"])) {
                        CSaleBasket::Update(
                            $arBasket["ID"],
                            array("RESERVE_QUANTITY" => $res["QUANTITY_NOT_RESERVED"])
                        );
                    }
                }
        ...
        if ($arBasket) {
            ...
            if ($productProvider = CSaleBasket::GetProductProvider($arBasket)) {
                ...
                if ($res["RESULT"]) {
                    if (defined("SALE_DEBUG") && SALE_DEBUG) {
                        CSaleHelper::WriteToLog(
                            "Product #" . $arBasket["PRODUCT_ID"] . " deducted successfully",
                            array(),
                            "DBP3"
                        );
                    }
                    ...
                    if (defined("SALE_DEBUG") && SALE_DEBUG) {
                        CSaleHelper::WriteToLog(
                            "Product #" . $arBasket["PRODUCT_ID"] . " deduction error",
                            array(),
                            "DBP4"
                        );
                    }
     ...
     * @return mixed array
     */
    public static function OrderDeduction(
        $orderID,
        $bUndoDeduction = false,
        $recurringID = 0,
        $bAutoDeduction = true,
        ...
        while ($arBasket = $dbBasketList->Fetch()) {
            ...
            if ($productProvider = CSaleBasket::GetProductProvider($arBasket)) {
                ...
                if (is_null($bAutoDeductionAllowed)) {
                    ...
                    if ($storesCount == 1 || $storesCount == -1 || intval(
                            $defaultDeductionStore
                        ) > 0) // if stores' count = 1 or stores aren't used or default deduction store is defined
                    {
                ...
                if (defined("SALE_DEBUG") && SALE_DEBUG) {
                    CSaleHelper::WriteToLog(
                        "auto deduction allowed: " . intval($bAutoDeductionAllowed),
                        array(),
                        "OD4"
                    );
                }
                ...
                if ($bAutoDeduction && !$bAutoDeductionAllowed && !$bUndoDeduction) {
                    ...
                    $APPLICATION->ThrowException(
                        Loc::getMessage("DDCT_AUTO_DEDUCT_WRONG_STORES_QUANTITY"),
                    ...
                    if ($bAutoDeduction && $arBasket["BARCODE_MULTI"] == "Y" && !$bUndoDeduction) {
                        ...
                        $APPLICATION->ThrowException(
                            Loc::getMessage(
                                ...
                                array("#PRODUCT_ID#" => $arBasket["PRODUCT_ID"])
                            ),
                        ...
                        if ($bUndoDeduction) {
                            ...
                            if ($storesCount == 1) {
                                ...
                                if (intval($defaultDeductionStore) > 0) // if default deduction store is defined
                                {
                                    $arFields["QUANTITY"] = 0;
                        ...
                        if (defined("SALE_DEBUG") && SALE_DEBUG) {
                            CSaleHelper::WriteToLog(
                                "Emulating ::DeductProduct call",
                                array("arFields" => $arFields),
                                "OD7"
                            );
                        }
                ...
                if ($productProvider = CSaleBasket::GetProductProvider($arItem)) {
                    ...
                    if ($res["RESULT"]) {
                        ...
                        if (!$bUndoDeduction && $storesCount > 0) {
                            if ($bAutoDeduction) {
                                $arStoreBarcodeFields = array(
                                    ...
                                    "QUANTITY" => $arItem["QUANTITY"],
                                    "CREATED_BY" => ((intval($GLOBALS["USER"]->GetID()) > 0) ? intval(
                                        $GLOBALS["USER"]->GetID()
                                    ) : ""),
                                    "MODIFIED_BY" => ((intval($GLOBALS["USER"]->GetID()) > 0) ? intval(
                                        $GLOBALS["USER"]->GetID()
                                    ) : ""),
                                );
                                ...
                                if (defined("SALE_DEBUG") && SALE_DEBUG) {
                                    CSaleHelper::WriteToLog(
                                        ...
                                        array("arStoreBarcodeFields" => $arStoreBarcodeFields),
                                        "OD11"
                                    );
                                }
                        ...
                        if ($bUndoDeduction) {
                            ...
                            if (CSaleBasketHelper::isSetItem($arItem) && CSaleBasketHelper::isSetDeducted(
                                    $arItem["SET_PARENT_ID"]
                                )) {
                        ...
                        if (defined("SALE_DEBUG") && SALE_DEBUG) {
                            CSaleHelper::WriteToLog(
                                "Call ::DeductProduct - Success (DEDUCTED = " . $tmpRes . ")",
                                array(),
                                "OD11"
                            );
                        }
                ...
                if ($basketFromFUser->count() > 0) {
                    ...
                    foreach ($basketFromFUser as $basketItemFrom) {
                        ...
                        if ($basketItem) {
                            $basketItem->setField(
                                'QUANTITY',
                                $basketItem->getQuantity() + $basketItemFrom->getQuantity()
                            );
        ...
        if ($isOrderConverted != 'N') {
            ...
            if (!empty($basketItems) && is_array($basketItems)) {
                ...
                foreach ($basketItems as $basketItem) {
                    ...
                    if (strval(trim($basketItem['PRODUCT_PROVIDER_CLASS'])) !== '' || strval(
                            trim($basketItem['CALLBACK_FUNC'])
                        ) !== '') {
                        ...
                        if ($productProvider = CSaleBasket::GetProductProvider($basketItem)) {
                            $fields = $productProvider::GetProductData(
                                array(
                        ...
                        if (!empty($fields) && is_array($fields)) {
                            if ($isOrderConverted != 'N' && $basketItem['DELAY'] == 'N') {
                                ...
                                if (Sale\Compatible\DiscountCompatibility::usedByClient()) {
                                    Sale\Compatible\DiscountCompatibility::setBasketItemData(
                                        $basketItem['ID'],
                                        $fields
                                    );
                                }
                    ...
                    if (array_key_exists('MEASURE_RATIO', $basketItem)) {
                        ...
                        if ($mod != 0) {
                            $fields['QUANTITY'] = floor(
                                    ceil($basketItemQuantity) / $basketItemRatio
                                ) * $basketItemRatio;
                        }
                    }
...
class CAllSaleUser
{
    ...
    public static function GetAnonymousUserID()
    {
        ...
        if ($anonUserID > 0) {
            $dbUser = CUser::GetList(
                'id',
                'asc',
                array("ID_EQUAL_EXACT" => $anonUserID),
    ...
    {
        global $USER;
        if ((string)$_SESSION["SALE_USER_ID"] !== "" && intval(
