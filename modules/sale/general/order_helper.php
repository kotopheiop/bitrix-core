    ...
    public static function checkQuantity($arProducts)
    {
        ...
        foreach ($arProducts as $id => $arProduct) {
            ...
            if (!empty($arProduct["STORES"]) && is_array($arProduct["STORES"])) {
                ...
                $GLOBALS["APPLICATION"]->ThrowException(
                    GetMessage("NEWO_ERR_STORE_WRONG_INFO", array("#PRODUCT_NAME#" => $arProduct["NAME"])) . "<br>"
                );
                $result = false;
                break;
            }
                    ...
                    if (
                        ...
                        $GLOBALS["APPLICATION"]->ThrowException(
                            GetMessage(
                                ...
                                array("#PRODUCT_NAME#" => $arProduct["NAME"], "#STORE_ID#" => $arStore["STORE_ID"])
                            ) . "<br>"
                        );
                        $result = false;
                        break 2;
