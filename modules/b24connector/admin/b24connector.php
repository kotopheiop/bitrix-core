        ...
        <div class="connector-content">
            ...
            <div class="connector-video-container">
                <div class="connector-video-block">
                    <iframe class="connector-video" src="https://www.youtube.com/embed/bhKF6cq2E2M?rel=0"
                    ...
                    <? if (Connection::isExist()): ?>
                        ...
                        <? if ($moduleAccess > "R"): ?>
                            <a href="javascript:void(0)"
                               onclick="if(confirm('<?= Loc::getMessage('B24C_B24C_DEL_CONFIRM') ?>')) window.location.href='?lang=<?= LANGUAGE_ID ?>&action=delete_connection&<?= bitrix_sessid_get() ?>';"
                               class="connector-newbtx-link"><?= Loc::getMessage('B24C_B24C_DEL') ?></a>
                        <? endif; ?>
                    <? else: ?>
                        <a href="https://www.bitrix24.<? if (LANGUAGE_ID == "ru") echo "ru"; elseif (LANGUAGE_ID == "de") echo "de"; else echo "com"; ?>/"
                           class="connector-btn-green"><?= Loc::getMessage('B24C_B24C_CREATE') ?></a>
                        <p class="connector-newbtx-text"><?= Loc::getMessage('B24C_B24C_OR') ?></p>
                        <?= Connection::getButtonHtml() ?>
                    <? endif ?>
                </div>
                <div class="connector-newbtx__item connector-newbtx__item-1">
                    <span><?= Loc::getMessage('B24C_B24C_ITEM1') ?></span></div>
                <div class="connector-newbtx__item connector-newbtx__item-2">
                    <span><?= Loc::getMessage('B24C_B24C_ITEM2') ?></span></div>
                <div class="connector-newbtx__item connector-newbtx__item-3">
                    <span><?= Loc::getMessage('B24C_B24C_ITEM3') ?></span></div>
                <div class="connector-newbtx__item connector-newbtx__item-4">
                    <span><?= Loc::getMessage('B24C_B24C_ITEM4') ?></span></div>
                <div class="connector-newbtx__item connector-newbtx__item-5">
                    <span><?= Loc::getMessage('B24C_B24C_ITEM5') ?></span></div>
                <div class="connector-newbtx__item connector-newbtx__item-6">
                    <span><?= Loc::getMessage('B24C_B24C_ITEM6') ?></span></div>
                <div class="connector-newbtx__item connector-newbtx__item-7">
                    <span><?= Loc::getMessage('B24C_B24C_ITEM7') ?></span></div>
            </div>
        </div>
    </div>
