...
class Landing extends \Bitrix\Landing\Internals\BaseTable
{
    ...
    public static function markDelete($id)
    {
        ...
        $event = new Event(
            'landing', 'onBeforeLandingRecycle', array(
            'id' => $id,
            'delete' => 'Y'
        )
        );
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0, $c = count($matches[0]); $i < $c; $i++) {
                if (mb_strtoupper($matches[2][$i]) == 'LANDING') {
                    ...
                    if (mb_strtoupper($matches[2][$i]) == 'DYNAMIC') {
    ...
    public function unpublic()
    {
        ...
        if ($res->isSuccess()) {
            ...
            // send event
            $event = new Event(
                'landing', 'onLandingAfterUnPublication', array(
                'id' => $this->getId()
            )
            );
            $event->send();
    ...
    protected function changeParentOfBlock($block, $params)
    {
        ...
        if ($this->exist() && $fromLanding->exist()) {
            ...
            if ($move) {
                ...
                if (isset($fromLandingBlocks[$block])) {
    ...
    {
        $blockId = $this->changeParentOfBlock(
            $id,
            array(
                'MOVE' => false,
                'AFTER_ID' => $afterId
