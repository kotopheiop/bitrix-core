...
class Designer
{
                    ...
                    foreach ($repoItem['MANIFEST'][$category] as $selector => $item) {
                        ...
                        if (!isset($selectorName[$selector])) {
                            $nodeTitle = (isset($item['type']) && is_string($item['type']))
                                ? $item['type']
                                : $selector;
                            $selectorName[$selector] = Loc::getMessage(
                                'LANDING_DESIGNER_NODE_' . mb_strtoupper($nodeTitle)
                            );
                        }
    ...
    public static function parseManifest(string $content): array
    {
        ...
        if (preg_match_all(
            '/[\s"]+((' . implode('|', array_keys($references)) . ')[-\d]*)[\s"]+/is',
            $content,
            $matches
        )) {
    ...
    public static function getRepository(bool $installRepo = true): array
    {
        ...
        foreach (self::getRepo() as $row) {
            $repo[] = [
                'name' => $row['TITLE'] ?: Loc::getMessage(
                    'LANDING_DESIGNER_REPO_ELEM_' . mb_strtoupper($row['XML_ID'])
                ),
                'code' => $row['XML_ID'],
                'html' => $row['HTML'],
