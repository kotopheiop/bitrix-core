...
class Lazyload
{
    ...
    protected function parseImgTag(DOM\Element $node, string $selector): void
    {
        ...
        if (
            ...
            if ($manifestSize = $this->getPlaceholderSizeFromManifest($selector)) {
    ...
    protected function createPlaceholderImage(int $width, int $height): string
    {
        return "data:image/svg+xml;base64," . base64_encode(
                '<svg xmlns="http://www.w3.org/2000/svg" width="' . $width . '" height="' . $height . '"><rect id="backgroundrect" width="100%" height="100%" x="0" y="0" fill="#ddd" fill-opacity=".7" stroke="none"/></svg>'
            );
