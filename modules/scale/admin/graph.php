...
if (\Bitrix\Scale\Helper::checkBxEnvVersion()) {
    if (\Bitrix\Scale\Monitoring::isEnabled()) {
        ...
        <form name="form1" method="GET" action="<?= $APPLICATION->GetCurPage() ?>?">
            <? $oFilter->Begin(); ?>
            <tr valign="center">
                <td class="bx-digit-cell" width="0%" nowrap><? echo Loc::getMessage("SCALE_GRAPH_SELECT_SERVER") ?>:
                </td>
                <td width="0%" nowrap>
            ...
            <tr valign="center">
                <td class="bx-digit-cell" width="0%" nowrap><? echo Loc::getMessage("SCALE_GRAPH_SELECT_CATEGORY") ?>:
                </td>
                <td width="0%" nowrap>
            ...
            <tr valign="center">
                <td class="bx-digit-cell" width="0%" nowrap><? echo Loc::getMessage("SCALE_GRAPH_SELECT_PERIOD") ?>:
                </td>
                <td width="0%" nowrap>
            ...
            <span class="adm-btn-wrap"><input type="submit" class="adm-btn" name="set_filter"
                                              value="<?= Loc::getMessage("SCALE_GRAPH_SHOW") ?>"
                                              title="<?= Loc::getMessage("SCALE_GRAPH_SHOW") ?>"></span>
            <?
            $oFilter->End();
            ?>
        </form>
            ...
            if (BX.Scale.graphCategoriesList[srv.value]) {
                ...
                for (var i in BX.Scale.graphCategoriesList[srv.value]) {
                    var option = BX.create('option', {
                        props: {
                            value: i,
                            text: BX.Scale.graphCategoriesList[srv.value][i]
                        }
                    });
                    gCats.add(option);
                }
