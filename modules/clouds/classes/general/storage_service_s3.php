...
class CCloudStorageService_S3 extends CCloudStorageService
{
    ...
    function SendRequest(
        ...
        $params = '',
        $content = '',
        $additional_headers =/*.(array[string]string).*/ array()
    ) {
        global $APPLICATION;
        $this->status = 0;
    ...
    function FileCopy($arBucket, $arFile, $filePath)
    {
        ...
        $additional_headers["x-amz-copy-source"] = CCloudUtil::URLEncode(
        ...
        if ($this->status == 200) {
            ...
            && is_object($e)
            && preg_match(
                "/The specified copy source is larger than the maximum allowable size for a copy source: ([0-9]+)/i",
                $e->GetString(),
                $match
            )
        ) {
            ...
            $this->SendRequest(
                ...
                CCloudUtil::URLEncode(
                    "/" . ($arBucket["PREFIX"] ? $arBucket["PREFIX"] . "/" : "") . ($arFile["SUBDIR"] ? $arFile["SUBDIR"] . "/" : "") . $arFile["FILE_NAME"],
                    "UTF-8",
                    true
                )
            );
            ...
            while ($pos < $fileSize) {
                ...
                $additional_headers["x-amz-copy-source-range"] = "bytes=" . $pos . "-" . (min(
                            $fileSize,
                            $pos + $sizeLimit
                        ) - 1);
    ...
    function SaveFile($arBucket, $filePath, $arFile)
    {
        ...
        $additional_headers["Content-Length"] = (array_key_exists("content", $arFile) ? CUtil::BinStrlen(
            $arFile["content"]
    ...
    function ListFiles($arBucket, $filePath, $bRecursive = false, $pageSize = 0, $pageMarker = '')
    {
        ...
        while (true) {
            $response = $this->SendRequest(
                ...
                $arBucket["BUCKET"],
                '/',
                '?' . ($bRecursive ? '' : 'delimiter=%2F&') . 'prefix=' . str_replace(
                    $search,
                    $replace,
                    urlencode($filePath)
                )
