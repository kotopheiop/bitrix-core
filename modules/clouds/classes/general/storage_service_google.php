...
class CCloudStorageService_GoogleStorage extends CCloudStorageService
{
    ...
    function GetSettingsHTML($arBucket, $bServiceSet, $cur_SERVICE_ID, $bVarsFromForm)
    {
		...
		<tr id="SETTINGS_0_' . $htmlID . '" style="display:' . ($cur_SERVICE_ID == $this->GetID(
			...
			<td><input type="hidden" name="SETTINGS[' . $htmlID . '][PROJECT_ID]" id="' . $htmlID . 'PROJECT_ID" value="' . htmlspecialcharsbx(
                $arSettings['PROJECT_ID']
            ) . '"><input type="text" size="55" name="' . $htmlID . 'INP_" id="' . $htmlID . 'INP_PROJECT_ID" value="' . htmlspecialcharsbx(
                $arSettings['PROJECT_ID']
            ) . '" ' . ($arBucket['READ_ONLY'] == 'Y' ? '"disabled"' : '') . ' onchange="BX(\'' . $htmlID . 'PROJECT_ID\').value = this.value"></td>
		</tr>
		...
		<tr id="SETTINGS_2_' . $htmlID . '" style="display:' . ($cur_SERVICE_ID == $this->GetID(
            ) || !$bServiceSet ? '' : 'none') . '" class="settings-tr adm-detail-required-field">
		...
		<tr id="SETTINGS_3_' . $htmlID . '" style="display:' . ($cur_SERVICE_ID == $this->GetID(
            ) || !$bServiceSet ? '' : 'none') . '" class="settings-tr">
			<td>&nbsp;</td>
    ...
    function CheckSettings($arBucket, &$arSettings)
    {
        ...
        if ($arBucket["READ_ONLY"] !== "Y" && !mb_strlen($result["PROJECT_ID"])) {
            $aMsg[] = array(
                "id" => $this->GetID() . "INP_PROJECT_ID",
                "text" => GetMessage("CLO_STORAGE_GOOGLE_EMPTY_PROJECT_ID")
            );
        }
        ...
        if ($arBucket["READ_ONLY"] !== "Y" && !mb_strlen($result["ACCESS_KEY"])) {
            $aMsg[] = array(
                ...
                "text" => GetMessage("CLO_STORAGE_GOOGLE_EMPTY_ACCESS_KEY")
            );
        }
        ...
        if ($arBucket["READ_ONLY"] !== "Y" && !mb_strlen($result["SECRET_KEY"])) {
            $aMsg[] = array(
                ...
                "text" => GetMessage("CLO_STORAGE_GOOGLE_EMPTY_SECRET_KEY")
            );
        }
    ...
    function FileCopy($arBucket, $arFile, $filePath)
    {
        ...
        $response = $this->SendRequest(
            ...
            array(
                ...
                "x-goog-copy-source" => CCloudUtil::URLEncode(
    ...
    function SaveFile($arBucket, $filePath, $arFile)
    {
        ...
        $response = $this->SendRequest(
            ...
            array(
                ...
                "Content-Length" => (array_key_exists("content", $arFile) ? CUtil::BinStrlen(
                    $arFile["content"]
                ) : filesize($arFile["tmp_name"])),
            )
        );
    ...
    function ListFiles($arBucket, $filePath, $bRecursive = false)
    {
        ...
        while (true) {
            $response = $this->SendRequest(
                ...
                $arBucket["BUCKET"],
                '/',
                '?' . ($bRecursive ? '' : 'delimiter=/&') . 'prefix=' . urlencode($filePath) . '&marker=' . urlencode(
                    $marker
                )
            );
            if (
    ...
    function UploadPartNo($arBucket, &$NS, $data, $part_no)
    {
        ...
        if (
            ...
            || (($NS["Parts"][$found]["part_no"] * $this->GetMinUploadPartSize() + $this->GetMinUploadPartSize(
                    )) >= $NS["fileSize"])
        ) {
            $data_len = CUtil::BinStrlen($data);
                    ...
                    if (is_array($arXML) && is_string($arXML["Error"]["#"]["Message"][0]["#"])) {
                        $APPLICATION->ThrowException(
                            GetMessage(
                                ...
                                array('#errmsg#' => trim($arXML["Error"]["#"]["Message"][0]["#"], '.'))
                            )
                        );
                        return false;
                    }
