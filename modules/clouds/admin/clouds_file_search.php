...
while (is_array($arRes = $rsData->NavNext())) {
    if ($arRes["TYPE"] === "dir") {
        if ($arRes["NAME"] === "..") {
            $link = 'clouds_file_search.php?lang=' . LANGUAGE_ID . '&n=' . urlencode(
                    $n
                ) . '&bucket=' . $obBucket->ID . '&path=' . urlencode(preg_replace('#([^/]+)/$#', '', $path));
        } else {
            $link = 'clouds_file_search.php?lang=' . LANGUAGE_ID . '&n=' . urlencode(
                    $n
                ) . '&bucket=' . $obBucket->ID . '&path=' . urlencode($path . $arRes["NAME"] . '/');
        }
    } else {
        $link = 'clouds_file_search.php?lang=' . LANGUAGE_ID . '&n=' . urlencode(
                $n
            ) . '&file=y&bucket=' . $obBucket->ID . '&path=' . urlencode($path . $arRes["NAME"]);
    }
    ...
    $showFieldText = "";
    if ($arRes["TYPE"] === "dir") {
        $showFieldIcon = '<a href="' . htmlspecialcharsbx(
                $link
            ) . '"><span id="fileman_menu_icon_sections" class="adm-submenu-item-link-icon"></span></a>';
            ...
            <td><select name="bucket">
                    ...
                    <? foreach ($arBuckets as $id => $name): ?>
                        <option value="<? echo htmlspecialcharsbx(
                            $id
                        ) ?>" <? if ($id == $bucket) echo "selected" ?>><? echo htmlspecialcharsex($name) ?></option>
                    <? endforeach ?>
                </select></td>
        </tr>
