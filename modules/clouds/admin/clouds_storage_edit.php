...
?>
    <script>
        ...
        function editAddRule() {
            ...
            oCell1.innerHTML = '<input name="MODULE[]" type="text" style="width:100%">';
            var oCell2 = oRow.insertCell(1);
            oCell2.innerHTML = '<input name="EXTENSION[]" type="text" style="width:100%">';
            var oCell3 = oRow.insertCell(2);
            oCell3.innerHTML = '<input name="SIZE[]" type="text" style="width:100%">';
            var oCell4 = oRow.insertCell(3);
            oCell4.innerHTML = '<img src="/bitrix/themes/.default/images/actions/delete_button.gif" onclick="editDeleteRule(this) "/>';
    ...
    <form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>" enctype="multipart/form-data" name="editform"
        ...
        <tr>
            ...
            <td width="60%"><input type="hidden" name="ACTIVE" value="N"><input type="checkbox" name="ACTIVE"
                                                                                value="Y"<? if ($arRes["ACTIVE"] === "Y") echo " checked" ?>>
            </td>
        </tr>
        <tr>
        ...
        <? if ($ID > 0) {
            ...
            <tr>
                <td><?
                    echo GetMessage("CLO_STORAGE_EDIT_SERVICE_ID") ?>:
                </td>
                <td>
                    <?
                    ...
                    echo htmlspecialcharsbx($arRes["SERVICE_ID"]); ?>">
                </td>
            </tr>
            <tr id="LOCATION_<?
            echo htmlspecialcharsbx($arRes["SERVICE_ID"]) ?>" class="location-tr">
                <td><?
                ...
                <td>
                    ...
                    echo htmlspecialcharsbx($arRes["SERVICE_ID"]); ?>]" value="<?
                    echo htmlspecialcharsbx($arRes["LOCATION"]); ?>">
                </td>
            </tr>
            <?
        ...
        <?
                ...
                <td>
                    <select name="SERVICE_ID" onchange="ChangeLocation(this)">
                        ...
                        foreach (CCloudStorage::GetServiceList() as $SERVICE_ID => $obService) {
                            ...
                            if ($arRes["SERVICE_ID"] === $SERVICE_ID) echo " selected" ?>><?
                            echo htmlspecialcharsex($obService->GetName()) ?></option><?
                            if ($arRes["SERVICE_ID"] === $SERVICE_ID)
                                $bServiceSet = true;
                        }
                        ?>
            ...
            <? foreach (CCloudStorage::GetServiceList() as $SERVICE_ID => $obService) {
                ?>
                <tr id="LOCATION_<? echo htmlspecialcharsbx($SERVICE_ID) ?>"
                    style="display:<? echo $arRes["SERVICE_ID"] === $SERVICE_ID || !$bServiceSet ? "" : "none" ?>"
                    class="location-tr">
                    ...
                    <td>
                        <select name="LOCATION[<? echo htmlspecialcharsbx($SERVICE_ID) ?>]">
                            ...
                            foreach (CCloudStorage::GetServiceLocationList($SERVICE_ID) as $LOCATION_ID => $LOCATION_NAME) {
                                ...
                                if ($arRes["SERVICE_ID"] === $LOCATION_ID) echo " selected" ?>><?
                                echo htmlspecialcharsex($LOCATION_NAME) ?></option><?
                            }
                            ?>
                        </select>
                    </td>
        ...
        <? if ($ID > 0) {
            ...
            <tr class="adm-detail-required-field">
                <td><?
                    echo GetMessage("CLO_STORAGE_EDIT_BUCKET") ?>:
                </td>
                <td><input type="hidden" name="BUCKET" value="<?
                    echo htmlspecialcharsbx($arRes["BUCKET"]) ?>"><?
                    echo htmlspecialcharsex($arRes["BUCKET"]) ?></td>
            </tr>
            <?
        } else {
            ?>
            <tr class="adm-detail-required-field">
                ...
                <td><input type="text" size="55" name="BUCKET" value="<? echo htmlspecialcharsbx($arRes["BUCKET"]) ?>">
                </td>
            </tr>
        <? } ?>
        <tr>
            <td><? echo GetMessage("CLO_STORAGE_EDIT_READ_ONLY") ?>:</td>
            <td><input type="hidden" name="READ_ONLY" value="N"><input type="checkbox" name="READ_ONLY"
                                                                       value="Y"<? if ($arRes["READ_ONLY"] === "Y") echo " checked" ?>>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td align="center">
                ...
                <table border="0" cellspacing="0" cellpadding="0" class="internal" align="center" id="tblRULES">
                    <tr class="heading">
                        <td><? echo GetMessage("CLO_STORAGE_EDIT_MODULE") ?><sup><span class="required">1</span></sup>
                        </td>
                        <td><? echo GetMessage("CLO_STORAGE_EDIT_EXTENSIONS") ?><sup><span
                                        class="required">2</span></sup></td>
                        <td><? echo GetMessage("CLO_STORAGE_EDIT_SIZE") ?><sup><span class="required">3</span></sup>
                        </td>
                        <td>&nbsp;</td>
                    </tr>
                    <?
                    ...
                    foreach ($arRules as $rule) {
                        ?>
                        <tr>
                            <td><input name="MODULE[]" type="text" value="<?
                                echo htmlspecialcharsbx($rule["MODULE"]) ?>" style="width:100%"></td>
                            <td><input name="EXTENSION[]" type="text" value="<?
                                echo htmlspecialcharsbx($rule["EXTENSION"]) ?>" style="width:100%"></td>
                            <td><input name="SIZE[]" type="text" value="<?
                                echo htmlspecialcharsbx($rule["SIZE"]) ?>" style="width:100%"></td>
                            <td><img src="/bitrix/themes/.default/images/actions/delete_button.gif"
                                     onclick="editDeleteRule(this)"/></td>
                        </tr>
                        <?
                        $ii++;
                    }
                ...
                <br>
                <a class="adm-btn" href="javascript:void(0)" onclick="editAddRule(this)" hidefocus="true"
                   class="bx-action-href"><? echo GetMessage("CLO_STORAGE_EDIT_ADD_FILE_RULE") ?></a>
            </td>
        </tr>
        <tr>
            <td>
        ...
        if (CCloudFailover::IsEnabled()) {
            ...
            <tr>
                <td width="40%"><?
                    echo GetMessage("CLO_STORAGE_EDIT_FAILOVER_ACTIVE") ?>:
                </td>
                <td width="60%">
                    <input type="hidden" name="FAILOVER_ACTIVE" value="N">
                    <input type="checkbox" name="FAILOVER_ACTIVE" value="Y"<?
                    if ($arRes["FAILOVER_ACTIVE"] === "Y") echo " checked" ?>>
                </td>
            </tr>
            <tr valign="top">
                <td><?
                    echo GetMessage("CLO_STORAGE_EDIT_FAILOVER_BUCKET_ID") ?>:
                </td>
                <td>
                    <select name="FAILOVER_BUCKET_ID">
                        ...
                        while ($arBucket = $rsBucketList->Fetch()) {
                            ...
                            ?>
                            <option value="<?
                            echo htmlspecialcharsbx($arBucket["ID"]) ?>"<?
                            if ($arRes["FAILOVER_BUCKET_ID"] === $arBucket["ID"]) echo " selected" ?>><?
                            echo htmlspecialcharsex($arBucket["BUCKET"]) ?></option><?
                        }
                        ?>
                    </select>
                    <?
            ...
            <tr>
                <td width="40%"><?
                    echo GetMessage("CLO_STORAGE_EDIT_FAILOVER_COPY") ?>:
                </td>
                <td width="60%">
                    <input type="hidden" name="FAILOVER_COPY" value="N">
                    <input type="checkbox" name="FAILOVER_COPY" value="Y"<?
                    if ($arRes["FAILOVER_COPY"] === "Y") echo " checked" ?>>
                </td>
            </tr>
            <tr>
                <td width="40%"><?
                    echo GetMessage("CLO_STORAGE_EDIT_FAILOVER_DELETE") ?>:
                </td>
                <td width="60%">
                    <input type="hidden" name="FAILOVER_DELETE" value="N">
                    <input type="checkbox" name="FAILOVER_DELETE" value="Y"<?
                    if ($arRes["FAILOVER_DELETE"] === "Y") echo " checked" ?>>
                </td>
            </tr>
            <tr>
                <td><?
                    echo GetMessage("CLO_STORAGE_EDIT_FAILOVER_DELETE_DELAY") ?>:
                </td>
                <td>
                    <?
            ...
            <tr>
                <td width="40%"><?
                    echo GetMessage("CLO_STORAGE_EDIT_FAILOVER_SYNC") ?>:
                </td>
                <td width="60%">
                    <?
