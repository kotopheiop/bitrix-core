...
class CSupportTableFields
{
    ...
    static function ConvertForSQL($type, $value)
    {
        ...
        switch ($type) {
            ...
            case self::VT_DATE:
                $sf = "SHORT";
            case self::VT_DATE_TIME:
                return (is_int($value) ? $DB->CharToDateFunction(GetTime($value, $sf)) : $value);
        }
    }
    ...
    static function ConvertForHTML($type, $place, $value, $op)
    {
        switch ($type) {
            case self::VT_DATE:
                return GetTime($value, "SHORT");
            case self::VT_DATE_TIME:
                return GetTime($value, "FULL");
            case self::VT_STRING:
                break;
            default:
                return $value;
        ...
        switch ($place) {
            ...
            case self::JS_HREF_ALERT:
                return urlencode(CUtil::addslashes($value));
            case self::JS_IN_QUOTES:
                return CUtil::JSEscape($value);
            case self::JS_EVENT:
                return CUtil::addslashes(htmlspecialcharsbx($value));
            case self::ATTRIBUTE:
                return htmlspecialcharsbx($value);
            case self::ATTRIBUTE_EX:
