...
$arHeaders[] = Array(
    'id' => 'XML_ID',
    'content' => GetMessage('SUP_GL_XML_ID'),
    'default' => false,
    'sort' => 'XML_ID'
);
$arHeaders[] = Array(
    'id' => 'IS_TEAM_GROUP',
    'content' => GetMessage('SUP_GL_IS_TEAM_GROUP'),
    'default' => true,
    'sort' => 'IS_TEAM_GROUP'
);
...
while ($arGroup = $rsData->GetNext()) {
    ...
    $row->AddViewField(
        'NAME',
        '<a href="' . $EDIT_URL . '?lang=' . LANGUAGE_ID . '&amp;ID=' . $arGroup['ID'] . '">' . $arGroup['NAME'] . '</a>'
    );
    ...
    $arActions[] = array(
        ...
        'ACTION' => 'if(confirm(\'' . GetMessage('SUP_GL_DELETE_CONFIRMATION') . '\')) ' . $lAdmin->ActionDoGroup(
                $arGroup['ID'],
                'delete'
            ),
    );
...
<form name="form1" method="GET" action="<?= $APPLICATION->GetCurPage() ?>?"><?
    ...
    <tr>
        ...
        <td><input type="text" name="FIND_NAME" size="47" value="<?= htmlspecialcharsbx($FIND_NAME) ?>"><?= InputType(
                ...
                false,
                "",
                "title='" . GetMessage('SUP_GL_EXACT_MATCH') . "'"
    ...
    <tr>
        ...
        <td><?
            $arr = array(
                "reference" => array(GetMessage("SUP_GL_FLT_SUPPORT"), GetMessage("SUP_GL_FLT_CLIENT")),
