...
{
    if (mb_strlen($_POST["MINUTE_FROM_" . $i . "_" . $j]) < 0 && mb_strlen(
            $_POST["MINUTE_TILL_" . $i . "_" . $j]
        ) < 0) {
        return false;
    }
...
if (($save <> '' || $apply <> '') && $bAdmin == "Y" && $_SERVER["REQUEST_METHOD"] == "POST" && check_bitrix_sessid()) {
    ...
    if (intval($ID) <= 0) {
        ...
        if ($save <> '') {
            ...
            LocalRedirect(
                $EDIT_URL . "?ID=" . $ID . "&lang=" . LANGUAGE_ID . "&tabControl_active_tab=" . urlencode(
...
$APPLICATION->SetTitle(
    ($ID > 0 ? GetMessage("SUP_PAGE_TITLE_EDIT_RECORD", array("#ID#" => $ID)) : GetMessage("SUP_PAGE_TITLE_NEW_RECORD"))
);
    ...
    if ($ID > 1 && $bAdmin == "Y") {
        $aMenu[] = array(
            ...
            "LINK" => "javascript:if(confirm('" . GetMessage(
                ...
                ) . "'))window.location='" . $LIST_URL . "?action=delete&ID=" . $ID . "&lang=" . LANGUAGE_ID . "&" . bitrix_sessid_get(
                ) . "';",
        );
    }
}
...
<script type="text/javascript">
    ...
    arCategory["<?=$sid?>"] = Array(<?
        echo "0";
        foreach ($arr as $ar) {
            echo "," . addslashes(htmlspecialcharsbx($ar["REFERENCE_ID"]));
        }
        ?>);
    <?
    endif;
...
<form name="form1" method="POST" action="<?= $APPLICATION->GetCurPage() ?>?ID=<?= $ID ?>&lang=<?= LANGUAGE_ID ?>"
    ...
    $aTabs[] = array(
        ...
        "ICON" => "ticket_edit",
        "TITLE" => GetMessage("SUP_RECORD_S")
    );
    $aTabs[] = array(
        "DIV" => "edit2",
        "TAB" => GetMessage("SUP_USER_GROUPS_S"),
        "ICON" => "ticket_edit",
        "TITLE" => GetMessage("SUP_USER_ACCESS")
    );
    ...
    if ($OLD_FUNCTIONALITY == "Y") {
        $aTabs[] = array(
            "DIV" => "edit4",
            "TAB" => GetMessage("SUP_WHAT_ACCESSIBLE_S"),
            "ICON" => "ticket_edit",
            "TITLE" => GetMessage("SUP_WHAT_ACCESSIBLE")
        );
    } else {
        $aTabs[] = array(
            "DIV" => "edit4",
            "TAB" => GetMessage("SUP_WHAT_ACCESSIBLE_S_N"),
            "ICON" => "ticket_edit",
            "TITLE" => GetMessage("SUP_WHAT_ACCESSIBLE_N")
        );
    }
                    ...
                    if (intval($str_CREATED_USER_ID) > 0) :
                        if (!in_array($str_CREATED_USER_ID, array_keys($arrUSER))) {
                            ...
                            if ($arUser = $rsUser->Fetch()) {
                                $arrUSER[$arUser["ID"]] = $arUser;
                            }
                        } else {
                            $arUser = $arrUSER[$str_CREATED_USER_ID];
                        }
                        echo "&nbsp;&nbsp;[<a title=\"" . GetMessage(
                                "MAIN_ADMIN_MENU_EDIT"
                            ) . "\" href=\"/bitrix/admin/user_edit.php?ID=" . $str_CREATED_USER_ID . "\">" . $str_CREATED_USER_ID . "</a>]&nbsp;(" . htmlspecialcharsbx(
                                $arUser["LOGIN"]
                ...
                <td><?= $str_DATE_MODIFY_F ?><?
                    if (intval($str_MODIFIED_USER_ID) > 0) :
                        if (!in_array($str_MODIFIED_USER_ID, array_keys($arrUSER))) {
                            ...
                            $arUser = $arrUSER[$str_MODIFIED_USER_ID];
                        }
                        echo "&nbsp;&nbsp;[<a title=\"" . GetMessage(
                                "MAIN_ADMIN_MENU_EDIT"
                            ) . "\" href=\"/bitrix/admin/user_edit.php?ID=" . $str_MODIFIED_USER_ID . "\">" . $str_MODIFIED_USER_ID . "</a>]&nbsp;(" . htmlspecialcharsbx(
                                $arUser["LOGIN"]
                            ) . ") " . htmlspecialcharsbx($arUser["NAME"]) . " " . htmlspecialcharsbx(
                                $arUser["LAST_NAME"]
                            );
        ...
        <td>
            <div class="adm-list">
                <div class="adm-list-item">
                    <div class="adm-list-control"><input name="arSITES[]" type="checkbox" id="all_sites" value="ALL"
                                                         onclick="OnSelectAll(this.checked, 'arSITES[]')" <? if ((is_array(
                                    $arSITES
                                ) && (in_array('ALL', $arSITES) || count($arSITES) <= 0)) || !is_array(
                                $arSITES
                            )) echo "checked" ?>></div>
                    <div class="adm-list-label"><label for="all_sites">(<?= GetMessage("SUP_ALL_CURRENT_FUTURE") ?>
                            )</label></div>
                </div>
                <?
                foreach ($arrSites as $sid => $arrS):
                    $checked = ((is_array($arSITES) && in_array(
                                $sid,
                                $arSITES
                    ...
                    ?>
                    <div class="adm-list-item">
                        <div class="adm-list-control"><input onClick="OnSiteClick()" type="checkbox" name="arSITES[]"
                                                             value="<?= htmlspecialcharsex($sid) ?>"
    ...
    <tr>
        <td colspan="2" align="center"><textarea style="width:60%; height:150px;" name="DESCRIPTION"
                                                 wrap="VIRTUAL"><? echo $str_DESCRIPTION ?></textarea></td>
    </tr>
            ...
            <div class="adm-list"><?
                ...
                while ($ar = $rs->Fetch()):
                    ...
                    if (in_array(CTicket::GetSupportClientRoleID(), $arRoles) ||
                        ...
                        echo "<div class=\"adm-list-control\">" . InputType(
                                "checkbox",
                                "arGROUPS[]",
                                $ar["ID"],
                                $arGROUPS,
                            ...
                            "<div class=\"adm-list-label\"><label for=\"$idR\">" . htmlspecialcharsbx(
                                $ar["NAME"]
                            ) . "</label>" .
                            " [<a title=\"" . GetMessage(
                                "MAIN_ADMIN_MENU_EDIT"
        ...
        <td width="60%" nowrap>
            <div class="adm-list">
                <div id="category_0" class="adm-list-item">
                    <div class="adm-list-control"><input name="arCATEGORIES[]" type="checkbox" value="0"
                                                         id="category_chbox_0"
                                                         onclick="OnSelectAll(this.checked, 'arCATEGORIES[]')" <? if ((is_array(
                                    $arCATEGORIES
                                ) && (in_array(0, $arCATEGORIES) || count($arCATEGORIES) <= 0)) || !is_array(
                                $arCATEGORIES
                            )) echo "checked" ?>></div>
                    <div class="adm-list-label"><label for="category_chbox_0">(<?= GetMessage(
                                "SUP_ALL_CURRENT_FUTURE"
                            ) ?>)</label></div>
                </div><?
                foreach ($arCategory as $ar) {
                    ...
                    echo "<div class=\"adm-list-control\">" . InputType(
                        ...
                        "<div class=\"adm-list-label\"><label for=\"category_chbox_" . $idR . "\">" . htmlspecialcharsbx(
                            $ar["NAME"]
                        ) . "</label>" .
                        " [<a title=\"" . GetMessage(
                            "MAIN_ADMIN_MENU_EDIT"
                        ) . "\" href=\"/bitrix/admin/ticket_dict_edit.php?ID=" . intval(
                            $ar["ID"]
                        ) . "&find_type=C&lang=" . LANGUAGE_ID . "\">" .
                        intval($ar["ID"]) .
                        "</a>]</div>";
                    ?></div><?
                } ?>
            ...
            <td nowrap>
                <div class="adm-list">
                    <div class="adm-list-item" id="criticality_0">
                        <div class="adm-list-control"><input name="arCRITICALITIES[]" type="checkbox" value="0"
                                                             id="criticality_chbox_0"
                                                             onclick="OnSelectAll(this.checked, 'arCRITICALITIES[]')" <? if ((is_array(
                                        $arCRITICALITIES
                                    ) && (in_array(0, $arCRITICALITIES) || count($arCRITICALITIES) <= 0)) || !is_array(
                                    $arCRITICALITIES
                                )) echo "checked" ?>></div>
                        <div class="adm-list-label"><label for="criticality_chbox_0">(<?= GetMessage(
                                    "SUP_ALL_CURRENT_FUTURE"
                                ) ?>)</label></div>
                    </div>
                    <?
                    foreach ($arCriticality as $ar) {
                        ...
                        echo "<div class=\"adm-list-control\">" . InputType(
                                ...
                                "",
                                "",
                                "criticality_chbox_" . $idR
                            ) . "</div>" .
                            "<div class=\"adm-list-label\"><label for=\"criticality_chbox_" . $idR . "\">" . htmlspecialcharsbx(
                                $ar["NAME"]
                            ) . "</label>" .
                            " [<a title=\"" . GetMessage(
                                "MAIN_ADMIN_MENU_EDIT"
            ...
            <td nowrap>
                <div class="adm-list">
                    <div class="adm-list-item" id="mark_0">
                        <div class="adm-list-control"><input name="arMARKS[]" type="checkbox" value="0"
                                                             id="mark_chbox_0"
                                                             onclick="OnSelectAll(this.checked, 'arMARKS[]')" <? if ((is_array(
                                        $arMARKS
                                    ) && (in_array(0, $arMARKS) || count($arMARKS) <= 0)) || !is_array(
                                    $arMARKS
                                )) echo "checked" ?>></div>
                        <div class="adm-list-label"><label for="mark_chbox_0">(<?= GetMessage(
                                    "SUP_ALL_CURRENT_FUTURE"
                                ) ?>)</label></div>
                    </div>
                    <?
                    foreach ($arMark as $ar) {
                        ...
                        echo "<div class=\"adm-list-control\">" . InputType(
                            ...
                            "<div class=\"adm-list-label\"><label for=\"mark_chbox_" . $idR . "\">" . htmlspecialcharsbx(
                                $ar["NAME"]
                            ) . "</label>" .
                            " [<a title=\"" . GetMessage(
                                "MAIN_ADMIN_MENU_EDIT"
    ...
    <SCRIPT LANGUAGE="JavaScript">
        <!--
        OnSelectAll(<?echo ((is_array($arCATEGORIES) && (in_array(0, $arCATEGORIES) || count(
                        $arCATEGORIES
                    ) <= 0)) || !is_array($arCATEGORIES)) ? "true" : "false"?>, 'arCATEGORIES[]');
        OnSelectAll(<?echo ((is_array($arCRITICALITIES) && (in_array(0, $arCRITICALITIES) || count(
                        $arCRITICALITIES
                    ) <= 0)) || !is_array($arCRITICALITIES)) ? "true" : "false"?>, 'arCRITICALITIES[]');
        OnSelectAll(<?echo ((is_array($arMARKS) && (in_array(0, $arMARKS) || count($arMARKS) <= 0)) || !is_array(
                $arMARKS
            )) ? "true" : "false"?>, 'arMARKS[]');
        OnSelectAll(<?echo ((is_array($arSITES) && (in_array('ALL', $arSITES) || count($arSITES) <= 0)) || !is_array(
                $arSITES
            )) ? "true" : "false"?>, 'arSITES[]');
        OnSiteClick();
        //-->
