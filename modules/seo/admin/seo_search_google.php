            ...
            BX.ready(function () {
                ...
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    if (typeof res[domain] != 'undefined') {
                        switch (role) {
                            case 'site-binded':
                                nodes[i].innerHTML = res[domain]['binded'] == true ? '<?=CUtil::JSEscape(
        ...
        <div id="auth_result" class="seo-auth-result">
            ...
            <div style="width: 300px; padding: 10px 0 0 0;">
                ...
                if ($currentUser['picture']) {
                    ?>
                    <img src="<?= Converter::getHtmlConverter()->encode($currentUser['picture']) ?>"
                         style="float: left; margin: 0 13px 0 0; max-width: 55px;"/>
                    <?
                }
                ?>
                <a href="<?= Converter::getHtmlConverter()->encode($currentUser['profile']) ?>"
                   target="_blank"><?= Converter::getHtmlConverter()->encode($currentUser['name']); ?></a><br/><br/>
