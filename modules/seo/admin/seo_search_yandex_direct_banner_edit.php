...
?>
    ...
    echo $APPLICATION->GetCurPage() ?>?lang=<?= LANGUAGE_ID ?>&amp;ID=<?= $ID ?>&amp;campaign=<?= $campaignId ?>"
        ...
        <tr class="adm-detail-required-field">
            ...
            <td width="0" valign="top">
                <input type="text" name="SETTINGS[Title]" placeholder="<?= Loc::getMessage('SEO_BANNER_DATA_TITLE') ?>"
                       value="<?= HtmlFilter::encode($banner["SETTINGS"]["Title"]) ?>" id="title_content"
                       style="width: 250px;" maxlength="<?= Adv\YandexBannerTable::MAX_TITLE_LENGTH; ?>"
        ...
        <tr class="adm-detail-required-field">
            ...
            <td colspan="3">
                <input type="text" name="SETTINGS[Href]" value="<?= HtmlFilter::encode($banner["SETTINGS"]["Href"]) ?>"
                       id="link_content" style="width: 500px;" onkeyup="updateAdv()" onchange="updateAdv()"
                       onpaste="updateAdv()" tabindex="3">
        ...
        if (($ID > 0 || $elementId > 0) && Main\Loader::includeModule('iblock')) {
            ?>
            <tr class="heading">
                <td colspan="4"><?= Loc::getMessage("SEO_BANNER_SALE_SECTION") ?></td>
            </tr>
            <tr>
        ...
        <input type="hidden" id="geo_settings" name="SETTINGS[Geo]"
               value="<?= implode(',', $banner["SETTINGS"]["Geo"]) ?>"/>
        <tr>
        ...
        <script>
            ...
            BX.ready(function () {
                ...
                function checkRegionWithChildren(id, bChecked) {
                    ...
                    if (targetRegion) {
                        var subRegion = BX.findChildren(targetRegion, {
                            tag: 'INPUT',
                            class: 'region',
                ...
                function drawRegionWithChildren(id, bParentChecked, drawRes, valueRes) {
                    ...
                    if (targetRegionGroup) {
                        var subReg = BX.findChildren(targetRegionGroup, {
                            tag: 'INPUT',
                            class: 'region',
                            property: {type: 'checkbox'},
                            attribute: {'data-parent': id}
                        }, true);
                        if (subReg) {
                ...
                BX.bindDelegate(listCont, 'click', {
                    tag: 'INPUT',
                    class: 'region',
                    property: {type: 'checkbox'}
                }, onRegionClick);
...
//		phrases hint binds
                ...
                BX.bind(textInput, 'blur', function () {
                    hint.close();
...
//		minus keywords hint binds
                ...
                BX.bind(minusKWInput, 'blur', function () {
                    hintMinus.close();
                    clearTimeout(timeOut);
                });
        ...
        if ($bShowAuto) {
            ...
            <tr>
                ...
                <td width="40%">
                    ...
                    <input type="checkbox" name="AUTO_QUANTITY_OFF" id="AUTO_QUANTITY_OFF"
                           value="<?= Adv\YandexBannerTable::ACTIVE ?>"<?= $banner["AUTO_QUANTITY_OFF"] == Adv\YandexBannerTable::ACTIVE || $banner["AUTO_QUANTITY_OFF"] == Adv\YandexBannerTable::MARKED ? ' checked="checked"' : '' ?>>
            ...
            <tr>
                ...
                <td width="40%">
                    ...
                    <input type="checkbox" name="AUTO_QUANTITY_ON" id="AUTO_QUANTITY_ON"
                           value="<?= Adv\YandexBannerTable::ACTIVE ?>"<?= $banner["AUTO_QUANTITY_ON"] == Adv\YandexBannerTable::ACTIVE || $banner["AUTO_QUANTITY_ON"] == Adv\YandexBannerTable::MARKED ? ' checked="checked"' : '' ?>>
        ...
        if (!$bReadOnly):
            ...
            if ($back_url != ''):
                ...
                echo HtmlFilter::encode($back_url) ?>">
            <?
            endif;
            ?>
