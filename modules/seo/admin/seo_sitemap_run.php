    ...
    if ($v == $arValueSteps['init']) {
        ...
        if (!$bFinished) {
            ...
            $msg = Loc::getMessage(
                ...
                array('#FILE#' => $arSitemap['SETTINGS']['FILENAME_FILES'])
            );
        }
...
//							remove or replace SERVER_NAME
                                $url = SitemapIblock::prepareUrlToReplace(
            ...
            if ($v < $arValueSteps['forum']) {
                ...
                while (!$bFinished && microtime(true) <= $ts_finish && CModule::IncludeModule("forum")) {
                    ...
                    if (!$currentForum) {
                        ...
                        $bActive = (array_key_exists(
                                $forumId,
                                $arSitemap['SETTINGS']['FORUM_TOPIC']
                            ) && $arSitemap['SETTINGS']['FORUM_TOPIC'][$forumId] == "Y");
                        if ($bActive) {
    ...
    if ($v < $arValueSteps['index']) {
        ...
        if (isset($arExistedSitemaps) && count($arExistedSitemaps) > 0) {
            echo BeginNote(), Loc::getMessage(
                'SEO_SITEMAP_RUN_ROBOTS_WARNING',
                array(
