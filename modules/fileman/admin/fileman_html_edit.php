...
//$aMenu = array();
    ...
    $u->Show();
    ?>
    <form action="fileman_html_edit.php?lang=<?= LANG ?>" method="post" enctype="multipart/form-data"
          name="ffilemanedit" id="ffilemanedit">
        ...
        <? if (!$bEdit):?>
            <tr>
                <td><label for="bx_template"><?= GetMessage("FILEMAN_FILEEDIT_TEMPLATE") ?></label></td>
                <td>
            ...
            <tr>
                ...
                <td width="70%"><input type="text" id="title" name="title" size="60" maxlength="255"
        ...
        <tr>
            ...
            <td width="70%"><input type="text" id="title" name="title" size="60" maxlength="255"
                ...
                <input type="hidden" name="filename" id="filename"
                       value="<?= htmlspecialcharsbx($arParsedPath["LAST"]) ?>">
        ...
        <tr>
            <td colspan="2">
                ...
                function __FE_editorScripts($editorName, $arEditorParams)
                {
                    return array("JS" => array('html_edit_editor.js'));
                }
                        ...
                        for ($i = 0; $i < $cntProp; $i++) {
                            ...
                            <tr>
                                <td valign="top">
                                    ...
                                    <? else:?>
                                        <input type="text" name="CODE_<?= $i ?>" id="CODE_<?= $i ?>" value="<?
                                ...
                                <td>
                                    ...
                                    if ($APPLICATION->GetDirProperty($arProp["CODE"], Array($site, $path))) {
                                        ?><br><small><b><?= GetMessage("FILEMAN_FILE_EDIT_FOLDER_PROP") ?></b> <?
                                        echo htmlspecialcharsbx($APPLICATION->GetDirProperty($arProp["CODE"], Array($site, $path))); ?>
                                        </small><?
                                    } ?>
                                </td>
                            </tr>
        ...
        if ($USER->CanDoOperation('fileman_add_element_to_menu') && $USER->CanDoFileOperation('fm_add_to_menu', $arPath)):
            ...
            <tr>
                ...
                <td width="60%"><input type="checkbox" id="add_to_menu" name="add_to_menu" value="Y"
                                       onclick="__heAddToMenu()" <?
                    if ($_POST['add_to_menu'] == 'Y') echo 'checked'; ?>></td>
            </tr>

            <tr id="ex"<?
            if ($_POST['add_to_menu'] != 'Y') echo ' style="display:none;"'; ?>>
            ...
            <tr id="e0"<?
            if ($_REQUEST['add_to_menu'] != 'Y') echo ' style="display:none;"'; ?>>
                <td valign="top"><?= GetMessage("FILEMAN_H_EDIT_MENUIT") ?></td>
                <td>
                    <input type="radio" name="itemtype" id="itemtype_n" value="n" onclick="chitemtype()"
                        <?
                        if ($n = (!isset($_POST['itemtype']) || $_POST['itemtype'] != 'e')) echo 'checked'; ?>> <label
                            for="itemtype_n"><?= GetMessage("FILEMAN_H_EDIT_MENUITNEW") ?></label><br>
                    <input type="radio" name="itemtype" id="itemtype_e" value="e" onclick="chitemtype()"<?
                    if (!$n) echo 'checked'; ?>> <label for="itemtype_e"><?
                        echo GetMessage("FILEMAN_H_EDIT_MENUITEX") ?></label>
                </td>
            </tr>
            <tr id="e1"<?
            if ($_REQUEST['add_to_menu'] != 'Y') echo ' style="display:none;"'; ?>>
                <td><?
                    echo GetMessage("FILEMAN_H_EDIT_MENU_NEW_NAME") ?></td>
                <td><input type="text" name="newp" id="newp" value="<?
                    if (isset($_POST['newp'])) echo htmlspecialcharsbx($_POST['newp']); ?>"></td>
            </tr>
            <tr id="e2"<?
            if ($_REQUEST['add_to_menu'] != 'Y') echo ' style="display:none;"'; ?>>
                <td><?
                    echo GetMessage("FILEMAN_H_EDIT_MENU_INS_BEFORE") ?></td>
                <td>
                    <select name="newppos" id="newppos"><?
                        ...
                        for ($i = 0; $i < $l; $i++):
                            ?>
                            <option value="<?= $i + 1 ?>" <?
                            if (isset($_POST['newppos']) && $_POST['newppos'] == $i + 1) echo 'selected'; ?>><?= $arItems[$i] ?></option><?
                        endfor;
                        ?>
                        <option value="0" <?
                        if (isset($_POST['newppos']) && $_POST['newppos'] == 0) echo 'selected'; ?>><?
            ...
            if ($_REQUEST['add_to_menu'] != 'Y') echo ' style="display:none;"'; ?>>
                <td><?
                    echo GetMessage("FILEMAN_H_EDIT_MENU_ITEM") ?></td>
                <td>
                    <select name="menuitem" id="menuitem"><?
                        ...
                        for ($i = 0; $i < $l; $i++):
                            ?>
                            <option value="<?= $i + 1 ?>" <?
                            if (isset($_POST['menuitem']) && $_POST['menuitem'] == $i + 1) echo 'selected'; ?>><?= $arItems[$i] ?></option><?
                        endfor;
                        ?></select>
