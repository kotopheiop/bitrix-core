...
<table cellspacing="0">
    <tr>
        ...
        <td style="padding-left:3px; padding-right:3px;"><input class="form-button" type="button" value="OK"
                                                                ...
                                                                OnClick="<?= $sTableID ?>.GetAdminList('fileman_admin.php?<?= $addUrl ?>&site=<?= urlencode($site) ?>&path='+jsUtils.urlencode(BX('quick_path').value)+'&show_perms_for=<?= IntVal($show_perms_for) ?>&check_for_file=Y', GALCallBack)">
        </td>
    </tr>
</table>
    ...
    window.InitQuickPath = function () {
        ...
        new BXFMInpSel({
            ...
            OnEnterPress: function (res) {
                <?= $sTableID ?>.
                GetAdminList('fileman_admin.php?<?=$addUrl?>&site=<?= urlencode($site) ?>&path=' + res.value + '&show_perms_for=<?= IntVal($show_perms_for) ?>&check_for_file=Y', GALCallBack);
                return false;
            }
        });
    }
...
<? if (!$bSearch): ?>
    <form name="find_form" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?">
        ...
        <tr>
            ...
            <td width="0%"
                nowrap><?= CalendarPeriod("find_timestamp_1", htmlspecialcharsbx($find_timestamp_1), "find_timestamp_2", htmlspecialcharsbx($find_timestamp_2), "find_form", "Y") ?></td>
        </tr>
        <tr>
