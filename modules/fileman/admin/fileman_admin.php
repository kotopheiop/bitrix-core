...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["perms"] <> '' && is_array($_POST["files"]) && count(
        $_POST["files"]
    ) > 0) {
...
if (isset($_GET['check_for_file']) && $_GET['check_for_file'] == 'Y' && $io->FileExists($absPath)) {
    die(
        '<script>top.location="' . "fileman_file_view.php?path=" . urlencode($path) . '&' . bitrix_sessid_get(
        ) . '&' . $addUrl . '"</script>'
    );
}
...
// Check user rights
if ($lAdmin->EditAction() && ($USER->CanDoOperation('fileman_admin_files') || $USER->CanDoOperation(
    ...
    foreach ($FIELDS as $ID => $arFields) {
        ...
        if (!($USER->CanDoFileOperation('fm_rename_file', $arPath_i) || $USER->CanDoFileOperation(
                'fm_rename_file',
                $arPath_i
            ))) {
        ...
        if ($arFields["NAME"] == '') {
            ...
            if (!($USER->CanDoFileOperation('fm_rename_file', $arPath_i) || $USER->CanDoFileOperation(
            ...
            } elseif (!$USER->CanDoOperation('edit_php') && (mb_substr($prev_name_i, 0, 1) == "." || mb_substr(
                        $name_i,
                        0,
                        1
                    ) == "." || (!$isPhpFrom && $isPhpTo))) {
                ...
                if ($strWarningTmp <> '') {
                    ...
                    } elseif (!$io->FileExists($documentRoot . $pathFrom) && !$io->DirectoryExists(
                            $documentRoot . $pathFrom
                        )) {
                        $lAdmin->AddGroupError(
                            GetMessage("FILEMAN_RENAME_FILE") . " \"" . $pathFrom . "\" " . GetMessage(
                                "FILEMAN_RENAME_NOT_FOUND"
                            ),
                            $ID
                        );
                    } elseif ($io->FileExists($documentRoot . $pathTo) || $io->DirectoryExists(
                            $documentRoot . $pathTo
                        )) {
                        $lAdmin->AddGroupError(
                            GetMessage("FILEMAN_RENAME_ALREADY_EXIST", Array("#FILE_NAME#" => $pathTo)),
                            $ID
                        );
                    } elseif (!$io->Rename($documentRoot . $pathFrom, $documentRoot . $pathTo)) {
                        $lAdmin->AddGroupError(
                            GetMessage("FILEMAN_RENAME_ERROR") . " \"" . $pathFrom . "\" " . GetMessage(
                                "FILEMAN_RENAME_IN"
                            ) . " \"" . $pathTo . "\"",
                            $ID
                        );
...
if (($arID = $lAdmin->GroupAction()) && ($USER->CanDoOperation('fileman_admin_files') || $USER->CanDoOperation(
            'fileman_admin_folders'
        )) && $handle_action) {
    ...
    foreach ($arID as $ID) {
        ...
        switch ($_REQUEST['action']) {
            case "delete":
                ...
                if (!($USER->CanDoFileOperation('fm_delete_file', $arPath_i) || $USER->CanDoFileOperation(
                        'fm_delete_folder',
                        $arPath_i
                    ))) {
                    break;
            ...
            case "move":
                ...
                if (($mess = CFileMan::CheckFileName(str_replace('/', '', $copy_to))) !== true) {
                    ...
                    $strWarning_tmp = CFileMan::CopyEx(
                        Array($site, CFileMan::NormalizePath($pathEx)),
                        Array(
                            $copy_to_site,
                            CFileMan::NormalizePath($copy_to . "/" . $name_i)
                        ),
                        ($_REQUEST['action'] == "move" ? true : false)
                    );
                }
...
if ($logical == 'Y') {
    $arHeaders = array(
        ...
        array(
            "id" => "DATE",
            "content" => GetMessage('FILEMAN_ADMIN_FILE_TIMESTAMP'),
            "sort" => "timestamp",
            "default" => true
        ),
...
if (!CFileMan::IsWindows()) {
    $arHeaders[] = array(
        "id" => "PERMS",
        "content" => GetMessage('FILEMAN_ADMIN_ACCESS_PERMS'),
        "sort" => "",
        "default" => true
    );
}
$arHeaders[] = array(
    "id" => "PERMS_B",
    "content" => GetMessage('FILEMAN_ADMIN_ACCESS_PERMS_B'),
    "sort" => "",
    "default" => true
);
...
if (!$bSearch && $path <> '' && ($logical != "Y" || rtrim($arSite["DIR"], "/") != rtrim($arParsedPath["FULL"], "/"))) {
    ...
    if ($logical == "Y") {
        $showField = "<a href=\"javascript:" . $sTableID . ".GetAdminList('fileman_admin.php?" . $addUrl_s . "&site=" . $site . "&path=" . urlencode(
                urlencode($arParsedPath["PREV"])
            ) . "&show_perms_for=" . intval(
                $show_perms_for
            ) . "', GALCallBack);\"><span class=\"adm-submenu-item-link-icon fileman_icon_folder_up\" alt=\"" . GetMessage(
                "FILEMAN_UP"
            ) . "\"></span>&nbsp;<a href=\"javascript:" . $sTableID . ".GetAdminList('fileman_admin.php?" . $addUrl_s . "&site=" . $site . "&path=" . urlencode(
                urlencode($arParsedPath["PREV"])
            ) . "&show_perms_for=" . intval($show_perms_for) . "', GALCallBack);\">..</a>";
    } else {
        $showField = "<a href=\"javascript:" . $sTableID . ".GetAdminList('fileman_admin.php?" . $addUrl_s . "&site=" . $site . "&path=" . urlencode(
                urlencode($arParsedPath["PREV"])
            ) . "&show_perms_for=" . intval(
    ...
    $arActions[] = array(
        ...
        "DEFAULT" => true,
        "ACTION" => "javascript:" . $sTableID . ".GetAdminList('fileman_admin.php?" . $addUrl_s . "&site=" . $site . "&path=" . urlencode(
                $arParsedPath["PREV"]
            ) . "&show_perms_for=" . intval($show_perms_for) . "', GALCallBack);"
    );
...
while ($Elem = $db_DirContent->NavNext(true, "f_")) {
    ...
    $showFieldText = "";
    if ($Elem["TYPE"] == "D") {
        $showFieldIcon = "<a href=\"fileman_admin.php?" . $addUrl_s . "&site=" . urlencode(
                $site
            ) . "&path=" . $fpathUrl . "&show_perms_for=" . intval(
                $show_perms_for
            ) . "\" onclick=\"" . $sTableID . ".GetAdminList('fileman_admin.php?" . $addUrl_s . "&site=" . urlencode(
                $site
            ) . "&path=" . $fpathUrl . "&show_perms_for=" . intval(
                $show_perms_for
            ) . "', GALCallBack);return false;\"><span class='adm-submenu-item-link-icon fileman_icon_folder'></span></a>";
        $showFieldText = "<a href=\"fileman_admin.php?" . $addUrl_s . "&site=" . urlencode(
                $site
            ) . "&path=" . $fpathUrl . "&show_perms_for=" . intval(
                $show_perms_for
            ) . "\" onclick=\"" . $sTableID . ".GetAdminList('fileman_admin.php?" . $addUrl_s . "&site=" . urlencode(
                ...
                $show_perms_for
            ) . "', GALCallBack);return false;\">" . $f_NAME . "</a>";
    } else {
        $curFileType = CFileMan::GetFileTypeEx($f_NAME);
        if (preg_match('/^\.(.*)?\.menu\.(php|html|php3|php4|php5|php6|phtml)$/', $f_NAME, $regs)) {
            $showFieldIcon = "";
            $showFieldText = GetMessage("FILEMAN_ADMIN_MENU_TYPE") . "&laquo;" . htmlspecialcharsbx(
                    $regs[1]
                ) . "&raquo;";
        } else {
    ...
    if ($logical == 'Y') {
        ...
        if ($Elem["TYPE"] == "D") {
            $showFieldIcon = "<a href=\"javascript:" . $sTableID . ".GetAdminList('fileman_admin.php?" . $addUrl_s . "&site=" . urlencode(
                    $site
                ) . "&path=" . $fpathUrl . "&show_perms_for=" . intval(
                    $show_perms_for
                ) . "', GALCallBack);\" title=\"" . htmlspecialcharsbx(
                    $fpath
                ) . "\"><span class='adm-submenu-item-link-icon fileman_icon_folder'></span></a>";
            $showFieldText = "<a href=\"javascript:" . $sTableID . ".GetAdminList('fileman_admin.php?" . $addUrl_s . "&site=" . urlencode(
                    $site
                ) . "&path=" . $fpathUrl . "&show_perms_for=" . intval(
                    $show_perms_for
                ) . "', GALCallBack);\" title=\"" . htmlspecialcharsbx($fpath) . "\">" . $f_LOGIC_NAME . "</a>";
        } else {
            $curFileType = CFileMan::GetFileTypeEx($f_NAME);
            if (preg_match('/^\.(.*)?\.menu\.(php|html|php3|php4|php5|phtml)$/', $f_NAME, $regs)) {
                ...
                $showFieldIcon = "<IMG SRC=\"/bitrix/images/fileman/types/" . $curFileType . ".gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=0 ALT=\"\"  title=\"" . htmlspecialcharsbx(
                        $fpath
                    ) . "\">";
    ...
    $row->AddField(
        "TYPE",
        ($Elem["TYPE"] == "D") ? GetMessage('FILEMAN_FOLDER') : htmlspecialcharsbx(
            $arFilemanPredifinedFileTypes[$curFileType]["name"]
        )
    );
    ...
    if (in_array("PERMS_B", $arVisibleColumns)) {
        $showField = "&nbsp;";
        if (($USER->CanDoOperation('fileman_view_permissions') || $USER->CanDoOperation(
                    'fileman_edit_all_settings'
                )) && $USER->CanDoFileOperation('fm_view_permission', $arPath)) {
            $arP = $APPLICATION->GetFileAccessPermission(
                Array($site, $fpath),
                ((intval($show_perms_for) > 0) ? array($show_perms_for) : false),
                true
            );
    ...
    if ($Elem["TYPE"] == "F") {
        if ($USER->CanDoFileOperation('fm_view_listing', $arPath)) {
            if ($USER->CanDoOperation('fileman_edit_menu_elements') && preg_match(
                    '/^\.(.*)?\.menu\.(php|html|php3|php4|php5|phtml)$/',
                    $f_NAME,
                    $regs
                ...
                $arActions[] = array(
                    ...
                    "ACTION" => $lAdmin->ActionRedirect(
                        "fileman_menu_edit.php?path=" . urlencode($path) . "&site=" . $site . "&name=" . urlencode(
                            $regs[1]
                        ) . "&" . $addUrl . "&" . GetFilterParams("filter_") . ""
                    )
                );
                if ($USER->CanDoOperation('edit_php') && $USER->CanDoFileOperation('fm_edit_existent_file', $arPath)) {
                    $arActions[] = array(
                        ...
                        "DEFAULT" => false,
                        "ACTION" => $lAdmin->ActionRedirect(
                            "fileman_file_edit.php?path=" . $fpathUrl . "&full_src=Y&site=" . $site . "&" . $addUrl . "&" . GetFilterParams(
                                "filter_"
                            ) . ""
                        )
                    );
                ...
                if ($curFilePreType == "text") {
                    if ($USER->CanDoFileOperation('fm_edit_existent_file', $arPath)) {
                        if (!HasScriptExtension($f_NAME) || $USER->CanDoFileOperation(
                                'fm_lpa',
                                $arPath
                            ...
                            $arActions[] = array(
                                ...
                                "ACTION" => $lAdmin->ActionRedirect(
                                    "fileman_html_edit.php?path=" . $fpathUrl . "&site=" . $site . "&" . $addUrl . "&" . GetFilterParams(
                                        "filter_"
                                    ) . ""
                                )
                            );
                            ...
                            $arActions[] = array(
                                ...
                                "ACTION" => $lAdmin->ActionRedirect(
                                    "fileman_file_edit.php?path=" . $fpathUrl . "&site=" . $site . "&" . $addUrl . "&" . GetFilterParams(
                                        "filter_"
                                    ) . ""
                                )
                            );
                    ...
                    if (CModule::IncludeModule("workflow") && $USER->CanDoFileOperation(
                            'fm_edit_in_workflow',
                            $arPath
                        )) {
                        $st = '';
                ...
                if ($USER->CanDoFileOperation('fm_view_file', $arPath) && ($USER->CanDoOperation(
                            'edit_php'
                        ) || $USER->CanDoFileOperation('fm_lpa', $arPath) || !(HasScriptExtension($f_NAME) || mb_substr(
                                $Elem["NAME"],
                                0,
                                1
                            ) == "."))) {
                    $arActions[] = array(
                        ...
                        "ACTION" => $lAdmin->ActionRedirect(
                            "fileman_file_view.php?path=" . $fpathUrl . "&site=" . $site . "&" . $addUrl
                        )
                    );
                }

                if (($USER->CanDoFileOperation('fm_download_file', $arPath) && !(HasScriptExtension(
                                $f_NAME
                            ) || mb_substr($Elem["NAME"], 0, 1) == ".")) || $USER->CanDoOperation('edit_php')) {
                    $arActions[] = array(
                        "ICON" => "btn_download",
                        "TEXT" => GetMessage("FILEMAN_DOWNLOAD"),
                        "ACTION" => $lAdmin->ActionRedirect(
                                "fileman_file_download.php?path=" . $fpathUrl . "&site=" . $site . "&" . $addUrl
                            ) . " setTimeout(function(){ CloseWaitWindow(); }, 3000);"
                    );
                }
            }
        }
        ...
        if ($USER->CanDoFileOperation('fm_view_listing', $arPath)) {
            $arActions[] = array(
                ...
                "TEXT" => GetMessage('FILEMAN_N_OPEN'),
                "DEFAULT" => true,
                "ACTION" => "javascript:" . $sTableID . ".GetAdminList('fileman_admin.php?" . $addUrl . "&site=" . urlencode(
                        $site
                    ) . "&path=" . $fpathUrl . "&show_perms_for=" . intval($show_perms_for) . "', GALCallBack);"
            );
        }
        ...
        if ($USER->CanDoFileOperation('fm_edit_existent_folder', $arPath)) {
            $arActions[] = array(
                ...
                "ACTION" => $lAdmin->ActionRedirect(
                    "fileman_folder.php?" . $addUrl . "&site=" . urlencode($site) . "&path=" . $fpathUrl . ""
                )
            );
        }
    }
    ...
    if ($logical != "Y") {
        if ($Elem["TYPE"] == "F" && $USER->CanDoFileOperation('fm_view_file', $arPath) && ($USER->CanDoOperation(
                    'edit_php'
                ) || $USER->CanDoFileOperation('fm_lpa', $arPath) || !(HasScriptExtension($f_NAME) || mb_substr(
                        $Elem["NAME"],
                        0,
            ...
            $arActions[] = array(
                ...
                "ACTION" => "window.PackUnpackRun([{'path' : '" . CUtil::JSEscape(
                        $fpath
                    ) . "', 'isDir' : '" . $arrIsDir[$fpath] . "'}], true); return false;"
            );
        ...
        if ($USER->CanDoFileOperation('fm_rename_' . $type, $arPath)) {
            ...
            $arActions[] = array(
                ...
                "ACTION" => 'setCheckbox(\'' . CUtil::JSEscape(
                        $f_NAME
                    ) . '\'); if(' . $lAdmin->table_id . '.IsActionEnabled(\'edit\')){document.forms[\'form_' . $lAdmin->table_id . '\'].elements[\'action_button\'].value=\'edit\'; ' . $lAdmin->ActionPost(
                    ) . '}else{document.location.href=\'fileman_rename.php?' . $addUrl . '&path=' . urlencode(
                        $path
                    ) . '&site=' . $site . '&files[]=' . CFileman::GetFileName($arPath[1]) . '\'}'
            );
        }
        ...
        if (($USER->CanDoFileOperation('fm_view_file', $arPath) && ($USER->CanDoOperation(
                        'edit_php'
                            ...
                            0,
                            1
                        ) == "."))) && $Elem["TYPE"] == "F" || $Elem["TYPE"] == "D" && $USER->CanDoFileOperation(
                'fm_view_listing',
                $arPath
            )) {
            $arActions[] = array(
                ...
                "ACTION" => "window.CopyMoveRun([{'path' : '" . CUtil::JSEscape(
                        $fpath
                    ) . "', 'isDir' : '" . $arrIsDir[$fpath] . "'}], true); return false;"
            );
        }
        ...
        // Move
        if ($USER->CanDoOperation('fileman_admin_folders') && $USER->CanDoFileOperation(
                'fm_delete_' . $type,
                $arPath
            )) {
            if (($USER->CanDoFileOperation('fm_view_file', $arPath) && ($USER->CanDoOperation(
                ...
                $arActions[] = array(
                    ...
                    "ACTION" => "window.CopyMoveRun([{'path' : '" . CUtil::JSEscape(
                            $fpath
                        ) . "', 'isDir' : '" . $arrIsDir[$fpath] . "'}], false); return false;"
                );
            }
            ...
            $arActions[] = array(
                ...
                "ACTION" => "if(confirm('" . GetMessage('FILEMAN_ALERT_DELETE') . "')) " . $lAdmin->ActionDoGroup(
                        urlencode($f_NAME),
                        "delete",
                        $addUrl . "&site=" . urlencode($site) . "&path=" . urlencode(
                            $path
                        ) . "&show_perms_for=" . intval($show_perms_for)
                    )
            );
        }
        ...
        if ($USER->CanDoFileOperation('fm_edit_permission', $arPath)) {
            ...
            $arActions[] = array(
                ...
                "ACTION" => "setCheckbox('" . Cutil::JSEscape($f_NAME) . "'); setAccess('" . Cutil::JSEscape(
                        $site
                    ) . "', '" . Cutil::JSEscape(urlencode($path)) . "');"
            );
...
if ($USER->CanDoFileOperation('fm_create_new_' . $type, $arPath)) {
    ...
    $arGrActionAr["copy"] = array(
        "action" => "setCopyMove('" . Cutil::JSEscape($site) . "', '" . Cutil::JSEscape(
                $path
            ) . "', true, " . CUtil::PhpToJSObject($arrIsDir) . ")",
        "value" => "copy",
        "name" => GetMessage("FILEMAN_ADM_COPY")
    );

    $arGrActionAr["pack"] = array(
        "action" => "setPackUnpack('" . Cutil::JSEscape($site) . "', '" . Cutil::JSEscape(
                $path
            ) . "', true, " . CUtil::PhpToJSObject($arrIsDir) . ")",
        "value" => "pack",
...
if ($USER->CanDoFileOperation('fm_create_new_' . $type, $arPath) && $USER->CanDoFileOperation(
        'fm_delete_' . $type,
        $arPath
    )) {
    ...
    $arGrActionAr["move"] = array(
        "action" => "setCopyMove('" . Cutil::JSEscape($site) . "', '" . Cutil::JSEscape(
                $path
            ) . "', false, " . CUtil::PhpToJSObject($arrIsDir) . ")",
        "value" => "move",
    ...
    while ($arRes = $dbRes->Fetch()) {
        ...
        $arDDMenu[] = array(
            ...
            "ACTION" => $lAdmin->ActionAjaxReload(
                    "fileman_admin.php?" . $addUrl_s . "&site=" . urlencode($site) . "&path=" . urlencode(
                        $path
                    ) . "&show_perms_for=" . $arRes["REFERENCE_ID"]
                ) . ';return false;',
            ...
            "LINK" =>
                ($defaultEdit == 'html' ?
                    ...
                    $defaultEdit == 'php' && $USER->IsAdmin() ?
                        "fileman_file_edit.php?" . $addUrl . "&site=" . $site . "&full_src=Y&path=" . urlencode(
                            $path
                        ) . "&new=y"
                        :
...
if (!$bSearch && $USER->CanDoOperation('fileman_edit_existent_folders') && $USER->CanDoFileOperation(
...
if ($USER->CanDoOperation('view_groups') && $USER->CanDoFileOperation(
        'fm_view_permission',
        $arPath
    ) && $USER->CanDoFileOperation('fm_edit_existent_folder', $arPath)) {
    $addProp[] = Array(
...
<table cellspacing="0">
    <tr>
        ...
        <td style="padding-left:5px;"><input class="bx-quick-path" type="text" name="quick_path" id="quick_path"
                                             size="50" value="<?= htmlspecialcharsbx($path) ?>"/></td>
        <td style="padding-left:3px; padding-right:3px;"><input class="form-button" type="button" value="OK"
                                                                title="<?= GetMessage("FILEMAN_FAST_PATH_BUTTON") ?>"
                                                                OnClick="<?= $sTableID ?>.GetAdminList('fileman_admin.php?<?= $addUrl ?>&site=<?= urlencode(
                                                                    $site
                                                                ) ?>&path='+jsUtils.urlencode(BX('quick_path').value)+'&show_perms_for=<?= intval(
                                                                    $show_perms_for
                                                                ) ?>&check_for_file=Y', GALCallBack)"></td>
    </tr>
</table>
<script>top.BX.ready(function () {
        setTimeout(top.InitQuickPath, 100)
    });</script>
<?
$s = ob_get_contents();
ob_end_clean();
...
if (isset($_POST['bx_search_file'])) {
    $sFormValues = CUtil::PhpToJSObject(
        array(
...
$aContext[] = Array(
    ...
    "LINK" => "javascript: window.SearchReplaceRun('" . CUtil::JSEscape(
            $path
        ) . "', " . ($bSearch ? 'true' : 'false') . ", '" . CUtil::JSEscape($searchSess) . "', " . urlencode(
            $sFormValues
        ) . ");",
    ...
    foreach ($arParsedPath["AR_PATH"] as $chainLevel) {
        $chain->AddItem(
            array(
                ...
                "ONCLICK" => (($chainLevel["LINK"] <> '') ? $lAdmin->ActionAjaxReload(
                        $chainLevel["LINK"]
                    ) . ';return false;' : ""),
            )
        );
...
<script>
    window.InitQuickPath = function () {
        ...
        new BXFMInpSel({
            ...
            OnEnterPress: function (res) {
                <?= $sTableID ?>.
                GetAdminList('fileman_admin.php?<?=$addUrl?>&site=<?= urlencode(
                    $site
                ) ?>&path=' + res.value + '&show_perms_for=<?= intval(
                    $show_perms_for
    ...
    function setCopyMove(site, path, bCopy, isDir) {
        ...
        if (!expType) {
            ...
            for($i = 0, $l = count($searchRes); $i < $l; $i++):?>
            arFiles.push({
                'path': '<?= CUtil::JSEscape($searchRes[$i]['path'])?>',
                'isDir': '<?= CUtil::JSEscape($searchRes[$i]['b_dir'])?>'
            });
            <?endfor;
            endif;?>
        }
...
<? if (!$bSearch): ?>
    <form name="find_form" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?">
        ...
        <tr>
            ...
            <td width="0%" nowrap><?= CalendarPeriod(
                    "find_timestamp_1",
        ...
        <tr>
            ...
            <td nowrap><?
                $arr = array(
                    "reference" => array(GetMessage("FILEMAN_FILE"), GetMessage("FILEMAN_FOLDER")),
                    "reference_id" => array("F", "D")
                );
        ...
        $oFilter->Buttons(
            array(
                "table_id" => $sTableID,
                "url" => $APPLICATION->GetCurPage() . '?' . $addUrl_s . "&site=" . urlencode(
                        $site
                    ) . "&path=" . urlencode($path . "/" . $Elem["NAME"]),
                "form" => "find_form"
            )
        );
        $oFilter->End();
