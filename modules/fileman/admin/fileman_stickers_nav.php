    ...
    while ($NavRecordGroup <= $nEndPage) {
        ...
        <? if ($NavRecordGroup == $this->NavPageNomer) : ?>
            ...
            <div class="bxstl-navi-item-cur" style="width: <?= $w ?>px;">
                ...
                <div class="bxstl-navi-it-c"><?= $NavRecordGroup ?></div>
                <div class="bxstl-navi-it-r"></div>
            </div>
        <? else: ?>
            <a class="bxstl-navi-item" href="javascript: void('');"
               onclick="if (window.oBXSticker){window.oBXSticker.List.NaviGet('<?= ($NavRecordGroup) ?>', '<?= $this->NavNum ?>');}; return false;"><?= $NavRecordGroup ?></a>
        <?endif; ?>
        <?
        $NavRecordGroup++;
    }
