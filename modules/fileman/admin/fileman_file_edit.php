...
if ($new <> '' && $filename <> '') {
    $path = $path . "/" . $filename;
} // let's set this new name to us path
...
if (!($USER->CanDoOperation('fileman_admin_files') || $USER->CanDoOperation(
...
if (($new <> '' &&
    ...
    if ($new <> '' && $filename <> '' && $io->FileExists(
            $abs_path
        ))        // if we want to make new file , but the file with the same name alredy exist, let's abuse
    {
...
if ($strWarning == '') {
    ...
    if ($REQUEST_METHOD == "POST" && $save <> '' && $propeditmore == '' && !$only_read) {
        if (!check_bitrix_sessid()) {
            ...
            if ((CFileman::IsPHP($filesrc) || $isScriptExt) && !($USER->CanDoOperation(
                        'edit_php'
                    ) || $limit_php_access)) {
                ...
                if ($apply == '') {
                    ...
                    LocalRedirect(
                        "/bitrix/admin/fileman_admin.php?" . $addUrl . "&site=" . Urlencode(
                            $site
                        ) . "&path=" . UrlEncode($arParsedPath["PREV"])
                    );
                } else {
                    LocalRedirect(
                        "/bitrix/admin/fileman_file_edit.php?" . $addUrl . "&site=" . Urlencode(
                            $site
                        ) . "&path=" . UrlEncode($path) . ($full_src == "Y" ? "&full_src=Y" : "")
                    );
                }
            }
...
<? if ($strWarning == '' || $bVarsFromForm): ?>
    ...
    if (!$ismenu) {
        $aDDMenuEdit[] = array(
            ...
            "ACTION" => ($mode == 'text' ? "return; " : "") . "window.location='fileman_file_edit.php?" . $addUrl . "&amp;site=" . Urlencode(
                    $site
                ) . "&amp;path=" . UrlEncode(
                    $originalPath
                ) . ($new <> '' ? "&amp;new=Y" : "") . ($back_url <> '' ? "&amp;back_url=" . urlencode(
                        $back_url
                    ) : "") . ($template <> '' ? "&amp;template=" . urlencode(
                        $template
    ...
    if ($USER->CanDoOperation('edit_php')) {
        $aDDMenuEdit[] = array(
            ...
            "ACTION" => ($mode == 'php' ? "return; " : "") . "window.location='fileman_file_edit.php?" . $addUrl . "&amp;site=" . Urlencode(
                    $site
                ) . "&amp;path=" . UrlEncode(
                    $originalPath
                ) . "&amp;full_src=Y" . ($new <> '' ? "&amp;new=Y" : "") . ($back_url <> '' ? "&amp;back_url=" . urlencode(
                        $back_url
                    ) : "") . ($template <> '' ? "&amp;template=" . urlencode(
                        $template
                    ) : "") . ($template <> '' ? "&amp;template=" . urlencode(
    ...
    if ($ismenu) {
        $aDDMenuEdit[] = array(
            ...
            "ACTION" => "window.location='" . "fileman_menu_edit.php?" . $addUrl . "&amp;site=" . Urlencode(
                    $site
                ) . "&amp;path=" . UrlEncode($arParsedPath["PREV"]) . "&amp;name=" . UrlEncode(
                    $regs[1]
                ) . ($new <> '' ? "&amp;new=Y" : "") . ($back_url <> '' ? "&amp;back_url=" . urlencode(
                        $back_url
                    ) : "") . "';"
        ...
        $aDDMenuEdit[] = array(
            ...
            "ACTION" => "window.location='" . "fileman_html_edit.php?" . $addUrl . "&amp;site=" . Urlencode(
                    $site
                ) . "&amp;path=" . UrlEncode(
                    $originalPath
                ) . ($new <> '' ? "&amp;new=Y" : "") . ($back_url <> '' ? "&amp;back_url=" . urlencode(
    ...
    <form method="POST" action="<?= $APPLICATION->GetCurPage() . '?' . $addUrl . "&amp;path=" . UrlEncode(
        $path
    ) . ($new <> '' ? "&amp;new=Y" : "") ?>" name="ffilemanedit">
        ...
        $aTabs = array(
            array(
                "DIV" => "edit1",
                "TAB" => (($bEdit) ? GetMessage('FILEMAN_EDIT_TAB') : GetMessage('FILEMAN_EDIT_TAB1')),
                "ICON" => "fileman",
                "TITLE" => (($bEdit) ? GetMessage('FILEMAN_EDIT_TAB_ALT') : GetMessage('FILEMAN_EDIT_TAB_ALT1'))
            ),
        );
        ...
        <? if (!$bEdit): ?>
            ...
            <tr>
                ...
                <td width="70%">
                    <select name="template"
                            onchange="window.location='/bitrix/admin/fileman_file_edit.php?lang=<?= LANG ?>&site=<?= Urlencode(
                                $site
                            ) ?>&path=<?= UrlEncode(
                                $path
                            ) ?><? echo($full_src == "Y" ? "&full_src=Y" : "") ?>&new=y&template='+escape(this[this.selectedIndex].value)">
                        <? for ($i = 0, $l = count($arTemplates); $i < $l; $i++): ?>
            ...
            <? if (!$bFullPHP): ?>
                <tr>
                    ...
                    <td width="70%"><input id="bxfm_title" type="text" name="title" size="50" maxlength="255"
                                           value="<?= htmlspecialcharsbx($title) ?>"></td>
                </tr>
            <? endif; ?>
            <tr>
                ...
                <td>
                    ...
                    ?>
                    <input type="text" name="filename" id="bxfm_filename" style="float: left;" size="50" maxlength="255"
                           value="<?= htmlspecialcharsbx($filename) ?>"></td>
            </tr>
            <tr>
                <td></td>
                <td style="padding: 0!important;">
                    <table id='jserror_name' style="visibility:hidden">
                        <tr>
                            <td valign="top">
                                <IMG src="/bitrix/themes/.default/images/icon_warn.gif"
                                     title="<?= GetMessage("FILEMAN_NAME_ERR"); ?>">
                            </td>
                            <td id="jserror" class="jserror"></td>
                        </tr>
                    </table>
                    <script>
                        ...
                        oInput.oninput = function () {
                            var _this = this,
                                saveBut = BX.findChild(form, {
                                    tag: 'INPUT',
                                    attr: {'name': 'save', 'type': 'submit'}
                                }, true);
                            setTimeout(function () {
                                    var val = _this.value;
                ...
                <td colSpan="2">
                    <table border="0" cellspacing="1" cellpadding="3" id="proptab" class="internal">
                        ...
                        for ($i = 0, $l = count($arAllPropFields); $i < $l; $i++) {
                            ...
                            ?>
                            <tr>
                                <td valign="top" <? if (!$arProp["NAME"]) {
                                    echo 'nowrap';
                                } ?>>
                                    <input type="hidden" id="H_CODE_<?= $i; ?>" name="H_CODE_<?= $i; ?>"
                                           value="<?= htmlspecialcharsbx($arProp["CODE"]) ?>">
                                    <? if ($arProp["NAME"]):?>
                                        <input type="hidden" id="CODE_<?= $i; ?>" name="CODE_<?= $i; ?>"
                                               value="<?= htmlspecialcharsbx($arProp["CODE"]) ?>">
                                        <input type="hidden" id="NAME_<?= $i; ?>" name="NAME_<?= $i; ?>"
                                               value="<?= htmlspecialcharsbx($arProp["NAME"]); ?>">
                        ...
                        <tr>
                            <td colspan="2">
                                ...
                                <input type="button" name="propeditmore"
                                       value="<?= GetMessage("FILEMAN_EDIT_PROPSMORE") ?>" onClick="_MoreRProps()">
                            </td>
                        </tr>
                    </table>
                    <br>
        ...
        <tr>
            <td colspan="2">
                <textarea id="bx-filesrc" name="filesrc" rows="37" style="width:100%; overflow:auto;"
                          wrap="OFF"><?= htmlspecialcharsbx($filesrc) ?></textarea></td>
        </tr>
        ...
        $tabControl->Buttons(
            array(
                ...
                "back_url" => ($back_url <> '' && mb_strpos(
                    ...
                    ) !== 0) ? htmlspecialcharsbx(
                    $back_url
                ) : "/bitrix/admin/fileman_admin.php?" . $addUrl . "&site=" . Urlencode($site) . "&path=" . UrlEncode(
                        $arParsedPath["PREV"]
                    )
            )
        );
    ...
    <script>
        function AjaxApply(e) {
            ...
            if (!applyInp)
                applyInp = form.appendChild(BX.create('INPUT', {
                    props: {
                        id: 'bx-ffilemanedit-ajax-apply',
                        type: 'hidden',
                        name: 'AJAX_APPLY',
                        value: 'Y'
                    }
            ...
            BX.ajax.submit(form, function () {
                ...
                if (top.strWarning && top.strWarning.length > 0) {
                    ...
                    if (filename) {
                        document.location.href = '<?$APPLICATION->getCurPage()?>?<?=CUtil::JSEscape(
                            $addUrl . "&site=" . Urlencode($site)
                            . "&path=" . UrlEncode($path . "/")
                        )?>' + encodeURIComponent(filename.value) + '<?if ($full_src == "Y") {
                            echo "&full_src=Y";
                        }?>';
                    }
                }
