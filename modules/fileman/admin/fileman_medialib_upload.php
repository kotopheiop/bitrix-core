...
class filemanMedialibUpload
{
    ...
    function onBeforeUpload(&$package, $upload, $post, $files, &$error)
    {
        ...
        if ($post["collectionId"] <= 0) {
            ...
            if (!CMedialib::CanDoOperation("medialib_new_item", $post["collectionId"])) // Check access
            {
                $error = "Access denied";
                return false;
            }
    ...
    function handleFile($hash, $file, &$package, &$upload)
    {
        ...
        if (!isset($upload["redirectUrl"]) && $res && $res['ID'] > 0) {
            ...
            $upload["redirectUrl"] = $APPLICATION->GetCurPageParam(
                $upload["redirectUrlPart"],
                array(
                    "action",
                    "ml_type",
                    "first_id",
...
if ($action == 'uploadhtml5' && is_object($uploader)) {
    ...
    if ($action == 'upload') {
        ...
        die(
            '#JS#&first_id=' . $firstId . '&col_id=' . $collectionId . '&ml_type=' . htmlspecialcharsEx(
                $_GET['ml_type']
            ) . '#JS#'
        );
    } elseif ($action == 'redirect')  //Redirect after files uploading
    {
        ...
        if ($len > 0) {
            ...
            $strSel = '<option value="0">' . GetMessage(
                    'ML_COL_SELECT'
                ) . '</option>' . CMedialib::_BuildCollectionsSelectOptions($res['Collections'], $res['arColTree']);
            $module_id = "fileman";
            ...
            <form name="ml_mu_form" action="/bitrix/admin/fileman_medialib_upload.php?action=postsave" method="POST">
                <table class="mu-items-list" border="0"><?
                    for ($i = 0; $i < $len; $i++) {
                        ...
                        ?>
                        <tr>
                            <td class="mu-label" style="width: 150px;"><label
                                        for="item_name_<?= $i ?>"><b><?= GetMessage('ML_NAME') ?>:</b></label></td>
                            <td><input class="mu-text-inp" id="item_name_<?= $i ?>" name="item_name_<?= $i ?>"
                                       type="text" value="<?= htmlspecialcharsbx($Item['NAME']) ?>" size="52"></td>
                        ...
                        <tr>
                            <td class="mu-label" style="vertical-align: top; padding-top: 2px;"><label
                                        for="item_desc_<?= $i ?>"><?= GetMessage('ML_DESC') ?>:</label></td>
                            <td style="vertical-align: top;"><textarea id="item_desc_<?= $i ?>"
                                                                       name="item_desc_<?= $i ?>" rows="3"
                                                                       cols="40"><?= htmlspecialcharsbx(
                                        $Item["DESCRIPTION"]
                                    ) ?></textarea></td>
                        </tr>
                        <tr>
                            <td class="mu-label"><label for="item_keys_<?= $i ?>"><?= GetMessage('ML_KEYWORDS') ?>
                                    :</label></td>
                            <td><input class="mu-text-inp" id="item_keys_<?= $i ?>" name="item_keys_<?= $i ?>"
                                       type="text" value="" size="52"></td>
                        </tr>
                        <tr>
                            <td class="mu-label" style="vertical-align: top; padding-top: 2px;"><label
                                        for="item_cols_sel_<?= $i ?>"><b><?= GetMessage('ML_COLLECTIONS') ?>
                                        :</b></label></td>
                            <td style="padding-bottom: 8px;">
                                <div class="mu-col-sel"><select id="item_cols_sel_<?= $i ?>"
                                                                onchange="itemColsSelChange(this);"
                                                                style="margin-top: 2px"><?= $strSel ?></select></div>
                                <input id="item_colls_<?= $i ?>" name="item_colls_<?= $i ?>" type="hidden" value=""/>
                                <input name="item_id_<?= $i ?>" type="hidden" value="<?= $Item['ID'] ?>"/>
                                <script>
                                    BX.ready(function () {
                                        setTimeout(function () {
                                            addCollToItem(<?=$i?>, <?= $colId?>, BX("item_cols_sel_<?=$i?>"));
                                        }, 200);
                                        arKeywords[<?=$i?>] = {pKeys: BX('item_keys_<?=$i?>'), bFocusKeywords: false};
                                        arKeywords[<?=$i?>].pKeys.onchange = arKeywords[<?=$i?>].pKeys.onblur = function () {
                                            arKeywords[<?=$i?>].bFocusKeywords = true;
                                        };
                                    });
                                </script>
                            </td>
                            <td><input type="checkbox" name="item_del_<?= $i ?>" id="item_del_<?= $i ?>"/><label
                                        for="item_del_<?= $i ?>"><?= GetMessage('ML_DELETE') ?></label></td>
                        </tr>
                        <tr class="mu-separator">
                            <td colSpan="3"></td>
                        </tr>
                        <?
                    }
                ...
                <?= bitrix_sessid_post() ?>
                <input type="submit" title="<?= GetMessage('admin_lib_edit_save_title') ?>"
                       value="<?= GetMessage('admin_lib_edit_save') ?>" name="save"/>
                <input type="button" title="<?= GetMessage('admin_lib_edit_cancel_title') ?>"
                       onclick="window.location='/bitrix/admin/fileman_medialib_admin.php?lang=<?= LANGUAGE_ID ?>';"
                       name="cancel" value="<?= GetMessage('admin_lib_edit_cancel') ?>"/>
            </form>
            <script>
                ...
                function addCollToItem(ItemId, id, pSel) {
                    ...
                    var
                        ...
                        pDel = pDiv.appendChild(jsUtils.CreateElement("IMG", {
                            src: '/bitrix/images/1.gif',
                            className: 'mu-col-del',
                            id: 'mu_it_' + id,
                            title: '<?= GetMessage('ML_DEL_COL2ITEM')?>'
                        }));

                    curArCols.push(id);
                    pDiv.appendChild(BX.create("SPAN", {text: oCol.NAME}));
                    pDiv.onmouseover = function () {
                        BX.addClass(this, 'mu-col-over');
                    }
                    pDiv.onmouseout = function () {
                        BX.removeClass(this, 'mu-col-over');
                    }
        ...
        if ($itemsCount > 0) {
            for ($i = 0; $i < $itemsCount; $i++) {
                ...
                for ($j = 0, $n = count($arCols_); $j < $n; $j++) {
                    if (intval($arCols_[$j]) > 0 && CMedialib::CanDoOperation(
                            "medialib_edit_item",
                            $arCols_[$j]
                        )) // Check access
                    {
...
$select = CMedialib::_BuildCollectionsSelectOptions(
    $trees['Collections'],
    $trees['arColTree'],
    0,
    intval($_GET['col_id'])
...
?>
    <div class="upl-main-wrap">
        <form id="<?= $uploaderID ?>_form" name="<?= $uploaderID ?>_form" action="<?= $APPLICATION->GetCurPageParam(
            "type_ml=" . urlencode($_GET["type"]) . "&" . bitrix_sessid_get(),
            array("type_ml", "sessid")
        ) ?>" method="POST" enctype="multipart/form-data" class="bxiu-photo-form">
            <input type="hidden" name="action" id="id" value="uploadhtml5"/>
            <input type="hidden" name="sessid" id="sessid" value="<?= bitrix_sessid() ?>"/>
            <div class="bxu-thumbnails bxu-thumbnails-start<?= ($options["template"] == "full" ? "" : " bxu-main-block-reduced-size") ?>"
                 id="bxuMain<?= $uploaderID ?>"> <!-- bxu-thumbnails-loading bxu-thumbnails-start-->
                <div class="bxu-top-block">
                    <label class="upl-top-bar-text" for="collection_id<?= $uploaderID ?>"><?= GetMessage(
                            "MEDIALIB_UPLOADER_INTO"
                        ) ?>:</label><?
                    ?><select class="upl-select" name="collection_id" id="collection_id<?= $uploaderID ?>"
                              onchange="itemColsSelChange2(this, arguments[0] || window.event);">
                        <?= $select ?>
                    </select>
                    <span class="bxu-loading-block">
			<span class="bxu-loading-block-bar"><span class="bxu-loading-block-bar-inner"
                                                      id="bxuUploadBar<?= $uploaderID ?>"></span></span>
			<span class="bxu-loading-block-text"><?= GetMessage("MEDIALIB_UPLOADER_HAS_BEEN_UPLOADED") ?> <span
                        id="bxuUploaded<?= $uploaderID ?>"></span> <?= GetMessage(
                    "MEDIALIB_UPLOADER_UPLOAD_FROM"
                ) ?> <span id="bxuForUpload<?= $uploaderID ?>"></span></span>
			<span class="bxu-loading-block-cancel-btn" id="bxuCancel<?= $uploaderID ?>"><?= GetMessage(
                    "MEDIALIB_UPLOADER_CANCEL"
                ) ?></span>
		</span>
                    <div class="bxu-settings-block">
			<span class="bxu-settings-block-templates">
				<span class="bxu-templates-btn bxu-templates-btn-small<?= ($options["template"] == "full" ? "" : " bxu-templates-btn-active") ?>"
                      id="bxuReduced<?= $uploaderID ?>"
                      title="<?= GetMessage("MEDIALIB_UPLOADER_SIMPLIFIED") ?>"></span><?
                ?><span id="bxuEnlarge<?= $uploaderID ?>"
                        class="bxu-templates-btn bxu-templates-btn-big<?= ($options["template"] == "full" ? " bxu-templates-btn-active" : "") ?>"
                        title="<?= GetMessage("MEDIALIB_UPLOADER_NORMAL") ?>"></span>
			</span>
                    </div>
                </div>
                <div class="bxu-main-block" id="bxuDropzone<?= $uploaderID ?>">
                    <div class="bxu-start-block">
                        <div class="bxu-start-block-spacer-div">
                            <img class="bxu-start-block-spacer-img"
                                 src="/bitrix/images/fileman/medialib/uploader/upl-start-spacer.png"/>
                            <input type="file" id="bxuUploaderStartField<?= $uploaderID ?>" multiple="multiple"/>
                        </div>
                        <div class="bxu-start-block-cont">
                            <img src="/bitrix/images/fileman/medialib/uploader/start-img.png"
                                 class="bxu-start-block-img" alt=""/>
                            <div class="bxu-start-block-text">
                                <?= GetMessage("MEDIALIB_UPLOADER_UPLOAD1") ?>
                                <span class="bxu-start-block-description bxu-dnd"><?= GetMessage(
                                        "MEDIALIB_UPLOADER_DND"
                                    ) ?></span>
                            </div>
                        </div>
                        <div class="bxu-start-block-btn">
                            <a class="webform-button">
                                <span class="webform-button-left"></span><span
                                        class="webform-button-text"><?= GetMessage("MEDIALIB_UPLOADER_UPLOAD") ?><?
                                    ?><input type="file" id="bxuUploaderStart<?= $uploaderID ?>" class="bxu-file-input"
                                             multiple="multiple"/><?
                    ...
                    <div class="bxu-bottom-block">
                        ...
                        <div class="bxu-bottom-block-btns">
                            <a class="webform-button webform-button-accept" id="bxuStartUploading<?= $uploaderID ?>">
                                <span class="webform-button-left"></span><span
                                        class="webform-button-text"><?= GetMessage("MEDIALIB_UPLOADER_UPLOAD") ?></span><span
                                        class="webform-button-right"></span>
                            </a>
                            <a class="webform-button webform-button-add">
                                <span class="webform-button-left"></span><span
                                        class="webform-button-text"><?= GetMessage("MEDIALIB_UPLOADER_ADD") ?>
						<input type="file" id="bxuUploader<?= $uploaderID ?>" name="FILE" class="bxu-file-input"
                               multiple="multiple"/>
					</span><span class="webform-button-right"></span>
                            </a>
                        </div>
                        <div class="bxu-bottom-block-text"><?= GetMessage("MEDIALIB_UPLOADER_COUNT2") ?>: <span
                                    id="bxuImagesCount<?= $uploaderID ?>">0</span></div>
                    </div>
                </div>
            </div>
                ...
                $params = array_merge(
                    ...
                    array(
                        ...
                        "fields" => array(
                            ...
                            "description" => array(
                                "template" => '<input class="bxu-item-thumb-description-inp" name="description" placeholder="' . GetMessage(
                                        "MEDIALIB_UPLOADER_DESCRIPTION"
                                    ) . '" value="#description#" type="text" />',
                                "editorTemplate" => '<input name="description" placeholder="' . GetMessage(
                                        "MEDIALIB_UPLOADER_DESCRIPTION"
                                    ) . '" value="#description#" type="text" />',
