...
if ($USER->CanDoFileOperation('fm_create_new_folder', $arPath)) {
    ...
    echo $APPLICATION->GetCurPage() ?>?" name="fnew_folder">
        ...
        ?>
        <tr>
            <td><label for="bxfm_sectionname"
                       style="font-weight: bold;"><?= GetMessage("FILEMAN_NEWFOLDER_SEACTION_NAME") ?></label></td>
            <td><input type="text" id="bxfm_sectionname" name="sectionname"
                       value="<?= htmlspecialcharsex($sectionname) ?>" size="30" maxlength="255"></td>
        </tr>
        <tr>
            <td width="40%"><label for="bxfm_foldername"
                                   style="font-weight: bold;"><?= GetMessage("FILEMAN_NEWFOLDER_NAME") ?></label></td>
        ...
        if ($USER->CanDoFileOperation('fm_add_to_menu', $arPath) && $USER->CanDoOperation('fileman_add_element_to_menu')):?>
            ...
            <tr>
                ...
                <td><input type="checkbox" name="mkmenu" value="Y"<?
                    if ($mkmenu == "Y") echo " checked" ?>
                ...
                <td>
                    ...
                    if ($mkmenu != "Y") echo " disabled" ?>>
                        ...
                        for ($i = 0, $l = count($arMenuTypes); $i < $l; $i++):?>
                            <option value="<?
                            echo htmlspecialcharsex($arMenuTypes[$i][0]) ?>" <?
                            if ($menutype == $arMenuTypes[$i][0]) echo " selected" ?>><?
                                echo htmlspecialcharsex("[" . $arMenuTypes[$i][0] . "] " . $arMenuTypes[$i][1]) ?></option>
                        <?endfor; ?>
                    </select>
                </td>
            </tr>
            ...
            if ($mkmenu != "Y") echo " disabled" ?>>
                ...
                <td><input type="text" name="menuname" value="<?
                    echo htmlspecialcharsex($menuname) ?>"<?
                    if ($mkmenu != "Y") echo " disabled" ?>></td>
            </tr>
        <?endif; ?>
        ...
        if ($USER->CanDoFileOperation('fm_create_new_file', $arPath) && $USER->CanDoOperation('fileman_admin_files')):?>
            ...
            <tr>
                ...
                <td><input type="checkbox" name="mkindex" value="Y"<?
                    if ($mkindex == "Y") echo " checked" ?>
            ...
            <tr id="ff1">
                ...
                <td>
                    ...
                    if ($mkindex != "Y") echo " disabled" ?>>
                        <?
                        for ($i = 0, $l = count($arTemplates); $i < $l; $i++):?>
                            <option value="<?
                            echo htmlspecialcharsex($arTemplates[$i]["file"]) ?>"<?
                            if ($template == $arTemplates[$i]["file"]) echo " selected" ?>><?
                                echo htmlspecialcharsex($arTemplates[$i]["name"]) ?></option>
                        <?endfor; ?>
                    </select>
                </td>
            </tr>
            <tr id="ff2">
                ...
                <td><input type="checkbox" name="toedit" value="Y"<?
                    if ($toedit == "Y") echo " checked" ?><?
                    if ($mkindex != "Y") echo " disabled" ?>></td>
            </tr>
        <?endif; ?>
        <?
        $tabControl->EndTab();
