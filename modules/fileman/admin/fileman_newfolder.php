...
//check folder access
if (!$USER->CanDoFileOperation('fm_create_new_folder', $arPath)) {
    $strWarning = '<img src="/bitrix/images/fileman/deny.gif" width="28" height="28" border="0" align="left" alt="">' . GetMessage(
            "ACCESS_DENIED"
        );
} else {
                ...
                if ($io->FileExists($DOC_ROOT . $pathto) || $io->DirectoryExists($DOC_ROOT . $pathto)) {
                    ...
                    if ($strWarning == '') {
                        if ($USER->CanDoFileOperation('fm_add_to_menu', $arPath) &&
                            ...
                            $arParsedPathTmp = CFileMan::ParsePath(
                                Array($site, $pathto),
                                true,
                                false,
                                "",
                            ...
                            if ($USER->CanDoFileOperation('fm_view_file', Array($site, $menu_path))) {
                                ...
                                $aMenuLinksTmp[] = Array(
                                    $menuname,
                                    $arParsedPathTmp["PREV"] . "/" . $arParsedPathTmp["LAST"] . "/",
                                    Array(),
                                    Array(),
                                    ""
                                );
                        ...
                        if ($sectionname <> '') {
                            ...
                            $APPLICATION->SaveFileContent(
                                $DOC_ROOT . $pathto . "/.section.php",
                                "<?\n\$sSectionName=\"" . CFileMan::EscapePHPString(
                                    $sectionname
                                ) . "\";\n?>"
                            );
                        }
                        if ($e = $APPLICATION->GetException()) {
                            ...
                            if ($USER->CanDoFileOperation('fm_create_new_file', $arPath) &&
                                ...
                                if ($toedit == "Y") {
                                    LocalRedirect(
                                        "/bitrix/admin/fileman_html_edit.php?" . $addUrl . "&site=" . $site . "&template=" . Urlencode(
                                            $template
                                        ) . "&path=" . UrlEncode(
                                            $pathto
                                        ) . "&filename=index.php&new=Y" . ($back_url == '' ? "" : "&back_url=" . UrlEncode(
                                                $back_url
                                            )) . ($gotonewpage == '' ? "" : "&gotonewpage=" . UrlEncode(
                                                $gotonewpage
                                            )) . ($backnewurl == '' ? "" : "&backnewurl=" . UrlEncode($backnewurl))
                                    );
                                } else {
                                    $APPLICATION->SaveFileContent(
                                        $DOC_ROOT . $pathto . "/index.php",
                                        CFileman::GetTemplateContent($template)
                                    );
                                }
                            }
                        }
                        if ($e = $APPLICATION->GetException()) {
                            ...
                            if ($back_url <> '') {
                                ...
                                $arPathtoParsed = CFileMan::ParsePath(
                                    Array($site, $pathto),
                                    false,
                                    false,
                                    "",
                                ...
                                LocalRedirect(
                                    "/bitrix/admin/fileman_admin.php?" . $addUrl . "&site=" . $site . "&path=" . UrlEncode(
                                        $arPathtoParsed["PREV"]
                                    )
                                );
                            }
...
if ($USER->CanDoFileOperation('fm_create_new_folder', $arPath)) {
    ...
    <form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?" name="fnew_folder">
        ...
        ?>
        <tr>
            <td><label for="bxfm_sectionname" style="font-weight: bold;"><?= GetMessage(
                        "FILEMAN_NEWFOLDER_SEACTION_NAME"
                    ) ?></label></td>
            <td><input type="text" id="bxfm_sectionname" name="sectionname"
                       value="<?= htmlspecialcharsex($sectionname) ?>" size="30" maxlength="255"></td>
        </tr>
        <tr>
            <td width="40%"><label for="bxfm_foldername" style="font-weight: bold;"><?= GetMessage(
                        "FILEMAN_NEWFOLDER_NAME"
                    ) ?></label></td>
            <td width="60%"><input id="bxfm_foldername" type="text" name="foldername"
                                   value="<?= htmlspecialcharsex($foldername) ?>" size="30" maxlength="255"></td>
        </tr>

        <? if ($USER->CanDoFileOperation('fm_add_to_menu', $arPath) && $USER->CanDoOperation(
                'fileman_add_element_to_menu'
            )):?>

            <tr>
                <td><?= GetMessage("FILEMAN_NEWFOLDER_ADDMENU") ?></td>
                <td><input type="checkbox" name="mkmenu" value="Y"<? if ($mkmenu == "Y") echo " checked" ?>
                           onclick="document.fnew_folder.menuname.disabled=!this.checked;document.fnew_folder.menutype.disabled=!this.checked;if(this.checked && document.fnew_folder.sectionname.value.length!='' && document.fnew_folder.menuname.value=='') document.fnew_folder.menuname.value=document.fnew_folder.sectionname.value;fx1.disabled=!this.checked;fx2.disabled=!this.checked;">
                </td>
            </tr>
                ...
                <td>
                    <select name="menutype" <? if ($mkmenu != "Y") echo " disabled" ?>>
                        <? for ($i = 0, $l = count($arMenuTypes); $i < $l; $i++):?>
                            <option value="<? echo htmlspecialcharsex(
                                $arMenuTypes[$i][0]
                            ) ?>" <? if ($menutype == $arMenuTypes[$i][0]) echo " selected" ?>><? echo htmlspecialcharsex(
                                    "[" . $arMenuTypes[$i][0] . "] " . $arMenuTypes[$i][1]
                                ) ?></option>
                        <?endfor; ?>
                    </select>
                </td>
            ...
            <tr id="fx2"<? if ($mkmenu != "Y") echo " disabled" ?>>
                ...
                <td><input type="text" name="menuname"
                           value="<? echo htmlspecialcharsex($menuname) ?>"<? if ($mkmenu != "Y") echo " disabled" ?>>
                </td>
            </tr>
        <?endif; ?>

        <? if ($USER->CanDoFileOperation('fm_create_new_file', $arPath) && $USER->CanDoOperation(
                'fileman_admin_files'
            )):?>

            <tr>
                <td><?= GetMessage("FILEMAN_NEWFOLDER_MAKE_INDEX") ?></td>
                <td><input type="checkbox" name="mkindex" value="Y"<? if ($mkindex == "Y") echo " checked" ?>
                           onclick="document.fnew_folder.toedit.disabled=!this.checked;document.fnew_folder.template.disabled=!this.checked;ff1.disabled=!this.checked;ff2.disabled=!this.checked;">
                </td>
            </tr>
            <tr id="ff1">
                ...
                <td>
                    <select name="template" <? if ($mkindex != "Y") echo " disabled" ?>>
                        <? for ($i = 0, $l = count($arTemplates); $i < $l; $i++):?>
                            <option value="<? echo htmlspecialcharsex(
                                $arTemplates[$i]["file"]
                            ) ?>"<? if ($template == $arTemplates[$i]["file"]) echo " selected" ?>><? echo htmlspecialcharsex(
                                    $arTemplates[$i]["name"]
                                ) ?></option>
                        <?endfor; ?>
                    </select>
            ...
            <tr id="ff2">
                ...
                <td><input type="checkbox" name="toedit"
                           value="Y"<? if ($toedit == "Y") echo " checked" ?><? if ($mkindex != "Y") echo " disabled" ?>>
                </td>
            </tr>
        <?endif; ?>
        <?
        ...
        $tabControl->Buttons(
            array(
                "disabled" => false,
                "back_url" => ($back_url <> '' ? $back_url : "fileman_admin.php?" . $addUrl . "&site=" . $site . "&path=" . UrlEncode(
                        $path
                    ))
            )
        );
