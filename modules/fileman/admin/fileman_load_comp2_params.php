...
function GetProperties($componentName, $curTemplate = '')
{
    ...
    if (is_array($arProps['GROUPS'])) {
        foreach ($arProps['GROUPS'] as $k => $arGroup) {
            ?>window.arComp2Groups.push({name: '<?= CUtil::JSEscape($k) ?>', title: '<?= CUtil::JSEscape(
                $arGroup['NAME']
            ) ?>'});<?
        }
    }
    ...
    if (is_array($arParam)) {
        foreach ($arParam as $k => $prop) {
            if ($k == 'TYPE' && $prop == 'FILE') {
                ...
                $k,
                Array('FD_TARGET', 'FD_EXT', 'FD_UPLOAD', 'FD_MEDIALIB_TYPES', 'FD_USE_ONLY_MEDIALIB')
            )) {
                continue;
            }
...
function ShowFileDialogsScripts()
{
    ...
    for ($i = 0; $i < $l; $i++) {
        if ($arFD[$i]['USE_ML']) {
            ...
            ?>
            <script>window._bxMlBrowseButton_<?= mb_strtolower($arFD[$i]['NAME'])?> = '<?= CUtil::JSEscape(
                    $MLRes
                )?>';</script>
            <?
        }
