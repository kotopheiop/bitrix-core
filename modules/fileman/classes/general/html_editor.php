...
class CHTMLEditor
{
    ...
    private function Init($arParams)
    {
        ...
        $this->id = (isset($arParams['id']) && $arParams['id'] <> '') ? $arParams['id'] : 'bxeditor' . mb_substr(
                uniqid(mt_rand(), true),
                0,
                4
            );
        ...
        $this->jsConfig = array(
            ...
            'spellcheck_path' => $basePath . 'html-spell.js?v=' . filemtime(
                    $_SERVER['DOCUMENT_ROOT'] . $basePath . 'html-spell.js'
                ),
    ...
    function BuildSceleton($display = true)
    {
        ...
        ?>
        <div class="bx-html-editor" id="bx-html-editor-<?= $this->id ?>"
             style="width:<?= $width . $widthUnit ?>; height:<?= $height . $heightUnit ?>; <?= $display ? '' : 'display: none;' ?>">
            ...
            <div class="bxhtmled-search-cnt" id="bx-html-editor-search-cnt-<?= $this->id ?>"
            ...
            <div class="bxhtmled-taskbar-cnt bxhtmled-taskbar-hidden" id="bx-html-editor-tskbr-cnt-<?= $this->id ?>">
                ...
                <div class="bxhtmled-taskbar-search-nothing"
                     id="bxhed-tskbr-search-nothing-<?= $this->id ?>"><?= GetMessage('HTMLED_SEARCH_NOTHING') ?></div>
                <div class="bxhtmled-taskbar-search-cont" id="bxhed-tskbr-search-cnt-<?= $this->id ?>"
                     data-bx-type="taskbar_search">
                    <div class="bxhtmled-search-alignment" id="bxhed-tskbr-search-ali-<?= $this->id ?>">
                        <input type="text" class="bxhtmled-search-inp" id="bxhed-tskbr-search-inp-<?= $this->id ?>"
                               placeholder="<?= GetMessage('HTMLED_SEARCH_PLACEHOLDER') ?>"/>
                    </div>
                    <div class="bxhtmled-search-cancel" data-bx-type="taskbar_search_cancel"
                         title="<?= GetMessage('HTMLED_SEARCH_CANCEL') ?>"></div>
                </div>
            </div>
    ...
    {
        $mess_lang = \Bitrix\Main\Localization\Loc::loadLanguageFile(
            $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/fileman/classes/general/html_editor_js.php'
        );
        ?>
    ...
    public static function GetSnippets($templateId, $bClearCache = false)
    {
        return array(
            ...
            'rootDefaultFilename' => CSnippets::GetDefaultFileName(
                $_SERVER["DOCUMENT_ROOT"] . BX_PERSONAL_ROOT . "/templates/" . $templateId . "/snippets"
            )
        );
    }
    ...
    public static function RequestAction($action = '')
    {
        ...
        switch ($action) {
            ...
            case "spellcheck_add_word":
                CUtil::JSPostUnEscape();
                $spellChecker = new CSpellchecker(
                    array(
                        "lang" => $_REQUEST['lang'],
                        "skip_length" => 2,
                    ...
                    array(
                        "events" => array(
                            "onFileIsUploaded" => function ($hash, &$file, &$package, &$upload, &$error) {
                                ...
                                if (empty($error) && $io->ValidateFilenameString($fileName) && $io->ValidatePathString(
                                        self::GetUploadPath() . $fileName
                                    )) {
    ...
    public static function ShowResponse($reqId = false, $Res = false)
    {
        if ($Res !== false) {
            ...
            if ($reqId) {
                ?>
                <script>top.BXHtmlEditorAjaxResponse['<?= $reqId?>'] = <?= \Bitrix\Main\Web\Json::encode(
                        $Res
                    )?>;</script>
                <?
            }
                ...
                if ($tName == $template) {
                    ...
                    if (is_array($arTemplateProps)) {
                        foreach ($arTemplateProps as $k => $arTemplateProp) {
                            $result['templ_props'][] = self::_HandleComponentParam(
                                $k,
                                $arTemplateProp,
                                $arProps['GROUPS']
                            );
    ...
    public static function GetSiteTemplateParams($templateId, $siteId)
    {
        ...
        $params["STYLES"] = preg_replace(
            "/(url\(\"?)images\//is",
            "\\1" . $params['SITE_TEMPLATE_PATH'] . '/images/',
            $params["STYLES"]
        );

        if (is_array($params['EDITOR_STYLES'])) {
            for ($i = 0, $l = count($params['EDITOR_STYLES']); $i < $l; $i++) {
                $params['EDITOR_STYLES'][$i] = $params['EDITOR_STYLES'][$i] . '?' . @filemtime(
                        $_SERVER['DOCUMENT_ROOT'] . $params['EDITOR_STYLES'][$i]
                    );
            }
        }
    ...
    private static function GetSettingKey($params = array())
    {
        ...
        if (isset($params["controlsMap"]) && is_array($params["controlsMap"])) {
            foreach ($params["controlsMap"] as $control) {
                if ($control && (mb_strtolower($control['id']) == 'bbcode' || mb_strtolower(
                            $control['id']
                        ) == 'changeview')) {
