...
if (!defined("CACHED_b_iblock")) {
    define("CACHED_b_iblock", 36000);
}
if (!defined("CACHED_b_iblock_bucket_size")) {
    define("CACHED_b_iblock_bucket_size", 20);
}
if (!defined("CACHED_b_iblock_property_enum")) {
    define("CACHED_b_iblock_property_enum", 36000);
}
if (!defined("CACHED_b_iblock_property_enum_bucket_size")) {
 ...
 * @deprecated No longer used by internal code and not recommended.
 */
function GetIBlockElementListEx(
    $type,
    $arTypesInc = array(),
    $arTypesExc = array(),
    $arOrder = array("sort" => "asc"),
    ...
    return GetIBlockElementListExLang(
        SITE_ID,
        $type,
        $arTypesInc,
        $arTypesExc,
 ...
 * @deprecated No longer used by internal code and not recommended.
 */
function GetIBlockElementCountEx(
    $type,
    $arTypesInc = array(),
    $arTypesExc = array(),
    $arOrder = array("sort" => "asc"),
 ...
 * @deprecated No longer used by internal code and not recommended.
 */
function GetIBlockElementCountExLang(
    $lang,
    $type,
    $arTypesInc = array(),
    $arTypesExc = array(),
 ...
 * @deprecated No longer used by internal code and not recommended.
 */
function GetIBlockElementListExLang(
    $lang,
    $type,
    $arTypesInc = array(),
    $arTypesExc = array(),
    ...
    if (is_array($cnt)) {
        $arNavParams = $cnt;
    } //array("nPageSize"=>$cnt, "bShowAll"=>false);
    elseif ($cnt > 0) {
 ...
 * @deprecated No longer used by internal code and not recommended.
 */
function _GetIBlockElementListExLang_tmp(
    $lang,
    $type,
    $arTypesInc = array(),
    $arTypesExc = array(),
 ...
 * @deprecated No longer used by internal code and not recommended.
 */
function GetIBlockSectionListWithCnt(
    $IBLOCK,
    $SECT_ID = false,
 ...
 * @return string
 */
function GetIBlockDropDownListEx(
    $IBLOCK_ID,
    $strTypeName,
    $strIBlockName,
    $arFilter = false,
    ...
    $onChangeType = 'OnType_' . $filterId . '_Changed(this, \'' . CUtil::JSEscape(
            $strIBlockName
        ) . '\');' . $onChangeType . ';';
    $onChangeIBlock = trim($onChangeIBlock);

    $html .= '<select name="' . $htmlTypeName . '" id="' . $htmlTypeName . '" onchange="' . htmlspecialcharsbx(
            $onChangeType
        ) . '" ' . $strAddType . '>' . "\n";
    foreach ($arTypes[$filterId] as $key => $value) {
        ...
        $html .= '<option value="' . htmlspecialcharsbx(
                $key
            ) . '"' . ($IBLOCK_TYPE === $key ? ' selected' : '') . '>' . htmlspecialcharsEx(
                $value
            ) . '</option>' . "\n";
    }
    $html .= "</select>\n";
    $html .= "&nbsp;\n";
    $html .= '<select name="' . $htmlIBlockName . '" id="' . $htmlIBlockName . '"' . ($onChangeIBlock != '' ? ' onchange="' . htmlspecialcharsbx(
                $onChangeIBlock
            ) . '"' : '') . ' ' . $strAddIBlock . '>' . "\n";
    foreach ($arIBlocks[$filterId][$IBLOCK_TYPE] as $key => $value) {
        $html .= '<option value="' . htmlspecialcharsbx(
                $key
            ) . '"' . ($IBLOCK_ID == $key ? ' selected' : '') . '>' . htmlspecialcharsEx($value) . '</option>' . "\n";
    }
    $html .= "</select>\n";
 ...
 * @return string
 */
function GetIBlockDropDownList(
    $IBLOCK_ID,
    $strTypeName,
    $strIBlockName,
    $arFilter = false,
    ...
    return GetIBlockDropDownListEx(
        $IBLOCK_ID,
        $strTypeName,
        $strIBlockName,
        $arFilter,
...
function ImportXMLFile(
    ...
    if ($file_name <> '') {
        if (
            ...
            if ((mb_strlen(
                        $FILE_NAME
                    ) > 1) && ($FILE_NAME === "/" . $filename) && ($APPLICATION->GetFileAccessPermission(
                        $FILE_NAME
                    ) >= "W")) {
