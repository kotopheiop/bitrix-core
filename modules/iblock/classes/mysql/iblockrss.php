...
class CIBlockRSS extends CAllIBlockRSS
{
    ...
    public static function UpdateCache($cacheKey, $CACHE, $HOURS_CACHE, $bCACHED)
    {
        ...
        if ($bCACHED) {
            ...
            $db_res = $DB->Query(
                ...
                "VALUES('" . $DB->ForSql($cacheKey, 0) . "', '" . $DB->ForSql(
                    $CACHE,
                    0
                ) . "', date_add(NOW(), interval " . intval($TTL) . " second)) "
            );
        }
    ...
    public static function GetRSSText($arIBLOCK, $LIMIT_NUM = false, $LIMIT_DAY = false, $yandex = false)
    {
            ...
            if ($yandex) {
                ...
                if ($squareSize > 0) {
                    ...
                    if ($squarePicture) {
                        ...
                        $strRes .= "<yandex:logo type=\"square\">" . htmlspecialcharsbx(
                                $squareImage
                            ) . "</yandex:logo>\n";
                    }
                }
        ...
        $items = CIBlockElement::GetList(
            array("ACTIVE_FROM" => "DESC", "SORT" => "ASC", "ID" => "DESC"),
            $arFilter,
            false,
            $nav
        );
        ...
        while ($arItem = $items->GetNext()) {
            ...
            while ($arProp = $props->Fetch()) {
                if ($arProp["CODE"] <> '') {
                    $arProps[$arProp["CODE"]] = Array(
                        ...
                        "VALUE" => htmlspecialcharsex($arProp["VALUE"])
                    );
                } else {
            ...
            $strRes .= "<item>\n";
            if ($arNodes["title"] <> '') {
                $strRes .= "<title>" . htmlspecialcharsbx(
                        CIBlockRSS::ExtractProperties($arNodes["title"], $arProps, $arItem)
                    ) . "</title>\n";
            } else {
            ...
            if ($arNodes["link"] <> '') {
                ...
                $strRes .= "<link>" . $protocol . htmlspecialcharsbx(
                        $serverName
                    ) . (($arLinkProp["VALUE"]) ? $arLinkProp["VALUE"] : $arItem["DETAIL_PAGE_URL"]) . "</link>\n";
            }
            if ($arNodes["description"] <> '') {
                $strRes .= "<description>" . htmlspecialcharsbx(
                        CIBlockRSS::ExtractProperties($arNodes["description"], $arProps, $arItem)
                    ) . "</description>\n";
            } else {
                $strRes .= "<description>" . (($arItem["PREVIEW_TEXT"] || $yandex) ? htmlspecialcharsbx(
                        $arItem["PREVIEW_TEXT"]
                    ) : htmlspecialcharsbx($arItem["DETAIL_TEXT"])) . "</description>\n";
            }
            if ($arNodes["enclosure"] <> '') {
                $strRes .= "<enclosure url=\"" . htmlspecialcharsbx(
                        CIBlockRSS::ExtractProperties($arNodes["enclosure"], $arProps, $arItem)
                    ) . "\" length=\"" . htmlspecialcharsbx(
                        CIBlockRSS::ExtractProperties($arNodes["enclosure_length"], $arProps, $arItem)
                    ) . "\" type=\"" . htmlspecialcharsbx(
                        CIBlockRSS::ExtractProperties($arNodes["enclosure_type"], $arProps, $arItem)
                    ) . "\"/>\n";
            } else {
                ...
                if ($db_img_arr) {
                    ...
                    $strRes .= "<enclosure url=\"" . htmlspecialcharsbx(
                            $strImage
                        ) . "\" length=\"" . $db_img_arr["FILE_SIZE"] . "\" type=\"" . $db_img_arr["CONTENT_TYPE"] . "\" width=\"" . $db_img_arr["WIDTH"] . "\" height=\"" . $db_img_arr["HEIGHT"] . "\"/>\n";
                }
            }
            if ($arNodes["category"] <> '') {
                $strRes .= "<category>" . htmlspecialcharsbx(
                        CIBlockRSS::ExtractProperties($arNodes["category"], $arProps, $arItem)
                    ) . "</category>\n";
            } else {
                $strPath = "";
            ...
            if ($arNodes["pubDate"] <> '') {
                $strRes .= "<pubDate>" . htmlspecialcharsbx(
                        CIBlockRSS::ExtractProperties($arNodes["pubDate"], $arProps, $arItem)
                    ) . "</pubDate>\n";
            } else {
    ...
    public static function PreGenerateRSS($IBLOCK_ID, $yandex = true)
    {
        ...
        $strSql =
            "SELECT DISTINCT B.*, C.CHARSET, S.SERVER_NAME, " . $DB->DateToCharFunction(
                "B.TIMESTAMP_X"
            ) . " as TIMESTAMP_X " .
        ...
        $dbr = $DB->Query($strSql);
        $bAccessable = false;
        if (($arIBlock = $dbr->GetNext(
            )) && ($arIBlock["RSS_FILE_ACTIVE"] == "Y" && !$yandex || $arIBlock["RSS_YANDEX_ACTIVE"] == "Y" && $yandex)) {
            $bAccessable = true;
        }
