            ...
            foreach ($arFilter as $key => $val) {
                ...
                switch ($key) {
                    case "ID":
                        ...
                        break;
                    case "TIMESTAMP_FROM":
                        $strSqlSearch .= " and E.TIMESTAMP_X>=FROM_UNIXTIME('" . MkDateTime(
                                FmtDate($val, "D.M.Y"),
                                "d.m.Y"
                            ) . "')";
                        break;
    ...
    function prepareSql(
        ...
        $arIblockElementFields = array(
            ...
            $formatActiveDates
                ...
                :
                "IF(EXTRACT(HOUR_SECOND FROM BE.ACTIVE_FROM)>0, " . $DB->DateToCharFunction(
                    "BE.ACTIVE_FROM",
                    "FULL"
                ) . ", " . $DB->DateToCharFunction("BE.ACTIVE_FROM", "SHORT") . ")"
            ),
            "ACTIVE_TO" => (
            $formatActiveDates
                ?
                ...
                :
                "IF(EXTRACT(HOUR_SECOND FROM BE.ACTIVE_TO)>0, " . $DB->DateToCharFunction(
                    "BE.ACTIVE_TO",
                    "FULL"
                ) . ", " . $DB->DateToCharFunction("BE.ACTIVE_TO", "SHORT") . ")"
            ),
            "DATE_ACTIVE_FROM" => (
            $formatActiveDates
                ?
        ...
        if (!empty($arAddSelectFields) || !empty($arAddWhereFields) || !empty($arAddOrderByFields)) {
            if (Loader::includeModule("catalog")) {
                $catalogQueryResult = \CProductQueryBuilder::makeQuery(
                    array(
        ...
        $sFrom = "
            ...
            . (in_array("CREATED_USER_NAME", $arSelectFields) || in_array(
    ...
    public function Update(
            ...
            if (
                ...
                if (
                    ...
                    $rsElement = CIBlockElement::GetList(
                        ...
                        Array(
                            "ID" => $ar_wf_element["ID"],
                            "IBLOCK_ID" => $ar_wf_element["IBLOCK_ID"],
                            "SHOW_HISTORY" => "Y"
                        ),
                        false,
                        false,
        ...
        if ($this->searchIncluded) {
            $arFields["SEARCHABLE_CONTENT"] = ToUpper(
                ...
                ($searchableFields['PREVIEW_TEXT_TYPE'] == "html" ? HTMLToTxt(
                    $searchableFields['PREVIEW_TEXT']
                ) : $searchableFields['PREVIEW_TEXT']) . "\r\n" .
                ($searchableFields['DETAIL_TEXT_TYPE'] == "html" ? HTMLToTxt(
                    $searchableFields['DETAIL_TEXT']
                ) : $searchableFields['DETAIL_TEXT'])
            );
        }
        ...
        if (!$this->CheckFields($arFields, $ID, $bCheckDiskQuota) || $strWarning != '') {
            ...
            if ($bWorkFlow) {
                    ...
                    if (is_array($arFields["PREVIEW_PICTURE"])) {
                        if (
                            ...
                            if (array_key_exists("description", $arFields["PREVIEW_PICTURE"])) {
                                ...
                                if ($arFields["PREVIEW_PICTURE"]["description"] != $arFile["DESCRIPTION"]) {//Description updated, so it's new file
                    ...
                    if (is_array($arFields["DETAIL_PICTURE"])) {
                        if (
                            ...
                            if (array_key_exists("description", $arFields["DETAIL_PICTURE"])) {
                                ...
                                if ($arFields["DETAIL_PICTURE"]["description"] != $arFile["DESCRIPTION"]) {//Description updated, so it's new file
                ...
                if ($NID > 0) {
                    if ($arFields["WF_STATUS_ID"] == 1) {
                        ...
                        $DB->Query(
                            "UPDATE b_iblock_element SET TIMESTAMP_X=TIMESTAMP_X, WF_NEW=null WHERE WF_PARENT_ELEMENT_ID=" . $ID
                        );
                    ...
                    if ($ar_element["WF_STATUS_ID"] != 1
                        ...
                        $DB->Query(
                            "UPDATE b_iblock_element SET TIMESTAMP_X=TIMESTAMP_X, WF_STATUS_ID=" . (int)$arFields["WF_STATUS_ID"] . " WHERE ID=" . $ID
                        );
                    }
                }
                ...
                if (
                    (is_set(
                            $arFields,
                            "WF_STATUS_ID"
                        ) && $arFields["WF_STATUS_ID"] != 1 && $ar_element["WF_STATUS_ID"] == 1)
                ...
                $rs = $DB->Query(
                    "SELECT PREVIEW_PICTURE, DETAIL_PICTURE from b_iblock_element WHERE ID = " . (int)$NID
                );
            ...
            if (is_set(
                    $arFields,
                    "TIMESTAMP_X"
                ) && ($arFields["TIMESTAMP_X"] === null || $arFields["TIMESTAMP_X"] === false)) {
                $bTimeStampNA = true;
    ...
    public static function SetPropertyValues($ELEMENT_ID, $IBLOCK_ID, $PROPERTY_VALUES, $PROPERTY_CODE = false)
    {
        ...
        foreach (GetModuleEvents("iblock", "OnIBlockElementSetPropertyValues", true) as $arEvent) {
            ExecuteModuleEventEx(
                $arEvent,
                array($ELEMENT_ID, $IBLOCK_ID, $PROPERTY_VALUES, $PROPERTY_CODE, $ar_prop, $arDBProps)
            );
        }
        if (isset($arEvent)) {
            unset($arEvent);
                            ...
                            if ($prop["VERSION"] == 2 && $prop["MULTIPLE"] == "N") {
                                ...
                                $DB->Query(
									...
									SET VALUE = '" . $DB->ForSql($val) . "'
										...
										" . ($val_desc !== false ? ",DESCRIPTION = '" . $DB->ForSql(
                                            $val_desc,
                                            255
                                        ) . "'" : "") . "
									WHERE ID=" . $res["ID"] . "
