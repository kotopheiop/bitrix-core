...
class CIBlockResult extends CDBResult
{
    ...
    function Fetch()
    {
        ...
        if ($res) {
            ...
            if (isset($res["ID"]) && $res["ID"] != "" && !empty($this->arIBlockMultProps) && is_array(
                ...
                foreach ($this->arIBlockMultProps as $field_name => $db_prop) {
                    if (array_key_exists($field_name, $res)) {
                        ...
                        if ($update) {
                            ...
                            $arUpdate["b_iblock_element_prop_s" . $db_prop["IBLOCK_ID"]]["PROPERTY_" . $db_prop["ORIG_ID"]] = serialize(
                                array(
                ...
                foreach ($this->arIBlockConvProps as $strFieldName => $arCallback) {
                    if (is_array($res[$strFieldName])) {
                        ...
                        $arValue = call_user_func_array(
                            ...
                            array(
                                $arCallback["PROPERTY"],
                                array("VALUE" => $res[$strFieldName], "DESCRIPTION" => "")
                            )
                        );
            ...
            if (isset($res["UC_ID"])) {
                $res["CREATED_BY_FORMATTED"] = CUser::FormatName(
                    $this->nameTemplate,
                    array(
                        "NAME" => $res["UC_NAME"],
                    ...
                    if ($this->arSectionContext) {
                        ...
                        if (
                            ...
                            if (!isset($arSectionPathCache[$this->arSectionContext["ID"]])) {
                                ...
                                if (!empty($rs)) {
                                    foreach ($rs as $a) {
                                        $arSectionPathCache[$this->arSectionContext["ID"]] .= rawurlencode(
                                                $a["CODE"]
                                            ) . "/";
                                    }
                                    unset($a);
