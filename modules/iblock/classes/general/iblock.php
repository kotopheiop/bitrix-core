...
class CAllIBlock
{
    ...
    public static function AddPanelButtons($mode, $componentName, $arButtons)
    {
        ...
        $arImages = array(
            "add_element" => (defined(
                "PANEL_ADD_ELEMENT_BTN"
            )) ? PANEL_ADD_ELEMENT_BTN : "/bitrix/images/iblock/icons/new_element.gif",
            "edit_element" => (defined(
                "PANEL_EDIT_ELEMENT_BTN"
            )) ? PANEL_EDIT_ELEMENT_BTN : "/bitrix/images/iblock/icons/edit_element.gif",
            "edit_iblock" => (defined(
                "PANEL_EDIT_IBLOCK_BTN"
            )) ? PANEL_EDIT_IBLOCK_BTN : "/bitrix/images/iblock/icons/edit_iblock.gif",
            "history_element" => (defined(
                "PANEL_HISTORY_ELEMENT_BTN"
            )) ? PANEL_HISTORY_ELEMENT_BTN : "/bitrix/images/iblock/icons/history.gif",
            "edit_section" => (defined(
                "PANEL_EDIT_SECTION_BTN"
            )) ? PANEL_EDIT_SECTION_BTN : "/bitrix/images/iblock/icons/edit_section.gif",
            "add_section" => (defined(
                "PANEL_ADD_SECTION_BTN"
            )) ? PANEL_ADD_SECTION_BTN : "/bitrix/images/iblock/icons/new_section.gif",
    ...
    public static function GetPanelButtons($IBLOCK_ID = 0, $ELEMENT_ID = 0, $SECTION_ID = 0, $arOptions = array())
    {
        ...
        $bCatalog = false;
        $useCatalogButtons = (($ELEMENT_ID <= 0 || isset($arOptions['SHOW_CATALOG_BUTTONS'])) && !empty($arOptions['USE_CATALOG_BUTTONS']) && is_array(
                $arOptions['USE_CATALOG_BUTTONS']
            ));
        ...
        if ($ELEMENT_ID > 0 && CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ELEMENT_ID, "element_edit")) {
            $url = "/bitrix/admin/" . CIBlock::GetAdminElementEditLink(
                    $IBLOCK_ID,
                    $ELEMENT_ID,
                    array(
            ...
            if ($bWorkflow) {
                $url = "/bitrix/admin/iblock_history_list.php?type=" . $arIBlock["IBLOCK_TYPE_ID"] . "&lang=" . LANGUAGE_ID . "&IBLOCK_ID=" . $IBLOCK_ID . "&ELEMENT_ID=" . $ELEMENT_ID . "&filter_section=" . $SECTION_ID . "&return_url=" . UrlEncode(
                        $return_url["history_element"]
                    );
                $arButton = array(
        ...
        if (CIBlockSectionRights::UserHasRightTo($IBLOCK_ID, $SECTION_ID, "section_element_bind")) {
            ...
            if ($useCatalogButtons) {
                ...
                CCatalogAdminTools::setCatalogPanelButtons(
                    $arButtons,
                    $IBLOCK_ID,
                    $catalogButtons,
                    $params,
        ...
        if ($ELEMENT_ID > 0 && CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ELEMENT_ID, "element_delete")) {
            ...
            $url .= '&ID=' . (preg_match(
                    ...
                    $url
                ) ? "E" : "") . $ELEMENT_ID . "&return_url=" . UrlEncode($return_url["delete_element"]);
            ...
            $arButton = array(
                ...
                "ACTION" => "javascript:if(confirm('" . GetMessageJS(
                        "IBLOCK_PANEL_ELEMENT_DEL_CONF"
                    ) . "'))jsUtils.Redirect([], '" . CUtil::JSEscape($url) . "')",
                "ACTION_URL" => $url,
                "ONCLICK" => "if(confirm('" . GetMessageJS(
                        "IBLOCK_PANEL_ELEMENT_DEL_CONF"
                    ) . "'))jsUtils.Redirect([], '" . CUtil::JSEscape($url) . "')",
        ...
        if ($ELEMENT_ID <= 0 && $bSectionButtons) {
            ...
            if (($arIBTYPE = $rsIBTYPE->Fetch()) && ($arIBTYPE["SECTIONS"] == "Y")) {
                if ($SECTION_ID > 0 && CIBlockSectionRights::UserHasRightTo($IBLOCK_ID, $SECTION_ID, "section_edit")) {
                    ...
                    $url = "/bitrix/admin/" . CIBlock::GetAdminSectionEditLink(
                            $IBLOCK_ID,
                            $SECTION_ID,
                            array(
                                "force_catalog" => $bCatalog,
                ...
                //Delete section
                if ($SECTION_ID > 0 && CIBlockSectionRights::UserHasRightTo(
                        $IBLOCK_ID,
                        $SECTION_ID,
                        "section_delete"
                    )) {
                    ...
                    $url .= '&ID[]=' . (preg_match(
                            '/^iblock_list_admin\.php/',
                            $url
                        ) ? "S" : "") . $SECTION_ID . "&return_url=" . UrlEncode($return_url["delete_section"]);
                    $url = "/bitrix/admin/" . $url;
        ...
        if (($IBLOCK_ID > 0) && CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, "iblock_admin_display")) {
            ...
            if ($SECTION_ID > 0) {
                $url = "/bitrix/admin/" . CIBlock::GetAdminElementListLink(
                        $IBLOCK_ID,
                        array('find_section_section' => $SECTION_ID)
                    );
            } else {
                $url = "/bitrix/admin/" . CIBlock::GetAdminElementListLink(
                        $IBLOCK_ID,
            ...
            $url = "/bitrix/admin/" . CIBlock::GetAdminSectionListLink(
                    $IBLOCK_ID,
                    array('find_section_section' => $SECTION_ID)
                );
            $arButton = array(
            ...
            if (CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, "iblock_edit")) {
                $url = "/bitrix/admin/iblock_edit.php?type=" . $arIBlock["IBLOCK_TYPE_ID"] . "&lang=" . LANGUAGE_ID . "&ID=" . $IBLOCK_ID . "&return_url=" . UrlEncode(
                        $return_url["edit_iblock"]
                    );
                $arButton = array(
                    "TEXT" => GetMessage(
                        "IBLOCK_PANEL_EDIT_IBLOCK_BUTTON",
                        array("#IBLOCK_NAME#" => $arIBlock["NAME"])
                    ),
                    "TITLE" => GetMessage(
                        "IBLOCK_PANEL_EDIT_IBLOCK_BUTTON",
                        array("#IBLOCK_NAME#" => $arIBlock["NAME"])
    ...
    public static function GetSite($iblock_id)
    {
        ...
        $strSql = "SELECT L.*, BS.* FROM b_iblock_site BS, b_lang L WHERE L.LID=BS.SITE_ID AND BS.IBLOCK_ID=" . intval(
                $iblock_id
            );
    ...
    function Update($ID, $arFields)
    {
        ...
        if (is_set(
                $arFields,
                "SECTION_CHOOSER"
            ) && $arFields["SECTION_CHOOSER"] != "D" && $arFields["SECTION_CHOOSER"] != "P") {
            $arFields["SECTION_CHOOSER"] = "L";
        }
        ...
        if (is_set(
                $arFields,
                "PROPERTY_INDEX"
            ) && $arFields["PROPERTY_INDEX"] != "I" && $arFields["PROPERTY_INDEX"] != "Y") {
        ...
        if (is_set($arFields, "API_CODE")) {
            if ($arFields['API_CODE'] == '') {
                ...
                if (!preg_match('/^[a-z][a-z0-9]{0,49}$/i', $arFields['API_CODE'])) {
                    ...
                    $count = Iblock\IblockTable::getCount(
        ...
        if ($ID > 0) {
                ...
                if (is_array($arDef)) {
                    $arDef = serialize(
                        array(
                            ...
                            "COMPRESSION" => intval($arDef["COMPRESSION"]) > 100 ? 100 : (intval(
                                $arDef["COMPRESSION"]
                            ) > 0 ? intval($arDef["COMPRESSION"]) : ""),
                            ...
                            "WATERMARK_TEXT_SIZE" => intval($arDef["WATERMARK_TEXT_SIZE"]) > 0 ? intval(
                                $arDef["WATERMARK_TEXT_SIZE"]
                            ) : "",
                            ...
                            "WATERMARK_FILE_ALPHA" => intval($arDef["WATERMARK_FILE_ALPHA"]) > 0 ? intval(
                                $arDef["WATERMARK_FILE_ALPHA"]
                            ) : "",
            ...
            while ($ar = $res->Fetch()) {
                if (array_key_exists($ar["FIELD_ID"], $arFields) && array_key_exists($ar["FIELD_ID"], $arDefFields)) {
                    ...
                    $DB->Query(
                        "DELETE FROM b_iblock_fields WHERE IBLOCK_ID = " . $ID . " AND FIELD_ID = '" . $DB->ForSQL(
                            $ar["FIELD_ID"]
                        ) . "'"
                    );
                    $arUpdate = array();
    ...
    public static function OnBeforeLangDelete($lang)
    {
        ...
        if (count($arIBlocks) > 0) {
            $APPLICATION->ThrowException(
                GetMessage("IBLOCK_SITE_LINKS_EXISTS", array("#ID_LIST#" => implode(", ", $arIBlocks)))
            );
            return false;
        } else {
            return true;
                ...
                switch ($type) {
                    case "string_equal":
                        if ($cOperationType == "?") {
                            ...
                            $res[] = ($cOperationType == "NS" ? " " . $fname . " IS NULL OR NOT " : "") . "(" . CIBlock::_Upper(
                                    $fname
                                ) . " LIKE " . CIBlock::_Upper("'%" . CIBlock::ForLIKE($val) . "%'") . ")";
                        } elseif (($cOperationType == "B" || $cOperationType == "NB") && is_array($val) && count(
                                $val
                            ) == 2) {
                            $res[] = ($cOperationType == "NB" ? " " . $fname . " IS NULL OR NOT " : "") . "(" . CIBlock::_Upper(
                                    $fname
                                ) . " " . $strOperation[0] . " '" . CIBlock::_Upper(
                                    $DB->ForSql($val[0])
                                ) . "' " . $strOperation[1] . " '" . CIBlock::_Upper($DB->ForSql($val[1])) . "')";
                        } else {
                            if ((string)$val == '') {
                    ...
                    case "string":
                        if ($cOperationType == "?") {
                            ...
                            if ((string)$val == '') {
                                $res[] = ($bNegative ? "NOT" : "") . "(" . $fname . " IS NULL OR " . $DB->Length(
                                        $fname
                                    ) . "<=0)";
                            } else {
                                if ($strOperation == "=" && $cOperationType != "I" && $cOperationType != "NI") {
                                    $res[] = ($cOperationType == "N" ? " " . $fname . " IS NULL OR NOT " : "") . "(" . ($DB->type == "ORACLE" ? CIBlock::_Upper(
                                                ...
                                                "'" . $DB->ForSqlLike($val) . "'"
                                            ) . " ESCAPE '\\'" : $fname . " LIKE '" . $DB->ForSqlLike(
                                                $val
                                            ) . "'") . ")";
                                } else {
                                    $res[] = ($bNegative ? " " . $fname . " IS NULL OR NOT " : "") . "(" . ($DB->type == "ORACLE" ? CIBlock::_Upper(
                                                $fname
                                            ) . " " . $strOperation . " " . CIBlock::_Upper(
                                                "'" . $DB->ForSql($val) . "'"
                                            ) . " " : $fname . " " . $strOperation . " '" . $DB->ForSql(
                                                $val
                                            ) . "'") . ")";
                                }
                            }
                    ...
                    case "date":
                        if (!is_array($val) && $val == '') {
                            ...
                            $res[] = ($cOperationType == 'NB' ? ' ' . $fname . ' IS NULL OR NOT ' : '') . '(' . $fname . ' ' . $strOperation[0] . ' ' . $DB->CharToDateFunction(
                                    $DB->ForSql($val[0]),
                                    "FULL"
                                ) . ' ' . $strOperation[1] . ' ' . $DB->CharToDateFunction(
                                    $DB->ForSql($val[1]),
                                    "FULL"
                                ) . ')';
                        } else {
                            $res[] = ($bNegative ? " " . $fname . " IS NULL OR NOT " : "") . "(" . $fname . " " . $strOperation . " " . $DB->CharToDateFunction(
                                    $DB->ForSql($val),
                                    "FULL"
                                ) . ")";
                        }
                    ...
                    case "number":
                        if ($val === '' || $val === null || $val === false) {
                            ...
                            if (is_array($val)) {
                                if (count($val) == 2) {
                                    $res[] = ($cOperationType == 'NB' ? ' ' . $fname . ' IS NULL OR NOT ' : '') . '(' . $fname . ' ' . $strOperation[0] . ' \'' . DoubleVal(
                                            $val[0]
                                        ) . '\' ' . $strOperation[1] . ' \'' . DoubleVal($val[1]) . '\')';
                                } else {
                                    $res[] = ($cOperationType == 'NB' ? ' ' . $fname . ' IS NULL OR NOT ' : '') . '(' . $fname . ' = \'' . DoubleVal(
                                            array_pop($val[0])
                                        ) . '\')';
                                }
                            } else {
                            ...
                            $res[] = " " . $fname . " IS NULL OR NOT (" . $fname . " " . $strOperation . " '" . DoubleVal(
                                    $val
                                ) . "')";
                            if ($strOperation == '=') {
                                $arIn[] = DoubleVal($val);
        ...
        if (
            ...
            if (!isset($arIBlockCache[$OF_IBLOCK_ID])) {
                ...
                if (is_array($arIBlockCache[$OF_IBLOCK_ID])) {
                    $arIBlockCache[$OF_IBLOCK_ID]["PRODUCT_IBLOCK"] = CIBlock::GetArrayByID(
                        $arIBlockCache[$OF_IBLOCK_ID]["PRODUCT_IBLOCK_ID"]
                    );
                }
            }
            ...
            if (is_array($arIBlockCache[$OF_IBLOCK_ID])) {
                ...
                if (is_array($arElementCache[$OF_ELEMENT_ID])) {
                    $product_url = CIBlock::ReplaceDetailUrl(
                        $arIBlockCache[$OF_IBLOCK_ID]["PRODUCT_IBLOCK"]["DETAIL_PAGE_URL"],
                        $arElementCache[$OF_ELEMENT_ID],
                        $server_name,
                        $arrType
                    );
                }
        ...
        while ($arIBlock = $dbrIBlock->Fetch()) {
            ...
            if ($arIBlock["INDEX_ELEMENT"] == 'Y' && ($strNSFilter2 !== false)) {
                ...
                while ($arIBlockElement = $dbrIBlockElement->Fetch()) {
                    ...
                    $dbrProperties = CIBlockElement::GetProperty(
                        $IBLOCK_ID,
                        $arIBlockElement["ID"],
                        "sort",
                        "asc",
            ...
            if ($arIBlock["INDEX_SECTION"] == 'Y') {
                ...
                while ($arIBlockSection = $dbrIBlockSection->Fetch()) {
                    ...
                    $BODY .= $USER_FIELD_MANAGER->OnSearchIndex(
                        "IBLOCK_" . $arIBlockSection["IBLOCK_ID"] . "_SECTION",
                        $arIBlockSection["ID"]
                    );
    ...
    public static function GetAdminSubElementEditLink(
        $IBLOCK_ID,
        $ELEMENT_ID,
        $SUBELEMENT_ID,
        $arParams = array(),
        ...
        $url = ($absoluteUrl ? $selfFolderUrl : '') . 'iblock_subelement_edit.php?IBLOCK_ID=' . (int)$IBLOCK_ID . '&type=' . urlencode(
                CIBlock::GetArrayByID($IBLOCK_ID, 'IBLOCK_TYPE_ID')
            );
    ...
    public static function GetAdminSectionListScriptName($IBLOCK_ID, $arParams = array())
    {
        ...
        if ((defined("CATALOG_PRODUCT") || array_key_exists('catalog', $arParams)) && !array_key_exists(
                "menu",
                $arParams
            )) {
            if (CIBlock::GetAdminListMode($IBLOCK_ID) == Iblock\IblockTable::LIST_MODE_COMBINED) {
