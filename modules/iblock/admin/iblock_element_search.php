...
while ($arFProps = $dbrFProps->GetNext()) {
    ...
    $arFProps["PROPERTY_USER_TYPE"] = ($arFProps["USER_TYPE"] !== '' ? CIBlockProperty::GetUserType(
        $arFProps["USER_TYPE"]
    ) : array());
...
foreach ($arProps as $prop) {
    ...
    if (!empty($prop['PROPERTY_USER_TYPE']) && isset($prop["PROPERTY_USER_TYPE"]["AddFilterFields"])) {
        call_user_func_array(
            $prop["PROPERTY_USER_TYPE"]["AddFilterFields"],
            array(
                $prop,
...
$arHeader[] = array(
    "id" => "ACTIVE_FROM",
    "content" => GetMessage("IBLOCK_FIELD_ACTIVE_FROM"),
...
$arHeader[] = array(
    ...
    "content" => GetMessage("IBLOCK_FIELD_CREATED_USER_NAME"),
    "sort" => "created_by"
);
...
$arHeader[] = array(
    "id" => "SHOW_COUNTER",
    "content" => GetMessage("IBLOCK_FIELD_SHOW_COUNTER"),
    "sort" => "show_counter",
    "align" => "right"
);
$arHeader[] = array(
    "id" => "SHOW_COUNTER_START",
    "content" => GetMessage("IBLOCK_FIELD_SHOW_COUNTER_START"),
    "sort" => "show_counter_start",
    "align" => "right"
);
$arHeader[] = array(
    "id" => "PREVIEW_PICTURE",
    "content" => GetMessage("IBLOCK_FIELD_PREVIEW_PICTURE"),
    "align" => "right"
);
$arHeader[] = array("id" => "PREVIEW_TEXT", "content" => GetMessage("IBLOCK_FIELD_PREVIEW_TEXT"));
$arHeader[] = array(
    "id" => "DETAIL_PICTURE",
    "content" => GetMessage("IBLOCK_FIELD_DETAIL_PICTURE"),
    "align" => "center"
);
...
while ($arRes = $rsData->GetNext()) {
    ...
    $row =& $lAdmin->AddRow(
        $arRes["ID"],
        $arRes,
        "javascript:SelEl('" . CUtil::JSEscape($index) . "', '" . htmlspecialcharsbx(
            CUtil::JSEscape($arRes["~NAME"]),
            ENT_QUOTES
        ) . "')",
    ...
    $row->AddViewField(
        "NAME",
        $arRes["NAME"] . '<input type="hidden" name="n' . $arRes["ID"] . '" id="index_' . $arRes["ID"] . '" value="' . $index . '"><div style="display:none" id="name_' . $arRes["ID"] . '">' . $arRes["NAME"] . '</div>'
    );
    if ($arRes["MODIFIED_BY"] > 0) {
        $row->AddViewField(
            "USER_NAME",
            '[<a target="_blank" href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arRes["MODIFIED_BY"] . '">' . $arRes["MODIFIED_BY"] . '</a>]&nbsp;' . $arRes["USER_NAME"]
        );
    } else {
    ...
    if ($arRes["CREATED_BY"] > 0) {
        $row->AddViewField(
            "CREATED_USER_NAME",
            '[<a target="_blank" href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arRes["CREATED_BY"] . '">' . $arRes["CREATED_BY"] . '</a>]&nbsp;' . $arRes["CREATED_USER_NAME"]
        );
    } else {
    ...
    $row->AddViewField(
        "WF_STATUS_ID",
        htmlspecialcharsbx(
            CIBlockElement::WF_GetStatusTitle($arRes["WF_STATUS_ID"])
        ) . '<input type="hidden" name="n' . $arRes["ID"] . '" value="' . $arRes["NAME"] . '">'
    );
    if ($arRes["WF_LOCKED_BY"] > 0) {
        $row->AddViewField(
            "LOCKED_USER_NAME",
            '&nbsp;<a href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arRes["WF_LOCKED_BY"] . '" title="' . GetMessage(
                "IBLOCK_ELSEARCH_USERINFO"
            ) . '">' . $arRes["LOCKED_USER_NAME"] . '</a>'
        );
    } else {
            ...
            if (isset($arUserType["GetAdminListViewHTML"])) {
                ...
                if ($t) {
                    $res = $t['NAME'] . ' [<a href="' . htmlspecialcharsbx(
                        ...
                        ) . '" title="' . GetMessage("IBLOCK_ELSEARCH_SECTION_EDIT") . '">' . $t['ID'] . '</a>]';
                }
                ...
                if ($t) {
                    $res = $t['NAME'] . ' [<a href="' . htmlspecialcharsbx(
                        ...
                        ) . '" title="' . GetMessage("IBLOCK_ELSEARCH_ELEMENT_EDIT") . '">' . $t['ID'] . '</a>]';
                }
    ...
    $row->AddActions(
        array(
            array(
                ...
                "ACTION" => "javascript:SelEl('" . CUtil::JSEscape($index) . "', '" . htmlspecialcharsbx(
                        htmlspecialcharsbx(CUtil::JSEscape($arRes["~NAME"]), ENT_QUOTES)
                    ) . "')",
            ),
        )
    );
...
?>
    <form name="form1" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>">
        ...
        function _ShowGroupPropertyField($name, $property_fields, $values)
        {
            ...
            echo '<option value=""' . (!$bWas ? ' selected' : '') . '>' . GetMessage(
                    "IBLOCK_ELSEARCH_NOT_SET"
                ) . '</option>';
            echo $res;
            echo '</select>';
        }
        ...
        <script type="text/javascript">
            ...
            function applyFilter(el) {
                if (blockedFilter)
                    return false;
                <?=$sTableID . "_filter";?>.
                OnSet('<?=CUtil::JSEscape($sTableID); ?>', '<?=CUtil::JSEscape($reloadUrl); ?>');
                return false;
            }
            ...
            function deleteFilter(el) {
                ...
                <?=$sTableID . "_filter"?>.
                OnClear('<?=CUtil::JSEscape($sTableID); ?>', '<?=CUtil::JSEscape($reloadUrl)?>');
                return false;
            }
        ...
        <tr>
            ...
            <td>
                <input type="text" name="filter_id_start" size="10"
                       value="<? echo htmlspecialcharsbx($filter_id_start) ?>">
                ...
        ...
        <tr>
            ...
            <td><? echo CalendarPeriod(
                    "filter_timestamp_from",
                    htmlspecialcharsbx($filter_timestamp_from),
                    "filter_timestamp_to",
                    htmlspecialcharsbx($filter_timestamp_to),
                    "form1"
                ) ?></td>
        </tr>
        ...
        <? if (CModule::IncludeModule("workflow")): ?>
            <tr>
                ...
                <td><input type="text" name="filter_status_id" value="<? echo htmlspecialcharsbx($filter_status_id) ?>"
                           size="3">
            ...
            <tr>
                ...
                <td>
                    <select name="filter_section">
                        ...
                        <option value="0"<? if ($filter_section == "0") echo " selected" ?>><? echo GetMessage(
                                "IBLOCK_UPPER_LEVEL"
                            ) ?></option>
                        <?
                        $bsections = CIBlockSection::GetTreeList(
                            array("IBLOCK_ID" => $IBLOCK_ID),
                            array("ID", "NAME", "DEPTH_LEVEL")
                        );
                        while ($arSection = $bsections->GetNext()):
                            ?>
                            <option value="<? echo $arSection["ID"] ?>"<? if ($arSection["ID"] == $filter_section) echo " selected" ?>><? echo str_repeat(
                            "&nbsp;.&nbsp;",
                    ...
                    <input type="checkbox" name="filter_subsections"
                           value="Y"<? if ($filter_subsections == "Y") echo " checked" ?>> <? echo GetMessage(
                        "IBLOCK_ELSEARCH_INCLUDING_SUBSECTIONS"
                    ) ?>
        ...
        <tr>
            ...
            <td>
                <select name="filter_active">
                    ...
                    <option value="Y"<? if ($filter_active == "Y") echo " selected" ?>><?= htmlspecialcharsbx(
                            GetMessage("IBLOCK_YES")
                        ) ?></option>
                    <option value="N"<? if ($filter_active == "N") echo " selected" ?>><?= htmlspecialcharsbx(
                            GetMessage("IBLOCK_NO")
                        ) ?></option>
                </select>
            </td>
        </tr>
        <tr>
            ...
            <td><input type="text" name="filter_external_id" value="<? echo htmlspecialcharsbx($filter_external_id) ?>"
                       size="30"></td>
        </tr>
        <tr>
                ...
                <td>
                    <? if (array_key_exists("GetAdminFilterHTML", $prop["PROPERTY_USER_TYPE"])):
                        echo call_user_func_array(
                            $prop["PROPERTY_USER_TYPE"]["GetAdminFilterHTML"],
                            array(
                                $prop,
                                array(
                                    "VALUE" => "find_el_property_" . $prop["ID"],
                    ...
                    elseif ($prop["PROPERTY_TYPE"] == 'G'):
                        _ShowGroupPropertyField(
                            'find_el_property_' . $prop["ID"],
                            $prop,
                            ${'find_el_property_' . $prop["ID"]}
                        );
                    else:
                        ?>
                        <input type="text" name="find_el_property_<?= $prop["ID"] ?>"
                               value="<? echo htmlspecialcharsbx(${"find_el_property_" . $prop["ID"]}) ?>"
        ...
        <span class="adm-btn-wrap"><input type="submit" class="adm-btn" name="set_filter"
                                          ...
                                          title="<? echo GetMessage("admin_lib_filter_set_butt_title"); ?>"
                                          onclick="return applyFilter(this);"></span>
        <span class="adm-btn-wrap"><input type="submit" class="adm-btn" name="del_filter"
                                          ...
                                          onclick="return deleteFilter(this);"></span>
        <? $oFilter->End(); ?>
    </form>
<?
$lAdmin->DisplayList();
