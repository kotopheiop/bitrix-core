...
/* autocomplete */
$strLookup = (isset($_REQUEST['lookup']) && is_string($_REQUEST['lookup']) ? preg_replace(
    "/[^a-zA-Z0-9_:]/",
    "",
    $_REQUEST['lookup']
) : '');
...
if ($bCatalog) {
    ...
    if (!empty($arMainCatalog)) {
        ...
        $arCatalogTabs = CCatalogAdminTools::getShowTabs(
            $IBLOCK_ID,
            ($copyID > 0 && $ID == 0 ? $copyID : $ID),
            $arMainCatalog
        );
...
if ($bAutocomplete) {
    ...
    if ($return_url != '' && mb_strtolower(
            mb_substr($return_url, mb_strlen($APPLICATION->GetCurPage()))
        ) == mb_strtolower($APPLICATION->GetCurPage())) {
        $return_url = '';
    }
    if ($return_url == '') {
        if ($from == "iblock_section_admin") {
            $return_url = CIBlock::GetAdminSectionListLink(
                $IBLOCK_ID,
                array(
...
do { //one iteration loop
    ...
    if ($arIBlock) {
        if (($ID > 0 && !$bCopy) && !CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ID, "iblock_admin_display")) {
            $bBadBlock = true;
        } elseif (($ID <= 0 || $bCopy) && !CIBlockSectionRights::UserHasRightTo(
                $IBLOCK_ID,
                $MENU_SECTION_ID,
            ...
            $bBadBlock = false;
        } elseif (($ID > 0 && !$bCopy) && CIBlockElementRights::UserHasRightTo(
                $IBLOCK_ID,
                $ID,
                "iblock_admin_display"
            )) {
        ...
        if ($ID > 0 && $WF == "Y") {
            ...
            if (!$errorTriger) {
                ...
                if ($isLocked) {
                    ...
                    $error = new _CIBlockError(
                        2,
                        "BLOCKED",
                        GetMessage(
                            "IBLOCK_DOCUMENT_LOCKED",
                            array("#ID#" => $locked_by, "#DATE#" => $date_lock)
                        )
                    );
                    $errorTriger = true;
                }
    ...
    $customTabber = new CAdminTabEngine(
        "OnAdminIBlockElementEdit",
        array("ID" => $ID, "IBLOCK" => $arIBlock, "IBLOCK_TYPE" => $arIBTYPE)
    );
    ...
    if (!$errorTriger) {
            ...
            foreach ($PROP as $k1 => $val1) {
                if (is_array($val1)) {
                    foreach ($val1 as $k2 => $val2) {
                        $text_name = preg_replace(
                            "/([^a-z0-9])/is",
                            "_",
                            "PROP[" . $k1 . "][" . $k2 . "][VALUE][TEXT]"
                        );
                        if (array_key_exists($text_name, $_POST)) {
                            $type_name = preg_replace(
                                "/([^a-z0-9])/is",
                                "_",
                                "PROP[" . $k1 . "][" . $k2 . "][VALUE][TYPE]"
                            );
        ...
        if (is_array($DESCRIPTION_PROP)) {
            foreach ($DESCRIPTION_PROP as $k1 => $val1) {
                foreach ($val1 as $k2 => $val2) {
                    if (is_set($PROP[$k1], $k2) && is_array($PROP[$k1][$k2]) && is_set(
                            $PROP[$k1][$k2],
                            "DESCRIPTION"
                        )) {
        ...
        if (
            ...
            if (!empty($ar)) {
                ...
                $adminSidePanelHelper->localRedirect(
                    $APPLICATION->GetCurPageParam("", array("stop_bizproc", "sessid"))
                );
        ...
        if (
            ...
            if (CCatalogAdminTools::changeTabs($IBLOCK_ID, $ID, $arMainCatalog)) {
                ...
                $adminSidePanelHelper->localRedirect(
                    $selfFolderUrl . CIBlock::GetAdminElementEditLink(
                        $IBLOCK_ID,
                        $ID,
                        $arUrlParams,
        ...
        if (
            ...
            if (!check_bitrix_sessid() && !$bCustomForm) {
                ...
                $strWarning .= GetMessage(
                        "IBLOCK_ACCESS_DENIED_STATUS"
                    ) . " [" . $_POST["WF_STATUS_ID"] . "]." . "<br>";
            } elseif (0 >= $ID && !isset($_POST["IBLOCK_SECTION"]) && !CIBlockRights::UserHasRightTo(
                    $IBLOCK_ID,
                    $IBLOCK_ID,
                    "section_element_bind"
                )) {
                ...
                if ($bCatalog) {
                    ...
                    if (
                        $arShowTabs['catalog']
                        && file_exists(
                            $_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/catalog/admin/templates/product_edit_validator.php"
                        )
                    ) {
                        // errors'll be appended to $strWarning;
                        $boolSKUExists = false;
                        if (CCatalogSku::TYPE_FULL == $arMainCatalog['CATALOG_TYPE']) {
                            $boolSKUExists = CCatalogSku::IsExistOffers(
                                ($ID > 0 && !$bCopy ? $ID : '-' . $str_TMP_ID),
                                $IBLOCK_ID
                            );
                        }
                    ...
                    if ($arShowTabs['product_set']) {
                        CCatalogAdminProductSetEdit::setProductFormParams(
                            array('TYPE' => CCatalogProductSet::TYPE_SET)
                        );
                    ...
                    if ($arShowTabs['product_group']) {
                        CCatalogAdminProductSetEdit::setProductFormParams(
                            array('TYPE' => CCatalogProductSet::TYPE_GROUP)
                        );
                        if (!CCatalogAdminProductSetEdit::checkFormValues($arCatalogItem)) {
                            $strWarning .= implode('<br>', CCatalogAdminProductSetEdit::getErrors());
                        }
                ...
                if ($strWarning == '') {
                    ...
                    $arPREVIEW_PICTURE = CIBlock::makeFileArray(
                        array_key_exists(
                            "PREVIEW_PICTURE",
                            $_FILES
                        ) ? $_FILES["PREVIEW_PICTURE"] : $_REQUEST["PREVIEW_PICTURE"],
                    ...
                    $arDETAIL_PICTURE = CIBlock::makeFileArray(
                        array_key_exists(
                            "DETAIL_PICTURE",
                            $_FILES
                        ) ? $_FILES["DETAIL_PICTURE"] : $_REQUEST["DETAIL_PICTURE"],
            ...
            if ($strWarning != '') {
                ...
                if ($apply == '' && $save_and_add == '') {
                    if ($bAutocomplete) {
                        ...
                        $saveUrl = $selfFolderUrl . CIBlock::GetAdminElementListLink(
                                $IBLOCK_ID,
                                array('find_section_section' => $find_section_section)
                            );
                    ...
                    if (defined('BX_PUBLIC_MODE') && BX_PUBLIC_MODE == 1) {
                        ...
                        $saveAndAddUrl = $selfFolderUrl . CIBlock::GetAdminElementEditLink(
                                $IBLOCK_ID,
                                0,
                                $params,
                                "&" .
                    ...
                    $applyUrl = $selfFolderUrl . CIBlock::GetAdminElementEditLink(
                            $IBLOCK_ID,
                            $ID,
                            array(
                                "WF" => ($WF == "Y" ? "Y" : null),
        ...
        if (!empty($dontsave) && check_bitrix_sessid()) {
            ...
            if ($return_url <> '') {
                ...
                if ($bAutocomplete) {
                    ...
                    $donSaveUrl = $selfFolderUrl . CIBlock::GetAdminElementListLink(
                            $IBLOCK_ID,
                            array('find_section_section' => $find_section_section)
                        );
...
if ($error && $error->err_level == 1) {
    ...
    if (!$arIBlock["ELEMENT_NAME"]) {
        $arIBlock["ELEMENT_NAME"] = $arIBTYPE["ELEMENT_NAME"] ? $arIBTYPE["ELEMENT_NAME"] : GetMessage(
            "IBEL_E_IBLOCK_ELEMENT"
        );
    }
    if (!$arIBlock["SECTIONS_NAME"]) {
        $arIBlock["SECTIONS_NAME"] = $arIBTYPE["SECTION_NAME"] ? $arIBTYPE["SECTION_NAME"] : GetMessage(
            "IBEL_E_IBLOCK_SECTIONS"
        );
    }
    ...
    if ($ID > 0 && !$bCopy) {
        if ($view == "Y" || ($bBizproc && !$canWrite)) {
            $APPLICATION->SetTitle(
                $arIBlock["NAME"] . ": " . $arIBlock["ELEMENT_NAME"] . ": " . $arElement["NAME"] . " - " . GetMessage(
                    "IBLOCK_ELEMENT_EDIT_VIEW"
                )
            );
        } else {
            $APPLICATION->SetTitle(
        ...
        $APPLICATION->SetTitle(
            $arIBlock["NAME"] . ": " . $arIBlock["ELEMENT_NAME"] . ": " . GetMessage("IBLOCK_NEW_TITLE")
        );
    }
    ...
    if (!defined("CATALOG_PRODUCT")) {
        ...
        if ($find_section_section > 0) {
            ...
            while ($ar_nav = $nav->GetNext()) {
                $sSectionUrl = CIBlock::GetAdminSectionListLink(
                    $IBLOCK_ID,
                    array('find_section_section' => $ar_nav["ID"])
                );
                $adminChain->AddItem(
                    array(
                        "TEXT" => $ar_nav["NAME"],
    ...
    if ($bCustomForm) {
        $useCustomFormPropertyId = ((string)Main\Config\Option::get(
                'iblock',
        ...
        if (
            ...
            if (isset($db_prop_values[$prop_fields["ID"]])) {
                foreach ($db_prop_values[$prop_fields["ID"]] as $res) {
                    ...
                    $prop_values_with_descr[$res["PROPERTY_VALUE_ID"]] = array(
                        "VALUE" => $res["VALUE"],
            ...
            if (empty($clearedByCopyProperties) || !in_array($prop_fields["ID"], $clearedByCopyProperties)) {
                if ($db_prop_values === false) {
                    $db_prop_values = array();
                    $rs_prop_values = CIBlockElement::GetProperty(
                        $IBLOCK_ID,
                        $WF_ID,
                        "id",
                        "asc",
                ...
                if (isset($db_prop_values[$prop_fields["ID"]])) {
                    foreach ($db_prop_values[$prop_fields["ID"]] as $res) {
                        if ($res["WITH_DESCRIPTION"] == "Y") {
                            $prop_values[$res["PROPERTY_VALUE_ID"]] = array(
                                "VALUE" => $res["VALUE"],
                                "DESCRIPTION" => $res["DESCRIPTION"]
                            );
                        } else {
    ...
    if (!$bAutocomplete && !$bPropertyAjax) {
        ...
        if (!$bCopy && $ID > 0) {
            $copyUrl = $selfFolderUrl . CIBlock::GetAdminElementEditLink(
                    $IBLOCK_ID,
                    $ID,
                    array(
        ...
        if ($ID > 0 && !$bCopy) {
            ...
            if (CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ID, "element_delete")) {
                ...
                $arSubMenu[] = array(
                    ...
                    $buttonAction => "javascript:if(confirm('" . GetMessageJS(
                            "IBLOCK_ELEMENT_DEL_CONF"
                        ) . "')) top.window.location.href='" . CUtil::JSEscape($urlDelete) . "';",
                    'ICON' => 'delete',
                );
            }
            ...
            if ($bWorkflow && !defined("CATALOG_PRODUCT")) {
                $arSubMenu[] = array(
                    ...
                    "LINK" => $selfFolderUrl . '/iblock_history_list.php?ELEMENT_ID=' . $ID . '&type=' . urlencode(
                            $arIBlock["IBLOCK_TYPE_ID"]
                        ) . '&lang=' . LANGUAGE_ID . '&IBLOCK_ID=' . $IBLOCK_ID . '&find_section_section=' . $find_section_section,
                );
            }
    ...
    if ($customFormFile):
        ...
        <script type="text/javascript">
            var InheritedPropertiesTemplates = new JCInheritedPropertiesTemplates(
                ...
                '<?=$selfFolderUrl?>iblock_templates.ajax.php?ENTITY_TYPE=E&IBLOCK_ID=<?echo intval(
                    $IBLOCK_ID
                )?>&ENTITY_ID=<?echo intval($ID)?>&bxpublic=y'
            );
            BX.ready(function () {
                setTimeout(function () {
                ...
                if (!empty($importProduct)) {
                    ...
                    <tr>
                        ...
                        <td width="60%">
                            ...
                            <?= $accountName ?>
                            <?= ($linkToProduct ? "<a href=\"{$linkToProduct}\" target=\"_blank\">" . GetMessage(
                                    'IBLOCK_LINK_TO_MEDIA'
                                ) . "</a>" : '') ?>
                        </td>
                    </tr>
                    <?
                    ...
                    <td width="60%"><? echo $str_DATE_CREATE ?><?
                        if (intval($str_CREATED_BY) > 0):
                            if (!$adminSidePanelHelper->isPublicSidePanel()):
                                ?>&nbsp;&nbsp;&nbsp;[<a
                                    href="user_edit.php?lang=<?= LANGUAGE_ID; ?>&amp;ID=<?= $str_CREATED_BY; ?>"><? echo $str_CREATED_BY ?></a>]<?
                            endif;
        ...
        if ($ID > 0 && !$bCopy) {
            ...
            <td width="60%"><? echo $str_TIMESTAMP_X; ?><?
                if (intval($str_MODIFIED_BY) > 0):
                    if (!$adminSidePanelHelper->isPublicSidePanel()):
                        ?>&nbsp;&nbsp;&nbsp;[<a
                            href="user_edit.php?lang=<?= LANGUAGE_ID; ?>&amp;ID=<?= $str_MODIFIED_BY; ?>"><? echo $str_MODIFIED_BY ?></a>]<?
                    endif;
        ...
        $tabControl->AddCheckBoxField(
            "ACTIVE",
            GetMessage("IBLOCK_FIELD_ACTIVE") . ":",
            false,
            array("Y", "N"),
            $str_ACTIVE == "Y"
        );
        $tabControl->BeginCustomField(
            "ACTIVE_FROM",
            GetMessage("IBLOCK_FIELD_ACTIVE_PERIOD_FROM"),
        ...
        if ($arTranslit["TRANSLITERATION"] == "Y") {
            ...
            <tr id="tr_NAME">
                ...
                <td style="white-space: nowrap;">
                    <input type="text" size="70" name="NAME" id="NAME" maxlength="255" value="<? echo $str_NAME ?>"><img
                            id="name_link" title="<? echo GetMessage("IBEL_E_LINK_TIP") ?>" class="linked"
                            src="/bitrix/themes/.default/icons/iblock/<? if ($bLinked) {
                                echo 'link.gif';
                            } else {
                                echo 'unlink.gif';
                            } ?>" onclick="set_linked()"/>
            ...
            $tabControl->BeginCustomField(
                ...
                $arIBlock["FIELDS"]["CODE"]["IS_REQUIRED"] === "Y"
            );
            ?>
            <tr id="tr_CODE">
                <td><? echo $tabControl->GetCustomLabelHTML() ?></td>
                <td style="white-space: nowrap;">
                    <input type="text" size="70" name="CODE" id="CODE" maxlength="255" value="<? echo $str_CODE ?>"><img
                            id="code_link" title="<? echo GetMessage("IBEL_E_LINK_TIP") ?>" class="linked"
                            src="/bitrix/themes/.default/icons/iblock/<? if ($bLinked) {
                                echo 'link.gif';
                            } else {
                                echo 'unlink.gif';
        ...
        if (
            ...
            $tabControl->BeginCustomField(
                "IBLOCK_ELEMENT_SECTION_ID",
                GetMessage("IBEL_E_MAIN_IBLOCK_SECTION_ID") . ":",
                false
            );
            ?>
        ...
        if (COption::GetOptionString("iblock", "show_xml_id", "N") == "Y") {
            if ($bCopy || $ID == 0) {
                $tabControl->BeginCustomField(
                    ...
                    $arIBlock["FIELDS"]["XML_ID"]["IS_REQUIRED"] === "Y"
                );
                ?>
                <tr id="tr_XML_ID">
                <td><span id="hint_XML_ID"></span>
                    <script type="text/javascript">
                        BX.hint_replace(BX('hint_XML_ID'), '<?=CUtil::JSEscape(
                            htmlspecialcharsbx(GetMessage('IBLOCK_FIELD_HINT_XML_ID'))
                        )?>');
                ...
                $tabControl->EndCustomField(
                    "XML_ID",
                    '<input type="hidden" name="XML_ID" id="XML_ID" value="' . $str_XML_ID . '">'
                );
            } else {
                $tabControl->AddEditField(
                    "XML_ID",
                    GetMessage("IBLOCK_FIELD_XML_ID") . ":",
                    $arIBlock["FIELDS"]["XML_ID"]["IS_REQUIRED"] === "Y",
                    array("size" => 70, "maxlength" => 255, "id" => "XML_ID"),
                    $str_XML_ID
                );
            }
        }
        ...
        $tabControl->AddEditField(
            "SORT",
            GetMessage("IBLOCK_FIELD_SORT") . ":",
            $arIBlock["FIELDS"]["SORT"]["IS_REQUIRED"] === "Y",
        ...
        if (!empty($PROP)):
            if ($arIBlock["SECTION_PROPERTY"] === "Y" || defined("CATALOG_PRODUCT")) {
                ...
                $tabControl->AddFieldGroup(
                    ...
                    GetMessage("IBLOCK_ELEMENT_PROP_VALUE"),
                    $arPropLinks,
                    $bPropertyAjax
                );
            }
            ...
            foreach ($PROP as $prop_code => $prop_fields):
                ...
                <tr id="tr_PROPERTY_<? echo $prop_fields["ID"]; ?>"<? if ($prop_fields["PROPERTY_TYPE"] == "F"):?> class="adm-detail-file-row"<?endif ?>>
                    <td class="adm-detail-valign-top" width="40%"><? if ($prop_fields["HINT"] != ""):
                            ...
                            <script type="text/javascript">BX.hint_replace(BX('hint_<?echo $prop_fields["ID"];?>'), '<?echo CUtil::JSEscape(
                                    htmlspecialcharsbx($prop_fields["HINT"])
                                )?>');</script>&nbsp;<?
                        endif; ?><? echo $tabControl->GetCustomLabelHTML(); ?>:
                    </td>
                    <td width="60%"><? _ShowPropertyField(
                            'PROP[' . $prop_fields["ID"] . ']',
                            $prop_fields,
                            $prop_fields["VALUE"],
                            (($historyId <= 0) && (!$bVarsFromForm) && ($ID <= 0) && (!$bPropertyAjax)),
                            $bVarsFromForm || $bPropertyAjax,
                            50000,
                ...
                foreach ($values as $key => $val) {
                    ...
                    if (is_array($val) && array_key_exists("VALUE", $val)) {
                        $hidden .= _ShowHiddenValue(
                            'PROP[' . $prop_fields["ID"] . '][' . $key . '][VALUE]',
                            $val["VALUE"]
                        );
                        $hidden .= _ShowHiddenValue(
                            'PROP[' . $prop_fields["ID"] . '][' . $key . '][DESCRIPTION]',
                            $val["DESCRIPTION"]
                        );
                    } else {
        ...
        if (!$bAutocomplete && ($ID > 0 && !$bCopy)) {
            $rsLinkedProps = CIBlockProperty::GetList(
                array(),
                array(
                    "PROPERTY_TYPE" => "E",
            ...
            if ($arLinkedProp && !$adminSidePanelHelper->isPublicSidePanel()) {
                ...
                do {
                    ...
                    <?
                    $href = $selfFolderUrl . htmlspecialcharsbx(
                            CIBlock::GetAdminElementListLink(
                                $arLinkedProp["IBLOCK_ID"],
                                array(
                                    'apply_filter' => 'Y',
                                    'PROPERTY_' . $arLinkedProp["ID"] => $ID,
                    ...
                    <td colspan="2">
                        <a title="<?= $linkedTitle; ?>" href="<?= $href ?>">
                            <?= htmlspecialcharsbx(
                                CIBlock::GetArrayByID($arLinkedProp["IBLOCK_ID"], "NAME") . ": " . $elements_name
                            ); ?>
                        </a>
                    </td>
                    </tr><?
        ...
        $tabControl->BeginCustomField(
            "PREVIEW_PICTURE",
            GetMessage("IBLOCK_FIELD_PREVIEW_PICTURE"),
            $arIBlock["FIELDS"]["PREVIEW_PICTURE"]["IS_REQUIRED"] === "Y"
        );
        ...
        $tabControl->BeginCustomField(
            "PREVIEW_TEXT",
            GetMessage("IBLOCK_FIELD_PREVIEW_TEXT"),
            $arIBlock["FIELDS"]["PREVIEW_TEXT"]["IS_REQUIRED"] === "Y"
        );
        ?>
                ...
                <? CFileMan::AddHTMLEditorFrame(
                    ...
                    array(
                        'toolbarConfig' => CFileMan::GetEditorToolbarConfig(
                            "iblock_" . (defined('BX_PUBLIC_MODE') && BX_PUBLIC_MODE == 1 ? 'public' : 'admin')
                        ),
        ...
        <tr id="tr_PREVIEW_TEXT_TYPE">
            ...
            <td>
                <? if ($arIBlock["FIELDS"]["PREVIEW_TEXT_TYPE_ALLOW_CHANGE"]["DEFAULT_VALUE"] === "N"):?>
                    <input type="hidden" name="PREVIEW_TEXT_TYPE"
                           value="<? echo $str_PREVIEW_TEXT_TYPE ?>"><? echo $str_PREVIEW_TEXT_TYPE != "html" ? GetMessage(
                        "IBLOCK_DESC_TYPE_TEXT"
                    ) : GetMessage("IBLOCK_DESC_TYPE_HTML") ?>
                <? else:?>
                    <input type="radio" name="PREVIEW_TEXT_TYPE" id="PREVIEW_TEXT_TYPE_text"
                           value="text"<? if ($str_PREVIEW_TEXT_TYPE != "html") echo " checked" ?>> <label
                            for="PREVIEW_TEXT_TYPE_text"><? echo GetMessage("IBLOCK_DESC_TYPE_TEXT") ?></label> / <input
                            type="radio" name="PREVIEW_TEXT_TYPE" id="PREVIEW_TEXT_TYPE_html"
                            value="html"<? if ($str_PREVIEW_TEXT_TYPE == "html") echo " checked" ?>> <label
                            for="PREVIEW_TEXT_TYPE_html"><? echo GetMessage("IBLOCK_DESC_TYPE_HTML") ?></label>
                <?endif ?>
            </td>
        </tr>
        <tr id="tr_PREVIEW_TEXT">
            <td colspan="2" align="center">
                <textarea cols="60" rows="10" name="PREVIEW_TEXT"
                          style="width:100%"><? echo $str_PREVIEW_TEXT ?></textarea>
            </td>
        </tr>
    <?endif;
        $tabControl->EndCustomField(
        ...
        $tabControl->BeginCustomField(
            "DETAIL_PICTURE",
            GetMessage("IBLOCK_FIELD_DETAIL_PICTURE"),
            $arIBlock["FIELDS"]["DETAIL_PICTURE"]["IS_REQUIRED"] === "Y"
        );
        ...
        $tabControl->BeginCustomField(
            "DETAIL_TEXT",
            GetMessage("IBLOCK_FIELD_DETAIL_TEXT"),
            $arIBlock["FIELDS"]["DETAIL_TEXT"]["IS_REQUIRED"] === "Y"
        );
        ?>
        ...
        <tr id="tr_DETAIL_TEXT_TYPE">
            ...
            <td>
                <? if ($arIBlock["FIELDS"]["DETAIL_TEXT_TYPE_ALLOW_CHANGE"]["DEFAULT_VALUE"] === "N"):?>
                    <input type="hidden" name="DETAIL_TEXT_TYPE"
                           value="<? echo $str_DETAIL_TEXT_TYPE ?>"><? echo $str_DETAIL_TEXT_TYPE != "html" ? GetMessage(
                        "IBLOCK_DESC_TYPE_TEXT"
                    ) : GetMessage("IBLOCK_DESC_TYPE_HTML") ?>
                <? else:?>
                    <input type="radio" name="DETAIL_TEXT_TYPE" id="DETAIL_TEXT_TYPE_text"
                           value="text"<? if ($str_DETAIL_TEXT_TYPE != "html") echo " checked" ?>> <label
                            for="DETAIL_TEXT_TYPE_text"><? echo GetMessage("IBLOCK_DESC_TYPE_TEXT") ?></label> / <input
                            type="radio" name="DETAIL_TEXT_TYPE" id="DETAIL_TEXT_TYPE_html"
                            value="html"<? if ($str_DETAIL_TEXT_TYPE == "html") echo " checked" ?>> <label
                            for="DETAIL_TEXT_TYPE_html"><? echo GetMessage("IBLOCK_DESC_TYPE_HTML") ?></label>
                <?endif ?>
            </td>
        </tr>
        <tr id="tr_DETAIL_TEXT">
            <td colspan="2" align="center">
                <textarea cols="60" rows="20" name="DETAIL_TEXT"
                          style="width:100%"><? echo $str_DETAIL_TEXT ?></textarea>
            </td>
        </tr>
    <?endif ?>
        <?
        ...
        <tr class="adm-detail-valign-top">
            ...
            <td width="60%"><? echo IBlockInheritedPropertyInput(
                    ...
                    "E",
                    GetMessage("IBEL_E_SEO_OVERWRITE")
                ) ?></td>
        </tr>
        <?
        $tabControl->EndCustomField(
        ...
        $tabControl->BeginCustomField(
            ...
            GetMessage("IBEL_E_SEO_META_KEYWORDS")
        );
        ?>
        <tr class="adm-detail-valign-top">
            <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        ...
        $tabControl->BeginCustomField(
            ...
            GetMessage("IBEL_E_SEO_META_DESCRIPTION")
        );
        ?>
        <tr class="adm-detail-valign-top">
            <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        ...
        $tabControl->AddSection(
            ...
            GetMessage("IBEL_E_SEO_FOR_ELEMENTS_PREVIEW_PICTURE")
        );
        $tabControl->BeginCustomField(
            "IPROPERTY_TEMPLATES_ELEMENT_PREVIEW_PICTURE_FILE_ALT",
            GetMessage("IBEL_E_SEO_FILE_ALT")
        );
        ?>
        <tr class="adm-detail-valign-top">
            <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        ...
        $tabControl->BeginCustomField(
            ...
            GetMessage("IBEL_E_SEO_FILE_TITLE")
        );
        ?>
        <tr class="adm-detail-valign-top">
            <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        ...
        $tabControl->AddSection(
            ...
            GetMessage("IBEL_E_SEO_FOR_ELEMENTS_DETAIL_PICTURE")
        );
        $tabControl->BeginCustomField(
        ...
        $tabControl->BeginCustomField(
            "TAGS",
            GetMessage("IBLOCK_FIELD_TAGS") . ":",
            $arIBlock["FIELDS"]["TAGS"]["IS_REQUIRED"] === "Y"
        );
        ?>
        <tr id="tr_TAGS">
            <td><? echo $tabControl->GetCustomLabelHTML() ?><br><? echo GetMessage("IBLOCK_ELEMENT_EDIT_TAGS_TIP") ?>
            </td>
            <td>
        ...
        $tabControl->BeginCustomField(
            "SECTIONS",
            GetMessage("IBLOCK_SECTION"),
            $arIBlock["FIELDS"]["IBLOCK_SECTION"]["IS_REQUIRED"] === "Y"
        );
        ?>
        <tr id="tr_SECTIONS">
            <? if ($arIBlock["SECTION_CHOOSER"] != "D" && $arIBlock["SECTION_CHOOSER"] != "P"):?>
                ...
                <td width="60%">
                    <select name="IBLOCK_SECTION[]" size="14" multiple onchange="onSectionChanged()">
                        <option value="0"<? if (is_array($str_IBLOCK_ELEMENT_SECTION) && in_array(
                                0,
                                $str_IBLOCK_ELEMENT_SECTION
                            )) echo " selected" ?>><? echo GetMessage("IBLOCK_UPPER_LEVEL") ?></option>
                        <?
                        while ($ar_l = $l->GetNext()):
                            ?>
                            <option value="<? echo $ar_l["ID"] ?>"<? if (is_array(
                                $str_IBLOCK_ELEMENT_SECTION
                            ) && in_array(
                                $ar_l["ID"],
                                $str_IBLOCK_ELEMENT_SECTION
                            )) echo " selected" ?>><? echo str_repeat(
                            " . ",
                            $ar_l["DEPTH_LEVEL"]
                        ...
                        if (is_array($str_IBLOCK_ELEMENT_SECTION)) {
                            ...
                            foreach ($str_IBLOCK_ELEMENT_SECTION as $section_id) {
                                ...
                                if ($strPath <> '') {
                                    ...
                                    <td>
                                        <input type="button" value="<? echo GetMessage("MAIN_DELETE") ?>"
                                               OnClick="deleteRow(this)">
                                        <input type="hidden" name="IBLOCK_SECTION[]"
                                               value="<? echo intval($section_id) ?>">
                                    </td>
                                    </tr><?
                                }
                                $i++;
                        ...
                        <tr>
                            <td>
                                <script type="text/javascript">
                                    ...
                                    function addPathRow() {
                                        ...
                                        if (table) {
                                            ...
                                            if (section_id > 0) {
                                                ...
                                                var oCell = oRow.insertCell(i++);
                                                oCell.innerHTML =
                                                    '<input type="button" value="<?echo GetMessage(
                                                        "MAIN_DELETE"
                                                    )?>" OnClick="deleteRow(this)">' +
                                    ...
                                    function change_selection(name_prefix, prop_id, value, level, id) {
                                        ...
                                        while (oSelect = document.getElementById(name_prefix + lev)) {
                                            ...
                                            jsSelectUtils.addNewOption(oSelect, '0', '(<?echo GetMessage(
                                                "MAIN_NO"
                                            )?>)');
                                            lev++;
                                        }
                                    ...
                                    <?
                                    $rsItems = CIBlockSection::GetTreeList(
                                        Array("IBLOCK_ID" => $IBLOCK_ID),
                                        array("ID", "NAME", "DEPTH_LEVEL")
                                    );
                                    $depth = 0;
                                    ...
                                    while ($arItem = $rsItems->GetNext()) {
                                        ...
                                        echo " = { id : " . $arItem["ID"] . ", name : '" . CUtil::JSEscape(
                                                $arItem["NAME"]
                                            ) . "', children : Array() };\n";
                                ...
                                for ($i = 0; $i < $max_depth; $i++) {
                                    echo '<select id="select_IBLOCK_SECTION_' . $i . '" onchange="change_selection(\'select_IBLOCK_SECTION_\',  0, this.value, ' . $i . ', \'IBLOCK_SECTION[n' . $key . ']\')"><option value="0">(' . GetMessage(
                                            "MAIN_NO"
                                        ) . ')</option></select>&nbsp;';
                                }
                                ?>
                            ...
                            <td><input type="button" value="<? echo GetMessage("IBLOCK_ELEMENT_EDIT_PROP_ADD") ?>"
                                       onClick="addPathRow()"></td>
                        </tr>
                    </table>
                </td>
            ...
            <? else:?>
                ...
                <td width="60%">
                    ...
                    <input type="button" value="<? echo GetMessage("IBLOCK_ELEMENT_EDIT_PROP_ADD") ?>..."
                           onClick="jsUtils.OpenWindow('<?= $selfFolderUrl ?>iblock_section_search.php?lang=<? echo LANGUAGE_ID ?>&amp;IBLOCK_ID=<? echo $IBLOCK_ID ?>&amp;n=input_IBLOCK_SECTION&amp;m=y&amp;iblockfix=y&amp;tableId=iblocksection-<?= $IBLOCK_ID; ?>', 900, 700);">
                </td>
            <?endif; ?>
        </tr>
            ...
            function onSectionChanged() {
                ...
                if (selectedTab && selectedTab.value) {
                    url += '&<?echo CUtil::JSEscape(
                        "form_element_" . $IBLOCK_ID . "_active_tab"
                    )?>=' + selectedTab.value;
                }
                <?if($arIBlock["SECTION_PROPERTY"] === "Y" || defined("CATALOG_PRODUCT")):?>
        ...
        if ($arShowTabs['sku']) {
            ...
            <td colspan="2"><?
                ...
                $boolIncludeOffers = CIBlockRights::UserHasRightTo(
                    $intSubIBlockID,
                    $intSubIBlockID,
                    "iblock_admin_display"
                );;
                ...
                $strSubElementAjaxPath = $selfFolderUrl . 'iblock_subelement_admin.php?WF=Y&IBLOCK_ID=' . $intSubIBlockID . '&type=' . urlencode(
                        $strSubIBlockType
                    ) . '&lang=' . LANGUAGE_ID . '&find_section_section=0&find_el_property_' . $arSubCatalog['SKU_PROPERTY_ID'] . '=' . ((0 == $ID) || ($bCopy) ? '-' . $TMP_ID : $ID) . '&TMP_ID=' . urlencode(
                        $strSubTMP_ID
                    ) . $additionalParams;
                if ($boolIncludeOffers && file_exists(
                        $_SERVER["DOCUMENT_ROOT"] . '/bitrix/modules/iblock/admin/templates/iblock_subelement_list.php'
                    )) {
        ...
        if ($arShowTabs['workflow']):?>
                ...
                <tr id="tr_WF_CREATED">
                    ...
                    <td width="60%"><? echo $pr["DATE_CREATE"] ?><?
                        if (intval($pr["CREATED_BY"]) > 0):
                            ?>&nbsp;&nbsp;&nbsp;[<a
                                href="user_edit.php?lang=<?= LANGUAGE_ID ?>&amp;ID=<?= $pr["CREATED_BY"] ?>"><? echo $pr["CREATED_BY"] ?></a>]&nbsp;<?= htmlspecialcharsex(
                            $pr["CREATED_USER_NAME"]
                        ) ?><?
                        endif;
                        ?></td>
                </tr>
            ...
            <? if ($str_TIMESTAMP_X <> '' && !$bCopy):?>
                <tr id="tr_WF_MODIFIED">
                    ...
                    <td><? echo $str_TIMESTAMP_X ?><?
                        if (intval($str_MODIFIED_BY) > 0):
                            ?>&nbsp;&nbsp;&nbsp;[<a
                                href="user_edit.php?lang=<?= LANGUAGE_ID ?>&amp;ID=<?= $str_MODIFIED_BY ?>"><? echo $str_MODIFIED_BY ?></a>]&nbsp;<?= $str_USER_NAME ?><?
                        endif;
                        ?></td>
                </tr>
            <?endif ?>
                ...
                <tr id="tr_WF_LOCKED">
                    ...
                    <td><? echo $prn_WF_DATE_LOCK ?><?
                        if (intval($prn_WF_LOCKED_BY) > 0):
                            ?>&nbsp;&nbsp;&nbsp;[<a
                                href="user_edit.php?lang=<?= LANGUAGE_ID ?>&amp;ID=<?= $prn_WF_LOCKED_BY ?>"><? echo $prn_WF_LOCKED_BY ?></a>]&nbsp;<?= $prn_LOCKED_USER_NAME ?><?
                        endif;
                        ?></td>
                </tr>
            <?endif;
            ...
            $tabControl->EndCustomField(
                "WF_COMMENTS",
                '<input type="hidden" name="WF_COMMENTS" value="' . $str_WF_COMMENTS . '">'
            );
        endif;
        ...
        if ($arShowTabs['bizproc']):
            ...
            <tr id="tr_BIZPROC_WF_STATUS">
                ...
                <td style="width:60%;"><?= ($str_BP_PUBLISHED == "Y" ? GetMessage("MAIN_YES") : GetMessage(
                        "MAIN_NO"
                    )) ?></td>
            </tr>
            <?
            ...
            foreach ($arDocumentStates as $arDocumentState) {
                ...
                <? if ($arDocumentState["STATE_NAME"] <> ''):?>
                    <tr>
                        ...
                        <td width="60%"><? if ($arDocumentState["ID"] <> ''): ?><a
                                    href="<?= $selfFolderUrl ?>bizproc_log.php?ID=<?= $arDocumentState["ID"] ?>&back_url=<?= urlencode(
                                        $APPLICATION->GetCurPageParam("", array())
                                    ) ?>"><?endif;
                                ?><?= $arDocumentState["STATE_TITLE"] <> '' ? $arDocumentState["STATE_TITLE"] : $arDocumentState["STATE_NAME"] ?><? if ($arDocumentState["ID"] <> ''): ?></a><?endif;
                        ?></td>
                    </tr>
                <?endif; ?>
                <?
                        ...
                        <td width="60%">
                            ...
                            <select name="bizproc_event_<?= $bizProcIndex ?>">
                                ...
                                foreach ($arEvents as $e) {
                                    ...
                                    $e["NAME"]
                                ) ?>"<?= ($_REQUEST["bizproc_event_" . $bizProcIndex] == $e["NAME"]) ? " selected" : "" ?>><?= htmlspecialcharsbx(
                                    $e["TITLE"]
                                ) ?></option><?
                                }
                                ?>
                        ...
                        <tr>
                            ...
                            <td width="60%">
                                ...
                                foreach ($arTasks as $arTask) {
                                    ?><a
                                    href="<?= $selfFolderUrl ?>bizproc_task.php?id=<?= $arTask["ID"] ?>&back_url=<?= urlencode(
                                        $APPLICATION->GetCurPageParam("", array())
                                    ) ?>" title="<?= strip_tags($arTask["DESCRIPTION"]) ?>"><?= $arTask["NAME"] ?></a>
                                    <br/><?
                                }
                                ?>
                            </td>
            ...
            if ($ID > 0):
                ...
                if ($bStartWorkflowPermission):
                    ...
                    <tr>
                        <td colspan="2" align="center">
                            <a href="<?= $selfFolderUrl . MODULE_ID ?>_start_bizproc.php?document_id=<?= $ID ?>&document_type=<?= DOCUMENT_TYPE ?>&back_url=<?= urlencode(
                                $APPLICATION->GetCurPageParam("", array('bxpublic'))
                            ) ?>"><? echo GetMessage("IBEL_BIZPROC_START") ?></a>
                        </td>
                    </tr>
                <?
                endif;
        ...
        elseif (!defined('BX_PUBLIC_MODE') || BX_PUBLIC_MODE != 1):
            ...
            <input <? if ($bDisabled) {
                echo "disabled";
            } ?> type="submit" class="adm-btn-save" name="save" id="save"
                 value="<? echo GetMessage("IBLOCK_EL_SAVE") ?>">
            <? if (!$bAutocomplete) {
            ?><input <? if ($bDisabled) {
                echo "disabled";
            } ?> type="submit" class="button" name="apply" id="apply" value="<? echo GetMessage('IBLOCK_APPLY') ?>"><?
        }
            ?>
            <input <? if ($bDisabled) {
                echo "disabled";
            } ?> type="submit" class="button" name="dontsave" id="dontsave"
                 value="<? echo GetMessage("IBLOCK_EL_CANC") ?>">
            <? if (!$bAutocomplete) {
            ?><input <? if ($bDisabled) {
                echo "disabled";
            } ?> type="submit" class="adm-btn-add" name="save_and_add" id="save_and_add"
                 value="<? echo GetMessage("IBLOCK_EL_SAVE_AND_ADD") ?>"><?
        }
        ...
        if (
            ...
            GetMessage("IBEL_E_IBLOCK_MANAGE_HINT"),
                ' <a href="' . $selfFolderUrl . 'iblock_edit.php?type=' . htmlspecialcharsbx(
                    $type
                ) . '&amp;lang=' . LANGUAGE_ID . '&amp;ID=' . $IBLOCK_ID . '&amp;admin=Y&amp;return_url=' . urlencode(
                    $selfFolderUrl . CIBlock::GetAdminElementEditLink(
                        $IBLOCK_ID,
                        $ID,
                        array(
