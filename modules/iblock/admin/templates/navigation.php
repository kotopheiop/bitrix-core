    ...
    <div class="adm-navigation">
        <div class="adm-nav-pages-block">
            ...
            if ($this->NavPageNomer > 1) {
                ?>
                <a class="adm-nav-page adm-nav-page-prev" href="javascript:<?
                echo $this->table_id ?>.<?= $nav_func_name ?>('<?
                echo $sJSUrlPath . '?PAGEN_' . $this->NavNum . '=' . ($this->NavPageNomer - 1) . '&amp;SIZEN_' . $this->NavNum . '=' . $this->NavPageSize . $sQueryString; ?>');"><span
                            class="adm-subnav-page-prev-before"></span></a>
                <?
            ...
            while ($NavRecordGroup <= $this->NavPageCount) {
                ...
                {
                    ?>
                    <a href="javascript:<?= $this->table_id ?>.<?= $nav_func_name ?>('<?= $sJSUrlPath . '?PAGEN_' . $this->NavNum . '=' . $NavRecordGroup . '&amp;SIZEN_' . $this->NavNum . '=' . $this->NavPageSize . $sQueryString ?>');"
                       class="adm-nav-page"><?= $NavRecordGroup ?></a>
                    <?
                ...
                if ($NavRecordGroup == 2 && $this->nStartPage > 3) {
                    if ($this->nStartPage - $NavRecordGroup > 1) {
                        ...
                        ?>
                        <a href="javascript:<?= $this->table_id ?>.<?= $nav_func_name ?>('<?= $sJSUrlPath . '?PAGEN_' . $this->NavNum . '=' . $middlePage . '&amp;SIZEN_' . $this->NavNum . '=' . $this->NavPageSize . $sQueryString ?>');"
                           class="adm-nav-page-separator"><?= $middlePage ?></a>
                        <?
                    }
                    $NavRecordGroup = $this->nStartPage;
            ...
            if ($this->NavPageNomer < $this->NavPageCount) {
                ?>
                <a class="adm-nav-page adm-nav-page-next" href="javascript:<?
                echo $this->table_id ?>.<?= $nav_func_name ?>('<?
                echo $sJSUrlPath . '?PAGEN_' . $this->NavNum . '=' . ($this->NavPageNomer + 1) . '&amp;SIZEN_' . $this->NavNum . '=' . $this->NavPageSize . $sQueryString; ?>');"><span
                            class="adm-subnav-page-next-before"></span></a>
                <?
