...
if ($bBadBlock) {
    ...
    <a href="iblock_admin.php?lang=<? echo LANGUAGE_ID ?>&amp;type=<? echo htmlspecialcharsbx(
        $type
    ) ?>"><? echo GetMessage("IBSEC_A_BACK_TO_ADMIN") ?></a>
    <?
...
while ($arRes = $rsData->Fetch()) {
    ...
    $sec_add_url = htmlspecialcharsbx(
        $urlBuilder->getSectionDetailUrl(
            null,
            array(
    ...
    $sec_list_url = htmlspecialcharsbx(
        $urlBuilder->getSectionListUrl(
            $arRes["ID"],
    ...
    $row->AddViewField(
        "ID",
        '<a href="' . $edit_url . '" title="' . GetMessage("IBSEC_A_EDIT") . '">' . $arRes["ID"] . '</a>'
    );
    $row->AddViewField(
        "NAME",
        '<a href="' . CHTTP::URN2URI(
            $sec_list_url
        ) . '" ' . ($useTree ? 'style="padding-left:' . (($arRes["DEPTH_LEVEL"] - 1) * 22) . 'px"' : '') . ' class="adm-list-table-icon-link" title="' . GetMessage(
            "IBSEC_A_LIST"
    ...
    if (isset($arVisibleColumnsMap["ELEMENT_CNT"])) {
        ...
        $row->AddViewField(
            "ELEMENT_CNT",
            '<a href="' . CHTTP::URN2URI($elementListUrl) . '" title="' . GetMessage(
                "IBSEC_A_ELLIST"
            ) . '">' . $elementCount . '</a>' .
            ' (<a href="' . CHTTP::URN2URI($nestedElementListUrl) . '" title="' . GetMessage(
                "IBSEC_A_ELLIST_TITLE"
            ) . '">' . $fullElementCount . '</a>)' .
    ...
    if (isset($arVisibleColumnsMap["SECTION_CNT"])) {
        ...
        $row->AddViewField(
            "SECTION_CNT",
            '<a href="' . CHTTP::URN2URI($sec_list_url) . '" title="' . GetMessage(
                "IBSEC_A_LIST"
            ) . '">' . intval(
                CIBlockSection::GetCount($arFilter)
            ) . '</a> [<a href="' . $sec_add_url . '" title="' . GetMessage(
                "IBSEC_A_SECTADD_TITLE"
            ) . '">+</a>]'
        );
    }
    ...
    if (isset($arVisibleColumnsMap["DESCRIPTION"])) {
        $row->AddViewField(
            "DESCRIPTION",
            ($row->arRes["DESCRIPTION_TYPE"] == "text" ? htmlspecialcharsEx($row->arRes["DESCRIPTION"]) : HTMLToTxt(
                $row->arRes["DESCRIPTION"]
            ))
        );
    }
}
...
foreach ($arRows as $id => $row) {
    if (isset($arSectionOps[$id]) && isset($arSectionOps[$id]["section_edit"])) {
        ...
        if (isset($arVisibleColumnsMap["DESCRIPTION"])) {
            ...
            $sHTML .= '<textarea rows="10" cols="50" name="FIELDS[' . $id . '][DESCRIPTION]">' . htmlspecialcharsbx(
                    $row->arRes["DESCRIPTION"]
                ) . '</textarea>';
        ...
        if ($useSectionTranslit) {
            $arActions[] = array(
                ...
                "ACTION" => "if(confirm('" . GetMessageJS(
                        "IBSEC_A_CODE_TRANSLIT_SECTION_CONFIRM"
                    ) . "')) " . $lAdmin->ActionDoGroup($id, ActionType::CODE_TRANSLIT, $sThisSectionUrl),
                "ONCLICK" => ""
            );
        }
    }
    ...
    if (isset($arSectionOps[$id]) && isset($arSectionOps[$id]["section_delete"])) {
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . GetMessageJS("IBSEC_A_CONFIRM_DEL_MESSAGE") . "')) " . $lAdmin->ActionDoGroup(
                    $id,
                    ActionType::DELETE,
                    $sThisSectionUrl
                ),
...
if ($pageConfig['IBLOCK_EDIT'] && CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, 'iblock_edit')) {
    ...
    GetMessage("IBSEC_A_IBLOCK_MANAGE_HINT"),
        ' <a href="iblock_edit.php?type=' . htmlspecialcharsbx(
            ...
            CIBlock::GetAdminSectionListLink(
                $IBLOCK_ID,
                array(
                    "find_section_section" => $find_section_section,
                )
