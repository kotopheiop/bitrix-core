...
<form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?lang=<? echo htmlspecialcharsbx(LANG) ?>" name="form1"
    ...
    if ($bHaveClouds):?>
        <tr>
            <td><label for="CK_DOWNLOAD_CLOUD_FILES"><? echo GetMessage("IBLOCK_CML2_DOWNLOAD_CLOUD_FILES") ?>:</label>
            </td>
            <td>
                <input name="DOWNLOAD_CLOUD_FILES" type="hidden" value="N">
                <input name="DOWNLOAD_CLOUD_FILES" id="CK_DOWNLOAD_CLOUD_FILES" type="checkbox" value="Y"
                       checked="checked">
            </td>
        </tr>
    <? endif; ?>
    <? $tabControl->Buttons(); ?>
    <input type="button" id="start_button" value="<? echo GetMessage("IBLOCK_CML2_START_EXPORT") ?>"
           OnClick="StartExport();" class="adm-btn-save">
    <input type="button" id="stop_button" value="<? echo GetMessage("IBLOCK_CML2_STOP_EXPORT") ?>"
           OnClick="EndExport();">
    <? $tabControl->End(); ?>
</form>
