...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    $row->AddViewField(
        "NAME",
        '<a href="' . $sec_list_url . '" onclick="' . $lAdmin->ActionRedirect(
            $sec_list_url
        ) . '; return false;" title="' . GetMessage(
            "IBLOCK_SECSEARCH_LIST"
        ) . '">' . $f_NAME . '</a><div style="display:none" id="name_' . $f_ID . '">' . $strPath . $f_NAME . $nameSeparator . '</div>'
    );
    ...
    if (array_key_exists("SECTION_CNT", $arVisibleColumnsMap)) {
        ...
        $row->AddViewField(
            ...
            '<a href="' . $sec_list_url . '" onclick="' . $lAdmin->ActionRedirect(
                $sec_list_url
            ) . '; return false;" title="' . GetMessage("IBLOCK_SECSEARCH_LIST") . '">' . intval(
    ...
    if (array_key_exists("MODIFIED_BY", $arVisibleColumnsMap) && intval($f_MODIFIED_BY) > 0) {
        ...
        if ($arUser = $arUsersCache[$f_MODIFIED_BY]) {
            $row->AddViewField(
                "MODIFIED_BY",
                '[<a href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $f_MODIFIED_BY . '" title="' . GetMessage(
                    "IBLOCK_SECSEARCH_USERINFO"
                ) . '">' . $f_MODIFIED_BY . "</a>]&nbsp;(" . htmlspecialcharsEx(
                    $arUser["LOGIN"]
                ) . ") " . htmlspecialcharsEx($arUser["NAME"] . " " . $arUser["LAST_NAME"])
            );
        }
    }
    ...
    if (array_key_exists("CREATED_BY", $arVisibleColumnsMap) && intval($f_CREATED_BY) > 0) {
        ...
        if ($arUser = $arUsersCache[$f_MODIFIED_BY]) {
            $row->AddViewField(
                "CREATED_BY",
                '[<a href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $f_CREATED_BY . '" title="' . GetMessage(
                    "IBLOCK_SECSEARCH_USERINFO"
                ) . '">' . $f_CREATED_BY . "</a>]&nbsp;(" . htmlspecialcharsEx(
                    $arUser["LOGIN"]
                ) . ") " . htmlspecialcharsEx($arUser["NAME"] . " " . $arUser["LAST_NAME"])
            );
        }
    ...
    $row->AddActions(
        array(
            array(
                ...
                "ACTION" => "javascript:SelEl('" . ($get_xml_id ? $f_XML_ID : $f_ID) . "', '" . htmlspecialcharsbx(
                        $jsPath . htmlspecialcharsbx(CUtil::JSEscape($arRes["NAME"]), ENT_QUOTES)
                    ) . $nameSeparator . "')",
            ),
        )
    );
            ...
            if ($find_section_section == $ar_nav["ID"]) {
                ...
                $chain->AddItem(
                    array(
                        ...
                        "ONCLICK" => $lAdmin->ActionRedirect(
                                $extReloadUrl . '&find_section_section=' . $ar_nav["ID"]
                            ) . ';return false;',
                    )
                );
            }
...
?>
    <form method="GET" name="find_section_form" action="<? echo $APPLICATION->GetCurPage() ?>">
        ...
        if ($useParentFilter) {
            ...
            <td>
                <select name="find_section_section">
                    ...
                    <option value="0"<? if ($find_section_section == "0") echo " selected" ?>><? echo GetMessage(
                            "IBLOCK_SECSEARCH_ROOT_PARENT_ID"
                        ) ?></option>
                    <?
        ...
        <tr>
            ...
            <td><? echo CalendarPeriod(
                    "find_section_date_create_1",
                    htmlspecialcharsbx($find_section_date_create_1),
                    "find_section_date_create_2",
                    htmlspecialcharsbx($find_section_date_create_2),
                    "find_section_form"
                ) ?></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <select name="find_section_active">
                    ...
                    <option value="Y"<? if ($find_section_active == "Y") echo " selected" ?>><?= htmlspecialcharsbx(
                            GetMessage("IBLOCK_YES")
                        ) ?></option>
                    <option value="N"<? if ($find_section_active == "N") echo " selected" ?>><?= htmlspecialcharsbx(
