...
?>
    <form method="GET" name="find_section_form" action="<? echo $APPLICATION->GetCurPage() ?>">
        ...
        if (!$iblockFix) {
            ...
            <tr>
            <td><b><?
                    echo GetMessage("IBLOCK_SECSEARCH_IBLOCK") ?></b></td>
            <td><?
        ...
        if ($useParentFilter) {
            ...
            <tr>
            <td><?
                echo GetMessage("IBLOCK_SECSEARCH_PARENT_ID") ?>:
            </td>
            <td>
                <select name="find_section_section">
                    <option value=""><?
                        echo GetMessage("IBLOCK_SECSEARCH_ALL_PARENTS") ?></option>
                    <option value="0"<?
                    if ($find_section_section == "0") echo " selected" ?>><?
                        echo GetMessage("IBLOCK_SECSEARCH_ROOT_PARENT_ID") ?></option>
                    <?
                    ...
                    while ($arSection = $bsections->GetNext()):
                        ...
                        <option value="<?
                        echo $arSection["ID"] ?>"<?
                        if ($arSection["ID"] == $find_section_section) echo " selected" ?>><?
