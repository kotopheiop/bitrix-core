...
$arSubCatalogTabs = CCatalogAdminTools::getShowTabs(
    $IBLOCK_ID,
...
do { //one iteration loop
    ...
    $arShowTabs['catalog'] = $view != "Y" && $bCatalog && ($USER->CanDoOperation(
                'catalog_read'
    ...
    if (
        ...
        if (!(check_bitrix_sessid() || $_SESSION['IBLOCK_CUSTOM_FORM'] === true)) {
            ...
            if (
                $arShowTabs['catalog']
                && file_exists(
                    $_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/catalog/admin/templates/subproduct_edit_validator.php"
                )
            ) {
            ...
            if ($strWarning == '') {
                ...
                $arPREVIEW_PICTURE = CIBlock::makeFileArray(
                    array_key_exists(
                        "SUB_PREVIEW_PICTURE",
                        $_FILES
                    ) ? $_FILES["SUB_PREVIEW_PICTURE"] : $_REQUEST["SUB_PREVIEW_PICTURE"],
                ...
                $arDETAIL_PICTURE = CIBlock::makeFileArray(
                    array_key_exists(
                        "SUB_DETAIL_PICTURE",
                        $_FILES
                    ) ? $_FILES["SUB_DETAIL_PICTURE"] : $_REQUEST["SUB_DETAIL_PICTURE"],
                ...
                if ($arIBlock["RIGHTS_MODE"] === "E" && CIBlockElementRights::UserHasRightTo(
                        $IBLOCK_ID,
                        $ID,
                        "element_rights_edit"
                    )) {
...
if ($error && $error->err_level == 1) {
    ...
    $tabControl = new CAdminSubForm(
        $bCustomForm ? "tabControl_sub" : "form_subelement_" . $IBLOCK_ID,
        $aTabs,
        true,
        false,
        $arListUrl,
    ...
    while ($prop_fields = $properties->Fetch()) {
        ...
        if ($bVarsFromForm && $prop_fields["PROPERTY_TYPE"] == Iblock\PropertyTable::TYPE_FILE) {
            ...
            $vx = $arResult["DOCUMENT"]["PROPERTIES"][(trim(
                    $prop_fields["CODE"]
    ...
    $tabControl->EndCustomField(
        "SUB_ACTIVE_FROM",
        '<input type="hidden" id="SUB_ACTIVE_FROM" name="SUB_ACTIVE_FROM" value="' . $str_ACTIVE_FROM . '">'
    );
    $tabControl->BeginCustomField(
        "SUB_ACTIVE_TO",
    ...
    $tabControl->EndCustomField(
        "SUB_ACTIVE_TO",
        '<input type="hidden" id="SUB_ACTIVE_TO" name="SUB_ACTIVE_TO" value="' . $str_ACTIVE_TO . '">'
    );
    ...
    if ($arTranslit["TRANSLITERATION"] == "Y") {
        ...
        <td style="white-space: nowrap;">
            <input type="text" size="70" name="SUB_NAME" id="SUB_NAME" maxlength="255" value="<? echo $str_NAME ?>"><img
                    id="sub_name_link" title="<? echo GetMessage("IBEL_E_LINK_TIP") ?>" class="linked"
        ...
        $tabControl->EndCustomField(
            "SUB_NAME",
            '<input type="hidden" name="SUB_NAME" id="SUB_NAME" value="' . $str_NAME . '">'
        );
        ...
        <td><? echo $tabControl->GetCustomLabelHTML() ?></td>
        <td style="white-space: nowrap;">
            <input type="text" size="70" name="SUB_CODE" id="SUB_CODE" maxlength="255" value="<? echo $str_CODE ?>"><img
                    id="sub_code_link" title="<? echo GetMessage("IBEL_E_LINK_TIP") ?>" class="linked"
                    src="/bitrix/themes/.default/icons/iblock/<? if ($bLinked) {
                        echo 'link.gif';
                    } else {
                        echo 'unlink.gif';
        ...
        $tabControl->EndCustomField(
            "SUB_CODE",
            '<input type="hidden" name="SUB_CODE" id="SUB_CODE" value="' . $str_CODE . '">'
        );
    } else {
    ...
    if (COption::GetOptionString("iblock", "show_xml_id", "N") == "Y") {
        if ($bCopy || $ID == 0) {
            ...
            <td><span id="hint_SUB_XML_ID"></span>
                <script type="text/javascript">
                    BX.hint_replace(BX('hint_SUB_XML_ID'), '<?=CUtil::JSEscape(
                        htmlspecialcharsbx(GetMessage('IB_SE_FIELD_HINT_XML_ID'))
                    )?>');
                </script> <?= $tabControl->GetCustomLabelHTML(); ?></td>
            <td>
                <input type="text" name="SUB_XML_ID" id="SUB_XML_ID" size="70" maxlength="255"
                       value="<?= $str_XML_ID; ?>">
            </td>
            </tr><?
            $tabControl->EndCustomField(
                "SUB_XML_ID",
                '<input type="hidden" name="SUB_XML_ID" id="SUB_XML_ID" value="' . $str_XML_ID . '">'
            );
        } else {
            $tabControl->AddEditField(
                "SUB_XML_ID",
                GetMessage("IBLOCK_FIELD_XML_ID") . ":",
                $arIBlock["FIELDS"]["XML_ID"]["IS_REQUIRED"] === "Y",
                array("size" => 70, "maxlength" => 255, "id" => "SUB_XML_ID"),
                $str_XML_ID
            );
        }
    }
            ...
            if ($arSubCatalog['SKU_PROPERTY_ID'] != $prop_fields['ID']) {
                ...
                <tr id="tr_PROPERTY_<? echo $prop_fields["ID"]; ?>"<? if ($prop_fields["PROPERTY_TYPE"] == "F"):?> class="adm-detail-file-row"<?endif ?>>
                    <td class="adm-detail-valign-top" width="40%"><? if ($prop_fields["HINT"] != ""):
                            ?><span id="hint_<?= $ID . '_' . $prop_fields["ID"]; ?>"></span>
                            <script type="text/javascript">BX.hint_replace(BX('hint_<?=$ID . '_' . $prop_fields["ID"];?>'), '<?echo CUtil::JSEscape(
                ...
                <? CFileMan::AddHTMLEditorFrame(
                    ...
                    array(
                        'toolbarConfig' => CFileMan::GetEditorToolbarConfig(
                            "iblock_" . (defined('BX_SUB_SETTINGS') && BX_SUB_SETTINGS == true ? 'admin' : 'public')
                        ),
    ...
    <? else:?>
        <tr id="tr_SUB_PREVIEW_TEXT_TYPE">
            ...
            <td>
                <? if ($arIBlock["FIELDS"]["PREVIEW_TEXT_TYPE_ALLOW_CHANGE"]["DEFAULT_VALUE"] === "N"):?>
                    <input type="hidden" name="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>"
                ...
                <? else:?>
                    <input type="radio" name="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>"
                           id="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>_text"
                           value="text"<? if ($str_PREVIEW_TEXT_TYPE != "html") echo " checked" ?>> <label
                            for="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>_text"><? echo GetMessage(
                            "IBLOCK_DESC_TYPE_TEXT"
                        ) ?></label> / <input type="radio" name="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>"
                                              id="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>_html"
                                              value="html"<? if ($str_PREVIEW_TEXT_TYPE == "html") echo " checked" ?>>
                    <label for="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>_html"><? echo GetMessage(
        ...
        <tr id="tr_SUB_PREVIEW_TEXT">
            <td colspan="2" align="center">
                <textarea cols="60" rows="10" name="SUB_PREVIEW_TEXT_<?= $ID; ?>"
    ...
    <? else:?>
        <tr id="tr_SUB_DETAIL_TEXT_TYPE">
            ...
            <td>
                <? if ($arIBlock["FIELDS"]["DETAIL_TEXT_TYPE_ALLOW_CHANGE"]["DEFAULT_VALUE"] === "N"):?>
                    <input type="hidden" name="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>"
                           value="<? echo $str_DETAIL_TEXT_TYPE ?>"><? echo $str_DETAIL_TEXT_TYPE != "html" ? GetMessage(
                        "IBLOCK_DESC_TYPE_TEXT"
                    ) : GetMessage("IBLOCK_DESC_TYPE_HTML") ?>
                <? else:?>
                    <input type="radio" name="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>"
                           id="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>_text"
                           value="text"<? if ($str_DETAIL_TEXT_TYPE != "html") echo " checked" ?>> <label
                            for="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>_text"><? echo GetMessage(
                            "IBLOCK_DESC_TYPE_TEXT"
                        ) ?></label> / <input type="radio" name="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>"
                                              id="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>_html"
                                              value="html"<? if ($str_DETAIL_TEXT_TYPE == "html") echo " checked" ?>>
                    <label for="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>_html"><? echo GetMessage(
                            "IBLOCK_DESC_TYPE_HTML"
                        ) ?></label>
                <?endif ?>
            </td>
        ...
        <tr id="tr_SUB_DETAIL_TEXT">
            <td colspan="2" align="center">
                <textarea cols="60" rows="20" name="SUB_DETAIL_TEXT_<?= $ID; ?>"
        ...
        if ($pr["DATE_CREATE"] <> ''):
            ...
            <tr id="tr_WF_CREATED">
                ...
                <td width="60%"><? echo $pr["DATE_CREATE"] ?><?
                    ...
                    <? else:?>
                        [
                        <a href="user_edit.php?lang=<?= LANGUAGE_ID ?>&amp;ID=<?= $pr["CREATED_BY"] ?>"><?= $pr["CREATED_BY"] ?></a>]&nbsp;<?= htmlspecialcharsex(
            ...
            <? if ($arDocumentState["STATE_NAME"] <> ''):?>
                <tr>
                    ...
                    <td width="60%"><? if ($arDocumentState["ID"] <> ''): ?><a
                                href="<?= $selfFolderUrl ?>bizproc_log.php?ID=<?= $arDocumentState["ID"] ?>"><?endif;
                    ...
                    <tr>
                        ...
                        <td width="60%">
                            ...
                            foreach ($arTasks as $arTask) {
                                ?><ahref="bizproc_task.php?id=<?= $arTask["ID"] ?>&back_url=<?= urlencode(
                                    $APPLICATION->GetCurPageParam("", array())
                                ) ?>"
                                    title="<?= htmlspecialcharsbx($arTask["DESCRIPTION"]) ?>"><?= $arTask["NAME"] ?></a>
                                <br/><?
                            }
                            ?>
                        </td>
    ...
    if ($arShowTabs['edit_rights']):
        ...
        if ($ID > 0) {
            ...
            foreach ($obRights->GetRights() as $RIGHT_ID => $arRight) {
				...
				<input type="hidden" name="SUB_RIGHTS[][GROUP_CODE]" value="' . htmlspecialcharsbx(
                        $arRight["GROUP_CODE"]
                    ) . '">
    ...
    if ($ID > 0 && !$bSubCopy && BX_SUB_SETTINGS) {
        if (!$ajaxReload) {
            ...
            if ($arShowTabs['product_group']) {
				...
				action: function () {
					top.BX.showWait();
					top.BX.ajax.get('" . CIBlock::GetAdminSubElementEditLink(
                        $IBLOCK_ID,
                        $intProductID,
                        $ID,
                        $reloadParams,
