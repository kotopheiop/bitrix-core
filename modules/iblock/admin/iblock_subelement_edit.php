...
if ($error && $error->err_level == 1) {
    ...
    <tr id="tr_SUB_ACTIVE_FROM">
        ...
        <td><?
            echo CAdminCalendar::CalendarDate("SUB_ACTIVE_FROM", $str_ACTIVE_FROM, 19, true) ?></td>
    </tr>
    <?
    ...
    <tr id="tr_SUB_ACTIVE_TO">
        ...
        <td><?
            echo CAdminCalendar::CalendarDate("SUB_ACTIVE_TO", $str_ACTIVE_TO, 19, true) ?></td>
    </tr>
    ...
    if ($arTranslit["TRANSLITERATION"] == "Y") {
        ...
        <td style="white-space: nowrap;">
            <input type="text" size="50" name="SUB_NAME" id="SUB_NAME" maxlength="255" value="<?
            echo $str_NAME ?>">
            <image id="sub_name_link" title="<?
        ...
        <td><?
            echo $tabControl->GetCustomLabelHTML() ?></td>
        <td style="white-space: nowrap;">
            <input type="text" size="50" name="SUB_CODE" id="SUB_CODE" maxlength="255" value="<?
            echo $str_CODE ?>">
            <image id="sub_code_link" title="<?
            echo GetMessage("IBEL_E_LINK_TIP") ?>" class="linked" src="/bitrix/themes/.default/icons/iblock/<?
            if ($bLinked) echo 'link.gif'; else echo 'unlink.gif'; ?>" onclick="set_linked()">
        </td>
        </tr><?
    ...
    if (COption::GetOptionString("iblock", "show_xml_id", "N") == "Y") {
        if ($bCopy || $ID == 0) {
            ...
            <td>
                <input type="text" name="SUB_XML_ID" id="SUB_XML_ID" size="20" maxlength="255"
                       value="<?= $str_XML_ID; ?>">
            </td>
            </tr><?
                ...
                if ($prop_fields["PROPERTY_TYPE"] == "F"):?> class="adm-detail-file-row"<?endif ?>>
                    <td class="adm-detail-valign-top" width="40%"><?
                        if ($prop_fields["HINT"] != ""):
                            ...
                            <script type="text/javascript">BX.hint_replace(BX('hint_<?=$ID . '_' . $prop_fields["ID"];?>'), '<?echo CUtil::JSEscape(htmlspecialcharsbx($prop_fields["HINT"]))?>');</script>&nbsp;<?
                        endif; ?><?
                        echo $tabControl->GetCustomLabelHTML(); ?>:
                    </td>
                    <td width="60%"><?
                        _ShowPropertyField('PROP[' . $prop_fields["ID"] . ']', $prop_fields, $prop_fields["VALUE"], (($historyId <= 0) && (!$bVarsFromForm) && ($ID <= 0)), $bVarsFromForm, 50000, $tabControl->GetFormName()); ?></td>
                </tr>
                <?
                $hidden = "";
    ...
    <? else:?>
        <tr id="tr_SUB_PREVIEW_TEXT_TYPE">
            ...
            <td>
                ...
                if ($arIBlock["FIELDS"]["PREVIEW_TEXT_TYPE_ALLOW_CHANGE"]["DEFAULT_VALUE"] === "N"):?>
                    <input type="hidden" name="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>" value="<?
                ...
                <? else:?>
                    <input type="radio" name="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>"
                           id="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>_text" value="text"<?
                    if ($str_PREVIEW_TEXT_TYPE != "html") echo " checked" ?>> <label
                            for="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>_text"><?
                        echo GetMessage("IBLOCK_DESC_TYPE_TEXT") ?></label> / <input type="radio"
                                                                                     name="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>"
                                                                                     id="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>_html"
                                                                                     value="html"<?
                    if ($str_PREVIEW_TEXT_TYPE == "html") echo " checked" ?>> <label
                            for="SUB_PREVIEW_TEXT_TYPE_<?= $ID; ?>_html"><?
        ...
        <tr id="tr_SUB_PREVIEW_TEXT">
            <td colspan="2" align="center">
                <textarea cols="60" rows="10" name="SUB_PREVIEW_TEXT_<?= $ID; ?>" style="width:100%"><?
    ...
    <? else:?>
        <tr id="tr_SUB_DETAIL_TEXT_TYPE">
            ...
            <td>
                ...
                if ($arIBlock["FIELDS"]["DETAIL_TEXT_TYPE_ALLOW_CHANGE"]["DEFAULT_VALUE"] === "N"):?>
                    <input type="hidden" name="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>" value="<?
                ...
                <? else:?>
                    <input type="radio" name="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>"
                           id="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>_text" value="text"<?
                    if ($str_DETAIL_TEXT_TYPE != "html") echo " checked" ?>> <label
                            for="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>_text"><?
                        echo GetMessage("IBLOCK_DESC_TYPE_TEXT") ?></label> / <input type="radio"
                                                                                     name="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>"
                                                                                     id="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>_html"
                                                                                     value="html"<?
                    if ($str_DETAIL_TEXT_TYPE == "html") echo " checked" ?>> <label
                            for="SUB_DETAIL_TEXT_TYPE_<?= $ID; ?>_html"><?
                        echo GetMessage("IBLOCK_DESC_TYPE_HTML") ?></label>
                <?endif ?>
            </td>
        </tr>
        <tr id="tr_SUB_DETAIL_TEXT">
            <td colspan="2" align="center">
                <textarea cols="60" rows="20" name="SUB_DETAIL_TEXT_<?= $ID; ?>" style="width:100%"><?
    ...
    <tr id="tr_SUB_TAGS">
        ...
        <td>
            ...
            if (CModule::IncludeModule('search')):
                ...
                <input type="text" size="20" name="SUB_TAGS" maxlength="255" value="<?
                echo $str_TAGS ?>">
            <?endif ?>
        </td>
    </tr>
            ...
            <tr id="tr_WF_MODIFIED">
                ...
                <td><?
                    ...
                    if (intval($str_MODIFIED_BY) > 0):
                        ...
                        <? else:?>
                            [<a href="user_edit.php?lang=<?= LANGUAGE_ID ?>&amp;ID=<?= $str_MODIFIED_BY ?>"><?
    ...
    if ($arShowTabs['bizproc']):
                ...
                <tr>
                    ...
                    <td width="60%"><?
                        if (strlen($arDocumentState["ID"]) > 0): ?><a
                                href="<?= $selfFolderUrl ?>bizproc_log.php?ID=<?= $arDocumentState["ID"] ?>"><?
                            endif;
                    ...
                    <tr>
                        ...
                        <td width="60%">
                            ...
                            foreach ($arTasks as $arTask) {
                                ?><a
                                href="bizproc_task.php?id=<?= $arTask["ID"] ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>"
        ...
        if ($ID > 0):
            ...
            if ($bStartWorkflowPermission):
                ...
                <tr>
                    <td colspan="2" align="center">
                        <a href="<?= $selfFolderUrl . MODULE_ID ?>_start_bizproc.php?document_id=<?= $ID ?>&document_type=<?= DOCUMENT_TYPE ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>"><?
