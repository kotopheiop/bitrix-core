...
if (!$zr = $z->Fetch()) {
    $APPLICATION->SetTitle(GetMessage("IBLOCK_ADM_HISTORY_TITLE", array("#ID#" => $ELEMENT_ID)));
    require($_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/main/include/prolog_admin_after.php");
    ?><ahref="/bitrix/admin/iblock_admin.php?type=<? echo htmlspecialcharsbx(
        $type
    ) ?>&amp;lang=<? echo LANG ?>"><? echo htmlspecialcharsex($arIBTYPE["NAME"]) ?></a> - <a
    href="<? echo htmlspecialcharsbx(
        CIBlock::GetAdminElementListLink($IBLOCK_ID, array())
    ) ?>"><? echo htmlspecialcharsbx($arIBlock["NAME"]) ?></a><?
    ...
    foreach ($arID as $ID) {
        ...
        if (CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ID, "element_edit_any_wf_status ")) // only writers
        {
            ...
            $STATUS_PERMISSION = CIBlockElement::WF_GetStatusPermission(
                $arRes["WF_STATUS_ID"],
                $_REQUEST['action'] == "delete" ? $ID : false
            );
                        ...
                        if (!CIBlockElement::Delete(intval($ID))) {
                            if ($ex = $APPLICATION->GetException()) {
                                $lAdmin->AddGroupError(
                                    GetMessage("IBLOCK_ADM_HISTORY_DELETE_ERROR") . " [" . $ex->GetString() . "]",
                                    $ID
                                );
                            } else {
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    if ($f_MODIFIED_BY > 0) {
        $row->AddViewField(
            "MODIFIED_BY",
            '[<a href="user_edit.php?lang=' . LANG . '&ID=' . $f_MODIFIED_BY . '">' . $f_MODIFIED_BY . '</a>] ' . $f_USER_NAME . '</a>'
        );
    }

    $row->AddViewField(
        "WF_STATUS_ID",
        '[<a href="workflow_status_edit.php?ID=' . $f_WF_STATUS_ID . '&lang=' . LANG . '">' . $f_WF_STATUS_ID . '</a>] ' . htmlspecialcharsex(
            CIBlockElement::WF_GetStatusTitle($f_WF_STATUS_ID)
        )
    );
    ...
    $arActions[] = array(
        ...
        "ACTION" => $lAdmin->ActionRedirect(
            'iblock_element_edit.php?type=' . $type . '&ID=' . $f_ID . '&lang=' . LANG . '&IBLOCK_ID=' . $IBLOCK_ID . '&view=Y&find_section_section=' . $find_section_section
        )
    );
    ...
    $arActions[] = array(
        ...
        "ACTION" => "if(confirm('" . GetMessageJS("IBLOCK_ADM_HISTORY_CONFIRM_DEL") . "')) " . $lAdmin->ActionDoGroup(
                $f_ID,
                "delete",
                'type=' . htmlspecialcharsbx(
                    $type
                ) . '&ELEMENT_ID=' . $ELEMENT_ID . '&IBLOCK_ID=' . $IBLOCK_ID . '&find_section_section=' . $find_section_section
            )
    );
    $arActions[] = array(
        "ICON" => "restore",
        ...
        "ACTION" => "if(confirm('" . GetMessageJS(
                "IBLOCK_ADM_HISTORY_RESTORE_CONFIRM"
...
$aContext = array(
    array(
        ...
        "LINK" => "iblock_element_edit.php?WF=Y&ID=" . $ELEMENT_ID . "&type=" . htmlspecialcharsbx(
                $type
            ) . "&lang=" . LANG . "&IBLOCK_ID=" . $IBLOCK_ID . "&find_section_section=" . $find_section_section,
...
<script language="JavaScript">
    ...
    function Diff() {
        ...
        if (j < 2 || j > 2) {
            ...
            window.location = 'iblock_element_edit.php?type=<?echo urlencode($type)?>&lang=<?echo urlencode(
                LANG
            )?>&IBLOCK_ID=<?echo urlencode(
                $IBLOCK_ID
            )?>&view=Y&find_section_section=<?echo $find_section_section?>&ID=' + selection[0] + '&PREV_ID=' + selection[1];
        }
    }
...
<form name="find_form" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?">
    ...
    <tr>
        ...
        <td><input type="text" name="find_name"
                   value="<? echo ($find_name <> '') ? htmlspecialcharsbx($find_name) : "" ?>" size="38"></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td><? echo CalendarPeriod(
                "find_timestamp_from",
                htmlspecialcharsbx($find_timestamp_from),
                "find_timestamp_to",
                htmlspecialcharsbx($find_timestamp_to),
                "find_form"
            ) ?></font></td>
    </tr>
    <tr>
    ...
    $oFilter->Buttons(
        array(
            "table_id" => $sTableID,
            "url" => $APPLICATION->GetCurPage() . "?type=" . htmlspecialcharsbx($type) . "&IBLOCK_ID=" . intval(
                    $IBLOCK_ID
