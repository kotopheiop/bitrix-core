...
if ($bBadBlock)
{
    ...
    <a href="<? echo htmlspecialcharsbx(
        "iblock_admin.php?lang=" . LANGUAGE_ID . "&type=" . urlencode($_REQUEST["type"])
 ...
 */
            ...
            foreach ($_POST['FIELDS'] as $ID => $arFields) {
                ...
                if (!$ib->Update($ID, $arFields, true, true, true)) {
                    $lAdmin->AddGroupError(
                        GetMessage("IBEL_A_SAVE_ERROR", array("#ID#" => $ID, "#ERROR_TEXT#" => $ib->LAST_ERROR)),
                        $ID
                    );
                    $DB->Rollback();
                } else {
                    ...
                    if (
                        ...
                        if (isset($arFields['CATALOG_MEASURE_RATIO'])) {
                            ...
                            if ($newValue != '') {
                                ...
                                if ($intRatioID > 0) {
                                    $ratioResult = CCatalogMeasureRatio::update(
                                        $intRatioID,
                                        array('RATIO' => $newValue)
                                    );
                                } else {
                        ...
                        if (CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ID, "element_delete")) {
                            ...
                            if (!CIBlockElement::Delete($ID)) {
                                ...
                                if ($ex = $APPLICATION->GetException()) {
                                    $lAdmin->AddGroupError(
                                        GetMessage("IBLOCK_DELETE_ERROR") . " [" . $ex->GetString() . "]",
                                        $ID
                                    );
                                } else {
                            ...
                            if ($new_section >= 0) {
                                if (CIBlockSectionRights::UserHasRightTo(
                                    ...
                                    if ($actionId == ActionType::ADD_TO_SECTION) {
                                        $rsSections = $obE->GetElementGroups(
                                            $ID,
                                            true,
                                            array('ID', 'IBLOCK_ELEMENT_ID')
                                        );
                                    ...
                                    if (!$obE->Update($ID, $arFields)) {
                                        $lAdmin->AddGroupError(
                                            GetMessage(
                                                "IBEL_A_SAVE_ERROR",
                                                array("#ID#" => $ID, "#ERROR_TEXT#" => $obE->LAST_ERROR)
                                            ),
                                            $ID
                                        );
                                    }
                                    ...
                                    $lAdmin->AddGroupError(
                                        GetMessage("IBEL_A_UPDERR_ACCESS", array("#ID#" => $ID)),
                                        $ID
                                    );
                                }
                            }
    ...
    foreach (array_keys($rawRows) as $rowId) {
        ...
        if ($bCatalog) {
            ...
            if (isset($arSelectedFieldsMap['CATALOG_TYPE'])) {
                ...
                if (
                    ...
                    if (isset($arRes['VAT_INCLUDED'])) {
                        $arRes['VAT_INCLUDED'] = '';
                    }
                    if (isset($arRes['VAT_ID'])) {
                        $arRes['VAT_ID'] = '';
                    }
                }
            }
        ...
        $row->AddViewField(
            "ID",
            '<a href="' . $arRes["edit_url"] . '" title="' . GetMessage(
                "IBEL_A_EDIT_TITLE"
            ) . '">' . $itemId . '</a>'
        );
        ...
        if (isset($arRes["LOCKED_USER_NAME"]) && $arRes["LOCKED_USER_NAME"] != '') {
            ...
            $userName = (!$canViewUser
                ...
                : '[<a href="' . $selfFolderUrl . 'user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arRes["WF_LOCKED_BY"] . '" title="' . GetMessage(
                    "IBEL_A_USERINFO"
        ...
        if (isset($arRes["USER_NAME"]) && $arRes["USER_NAME"] != '') {
            ...
            $userName = (!$canViewUser
                ...
                : '[<a href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arRes["MODIFIED_BY"] . '" title="' . GetMessage(
                    "IBEL_A_USERINFO"
        ...
        if (isset($arRes["CREATED_USER_NAME"]) && $arRes["CREATED_USER_NAME"] != '') {
            ...
            $userName = (!$canViewUser
                ...
                : '[<a href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arRes["CREATED_BY"] . '" title="' . GetMessage(
                    "IBEL_A_USERINFO"
            ...
            foreach ($arProperties[$aProp["ID"]] as $valueId => $prop) {
                ...
                if ($bUserMultiple) {
                    ...
                    if ($prop["WITH_DESCRIPTION"] == "Y") {
                        $html .= ' <span title="' . GetMessage("IBLOCK_ELEMENT_EDIT_PROP_DESC") . '">' . GetMessage(
                    ...
                    if ($prop['MULTIPLE'] == "Y") {
                        ...
                        if (class_exists('\Bitrix\Main\UI\FileInput', true)) {
                            ...
                            $arEditHTML[] = CFileInput::ShowMultiple(
                                $inputName,
                                'FIELDS[' . $itemId . '][PROPERTY_' . $prop['ID'] . '][n#IND#]',
                                array(
                                    "IMAGE" => "Y",
                                    "PATH" => "Y",
                                    "FILE_SIZE" => "Y",
            ...
            foreach (array_chunk($operateIdList, 500) as $pageIds) {
                ...
                while ($row = $iterator->fetch()) {
                    ...
                    if (isset($productEditPrices[$productId])) {
                        ...
                        $arRows[$productId]->AddMoneyField(
        ...
        if (!empty($sectionIds)) {
            ...
            foreach (array_chunk($sectionIds, 500) as $pageIds) {
                ...
                while ($row = $iterator->Fetch()) {
                    ...
                    $sectionList[$rowId] = '<a href="' . CHTTP::URN2URI($sectionLink) . '" title="' . GetMessage(
                            'IBEL_SECTIONS_LINK_TITLE'
                        ) . '">' . htmlspecialcharsEx($row['NAME']) . '</a>';
    ...
    foreach ($arRows as $idRow => $row) {
        ...
        if (isset($arElementOps[$idRow]) && isset($arElementOps[$idRow]["element_edit"])) {
            ...
            $row->AddViewField(
                "NAME",
                '<a href="' . $row->arRes["edit_url"] . '" title="' . GetMessage(
                    "IBEL_A_EDIT_TITLE"
                ) . '">' . htmlspecialcharsEx($row->arRes["NAME"]) . '</a>'
            );
            ...
            if ($row->arRes['orig']['WF_NEW'] == 'Y' || $row->arRes['WF_STATUS_ID'] == '1') {
                ...
                $row->AddViewField(
                    "WF_STATUS_ID",
                    '<a href="' . $row->arRes["edit_url"] . '" title="' .
                    GetMessage("IBEL_A_ED_TITLE") . '">' . $arWFStatusAll[$row->arRes['WF_STATUS_ID']] .
                    '</a> / <a href="' . htmlspecialcharsbx(
                        $selfFolderUrl . CIBlock::GetAdminElementEditLink(
                            $IBLOCK_ID,
                            $row->arRes['orig']['ID'],
                            array(
                        ...
                        "IBEL_A_ED2_TITLE"
                    ) . '">' . $arWFStatusAll[$row->arRes['orig']['WF_STATUS_ID']] . '</a>'
                );
            }
            ...
            if (isset($arSelectedFieldsMap["PREVIEW_TEXT"])) {
                ...
                $sHTML .= '<textarea rows="10" cols="50" name="FIELDS[' . $idRow . '][PREVIEW_TEXT]">' . htmlspecialcharsbx(
                        $row->arRes["PREVIEW_TEXT"]
                    ) . '</textarea>';
            ...
            if (isset($arSelectedFieldsMap["DETAIL_TEXT"])) {
                ...
                $sHTML .= '<textarea rows="10" cols="50" name="FIELDS[' . $idRow . '][DETAIL_TEXT]">' . htmlspecialcharsbx(
                        $row->arRes["DETAIL_TEXT"]
                    ) . '</textarea>';
        ...
        $clearCounter = array(
            ...
            "ACTION" => "if(confirm('" . GetMessageJS("IBLOCK_CLEAR_COUNTER_CONFIRM") . "')) " . $lAdmin->ActionDoGroup(
                    $row->arRes['orig']['ID'],
        ...
        $elementCodeTranslitAction = array(
            ...
            "ACTION" => "if(confirm('" . GetMessageJS("IBEL_A_CODE_TRANSLIT_CONFIRM") . "')) " . $lAdmin->ActionDoGroup(
                    $row->arRes['orig']['ID'],
        ...
        if ($bWorkFlow) {
            ...
            $arUnLock = array(
                ...
                "ACTION" => "if(confirm('" . GetMessageJS("IBLOCK_UNLOCK_CONFIRM") . "')) " . $lAdmin->ActionDoGroup(
                        $row->arRes['orig']['ID'],
			 ...
			 */
                ...
                if (!$pageConfig['CATALOG']) {
                    $arActions[] = array(
                        ...
                        "ACTION" => $lAdmin->ActionRedirect(
                            'iblock_history_list.php?ELEMENT_ID=' . $row->arRes['orig']['ID'] . '&' . $sThisSectionUrl
                        ),
                        "ONCLICK" => "",
                    );
                }
                ...
                if ($row->arRes['DETAIL_PAGE_URL'] <> '' && $pageConfig['ALLOW_EXTERNAL_LINK']) {
                    $tmpVar = CIBlock::ReplaceDetailUrl(
                        $row->arRes['orig']["DETAIL_PAGE_URL"],
                        $row->arRes['orig'],
                        true,
                        "E"
                    );
            ...
            if (
                ...
                $arActions[] = array(
                    ...
                    "ACTION" => $lAdmin->ActionRedirect(
                        'iblock_start_bizproc.php?document_id=' . $idRow . '&document_type=iblock_' . $IBLOCK_ID . '&back_url=' . urlencode(
            ...
            if ($row->arRes['lockStatus'] == "red") {
                if (CBPDocument::IsAdmin()) {
                    $arActions[] = array(
                        ...
                        "ACTION" => "if(confirm('" . GetMessageJS(
                                "IBEL_A_UNLOCK_CONFIRM"
                            ) . "')) " . $lAdmin->ActionDoGroup($idRow, ActionType::ELEMENT_UNLOCK, $sThisSectionUrl),
                        "ONCLICK" => "",
                    );
                }
                ...
                if (!$pageConfig['CATALOG']) {
                    $arActions[] = array(
                        ...
                        "ACTION" => $lAdmin->ActionRedirect(
                            'iblock_bizproc_history.php?document_id=' . $idRow . '&back_url=' . urlencode(
                                $APPLICATION->GetCurPageParam(
                                    "",
                                    array(
                                        "internal",
                ...
                $arActions[] = array(
                    ...
                    "ACTION" => "if(confirm('" . GetMessageJS(
                            'IBLOCK_CONFIRM_DEL_MESSAGE'
                        ) . "')) " . $lAdmin->ActionDoGroup($idRow, ActionType::DELETE, $sThisSectionUrl),
                    "ONCLICK" => "",
                );
            }
        } else {
...
if ($bBizproc && IsModuleInstalled("bizprocdesigner")) {
    ...
    if ($bCanDoIt) {
        $aContext[] = array(
            ...
            "LINK" => $selfFolderUrl . 'iblock_bizproc_workflow_admin.php?document_type=iblock_' . $IBLOCK_ID . '&lang=' . LANGUAGE_ID . '&back_url_list=' . urlencode(
                    $_SERVER['REQUEST_URI']
                ),
            "LINK_PARAM" => "",
        );
...
if ($pageConfig['IBLOCK_EDIT'] && CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, "iblock_edit")) {
    ...
    GetMessage("IBEL_A_IBLOCK_MANAGE_HINT"),
        ' <a href="' . htmlspecialcharsbx(
            'iblock_edit.php?type=' . urlencode($type) . '&lang=' . LANGUAGE_ID . '&ID=' . urlencode(
                $IBLOCK_ID
            ) . '&admin=Y&return_url=' . urlencode(
                CIBlock::GetAdminElementListLink(
                    $IBLOCK_ID,
                    array('find_section_section' => intval($find_section_section))
                )
            )
        ) . '">',
