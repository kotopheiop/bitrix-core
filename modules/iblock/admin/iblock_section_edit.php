...
if ($bBadBlock) {
    ...
    ?><a href="iblock_admin.php?lang=<?= LANGUAGE_ID ?>&amp;type=<?= urlencode($type) ?>"><?
    echo GetMessage("IBSEC_E_BACK_TO_ADMIN") ?></a><?
        ...
        <tr>
            ...
            <td width="60%"><? echo $str_DATE_CREATE ?><?
                if (intval($str_CREATED_BY) > 0):
                    ?>&nbsp;&nbsp;&nbsp;[<a
                        href="user_edit.php?lang=<?= LANGUAGE_ID ?>&amp;ID=<?= $str_CREATED_BY ?>"><?
        ...
        <td>
            ...
            <select name="IBLOCK_SECTION_ID">
                ...
                while ($a = $l->Fetch()):
                    ...
                    <option value="<?
                    echo intval($a["ID"]) ?>"<?
                    if ($str_IBLOCK_SECTION_ID == $a["ID"]) echo " selected" ?>><?
                    echo str_repeat(".", $a["DEPTH_LEVEL"]) ?><?
...
if ($arTranslit["TRANSLITERATION"] == "Y") {
    ...
    <tr id="tr_NAME">
        ...
        <td nowrap>
            ...
            echo $str_NAME ?>">
            <image id="name_link" title="<?
            echo GetMessage("IBSEC_E_LINK_TIP") ?>" class="linked" src="/bitrix/themes/.default/icons/iblock/<?
...
<? else: ?>
    <tr>
        <td><?
            echo GetMessage("IBSEC_E_DESC_TYPE") ?></td>
        <td>
            <input type="radio" name="DESCRIPTION_TYPE" id="DESCRIPTION_TYPE_text" value="text"<?
            if ($str_DESCRIPTION_TYPE != "html") echo " checked" ?>> <label for="DESCRIPTION_TYPE_text"><?
                echo GetMessage("IBSEC_E_DESC_TYPE_TEXT") ?></label> /
            <input type="radio" name="DESCRIPTION_TYPE" id="DESCRIPTION_TYPE_html" value="html"<?
            if ($str_DESCRIPTION_TYPE == "html") echo " checked" ?>> <label for="DESCRIPTION_TYPE_html"><?
                echo GetMessage("IBSEC_E_DESC_TYPE_HTML") ?></label>
        </td>
    </tr>
    <tr>
        <td colspan="2" align="center">
...
if ($arIBlock["SECTION_PROPERTY"] === "Y") {
    ...
    <tr>
        <td align="right" colspan="2">
            <a id="modeChangeToTree" href="javascript:setMode(BX('table_SECTION_PROPERTY'), 'tree')"><?
                echo GetMessage("IBSEC_E_PROP_TREE_MODE"); ?></a>
            <a id="modeChangeToFlat" style="display: none;"
               href="javascript:setMode(BX('table_SECTION_PROPERTY'), 'flat')"><?
                echo GetMessage("IBSEC_E_PROP_FLAT_MODE"); ?></a>
        </td>
    </tr>
    <tr>
        <td align="center" colspan="2">
            <table class="internal" id="table_SECTION_PROPERTY"
                ...
                >
                    <td class="internal-left"><?
                        echo GetMessage("IBSEC_E_PROP_TABLE_NAME"); ?></td>
                    <td><?
                        echo GetMessage("IBSEC_E_PROP_TABLE_CODE"); ?></td>
                    <td><?
                        echo GetMessage("IBSEC_E_PROP_TABLE_TYPE"); ?></td>
                    <td><?
                        echo GetMessage("IBSEC_E_PROP_TABLE_SMART_FILTER"); ?></td>
                    <td><?
                        echo GetMessage("IBSEC_E_PROP_TABLE_DISPLAY_TYPE"); ?></td>
                    <td><?
                        echo GetMessage("IBSEC_E_PROP_TABLE_DISPLAY_EXPANDED"); ?></td>
                    <td><?
                        echo GetMessage("IBSEC_E_PROP_TABLE_FILTER_HINT"); ?></td>
                    <td class="internal-right"><?
                        echo GetMessage("IBSEC_E_PROP_TABLE_ACTION"); ?></td>
                </tr>
                <?
                ...
                foreach ($arParents as $parent) {
                    ...
                    >
                        <td align="left" colspan="4" style="background-color:#f5f9f9; border: none;">
                            ...
                            if ($parent["ID"] > 0):?>
                                <br><br><a href="<?
                                echo CIBlock::GetAdminSectionEditLink($IBLOCK_ID, $parent["ID"], array("form_section_" . $IBLOCK_ID . "_active_tab" => "edit4")); ?>"><?
                                    echo htmlspecialcharsex($parent["TITLE"]) ?></a>
                            <? else:?>
                                <br><br><a href="<?
                                echo CIBlock::GetAdminIBlockEditLink($IBLOCK_ID, array("form_catalog_edit_" . $IBLOCK_ID . "_active_tab" => "edit3")); ?>"><?
                                    echo htmlspecialcharsex($parent["TITLE"]) ?></a>
                            <?endif; ?>
                        </td>
                    </tr>
                ...
                while ($arProp = $rsProps->Fetch()):
                    ...
                    ?>
                    <tr
                            id="tr_SECTION_PROPERTY_<?
                            echo $arProp["ID"] ?>"
                            mode="<?
                            echo(is_array($arLink) ? "both" : "none"); ?>"
                            prop_sort="<?
                            echo $arProp["SORT"] ?>"
                            prop_id="<?
                            echo $arProp["ID"] ?>"
                            left_margin="<?
                            echo is_array($arLink) ? $arLink["LEFT_MARGIN"] : $maxMargin; ?>"
                        <?
                    ...
                    >
                        <td align="left" class="internal-left">
                            ...
                            if (!is_array($arLink) || $arLink["INHERITED"] == "N"):?>
                                <input type="hidden" name="SECTION_PROPERTY[<?
                                echo $arProp["ID"] ?>][SHOW]" id="hidden_SECTION_PROPERTY_<?
                                echo $arProp["ID"] ?>" value="<?
                                echo is_array($arLink) ? "Y" : "N"; ?>">
                            <?endif ?>
                            <?
                            echo htmlspecialcharsex($arProp["NAME"]) ?>
                        </td>
                        <td align="left"><?
                            echo htmlspecialcharsex($arProp["CODE"]) ?></td>
                        <td align="left"><?
            ...
            echo $arIBlock["ID"]; ?>, 'select_SECTION_PROPERTY', 'shadow_SECTION_PROPERTY', 'table_SECTION_PROPERTY')">
            <script type="text/javascript">
    ...
    if (is_array($arCatalog)) {
        ...
        <tr class="heading">
            <td align="center" colspan="3"><?
                echo GetMessage("IBSEC_E_PROP_SKU_SECTION"); ?></td>
        </tr>
        <tr>
            <td align="center" colspan="2">
