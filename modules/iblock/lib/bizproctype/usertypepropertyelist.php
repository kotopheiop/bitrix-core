...
if (Loader::requireModule('bizproc')) {
    class UserTypePropertyElist extends UserTypeProperty
    {
        ...
        public static function renderControlOptions(FieldType $fieldType, $callbackFunctionName, $value)
        {
            ...
            $result = '<select id="WFSFormOptionsX" onchange="' . Main\Text\HtmlFilter::encode(
            ...
            foreach ($iblockTypeIterator as $iblockTypeId => $iblockTypeName) {
                ...
                $iblockIterator = \CIBlock::getList(
                    array('SORT' => 'ASC'),
                    array('TYPE' => $iblockTypeId, 'ACTIVE' => 'Y')
                );
            ...
            $result .= '</select><!--__defaultOptionsValue:' . $defaultIBlockId . '--><!--__modifyOptionsPromt:' . Loc::getMessage(
