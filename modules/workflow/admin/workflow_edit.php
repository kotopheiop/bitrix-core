...
?>
    ...
    <form method="POST" name="form1" enctype="multipart/form-data">
            ...
            <tr>
                ...
                <td><select name="template"
                        ...
                        foreach ($arTemplates as $Template) {
                            ?>
                            <option value="<?
                            echo htmlspecialcharsbx($Template["file"]) ?>"<?
                            if ($template == $Template["file"]) echo " selected" ?>><?
                            echo htmlspecialcharsbx($Template["name"]) ?></option><?
                        }
                        ?></select></td>
            </tr>
        <? endif; ?>
        <tr>
            ...
            <td width="60%"><?
                if ($str_STATUS_ID != 1 || $message) :
                    ...
                    ?>[<a
                        href="workflow_status_edit.php?lang=<?= LANG ?>&amp;ID=<?= $str_STATUS_ID ?>"><?= $str_STATUS_ID ?></a>]&nbsp;<?= $str_STATUS_TITLE ?><?
                endif;
                ?></td>
        </tr>
        <? if ($ID > 0): ?>
            ...
            <tr>
                <td colspan="2" align="center"><? echo InputType("radio", "BODY_TYPE", "text", $str_BODY_TYPE, false) ?>
                    ...
                    echo InputType("radio", "BODY_TYPE", "html", $str_BODY_TYPE, false) ?>&nbsp;HTML&nbsp;<br><textarea
                            name="BODY" style="width:100%" rows="30" wrap="VIRTUAL"><?
                        echo $str_BODY ?></textarea></td>
            </tr>
        <?
        endif;
        ?>
                ...
                <td colspan="2">
                    <table border="0" cellspacing="0" cellpadding="0" width="100%" class="internal">
                        ...
                        <? foreach ($arDocFiles as $zr) :?>
                            <tr>
                                <td><?= $zr["ID"] ?></td>
                                <td><a title="<?= GetMessage("FLOW_VIEW_IMAGE") ?>" href="javascript:void(0)"
                                       OnClick="ShowFile(<?= $ID ?>,'<?= $zr["FILENAME"] ?>')"><?= $zr["FILENAME"] ?></a><?
                                    $ext = GetFileExtension($zr["FILENAME"]);
                                    if ($USER->IsAdmin() || !in_array($ext, $arExt)) :
                                        ?>&nbsp;&nbsp;<a href="workflow_file_download.php?did=<?= $ID ?>&amp;fname=<?
                                    echo $zr["FILENAME"] ?>" title="<?= GetMessage("FLOW_DOWNLOAD_FILE") ?>"><img
                                                onmouseover="this.src='/bitrix/images/workflow/download_file.gif'"
                                                onmouseout="this.src='/bitrix/images/workflow/download_file_t.gif'"
                                                src="/bitrix/images/workflow/download_file_t.gif" width="16" height="16"
                                                border=0></a><?
                                    endif;
                                    ?></td>
                                <td><?= $zr["FILESIZE"] ?></td>
                                <td><?= $zr["TIMESTAMP_X"] ?></td>
                                <td>[<a href="user_edit.php?ID=<? echo $zr["MODIFIED_BY"] ?>&lang=<?= LANG ?>"
                                        title="<?= GetMessage('FLOW_USER_ALT') ?>"><? echo $zr["MODIFIED_BY"] ?></a>]&nbsp;<? echo $zr["USER_NAME"] ?>
                                </td>
            ...
            <td colspan=2>
                <table border="0" cellspacing="0" cellpadding="0" width="100%" class="internal">
                    ...
                    for ($i = 1; $i <= 3; $i++):
                        ?>
                        <tr>
                            <td><input type="text" name="fname_<?= $i ?>" size="30" maxlength="255"
                                       value="<?= htmlspecialcharsbx(${"fname_" . $i}) ?>" id="fname_<?= $i ?>"
                                       style="width:100%"></td>
                            <td style="text-align:center"><input type="file" name="file_<?= $i ?>" size="30"
                                                                 maxlength="255" value=""
                                                                 onChange="NewFileName(this, <?= $i ?>)"
                                                                 id="file_<?= $i ?>">
                            </td>
                        </tr>
                    <? endfor ?>
        ...
        <? $tabControl->BeginNextTab(); ?>
        <tr>
            <td align="center" colspan="2"><textarea name="COMMENTS" rows="15"
