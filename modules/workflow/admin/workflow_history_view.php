...
<tr>
    ...
    <td width="60%"><?
        if (CWorkflow::IsHaveEditRights($str_DOCUMENT_ID)) :
            ?><a href="workflow_edit.php?lang=<?= LANG ?>&ID=<?= $str_DOCUMENT_ID ?>"
                 title="<?= GetMessage('FLOW_VIEW_DOC_ALT') ?>"><?= $str_DOCUMENT_ID ?></a><?
        else :
            ?><?= $str_DOCUMENT_ID ?><?
        endif;
        ?></td>
...
<tr>
    ...
    <td><a href="/bitrix/admin/site_edit.php?lang=<?= LANG ?>&LID=<?= $str_SITE_ID ?>"
           title="<?= GetMessage('FLOW_SITE_ALT') ?>"><?= $str_SITE_ID ?></a></td>
</tr>
<tr>
...
<tr>
    ...
    <td><?= $str_TIMESTAMP_X ?>&nbsp;&nbsp;[<a href="user_edit.php?ID=<?= $str_MODIFIED_BY ?>&lang=<?= LANG ?>"
                                               title="<?= GetMessage('FLOW_USER_ALT') ?>"><?= $str_MODIFIED_BY ?></a>]&nbsp;<? echo $str_USER_NAME ?>
    </td>
</tr>
<tr>
    <td><?= GetMessage("FLOW_STATUS") ?></td>
    <td>[<a href="workflow_status_edit.php?ID=<?= $str_STATUS_ID ?>&lang=<?= LANG ?>"
            title="<?= GetMessage('FLOW_STATUS_ALT') ?>"><? echo $str_STATUS_ID ?></a>]&nbsp;<?= $str_STATUS_TITLE ?>
    </td>
</tr>
<?
if ($ID > 0 && $PREV_ID > 0):?>
...
if ($document_exist == "Y"):
    ...
    <tr>
        ...
        <td width="60%"><?
            if (CWorkflow::IsHaveEditRights($zr["ID"])) :
                ?><a href="workflow_edit.php?lang=<?= LANG ?>&ID=<?= $zr["ID"] ?>"
                     title="<?= GetMessage('FLOW_VIEW_DOC_ALT') ?>"><?= $zr["ID"] ?></a><?
            else :
                ?><?= $zr["ID"] ?><?
            endif;
            ?></td>
    ...
    <tr>
        ...
        <td><a href="<?= htmlspecialcharsbx($zr["FILENAME"]) ?>"
               title="<?= GetMessage('FLOW_VIEW_FILE_ALT') ?>"><?= htmlspecialcharsbx($zr["FILENAME"]) ?></a></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td><?= $zr["DATE_ENTER"] ?>&nbsp;&nbsp;[<a href="user_edit.php?ID=<?= $zr["ENTERED_BY"] ?>&lang=<?= LANG ?>"
                                                    title="<?= GetMessage('FLOW_USER_ALT') ?>"><?= $zr["ENTERED_BY"] ?></a>]&nbsp;<?
            echo htmlspecialcharsbx($zr["EUSER_NAME"]) ?></td>
    </tr>
    <tr>
