...
<form method="POST" name="form1" action="<? echo $APPLICATION->GetCurPage() ?>?" enctype="multipart/form-data">
    ...
    <? if ($ID > 0) : ?>
        <tr>
            ...
            <td><a href="workflow_list.php?lang=<?= LANG ?>&find_status=<?= $ID ?>&set_filter=Y"
                   title="<?= GetMessage('FLOW_DOCUMENTS_ALT') ?>"><? echo intval($str_DOCUMENTS) ?></a></td>
        </tr>
    <? endif; ?>
    <? if ($ID != 1): ?>
        <tr>
    ...
    <tr>
        <td class="adm-detail-valign-top"><?= GetMessage('FLOW_MOVE_RIGHTS'); ?><br><img
                    src="/bitrix/images/workflow/mouse.gif" width="44" height="21" border=0 alt=""></td>
        <td><? echo SelectBoxM("arPERMISSION_M[]", CGroup::GetDropDownList(""), $arPERMISSION_M, "", true, 8); ?></td>
    </tr>
    <tr>
        <td class="adm-detail-valign-top"><? echo GetMessage('FLOW_EDIT_RIGHTS'); ?><br><img
                    src="/bitrix/images/workflow/mouse.gif" width="44" height="21" border=0 alt=""></td>
