...
if ($WORKFLOW_RIGHT >= "R") {
        ...
        <tr>
            ...
            <td width="60%">
                ...
                if ($type[0] == "checkbox") {
                    ...
                    if ($Option[4]) {
                        ?>&nbsp;<label for="<?
                        echo htmlspecialcharsbx($Option[0]) ?>_clear"><?= GetMessage("FLOW_CLEAR") ?>:</label><input
                        type="checkbox" name="<?
                        echo htmlspecialcharsbx($Option[0]) ?>_clear" id="<?
                        echo htmlspecialcharsbx($Option[0]) ?>_clear" value="Y"><?
                    }
    ...
    <tr>
        <td><?
            echo GetMessage("FLOW_ADMIN") ?></td>
        <td><?
            echo SelectBox("WORKFLOW_ADMIN_GROUP_ID", CGroup::GetDropDownList(""), GetMessage("MAIN_NO"), htmlspecialcharsbx($WORKFLOW_ADMIN_GROUP_ID)); ?></td>
    </tr>
    ...
    ?>
    <input <?
    if ($WORKFLOW_RIGHT < "W") echo "disabled" ?> type="submit" name="Update" value="<?= GetMessage("FLOW_SAVE") ?>"
                                                  class="adm-btn-save">
    <input type="hidden" name="Update" value="Y">
    <input type="reset" name="reset" value="<?= GetMessage("FLOW_RESET") ?>">
    <input <?
    if ($WORKFLOW_RIGHT < "W") echo "disabled" ?> type="submit" title="<?
    ...
    echo GetMessage("MAIN_RESTORE_DEFAULTS") ?>" name="RestoreDefaults">
    <?= bitrix_sessid_post(); ?>
    <?
    $tabControl->End(); ?>
    </form>
