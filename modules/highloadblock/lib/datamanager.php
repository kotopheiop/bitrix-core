...
abstract class DataManager extends Entity\DataManager
{
    ...
    public static function checkFields(Entity\Result $result, $primary, array $data)
    {
        ...
        foreach ($data as $k => $v) {
            if (!(static::getEntity()->hasField($k) && static::getEntity()->getField(
                    $k
                ) instanceof Entity\ScalarField)) {
    ...
    public static function add(array $data)
    {
        ...
        try {
            ...
            if (!empty($data)) {
                ...
                $sql = "UPDATE " . $helper->quote($tableName) . " SET " . $update[0] . " WHERE " . $helper->quote(
                        $identity
                    ) . " = " . ((int)$id);
