            ...
            <td><?
                if (!$isEditMode):
                    ...
                    ?><input type="text" name="TABLE_NAME" size="30"
                             value="<?= htmlspecialcharsbx($hlblock['TABLE_NAME']) ?>"><?
                endif;
                ?></td>
        </tr>
        <? if ($is_update_form): ?>
                                ...
                                <td>
                                    <select name="RIGHTS[TASK_ID][]">
                                        <? foreach ($tasks as $tid => $tname):?>
                                            <option value="<?= $tid ?>"<? if ($task == $tid) {
                                                ?> selected="selected"<? } ?>><?= $tname ?></option>
                                        <? endforeach; ?>
                                    </select>
                                    <input type="hidden" name="RIGHTS[RIGHT_ID][]" value="<?= $i ?>">
                                    <input type="hidden" name="RIGHTS[ACCESS_CODE][]"
                                           value="<?= htmlspecialcharsbx($code) ?>">
                                    <a href="javascript:void(0);" onclick="deleteRow(this);"
                                       data-id="<?= htmlspecialcharsbx($code) ?>" class="access-delete"></a>
                                </td>
                            </tr>
                        <? endforeach; ?>
                    <? endif; ?>
