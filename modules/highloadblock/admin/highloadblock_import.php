...
?>
    ...
    <form name="form_tools" method="get" action="<?= $APPLICATION->GetCurPage() ?>">
        ...
        <tr>
            ...
            <td>
                <select id="object" onchange="selectHL(this);">
                    ...
                    <? foreach ($hls as $row): ?>
                        <option value="<?= $row['ID'] ?>"><?= htmlspecialcharsbx($row['NAME']) ?> [<?= $row['ID'] ?>]
                        </option>
                    <? endforeach; ?>
                </select>
            </td>
        ...
        <tr>
            <td><?= Loc::getMessage('ADMIN_TOOLS_FIELD_IMPORT_XML_ID') ?>:<sup><span class="required">1</span></sup>
            </td>
            <td>
        ...
        <tr>
            <td><?= Loc::getMessage('ADMIN_TOOLS_FIELD_SAVE_REFERENCE') ?><sup><span class="required">2</span></sup>:
            </td>
            <td>
        ...
        <? $tabControl->Buttons(); ?>
        <input type="button" id="start_button" value="<?= Loc::getMessage('ADMIN_TOOLS_START_IMPORT') ?>"
               OnClick="StartImport();" class="adm-btn-save"/>
        <input type="button" id="stop_button" value="<?= Loc::getMessage('ADMIN_TOOLS_STOP_IMPORT') ?>"
               OnClick="EndImport();"/>
        <? $tabControl->End(); ?>
    </form>
<?= BeginNote(); ?>
