...
function PhotoDateFormat($format = "", $timestamp = "")
{
    ...
    if (LANG == "en") {
        ...
        foreach ($matches[0] as $i => $match) {
            switch ($match) {
                case "F":
                    $match = GetMessage("P_MONTH_" . date("n", $timestamp));
                    break;
                case "M":
                    $match = GetMessage("P_MON_" . date("n", $timestamp));
                    break;
                case "l":
                    $match = GetMessage("P_DAY_OF_WEEK_" . date("w", $timestamp));
                    break;
                case "D":
                    $match = GetMessage("P_DOW_" . date("w", $timestamp));
                    break;
            }
