...
//	static $arIBlock = array(); 
    ...
    public static function CheckElement($ID, &$arElement, &$arSection, &$arGallery)
    {
        ...
        $tmp_db_res = CIBlockElement::GetProperty(
            $arElement["IBLOCK_ID"],
            $ID,
            $by = "sort",
            $order = "asc",
            array("CODE" => "REAL_PICTURE")
        );
        ...
        if (!is_set($GLOBALS["PHOTOGALLERY_VARS"]["arIBlock"], $arElement["IBLOCK_ID"])) {
            ...
            $db_res = CUserTypeEntity::GetList(
                array($by => $order),
                array(
                    "ENTITY_ID" => "IBLOCK_" . $arElement["IBLOCK_ID"] . "_SECTION",
    ...
    public static function OnBeforeIBlockElementDelete($ID)
    {
        ...
        if (CPhotogalleryElement::CheckElement($ID, $arElement, $arSection, $arGallery)) {
            $arGallery["UF_GALLERY_SIZE"] = (doubleval(
                    $arGallery["UF_GALLERY_SIZE"]
                ) - $arElement["FILE"]["FILE_SIZE"]);
            ...
            $GLOBALS["USER_FIELD_MANAGER"]->EditFormAddFields(
                "IBLOCK_" . $arElement["IBLOCK_ID"] . "_SECTION",
                $arFields
            );
    ...
    public static function OnRecalcGalleries($ID, $INDEX)
    {
        ...
        if (CPhotogalleryElement::CheckElement($ID, $arElement, $arSection, $arGallery)) {
            ...
            $arFields["UF_GALLERY_SIZE"] = $GLOBALS["UF_GALLERY_SIZE"] = $GLOBALS["PHOTOGALLERY_VARS"]["arGalleries"][$arGallery["ID"]]["UF_GALLERY_SIZE"] = (doubleval(
                    $arGallery["UF_GALLERY_SIZE"]
                ) + $arElement["FILE"]["FILE_SIZE"]);
            $bs = new CIBlockSection;
            $GLOBALS["USER_FIELD_MANAGER"]->EditFormAddFields(
    ...
    public static function OnAfterIBlockElementAdd($res)
    {
        $ID = doubleval($res["ID"]);
        if (CPhotogalleryElement::CheckElement($ID, $arElement, $arSection, $arGallery)) {
            $GLOBALS["UF_GALLERY_SIZE"] = $GLOBALS["PHOTOGALLERY_VARS"]["arGalleries"][$arGallery["ID"]]["UF_GALLERY_SIZE"] = (doubleval(
                    $arGallery["UF_GALLERY_SIZE"]
                ) + $arElement["FILE"]["FILE_SIZE"]);
            $arFields = array(
                "IBLOCK_ID" => $arElement["IBLOCK_ID"],
