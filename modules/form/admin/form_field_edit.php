...
<form name="form1" method="POST" action=""
      enctype="multipart/form-data"<? if ($additional != "Y"): ?> onSubmit="return FormSubmit();"<? endif ?>>
...
//********************
        ...
        <tr>
            ...
            <td><?
                ...
                if ($str_IMAGE_ID > 0):?><br/><?
                    echo CFile::ShowImage($str_IMAGE_ID, 200, 200, "border=0", "", true) ?><?endif; ?>
            </td>
        </tr>
    <?
    ...
    <?
        ...
        <tr>
            <td align="center" colspan="2"><textarea name="TITLE" style="width:100%" rows="23"><?
                    echo $str_TITLE ?></textarea></td>
        </tr>
    <?
    endif;
    ?>
            ...
            <td colspan="2" style="padding:0px;">
                <script>
                    function addAnswer() {
                        ...
                        obTable.tBodies[0].appendChild(input1);
                        obTable.tBodies[0].appendChild(input2);
                ...
                <table border="0" cellspacing="1" cellpadding="0" width="100%" id="answers_table">
                    <thead>
                    <tr class="heading">
                        <td nowrap="nowrap" width="0%">ID</td>
                        <td nowrap="nowrap" width="100%"><?
                            echo GetMessage("FORM_MESSAGE") ?><span class="required"><sup>1</sup></span><br/>[&nbsp;ANSWER_TEXT&nbsp;]
                        </td>
                        <td nowrap="nowrap"><?
                            echo GetMessage("FORM_FIELD_VALUE") ?><sup>&nbsp;</sup><br/>[&nbsp;ANSWER_VALUE&nbsp;]
                        </td>
                        <td nowrap="nowrap"><?
                            echo GetMessage("FORM_FIELD_TYPE") ?></td>
                        <td nowrap="nowrap"><?
                            echo GetMessage("FORM_FIELD_WIDTH") ?></td>
                        <td nowrap="nowrap"><?
                            echo GetMessage("FORM_FIELD_HEIGHT") ?></td>
                        <td nowrap="nowrap"><?
                            echo GetMessage("FORM_FIELD_PARAM") ?></td>
                        <td nowrap="nowrap"><?
                            echo GetMessage("FORM_SORT") ?></td>
                        <td nowrap="nowrap"><?
                            echo GetMessage("FORM_ACT") ?></td>
                        <td nowrap="nowrap"><?
                            echo GetMessage("FORM_DEL") ?></td>
                    </tr>
                    </thead>
                    <tbody>
                    <?
                    ...
                    while ($zr = $z->ExtractFields("p_")) :
                        ...
                        <tr>
                            <td><?= $p_ID ?></td>
                            <td><input type="text" name="MESSAGE_<?= $i ?>" value="<?= $p_MESSAGE ?>" style="width:100%"
                            ...
                            <td nowrap="nowrap"><input <?
                                if ($p_FIELD_TYPE != "text" && $p_FIELD_TYPE != "textarea" && $p_FIELD_TYPE != "image" && $p_FIELD_TYPE != "date" && $p_FIELD_TYPE != 'email') echo "disabled" ?>
                                        type="text" id="FIELD_WIDTH_<?= $i ?>" name="FIELD_WIDTH_<?= $i ?>" value="<?
                                if (intval($p_FIELD_WIDTH) > 0) echo $p_FIELD_WIDTH ?>" size="3"/></td>
                            <td nowrap="nowrap"><input <?
                                if ($p_FIELD_TYPE != "textarea" && $p_FIELD_TYPE != "multiselect") echo "disabled" ?>
                                        type="text" id="FIELD_HEIGHT_<?= $i ?>" name="FIELD_HEIGHT_<?= $i ?>" value="<?
                                if (intval($p_FIELD_HEIGHT) > 0) echo $p_FIELD_HEIGHT ?>" size="3"/></td>
                            <td nowrap="nowrap"><input type="text" name="FIELD_PARAM_<?= $i ?>"
                                                       value="<?= $p_FIELD_PARAM ?>" size="8"/></td>
                            <td nowrap="nowrap"><input type="text" name="C_SORT_<?= $i ?>" value="<?= $p_C_SORT ?>"
                                                       size="3"/></td>
                            <td><?
                    ...
                    while ($i <= $count) :
                        ...
                        <tr>
                            <td></td>
                            <td><input type="text" name="MESSAGE_<?= $i ?>" value="<?= $message ?>" style="width:100%"
                                       onchange="jsFormValidatorSettings.UpdateAll();"/></td>
                            ...
                            <td nowrap="nowrap"><?
                                ...
                                ?></td>
                            <td nowrap="nowrap"><input <?
                                if ($ftype != "text" && $ftype != "textarea" && $ftype != "image" && $ftype != "date" && $ftype != 'email') echo "disabled" ?>
                                        type="text" id="FIELD_WIDTH_<?= $i ?>" name="FIELD_WIDTH_<?= $i ?>"
                                        value="<?= $width ?>" size="3"/></td>
                            <td nowrap="nowrap"><input <?
                                if ($ftype != "textarea" && $ftype != "multiselect") echo "disabled" ?> type="text"
                                                                                                        id="FIELD_HEIGHT_<?= $i ?>"
                                                                                                        name="FIELD_HEIGHT_<?= $i ?>"
                                                                                                        value="<?= $height ?>"
                                                                                                        size="3"/></td>
                            <td nowrap="nowrap"><input type="text" name="FIELD_PARAM_<?= $i ?>" value="<?= $param ?>"
                                                       size="8"/></td>
                            <td nowrap="nowrap"><input type="text" name="C_SORT_<?= $i ?>" value="<?
                                echo (strlen(${"C_SORT_" . $i}) > 0 && strlen($message) > 0) ? htmlspecialcharsbx(${"C_SORT_" . $i}) : $s ?>"
                                                       size="3"/></td>
                            <td><?
                    ...
                    <tr>
                        <td></td>
                        <td colspan="9">
                            <div onclick="addAnswer()" id="btn_new"
                                 style="height: 20px; width: 110px; background-repeat: no-repeat; cursor: pointer;"
                                 title="<?= GetMessage('FORM_ADD_ANSWER') ?>">
                                <nobr>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?= GetMessage('FORM_ADD_ANSWER') ?></nobr>
                            </div>
                        </td>
                    </tr>
                    </tfoot>
    ...
    ?>
        ...
        <script language="JavaScript">
            ...
            arCurrentValidators[arCurrentValidators.length] = {
                ...
                PARAMS: [<?
                    ...
                    {
                        NAME: '<?=CUtil::JSEscape($key)?>',
                        VALUE: '<?=CUtil::JSEscape($value)?>',
                        TITLE: '<?=CUtil::JSEscape($arVal["PARAMS_FULL"][$key]["TITLE"]) ?>:'
                    }<?
                    }
                    ?>
        ...
        <tr style="display: none;">
            <td colspan="2"><?= CalendarDate('__test', '', 'form1') ?></td>
        </tr>
        <tr>
    ...
    if ($additional != "Y"):
        ...
        <tr>
            <td><?
                echo GetMessage("FORM_FILTER_FOR_USER") ?><br><img src="/bitrix/images/form/mouse.gif" width="44"
        ...
        <tr>
            <td><?
                echo GetMessage("FORM_FILTER_FOR_ANSWER_TEXT") ?><br><img src="/bitrix/images/form/mouse.gif" width="44"
                                                                          height="21" border=0 alt=""></td>
            <td><?
        ...
        <tr>
            <td><?
                echo GetMessage("FORM_FILTER_FOR_ANSWER_VALUE") ?><br><img src="/bitrix/images/form/mouse.gif"
