...
$lAdmin->AddHeaders(
    array(
        ...
        array(
            "id" => "DEFAULT_VALUE",
            "content" => GetMessage("FORM_DEFAULT"),
            "sort" => "s_default",
            "default" => true
        ),
        ...
        array(
            "id" => "DESCRIPTION",
            "content" => GetMessage("FORM_DESCRIPTION"),
            "sort" => "s_description",
            "default" => true
        ),
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    $row->AddViewField(
        "RESULTS",
        "<a href='form_result_list.php?lang=" . LANGUAGE_ID . "&WEB_FORM_ID=$WEB_FORM_ID&find_status=$f_ID&set_filter=Y' title='" . GetMessage(
            "FORM_RESULT_TITLE"
        ) . "'>$f_RESULTS</a>"
    );
    $row->AddViewField(
        "TITLE",
        "<a href='form_status_edit.php?lang=" . LANGUAGE_ID . "&WEB_FORM_ID=$WEB_FORM_ID&ID=$f_ID' title='" . GetMessage(
            "FORM_EDIT"
        ) . "'>$f_TITLE</a>"
    );
    ...
    $arActions[] = array(
        ...
        "ACTION" => $lAdmin->ActionRedirect(
            "form_status_edit.php?lang=" . LANGUAGE_ID . "&ID=$f_ID&WEB_FORM_ID=$WEB_FORM_ID"
        ),
    ...
    if ($F_RIGHT >= 30) {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect(
                "form_status_list.php?lang=" . LANGUAGE_ID . "&cp_id=$f_ID&WEB_FORM_ID=$WEB_FORM_ID&" . bitrix_sessid_get(
                )
            ),
        ...
        $arActions[] = array(
            ...
            "ACTION" => "javascript:if(confirm('" . GetMessage(
                    "FORM_DELETE_STATUS_CONFIRM"
                ) . "')) " . $lAdmin->ActionRedirect(
                    "?lang=" . LANGUAGE_ID . "&action=delete&ID=$f_ID&WEB_FORM_ID=$WEB_FORM_ID&" . bitrix_sessid_get()
                ),
...
?>
    <form name="form1" method="GET" action="<?= $APPLICATION->GetCurPage() ?>">
        ...
        <tr>
            ...
            <td nowrap><input type="text" name="find_title" value="<? echo htmlspecialcharsbx($find_title) ?>"
                              size="47"><?= InputType(
                    "checkbox",
                    "find_title_exact_match",
                    "Y",
                    ...
                    false,
                    "",
                    "title='" . GetMessage("FORM_EXACT_MATCH") . "'"
        ...
        <tr>
            ...
            <td nowrap><input type="text" name="find_description"
                              value="<? echo htmlspecialcharsbx($find_description) ?>" size="47"><?= InputType(
                    "checkbox",
                    "find_description_exact_match",
                    "Y",
                    $find_description_exact_match,
        ...
        <tr>
            ...
            <td nowrap><input type="text" name="find_results_1" value="<?= htmlspecialcharsbx($find_results_1) ?>"
                              size="5">&nbsp;<?= GetMessage("FORM_TILL") ?>&nbsp;<input type="text"
                                                                                        name="find_results_2"
        ...
        $oFilter->Buttons(
            array(
                "table_id" => $sTableID,
                "url" => "form_status_list.php?lang=" . LANGUAGE_ID . "&WEB_FORM_ID=$WEB_FORM_ID"
            )
        );
        $oFilter->End();
        ?>
