    ...
    if ($old_module_version == "Y"):
                ...
                if (CModule::IncludeModule("statistic")):?>
                    ...
                    if (intval($arrResult["STAT_GUEST_ID"]) > 0):?>
                        <tr>
                            ...
                            <td>[<a title="<?= GetMessage("FORM_GUEST_ALT") ?>"
                                    href="/bitrix/admin/guest_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $arrResult["STAT_GUEST_ID"] ?>&set_filter=Y"><?= $arrResult["STAT_GUEST_ID"] ?></a>]
                            </td>
                        </tr>
                    <?endif; ?>
                    <?
                    if (intval($arrResult["STAT_SESSION_ID"]) > 0):?>
                        <tr>
                            ...
                            <td>[<a title="<?= GetMessage("FORM_SESSION_ALT") ?>"
                                    href="/bitrix/admin/session_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $arrResult["STAT_SESSION_ID"] ?>&set_filter=Y"><?= $arrResult["STAT_SESSION_ID"] ?></a>]
                            </td>
                        </tr>
                    <?endif; ?>
                <?endif; ?>
        ...
        if ($F_RIGHT >= 25):?>
            <form name="form1" action="" method="GET">
                ...
                ?>&nbsp;<input <?
                if ($F_RIGHT < 30) echo "disabled" ?> type="submit" name="save" value="<?= GetMessage("FORM_SAVE") ?>">
            </form>
        <?endif; ?>
        <hr/><br/>
        <?
...
// *************************************** NORMAL FORM WITHOUT ARCHAISTIC PERVERSIONS ***********************
        ?>
        <form name="form1" action="/bitrix/admin/form_result_edit.php?lang=<?
        echo LANG ?>&WEB_FORM_ID=<?
        echo $WEB_FORM_ID ?><?
        if ($RESULT_ID > 0): echo '&RESULT_ID=' . $RESULT_ID; endif; ?>" method="POST" enctype="multipart/form-data">
            ...
            <tr class="heading">
                <td colspan="2"><?
                    echo GetMessage('FORM_RESULT_EDIT_COMMON') ?></td>
            </tr>
            <?
            if ($RESULT_ID > 0):
                ?>
            ...
            <tr>
                <td><?
                    echo GetMessage('FORM_RESULT_EDIT_FORM') ?>:
                </td>
                <td>
            ...
            <tr>
                ...
                <td>
                    ...
                    echo GetMessage('FORM_RESULT_EDIT_USER') ?>"
                        ...
                        (<?= htmlspecialcharsbx($arUser['LOGIN']) ?>)</a><?
                        if ($arrResult["RESULT_USER_AUTH"] == "N"): ?>&nbsp;<?
                            echo GetMessage('FORM_RESULT_EDIT_USER_NOTAUTH') ?><?endif; ?>
                    <?
                    else:
                        ?>
                        <?
            ...
            if ($RESULT_ID > 0):
                ...
                <tr>
                    <td><?
                        echo GetMessage('FORM_RESULT_EDIT_CREATED') ?>:
                    </td>
                    <td><?= $arrResult["DATE_CREATE"] ?></td>
                </tr>
                <tr>
                    <td><?
                        echo GetMessage('FORM_RESULT_EDIT_CHANGED') ?>:
                    </td>
                    <td><?= $arrResult["TIMESTAMP_X"] ?></td>
                </tr>
                <?
                if (IsModuleInstalled('statistic')) {
                    ...
                    <tr>
                        ...
                        <td>[<a title="<?= GetMessage("FORM_GUEST_ALT") ?>"
                                href="/bitrix/admin/guest_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $arrResult["STAT_GUEST_ID"] ?>&find_id_exact_match=Y&set_filter=Y"><?= $arrResult["STAT_GUEST_ID"] ?></a>]
                        </td>
                    </tr>
                    <tr>
                        <td><?= GetMessage("FORM_SESSION") ?></td>
                        <td>
                            [<a href="/bitrix/admin/session_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $arrResult["STAT_SESSION_ID"] ?>&find_id_exact_match=Y&set_filter=Y"><?= $arrResult["STAT_SESSION_ID"] ?></a>]
                        </td>
                    </tr>
                    <?
                }
            ...
            if ($EDIT_STATUS == 'Y'):
                ...
                <tr class="heading">
                    <td colspan="2"><?
                        echo GetMessage('FORM_RESULT_EDIT_STATUS') ?></td>
                </tr>
                <?
                if ($RESULT_ID > 0):
                    ?>
                    <tr>
                        <td valign="top"><?
                            echo GetMessage('FORM_RESULT_EDIT_STATUS_CURRENT') ?>:
                        </td>
                        <td><b><?
                                echo htmlspecialcharsEx($arrResult["STATUS_TITLE"]) ?></b></td>
                    </tr>
                <?
                endif;// ($RESULT_ID > 0):
                ?>
                <tr>
                    <td valign="top"><?
                        echo GetMessage('FORM_RESULT_EDIT_STATUS_' . ($RESULT_ID > 0 ? 'CHANGE' : 'SET')) ?>:
                    </td>
                    <td><?
                        echo $RESULT_STATUS_FORM ?></td>
                </tr>
            <?
            endif;// EDIT_STATUS=Y
            ...
            <tr class="heading">
                <td colspan="2"><?
                    echo GetMessage('FORM_RESULT_EDIT_FIELDS') ?></td>
            </tr>
            <?
            ...
            foreach ($arQuestions as $key => $arQuestion) {
                ...
                if ($arQuestion['ADDITIONAL'] == 'Y' && ($q++) == 0):
                    ...
                    <tr class="heading">
                        <td colspan="2"><?
                            echo GetMessage('FORM_RESULT_EDIT_FIELDS_ADDITIONAL') ?></td>
                    </tr>
                <?
                endif;
                ?>
