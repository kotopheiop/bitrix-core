...
if (intval($copy_id) > 0 && check_bitrix_sessid() && $F_RIGHT >= 30) {
    ...
    if ($strError == '' && intval($new_id) > 0) {
        LocalRedirect(
            "form_field_edit_simple.php?ID=" . $new_id . "&additional=" . $additional . "&WEB_FORM_ID=" . $WEB_FORM_ID . "&lang=" . LANGUAGE_ID . "&strError=" . urlencode(
                $strError
            )
        );
    }
...
if (($save <> '' || $apply <> '') && $REQUEST_METHOD == "POST" && $F_RIGHT >= 30 && check_bitrix_sessid()) {
    ...
    if (is_array($DELETE)) {
        ...
        foreach ($DELETE as $key => $val) {
            if ($val == "Y" || ($i > 0 && !in_array(
                        $FIELD_TYPE,
                        $arTypeList
                    ))) // if it's not a list kill all answers except first one
            {
                $arrA = array();
                $arrA["ID"] = $key;
                $arrA["DELETE"] = "Y";
    ...
    if ($res > 0) {
        ...
        if ($strError == '') {
            if ($save <> '') {
                ...
                LocalRedirect(
                    "form_field_edit_simple.php?ID=" . $ID . "&WEB_FORM_ID=" . $WEB_FORM_ID . "&additional=" . $additional . "&lang=" . LANGUAGE_ID . "&" . $tabControl->ActiveTabParam(
                    )
                );
            }
        }
...
?>
    ...
    <form name="form1" method="POST" action="" enctype="multipart/form-data" onSubmit="return FormSubmit();">
        ...
        <tr>
            ...
            <td><? echo CFile::InputFile("IMAGE_ID", 20, $str_IMAGE_ID); ?><? if ($str_IMAGE_ID > 0): ?>
                    <br><? echo CFile::ShowImage($str_IMAGE_ID, 200, 200, "border=0", "", true) ?><? endif; ?></td>
        </tr>
        <tr>
    ...
    <? else:?>
        ...
        <tr>
            <td align="center" colspan="2"><textarea name="TITLE" style="width:100%"
                                                     rows="5"><? echo $str_TITLE ?></textarea></td>
        </tr>
    <? endif; ?>
        <?
        //********************
        ...
        <script language="JavaScript">
            ...
            function RowInsert() {
                ...
                BX.adjust(oRow.insertCell(-1), {
                    html: '<div onclick="RowDelete(\'' + oRow.id + '\',0)" title="<?=GetMessage(
                        "FORM_ANS_DEL"
                    )?>" id="btn_delete" style="width:20px;height:20px;cursor:pointer"></div>',
        ...
        <tr>
            ...
            <td><select name="FIELD_TYPE" id="selected_type"
                        onchange="ShowSelected(); jsFormValidatorSettings.UpdateAll();">
                    <?
                    $arDisplay = array(
                        'text' => 'none',
                        'textarea' => 'none',
            ...
            <td>
                ...
                <div id="type_textarea">
                    <table class="internal">
                        ...
                        <tr>
                            ...
                            <td><input type="text" size="5" name="FIELD_HEIGHT" value="<?= $arRows[0]['HEIGHT'] ?>"/>
                            </td>
                        </tr>
                    </table>
                </div>
                <div id="type_list">
                    <table class="internal" id="tbl_answers">
                        ...
                        if (is_array($arRows)) {
                            foreach ($arRows AS $arRow):
                                ...
                                <tr id="row<?= $i ?>">
                                    <td><input type="hidden" name="ANSWER[]" value="<?= $arRow['ID'] ?>"/><input
                                                type="text" name="MESSAGE[]" value="<?= $arRow['MESSAGE'] ?>"/></td>
                                    <td><input id="row<?= $i ?>_sort" name="SORT[]" value="<?= $arRow['SORT'] ?>"
                                               type="text" size="5"/></td>
                                    <td><input name="DEF[]" type="hidden" value="<?= $arRow['DEF_HIDDEN'] ?>"
                                               id="row<?= $i ?>_def_hidden"/><input OnClick="ClearAll('row<?= $i ?>')"
                                                                                    type="checkbox" <?= $arRow['DEF'] ?>
                                                                                    id="row<?= $i ?>_def"
                                                                                    name="row<?= $i ?>_def"/></td>
                                    <td id="row<?= $i ?>_del" <? if (count(
                                            $arRows
                                        ) < 2) print 'style="visibility:hidden"' ?>>
                                        <div onclick="RowDelete('row<?= $i ?>',1)"
                                             title="<?= GetMessage("FORM_ANS_DEL") ?>" id="btn_delete"
                                             style="width:20px;height:20px;cursor:pointer;"></div>
                                    </td>
                                </tr>
                            <?
                            endforeach;
                            ...
                            <tr id="row1">
                                ...
                                <td><input name="DEF[]" type="hidden" id="row1_def_hidden"/><input
                                            OnClick="ClearAll('row1')" type="checkbox" id="row1_def" name="row1_def"/>
                                </td>
                                <td id="row1_del" style="visibility:hidden">
                                    <div onclick="RowDelete('row1',0)" title="<?= GetMessage("FORM_ANS_DEL") ?>"
                                         id="btn_delete" style="width:20px;height:20px;cursor:pointer;"></div>
                                </td>
                            </tr>
                            <?
                    ...
                    <table OnClick="RowInsert()" style="cursor:pointer" ;>
                        <tr>
                            <td>
                                <div title="<?= GetMessage("FORM_ANS_ADD") ?>" id="btn_new"
        ...
        if ($rsValidators->SelectedRowsCount() > 0) {
            ...
            <script language="javascript">
                ...
                arValidators['<?=CUtil::JSEscape($arValidatorInfo["NAME"])?>'] = {
                    ...
                    HAS_SETTINGS: '<?=is_callable($arValidatorInfo['SETTINGS']) ? "Y" : "N"?>'
                };
                <?
        ...
        ?>
        <tr>
            <td colspan="2" align="center"><textarea name="COMMENTS" cols="80"
                                                     rows="5"><? echo $str_COMMENTS ?></textarea></td>
        </tr>
        <?
        $tabControl->EndTab();
        $tabControl->Buttons(
            array(
                "disabled" => ($F_RIGHT < 30),
                "back_url" => "form_field_list.php?WEB_FORM_ID=" . $WEB_FORM_ID . "&lang=" . LANGUAGE_ID
            )
        );
        $tabControl->End();
        ?>
