...
$aTabs[] = array(
    "DIV" => "edit2",
    "TAB" => GetMessage("FORM_MAIL"),
    "ICON" => "form_edit",
    "TITLE" => GetMessage("FORM_MAIL_TITLE")
);
$aTabs[] = array(
    "DIV" => "edit3",
    "TAB" => GetMessage("FORM_ACCESS"),
    "ICON" => "form_edit",
    "TITLE" => GetMessage("FORM_ACCESS_TITLE")
);
...
if (intval($copy_id) > 0 && check_bitrix_sessid() && $F_RIGHT >= 30) {
    ...
    if ($strError == '' && intval($new_id) > 0) {
        LocalRedirect(
            "form_status_edit.php?ID=" . $new_id . "&WEB_FORM_ID=" . $WEB_FORM_ID . "&lang=" . LANGUAGE_ID . "&strError=" . urlencode(
                $strError
            )
        );
    }
    ...
    if ($res > 0) {
        ...
        if ($strError == '') {
            if ($save <> '') {
                ...
                LocalRedirect(
                    "form_status_edit.php?ID=" . $ID . "&WEB_FORM_ID=" . $WEB_FORM_ID . "&lang=" . LANGUAGE_ID . "&" . $tabControl->ActiveTabParam(
                    )
                );
            }
        }
...
if ($F_RIGHT >= 30 && $ID > 0) {
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('" . GetMessage(
                "FORM_CONFIRM_DELETE"
            ) . "'))window.location='form_status_list.php?action=delete&ID=" . $ID . "&WEB_FORM_ID=" . $WEB_FORM_ID . "&" . bitrix_sessid_get(
            ) . "&lang=" . LANGUAGE_ID . "';",
        "WARNING" => "Y"
    );
...
?>
    ...
    <form method="POST"
          action="<? echo $APPLICATION->GetCurPage() . "?lang=" . LANGUAGE_ID . "&WEB_FORM_ID=" . $WEB_FORM_ID ?>"
          name="form_status_edit">
        <?= bitrix_sessid_post() ?>
        ...
        <? if ($ID > 0) : ?>
            <tr>
                ...
                <td><a title="<?= GetMessage("FORM_RESULTS_TITLE") ?>"
                       href="form_result_list.php?lang=<?= LANGUAGE_ID ?>&WEB_FORM_ID=<?= $WEB_FORM_ID ?>&find_status=<? echo $ID ?>&set_filter=Y"><? echo intval(
                            $str_RESULTS
                        ) ?></a></td>
            </tr>
        <? endif; ?>
        ...
        <?
        $disabled = "disabled";
        if ($DEFAULT_STATUS_ID <= 0 || $DEFAULT_STATUS_ID == $ID) {
            $disabled = "";
        }
        ?>
        <tr>
        ...
        <script>
            ...
            function GenerateMailTemplate() {
                ...
                var url = '/bitrix/admin/form_status_mail.php?lang=<?=LANGUAGE_ID?>&<?=bitrix_sessid_get(
                )?>&WEB_FORM_ID=<?=$WEB_FORM_ID;?>&STATUS_ID=<?=intval($ID)?>';
            ...
            function DeleteMailTemplate(template_id) {
                ...
                if (confirm('<?echo CUtil::JSEscape(GetMessage('FORM_CONFIRM_DEL_MAIL_TEMPLATE'))?>')) {
                    ...
                    var url = '/bitrix/admin/form_status_mail.php?action=delete&ID=' + template_id + '&lang=<?echo LANGUAGE_ID?>&<?=bitrix_sessid_get(
                ...
                foreach ($arrMAIL as $mail_id => $mail_name):
                    ...
                    <tr id="ft_<?= htmlspecialcharsbx($mail_id) ?>">
                        <td nowrap style="padding:0px"><input type="checkbox" name="arMAIL_TEMPLATE[]"
                                    ...
                                    $mail_id
                                ) . "&lang=" . LANGUAGE_ID . "'>" . htmlspecialcharsbx($mail_id) . "</a>]"; ?>
                            &nbsp;<label for="<?= htmlspecialcharsbx($mail_id) ?>"><?= htmlspecialcharsbx(
                                    $mail_name
                                ) ?></label></td>
                ...
                <table cellpadding="0" cellspacing="0">
                    <tr>
                        <td colspan=2 style="padding:0px"><? if (count($arrMAIL) > 0) echo "<br>" ?>&nbsp;[&nbsp;<a
                                    title="<?= GetMessage("FORM_GENERATE_TEMPLATE_ALT") ?>"
                                    onClick="GenerateMailTemplate()" href="javascript:void(0)"><? echo GetMessage(
                                    "FORM_CREATE_S"
                                ) ?></a>&nbsp;]<?
                            if (count($arrMAIL) > 0):
                                ?>&nbsp;&nbsp;&nbsp;[&nbsp;<a
                                    href="/bitrix/admin/message_admin.php?find_type_id=FORM_STATUS_CHANGE_<?= $arForm['SID'] ?>_<?= $ID ?>&set_filter=Y"><? echo GetMessage(
        ...
        ?>
        <tr>
            <td><?= GetMessage('FORM_MOVE_RIGHTS') . " [MOVE]"; ?><br><img src="/bitrix/images/form/mouse.gif"
                                                                           width="44" height="21" border=0 alt=""></td>
            <td><?
                echo SelectBoxMFromArray(
                    "arPERMISSION_MOVE[]",
                    array("REFERENCE" => $arr_ref, "REFERENCE_ID" => $arr_ref_id),
                    $arPERMISSION_MOVE,
                    "",
                    false,
                    8,
        ...
        <tr>
            <td><? echo GetMessage('FORM_VIEW_RIGHTS') . " [VIEW]" ?><br><img src="/bitrix/images/form/mouse.gif"
        ...
        <tr>
            <td><? echo GetMessage('FORM_EDIT_RIGHTS') . " [EDIT]" ?><br><img src="/bitrix/images/form/mouse.gif"
                                                                              width="44" height="21" border=0 alt="">
            </td>
            <td><?
                echo SelectBoxMFromArray(
        ...
        <tr>
            <td><? echo GetMessage('FORM_DELETE_RIGHTS') . " [DELETE]" ?><br><img src="/bitrix/images/form/mouse.gif"
