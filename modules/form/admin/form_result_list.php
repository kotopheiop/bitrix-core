...
if ($lAdmin->EditAction() && /*$FORM_RIGHT>="W"*/
    $F_RIGHT >= 20 && check_bitrix_sessid()) {
...
if (($arID = $lAdmin->GroupAction()) && /*$FORM_RIGHT>="W"*/
    $F_RIGHT >= 20 && check_bitrix_sessid()) {
...
if ($TABLE_RESULT_TEMPLATE <> '' && file_exists($_SERVER["DOCUMENT_ROOT"] . $TABLE_RESULT_TEMPLATE)) // use custom tpl
{
    ...
    if (!$bSimple) {
        $headers[] = array(
            "id" => "STATUS_ID",
            "content" => GetMessage("FORM_STATUS"),
            "sort" => "s_status",
            "default" => true
        );
    }
    ...
    if (is_array($arFormCrmLink)) {
        $headers[] = array(
            "id" => "SENT_TO_CRM",
            "content" => GetMessage("FORM_SENT_TO_CRM"),
            "sort" => "s_sent_to_crm",
            "default" => true
        );
    }
    ...
    $headers[] = array(
        "id" => "DATE_CREATE",
        "content" => GetMessage("FORM_DATE_CREATE"),
        "sort" => "s_date_create",
        "default" => false
    );
    $headers[] = array(
        "id" => "TIMESTAMP_X",
        "content" => GetMessage("FORM_TIMESTAMP"),
        "sort" => "s_timestamp",
        "default" => true
    );
    $headers[] = array(
        "id" => "USER_ID",
        "content" => GetMessage("FORM_USER"),
        "sort" => "s_user_id",
        "default" => true
    );
    ...
    if (CModule::IncludeModule("statistic")) {
        $headers[] = array(
            "id" => "STAT_GUEST_ID",
            "content" => GetMessage("FORM_GUEST_ID"),
            "sort" => "s_guest_id",
            "default" => true
        );
        $headers[] = array(
            "id" => "STAT_SESSION_ID",
            "content" => GetMessage("FORM_SESSION_ID"),
            "sort" => "s_session_id",
            "default" => true
        );
    }
    ...
    while ($arRes = $result->NavNext(true, "f_")) {
        ...
        foreach ($arrAnswers[$f_ID] as $arFieldValues) {
            if ($f_USER_ID > 0) {
                ...
                $txt = "[<a title='" . GetMessage(
                        "FORM_EDIT_USER"
                    ) . "' href=\"user_edit.php?lang=" . LANGUAGE_ID . "&ID=" . $f_USER_ID . "\">" . $f_USER_ID . "</a>] (" . htmlspecialcharsbx(
                        $f_LOGIN
                    ) . ") " . htmlspecialcharsbx($f_USER_NAME);
                $txt .= ($f_USER_AUTH == "N") ? GetMessage("FORM_NOT_AUTH") : "";
            } else {
                $txt = GetMessage("FORM_NOT_REGISTERED");
            }

            $row->AddViewField("USER_ID", $txt);
            unset($txt);
            ...
            if (CModule::IncludeModule("statistic")):
                if (intval($f_STAT_GUEST_ID) > 0) :
                    $row->AddViewField(
                        ...
                        " [<a title='" . GetMessage(
                            "FORM_GUEST_TITLE"
                        ) . "' href='/bitrix/admin/guest_list.php?lang=" . LANGUAGE_ID . "&find_id=" . $f_STAT_GUEST_ID . "&set_filter=Y'>" . $f_STAT_GUEST_ID . "</a>]"
                    );
                endif;
                if (intval($f_STAT_SESSION_ID) > 0) :
                    $row->AddViewField(
                        ...
                        " (<a title='" . GetMessage(
                            "FORM_SESSION_TITLE"
                        ) . "' href='/bitrix/admin/session_list.php?lang=" . LANGUAGE_ID . "&find_id=" . $f_STAT_SESSION_ID . "&set_filter=Y'>" . $f_STAT_SESSION_ID . "</a>)"
                    );
                endif;
            endif;
            ...
            if ($_REQUEST["mode"] == "excel") {
                $row->AddViewField(
                    'SENT_TO_CRM',
                    $f_SENT_TO_CRM == 'N' ? GetMessage('MAIN_NO') : GetMessage('MAIN_YES')
                );
            } else {
                $row->AddViewField(
                    'SENT_TO_CRM',
                    $f_SENT_TO_CRM == 'N' ? '<img src="/bitrix/images/form/red.gif" height="14" width="14" border="0" id="crm_sent_' . $f_ID . '" />' : '<img src="/bitrix/images/form/green.gif" height="14" width="14" border="0" id="crm_sent_' . $f_ID . '" />'
                );
            }
            ...
            foreach ($arFieldValues as $arrA) {
                if (trim($arrA["USER_DATE"]) <> '') {
                    // hack
                    $txt .= ((MakeTimeStamp($arrA["USER_TEXT"]) + date(
                                'Z'
                            )) % 86400 == 0 ? $arrA['USER_TEXT'] : $arrA['USER_DATE']) . "<br>";
                ...
                if (intval($arrA["USER_FILE_ID"]) > 0) {
                    if ($arrA["USER_FILE_IS_IMAGE"] == "Y") :
                        ...
                        $txt .= "<a title=\"" . GetMessage(
                                "FORM_VIEW_FILE"
                            ) . "\" href=\"/bitrix/tools/form_show_file.php?rid=$f_ID&hash=$arrA[USER_FILE_HASH]&lang=" . LANGUAGE_ID . "\">" . htmlspecialcharsbx(
                                $arrA["USER_FILE_NAME"]
                            ) . "</a><br>(";
                        $txt .= CFile::FormatSize($arrA["USER_FILE_SIZE"]);
                        $txt .= ")<br>[&nbsp;<a title=\"" . str_replace(
                                "#FILE_NAME#",
                                $arrA["USER_FILE_NAME"],
                                GetMessage("FORM_DOWNLOAD_FILE")
                            ) . "\" href=\"/bitrix/tools/form_show_file.php?rid=$f_ID&hash=$arrA[USER_FILE_HASH]&lang=" . LANGUAGE_ID . "&action=download\">" . GetMessage(
                                "FORM_DOWNLOAD"
                            ) . "</a>&nbsp;]";
                    endif;
                }
        ...
        if ($F_RIGHT >= 20 || ($F_RIGHT >= 15 && $USER_ID == $f_USER_ID)) {
            if (in_array("EDIT", $arrRESULT_PERMISSION) || in_array("VIEW", $arrRESULT_PERMISSION)) {
                $arActions[] = array(
                    ...
                    "ACTION" => $lAdmin->ActionRedirect(
                        "form_result_edit.php?lang=" . LANGUAGE_ID . "&WEB_FORM_ID=$WEB_FORM_ID&RESULT_ID=$f_ID&WEB_FORM_NAME=$WEB_FORM_NAME"
                    ),
                    'DEFAULT' => 'Y'
                );
            }
            if (is_array($arFormCrmLink)) {
                $arActions[] = array(
                    "ICON" => "copy",
                    "TITLE" => GetMessage('FORM_SEND_TO_CRM_TITLE'),
                    "TEXT" => GetMessage('FORM_SEND_TO_CRM'),
                    "ACTION" => "sendToCrm('" . $WEB_FORM_ID . "', '" . $f_ID . "')"
                );
            }
            ...
            if (in_array("DELETE", $arrRESULT_PERMISSION)) {
                $arActions[] = array(
                    "ICON" => "delete",
                    "TITLE" => GetMessage("FORM_DELETE_ALT"),
                    "TEXT" => GetMessage("FORM_DELETE"),
                    "ACTION" => "if(confirm('" . GetMessage("FORM_CONFIRM_DELETE") . "')) " . $lAdmin->ActionDoGroup(
                            $f_ID,
                            "delete",
                            "&WEB_FORM_ID=" . $WEB_FORM_ID . "&WEB_FORM_NAME=" . $WEB_FORM_NAME
                        )
                );
            }
        }
...
if ($FILTER_RESULT_TEMPLATE <> '' && file_exists($_SERVER["DOCUMENT_ROOT"] . $FILTER_RESULT_TEMPLATE)) {
    ...
    <form name="form1" method="GET" action="<?= $APPLICATION->GetCurPage() ?>?">
        ...
        <tr>
            <td width="0%" nowrap><? echo GetMessage("FORM_F_DATE_CREATE") . " (" . CSite::GetDateFormat(
                        "SHORT"
                    ) . "):" ?></td>
            <td width="0%" nowrap><?= CForm::GetDateFilter(
                    "date_create",
                    "form1",
                    "Y",
                    "class=\"typeselect\"",
        ...
        <tr>
            <td width="0%" nowrap><? echo GetMessage("FORM_F_TIMESTAMP") . " (" . CSite::GetDateFormat(
                        "SHORT"
                    ) . "):" ?></td>
            <td width="0%" nowrap><?= CForm::GetDateFilter(
                    "timestamp",
        ...
        $oFilter->Buttons(
            array(
                "table_id" => $sTableID,
                "url" => $APPLICATION->GetCurPage() . "?lang=" . LANGUAGE_ID . "&WEB_FORM_ID=$WEB_FORM_ID"
            )
        );
        $oFilter->End();
        ?>
...
if (is_array($arFormCrmLink)):
    ...
    <script type="text/javascript">
        function sendToCrm(FORM_ID, RESULT_ID) {
            BX.ajax.loadJSON('/bitrix/admin/form_crm.php?action=add_lead&FORM_ID=<?=$WEB_FORM_ID?>&RESULT_ID=' + parseInt(RESULT_ID) + '&<?=bitrix_sessid_get(
            )?>', function (result) {
                if (result.result == 'ok') {
                    ...
                    alert('<?=CUtil::JSEscape(
                        GetMessage('FORM_SEND_TO_CRM_ERROR')
...
if ($custom_table) {
    require_once($_SERVER["DOCUMENT_ROOT"] . $TABLE_RESULT_TEMPLATE);
}
