...
if ($additional == "Y") {
    $sess_filter = "FORM_ADDITIONAL_FIELD_LIST";
} else {
    $sess_filter = "FORM_FIELD_LIST";
}
    ...
    foreach ($FIELDS as $ID => $arFields) {
        ...
        foreach ($arFlds as $key) {
            if (is_set($arFields, $key)) {
                $arFieldsStore[$key] = "'" . ($key == "C_SORT" ? intval($arFields[$key]) : $DB->ForSql(
                        $arFields[$key]
                    )) . "'";
            }
        }
...
if (($arID = $lAdmin->GroupAction()) && $FORM_RIGHT == "W" && $F_RIGHT >= 30 && check_bitrix_sessid()) {
    ...
    if (!$_REQUEST["mode"]) {
        LocalRedirect(
            "form_field_list.php?lang=" . LANGUAGE_ID . "&WEB_FORM_ID=" . $WEB_FORM_ID . "&additional=" . $additional
        );
    }
}
...
if ($additional != 'Y') {
    $headers[] = array(
        "id" => "REQUIRED",
        "content" => GetMessage("FORM_REQUIRED"),
        "sort" => "s_required",
        "default" => true
    );
}
...
if ($additional == "Y") {
    $headers[] = array(
        "id" => "TITLE",
        "content" => GetMessage("FORM_ADDITIONAL_TITLE"),
        "sort" => "s_title",
        "default" => true
    );
    $headers[] = array(
        "id" => "FIELD_TYPE",
        "content" => GetMessage("FORM_FIELD_TYPE"),
        "sort" => "s_field_type",
        "default" => true
    );
} else {
...
$headers[] = array(
    "id" => "COMMENTS",
    "content" => GetMessage("FORM_COMMENTS"),
    "sort" => "s_comments",
    "default" => true
);
...
if (!$bSimple) {
    $headers[] = array(
        "id" => "IN_RESULTS_TABLE",
        "content" => GetMessage("FORM_IN_RESULTS_TABLE"),
        "sort" => "s_in_results_table",
        "default" => true
    );
    $headers[] = array(
        "id" => "IN_EXCEL_TABLE",
        "content" => GetMessage("FORM_IN_EXCEL_TABLE"),
        "sort" => "s_in_excel_table",
        "default" => true
    );
}
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    $row->AddViewField(
        "SID",
        "<a href=\"form_field_edit.php?lang=" . LANGUAGE_ID . "&ID=" . $f_ID . "&WEB_FORM_ID=" . $WEB_FORM_ID . "&additional=" . $additional . "\" title=\"" . $msg . "\">" . htmlspecialcharsbx(
            $f_SID
        ) . "</a>"
    );
    ...
    if ($bSimple) {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect(
                "form_field_edit_simple.php?lang=" . LANGUAGE_ID . "&ID=$f_ID&WEB_FORM_ID=$WEB_FORM_ID&additional=$additional"
            ),
        ...
        $arActions[] = array(
            ...
            "TITLE" => GetMessage("FORM_EDIT_ALT"),
            "ACTION" => $lAdmin->ActionRedirect(
                "form_field_edit.php?lang=" . LANGUAGE_ID . "&ID=$f_ID&WEB_FORM_ID=$WEB_FORM_ID&additional=$additional"
            ),
            "TEXT" => GetMessage("FORM_FIELD_EDIT")
        );
    }
    ...
    if ($F_RIGHT >= 30) {
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect(
                "form_field_list.php?copy_id=$f_ID&lang=" . LANGUAGE_ID . "&WEB_FORM_ID=$WEB_FORM_ID&additional=$additional&" . bitrix_sessid_get(
                )
            ),
        ...
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . GetMessageJS("FORM_CONFIRM_DELETE") . "')) " . $lAdmin->ActionDoGroup(
                    $f_ID,
                    "delete",
                    "&WEB_FORM_ID=" . $WEB_FORM_ID . "&additional=$additional"
                ),
...
?>
    <form name="form1" method="GET" action="<?= $APPLICATION->GetCurPage() ?>?">
        ...
        <tr>
            <td><b><? echo ($additional == "Y") ? GetMessage("FORM_ADDITIONAL_TITLE") : GetMessage("FORM_TITLE") ?>:</b>
            </td>
            <td><input type="text" name="find_title" size="47"
                       value="<? echo htmlspecialcharsbx($find_title) ?>"><?= InputType(
                    "checkbox",
                    "find_title_exact_match",
                    "Y",
                    $find_title_exact_match,
            ...
            <tr>
                ...
                <td nowrap><?
                    ...
                    echo SelectBoxFromArray(
                        "find_required",
                        $arr,
                        htmlspecialcharsbx($find_required),
                        GetMessage("FORM_ALL")
                    );
                    ?></td>
            </tr>
        ...
        <tr>
            ...
            <td nowrap><?
                ...
                echo SelectBoxFromArray(
                    "find_in_table",
                    $arr,
                    htmlspecialcharsbx($find_in_table),
                    GetMessage("FORM_ALL")
                );
                ?></td>
        </tr>
        <tr>
            ...
            <td nowrap><?
                ...
                echo SelectBoxFromArray(
                    "find_in_excel",
                    $arr,
                    htmlspecialcharsbx($find_in_excel),
                    GetMessage("FORM_ALL")
                );
                ?></td>
        </tr>
        <tr>
            ...
            <td nowrap><?
                ...
                echo SelectBoxFromArray(
                    "find_in_filter",
                    $arr,
                    htmlspecialcharsbx($find_in_filter),
                    GetMessage("FORM_ALL")
                );
                ?></td>
        </tr>
        ...
        $oFilter->Buttons(
            array(
                ...
                "url" => ($APPLICATION->GetCurPage(
                    )) . "?lang=" . LANGUAGE_ID . "&additional=$additional&WEB_FORM_ID=$WEB_FORM_ID"
            )
        );
        $oFilter->End();
        #############################################################
