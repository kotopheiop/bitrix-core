...
class CForm_old
{
    public static function GetFileValue($RESULT_ID, $ANSWER_ID)
    {
        ...
        $strSql = "SELECT USER_FILE_ID FROM b_form_result_answer WHERE RESULT_ID='" . intval(
                $RESULT_ID
            ) . "' and ANSWER_ID='" . intval($ANSWER_ID) . "'";
    ...
    public static function Show($WEB_FORM_VARNAME, $arrVALUES = false, $SHOW_TEMPLATE = false, $PREVIEW = "N")
    {
        ...
        if (intval($WEB_FORM_ID) > 0) {
            ...
            if (intval($F_RIGHT) >= 10) {
                if (trim($SHOW_TEMPLATE) <> '') {
                    $template = $SHOW_TEMPLATE;
                } else {
                    if (trim($arForm["SHOW_TEMPLATE"]) == '') {
                        $template = "default.php";
                    } else {
                        $template = $arForm["SHOW_TEMPLATE"];
                    }
                }
