...
class CFormResult_old
{
    ...
    public static function Edit($RESULT_ID, $arrVALUES, $TEMPLATE = "", $EDIT_ADDITIONAL = "N", $EDIT_STATUS = "N")
    {
        ...
        if ($zr = $z->Fetch()) {
            ...
            $WEB_FORM_ID = $FORM_ID = CForm::GetDataByID(
                $arrResult["FORM_ID"],
                $arForm,
                $arQuestions,
                $arAnswers,
            ...
            if ($F_RIGHT >= 20 || ($F_RIGHT >= 15 && $arrResult["USER_ID"] == $USER->GetID())) {
                ...
                if (in_array("EDIT", $arrRESULT_PERMISSION)) {
                    if (trim($TEMPLATE) <> '') {
                        ...
                        if ($arrResult["EDIT_RESULT_TEMPLATE"] == '') {
                            $template = "default.php";
                        } else {
                            $template = $arrResult["EDIT_RESULT_TEMPLATE"];
                        }
                    }
                ...
                if (in_array("VIEW", $arrRESULT_PERMISSION)) {
                    if (trim($TEMPLATE) <> '') {
                        ...
                        if ($TEMPLATE_TYPE == "show") {
                            if ($arrResult["SHOW_RESULT_TEMPLATE"] == '') {
                                $template = "default.php";
                            } else {
                                $template = $arrResult["SHOW_RESULT_TEMPLATE"];
                            }
                        } elseif ($TEMPLATE_TYPE == "print") {
                            if ($arrResult["PRINT_RESULT_TEMPLATE"] == '') {
                                $template = "default.php";
                            } else {
