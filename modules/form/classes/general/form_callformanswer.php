...
class CAllFormAnswer
{
    ...
    public static function Copy($ID, $NEW_QUESTION_ID = false)
    {
        ...
        if ($arAnswer = $rsAnswer->Fetch()) {
            ...
            $strError .= GetMessage("FORM_ERROR_ANSWER_NOT_FOUND") . "<br>";
        }
        return false;
    }
    ...
    public static function Delete($ID, $QUESTION_ID = false)
    {
        ...
        if (intval($QUESTION_ID) > 0) {
            $str = " FIELD_ID = " . intval($QUESTION_ID) . " and ";
        }
        $DB->Query(
            "DELETE FROM b_form_result_answer WHERE " . $str . " ANSWER_ID='" . $ID . "'",
            false,
            $err_mess . __LINE__
        );
        return true;
    ...
    public static function CheckFields($arFields, $ANSWER_ID = false)
    {
        ...
        if ($ANSWER_ID <= 0 || ($ANSWER_ID > 0 && is_set($arFields, "MESSAGE"))) {
            if ($arFields["MESSAGE"] == '') {
                $str .= GetMessage("FORM_ERROR_FORGOT_ANSWER_TEXT") . "<br>";
            }
        }
