...
class CAllFormField
{
            ...
            if ($RIGHT_OK == "Y") {
                if (!$NEW_FORM_ID) {
                    while (true) {
                        ...
                        $z = $DB->Query($strSql, false, $err_mess . __LINE__);
                        if (!($zr = $z->Fetch())) {
                            $strSql = "SELECT 'x' FROM b_form_field WHERE SID='" . $DB->ForSql(
                                    $SID,
                                    50
                                ) . "' AND FORM_ID='" . $arField["FORM_ID"] . "'";
                ...
                $z = CFormField::GetFilterList(
                    $arField["FORM_ID"],
                    Array("FIELD_ID" => $ID, "FIELD_ID_EXACT_MATCH" => "Y")
                );
                while ($zr = $z->Fetch()) {
    ...
    public static function Reset($ID, $CHECK_RIGHTS = "Y")
    {
        ...
        if ($arField = $rsField->Fetch()) {
            ...
            if ($F_RIGHT >= 30) {
                $DB->Query(
                    "DELETE FROM b_form_result_answer WHERE FIELD_ID='" . $ID . "'",
                    false,
                    $err_mess . __LINE__
                );
                ...
                if ($FIELD_ID <= 0 || ($FIELD_ID > 0 && is_set($arFields, "SID"))) {
                    ...
                    if (preg_match("/[^A-Za-z_01-9]/", $arFields["SID"])) {
                        ...
                        $strSql = "SELECT ID, ADDITIONAL FROM b_form_field WHERE SID='" . $DB->ForSql(
                                trim($arFields["SID"]),
                                50
                            ) . "' and ID<>'" . $FIELD_ID . "' AND FORM_ID='" . $DB->ForSql($arFields["FORM_ID"]) . "'";
                        $z = $DB->Query($strSql, false, $err_mess . __LINE__);
                        if ($zr = $z->Fetch()) {
                            $s = ($zr["ADDITIONAL"] == "Y") ?
                                str_replace(
