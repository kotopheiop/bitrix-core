...
//				'DESCRIPTION'	=>	'description',
    ...
    {
        $channel = $this->syncTransport->openCalendarListChannel(
            $this->makeChannelParams($name, self::CONNECTION_CHANNEL_TYPE)
        );
    ...
    public function saveEvent($eventData, $calendarId, $parameters = [])
    {
        ...
        $params['gEventId'] = $eventData['G_EVENT_ID'] || !strpos(
            $eventData['DAV_XML_ID'],
            '@google.com'
        ) ? $eventData['G_EVENT_ID'] : str_replace('@google.com', '', $eventData['DAV_XML_ID']);
    ...
    private function prepareEvent($event)
    {
        ...
        if (!empty($event['start']['dateTime']) && !empty($event['end']['dateTime'])) {
            $returnData['TZ_FROM'] = Util::isTimezoneValid(
                $event['start']['timeZone']
            ) ? $event['start']['timeZone'] : $this->defaultTimezone;
            $returnData['TZ_TO'] = Util::isTimezoneValid(
                $event['end']['timeZone']
            ) ? $event['end']['timeZone'] : $this->defaultTimezone;

            $eventStartDateTime = new Type\DateTime(
                $event['start']['dateTime'],
                \DateTime::RFC3339,
                new \DateTimeZone($this->defaultTimezone)
            );
            $returnData['DATE_FROM'] = \CCalendar::Date(
                \CCalendar::Timestamp(
                    $eventStartDateTime->setTimeZone(new \DateTimeZone($returnData['TZ_FROM']))->format(
            ...
            $returnData['DATE_TO'] = \CCalendar::Date(
                \CCalendar::Timestamp(
                    $eventStartDateTime->setTimeZone(new \DateTimeZone($returnData['TZ_TO']))->format(
                        Type\Date::convertFormatToPhp(FORMAT_DATETIME)
                    )
                )
            );
                ...
                if (preg_match('/(RRULE)/', $recurrence)) {
                    ...
                    foreach ($rRuleList as $rRuleElement) {
                        ...
                        switch ($rRuleProp) {
                            ...
                            case 'UNTIL':
                                try {
                                    $rRuleValDateTime = new Type\DateTime(
                                        $rRuleVal,
                                        'Ymd\THis\Z',
                                    ...
                                    $rRuleSet['UNTIL'] = \CCalendar::Date(
                                        \CCalendar::Timestamp(
                                            $rRuleValDateTime->format(Type\Date::convertFormatToPhp(FORMAT_DATETIME))
                                        ) - 60,
                                        false,
                                        false
                                    );
        ...
        if (!empty($event['recurringEventId'])) {
            ...
            if ($event['status'] == 'cancelled') {
                $exDates[] = date(
                    'd.m.Y',
                    strtotime(
                        !empty($event['originalStartTime']['dateTime']) ? $event['originalStartTime']['dateTime'] : $event['originalStartTime']['date']
                    )
                );
                ...
                if (!empty($event['originalStartTime']['dateTime'])) {
                    $originalTimeZone = Util::isTimezoneValid(
                        $event['originalStartTime']['timeZone']
                    ) ? $event['originalStartTime']['timeZone'] : $returnData['TZ_FROM'];
                    $eventOriginalDateFrom = new Type\DateTime(
                        $event['originalStartTime']['dateTime'],
                        \DateTime::RFC3339,
                        new \DateTimeZone($this->defaultTimezone)
                    );
                    $returnData['ORIGINAL_DATE_FROM'] = \CCalendar::Date(
                        \CCalendar::Timestamp(
                            $eventOriginalDateFrom->setTimeZone(new \DateTimeZone($originalTimeZone))->format(
                                Type\Date::convertFormatToPhp(FORMAT_DATETIME)
                            )
                        )
                    );
                ...
                if (!empty($event['originalStartTime']['date'])) {
                    $returnData['ORIGINAL_DATE_FROM'] = \CCalendar::Date(
                        strtotime($event['originalStartTime']['date']),
                        false
                    );
                }
            }
    ...
    private function prepareToSaveEvent($eventData, $params = null)
    {
        ...
        if (isset($eventData['ATTENDEES_CODES']) && count($eventData['ATTENDEES_CODES']) > 1) {
            ...
            $newEvent['description'] = Loc::getMessage('ATTENDEES_EVENT') . ': ' . implode(
                    ', ',
                    $users
        ...
        if (!empty($eventData['REMIND'])) {
            ...
            foreach ($eventData['REMIND'] as $remindRule) {
                ...
                if ($remindRule['type'] == 'min') {
                    ...
                    $tz = Util::isTimezoneValid($eventData['TZ_FROM']) ? $this->prepareTimezone(
                        $eventData['TZ_FROM']
                    ) : $this->defaultTimezone;
                    $dateFrom = new Type\DateTime(
                        $eventData['DATE_FROM'],
                        Type\Date::convertFormatToPhp(FORMAT_DATETIME),
                        $tz
                    );
                    $remind = new Type\DateTime(
                        $remindRule['value'],
                        Type\Date::convertFormatToPhp(FORMAT_DATETIME),
                        $tz
                    );
        ...
        if ($eventData['DT_SKIP_TIME'] == "Y") {
            ...
            $eventEndDateTime = new Type\DateTime(
                $eventData['DATE_TO'],
                Type\Date::convertFormatToPhp(FORMAT_DATETIME),
                $dateTimeZoneTo
            );
...
//						$exDateStr = 'EXDATE;TZID=UTC:' . date("Ymd", strtotime($exDate)) . Type\DateTime::createFromUserTime($eventData['DATE_FROM'])->setTimeZone(new \DateTimeZone('UTC'))->format('\\THis\\Z');
                        $exDateStr = 'EXDATE;TZID=UTC:' . date("Ymd", strtotime($exDate)) . (new Type\DateTime(
                                $eventData['DATE_FROM'],
                                Type\Date::convertFormatToPhp(FORMAT_DATETIME),
                                $dateTimeZoneFrom
        ...
        if (isset($eventData['ORIGINAL_DATE_FROM'])) {
            ...
            $eventOriginalStart = new Type\DateTime(
                $eventData['ORIGINAL_DATE_FROM'],
                Type\Date::convertFormatToPhp(FORMAT_DATETIME),
                $instanceOriginalTimeZone
            );
    ...
    private function sendToSaveEvent($newEvent, $params)
    {
        if ($params['editInstance'] === true) {
            ...
            if (is_array($instance['items'])) {
                $externalEvent = $this->syncTransport->updateEvent(
                    $newEvent,
                    urlencode($params['calendarId']),
                    $instance['items'][0]['id']
                );
            }
            ...
            if (!empty($params['gEventId'])) {
                $externalEvent = $this->syncTransport->patchEvent(
                    $newEvent,
                    urlencode($params['calendarId']),
                    $params['gEventId']
                );
    ...
    public static function getChannelOwner(string $channelId = null): ?int
    {
        ...
        $matches = [];
        preg_match(
            '/(' . self::CONNECTION_CHANNEL_TYPE . '|' . self::SECTION_CHANNEL_TYPE . ')_(\d+)_.+/',
            $channelId,
            $matches
        );
    ...
    private function setSyncSettings(array $response = null): void
    {
        ...
        $this->defaultTimezone = Util::isTimezoneValid(
            $response['timeZone']
        ) ? $response['timeZone'] : $this->defaultTimezone;
    }
}
