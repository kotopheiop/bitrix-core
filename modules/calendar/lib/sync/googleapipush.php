...
final class GoogleApiPush
{
    ...
    public static function renewWatchChannels()
    {
        global $DB;
        $result = $DB->query(
            "SELECT * FROM b_calendar_push WHERE " .
        ...
        if (!empty($pushRows)) {
            ...
            if (!empty($sectionIds)) {
                $sectionResult = $DB->query(
                    "SELECT * FROM b_calendar_section WHERE ID IN (" . implode(',', $sectionIds) . ")"
                );
            ...
            if (!empty($connectionIds)) {
                $connectionResult = $DB->query(
                    "SELECT * FROM b_dav_connections WHERE ID IN (" . implode(',', $connectionIds) . ")"
                );
        ...
        if (count($pushRows) < 4) {
            ...
            $row = $result->fetch();
            if ($row) {
                $nextAgentDate = \Bitrix\Main\Type\DateTime::createFromTimestamp(
                    strtotime($row['EXPIRES']) - (60 * 60 * 20)
                )->format(\Bitrix\Main\Type\Date::convertFormatToPhp(FORMAT_DATETIME));
            } else {
                $nextAgentDate = \Bitrix\Main\Type\DateTime::createFromTimestamp(
                    strtotime('now') + (60 * 60 * 20)
                )->format(\Bitrix\Main\Type\Date::convertFormatToPhp(FORMAT_DATETIME));
            }
    ...
    public static function checkSectionsPush($localSections, $userId, $connectionId)
    {
        ...
        while ($row = $pushChannels->fetch()) {
            ...
            if ($localCalendarIndex !== false) {
                $channelInfo = $googleApiConnection->startWatchCalendarList(
                    $localSections[$localCalendarIndex]['GAPI_CALENDAR_ID']
                );
        ...
        if (Loader::includeModule('dav')) {
                    ...
                    if (empty($pushChannels[$davConnection['ID']])) {
                        ...
                        if ($channelInfo && isset($channelInfo['id'], $channelInfo['resourceId'])) {
                            self::deletePushChannel(
                                ["ENTITY_TYPE" => 'CONNECTION', 'ENTITY_ID' => $davConnection['ID']]
                            );
                            $googleApiConnection->updateSuccessLastResultConnection();
                            PushTable::add(
                                [
            ...
            if ($lastId == $start) {
                ...
                \CAgent::addAgent(
                    ...
                    "",
                    "Y",
                    Type\DateTime::createFromTimestamp(strtotime('+1 hour'))->format(
    ...
    public static function stopChannel($row = array(), $ownerId = 0)
    {
        if ($row) {
            if ($row['ENTITY_TYPE'] == 'CONNECTION') {
                ...
                if ($ownerId > 0 && isset($connectionData) && !self::isConnectionError(
                        $connectionData['LAST_RESULT']
                    )) {
    ...
    {
        if (!empty($error) && preg_match(
                "^\[404\] Channel \'[a-z0-9 _]*\' not found for project \'[a-z0-9 _]*\'",
                $error
            )) {
            return true;
        }
        ...
        if (!empty($sectionIds)) {
            ...
            if (!empty($missedChannelSections)) {
                foreach ($missedChannelSections as $missedSection) {
                    ...
                    $googleApiConnection = new GoogleApiSync($connectionData['ENTITY_ID'], $connectionData['ID']);
                    $channelInfo = $googleApiConnection->startWatchEventsChannel(
                        $sections[$missedSection]['GAPI_CALENDAR_ID']
                    );
                    if ($channelInfo && isset($channelInfo['id'], $channelInfo['resourceId'])) {
                        $googleApiConnection->updateSuccessLastResultConnection();
                        PushTable::add(
