...
class Helper
{
            ...
            case 'WEEKLY':
                ...
                $res = (int)$rrule['INTERVAL'] === 1
                    ...
                    : Loc::getMessage(
                        ...
                        ['#WEEK#' => $rrule['INTERVAL'], '#DAYS_LIST#' => $daysList]
                    );
                break;
            case 'MONTHLY':
                $res = (int)$rrule['INTERVAL'] === 1
    ...
    public static function getPubEventLink(int $eventId, int $userId, int $dateCreateTimestamp): string
    {
        ...
        return "{$scheme}://{$domain}{$port}/pub/calendar-event/{$eventId}/" . self::getHashForPubEvent(
                $eventId,
                $userId,
                $dateCreateTimestamp
            ) . "/";
     ...
     * @return string
     */
    public static function getPubEventLinkWithParameters(
        int $eventId,
        int $userId,
