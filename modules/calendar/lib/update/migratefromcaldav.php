    ...
    public static function doMigration()
    {
        ...
        if (!\Bitrix\Main\Loader::includeModule('dav') || !$canDoMigration) {
            ...
            while ($row = $query->Fetch()) {
                ...
                $sectionsList = checkSerializedData($row['SECTION_DATA']) ? unserialize(
                    $row['SECTION_DATA'],
                ...
                if ($sectionsList) {
                    ...
                    if (!$errorCode) {
                            ...
                            if ($replaced == 2) {
                                ...
                                foreach ($googleCalendars as $googleCalendar) {
                                    if ($isVirtual && ($googleCalendar['id'] == $section['DESCRIPTION'] || $googleCalendar['summary'] == $section['NAME'])) {
                                        $DB->Query(
                                            "UPDATE b_calendar_section SET GAPI_CALENDAR_ID = '" . $DB->ForSql(
                                                $googleCalendar['id']
                                            ) . "', TEXT_COLOR = '" . $DB->ForSql(
                                                $googleCalendar['textColor']
                                            ) . "', COLOR = '" . $DB->ForSql(
                                                $googleCalendar['backgroundColor']
                                            ) . "' WHERE ID = '" . intval($section['ID']) . "'"
                                        );
                                        continue;
                                    }
                        ...
                        $DB->Query(
                            "UPDATE b_calendar_tmp_migration SET ATTEMPTS = ATTEMPTS + 1 WHERE ID = " . intval(
                                $row['ID']
                            )
                        );
                    }
