...
//				],
    ...
    public function getTimezoneListAction()
    {
        ...
        $defaultTimezone = \CCalendar::getGoodTimezoneForOffset(
            \CCalendar::getCurrentOffsetUTC(\CCalendar::getCurUserId())
        );
    ...
    public function getEditEventSliderAction()
    {
        ...
        if ($entryId > 0) {
            $fromTs = !empty($_REQUEST['date_from_offset']) ? \CCalendar::Timestamp(
                    $_REQUEST['date_from']
        ...
        if (!$entryId || !empty($entry) && \CCalendarSceleton::CheckBitrix24Limits(array('id' => $uniqueId))) {
            ...
            $sections = array_merge(
                $sections,
                \CCalendar::getSectionListAvailableForUser($userId, [$entry['SECTION_ID']])
            );
    ...
    public function getViewEventSliderAction()
    {
        ...
        if ($entryId) {
            $entry = \CCalendarEvent::getEventForViewInterface(
                $entryId,
                [
    ...
    public function changeRecurciveEntryUntilAction($entryId, $untilDate)
    {
        ...
        if ($response['result'] !== true) {
            $this->addError(
                new Error('[ed01]' . Loc::getMessage('EC_EVENT_DEL_ERROR'), 'change_recurcive_entry_until')
            );
        }
            ...
            if (empty($this->getErrors())) {
                                ...
                                if ($entFromTs < $toTs && $entToTs > $fromTs) {
                                    ...
                                    $this->addError(
                                        new Error(
                                            Loc::getMessage(
                                                'EC_USER_BUSY',
                                                ["#USER#" => \CCalendar::GetUserName($accUserId)]
                ...
                if (empty($this->getErrors())) {
                    $newId = \CCalendar::SaveEvent(
                        [
                            ...
                            'currentEventDateFrom' => \CCalendar::Date(
                                \CCalendar::Timestamp($request['current_date_from']),
                                false
                            ),
                    ...
                    if ($newId && !count($errors)) {
                        ...
                        } elseif ($id && $eventList && $eventList[0] && \CCalendarEvent::CheckRecurcion(
                                $eventList[0]
                            )) {
