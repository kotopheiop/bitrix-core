        ...
        if (count($params['section']) > 0) {
            ...
            foreach ($res as $entry) {
                if (in_array($entry['SECT_ID'], $params['section'])) {
                    ...
                    if ($loadNext && !\CCalendarEvent::CheckRecurcion(
                            $entry
                        ) && $entry['DATE_TO_TS_UTC'] > $lastDateTimestamp) {
                        $lastDateTimestamp = $entry['DATE_TO_TS_UTC'];
                    } elseif ($loadPrevious && !\CCalendarEvent::CheckRecurcion(
                            $entry
                        ) && $entry['DATE_FROM_TS_UTC'] < $lastDateTimestamp) {
