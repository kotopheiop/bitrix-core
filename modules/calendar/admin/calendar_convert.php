...
?>
    <form style="border: 2px solid #B8C1DD; padding: 10px; background: #F8F8F8;" method="post" name="calendar_form"
          action="/bitrix/admin/calendar_convert.php" enctype="multipart/form-data" encoding="multipart/form-data">
        <?= bitrix_sessid_post(); ?>
        ...
        <? if (!isset($RES)): ?>
            ...
            <input id="bxconv_parse" type="button" value="<?= GetMessage('CAL_PARSE_CONTENT') ?>"
        ...
        <? else: ?>
            ...
            <table class="edit-table">
                <? foreach ($RES['TYPES'] as $k => $type):
                    ...
                    <tr>
                        <td class="field-name">
                            <input name="<?= $name ?>[allow]" type="checkbox" <? if ($type['checked']) {
                                echo ' checked="checked"';
                            } ?> <? /*if($type['sys']){echo ' disabled';}*/
                            ?> value="Y" title="<?= GetMessage('TYPE_CONVERT_IT') ?>"/>
                        </td>
                        <td class="field-name">
                            <? if ($type['sys']):?>
                                ...
                                <input type="hidden" name="<?= $name ?>[title]"
                                       value="<?= htmlspecialcharsbx($type['title']) ?>"/>
                                <?= htmlspecialcharsbx($type['title']) ?>
                            <? else:?>
                                <span class="display: inline-block;"><?= GetMessage('TYPE_ID') ?>:</span>
                                <input size="40" name="<?= $name ?>[key]" type="text"
                                       value="<?= htmlspecialcharsbx($k) ?>"><br>
                                <span class="display: inline-block;"><?= GetMessage('TYPE_TITLE') ?>:</span>
                                <input size="40" name="<?= $name ?>[title]" type="text"
                                       value="<?= htmlspecialcharsbx($type['title']) ?>">
                            <? endif; ?>
                        </td>
                        <td class="field-name">
                        ...
                        <td style="text-align: left; padding: 3px 0 3px 10px;">
                            <select name="<?= $name ?>[iblock_type]"
                                    onchange="changeIblockList(this.value, [BX('type_iblock_sel_<?= $k ?>')]);">
                                <? foreach ($IB['types'] as $ibtype_id => $ibtype_name):?>
                                    <option value="<?= $ibtype_id ?>" <? if ($ibtype_id == $type['iblockType']) {
                            ...
                            <select id="type_iblock_sel_<?= $k ?>" name="<?= $name ?>[iblock_id]">
                                <? if ($type['iblockId']):?>
                                    <? foreach ($IB['iblocks'][$type['iblockType']] as $iblock_id => $iblock):?>
                                        <option value="<?= $iblock_id ?>"<? if ($iblock_id == $type['iblockId']) {
                                            echo ' selected="selected"';
                                        } ?>><?= $iblock ?></option>
                                    <? endforeach; ?>
                                <? else:?>
            ...
            <div>
                <table class="edit-table">
                    <tr>
                        <td class="field-name"><label for="cal_work_time"><?= GetMessage("CAL_WORK_TIME") ?>:</label>
                        </td>
                        <td>
                            <select id="cal_work_time" name="work_time_start">
                                <? foreach ($arWorTimeList as $key => $val): ?>
                                    <option value="<?= $key ?>" <? if ($SET['work_time_start'] == $key) {
                                        echo ' selected="selected" ';
                                    } ?>><?= $val ?></option>
                                <? endforeach; ?>
                            </select>
                            ...
                            <select id="cal_work_time" name="work_time_end">
                                <? foreach ($arWorTimeList as $key => $val): ?>
                                    <option value="<?= $key ?>" <? if ($SET['work_time_end'] == $key) {
                                        echo ' selected="selected" ';
                                    } ?>><?= $val ?></option>
                                <? endforeach; ?>
                            </select>
                    ...
                    <tr>
                        <td class="field-name" style="vertical-align: top;"><label
                                    for="cal_week_holidays"><?= GetMessage("CAL_WEEK_HOLIDAYS") ?>:</label></td>
                        <td>
                            <select size="7" multiple=true id="cal_week_holidays" name="week_holidays[]">
                                <? foreach ($arDays as $day): ?>
                                    <option value="<?= $day ?>" <? if (in_array($day, $SET['week_holidays'])) {
                                        echo ' selected="selected"';
                                    } ?>><?= GetMessage('CAL_OPTION_FIRSTDAY_' . $day) ?></option>
                                <? endforeach; ?>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        <td class="field-name"><label for="cal_year_holidays"><?= GetMessage("CAL_YEAR_HOLIDAYS") ?>
                                :</label></td>
                        <td>
                            <input name="year_holidays" type="text"
                                   value="<?= htmlspecialcharsbx($SET['year_holidays']) ?>" id="cal_year_holidays"
                                   size="60"/>
                        </td>
                    </tr>
                    <!-- Path parameters title -->
                    <tr class="heading">
                        <td colSpan="2"><?= GetMessage('CAL_PATH_TITLE') ?></td>
                    </tr>
                    <tr>
                        <td class="field-name"><label for="cal_path_to_user"><?= GetMessage("CAL_PATH_TO_USER") ?>
                                :</label></td>
                        <td>
                            <input name="path_to_user" type="text"
                                   value="<?= htmlspecialcharsbx($SET['path_to_user']) ?>" id="cal_path_to_user"
                                   size="60"/>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <input name="path_to_user_calendar" type="text"
                                   ...
                                   id="cal_path_to_user_calendar" size="60"/>
                        </td>
                    </tr>
                    <tr>
                        <td class="field-name"><label for="cal_path_to_group"><?= GetMessage("CAL_PATH_TO_GROUP") ?>
                                :</label></td>
                        <td>
                            <input name="path_to_group" type="text"
                                   value="<?= htmlspecialcharsbx($SET['path_to_group']) ?>" id="cal_path_to_group"
                                   size="60"/>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <input name="path_to_group_calendar" type="text"
                                   ...
                                   id="cal_path_to_group_calendar" size="60"/>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td class="field-name"><label for="cal_rm_iblock_type"><?= GetMessage("CAL_RM_IBLOCK_TYPE") ?>
                                :</label></td>
                        <td>
                            <select name="rm_iblock_type"
                                    onchange="changeIblockList(this.value, [BX('cal_rm_iblock_id'), BX('cal_vr_iblock_id')])">
                                <option value=""><?= GetMessage('CAL_NOT_SET') ?></option>
                                <? foreach ($IB['types'] as $ibtype_id => $ibtype_name): ?>
                                    <option value="<?= $ibtype_id ?>" <? if ($ibtype_id == $SET['rm_iblock_type']) {
                                        echo ' selected="selected"';
                    ...
                    <tr>
                        <td class="field-name"><label for="cal_rm_iblock_id"><?= GetMessage("CAL_RM_IBLOCK_ID") ?>
                                :</label></td>
                        <td>
                            <select id="cal_rm_iblock_id" name="rm_iblock_id">
                                <? if ($SET['rm_iblock_id']): ?>
                                    <? foreach ($IB['iblocks'][$SET['rm_iblock_type']] as $iblock_id => $iblock): ?>
                                        <option value="<?= $iblock_id ?>"<? if ($iblock_id == $SET['rm_iblock_id']) {
                                            echo ' selected="selected"';
                                        } ?>><?= $iblock ?></option>
                                    <? endforeach; ?>
                                <? else: ?>
                    ...
                    <tr>
                        <td class="field-name"><label for="cal_path_to_rm"><?= GetMessage("CAL_PATH_TO_RM") ?>:</label>
                        </td>
                        <td>
                            <input name="path_to_rm" type="text" value="<?= htmlspecialcharsbx($SET['path_to_rm']) ?>"
                                   id="cal_path_to_rm" size="60"/>
                        </td>
                    </tr>
                    <tr>
                        <td class="field-name"><label for="cal_vr_iblock_id"><?= GetMessage("CAL_VR_IBLOCK_ID") ?>
                                :</label></td>
                        <td>
                            <select id="cal_vr_iblock_id" name="vr_iblock_id"">
                            <? if ($SET['vr_iblock_id']): ?>
                                <? foreach ($IB['iblocks'][$SET['rm_iblock_type']] as $iblock_id => $iblock): ?>
                                    <option value="<?= $iblock_id ?>"<? if ($iblock_id == $SET['vr_iblock_id']) {
                                        echo ' selected="selected"';
                                    } ?>><?= $iblock ?></option>
                                <? endforeach; ?>
                            <? else: ?>
                    ...
                    <tr>
                        <td class="field-name"><label for="cal_path_to_vr"><?= GetMessage("CAL_PATH_TO_VR") ?>:</label>
                        </td>
                        <td><input name="path_to_vr" type="text" value="<?= htmlspecialcharsbx($SET['path_to_vr']) ?>"
                                   id="cal_path_to_vr" size="60"/></td>
                    </tr>
                </table>
            </div>
            ...
            <div id="bxconvert_cont" style="display: none;">
                ...
                <span id="bxconvert_result"
                      style="padding: 10px; display: inline-block; border-radius: 3px; border: 1px solid #A9A9A9; font-size: 11px; color:#808080; margin-bottom: 20px; background: #FFFFFF;">
				<div> - <?= GetMessage('CAL_CONVERT_STAGE_PREPARE') ?></div>
			</span>
            </div>

            <input id="bxconv_but_start" type="button" value="<?= GetMessage('CAL_CONVERT_START') ?>"
                   onclick="startConvert();"/>
            <input id="bxconv_go_to_settings" type="button" value="<?= GetMessage('CAL_GO_TO_SETTINGS') ?>"
                   onclick="window.location='/bitrix/admin/settings.php?lang=<?= LANG ?>&mid=calendar';"
                   style="display: none;"/>
        <? endif; ?>
    </form>
    <script>
