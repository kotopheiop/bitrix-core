...
$tabControl->Begin();
?>
<form method="post" name="cal_opt_form"
    ...
    <tr>
        <td style="vertical-align: top;"><label for="cal_week_holidays"><?= GetMessage("CAL_WEEK_HOLIDAYS") ?>:</label>
        </td>
        <td>
	...
	*/
    ...
    <tr>
        ...
        <td>
            <input name="year_holidays" type="text" value="<?= htmlspecialcharsbx($SET['year_holidays']) ?>"
                   id="cal_year_holidays" size="60"/>
        </td>
    </tr>
    <tr>
        <td><label for="cal_year_workdays"><?= GetMessage("CAL_YEAR_WORKDAYS") ?>:</label></td>
        <td>
            <input name="year_workdays" type="text" value="<?= htmlspecialcharsbx($SET['year_workdays']) ?>"
                   id="cal_year_workdays" size="60"/>
        </td>
    </tr>
    <? if (CCalendar::IsIntranetEnabled()): ?>
        <tr>
            ...
            <td>
                <input name="user_name_template" type="text"
                       value="<?= htmlspecialcharsbx($SET['user_name_template']) ?>" id="cal_user_name_template"
                       size="60"/>
            </td>
        </tr>
        <tr>
            <td><input name="sync_by_push" type="checkbox" value="Y"
                       id="cal_sync_by_push" <? if ($SET['sync_by_push']) {
                    echo 'checked';
                } ?>/></td>
            <td>
        ...
        <tr>
            <td><input name="user_show_login" type="checkbox" value="Y"
                       id="cal_user_show_login" <? if ($SET['user_show_login']) {
                    echo 'checked';
                } ?>/></td>
            <td>
        ...
        <tr title="<?= GetMessage('CAL_DEP_MANAGER_SUB_TITLE') ?>">
            <td><input name="dep_manager_sub" type="checkbox" value="Y"
                       id="cal_dep_manager_sub" <? if ($SET['dep_manager_sub']) {
                    echo 'checked';
                } ?>/></td>
            <td>
        ...
        <tr>
            <td style="vertical-align: top;"><label for="denied_superpose_types"><?= GetMessage("CAL_SP_TYPES") ?>
                    :</label></td>
            <td>
                        ...
                        <table>
                            ...
                            foreach ($arPathes as $pathName) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input name="pathes[<?= $siteId ?>][<?= $pathName ?>]" type="text"
                                               value="<?= htmlspecialcharsbx($val) ?>" id="cal_<?= $pathName ?>"
                                               size="60"/>
                                    </td>
                                </tr>
                            <? } ?>
        ...
        foreach ($arPathes as $pathName) {
            ...
            ?>
            <tr id="bx-cal-opt-path-<?= $pathName ?>" <? if (!$commonForSites) {
                echo 'style="display:none;"';
            } ?>>
                <td><label for="cal_<?= $pathName ?>"><?= htmlspecialcharsbx($title) ?>:</label></td>
                <td>
                    <input name="<?= $pathName ?>" type="text" value="<?= htmlspecialcharsbx($SET[$pathName]) ?>"
                           id="cal_<?= $pathName ?>" size="60"/>
                </td>
            </tr>
        <? } ?>
        ...
        <? if (count($sites) > 1): ?>
            <tr>
                <td>
                    <input name="reserve_meetings_for_sites" type="checkbox" id="cal_reserve_meetings_for_sites"
                           value="Y" <? if ($reserveMeetingForSites) {
                        echo 'checked';
                    } ?>>
                </td>
                <td>
            ...
            <tr id="bx-cal-opt-sites-rm-tr" <? if ($reserveMeetingForSites) {
                echo 'style="display:none;"';
            } ?>>
                <td colSpan="2" align="center">
                    <?
        ...
        <tr id="bx-cal-opt-common-rm-tr" <? if (!$reserveMeetingForSites) {
            echo 'style="display:none;"';
        } ?>>
    ...
    <!-- Comments settings -->
    <tr class="heading">
        <td colSpan="2"><?= GetMessage('CAL_COMMENTS_SETTINGS') ?></td>
    </tr>
    <tr>
    ...
    <tr class="">
        <td colspan="2" style="text-align: left;">
            <a class="bxco-add-type" href="javascript:void(0);" onclick="addType(); return false;"
               title="<?= GetMessage('CAL_ADD_TYPE_TITLE') ?>"><i></i><span><?= GetMessage('CAL_ADD_TYPE') ?></span></a>
        </td>
    </tr>
    <tr>
        <td colspan="2" align="center">
    ...
    <? $tabControl->Buttons(); ?>
    <input type="submit" class="adm-btn-save" name="Update" value="<?= GetMessage("MAIN_SAVE") ?>"
    ...
    <input type="submit" name="RestoreDefaults" title="<? echo GetMessage("MAIN_HINT_RESTORE_DEFAULTS") ?>"
           ...
           value="<? echo GetMessage("CAL_RESTORE_DEFAULTS") ?>">
    <? $tabControl->End(); ?>
</form>
...
<div id="edit_type_dialog" class="bxco-popup">
    <form method="POST" name="caltype_dialog_form" id="caltype_dialog_form"
        ...
        <table border="0" cellSpacing="0" class="bxco-popup-tbl">
            ...
            <tr>
                <td class="bxco-2-right"><label for="type_desc_inp"><?= GetMessage('CAL_TYPE_DESCRIPTION') ?>:</label>
                </td>
                <td><textarea name="type_desc" id="type_desc_inp" rows="3" cols="30" style="resize:none;"></textarea>
                </td>
            </tr>
        </table>
    </form>
            ...
            setTimeout(function () {
                ...
                top.BXCalAccess<?= $XML_ID?> = new top.ECCalendarAccess({
                    ...
                    GetAccessName: function (code) {
                        return top.accessNames[code] || code;
                    },
