...
//						'COLOR' => $externalCalendar['backgroundColor'],
                    ...
                    CCalendarSect::Edit(
                        array('arFields' => array('ID' => $localSections[$localCalendarIndex]['ID'], 'ACTIVE' => 'N'))
                    );
                }
            }
        ...
        if ($pushOptionEnabled) {
            $pushResult = PushTable::getByPrimary(
                array('ENTITY_TYPE' => 'CONNECTION', 'ENTITY_ID' => $connectionData['ID'])
            );

            if ($row = $pushResult->fetch()) {
                if (!GoogleApiPush::isConnectionError($connectionData['LAST_RESULT'])) {
                    if (!GoogleApiPush::checkSectionsPush(
                        $localSections,
                        $connectionData['ENTITY_ID'],
                        $connectionData['ID']
                    )) {
        ...
        foreach ($localSections as $localCalendar) {
            ...
            CCalendarSect::Edit(
                array('arFields' => array('ID' => $localCalendar['ID'], 'SYNC_TOKEN' => $eventsSyncToken))
            );
        }
    ...
    public static function ModifyEvent($calendarId, $arFields, $params = array())
    {
        ...
        if ($sectionId && ($section = CCalendarSect::GetById($sectionId, false))) {
            ...
            if ($arNewFields["SKIP_TIME"]) {
                ...
                $arNewFields["DATE_TO"] = CCalendar::Date(
                    CCalendar::Timestamp($arNewFields['DATE_TO']) - CCalendar::GetDayLen(),
                    false
                );
            }
            ...
            if (!empty($arFields['DETAIL_TEXT'])) {
                $arNewFields['DESCRIPTION'] = self::CutAttendeesFromDescription(
                    ...
                    self::getAttendeesCodesForCut(
                        $currentEvent['ATTENDEES_CODES']
                    )
                );
            }

            $arNewFields["DESCRIPTION"] = CCalendar::ClearExchangeHtml($arNewFields["DESCRIPTION"]);
            if (isset($arFields["PROPERTY_PERIOD_TYPE"]) && in_array(
                    $arFields["PROPERTY_PERIOD_TYPE"],
    ...
    public static function DoSaveToDav($params = array(), &$arFields, $event = false)
    {
        ...
        if ($event) {
            if ($event['SECT_ID'] != $sectionId) {
                $bCalDavCur = CCalendar::IsCalDAVEnabled(
                    ) && $event['CAL_TYPE'] == 'user' && $event['CAL_DAV_LABEL'] <> '';
        ...
        if ($arFields['REMIND'] && is_array($arFields['REMIND']) && is_array($arFields['REMIND'][0])) {
            $arDavFields['REMIND_SETTINGS'] = floatVal(
                    $arFields['REMIND'][0]['count']
        ...
        if ($bGoogleApi) {
            ...
            $responseFields = $modeSync ? $googleApiCalendar->saveEvent(
                $arDavFields,
...
//		$modeSyncExchange = ($params['editInstance'] && !$params['editParentEvents']) || (!$params['editNextEvents'] || !$params['editParentEvents']);
        ...
        elseif ($bExchange && $section['IS_EXCHANGE'] && $section['DAV_EXCH_CAL'] <> '' && $section['DAV_EXCH_CAL'] !== 0 && $modeSync) {
            ...
            if (($arFields['ID'] <= 0 || ($event && !$event['DAV_EXCH_LABEL'])) && $updateEvent) {
                ...
                $exchRes = CDavExchangeCalendar::DoUpdateItem(
                    $ownerId,
                    $event['DAV_XML_ID'],
                    $event['DAV_EXCH_LABEL'],
                    $arDavFields,
    ...
    public static function DoDeleteToDav($params, $event)
    {
        $sectionId = $params['sectionId'];
        $section = CCalendarSect::GetById($sectionId, false);
        $bGoogleApi = CCalendar::isGoogleApiEnabled() && $event['CAL_TYPE'] == 'user' && !is_null(
                $section['GAPI_CALENDAR_ID']
            );
        ...
        if ($bCalDav && $section['CAL_DAV_CON'] > 0) {
            $DAVRes = CDavGroupdavClientCalendar::DoDeleteItem(
                $section['CAL_DAV_CON'],
                $section['CAL_DAV_CAL'],
                $event['DAV_XML_ID']
            );
    ...
    public static function syncCalendarSections(
        $connectionType,
        $arCalendars,
        $entityType,
        $entityId,
            ...
            if (!$id) {
                ...
                if ($curPath) {
                    $curPath = CHTTP::urlDeleteParams(
                        $curPath,
                        array("action", "sessid", "bx_event_calendar_request", "EVENT_ID")
                    );
                }
        ...
        if (CCalendar::IsSocNet()) {
            ...
            if ($strValue != '') {
                ...
                if ($exchangeUseLogin == "Y") {
                    $strLogins = '';
                    foreach ($emailList as $email) {
                        if (!in_array(mb_strtolower($email), $checkedEmails) && mb_strtolower(
                                mb_substr($email, mb_strlen($email) - $exchangeMailboxStrlen)
                            ) == mb_strtolower($exchangeMailbox)) {
                    ...
                    if ($strLogins !== '') {
                        $res = $DB->Query(
                            "SELECT U.ID, U.LOGIN FROM b_user U WHERE U.ACTIVE = 'Y' AND U.LOGIN in (" . $strLogins . ")",
                            false,
...
//		global $USER;
        ...
        foreach ($externalEvents as $externalEvent) {
            ...
            if ($externalEvent['status'] == 'cancelled') {
                if ($eventExists) {
                    ...
                    $excludeDates = CCalendarEvent::GetExDate(
                        $localEvents[$externalEvent['recurringEventId']]['EXDATE']
                    );
                    ...
                    $newParentData['arFields']['RRULE'] = CCalendarEvent::ParseRRULE(
                        $newParentData['arFields']['RRULE']
                    );
            ...
            if ($externalEvent['isRecurring'] == "N") {
                if (!$eventExists) {
                    ...
                    $newEvent['ID'] = CCalendarEvent::Edit(
                        ['arFields' => $newEventData, 'path' => CCalendar::GetPath('user', $newEventData['OWNER_ID'])]
                    );
                    ...
                    $newParentData['arFields']['DESCRIPTION'] = self::CutAttendeesFromDescription(
                        ...
                        self::getAttendeesCodesForCut(
                            $localEvents[$externalEvent['G_EVENT_ID'] . '@google.com']['ATTENDEES_CODES']
                        )
                    );
                    ...
                    if ($recurrentParentEventExists) {
                        ...
                        if (isset($externalEvent['DESCRIPTION'])) {
                            $externalEvent['DESCRIPTION'] = self::CutAttendeesFromDescription(
                                ...
                                self::getAttendeesCodesForCut(
                                    $localEvents[$externalEvent['recurringEventId']]['ATTENDEES_CODES']
                                )
                            );
                        }
    ...
    {
        $localEventsList = CCalendarEvent::getList(
            array(
    ...
    public static function GetSyncInfoItem($userId, $syncType): array
    {
        ...
        if ($result['date']) {
            $result['date'] = CCalendar::Date(
                CCalendar::Timestamp($result['date']) + CCalendar::GetOffset($userId),
                true,
                true,
                true
            );
        ...
        if ($bCalDAV || $bGoogleApi) {
            ...
            while ($connection = $res->Fetch()) {
                if ($connection['ACCOUNT_TYPE'] === 'caldav') {
                    ...
                    if (Util::isGoogleConnection($connection['ACCOUNT_TYPE'])) {
