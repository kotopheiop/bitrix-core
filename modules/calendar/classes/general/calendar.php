...
class CCalendar
{
    ...
    function Init($params)
    {
        ...
        self::$userId = (isset($params['userId']) && $params['userId'] > 0) ? intval(
            $params['userId']
        ) : CCalendar::GetCurUserId(true);
    ...
    public function Show($params = [])
    {
        ...
        $JSConfig = Array(
            ...
            'locationFeatureEnabled' => !self::IsBitrix24() || \Bitrix\Bitrix24\Feature::isFeatureEnabled(
                    "calendar_location"
                ),
        ...
        if (Loader::includeModule('rest')) {
            $placementParams = [
                ...
                'gridPlacementList' => \Bitrix\Rest\PlacementTable::getHandlersList(
                    \CCalendarRestService::PLACEMENT_GRID_VIEW
                ),
        ...
        foreach ($sections as $i => $section) {
            $sections[$i]['~IS_MEETING_FOR_OWNER'] = $section['CAL_TYPE'] === 'user' && $section['OWNER_ID'] !== self::$userId && CCalendar::GetMeetingSection(
                    $section['OWNER_ID']
                ) === $section['ID'];
        ...
        if ($bCreateDefault) {
            $fullSectionsList = $groupOrUser ? self::GetSectionList(
                array('checkPermissions' => false, 'getPermissions' => false)
            ) : [];
    ...
    public static function DeleteEvent($id, $doExternalSync = true, $params = [])
    {
        ...
        if ($event = $res[0]) {
            ...
            if ($checkPermissions && !self::IsPersonal(
                    $event['CAL_TYPE'],
                    $event['OWNER_ID'],
                    self::$userId
                ) && !CCalendarSect::CanDo('calendar_edit', $event['SECT_ID'], self::$userId)) {
            ...
            if ($params['recursionMode'] != 'this' && $event['RECURRENCE_ID']) {
                self::DeleteEvent(
                    $event['RECURRENCE_ID'],
                    $doExternalSync,
                    array('sendNotification' => $sendNotification)
                );
            }
    ...
    public static function HandleImCallback($module, $tag, $value, $arNotify)
    {
        ...
        if ($module == "calendar" && $userId) {
            ...
            if ($arTag[0] == "CALENDAR" && $arTag[1] == "INVITE" && $eventId > 0 && $userId) {
                ...
                return $value == 'Y' ? Loc::getMessage('EC_PROP_CONFIRMED_TEXT_Y') : Loc::getMessage(
            ...
            if ($clearOptions) {
                ...
                if ($opt === 'rm_iblock_id' && !$settings['rm_for_sites']) {
                    ...
                    if ($settings[$opt]) {
                        ...
                        \CAgent::AddAgent(
                            "\\Bitrix\\Calendar\\Sync\\GoogleApiPush::checkPushChannel();",
                            "calendar",
                            "N",
    ...
    public static function ReminderAgent(
        $eventId = 0,
        $userId = 0,
        $viewPath = '',
        $calendarType = '',
    ...
    public static function GetNearestEventsList($params = [])
    {
        ...
        if ($params['bCurUserList'] && ($curUserId <= 0 || (class_exists(
                        'CSocNetFeatures'
        ...
        if ($allowReserveMeeting) {
            ...
            while ($obElement = $rsElement->GetNextElement()) {
                ...
                $arItem["DISPLAY_ACTIVE_TO"] = CIBlockFormatProperties::DateFormat(
                    self::DFormat(true),
                    MakeTimeStamp($arItem["ACTIVE_TO"])
                );
    ...
    public static function GetCalendarList($calendarId, $params = [])
    {
        ...
        if (CCalendarSync::isTaskListSyncEnabled(
            ) && $entityType == 'user' && ($sectionId === $TASK_ID || !$sectionId)) {
            $arCalendars[] = array(
                'ID' => $TASK_ID,
                '~NAME' => 'My tasks',
                'NAME' => 'My tasks',
    ...
    public static function UnParseTextLocation($loc = '')
    {
        ...
        if ($loc != "") {
            ...
            if ($location['mrid'] === false && $location['room_id'] === false) {
                ...
                if (!self::IsBitrix24() || (Loader::includeModule(
                            'bitrix24'
    ...
    public static function SaveEventEx($params = [])
    {
        ...
        $result = [];
        $sectionId = (is_array($arFields['SECTIONS']) && count(
                $arFields['SECTIONS']
            ) > 0) ? $arFields['SECTIONS'][0] : intval($arFields['SECTIONS']);
...
//				CCalendarEvent::Edit($params);
        ...
        if ($params['autoDetectSection'] && $sectionId <= 0) {
            ...
            if (!$sectionId) {
                $sectRes = self::GetSectionForOwner(
                    $arFields['CAL_TYPE'],
                    $arFields['OWNER_ID'],
        ...
        if (
            ...
            if ($params['recursionEditMode'] == 'this') {
                ...
                $excludeDate = self::Date(
                    self::Timestamp(
                        isset($params['currentEventDateFrom']) ? $params['currentEventDateFrom'] : $arFields['DATE_FROM']
                    ),
                    false
                );
                ...
                $length = self::Timestamp($newParams['arFields']['DATE_TO']) - self::Timestamp(
                ...
                $instanceDate = !isset($newParams['arFields']['DATE_FROM']) || self::Date(
                        self::Timestamp($curEvent['DATE_FROM']),
                        false
                    ) == self::Date($currentFromTs, false);

                if ($newParams['arFields']['SKIP_TIME']) {
                    if ($instanceDate) {
                        $newParams['arFields']['DATE_FROM'] = self::Date($fromTs, false);
                        $newParams['arFields']['DATE_TO'] = self::Date(
                            $fromTs + $length - CCalendar::GetDayLen(),
                            false
                        );
                    } else {
                        $newParams['arFields']['DATE_FROM'] = self::Date($currentFromTs, false);
                        $newParams['arFields']['DATE_TO'] = self::Date(
                            $currentFromTs + $length - CCalendar::GetDayLen(),
                            false
                        );
                    }
                } else {
            ...
            elseif ($params['recursionEditMode'] == 'next') {
                ...
                if (CCalendar::Date(self::Timestamp($curEvent['DATE_FROM']), false) == CCalendar::Date(
                    ...
                    foreach ($recRelatedEvents as $ev) {
                        ...
                        if ($evFromTs > $currentDateTimestamp) {
                            ...
                            if ($newParams['arFields']['SKIP_TIME']) {
                                ...
                                $newParams['arFields']['DATE_TO'] = self::Date(
                                    CCalendar::Timestamp($ev['DATE_TO']),
                                    false
                                );
                            } else {
            ...
            if ($curEvent && CCalendarEvent::CheckRecurcion($curEvent)
                ...
                if ($recId) {
                    $events_1 = CCalendarEvent::GetList(
                        array(
                            'arFilter' => array('RECURRENCE_ID' => $recId),
                            'parseRecursion' => false,
                            'setDefaultLimit' => false
                        )
                    );
    ...
    public static function CountPastEvents($params)
    {
        ...
        $dateFrom = new Main\Type\DateTime(
            $params['dateFrom'],
            Main\Type\Date::convertFormatToPhp(FORMAT_DATETIME),
            $dateFromTz
        );
    ...
    public static function GetUserUrl($userId = 0, $pathToUser = "")
    {
        ...
        return CUtil::JSEscape(
            CComponentEngine::MakePathFromTemplate($pathToUser, array("user_id" => $userId, "USER_ID" => $userId))
        );
    }
    ...
    public static function GetAccessTasks($binging = 'calendar_section')
    {
        \Bitrix\Main\Localization\Loc::loadLanguageFile(
            $_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/calendar/admin/task_description.php"
        );
        ...
        while ($arRes = $res->Fetch()) {
            if (!$bIntranet && (mb_strtolower($arRes['NAME']) == 'calendar_view_time' || mb_strtolower(
                        $arRes['NAME']
                    ) == 'calendar_view_title')) {
                continue;
            }
            ...
            } elseif ($con['del'] !== 'Y') // Edit connection
            {
                ...
                if (is_array($con['sections'])) {
                    foreach ($con['sections'] as $sectId => $active) {
                        ...
                        if (CCalendar::IsPersonal() || CCalendarSect::CanDo(
                                'calendar_edit_section',
                                $sectId,
                                self::$userId
                            )) {
                            CCalendarSect::Edit(
                                array('arFields' => array("ID" => $sectId, "ACTIVE" => $active == "Y" ? "Y" : "N"))
                            );
                        }
                    }
                }
    ...
    public static function CheckCalDavUrl($url, $username, $password)
    {
        ...
        if (Loader::includeModule("dav")) {
            return \CDavGroupdavClientCalendar::DoCheckCalDAVServer(
                $arServer["scheme"],
                $arServer["host"],
                $arServer["port"],
                $username,
    ...
    public static function RemoveConnection(array $params = [])
    {
        if (Loader::includeModule('dav')) {
            ...
            $connectionName = Util::isGoogleConnection(
                $connection['ACCOUNT_TYPE']
            ) ? 'google' : $connectionType . $connection['ID'];
            Util::addPullEvent(
    ...
    {
        $text = preg_replace(
            array("/\&lt;/is" . BX_UTF_PCRE_MODIFIER, "/\&gt;/is" . BX_UTF_PCRE_MODIFIER),
            array('<', '>'),
            $text
        );
    ...
    {
        $ac = CSocNetLogTools::FormatDestinationFromRights(
            $codes,
            array(
        ...
        if ($skipTime) {
            ...
            if (date('dmY', $fromTs) == date('dmY', $toTs)) {
                ...
                $html .= Loc::getMessage(
                    ...
                    array(
                        '#TIME_FROM#' => FormatDate($formatTime, $fromTs),
                        '#TIME_TO#' => FormatDate($formatTime, $toTs)
                    )
                );
            } else {
                $html = Loc::getMessage(
    ...
    public static function GetSocNetDestination($user_id = false, $selected = [], $userList = [])
    {
        ...
        if (method_exists('CSocNetLogDestination', 'GetDestinationSort')) {
            $DESTINATION = array(
                ...
                'DEST_SORT' => CSocNetLogDestination::GetDestinationSort(
                    array("DEST_CONTEXT" => \Bitrix\Calendar\Util::getUserSelectorContext())
                )
            );
        ...
        if ($obCache->InitCache($cacheTtl, $cacheId, $cacheDir)) {
            ...
            $DESTINATION['SONETGROUPS'] = CSocNetLogDestination::GetSocnetGroup(
                Array('features' => array("calendar", array("view")))
            );
        ...
        if (Loader::includeModule('extranet') && !CExtranet::IsIntranetUser(SITE_ID, $userId)) {
            ...
            $DESTINATION['USERS'] = array_merge(
                $DESTINATION['USERS'],
                CSocNetLogDestination::GetUsers(['id' => [$userId]])
            );
        } else {
    ...
    public static function SaveUserTimezoneName($user, $tzName = '')
    {
        ...
        CUserOptions::SetOption(
            "calendar",
            "timezone" . self::GetCurrentOffsetUTC($user['ID']),
            $tzName,
            false,
            $user['ID']
        );
    ...
    public static function IsSocNet()
    {
        if (!isset(self::$bSocNet)) {
            ...
            self::$bSocNet = class_exists('CSocNetUserToGroup') && CBXFeatures::IsFeatureEnabled(
                    "Calendar"
                ) && self::IsIntranetEnabled();
        }
    ...
    public static function GetSettings($params = [])
    {
        ...
        self::$settings = array(
            ...
            'year_holidays' => COption::GetOptionString(
                'calendar',
                'year_holidays',
                Loc::getMessage('EC_YEAR_HOLIDAYS_DEFAULT')
            ),
            'year_workdays' => COption::GetOptionString(
                'calendar',
                'year_workdays',
                Loc::getMessage('EC_YEAR_WORKDAYS_DEFAULT')
            ),
            ...
            'path_to_user_calendar' => COption::GetOptionString(
                'calendar',
                'path_to_user_calendar',
            ...
            'path_to_group_calendar' => COption::GetOptionString(
                'calendar',
                'path_to_group_calendar',
            ...
            'rm_iblock_id' => COption::GetOptionString(
                'calendar',
                'rm_iblock_id',
                "",
                $siteIdForResMeet,
            ...
            'denied_superpose_types' => unserialize(
                COption::GetOptionString('calendar', 'denied_superpose_types', serialize([])),
    ...
    public static function isGoogleApiEnabled()
    {
        if (!isset(self::$isGoogleApiEnabled)) {
            self::$isGoogleApiEnabled = \Bitrix\Main\ModuleManager::isModuleInstalled('socialservices') &&
                (is_null(
                        \Bitrix\Main\Config\Configuration::getValue("calendar_integration")
                    ) || \Bitrix\Main\Config\Configuration::getValue(
                        "calendar_integration"
                    ) === self::INTEGRATION_GOOGLE_API);

            if (self::$isGoogleApiEnabled
                && !self::IsBitrix24()
                && Loader::includeModule('socialservices')) {
                self::$isGoogleApiEnabled = CSocServGoogleOAuth::GetOption(
                        'google_appid'
            ...
            if (!self::IsBitrix24()) {
                if (!isset($params['RMiblockId']) && !isset($params['VMiblockId'])) {
                    ...
                    if (!self::$pathesForSite) {
                        self::$pathesForSite = self::GetSettings(
                            array('forseGetSitePathes' => true, 'site' => self::GetSiteId())
                        );
                    }
                ...
                if (intval($RMiblockId) > 0 && CIBlock::GetPermission(
                        $RMiblockId
    ...
    public static function GetGoodTimezoneForOffset($offset)
    {
        ...
        foreach ($timezones as $tz) {
            if ($tz['offset'] == $offset) {
                ...
                if (LANGUAGE_ID == 'ru') {
                    if (preg_match(
                        '/(kaliningrad|moscow|samara|yekaterinburg|novosibirsk|krasnoyarsk|irkutsk|yakutsk|vladivostok)/i',
                        $tz['timezone_id']
                    )) {
    ...
    public static function GetTimezoneList()
    {
        if (empty(self::$timezones)) {
            ...
            foreach (self::$timezones as $k => $z) {
                self::$timezones[$k]['title'] = '(UTC' . ($z['offset'] <> 0 ? ' ' . ($z['offset'] < 0 ? '-' : '+') . sprintf(
                            "%02d",
            ...
            while ($connection = $res->Fetch()) {
                ...
                if ($connection['ACCOUNT_TYPE'] == 'caldav_google_oauth' || $connection['ACCOUNT_TYPE'] == 'google_api_oauth') {
                    ...
                    $JSConfig['caldav'][$connection['ID']] = [
                        ...
                        'syncTimestamp' => (new Type\DateTime(
                            $JSConfig['googleCalDavStatus']['sync_date'],
                ...
                if ($googleApiStatus['googleCalendarPrimaryId'] || !empty($googleApiConnections)) {
                    ...
                    foreach ($googleApiConnections as $connection) {
                        if (($connection['link'] == $serverPath || $connection['account_type'] == 'caldav_google_oauth') || $connection['account_type'] == 'google_api_oauth') {
                            ...
                            $googleApiStatus['sync_date'] = CCalendar::Date(
                                self::Timestamp($connection['sync_date']) + CCalendar::GetOffset(self::$ownerId),
                                true,
                                true,
                                true
                            );
                    ...
                    if ($addConnection) {
                        ...
                        if ($conId) {
                            ...
                            if ($connection = $res->Fetch()) {
                                ...
                                $googleApiStatus['sync_date'] = CCalendar::Date(
                                    self::Timestamp($connection['SYNCHRONIZED']) + CCalendar::GetOffset(self::$ownerId),
                                    true,
                                    true,
                                    true
                                );
                ...
                if ($googleCalDavStatus && $googleCalDavStatus['googleCalendarPrimaryId']) {
                    ...
                    foreach ($connectionList as $connection) {
                        if ($connection['link'] == $serverPath) {
                            ...
                            $googleCalDavStatus['sync_date'] = CCalendar::Date(
                                self::Timestamp($connection['sync_date']) + CCalendar::GetOffset(self::$ownerId),
                                true,
                                true,
                                true
                    ...
                    if ($addConnection) {
                        ...
                        if ($conId) {
                            ...
                            if ($connection = $res->Fetch()) {
                                ...
                                $googleCalDavStatus['last_result'] = $connection['LAST_RESULT'];
                                $googleCalDavStatus['sync_date'] = CCalendar::Date(
                                    self::Timestamp($googleApiConnections['SYNCHRONIZED']) + CCalendar::GetOffset(
                                        self::$ownerId
                                    ),
                                    true,
                                    true,
    ...
    {
        return mktime(
            date("H", $timestampTime),
            date("i", $timestampTime),
            0,
            date("m", $timestampDate),
            date("d", $timestampDate),
            date("Y", $timestampDate)
        );
    }
    ...
    public static function GetSyncInfo($userId, $syncType)
    {
        ...
        if ($result['date']) {
            $result['date'] = CCalendar::Date(
                self::Timestamp($result['date']) + CCalendar::GetOffset($userId),
                true,
                true,
                true
            );
    ...
    public static function GetMeetingSection($userId, $autoCreate = false)
    {
        ...
        foreach (
            \Bitrix\Main\EventManager::getInstance()->findEventHandlers(
                "calendar",
                "OnGetMeetingSectionForUser"
            ) as $event
        ) {
    ...
    public static function GetOriginalDate($originalDateTime, $instanceDateTime, $timeZone = false, $format = null)
    {
        ...
        $parentEvent = new Main\Type\DateTime(
            $originalDateTime,
            Main\Type\Date::convertFormatToPhp(FORMAT_DATETIME),
            $dateTimeZone
        );
        ...
        $instanceEvent = new Main\Type\DateTime(
            $instanceDateTime,
            Main\Type\Date::convertFormatToPhp(FORMAT_DATETIME),
            $dateTimeZone
        );
        ...
        $sections = CCalendar::GetSectionList(
            [
                ...
                'ADDITIONAL_IDS' => array_merge(
                    ...
                    UserSettings::getFollowedSectionIdList($userId)
                )
            ]
        );
        return $sections;
            ...
            if ($sections[$i]['SUPERPOSED']) {
                ...
                if ($type === 'user') {
                    ...
                    $path = CComponentEngine::MakePathFromTemplate(
                        $path,
                        array("user_id" => $sections[$i]['OWNER_ID'])
                    );
                    $trackingUsers[] = $sections[$i]['OWNER_ID'];
                } elseif ($type === 'group') {
                    $path = self::$pathesForSite['path_to_group_calendar'];
                    $path = CComponentEngine::MakePathFromTemplate(
                        $path,
                        array("group_id" => $sections[$i]['OWNER_ID'])
                    );
                    $trackingGroups[] = $sections[$i]['OWNER_ID'];
                } else {
                    $path = self::$pathesForSite['path_to_type_' . $type];
