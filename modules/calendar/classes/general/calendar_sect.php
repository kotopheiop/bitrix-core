...
class CCalendarSect
{
    ...
    private static function GetFields()
    {
        ...
        if (!count(self::$Fields)) {
            self::$Fields = array(
                ...
                "DAV_EXCH_CAL" => Array("FIELD_NAME" => "CS.DAV_EXCH_CAL", "FIELD_TYPE" => "string"),
                // Exchange calendar
                "DAV_EXCH_MOD" => Array("FIELD_NAME" => "CS.DAV_EXCH_MOD", "FIELD_TYPE" => "string"),
                // Exchange calendar modification label
                "CAL_DAV_CON" => Array("FIELD_NAME" => "CS.CAL_DAV_CON", "FIELD_TYPE" => "string"),
                // CalDAV connection
                "CAL_DAV_CAL" => Array("FIELD_NAME" => "CS.CAL_DAV_CAL", "FIELD_TYPE" => "string"),
                // CalDAV calendar
                "CAL_DAV_MOD" => Array("FIELD_NAME" => "CS.CAL_DAV_MOD", "FIELD_TYPE" => "string"),
        ...
        if (!$cacheEnabled || !isset($arSectionIds)) {
            ...
            foreach ($sort as $by => $order) {
                if (isset($arFields[mb_strtoupper($by)])) {
                    $byName = isset($arFields[mb_strtoupper($by)]["~FIELD_NAME"]) ? $arFields[mb_strtoupper(
                        $by
                    )]["~FIELD_NAME"] : $arFields[mb_strtoupper($by)]["FIELD_NAME"];
                    $strOrderBy .= $byName . ' ' . (mb_strtolower(
                            $order
            ...
            if (isset($filter['ADDITIONAL_IDS']) && is_array($filter['ADDITIONAL_IDS']) && count(
                    $filter['ADDITIONAL_IDS']
                ) > 0) {
                $strTypes = "";
            ...
            while ($section = $res->Fetch()) {
                ...
                if (!in_array($sectId, $arSectionIds)
                    ...
                    $section['OUTLOOK_JS'] = CCalendarSect::GetOutlookLink(
                        array(
                            'ID' => intval($sectId),
        ...
        if (($checkPermissions || $getPermissions) && count($arSectionIds) > 0 && $userId >= 0) {
            ...
            foreach ($result as $section) {
                ...
                $isManager = Loader::includeModule(
                        'intranet'
                    ) && $section['CAL_TYPE'] == 'user' && $settings['dep_manager_sub'] && Bitrix\Calendar\Util::isManagerForUser(
                        $userId,
                        $section['OWNER_ID']
                    );
                ...
                if (self::CanDo('calendar_view_time', $sectId) && !in_array($sectId, $sectIds)) {
                    ...
                    if ($sect['CAL_TYPE'] == 'user') {
                        if (isset($sect['USER_NAME'], $sect['USER_LAST_NAME'])) {
                            $sect['OWNER_NAME'] = CCalendar::GetUserName(
                                array(
                                    "NAME" => $sect['USER_NAME'],
    ...
    public static function Edit($params)
    {
        ...
        if ($id > 0 && is_array($sectionFields['ACCESS'])) {
            if (($sectionFields['CAL_TYPE'] == 'user' && $sectionFields['OWNER_ID'] == $userId) || self::CanDo(
                ...
                if (empty($sectionFields['ACCESS'])) {
                    self::SavePermissions(
                        ...
                        CCalendarSect::GetDefaultAccess(
                            $sectionFields['CAL_TYPE'],
                            $sectionFields['OWNER_ID']
                        )
                    );
                } else {
    ...
    public static function Delete($id, $checkPermissions = true)
    {
        ...
        if (count($meetingIds) > 0) {
            $DB->Query(
                "DELETE from b_calendar_event WHERE PARENT_ID in (" . implode(',', $meetingIds) . ")",
                false,
    ...
    public static function SavePermissions($sectId, $arTaskPerm)
    {
        global $DB;
        $DB->Query(
            "DELETE FROM b_calendar_access WHERE SECT_ID='" . intval($sectId) . "'",
            false,
            "FILE: " . __FILE__ . "<br> LINE: " . __LINE__
        );
    ...
    public static function GetExportLink($sectionId, $type = '', $ownerId = false)
    {
        ...
        return $params . '&ncc=1&user=' . intval($userId) . '&' . 'sec_id=' . intval(
                $sectionId
    ...
    private static function FormatICal($section, $events)
    {
        ...
        foreach ($events as $event) {
            ...
            $dtStamp = str_replace(
                'T000000Z',
                '',
                date("Ymd\THisZ", CCalendar::Timestamp($event['TIMESTAMP_X']) - $localOffset)
            );
    ...
    public static function UpdateModificationLabel($arId = [])
    {
        ...
        if ($strIds) {
            $strSql =
                ...
                $DB->PrepareUpdate(
                    "b_calendar_section",
                    array('TIMESTAMP_X' => FormatDate(CCalendar::DFormat(true), time()))
                ) .
                " WHERE ID in (" . $strIds . ")";
            $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
        }
    ...
    public static function GetDefaultAccess($type, $ownerId)
    {
        ...
        if ($type !== 'user') {
            $access['U' . CCalendar::GetUserId()] = CCalendar::GetAccessTasksByName(
                'calendar_section',
                'calendar_access'
            );
        }
    ...
    public static function HandlePermission($section = [])
    {
        if ($section && $section['ID']) {
            ...
            {
                self::$Permissions[$sectionId]['U' . $section['OWNER_ID']] = CCalendar::GetAccessTasksByName(
                    'calendar_section',
                    'calendar_access'
                );
            }
            ...
            {
                self::$Permissions[$sectionId]['SG' . $section['OWNER_ID'] . '_A'] = CCalendar::GetAccessTasksByName(
                    'calendar_section',
                    'calendar_access'
                );
            }
    ...
    public static function CleanAccessTable()
    {
        ...
        if (count($items)) {
            $DB->Query(
                "DELETE FROM b_calendar_access WHERE SECT_ID in (" . implode(',', $items) . ")",
                false,
                "FILE: " . __FILE__ . "<br> LINE: " . __LINE__
            );
        }
