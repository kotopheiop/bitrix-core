...
class CCalendarEvent
{
    ...
    public static function Edit($params = [])
    {
        ...
        if (!$isNewEvent) {
            $currentEvent = isset($params['currentEvent']) ? $params['currentEvent'] : CCalendarEvent::GetById(
                $entryFields['ID']
            );
        ...
        if (self::CheckFields($entryFields, $currentEvent, $userId)) {
                ...
                if ($isNewEvent) {
                    if (
                        ...
                        $sectionId = CCalendarSect::GetLastUsedSection(
                            $entryFields['CAL_TYPE'],
                            $entryFields['OWNER_ID'],
                            $userId
                        );
                    }
            ...
            if (!isset($entryFields['IS_MEETING']) &&
                isset($entryFields['ATTENDEES']) && is_array(
            ...
            if ($entryFields['IS_MEETING']
                ...
                if (!$entryFields['PARENT_ID']) {
                    $DB->Query(
                        "UPDATE b_calendar_event SET " . $DB->PrepareUpdate(
                            "b_calendar_event",
                            array("PARENT_ID" => $eventId)
                        ) . " WHERE ID=" . intval($eventId),
                        false,
                ...
                if ((!$entryFields['PARENT_ID'] || $entryFields['PARENT_ID'] === $eventId) && $entryFields['RECURRENCE_ID']) {
                    self::UpdateParentEventExDate(
                        $entryFields['RECURRENCE_ID'],
                        $entryFields['DATE_FROM'],
                        $entryFields['ATTENDEES']
                    );
                }
                        ...
                        if (is_array($val)) {
                            ...
                            if ($sval != '') {
                                if (Util::isSectionStructureConverted()) {
                                    $q = count(
                                        $val
                                    ) === 1 ? 'CE.SECTION_ID=' . $sval : 'CE.SECTION_ID in (' . $sval . ')';
                                } else {
    ...
    private static function GetFields()
    {
        ...
        if (!count(self::$fields)) {
            ...
            self::$fields = array(
                ...
                "DATE_FROM" => Array(
                    "FIELD_NAME" => $DB->DateToCharFunction("CE.DATE_FROM") . ' as DATE_FROM',
                    "FIELD_TYPE" => "date"
                ),
                "DATE_TO" => Array(
                    "FIELD_NAME" => $DB->DateToCharFunction("CE.DATE_TO") . ' as DATE_TO',
                    "FIELD_TYPE" => "date"
                ),
                ...
                "ORIGINAL_DATE_FROM" => Array(
                    "FIELD_NAME" => $DB->DateToCharFunction(
                ...
                "TIMESTAMP_X" => Array(
                    "FIELD_NAME" => $DB->DateToCharFunction("CE.TIMESTAMP_X") . ' as TIMESTAMP_X',
                    "FIELD_TYPE" => "date"
                ),
                "DATE_CREATE" => Array(
                    "FIELD_NAME" => $DB->DateToCharFunction("CE.DATE_CREATE") . ' as DATE_CREATE',
                    "FIELD_TYPE" => "date"
                ),
                ...
                //
                "DAV_EXCH_LABEL" => Array("FIELD_NAME" => "CE.DAV_EXCH_LABEL", "FIELD_TYPE" => "string"),
                // Exchange sync label
                "G_EVENT_ID" => Array("FIELD_NAME" => "CE.G_EVENT_ID", "FIELD_TYPE" => "string"),
                // Google event id
                "CAL_DAV_LABEL" => Array("FIELD_NAME" => "CE.CAL_DAV_LABEL", "FIELD_TYPE" => "string"),
                // CalDAV sync label
                "VERSION" => Array("FIELD_NAME" => "CE.VERSION", "FIELD_TYPE" => "string"),
                // Version used for outlook sync
    ...
    public static function GetAttendees($eventIdList = array())
    {
        ...
        if (CCalendar::IsSocNet()) {
            $eventIdList = is_array($eventIdList) ? array_map('intval', array_unique($eventIdList)) : array(
                intval(
                    $eventIdList
                )
            );
    ...
    public static function ApplyAccessRestrictions($event, $userId = false)
    {
        ...
        $isManager = (Loader::includeModule(
                    'intranet'
                ) && $event['CAL_TYPE'] == 'user' && $settings['dep_manager_sub']) && Bitrix\Calendar\Util::isManagerForUser(
                $userId,
                $event['OWNER_ID']
            );
        ...
        if ($private || (!CCalendarSect::CanDo(
                    'calendar_view_full',
                    $sectId,
                    $userId
                ) && !$isManager && !$isAttendee)) {
            if ($private) {
                ...
                if (!CCalendarSect::CanDo('calendar_view_title', $sectId, $userId)) {
                    if (CCalendarSect::CanDo('calendar_view_time', $sectId, $userId)) {
                        $event['NAME'] = '[' . GetMessage(
                                'EC_ACCESSIBILITY_' . mb_strtoupper($event['ACCESSIBILITY'])
                            ) . ']';
                    } else {
                        return false;
                    }
                } else {
                    $event['NAME'] = $event['NAME'] . ' [' . GetMessage(
                            'EC_ACCESSIBILITY_' . mb_strtoupper($event['ACCESSIBILITY'])
                        ) . ']';
                }
            }
    ...
    private static function PreHandleEvent($item, $params = array())
    {
        ...
        if (isset($item['UF_CRM_CAL_EVENT']) && is_array($item['UF_CRM_CAL_EVENT']) && count(
            ...
            if ($rrule['FREQ'] == 'WEEKLY') {
                ...
                if ($rrule['BYDAY'][$weekDay]) {
                    if (($preciseLimits && $toTS >= $limitFromTSReal) || (!$preciseLimits && $toTS > $limitFromTS - $h24)) {
                        ...
                        $event['DATE_TO'] = CCalendar::Date(
                            $toTS - ($event['TZ_OFFSET_FROM'] - $event['TZ_OFFSET_TO']),
                            !$skipTime,
                            false
                        );
    ...
    public static function ParseRRULE($rule = '')
    {
            ...
            if ($arPar[0]) {
                switch ($arPar[0]) {
                    ...
                    case 'INTERVAL':
                        ...
                        break;
                    case 'UNTIL':
                        $res['UNTIL'] = CCalendar::Timestamp($arPar[1]) ? $arPar[1] : CCalendar::Date(
                            intval($arPar[1]),
                            false,
                            false
                        );
        ...
        if ($res['FREQ'] == 'WEEKLY' && (!isset($res['BYDAY']) || !is_array($res['BYDAY']) || count(
                    $res['BYDAY']
                ) == 0)) {
    ...
    public static function CheckFields(&$arFields, $currentEvent = array(), $userId = false)
    {
        ...
        // Check rrules
        if (is_array($arFields['RRULE']) && isset($arFields['RRULE']['FREQ']) && in_array(
                $arFields['RRULE']['FREQ'],
                array(
                    'HOURLY',
                    'DAILY',
        ...
        foreach ($fieldList as $fieldKey) {
            if ($fieldKey == 'LOCATION') {
                if (is_array($newFields[$fieldKey]) && $newFields[$fieldKey]['NEW'] != $currentFields[$fieldKey]) {
                    ...
                    if (!is_array($newFields[$fieldKey]) && $newFields[$fieldKey] != $currentFields[$fieldKey]
                        && CCalendar::GetTextLocation($newFields["LOCATION"]) !== CCalendar::GetTextLocation(
                    ...
                    if ($fieldKey == 'DATE_TO') {
                        ...
                        if ($fieldKey == 'IMPORTANCE') {
                            ...
                            if ($fieldKey == 'DESCRIPTION') {
                                if (mb_strtolower(trim($newFields[$fieldKey])) != mb_strtolower(
                                        trim($currentFields[$fieldKey])
                                    )) {
                                    $changes[] = [
                                        'fieldKey' => $fieldKey,
                                        'oldValue' => $currentFields[$fieldKey],
                                ...
                                if ($fieldKey == 'RRULE') {
                                    ...
                                    if ($newFields[$fieldKey] !== $currentFields[$fieldKey]) {
                                        $changes[] = [
                                            'fieldKey' => $fieldKey,
                                            'oldValue' => $currentFields[$fieldKey],
                                            'newValue' => $newFields[$fieldKey]
        ...
        foreach ($attendees as $userKey) {
            ...
            if ($attendeeId
                ...
                if ($userIndex[$attendeeId]
                    ...
                    if (empty($attaches)) {
                        $isChangeFiles = false;
                        $attaches = Helper::getMailAttaches(
                            $params['UF'],
                    ...
                    $additionalChildArFields['ICAL_ORGANIZER'] = self::getOrganizerForIcal(
                        $userIndex,
                        (int)$childParams['arFields']['MEETING_HOST'],
                        $sender['EMAIL']
                    );
...
//					$sender = $currentAttendeesIndex[$arFields['MEETING_HOST']];
                    ...
                    $additionalChildArFields['ICAL_ORGANIZER'] = self::getOrganizerForIcal(
                        $currentAttendeesIndex,
                        (int)$arFields['MEETING_HOST'],
                        $sender['EMAIL']
                    );
                    $additionalChildArFields['ICAL_ATTENDEES'] = self::createMailAttendeesCollection(
                        [$declinedUser['ID'] => $declinedUser],
                        false
                    );
    ...
    public static function GetEventFromToForUser($params, $userId)
    {
        ...
        if (!$skipTime) {
            $fromTs = $fromTs - (CCalendar::GetTimezoneOffset($params['TZ_FROM']) - CCalendar::GetCurrentOffsetUTC(
                        $userId
                    ));
            ...
            if ($entry) {
                if ($entry['IS_MEETING'] && $entry['PARENT_ID'] !== $entry['ID']) {
                    ...
                    if ($entry['IS_MEETING']) {
                        ...
                        foreach ($childEvents as $chEvent) {
                            ...
                            if ($chEvent["MEETING_STATUS"] !== "N" && $sendNotification) {
                                if ($chEvent['DATE_TO_TS_UTC'] + date("Z", $chEvent['DATE_TO_TS_UTC']) > (time(
                                        ) - 60 * 5)) {
                            ...
                            if (ICalUtil::isMailUser($chEvent['OWNER_ID']) && !$parentEvent) {
                                ...
                                $icalManagersCollection[] = SenderCancelInvitation::createInstance(
                                    array_merge(
                                        self::prepareChildParamsForIcalInvitation($chEvent),
                                        $additionalChildArFields
                                    ),
                                    IcalMailContext::createInstance(
                        ...
                        } else // Actual deleting
                        {
                            ...
                            if (!Util::isSectionStructureConverted()) {
                                ...
                                if (count($chEventIds) > 0) {
                                    $DB->Query(
                                        "DELETE FROM b_calendar_event_sect WHERE EVENT_ID in (" . $strChEvent . ")",
                                        false,
                                        "FILE: " . __FILE__ . "<br> LINE: " . __LINE__
                                    );
                                }
                    ...
                    if ($params['bMarkDeleted']) {
                        ...
                        // Real deleting
                        $DB->Query(
                            "DELETE from b_calendar_event WHERE ID=" . $id,
                            false,
                            "File: " . __FILE__ . "<br>Line: " . __LINE__
                        );
                        ...
                        if (!Util::isSectionStructureConverted()) {
                            $DB->Query(
                                "DELETE FROM b_calendar_event_sect WHERE EVENT_ID=" . $id,
                                false,
                                "FILE: " . __FILE__ . "<br> LINE: " . __LINE__
                            );
                        }
            ...
            if ($event['MEETING'] && $event['MEETING']['OPEN'] && ($status === 'Y' || $status === 'M')) {
                ...
                if (!$attendeeExist && is_array($event)) {
                    ...
                    $DB->Query(
                        ...
                        " WHERE PARENT_ID=" . intval($event['PARENT_ID']),
                        false,
                        "FILE: " . __FILE__ . "<br> LINE: " . __LINE__
                    );
    ...
    public static function GetMeetingStatus($userId, $eventId)
    {
        ...
        if ($event && $event['IS_MEETING'] && intval($event['PARENT_ID']) > 0) {
            if ($event['CREATED_BY'] == $userId) {
                ...
                $res = $DB->Query(
                    "SELECT MEETING_STATUS from b_calendar_event WHERE PARENT_ID=" . intval(
                        $event['PARENT_ID']
    ...
    public static function GetAbsent($users = false, $params = array())
    {
        ...
        foreach ($arEvents as $event) {
            ...
            if ($checkPermissions
                ...
                $private = $event['PRIVATE_EVENT'] && $event['CAL_TYPE'] == 'user';
                $isManager = (!$private && CCalendar::IsIntranetEnabled() && Loader::includeModule(
                            'intranet'
                        ) && $event['CAL_TYPE'] == 'user' && $settings['dep_manager_sub']) && Bitrix\Calendar\Util::isManagerForUser(
                        $curUserId,
                        $event['OWNER_ID']
    ...
    public static function DeleteEmpty()
    {
        ...
        if (count($itemIds) > 0) {
            $DB->Query(
                "DELETE FROM b_calendar_event WHERE ID in (" . implode(',', $itemIds) . ")",
                false,
                "FILE: " . __FILE__ . "<br> LINE: " . __LINE__
            );
        }
    ...
    {
        if (is_array($event['RELATIONS']) && array_key_exists(
                'COMMENT_XML_ID',
                $event['RELATIONS']
        ...
        if ($event['RRULE']) {
                ...
                case 'WEEKLY':
                    ...
                    if ($event['RRULE']['INTERVAL'] == 1) {
                        ...
                        $res = GetMessage(
                            ...
                            array('#WEEK#' => $event['RRULE']['INTERVAL'], '#DAYS_LIST#' => $daysList)
                        );
                    }
                    break;
                case 'MONTHLY':
            ...
            if ($showUntil && $event['RRULE']['UNTIL'] != CCalendar::GetMaxDate()) {
                $res .= ' ' . GetMessage(
                        'EC_RRULE_UNTIL',
                        array(
                            '#UNTIL_DATE#' => CCalendar::Date(
                                CCalendar::Timestamp($event['RRULE']['UNTIL']),
                                false
                            )
                        )
                    );
            ...
            if (is_array($event['ATTENDEE_LIST'])) {
                foreach ($event['ATTENDEE_LIST'] as $attendee) {
                    if ($attendee['status'] == 'Y') {
                        if ($event['DT_SKIP_TIME'] !== 'Y') {
                            $excludeDate = CCalendar::Date(
                                CCalendar::DateWithNewTime(CCalendar::Timestamp($event['DATE_FROM']), $excludeDateTs)
                            );
                        }
            ...
            if ($entry['IS_MEETING']) {
                ...
                if (!empty($entry['ATTENDEES_CODES'])) {
                    ...
                    $content .= ' ' . static::prepareToken(
                            join(' ', Bitrix\Socialnetwork\Item\LogIndex::getEntitiesName($attendeesCodes))
                        );
                }
            } else {
     ...
     */
    private static function createMailAttendeesCollection(
        iterable $userIndex,
        bool $hideGuests = true,
        array $attendeeIds = [],
        ?array $attendeesId = null
    ): AttendeesCollection {
