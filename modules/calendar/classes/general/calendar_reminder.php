...
class CCalendarReminder
{
    ...
    public static function ReminderAgent(
        ...
        if ($eventId > 0 && $userId > 0 && \Bitrix\Main\Loader::includeModule("im")) {
            ...
            if ($event && $event['MEETING_STATUS'] !== 'N') {
                ...
                CIMNotify::Add(
                    CCalendarReminder::getNotifyFields(
                        [
                            'userId' => $userId,
                            'entryId' => $eventId,
    ...
    public static function getNotifyFields($params = [])
    {
        ...
        $notifyFields['MESSAGE'] = GetMessage(
            'EC_EVENT_REMINDER_1',
            [
        ...
        $notifyFields["PUSH_MESSAGE"] = GetMessage(
        ...
        $notifyFields["PUSH_MESSAGE"] = mb_substr(
            $notifyFields["PUSH_MESSAGE"],
            0,
            \CCalendarNotify::PUSH_MESSAGE_MAX_LENGTH
        );
    ...
    public static function AddAgent($remindTime, $params)
    {
        ...
        if ($remindTime <> '' && $DB->IsDate($remindTime, false, LANG, "FULL")) {
            ...
            CAgent::AddAgent(
                "CCalendar::ReminderAgent(" . intval($params['eventId']) . ", " . intval(
                    $params['userId']
                ) . ", '" . addslashes($params['viewPath']) . "', '" . addslashes(
                    $params['calendarType']
                ) . "', " . intval($params['ownerId']) . $indexParam . ");",
                "calendar",
                "Y",
                0,
                "",
    ...
    public static function updateReminders($params = [])
    {
        ...
        $path = CHTTP::urlDeleteParams(
            $path,
            ["action", "sessid", "bx_event_calendar_request", "EVENT_ID", "EVENT_DATE"]
        );
    ...
    private static function getReminderTimestamp($eventTimestamp, $reminder, $timezoneName = null)
    {
        ...
        if (is_array($reminder) && isset($reminder['type'])) {
            ...
            if (in_array($type, self::SIMPLE_TYPE_LIST)) {
                ...
                $reminderTimestamp = mktime(
                    $hour,
                    $min,
                    0,
                    date("m", $eventTimestamp),
                    date("d", $eventTimestamp) - $daysBefore,
                    date("Y", $eventTimestamp)
                );

                if ($timezoneName) {
                    $timezoneServerOffset = \CCalendar::GetTimezoneOffset($timezoneName, $eventTimestamp) - date(
                            "Z",
                            $eventTimestamp
                        );
                ...
                $reminderTimestamp = $reminderTimestamp - \CCalendar::GetTimezoneOffset(
                        $timezoneName,
                        $reminderTimestamp
                    ) + date("Z", $reminderTimestamp);
            }
        }
                ...
                if ($value['type'] == 'min') {
                    ...
                    if (!$value['text']) {
                        $value['text'] = Loc::getMessage(
                            ...
                            array('#COUNT#' => intval($value['count']))
                        );
                    }
