...
class CCalendarRequest
{
    ...
    public static function Process($action = '', CCalendar $calendar)
    {
        ...
        if ($action == 'export') {
            ...
            if ($_GET['check'] == 'Y') // Just for access check from calendar interface
            {
                ...
                if (CCalendarSect::CheckSign(
                    $_GET['sign'],
                    intval($_GET['user']),
                    $sectId > 0 ? $sectId : 'superposed_calendars'
                )) {
    ...
    public static function deleteEntry()
    {
        ...
        $res = CCalendar::DeleteEvent(
            intval(self::$request['entry_id']),
            true,
    ...
    public static function SetStatus()
    {
        CCalendarEvent::SetMeetingStatusEx(
            array(
                ...
                'reccurentMode' => in_array(
                    $_REQUEST['reccurent_mode'],
                    array('this', 'next', 'all')
                ) ? $_REQUEST['reccurent_mode'] : false,
    ...
    public static function UpdatePlanner()
    {
        ...
        $result = CCalendarPlanner::PrepareData(
            array(
                'entry_id' => $curEventId,
                'user_id' => $curUserId,
                'codes' => $codes,
        ...
        if ($event) {
            ...
            if ($event["RECURRENCE_ID"] > 0) {
                ...
                if ($recParentEvent && CCalendarEvent::CheckRecurcion($recParentEvent)) {
                    ...
                    if ($recParentEvent['RRULE']['UNTIL'] && CCalendar::Timestamp(
    ...
    public static function getFilterData()
    {
        $fields = CalendarFilter::resolveFilterFields(
            CalendarFilter::getFilterId(CCalendar::GetType(), CCalendar::GetOwnerId(), CCalendar::GetCurUserId())
        );
        ...
        if (isset($fields['search']) && $fields['search']) {
            $arFilter[(CCalendarEvent::isFullTextIndexEnabled(
            ) ? '*' : '*%') . 'SEARCHABLE_CONTENT'] = CCalendarEvent::prepareToken($fields['search']);
        }
