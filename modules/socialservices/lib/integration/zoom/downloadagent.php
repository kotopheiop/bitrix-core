    ...
    public static function attach($activityId, array $recordingFields, $siteId = SITE_ID)
    {
        ...
        if (!preg_match($validSymbolsPattern, $recordingFields['EXTERNAL_ID'])) {
            return $result->addError(
                new Error('Can not create recording file name, external_id contains invalid symbols')
            );
        }
        ...
        $downloadResult = static::download(
            $recordingFields['DOWNLOAD_URL'],
            $recordingFields['DOWNLOAD_TOKEN'],
            $fileName
        );
        ...
        ZoomMeetingRecordingTable::update(
            $recordingFields['ID'],
            [
                'FILE_ID' => $fileId
            ]
