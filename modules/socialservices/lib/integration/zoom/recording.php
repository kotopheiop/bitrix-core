...
class Recording
{
    ...
    public static function getRecordings($conferenceId): Result
    {
        ...
        if ($meetingData = $meetingResult->fetch()) {
            ...
            while ($recording = $recordingsResult->fetch()) {
                ...
                if ($recording['FILE_ID'] > 0 && Loader::includeModule('disk') && $file = \Bitrix\Disk\File::loadById(
                        $recording['FILE_ID']
                    )) {
                    $recording['DOWNLOAD_URL'] = \Bitrix\Disk\Driver::getInstance()->getUrlManager(
                    )->getUrlForDownloadFile($file, true);
                } else {
                    $parsedDownloadUrl = new Uri($recording['DOWNLOAD_URL']);
                    $recording['DOWNLOAD_URL'] = $parsedDownloadUrl->addParams(
                        ['access_token' => $recording['DOWNLOAD_TOKEN']]
                    )->__toString();
                }
    ...
    public static function onRecordingStopped(int $conferenceId, array $recordingsData): Result
    {
        ...
        $updateResult = ZoomMeetingTable::update(
            $conferenceRecord['ID'],
            [
                'HAS_RECORDING' => 'Y'
            ]
