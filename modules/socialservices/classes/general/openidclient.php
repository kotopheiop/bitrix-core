...
class COpenIDClient
{
    ...
    public function GetRedirectUrl($identity, $return_to = false)
    {
        ...
        if (mb_strlen($identity) > 1024) {
            $identity = mb_substr($identity, 0, 1024);
        } // may be 256 ????

        if (mb_strpos(mb_strtolower($identity), 'http://') === false && mb_strpos(
                mb_strtolower($identity),
                'https://'
            ) === false) {
        ...
        if ($arOpenidServerTags = $this->GetOpenIDServerTags($identity)) {
            ...
            if ($return_to === false) {
                $return_to = $server_name . $GLOBALS['APPLICATION']->GetCurPageParam(
                        '',
                        array('SEF_APPLICATION_CUR_PAGE_URL'),
                        false
                    );
            }
            ...
            $url = $arOpenidServerTags['server'] . (mb_strpos(
                    $arOpenidServerTags['server'],
                    '?'
                ) !== false ? '&' : '?') .
    ...
    public static function CleanParam($state = false)
    {
        ...
        $redirect_url = $GLOBALS['APPLICATION']->GetCurPageParam(
            ($state == 'ERROR' ? 'auth_service_error=1' : ''),
            $arKillParams,
            false
        );
    ...
    public function Authorize()
    {
        ...
        if ($arOpenID = $this->Validate()) {
            ...
            if (array_key_exists(
                    'openid_sreg_gender',
                    $_GET
                ) && ($_GET['openid_sreg_gender'] == 'M' || $_GET['openid_sreg_gender'] == 'F')) {
                $arFields['PERSONAL_GENDER'] = $_GET['openid_sreg_gender'];
            }

            if (array_key_exists('openid_sreg_fullname', $_GET)) {
                $fullname = (defined("BX_UTF") ? $_GET['openid_sreg_fullname'] : CharsetConverter::ConvertCharset(
                    $_GET['openid_sreg_fullname'],
                    'UTF-8',
                    LANG_CHARSET
                ));
            ...
            if (array_key_exists('openid_sreg_dob', $_GET)) {
                $arFields['PERSONAL_BIRTHDAY'] = CDatabase::FormatDate(
                    $_GET['openid_sreg_dob'],
                    "YYYY-MM-DD",
                    FORMAT_DATE
                );
            }
            ...
            if ($GLOBALS["USER"]->IsAuthorized() && $GLOBALS["USER"]->GetID()) {
                ...
                $dbUsersOld = $GLOBALS["USER"]->GetList(
                    $by,
                    $ord,
                    array(
                ...
                $dbUsersNew = $GLOBALS["USER"]->GetList(
                    $by,
                    $ord,
                    array(
                        'XML_ID' => $arFields['XML_ID'],
    ...
    public static function GetOpenIDAuthStep($request_var = 'OPENID_IDENTITY')
    {
        if (array_key_exists('openid_mode', $_GET) && $_GET['openid_mode'] == 'id_res') {
            return 2;
        } elseif ($_SERVER['REQUEST_METHOD'] == 'POST' && array_key_exists($request_var, $_REQUEST) && mb_strlen(
