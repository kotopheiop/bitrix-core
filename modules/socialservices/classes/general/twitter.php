...
class CSocServTwitter extends CSocServAuth
{
    ...
    public function GetSettings()
    {
        return array(
            ...
            array(
                "note" => GetMessage(
                    "socserv_tw_sett_note",
                    array('#URL#' => \CHTTP::URN2URI("/bitrix/tools/oauth/twitter.php"))
                )
            ),
        );
    }
    ...
    public function GetFormHtml($arParams)
    {
        ...
        $arParams['BACKURL'] = $APPLICATION->GetCurPageParam(
            '',
            array(
                "logout",
                "auth_service_error",
        ...
        return '<a href="javascript:void(0)" onclick="BX.util.popup(\'' . htmlspecialcharsbx(
                CUtil::JSEscape($url)
            ) . '\', 800, 450)" class="bx-ss-button twitter-button"></a><span class="bx-spacer"></span><span>' . $phrase . '</span>';
    }
    ...
    public function getUrl($arParams)
    {
        ...
        return '/bitrix/tools/oauth/twitter.php?check_key=' . $_SESSION["UNIQUE_KEY"] . (isset($arParams['BACKURL']) ? "&backurl=" . urlencode(
                    urlencode($arParams['BACKURL'])
                ) : '');
    }
    ...
    public function Authorize()
    {
        ...
        if ($bSuccess) {
            ...
            $url = isset($_REQUEST['backurl']) ? $_REQUEST['backurl'] : $GLOBALS['APPLICATION']->GetCurPageParam(
                ($bSuccess === true ? '' : 'auth_service_id=' . self::ID . '&auth_service_error=' . $bSuccess),
                $aRemove
            );
            if (CModule::IncludeModule("socialnetwork") && mb_strpos($url, "current_fieldset=") === false) {
                $url = (preg_match(
                    "/\?/",
                    $url
                )) ? $url . "&current_fieldset=SOCSERV" : $url . "?current_fieldset=SOCSERV";
            }
            ...
            if (is_array($res) && is_array($res['users'])) {
                foreach ($res['users'] as $key => $contact) {
                    ...
                    if ($contact['profile_image_url']) {
                        ...
                        $res['users'][$key]['picture'] = preg_replace(
                            "/_normal\./i",
                            ".",
...
class CTwitterInterface
{
    ...
    public function RedirectAuthUrl()
    {
        ...
        LocalRedirect(
            self::AUTH_URL . "?oauth_token=" . urlencode($this->token) . '&check_key=' . $_SESSION["UNIQUE_KEY"]
    ...
    public function GetUserInfo($user_id)
    {
        ...
        $arParams["oauth_signature"] = urlencode(
            $this->BuildSignature($this->GetSignatureString($arParams, self::API_URL))
        );
    ...
    public function getUserFriends($user_id, $limit, &$next)
    {
        ...
        $arParams["oauth_signature"] = urlencode(
            $this->BuildSignature($this->GetSignatureString($arParams, self::FRIENDS_URL))
        );
    ...
    public function sendMessage($user_id, $uid, $message)
    {
        ...
        $arParams["oauth_signature"] = urlencode(
            $this->BuildSignature($this->GetSignatureString($arParams, self::MESSAGE_URL))
        );
    ...
    public function SearchByHash($hash, $socServUserArray, $sinceId)
    {
        ...
        $arParams["oauth_signature"] = urlencode(
            $this->BuildSignature($this->GetSignatureString($arParams, self::SEARCH_URL))
        );
        $arHeaders = array(
            "Authorization" => 'OAuth oauth_consumer_key="' . $arParams["oauth_consumer_key"] . '", oauth_nonce="' . $arParams["oauth_nonce"] . '", oauth_signature="' . $arParams["oauth_signature"] . '", oauth_signature_method="HMAC-SHA1", oauth_timestamp="' . $arParams["oauth_timestamp"] . '", oauth_token="' . $this->token . '", oauth_version="1.0"',
            "Content-type" => "application/x-www-form-urlencoded",
        ...
        $result = @CHTTP::sGetHeader(
            self::SEARCH_URL . "?count=100&include_entities=false&q=" . urlencode($hash) . "&since_id=" . $sinceId,
            $arHeaders,
            $this->httpTimeout
        );
        if ($result) {
    ...
    private function GetAllPages($arResult)
    {
        ...
        $arParams = array_merge(
            array(
                "count" => $searchMetaData["count"],
                "include_entities" => $searchMetaData["include_entities"],
        ...
        $result = CHTTP::sGetHeader(
            self::SEARCH_URL . "?count=" . $searchMetaData["count"] . "&include_entities=" . $searchMetaData["include_entities"] . "&max_id=" . $searchMetaData["max_id"] . "&q=" . urlencode(
                $searchMetaData["q"]
            ),
            $arHeaders,
            $this->httpTimeout
    ...
    public function SendTwit($socServUserId, $message, $messageId)
    {
        ...
        $arParams["oauth_signature"] = urlencode(
            $this->BuildSignature($this->GetSignatureString($arParams, $this::POST_URL))
        );
