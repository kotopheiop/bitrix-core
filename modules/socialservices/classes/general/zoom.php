...
class CSocServZoom extends CSocServAuth
{
    ...
    public function GetFormHtml($arParams)
    {
        ...
        return $arParams['FOR_INTRANET']
            ...
            : '<a href="javascript:void(0)" onclick="BX.util.popup(\'' . htmlspecialcharsbx(
                CUtil::JSEscape($url)
            ) . '\', 700, 700)" class="bx-ss-button zoom-button"></a><span class="bx-spacer"></span><span>' . $phrase . '</span>';
    }
    ...
    public function getUrl($arParams): string
    {
        ...
        if (defined('BX24_HOST_NAME') && IsModuleInstalled('bitrix24')) {
            ...
            $state = $this->getEntityOAuth()->GetRedirectURI() .
                urlencode(
                    '?state=' . JWT::urlsafeB64Encode('backurl=' . $backurl . '&check_key=' . $_SESSION['UNIQUE_KEY'])
                );
        } else {
    ...
    public function createConference($params)
    {
        ...
        $conference['join_url'] = $this->attachPasswordToUrl(
            $conferenceData['join_url'],
            $conferenceData['encrypted_password']
        );
    ...
    public function GetAccessToken($redirect_uri = ''): bool
    {
        ...
        if (is_array($token)) {
            ...
            if (!$this->code) {
                ...
                if (isset($token['REFRESH_TOKEN']) && $this->getNewAccessToken(
                        $token['REFRESH_TOKEN'],
                        $this->userId,
                        true
                    )) {
