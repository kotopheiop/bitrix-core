...
$aTabs = array(
    ...
    array(
        "DIV" => "edit3",
        "TAB" => GetMessage("SOC_OPT_CRYPTO_TAB_TITLE"),
        "ICON" => "",
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["activate_crypto_tokens"] <> '' && check_bitrix_sessid()) {
    ...
    LocalRedirect(
        ...
        ) . ($_REQUEST["siteTabControl_active_tab"] <> '' ? "&siteTabControl_active_tab=" . urlencode(
                $_REQUEST["siteTabControl_active_tab"]
            ) : '')
    );
}
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["Update"] . $_POST["Apply"] . $_POST["RestoreDefaults"] <> '' && check_bitrix_sessid(
    ...
    if ($_POST["RestoreDefaults"] <> '') {
        ...
        foreach ($arSiteList as $site) {
            ...
            COption::SetOptionString(
                ...
                serialize($_POST["AUTH_SERVICES" . $suffix])
            );
            COption::SetOptionString(
                "socialservices",
                "twitter_search_hash" . $suffix,
                $_POST["twitter_search_hash" . $suffix]
            );
    ...
    if (isset($_REQUEST["allow_registration"])) {
        COption::SetOptionString(
            "socialservices",
            "allow_registration",
            $_REQUEST["allow_registration"] == "N" ? "N" : "Y"
        );
    }
        ...
        return function (s) {
            if (BX.type.isString(s)) {
                    ...
                    if (res.error) {
                        ...
                        BX.adjust(BX('b24net_out'), {
                            children: [
                                BX.create('DIV', {
                                    html: '<?=GetMessageJS(
                                        'SOC_OPT_B24NET_CLIENT_ID'
                                    );?>: <b>' + BX.util.htmlspecialchars(res.client_id) + '</b><br /><?=GetMessageJS(
                                        'SOC_OPT_B24NET_CLIENT_SECRET'
                                    );?>: <b>' + BX.util.htmlspecialchars(res.client_secret) + '</b><br ><br />'
                                }),
                                BX.create('INPUT', {
                ...
                if (!document.forms.socserv_settings['bitrix24net_domain' + currentSite]) {
                    document.forms.socserv_settings.appendChild(BX.create('input', {
                        attrs: {
                            'type': 'hidden',
                ...
                if (!p) {
                    p = new BX.PopupWindow("socservB24NetPopup", null, {
                        ...
                        content: BX.create('DIV', {
                            ...
                            html: '<div id="b24net_in"><?=GetMessageJS(
                                    'SOC_OPT_B24NET_SITE'
                                ...
                                '<input type="button" onclick="networkRegister(BX(\'b24net_url\').value)" value="<?=GetMessageJS(
                                    'SOC_OPT_B24NET_GET'
                                )?>">' +
...
<form method="post" name="socserv_settings"
      action="<? echo $APPLICATION->GetCurPage() ?>?mid=<?= urlencode($module_id) ?>&amp;lang=<?= urlencode(
          LANGUAGE_ID
      ) ?>">
    <?
    $tabControl->Begin();
    ...
    <tr>
        <td colspan="2" style="padding-bottom: 20px;">
            ...
            $aSiteTabs = array(
                array(
                    "DIV" => "opt_common",
                    "TAB" => GetMessage("socserv_sett_common"),
                    'TITLE' => GetMessage("socserv_sett_common_title"),
            ...
            foreach ($arSites as $arSite) {
                $aSiteTabs[] = array(
                    ...
                    'TITLE' => GetMessage("socserv_sett_site") . ' [' . $arSite["ID"] . '] ' . htmlspecialcharsbx(
                            $arSite["NAME"]
                        ),
                    'ONSELECT' => "document.forms['socserv_settings'].siteTabControl_active_tab.value='opt_site_" . $arSite["ID"] . "'"
                );
            }
            ...
            foreach ($arSiteList as $site):
                ...
                $twitHashInput = "<input type=\"text\" name=\"twitter_search_hash" . $suffix . "\" id=\"twitter_search_hash" . $suffix . "\" size=15 value=\"" . htmlspecialcharsbx(
                        $hash
                    ) . "\">";
                ...
                <table cellpadding="0" cellspacing="0" border="0" width="100%" class="edit-table">
                    <tr>
                        <td width="50%" class="field-name"><label for="use_on_sites<?= $suffix ?>"><? echo GetMessage(
                                "socserv_sett_site_apply"
                            ) ?></td>
                        <td width="50%" style="padding-left:7px;">
                            <input type="hidden" name="use_on_sites[<?= htmlspecialcharsbx($site) ?>]" value="N">
                            <input type="checkbox" name="use_on_sites[<?= htmlspecialcharsbx($site) ?>]"
                                   value="Y"<? if ($arUseOnSites[$site] == "Y") echo ' checked' ?>
                                   id="use_on_sites<?= $suffix ?>"
                                   onclick="BX('site_settings<?= $suffix ?>').style.display=(this.checked? '':'none');">
                        </td>
                    </tr>
                </table>
            <?endif ?>
                <table cellpadding="0" cellspacing="0" border="0" class="edit-table" width="100%"
                    ...
                    <? if ($site == ''):?>
                        <? if (IsModuleInstalled("timeman")):?>
                            <tr>
                                <td> <?= GetMessage("soc_serv_send_activity"); ?> </td>
                                <td>
                                    <input type="checkbox" name="allow_send_user_activity" id="allow_send_user_activity"
                                           value="Y"
                        ...
                        <?endif; ?>
                        <tr>
                            <td> <?= str_replace("#hash#", $twitHashInput, GetMessage("socserv_twit_to_buzz")) ?> </td>
                            <td>
                                <input type="checkbox" name="get_message_from_twitter" id="get_message_from_twitter"
                                       value="Y"
                                    <? if (COption::GetOptionString(
                                            "socialservices",
                                            "get_message_from_twitter",
                                            "N"
                                        ) == 'Y') {
                                        echo " checked";
                                    } elseif (COption::GetOptionString(
                    ...
                    <tr valign="top">
                        ...
                        <td width="50%">
                            <table cellpadding="0" style="width:45%;" cellspacing="3" border="0" width=""
                                   class="padding-0">
                                <?
                                $dontShowForUkraine = [
                                    \CSocServMyMailRu::ID,
                                ...
                                foreach ($allowedServices as $id => $service):
                                    ...
                                    <tr>
                                        <td style="padding-top: 3px;">
                                            <input type="hidden"
                                                   name="AUTH_SERVICES<?= $suffix ?>[<?= htmlspecialcharsbx($id) ?>]"
                                                   value="N">
                                            <input type="checkbox"
                                                   name="AUTH_SERVICES<?= $suffix ?>[<?= htmlspecialcharsbx($id) ?>]"
                                        ...
                                        <td>
                                            <div class="bx-ss-icon <?= htmlspecialcharsbx($service["ICON"]) ?>"></div>
                                        </td>
                                        <td><label for="AUTH_SERVICES<?= $suffix ?><?= htmlspecialcharsbx(
                                                $id
                                            ) ?>"><?= htmlspecialcharsbx($service["NAME"]) ?></label></td>
                                        <td>&nbsp;</td>
                                        <td><a href="javascript:void(0)" onclick="MoveRowUp(this)"><img
                                                        src="/bitrix/images/socialservices/up.gif" width="16"
                                                        height="16" alt="<? echo GetMessage("soc_serv_opt_up") ?>"
                                                        border="0"></a></td>
                                        <td><a href="javascript:void(0)" onclick="MoveRowDown(this)"><img
                                                        src="/bitrix/images/socialservices/down.gif" width="16"
                                                        height="16" alt="<? echo GetMessage("soc_serv_opt_down") ?>"
                                                        border="0"></a></td>
                                    </tr>
                                <?
                                endforeach;
    ...
    <tr>
        ...
        <td>
            ...
            <input type="checkbox" name="allow_registration" id="allow_registration"
                   value="Y"<?= $allowAuthorization ? ' checked="checked"' : '' ?> />
        </td>
    </tr>
    <tr>
        ...
        <td>
            <select name="group_deny_auth[]" size="5" multiple="multiple">
                ...
                foreach ($groups as $groupId => $groupTitle) {
                    ?>
                    <option value="<?= $groupId ?>"<?= in_array(
                        $groupId,
                        $groupDenyAuth
                    ) ? ' selected="selected"' : '' ?>><?= Converter::getHtmlConverter()->encode(
                            $groupTitle
                        ) ?></option>
                    <?
                }
    ...
    <tr>
        ...
        if (COption::GetOptionString("socialservices", "allow_encrypted_tokens", false)) {
            ...
            if (\Bitrix\Main\Entity\CryptoField::cryptoAvailable()) {
                ?>
                <td colspan="2">
                    <input type="submit" value="<?= GetMessage('SOC_OPT_CRYPTO_ACTIVATE') ?>"
                           name="activate_crypto_tokens" class="adm-btn-green"
                           onclick="return confirmTokensEncryption()"/>
                    <?= BeginNote(); ?>
