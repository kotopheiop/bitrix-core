...
if (!$bBadCourse):
    ...
    function CustomizeEditor()
    {
        ...
        <div class="bxed-dialog">
            <table class="bx-image-dialog-tbl">
                ...
                <tr>
                    <td class="bx-par-title"><?
                        echo GetMessage("LEARNING_WIDTH") ?>:
                    </td>
                    <td width="80px"><input type="text" size="3" id="mediaWidth"/></td>
                    <td><?
                        echo GetMessage("LEARNING_HEIGHT") ?>:
                    </td>
        ...
        <script type="text/javascript">
            ...
            function _mediaParser(_str, pMainObj) {
                ...
                var ReplaceWMV = function (str, b1, b2, b3, b4) {
                    ...
                    return '<img  id="' + pMainObj.SetBxTag(false, {
                        tag: 'media',
                        params: arTagParams
                    }) + '" src="/bitrix/images/1.gif" style="border: 1px solid rgb(182, 182, 184); background-color: rgb(226, 223, 218); background-image: url(/bitrix/images/learning/icons/media.gif); background-position: center center; background-repeat: no-repeat; width: ' + w + 'px; height: ' + h + 'px;" width="' + w + '" height="' + h + '" />';
                }
                ...
                var ReplaceFLV = function (str, attr) {
                    ...
                    arParams.flashvars.replace(/(\w+?)=((?:\s|\S)*?)&/ig, function (s, name, val) {
                        arFlashvars[name] = val;
                        return '';
                    });
                ...
                'action': function () {
                    ...
                    if (path.length > 0 && parseInt(width) > 0 && parseInt(height) > 0) {
                        if (pElement && pElement.getAttribute && pElement.getAttribute("id")) {
                            ...
                            pEditor.insertHTML('<img id="' + pEditor.SetBxTag(false, {
                                tag: 'media',
                                params: arParams
                            }) + '" src="/bitrix/images/1.gif" style="border: 1px solid rgb(182, 182, 184); background-color: rgb(226, 223, 218); background-image: url(/bitrix/images/learning/icons/media.gif); background-position: center center; background-repeat: no-repeat; width: ' + width + 'px; height: ' + height + 'px;" width="' + width + '" height="' + height + '" />');
                        }
                    }
                    pElement = null;
                }
    ...
    <tr>
        ...
        <td width="60%">
            <select onchange="if(this[this.selectedIndex].value!='') window.location=this[this.selectedIndex].value;"<?php echo(defined('BX_PUBLIC_MODE') && BX_PUBLIC_MODE == 1 ? "disabled=\"disabled\"" : "") ?>>
                <option value="<?= $APPLICATION->GetCurPageParam("QUESTION_TYPE=S", array("QUESTION_TYPE", "tabControl_active_tab")) ?>" <?
                if ($str_QUESTION_TYPE == "S") echo "selected" ?>><?
                    echo GetMessage("LEARNING_SINGLE_CHOICE") ?></option>
                <option value="<?= $APPLICATION->GetCurPageParam("QUESTION_TYPE=M", array("QUESTION_TYPE", "tabControl_active_tab")) ?>" <?
                if ($str_QUESTION_TYPE == "M") echo "selected" ?>><?
                    echo GetMessage("LEARNING_MULTIPLE_CHOICE") ?></option>
                <option value="<?= $APPLICATION->GetCurPageParam("QUESTION_TYPE=R", array("QUESTION_TYPE", "tabControl_active_tab")) ?>" <?
                if ($str_QUESTION_TYPE == "R") echo "selected" ?>><?
                    echo GetMessage("LEARNING_SORTING") ?></option>
                <option value="<?= $APPLICATION->GetCurPageParam("QUESTION_TYPE=T", array("QUESTION_TYPE", "tabControl_active_tab")) ?>" <?
                if ($str_QUESTION_TYPE == "T") echo "selected" ?>><?
                    echo GetMessage("LEARNING_TEXT_ANSWER") ?></option>
            </select>
    ...
    <tr class="adm-detail-required-field">
        ...
        <td><?php
            ...
            <input id="attached_lesson_id" type="hidden" name="LESSON_ID" value="<?
            echo $LESSON_ID; ?>">
        </td>
    </tr>
    ...
    <tr>
        ...
        <td>
            <input type="text" name="POINT" size="7" maxlength="10" value="<?
            echo $str_POINT ?>">
        </td>
    </tr>
...
<?php endif ?>
    ...
    <tr>
        ...
        <td>
            <input type="checkbox" name="CORRECT_REQUIRED" value="Y"<?
            if ($str_CORRECT_REQUIRED == "Y") echo " checked" ?>>
        </td>
    </tr>
    ...
    <tr>
        <td><? echo $tabControl->GetCustomLabelHTML() ?>:</td>
        <td>
            <input type="text" name="INCORRECT_MESSAGE" size="50" maxlength="255"
                   value="<? echo $str_INCORRECT_MESSAGE ?>">
        </td>
    </tr>
<?php endif ?>
        ...
        <td valign="top" colspan="2">
            <?php if ($str_QUESTION_TYPE != "T"): ?>
                <table cellpadding="0" cellspacing="0" width="100%" class="internal" id="answers-table">
                    ...
                    <tr class="heading">
                        ...
                        <?php if ($str_QUESTION_TYPE != "R"): ?>
                            <td align="center"
                                width="40"><? echo GetMessage("LEARNING_QUESTION_ADM_CORRECT") ?></td><?php endif ?>
                    ...
                    while ($r = _GetOldAndNew($answers)):
                        ...
                        <tr>
                            <td align="center" width="40"
                                style="text-align:center;"><? echo($str_ANSWER_ID > 0 ? $str_ANSWER_ID : "") ?></td>
                            <?php if ($str_QUESTION_TYPE != "R"): ?>
                                <td align="center" width="40" style="text-align:center;">
                                    <? if ($str_QUESTION_TYPE == "M"): ?>
                                        <input type="checkbox" name="ANSWER_<?= $str_ANSWER_ID ?>_CORRECT"
                                               value="Y"<? if ($str_ANSWER_CORRECT == "Y") echo " checked" ?>>
                                    <? else: ?>
                                        <input type="radio" name="ANSWER_CORRECT"
                                               value="<?= $str_ANSWER_ID ?>"<? if ($str_ANSWER_CORRECT == "Y" || $SINGLE_ID == $str_ANSWER_ID) echo " checked" ?>>
                                    <? endif ?>
                                </td>
                            <?php endif ?>
                            <td align="center" style="text-align:center;">
                                <textarea style="width: 98%"
                                          name="ANSWER_<?= $str_ANSWER_ID ?>_ANSWER"><?php echo $str_ANSWER_ANSWER; ?></textarea>
                            </td>
                            <td align="center" width="40" style="text-align:center;">
                                <input type="text" size="3" name="ANSWER_<?= $str_ANSWER_ID ?>_SORT"
                                       value="<?= $str_ANSWER_SORT ?>">
                            </td>
                            <td align="center" width="40" style="text-align:center;">
                                <? if (intval($str_ANSWER_ID) > 0):?>
                                    <input type="checkbox" name="ANSWER_<?= $str_ANSWER_ID ?>_DEL" value="Y">
                                <? else:?>
                                    <a href="javascript:void(0);" onclick="BX.remove(this.parentNode.parentNode)"><img
                ...
                <a href="javascript:void(0)" class="adm-btn"
                   onclick="addAnswer();"><?php echo GetMessage("LEARNING_ADD_ANSWER") ?></a>
            <?php else: ?>
