...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $STEP > 1 && check_bitrix_sessid()) {
    ...
    if ($STEP > 2) {
        ...
        if ($DATA_FILE_NAME == '') {
            ...
            if ($arCourse["SCORM"] == "Y") {
                $dir = "/" . (COption::GetOptionString(
                        "main",
                        "upload_dir",
                        "upload"
                    )) . "/learning/scorm/" . $COURSE_ID . "/";
...
?>
    ...
    <form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?lang=<? echo LANG ?>"
                ...
                <td>
                    <select name="COURSE_ID" style="width:300px;">
                        ...
                        $course = CCourse::GetList(
                            array("SORT" => "ASC"),
                            array('ACCESS_OPERATIONS' => CLearnAccess::OP_LESSON_READ | CLearnAccess::OP_LESSON_WRITE)
                        );
                        while ($course->ExtractFields("f_")) {
                            ?>
                            <option value="<? echo $f_ID ?>" <? if (intval($f_ID) == $COURSE_ID) {
                                echo "selected";
                            } ?>><? echo $f_NAME ?></option><?
                        }
                        ?>
            ...
            <tr>
                ...
                <td valign="top">
                    <input type="text" name="DATA_FILE_NAME" size="40"
                           value="<? echo ($DATA_FILE_NAME <> '') ? htmlspecialcharsbx(
                               $DATA_FILE_NAME
                           ) : "package" . $COURSE_ID . ".tar.gz" ?>"><br>
                ...
                <td colspan="2">
                    ...
                    echo str_replace(
                        ...
                        "<a href=\"" . htmlspecialcharsbx(
                            $tmp
                        ) . "\" target=\"_blank\">" . htmlspecialcharsbx($DATA_FILE_NAME) . "</a>",
                        GetMessage("LEARNING_SU_ALL1")
                    );
                    ?>
                </td>
            </tr>
        ...
        <? if ($STEP < 3): ?>
            ...
            <? endif ?>
            <input type="submit" class="adm-btn-green"
                   value="<? echo ($STEP == 2) ? GetMessage("LEARNING_NEXT_STEP_F") : GetMessage(
                       "LEARNING_NEXT_STEP"
                   ) ?> &gt;&gt;" name="submit_btn">
        <? else: ?>
