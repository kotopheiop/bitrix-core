...
?>
    ...
    <form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?lang=<? echo LANG ?>"
                ...
                <td>
                    <select name="COURSE_ID" style="width:300px;">
                        ...
                        while ($course->ExtractFields("f_")) {
                            ...
                            <option value="<?
                            echo $f_ID ?>" <?
                            if (IntVal($f_ID) == $COURSE_ID) echo "selected"; ?>><?
                            echo $f_NAME ?></option><?
                        }
                        ?>
                    </select>
        ...
        if ($STEP == 2) {
            ...
            <tr class="heading">
                <td colspan="2"><?
                    echo GetMessage("LEARNING_DATA_FILE_NAME") ?></td>
            </tr>
            <tr>
                <td><?
                    echo GetMessage("LEARNING_DATA_FILE_NAME1") ?>:<br>&nbsp;
                </td>
                <td valign="top">
                    <input type="text" name="DATA_FILE_NAME" size="40" value="<?
                    echo (strlen($DATA_FILE_NAME) > 0) ? htmlspecialcharsbx($DATA_FILE_NAME) : "package" . $COURSE_ID . ".tar.gz" ?>"><br>
                    <small><?
                        echo GetMessage("LEARNING_DATA_FILE_NAME1_DESC") ?></small>
                </td>
            </tr>
            <?
        }
        ...
        if ($STEP > 2) {
            ...
            <tr>
                <td colspan="2"><b><?
                        echo GetMessage("LEARNING_SUCCESS") ?></b></td>
            </tr>
            <tr>
                <td colspan="2">
                    <?php
        ...
        <? if ($STEP < 3): ?>
            ...
            <input type="submit" class="adm-btn-green"
                   value="<? echo ($STEP == 2) ? GetMessage("LEARNING_NEXT_STEP_F") : GetMessage("LEARNING_NEXT_STEP") ?> &gt;&gt;"
                   name="submit_btn">
        <? else: ?>
