        ...
        switch ($_REQUEST['action']) {
            case "delete":
                ...
                if (!$oAccess->IsLessonAccessible(
                    CGradeBook::LessonIdByGradeBookId($ID),
...
$lAdmin->AddHeaders(
    array(
        ...
        array(
            "id" => "RESULT",
            "content" => GetMessage('LEARNING_ADMIN_RESULT'),
            "sort" => "result",
            "default" => true
        ),
        array(
            "id" => "MAX_RESULT",
            "content" => GetMessage('LEARNING_ADMIN_MAX_RESULT'),
            "sort" => "max_result",
            "default" => true
        ),
        array(
            "id" => "ATTEMPTS",
            "content" => GetMessage('LEARNING_ADMIN_ATTEMPTS'),
            "default" => true,
            "align" => "center"
        ),
        array(
            "id" => "COMPLETED",
            "content" => GetMessage('LEARNING_ADMIN_APPROVED'),
            "sort" => "completed",
            "default" => true
        ),
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    $row->AddViewField(
        "ATTEMPTS",
        "<a href=\"learn_attempt_admin.php?lang=" . LANG . "&filter_student_id=" . $f_STUDENT_ID . "&filter_test_id=" . $f_TEST_ID . "&set_filter=Y\">" . $f_ATTEMPTS . "</a>" . ($f_ATTEMPT_LIMIT > 0 ? " / " . $f_ATTEMPT_LIMIT : "")
    );
...
?>
    ...
    <form name="form1" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>"
        ...
        <tr>
            ...
            <td nowrap>
                <input type="text" name="filter_result_from" size="10"
                       value="<? echo htmlspecialcharsex($filter_result_from) ?>">
                ...
                <input type="text" name="filter_result_to" size="10"
                       value="<? echo htmlspecialcharsex($filter_result_to) ?>">
            </td>
        </tr>
