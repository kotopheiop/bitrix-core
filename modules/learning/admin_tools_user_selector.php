...
function _ShowUserPropertyField(
    ...
    if ($property_fields["MULTIPLE"] == "Y" && $arUserType["USER_TYPE"] !== "HTML" && !$bMultiple) {
        $html .= '<tr><td><input type="button" value="' . GetMessage(
            ...
            ) . '" onClick="learningJs.addNewRow(\'tb' . md5($name) . '\')"></td></tr>';
    }
    $html .= '</table>';
    echo $html;
}
    ...
    public static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
    {
        ...
        if ($default_value == $USER->GetID()) {
            ...
            $res = "[<a title='" . GetMessage(
                    "LEARNING_USER_SELECTOR_USER_PROFILE"
        ...
        <option value="none"<? if ($select == "none") echo " selected" ?>><?= GetMessage(
                "LEARNING_USER_SELECTOR_NONE"
            ) ?></option>
        <option value="CU"<? if ($select == "CU") echo " selected" ?>><?= GetMessage(
                "LEARNING_USER_SELECTOR_CURRENT"
            ) ?></option>
        <option value="SU"<? if ($select == "SU") echo " selected" ?>><?= GetMessage(
                "LEARNING_USER_SELECTOR_OTHER"
            ) ?></option>
        </select>&nbsp;
        <? echo Learning_FindUserIDNew(
	...
	if (!!DV_" . $tag_name_x . ")
	{
		...
		)
		{
			...
			if (tv" . $tag_name_x . "!='')
			{
				...
				{
					document.getElementById(\"hiddenframe" . $tag_name_escaped . "\").src='/bitrix/admin/get_user.php?ID=' + tv" . $tag_name_x . "+'&strName=" . $tag_name_escaped . "&lang=" . LANG . (defined(
                "ADMIN_SECTION"
                ...
                ) . "&lang=" . LANG . "\">" . $USER->GetID() . "</a>] (" . htmlspecialcharsbx(
                    $USER->GetLogin()
