    ...
    <form name="form1" method="POST" action="<? echo $APPLICATION->GetCurPage() ?>">
            ...
            <td width="60%">
                ...
                <select name="MAILBOX_ID">
                    ...
                    while ($mb->ExtractFields("mb_")):
                        ?>
                        <option value="<? echo $mb_ID ?>"<? if ($str_MAILBOX_ID == $mb_ID) echo " selected" ?>><? echo $mb_NAME ?>
                        [<? echo $mb_ID ?>]</option><?
                    endwhile;
                    ?>
                </select>
        ...
        <? foreach ($ar_CONDITIONS as $key => $COND): ?>
            <tr>
                ...
                <td class="adm-detail-valign-top">
                    <textarea name="CONDITIONS[<?= htmlspecialcharsbx($key) ?>][STRINGS]" style="width:70%"
                              rows="3"><? echo htmlspecialcharsbx($COND["STRINGS"]) ?></textarea><br>
                </td>
            </tr>
        <? endforeach ?>
        <tr>
        ...
        <? if ($USER->IsAdmin()): ?>
            <tr>
                ...
                <td><textarea style="width:70%" rows="5" wrap="off"
                              name="PHP_CONDITION"><?= $str_PHP_CONDITION ?></textarea></td>
            </tr>
        <? endif ?>
        <?
        ...
        <tr>
            ...
            <td><input type="checkbox" name="ACTION_DELETE_MESSAGE"
                       value="Y"<? if ($str_ACTION_DELETE_MESSAGE == "Y") echo " checked" ?>></td>
        </tr>

        <tr>
            <td><? echo GetMessage("MAIL_FLT_EDT_ACT_CANCEL") ?></td>
            <td><input type="checkbox" name="ACTION_STOP_EXEC"
