...
if ($MOD_RIGHT == "W" && $arID = $lAdmin->GroupAction()) {
    if ($_REQUEST['action_target'] == 'selected') {
        ...
        $rsData = Bitrix\Mail\MailMessageTable::getList(
            array(
                'select' => array('ID'),
...
$messageList = Bitrix\Mail\MailMessageTable::getList(
    array(
        'select' => array(
            'ID',
            'MAILBOX_ID',
...
$arHeaders[] = Array(
    "id" => "SUBJECT",
    "content" => GetMessage("MAIL_MSG_ADM_SUBJECT"),
    "default" => true,
    "sort" => "subject"
);
$arHeaders[] = Array(
...
$arHeaders[] = Array(
    "id" => "FIELD_REPLY_TO",
    "content" => GetMessage("MAIL_MSG_ADM_REPLY_TO"),
    "sort" => "field_reply_to"
);
...
$arHeaders[] = Array(
    "id" => "FIELD_DATE",
    "content" => GetMessage("MAIL_MSG_ADM_DATE"),
    "default" => true,
    "sort" => "field_date"
);
...
$arHeaders[] = Array(
    "id" => "MAILBOX_NAME",
    "content" => GetMessage("MAIL_MSG_ADM_MBOX"),
    "default" => true,
    "sort" => "mailbox_name"
);
$arHeaders[] = Array(
    "id" => "MESSAGE_SIZE",
    "content" => GetMessage("MAIL_MSG_ADM_SIZE"),
    "default" => true,
    "sort" => "message_size",
    "align" => "right"
);
...
$arHeaders[] = Array(
    "id" => "SPAM_RATING",
    "content" => GetMessage("MAIL_MSG_ADM_SPAM") . "<br>" . GetMessage("MAIL_MSG_ADM_SPAM_R"),
    "default" => true,
    "sort" => "spam_rating"
);
...
$arHeaders[] = Array(
    "id" => "ATTACHMENTS",
    "content" => GetMessage("MAIL_MSG_ADM_SPAM_ATTCH"),
    "default" => true,
    "sort" => "attachments"
);
...
while ($arRes = $messageList->fetch()) {
    ...
    if ($arRes['SPAM'] == "Y"):
        ...
        if ($arRes['NEW_MESSAGE'] != "Y"):
            $str .= '<div class="mail-message-spam adm-list-mail-icon" title="' . GetMessage(
                    "MAIL_MSG_ADM_READ_SPAM"
                ) . '"></div>';
        else:
            $str .= '<div class="mail-message-unread-spam adm-list-mail-icon" title="' . GetMessage(
                    "MAIL_MSG_ADM_NOTREAD_SPAM"
                ) . '"></div>';
        endif;
    ...
    elseif ($arRes['SPAM'] == "N"):
        ...
        if ($arRes['NEW_MESSAGE'] != "Y"):
            $str .= '<div class="mail-message-notspam adm-list-mail-icon" title="' . GetMessage(
                    "MAIL_MSG_ADM_READ_NOTSPAM"
                ) . '"></div>';
        else:
            $str .= '<div class="mail-message-unread-notspam adm-list-mail-icon" title="' . GetMessage(
                    "MAIL_MSG_ADM_NOTREAD_NOTSPAM"
                ) . '"></div>';
        endif;
        ...
        if ($arRes['NEW_MESSAGE'] != "Y"):
            $str .= '<div class="mail-message adm-list-mail-icon" title="' . GetMessage(
                    "MAIL_MSG_ADM_READ"
                ) . '"></div>';
        else:
            $str .= '<div class="mail-message-unread adm-list-mail-icon" title="' . GetMessage(
                    "MAIL_MSG_ADM_NOTREAD"
                ) . '"></div>';
        endif;
    ...
    if ($MOD_RIGHT == "W") {
        ...
        if ($arRes['SPAM'] != "Y") {
            $arActions[] = array(
                ...
                "ACTION" => $lAdmin->ActionAjaxReload(
                    ...
                    ) . "?action=mark_as_spam&ID=" . $arRes['ID'] . "&lang=" . LANG . "&" . bitrix_sessid_get()
                )
            );
        }
        ...
        if ($arRes['SPAM'] != "N") {
            $arActions[] = array(
                ...
                "ACTION" => $lAdmin->ActionAjaxReload(
                    $APPLICATION->GetCurPage(
                    ) . "?action=mark_as_notspam&ID=" . $arRes['ID'] . "&lang=" . LANG . "&" . bitrix_sessid_get()
                )
            );
        }
        ...
        if ($arRes['NEW_MESSAGE'] == "Y") {
            $arActions[] = array(
                ...
                "ACTION" => $lAdmin->ActionAjaxReload(
                    $APPLICATION->GetCurPage(
                    ) . "?action=mark_as_read&ID=" . $arRes['ID'] . "&lang=" . LANG . "&" . bitrix_sessid_get()
                )
            );
        } else {
            $arActions[] = array(
                ...
                "ACTION" => $lAdmin->ActionAjaxReload(
                    $APPLICATION->GetCurPage(
                    ) . "?action=mark_as_unread&ID=" . $arRes['ID'] . "&lang=" . LANG . "&" . bitrix_sessid_get()
                )
            );
        }
        ...
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . GetMessage('MAIL_MSG_ADM_FILTER_CONFIRM10') . "')) " . $lAdmin->ActionDoGroup(
                    $arRes['ID'],
                    "delete"
                )
        );
...
while ($flt_arr = $res->Fetch()) {
    $arActions["refilter_num_" . $flt_arr["ID"]] = GetMessage("MAIL_MSG_ADM_PROC_ACT_RULE") . " " . htmlspecialcharsbx(
            mb_substr($flt_arr["NAME"], 0, 30)
        );
}
    ...
    <table cellspacing="0">
        <tr>
            ...
            <td style="padding-left:3px; padding-right:3px;">
                <input class="form-button" type="submit" name="make_action"
                       value="<? echo GetMessage("MAIL_MSG_ADM_OK") ?>">
...
<form name="form1" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?">
    ...
    <tr>
        ...
        <td nowrap><input type="text" name="find_all" value="<? echo htmlspecialcharsbx($find_all) ?>"
                          size="47"><?= ShowFilterLogicHelp() ?></td>
    </tr>

    <tr>
        <td nowrap>ID:</td>
        <td nowrap><input type="text" name="find_id" value="<? echo htmlspecialcharsbx($find_id) ?>"
                          size="47"><?= ShowFilterLogicHelp() ?></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td nowrap>
            <select name="find_spam">
                ...
                <option value="Y"<? if ($find_spam == "Y") echo " selected" ?>><? echo GetMessage(
                        "MAIL_MSG_ADM_FILTER_YES"
                    ) ?></option>
                <option value="N"<? if ($find_spam == "N") echo " selected" ?>><? echo GetMessage(
                        "MAIL_MSG_ADM_FILTER_NO"
                    ) ?></option>
            </select>
        </td>
    </tr>
    <tr>
        ...
        <td nowrap>
            <select name="find_new">
                <option value=""><? echo GetMessage("MAIL_MSG_ADM_FILTER_ANY") ?></option>
                <option value="Y"<? if ($find_new == "Y") echo " selected" ?>><? echo GetMessage(
                        "MAIL_MSG_ADM_FILTER_NEW"
                    ) ?></option>
                <option value="N"<? if ($find_new == "N") echo " selected" ?>><? echo GetMessage(
                        "MAIL_MSG_ADM_FILTER_OLD"
                    ) ?></option>
            </select>
        </td>
    </tr>
    <tr>
        ...
        <td nowrap><input type="text" name="find_from" value="<? echo htmlspecialcharsbx($find_from) ?>"
                          size="47"><?= ShowFilterLogicHelp() ?></td>
    </tr>
    <tr>
        <td nowrap><? echo GetMessage("MAIL_MSG_ADM_FILTER_TO") ?>:</td>
        <td nowrap><input type="text" name="find_to" value="<? echo htmlspecialcharsbx($find_to) ?>"
                          size="47"><?= ShowFilterLogicHelp() ?></td>
    </tr>
    <tr>
        <td nowrap><? echo GetMessage("MAIL_MSG_ADM_FILTER_SUBJECT") ?>:</td>
        <td nowrap><input type="text" name="find_subject" value="<? echo htmlspecialcharsbx($find_subject) ?>"
                          size="47"><?= ShowFilterLogicHelp() ?></td>
    </tr>
    <tr>
        <td nowrap><? echo GetMessage("MAIL_MSG_ADM_FILTER_TEXT") ?>:</td>
        <td nowrap><input type="text" name="find_body" value="<? echo htmlspecialcharsbx($find_body) ?>"
                          size="47"><?= ShowFilterLogicHelp() ?></td>
    </tr>
...
<table border="0" cellspacing="3" cellpadding="0" class="adm-mail-table">
    <tr>
        <td class="mail-message-spam-right-cell adm-mail-cell-left">
            <div class="mail-message-spam"></div>
        </td>
        <td class="adm-mail-cell-right"><? echo GetMessage("MAIL_MSG_ADM_READ_SPAM") ?></td>
    </tr>
    <tr>
        <td class="mail-message-unread-spam-right-cell adm-mail-cell-left">
            <div class="mail-message-unread-spam"></div>
        </td>
        <td class="adm-mail-cell-right"><? echo GetMessage("MAIL_MSG_ADM_NOTREAD_SPAM") ?></td>
    </tr>
    <tr>
        <td class="mail-message-notspam-cell adm-mail-cell-left">
            <div class="mail-message-notspam"></div>
        </td>
        <td class="adm-mail-cell-right"><? echo GetMessage("MAIL_MSG_ADM_READ_NOTSPAM") ?></td>
    </tr>
    <tr>
        <td class="mail-message-unread-notspam-cell adm-mail-cell-left">
            <div class="mail-message-unread-notspam"></div>
        </td>
        <td class="adm-mail-cell-right"><? echo GetMessage("MAIL_MSG_ADM_NOTREAD_NOTSPAM") ?></td>
    </tr>
    <tr>
        <td class="mail-message-cell adm-mail-cell-left">
            <div class="mail-message"></div>
        </td>
        <td class="adm-mail-cell-right"><? echo GetMessage("MAIL_MSG_ADM_READ") ?></td>
    </tr>
    <tr>
        <td class="mail-message-unread-cell adm-mail-cell-left">
            <div class="mail-message-unread"></div>
        </td>
