...
class MailboxDirectoryHelper
{
    ...
    {
        return $dir->isIncome() ? 10 : ($dir->isOutcome() ? 2000 : ($dir->isTrash() ? 3000 : ($dir->isSpam(
        ) ? 4000 : ($dir->isDraft() ? 5000 : $dir->getLevel() * 100))));
    }
    ...
    private function orderByTime($dirs)
    {
        usort(
            $dirs,
            function ($a, $b) {
                $aSort = $a->getSyncTime() ?: ($a->isIncome() ? 100 : ($a->isOutcome() ? 200 : ($a->isTrash(
                ) ? 300 : ($a->isSpam() ? 400 : $a->getLevel() * 1000))));
                $bSort = $b->getSyncTime() ?: ($b->isIncome() ? 100 : ($b->isOutcome() ? 200 : ($b->isTrash(
                ) ? 300 : ($b->isSpam() ? 400 : $b->getLevel() * 1000))));
