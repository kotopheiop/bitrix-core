            ...
            if (File_Exists($_SERVER["DOCUMENT_ROOT"] . $this->logFileName)) {
                ...
                if ($logSize > $this->logMaxSize) {
                    if (($fp = @FOpen($_SERVER["DOCUMENT_ROOT"] . $this->logFileName, "rb"))
                        ...
                        @Copy(
                            $_SERVER["DOCUMENT_ROOT"] . $this->logFileName . "_",
                            $_SERVER["DOCUMENT_ROOT"] . $this->logFileName
                        );
...
class CSMTPServerHost
{
    ...
    {
        $this->WriteToLog(
            "Connection removed (" . $id . ", " . $this->arClients[$id]->ip . ", " . $this->arClients[$id]->sock . ")",
            3
        );
    ...
    function Start()
    {
        ...
        $this->sockServer = stream_socket_server(
            "tcp://" . ($this->arFields["SERVER"] == "*" ? "0.0.0.0" : $this->arFields["SERVER"]) . ":" . $this->arFields["PORT"],
            $errno,
            $errstr
        );
        ...
        $this->WriteToLog(
            "Server #" . $this->arFields["ID"] . " started: " . ($this->arFields["SERVER"] == "*" ? "0.0.0.0" : $this->arFields["SERVER"]) . ":" . $this->arFields["PORT"],
            1
        );
        return true;
    }
    ...
    function _Stop()
    {
        if ($this->sockServer) {
            ...
            $this->WriteToLog(
                "Server #" . $this->arFields["ID"] . " stopped: " . ($this->arFields["SERVER"] == "*" ? "0.0.0.0" : $this->arFields["SERVER"]) . ":" . $this->arFields["PORT"],
                1
            );
        }
    ...
    function __AuthPlainHandler()
    {
        ...
        if ($this->readBuffer == "*") {
            ...
            if ($pwd == '') {
                ...
                $pwd = ltrim($pwd, chr(0));
                $this->Authorize(
                    mb_substr($pwd, 0, mb_strpos($pwd, chr(0))),
                    mb_substr($pwd, mb_strpos($pwd, chr(0)) + 1)
                );
            }
        }
        ...
        if ($p > 0) {
            ...
            foreach ($this->arMsg["TO"] as $to) {
                if (is_array($this->arMsg["FOR_RELAY"]) && in_array($to, $this->arMsg["FOR_RELAY"])) {
                    ...
                    $this->WriteToLog(
                        '[' . $this->arMsg["LOCAL_ID"] . '] Relay message to ' . $to . ' (' . ($r ? 'OK' : 'FAILED') . ')',
                        7
                    );
                } else {
                    $arLocalTo[] = $to;
            ...
            if (count($arLocalTo) > 0) {
                ...
                $this->WriteToLog(
                    '[' . $this->arMsg["LOCAL_ID"] . '] Message add: ' . $message_header_add . $message,
                    9
                );
                ...
                $message_id = CMailMessage::AddMessage(
                    $this->server->arFields["ID"],
                    $message_header_add . $message,
                    $charset
                );

                $this->WriteToLog(
                    '[' . $this->arMsg["LOCAL_ID"] . '] Message sent to ' . implode(
                        ", ",
                        $arLocalTo
                    ) . ' (' . $message_id . ')',
                    7
