...
while ($arRes = $rsData->NavNext(true, "f_")):
    ...
    if ($f_USER_ID > 0) {
        $strUser = "[<a class='tablebodylink' href=\"/bitrix/admin/user_edit.php?ID=" . $f_USER_ID . "&amp;lang=" . LANG . "\" title=\"" . GetMessage(
                "subscr_user_edit_title"
    ...
    $row->AddViewField(
        "EMAIL",
        '<a href="subscr_edit.php?ID=' . $f_ID . '&amp;lang=' . LANG . '" title="' . GetMessage(
            "subscr_upd"
        ) . '">' . $f_EMAIL . '</a>'
    );
    ...
    if ($POST_RIGHT >= "W") {
        $arActions[] = array(
            ...
            "ACTION" => "if(confirm('" . GetMessage("subscr_del_conf") . "')) " . $lAdmin->ActionDoGroup(
                    $f_ID,
                    "delete"
                )
        );
    ...
    <form name="find_form" method="get" action="<? echo $APPLICATION->GetCurPage(); ?>">
        ...
        <tr>
            ...
            <td>
                <input type="text" size="25" name="find" value="<? echo htmlspecialcharsbx($find) ?>"
                       title="<?= GetMessage("POST_F_FIND_TITLE") ?>">
                <?
                $arr = array(
                    "reference" => array(
                        GetMessage("POST_F_EMAIL"),
        ...
        <tr>
            ...
            <td><input type="text" name="find_id" size="47"
                       value="<? echo htmlspecialcharsbx($find_id) ?>">&nbsp;<?= ShowFilterLogicHelp() ?></td>
        </tr>
        <tr>
            <td><? echo GetMessage("POST_F_INSERT") . " (" . FORMAT_DATE . "):" ?></td>
            <td><? echo CalendarPeriod(
                    "find_insert_1",
                    htmlspecialcharsbx($find_insert_1),
                    "find_insert_2",
                    htmlspecialcharsbx($find_insert_2),
                    "find_form",
                    "Y"
                ) ?></td>
        </tr>
        <tr>
            ...
            <td><? echo CalendarPeriod(
                    "find_update_1",
                    htmlspecialcharsbx($find_update_1),
                    "find_update_2",
                    htmlspecialcharsbx($find_update_2),
                    "find_form",
                    "Y"
                ) ?></td>
        </tr>
        <tr>
            ...
            <td><input type="text" name="find_email" size="47"
                       value="<? echo htmlspecialcharsbx($find_email) ?>">&nbsp;<?= ShowFilterLogicHelp() ?></td>
        </tr>
        <tr>
            ...
            <td><?
                ...
                echo SelectBoxFromArray(
                    "find_anonymous",
                    $arr,
                    htmlspecialcharsbx($find_anonymous),
                    GetMessage("MAIN_ALL")
                );
                ?></td>
        </tr>
        <tr>
            ...
            <td><input type="text" name="find_user_id" size="47" value="<? echo htmlspecialcharsbx($find_user_id) ?>">&nbsp;<?= ShowFilterLogicHelp(
                ) ?></td>
        </tr>
        <tr>
            <td><? echo GetMessage("POST_F_USER") ?>:</td>
            <td><input type="text" name="find_user" size="47"
                       value="<? echo htmlspecialcharsbx($find_user) ?>">&nbsp;<?= ShowFilterLogicHelp() ?></td>
        </tr>
        <tr>
            ...
            <td><?
                ...
                echo SelectBoxFromArray(
                    "find_confirmed",
                    $arr,
                    htmlspecialcharsbx($find_confirmed),
                    GetMessage("MAIN_ALL")
                );
                ?></td>
        </tr>
        ...
        <tr>
            ...
            <td><?
                $arr = array(
                    "reference" => array(GetMessage("POST_TEXT"), GetMessage("POST_HTML")),
                    "reference_id" => array("text", "html")
                );
