...
?>
    <form ENCTYPE="multipart/form-data" action="<? echo $APPLICATION->GetCurPage(); ?>" method="POST" name="impform">
        ...
        <tr>
            ...
            <td><input type="checkbox" name="SEND_CONFIRM" value="Y"<? if ($SEND_CONFIRM == "Y") echo " checked" ?>>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <input id="USER_TYPE_1" name="USER_TYPE" type="radio"
                       value="A"<? if ($USER_TYPE == "A") echo " checked" ?> onClick="DisableControls(true);"><label
                        for="USER_TYPE_1"><? echo GetMessage("imp_add_anonym") ?></label><br>
                <input id="USER_TYPE_2" name="USER_TYPE" type="radio"
                       value="U"<? if ($USER_TYPE == "U") echo " checked" ?> onClick="DisableControls(false);"><label
                        for="USER_TYPE_2"><? echo GetMessage("imp_add_users") ?></label></td>
        </tr>
        <tr>
            ...
            <tr>
                ...
                <td><select name="USER_GROUP_ID[]" multiple size=10><?
                        ...
                        while (($gr = $groups->Fetch())):
                            ...
                            <OPTION VALUE="<?
                            echo $gr["ID"] ?>"<?
                            if (in_array($gr["ID"], $USER_GROUP_ID)) echo " SELECTED" ?>><?
                            echo htmlspecialcharsbx($gr["NAME"]) . " [" . $gr["ID"] . "]" ?></OPTION><?
                        endwhile;
                        ?></SELECT>
                    <script language="JavaScript">
                        function DisableControls(bDisable) {
                ...
                <div class="adm-list">
                    ...
                    while (($rub = $rubrics->Fetch())):
                        ...
                        <div class="adm-list-item">
                            <div class="adm-list-control"><input type="checkbox" id="RUB_ID_<?
                                echo $n ?>" name="RUB_ID[]" value="<?
                                echo $rub["ID"] ?>"<?
                                if (!$bShowRes || in_array($rub["ID"], $RUB_ID)) echo " checked" ?>></div>
                            <div class="adm-list-label"><label for="RUB_ID_<?
                                echo $n ?>"><?
                                    echo "[" . $rub["LID"] . "]&nbsp;" . htmlspecialcharsbx($rub["NAME"]) ?></label>
                            </div>
                        </div>
                        <?
                        $n++;
        ...
        <tr>
            ...
            <td><input id="FORMAT_1" name="FORMAT" type="radio"
                       value="text"<? if ($FORMAT == "text") echo " checked" ?>><label
                        for="FORMAT_1"><? echo GetMessage("imp_fmt_text") ?></label>&nbsp;/<input id="FORMAT_2"
                                                                                                  name="FORMAT"
                                                                                                  type="radio"
                                                                                                  value="html"<? if ($FORMAT == "html") echo " checked" ?>><label
                        for="FORMAT_2">HTML</label></td>
        </tr>
        <tr>
