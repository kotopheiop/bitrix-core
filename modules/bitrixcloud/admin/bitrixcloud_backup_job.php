...
try {
    ...
    if (empty($arJobs) && $_GET["allow_manage"] === "y") {
        ...
        <div id="upload_form" <?
        if ($strError == "") echo 'style="display:none;height:200px;"' ?>>
            <form method="POST" action="<?
            echo htmlspecialcharsbx($APPLICATION->GetCurPageParam()) ?>" enctype="multipart/form-data" name="editform"
                  id="editform">
                <?
                $tabControl->Begin();
                ...
                <tr>
                    <td width="40%"><?
                        echo GetMessage("BCL_BACKUP_JOB_URL") ?>:
                    </td>
                    <?
                    if ($strError) {
                        $URL = $_POST["URL"];
                    ...
                    ?>
                    <td width="60%"><input type="text" name="URL" size="45" value="<?
                        echo htmlspecialcharsbx($URL) ?>"></td>
                </tr>
                <tr>
                    <td><?
                        echo GetMessage("BCL_BACKUP_JOB_TIME") ?>:
                    </td>
                    <?
                    if ($strError) {
                        $TIME = $_POST["TIME"];
                    ...
                    ?>
                    <td><input type="text" name="TIME" size="6" value="<?
                        echo htmlspecialcharsbx($TIME) ?>"></td>
                </tr>
                <tr>
                    <td class="adm-detail-valign-top"><?
                        echo GetMessage("BCL_BACKUP_JOB_WEEK_DAYS") ?>:
                    </td>
                    <td>
                        <?
                        $rand = mt_rand(0, 7);
                        for ($i = 0; $i < 7; $i++):
                            ?>
                            <input type="checkbox" name="WEEK_DAYS[]" value="<?
                            echo $i ?>" id="dow_<?
                            echo $i ?>" <?
                            if (
                                (
                ...
                echo GetMessage("BCL_BACKUP_JOB_SAVE_BTN") ?>" class="adm-btn-save">
                <input type="button" value="<?
                echo GetMessage("BCL_BACKUP_JOB_CANCEL_BTN") ?>" onclick="hide_upload_form()">
                <?
                $tabControl->End();
                ?>
            </form>
