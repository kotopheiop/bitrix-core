...
class CAllCurrencyLang
{
    ...
    public static function checkFields($action, &$fields, $currency = '', $language = '', $getErrors = false)
    {
        ...
        if ($action == 'ADD') {
            ...
            if (!isset($fields['FORMAT_STRING']) || empty($fields['FORMAT_STRING'])) {
                $errorMessages[] = array(
                    'id' => 'FORMAT_STRING',
                    'text' => Loc::getMessage('BT_CUR_LANG_ERR_FORMAT_STRING_IS_EMPTY', array('#LANG#' => $language))
                );
            }
    ...
    public static function Update($currency, $lang, $arFields)
    {
        ...
        if (!empty($strUpdate)) {
            $strSql = "update b_catalog_currency_lang set " . $strUpdate . " where CURRENCY = '" . $DB->ForSql(
                    $currency
                ) . "' and LID='" . $DB->ForSql($lang) . "'";
            $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
    ...
    public static function Delete($currency, $lang)
    {
        ...
        $strSql = "delete from b_catalog_currency_lang where CURRENCY = '" . $DB->ForSql(
                $currency
            ) . "' and LID = '" . $DB->ForSql($lang) . "'";
        $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
    ...
    public static function GetByID($currency, $lang)
    {
        ...
        $strSql = "select * from b_catalog_currency_lang where CURRENCY = '" . $DB->ForSql(
                $currency
            ) . "' and LID = '" . $DB->ForSql($lang) . "'";
    ...
    public static function GetFormatDescription($currency)
    {
        ...
        if (!isset(self::$arCurrencyFormat[$currency])) {
            ...
            if ($arCurFormat === false) {
                ...
                if ($safeFormat) {
                    $arCurFormat["FORMAT_STRING"] = strip_tags(
                        preg_replace(
                            '#<script[^>]*?>.*?</script[^>]*?>#is',
                            '',
    ...
    public static function isExistCurrencyLanguage($currency, $language)
    {
        ...
        $query = "select LID from b_catalog_currency_lang where CURRENCY = '" . $DB->ForSql(
                $currency
            ) . "' and LID = '" . $DB->ForSql($language) . "'";
    ...
    public static function getUnFormattedValue(
    ...
    protected static function unFormatValue(
        string $formattedValue,
