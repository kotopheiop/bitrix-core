...
$aTabs = array(
    array(
        "DIV" => "edit1",
        "TAB" => GetMessage("curr_rates_rate"),
        "ICON" => "currency",
        "TITLE" => GetMessage("curr_rates_rate_ex")
    ),
);
    ...
    if (!$res) {
        ...
        if (empty($_POST['apply'])) {
            if (!empty($return_url)) {
                ...
                LocalRedirect(
                    "/bitrix/admin/currencies_rates.php?lang=" . LANGUAGE_ID . GetFilterParams("filter_", false)
                );
            }
        }
        LocalRedirect(
            "/bitrix/admin/currency_rate_edit.php?lang=" . LANGUAGE_ID . "&ID=" . $ID . "&" . GetFilterParams(
                "filter_",
                false
            )
        );
    ...
    if ($CURRENCY_RIGHT == "W") {
        $aContext[] = array(
            ...
            "LINK" => "javascript:if(confirm('" . GetMessage(
                    "CONFIRM_DEL_MESSAGE"
                ) . "'))window.location='/bitrix/admin/currencies_rates.php?action=delete&ID=" . $ID . "&lang=" . LANGUAGE_ID . "&" . bitrix_sessid_get(
                ) . "';",
        );
    }
}
...
?>
    <form method="POST" action="<? $APPLICATION->GetCurPage() ?>" name="rate_edit">
        ...
        <tr class="adm-detail-required-field">
            ...
            <td width="60%"><?= CalendarDate(
                    "DATE_RATE",
                    $currencyRate['DATE_RATE'],
                    "rate_edit",
                    "10",
                    'class="typeinput"'
                ); ?></td>
            ...
            <td><select name="CURRENCY"><?
                    foreach ($currencyList as $currency => $title) {
                        ...
                        <option value="<?= htmlspecialcharsbx(
                            $currency
                        ); ?>"<?= ($currency == $currencyRate['CURRENCY'] ? ' selected' : ''); ?>><?= htmlspecialcharsbx(
                            $title
                        ); ?></option><?
                    }
        ...
        <tr class="adm-detail-required-field">
            ...
            <td><input type="hidden" name="BASE_CURRENCY"
                       value="<?= htmlspecialcharsbx($currencyRate['BASE_CURRENCY']); ?>"><?
        ...
        <tr class="adm-detail-required-field">
            <td><? echo GetMessage("curr_rates_rate_cnt") ?>: <span class="required"
                                                                    style="vertical-align: super; font-size: smaller;">1</span>
            </td>
            <td><input type="text" id="RATE_CNT" name="RATE_CNT"
                       value="<?= htmlspecialcharsbx($currencyRate['RATE_CNT']); ?>" size="5"></td>
        </tr>
        <tr class="adm-detail-required-field">
            <td><? echo GetMessage("curr_rates_rate") ?>: <span class="required"
                                                                style="vertical-align: super; font-size: smaller;">1</span>
            </td>
            <td>
                <input type="text" id="RATE" name="RATE" value="<?= htmlspecialcharsbx($currencyRate['RATE']); ?>"
                       size="12"><?
                if ($showGetRate) {
                    ?>
                    &nbsp;<input id="get_btn" type="button"
                                 title="<?= htmlspecialcharsbx(GetMessage("curr_rates_query_ex")); ?>"
                                 value="<?= htmlspecialcharsbx(GetMessage("curr_rates_query")); ?>">
    ...
    <script type="text/javascript">
        ...
        function resultCurrencyRate(result) {
            ...
            if (!BX.type.isPlainObject(result) || !BX.type.isNotEmptyString(result.STATUS)) {
                BX('currency_query_error_div').innerHTML = '<? echo GetMessageJS(
                    'BX_CURRENCY_GET_RATE_ERR_UNKNOWN'
                ); ?>';
            } else {
