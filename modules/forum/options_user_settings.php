...
if (CModule::IncludeModule("forum")):
    ...
    if ($USER->IsAdmin() || $GLOBALS["APPLICATION"]->GetGroupRight("forum") >= "W"):?>
        <tr>
            ...
            <td width="60%"><input type="checkbox" name="forum_ALLOW_POST" value="Y" <?
                if ($str_forum_ALLOW_POST == "Y") echo "checked"; ?>></td>
        </tr>
    <?endif; ?>
    <tr>
        <td><?= GetMessage("forum_HIDE_FROM_ONLINE") ?></td>
        <td><input type="checkbox" name="forum_HIDE_FROM_ONLINE" value="Y" <?
            if ($str_forum_HIDE_FROM_ONLINE == "Y") echo "checked"; ?>></td>
    </tr>
    <tr>
        <td><?= GetMessage("forum_SUBSC_GET_MY_MESSAGE") ?></td>
        <td><input type="checkbox" name="forum_SUBSC_GET_MY_MESSAGE" value="Y" <?
            if ($str_forum_SUBSC_GET_MY_MESSAGE == "Y") echo "checked"; ?>></td>
    </tr>
    <tr>
        <td><?= GetMessage("forum_SHOW_NAME") ?></td>
        <td><input type="checkbox" name="forum_SHOW_NAME" value="Y" <?
            if ($str_forum_SHOW_NAME == "Y") echo "checked"; ?>></td>
    </tr>
    <tr>
        <td><?= GetMessage('forum_DESCRIPTION') ?></td>
        <td><input class="typeinput" type="text" name="forum_DESCRIPTION" size="30" maxlength="255"
                   value="<?= $str_forum_DESCRIPTION ?>"></td>
    </tr>
    <tr>
        <td class="adm-detail-valign-top"><?= GetMessage('forum_INTERESTS') ?></td>
        <td><textarea class="typearea" name="forum_INTERESTS" rows="3" cols="35"><?
                echo $str_forum_INTERESTS; ?></textarea></td>
    </tr>
    <tr>
        <td class="adm-detail-valign-top"><?= GetMessage("forum_SIGNATURE") ?></td>
        <td><textarea class="typearea" name="forum_SIGNATURE" rows="3" cols="35"><?
                echo $str_forum_SIGNATURE; ?></textarea></td>
    </tr>
