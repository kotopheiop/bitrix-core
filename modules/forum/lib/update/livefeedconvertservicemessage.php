    ...
    public function execute(array &$result)
    {
        ...
        if ($postMessage = reset($result["handlers"])) {
            ...
            if ($postMessage === "innerForum") {
                ...
                if ($handler = (new Socialnetwork\CommentAux\HandlerManager())->getHandlerByPostText($postMessage)) {
    ...
    public function convert(Socialnetwork\CommentAux\Base $handler, $lastId)
    {
        ...
        if ($rowsCount > 0) {
            ...
            while ($comment = $comments->current()) {
                $socnetInfo = array_key_exists(
                    $comment["ID"],
                    $socnetComments
                ) ? $socnetComments[$comment["ID"]] : ["SHARE_DEST" => ""];
