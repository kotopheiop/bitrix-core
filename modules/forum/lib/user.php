...
class User implements \ArrayAccess
{
    ...
    protected function __construct($id)
    {
        ...
        if ($id > 0) {
            ...
            if ($user) {
                ...
                $this->data["SHOW_NAME"] = (\COption::GetOptionString(
                    "forum",
                    "USER_SHOW_NAME",
                    "Y"
                ) == "Y" ? "Y" : "N");
            ...
            $this->data["VISIBLE_NAME"] = ($this->data["SHOW_NAME"] === "Y" ? \CUser::FormatName(
    ...
    public function setLastVisit()
    {
        ...
        $update = $helper->prepareUpdate(
            $tableName,
            [
                'LAST_VISIT' => new Main\DB\SqlExpression(
    ...
    public function incrementStatistic(array $message)
    {
        ...
        $this->data["NUM_POSTS"]++;
        $this->data["POINTS"] = \CForumUser::GetUserPoints(
            $this->getId(),
            array("INCREMENT" => $this->data["NUM_POSTS"])
        );
    ...
    public function getUnreadMessageId($topicId = 0): ?int
    {
        ...
        if ($this->isAuthorized()) {
            ...
            $lastVisit = max(
                ...
                is_array($timestamps) && array_key_exists($topic->getId(), $timestamps) ? $timestamps[$topic->getId(
                )] : 0
            );
            if ($lastVisit > 0) {
