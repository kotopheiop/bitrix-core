    ...
    protected static function checkPattern(string $pattern)
    {
        ...
        if ($pattern == '') {
            ...
            if (strpos($pattern, "/") !== 0) {
                ...
                if (($modificators = substr($pattern, strrpos($pattern, "/"))) && strpos(
                        $modificators,
                        "e"
                    ) !== false) {
...
class Filter implements \ArrayAccess
{
    ...
    public function __construct($id)
    {
        ...
        if ($id <= 0) {
            ...
            if (!($data = FilterTable::getDataById($this->id))) {
    ...
    public function update(array $fields)
    {
        ...
        if (isset($fields["WORDS"]) && $fields["WORDS"] == '') {
            $result->addError(
                new Main\Orm\EntityError(
                    Loc::getMessage("FLT_ERR_DICT_PATT_MISSED"),
                    "emptyData"
                )
            );
        } else {
            if (isset($fields["DICTIONARY_ID"]) && intval($fields["DICTIONARY_ID"]) <= 0) {
                $result->addError(
                    new Main\Orm\EntityError(
                ...
                if (isset($fields["WORDS"]) && $fields["WORDS"] !== $this["WORDS"] ||
                    ...
                    if (($wordEqual = FilterTable::getList(
                        ...
                        if ($fields["PATTERN_CREATE"] === "PTTRN") {
                            $fields["PATTERN"] = $fields["WORDS"];
                        } else {
                            if ($fields["PATTERN_CREATE"] === "TRNSL") {
                                $fields["PATTERN"] = Dictionary::getById(
                                    $fields["DICTIONARY_ID"]
                                )->translitAndCreatePattern($fields["WORDS"]);
                            } else {
                                $fields["PATTERN_CREATE"] = "WORDS";
                                $fields["PATTERN"] = Dictionary::getById($fields["DICTIONARY_ID"])->createPattern(
                                    $fields["WORDS"]
                                );
                            }
                        }
    ...
    public function generatePattern()
    {
        ...
        if ($this["PATTERN_CREATE"] === "PTTRN") {
            ...
            if ($this["PATTERN_CREATE"] === "TRNSL") {
                $fields["PATTERN"] = Dictionary::getById($this["DICTIONARY_ID"])->translitAndCreatePattern(
                    $this["WORDS"]
                );
            } else {
                $fields["PATTERN_CREATE"] = "WORDS";
    ...
    public static function add(array $fields)
    {
        ...
        if (!isset($fields["WORDS"]) || $fields["WORDS"] == '') {
            ...
            if (intval($fields["DICTIONARY_ID"]) <= 0) {
                $result->addError(
                    new Main\Orm\EntityError(
                        Loc::getMessage("FLT_ERR_DICTIONARY_MISSED"),
                        "emptyDictionaryId"
