...
class Dictionary
{
        ...
        if (is_null($letters)) {
            ...
            while ($lett = $dbRes->fetch()) {
                ...
                for ($ii = 0; $ii < count($arrRepl); $ii++) {
                    ...
                    if (strlen($lett["LETTER"]) == 1) {
                        if (strlen($arrRepl[$ii]) == 1) {
                            ...
                            if (strpos($arrRepl[$ii], "(") === 0 && substr($arrRepl[$ii], -1, 1) == ")") {
                                $arrRes[$ii] = $arrRepl[$ii] . "+";
                            } else {
                                if (strpos($arrRepl[$ii], "(") === 0 && substr($arrRepl[$ii], -2, 1) == ")") {
                                    $arrRes[$ii] = $arrRepl[$ii];
                                } else {
                                    if (strlen($arrRepl[$ii]) > 1) {
                                        $arrRes[$ii] = "[" . $arrRepl[$ii] . "]+";
                ...
                if (strlen($lett["LETTER"]) == 1) {
                    ...
                    $letters["multiLetter"]["/" . preg_quote(
                        $lett["LETTER"]
                    ) . "/is" . BX_UTF_PCRE_MODIFIER] = "(" . implode("|", $arrRes) . ")";
                }
            }
    ...
    {
        $res = "/(?<=[" . self::NOT_WORD . "])(" . preg_quote(
                $word
            ) . ")(?=[" . self::NOT_WORD . "])/is" . BX_UTF_PCRE_MODIFIER;
        return $res;
    }
}