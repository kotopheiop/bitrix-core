    ...
    public static function GetListEx(
            ...
            switch ($key) {
                ...
                case "USER_ID":
                    if (intval($val) <= 0) {
                        ...
                        $arSqlSearch[] = ($strNegative == "Y" ? " PM." . $key . " IS NULL OR NOT " : "") . "(PM." . $key . " " . $strOperation . " (" . intval(
                                $val
                            ) . ") )";
                    } else {
                        $arSqlSearch[] = ($strNegative == "Y" ? " PM." . $key . " IS NULL OR NOT " : "") . "(PM." . $key . " " . $strOperation . " " . intval(
                ...
                case "OWNER_ID":
                    if (COption::GetOptionString("forum", "UsePMVersion", "2") == 2) {
                        ...
                        $arSqlSearch[] =
                            "((PM.AUTHOR_ID=" . intval($val) . ") AND (PM.IS_READ='N')) OR (PM.USER_ID=" . intval(
                                $val
                            ) . " AND (PM.FOLDER_ID=2))";
                    }
                    break;
                ...
                case "IS_READ":
                    if ($val == '') {
                        ...
                        $arSqlSearch[] = ($strNegative == "Y" ? " PM." . $key . " IS NULL OR NOT " : "") . "(PM." . $key . " " . $strOperation . " ('" . $DB->ForSql(
                                $val
                            ) . "') )";
                    } else {
                        $arSqlSearch[] = ($strNegative == "Y" ? " PM." . $key . " IS NULL OR NOT " : "") . "(PM." . $key . " " . $strOperation . " '" . $DB->ForSql(
                ...
                case "POST_DATE":
                    if ($val == '') {
                        ...
                        $arSqlSearch[] = ($strNegative == "Y" ? " PM." . $key . " IS NULL OR NOT " : "") . "(PM." . $key . " " . $strOperation . " " . $DB->CharToDateFunction(
                                $DB->ForSql($val),
                                "FULL"
                            ) . " )";
                    }
        ...
        $strSql =
            "SELECT 
				...
				CASE
					WHEN ((FU.SHOW_NAME='Y') AND (LENGTH(TRIM(CONCAT_WS(''," . CForumUser::GetNameFieldsForQuery(
                $arAddParams["sNameTemplate"]
            ) . ")))>0))
						THEN TRIM(REPLACE(CONCAT_WS(' '," . CForumUser::GetNameFieldsForQuery(
                $arAddParams["sNameTemplate"]
            ) . "), '  ', ' '))
						ELSE U.LOGIN
					END AS AUTHOR_NAME, 
				...
				CASE
					WHEN ((FUU.SHOW_NAME='Y') AND (LENGTH(TRIM(CONCAT_WS(''," . CForumUser::GetNameFieldsForQuery(
                $arAddParams["sNameTemplate"],
                "UU."
            ) . ")))>0))
					THEN TRIM(REPLACE(CONCAT_WS(' '," . CForumUser::GetNameFieldsForQuery(
