...
class forumTextParser extends CTextParser
{
    ...
    function convert($text, $allow = array(), $type = "html", $arFiles = false)
    {
        ...
        $this->userPath = str_replace(
            array("#UID#", "#uid#"),
            "#user_id#",
            (empty($this->userPath) && !empty($this->pathToUser) ? $this->pathToUser : $this->userPath)
        );
    ...
    function convert4mail($text, $files = false, $allow = array(), $params = array())
    {
        ...
        if (!empty($params)) {
            ...
            $allow = array_merge(
                ((is_array($allow) ? $allow : array()) + array(
                        "HTML" => "N",
                        "ANCHOR" => "Y",
                        "BIU" => "Y",
                        "IMG" => "Y",
    ...
    function ParserFile(&$text, &$obj, $type = "html")
    {
        if (method_exists($obj, "convert_attachment")) {
            ...
            $text = preg_replace_callback(
                "/\[file([^\]]*)id\s*=\s*([0-9]+)([^\]]*)\]/is" . BX_UTF_PCRE_MODIFIER,
                array($this, "convert_attachment"),
                $text
            );
            $obj->type = $tmpType;
        }
    ...
    function convert_open_tag($marker = "quote")
    {
        ...
        if ($this->type == "rss") {
            ...
            if ($this->type == "mail") {
                ...
                if ($this->bMobile) {
                    return "<div class='blog-post-" . $marker . "' title=\"" . ($marker == "quote" ? GetMessage(
                            "FRM_QUOTE"
                        ) : GetMessage("FRM_CODE")) . "\"><table class='blog" . $marker . "'><tr><td>";
                } else {
                    return '<div class="entry-' . $marker . '"><table class="forum-' . $marker . '"><thead><tr><th>' . ($marker == "quote" ? GetMessage(
                            "FRM_QUOTE"
                        ) : GetMessage("FRM_CODE")) . '</th></tr></thead><tbody><tr><td>';
                }
            }
        }
    }
    ...
    function convert_image_tag($url = "", $params = "")
    {
        ...
        $height = 0;
        if (preg_match_all(
            "/width\=(?P<width>\d+)|height\=(?P<height>\d+)/is" . BX_UTF_PCRE_MODIFIER,
            $params,
            $matches
        )):
    ...
    function convert_attachment($fileID = "", $p = "", $type = "", $text = "")
    {
        ...
        if (!array_key_exists($fileID, $this->arFiles) && in_array(
                $fileID,
                $this->arFiles
...
//				$path = ($bIsImage && !empty($arFile["SRC"]) ? $arFile["SRC"] : !$bIsImage && !empty($arFile["URL"]) ? $arFile["URL"] : $path);
                $path = preg_replace(
                    "'(?<!:)/+'s",
                    "/",
                    (mb_substr($path, 0, 1) == "/" ? CHTTP::URN2URI($path, $this->serverName) : $path)
                );
                switch ($type) {
                    case "bbcode":
    ...
    function render_user($fields)
    {
        ...
        return '<a class="blog-p-user-name' . $classAdditional . '" id="bp_' . $anchorId . '" href="' . CComponentEngine::MakePathFromTemplate(
                $pathToUser,
                array("user_id" => $userId)
            ) . '" bx-tooltip-user-id="' . (!$this->bMobile ? $userId : '') . '">' . $userName . '</a>';
    }
}
...
class CForumSimpleHTMLParser
{
    ...
    function skipTags($tmp)
    {
        ...
        for ($i = 0; $i < $n_tags; $i++) {
            if (preg_match('#^\s*' . $tags_quoted[$i]['open'] . '#i' . BX_UTF_PCRE_MODIFIER, $tmp) < 1) {
                continue;
            }
    ...
    function findTagEnd($startIndex)
    {
        ...
        while ($tmp <> '' && preg_match($this->parse_tag, $tmp, $matches) > 0) {
            ...
            if ($matches['closing'] == '/') // close tag
            {
                ...
                if ($matches['selfclosing'] == '/') // self close tag
                {
                    // do nothing
                } else {
                    if ($tag_name == 'LI' && end($arStack) == 'LI') // oh
                    {
                        // do nothing
                    } else // open tag
                    {
...
class CForumCacheManager
{
    ...
    public function OnMessageDelete($ID, $arMessage)
    {
        self::ClearTag(
            "T",
            isset($arMessage["FORUM_TOPIC_ID"]) ? $arMessage["FORUM_TOPIC_ID"] : $arMessage["TOPIC_ID"]
        );
...
class CForumAutosave
{
    ...
    public function LoadScript($arParams)
    {
        ...
        ?>
        <script>
            window.autosave_<?=$id?>_func = function () {
