...
class CAllFilterLetter
{
    ...
    public static function Add($arFields)
    {
        ...
        $db_res = CFilterLetter::GetList(
            array(),
            array(
                "DICTIONARY_ID" => $arFields["DICTIONARY_ID"],
                "LETTER" => trim($arFields["LETTER"])
            )
        );
        $db_res = $db_res->Fetch();
        if ($db_res["ID"] <= 0) {
...
class CAllFilterUnquotableWords
{
    ...
    public static function Add($arFields)
    {
        ...
        $arFields["PATTERN_CREATE"] = in_array(
            $arFields["PATTERN_CREATE"],
            array("WORDS", "PTTRN", "TRNSL")
        ) ? $arFields["PATTERN_CREATE"] : false;
        if (!$arFields["PATTERN_CREATE"]) {
            $arFields["PATTERN_CREATE"] = ($arFields["WORDS"] ? "TRNSL" : "PTTRN");
        }

        $arFields["WORDS"] = trim(
            $arFields["PATTERN_CREATE"] == "TRNSL" ? mb_strtolower($arFields["WORDS"]) : $arFields["WORDS"]
        );
    ...
    public static function Update($ID, $arFields)
    {
        ...
        if (($ID > 0) && (CFilterUnquotableWords::CheckFields("UPDATE", $ID, $arFields))) {
            ...
            $res = $DB->QueryBind(
                $strSql,
                Array("PATTERN" => $arFields["PATTERN"], "DESCRIPTION" => $arFields["DESCRIPTION"]),
                false,
        ...
        while ($lett = $letters->Fetch()) {
            ...
            for ($ii = 0; $ii < count($arrRepl); $ii++) {
                ...
                if (mb_strlen($lett["LETTER"]) == 1) {
                    ...
                    } elseif (mb_substr($arrRepl[$ii], 0, 1) == "(" && (mb_substr(
                                $arrRepl[$ii],
                                -1,
                                1
...
//					$lettPatt[$lett["LETTER"]] = str_replace("+", "*", $lettPatt[$lett["LETTER"]]);
                ...
                $lettersPatt["/" . preg_quote($lett["LETTER"]) . "/is" . BX_UTF_PCRE_MODIFIER] = "(" . implode(
                        "|",
                        $arrRes
                    ) . ")";
            }
    ...
    public static function Filter($message)
    {
        ...
        if (!array_key_exists(LANGUAGE_ID, $arFilterPattern)) {
            ...
            if (CACHED_b_forum_filter !== false && $CACHE_MANAGER->Read(
                ...
                $db_res = CFilterUnquotableWords::GetList(
                    array(),
                    array(
                        "USE_IT" => "Y",
                        "DICTIONARY_ID" => COption::GetOptionInt("forum", "FILTER_DICT_W", false, LANGUAGE_ID)
                    )
                );
