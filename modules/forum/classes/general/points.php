...
class CAllForumPoints
{
            ...
            while ($arLang = $db_lang->Fetch()) {
                ...
                if (!$bFound) {
                    $aMsg[] = array(
                        ...
                        "text" => str_replace(
                            "#LANG#",
                            $arLang["NAME"] . " [" . $arLang["LID"] . "]",
                            GetMessage("FORUM_PE_ERROR_NONAME")
                        )
                    );
                }
            }
    ...
    public static function GetList($arOrder = array("MIN_POINTS" => "ASC"), $arFilter = array())
    {
            ...
            switch ($key) {
                ...
                case "CODE":
                    if ($val == '') {
                        ...
                        $arSqlSearch[] = ($strNegative == "Y" ? " FR.CODE IS NULL OR NOT " : "") . "(FR.CODE " . $strOperation . " '" . $DB->ForSql(
                                $val
                            ) . "' )";
                    }
                    break;
        ...
        foreach ($arOrder as $by => $order) {
            ...
            if ($by == "ID") {
                ...
                $arSqlOrder[] = " FR.CODE " . $order . " ";
            } elseif ($by == "VOTES") {
                $arSqlOrder[] = " FR.VOTES " . $order . " ";
            } else {
    ...
    public static function GetByIDEx($ID, $strLang)
    {
        ...
        $strSql =
            "SELECT FR.ID, FR.MIN_POINTS, FR.CODE, FR.VOTES, FRL.LID, FRL.NAME " .
            "FROM b_forum_points FR " .
            "	LEFT JOIN b_forum_points_lang FRL ON (FR.ID = FRL.POINTS_ID AND FRL.LID = '" . $DB->ForSql(
                $strLang
            ) . "') " .
            "WHERE FR.ID = " . $ID . "";
        $db_res = $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
...
class CAllForumPoints2Post
{
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if (is_set($arFields, "MIN_NUM_POSTS") || $ACTION == "ADD") {
            ...
            } elseif (mb_strlen($arFields["MIN_NUM_POSTS"]) > 18 || preg_match(
                    "/[^0-9]/",
                    $arFields["MIN_NUM_POSTS"]
                )) {
                $aMsg[] = array(
                    "id" => 'POINTS2POST[MIN_NUM_POSTS]',
        ...
        if ((is_set($arFields, "POINTS_PER_POST") || $ACTION == "ADD") && DoubleVal(
                $arFields["POINTS_PER_POST"]
            ) <= 0) {
            $arFields["POINTS_PER_POST"] = 0;
        } else {
            $arFields["POINTS_PER_POST"] = round(doubleval($arFields["POINTS_PER_POST"]), 4);
            if (mb_strlen(round($arFields["POINTS_PER_POST"], 0)) > 14 || mb_strlen(
                    mb_strstr($arFields["POINTS_PER_POST"], ".")
                ) > 5 ||
...
class CAllForumUserPoints
{
    ...
    public static function CheckFields($ACTION, &$arFields)
    {
        ...
        if ((is_set($arFields, "POINTS") || $ACTION == "ADD") && intval($arFields["POINTS"]) <= 0) {
            return false;
        }
    ...
    public static function Delete($FROM_USER_ID, $TO_USER_ID)
    {
        ...
        $DB->Query(
            "DELETE FROM b_forum_user_points WHERE FROM_USER_ID = " . $FROM_USER_ID . " AND TO_USER_ID = " . $TO_USER_ID
        );
    ...
    public static function GetList($arOrder = array("TO_USER_ID" => "ASC"), $arFilter = array())
    {
        ...
        foreach ($arOrder as $by => $order) {
            ...
            if ($by == "FROM_USER_ID") {
                $arSqlOrder[] = " FR.FROM_USER_ID " . $order . " ";
            } elseif ($by == "POINTS") {
                $arSqlOrder[] = " FR.POINTS " . $order . " ";
            } elseif ($by == "DATE_UPDATE") {
                $arSqlOrder[] = " FR.DATE_UPDATE " . $order . " ";
            } else {
