...
class CAllForumTopic
{
    ...
    public static function CanUserAddTopic(
        $FID,
        $arUserGroups,
        $iUserID = 0,
        $arForum = false,
    ...
    public static function CanUserUpdateTopic($TID, $arUserGroups, $iUserID = 0, $ExternalPermission = false)
    {
        ...
        if ($arTopic) {
            ...
            $db_res = CForumMessage::GetList(
                array("ID" => "ASC"),
                array("TOPIC_ID" => $TID, "FORUM_ID" => $arTopic["FORUM_ID"]),
                false,
                2
            );
            $iCnt = 0;
    ...
    public static function CheckFields($ACTION, &$arFields)
    {
        ...
        if (is_set($arFields, "TITLE_SEO") || $ACTION == "ADD") {
            ...
            if ($arFields["TITLE_SEO"] == '' && $arFields["TITLE"] <> '') {
                $arFields["TITLE_SEO"] = CUtil::translit(
                    $arFields["TITLE"],
                    LANGUAGE_ID,
                    array(
                        "max_len" => 255,
        ...
        if (is_set($arFields, "USER_START_ID") || $ACTION == "ADD") {
            $arFields["USER_START_ID"] = (intval($arFields["USER_START_ID"]) > 0 ? intval(
                $arFields["USER_START_ID"]
            ) : false);
        }
        if (is_set($arFields, "LAST_POSTER_ID") || $ACTION == "ADD") {
            $arFields["LAST_POSTER_ID"] = (intval($arFields["LAST_POSTER_ID"]) > 0 ? intval(
                $arFields["LAST_POSTER_ID"]
            ) : false);
        }
        if (is_set($arFields, "LAST_MESSAGE_ID") || $ACTION == "ADD") {
            $arFields["LAST_MESSAGE_ID"] = (intval($arFields["LAST_MESSAGE_ID"]) > 0 ? intval(
                $arFields["LAST_MESSAGE_ID"]
            ) : false);
        }
        ...
        if (is_set($arFields, "SOCNET_GROUP_ID") || $ACTION == "ADD") {
            $arFields["SOCNET_GROUP_ID"] = (intval($arFields["SOCNET_GROUP_ID"]) > 0 ? intval(
...
//				$DB->Commit();
                ...
                $res_log["DESCRIPTION"] = str_replace(
                    array("#TOPIC_TITLE#", "#TOPIC_ID#", "#FORUM_TITLE#", "#FORUM_ID#"),
...
//		$DB->Commit();
    ...
    public static function GetByID($ID, $arAddParams = array())
    {
        ...
        $NoFilter = ($arAddParams["NoFilter"] == true || COption::GetOptionString(
            "forum",
            "FILTER",
            "Y"
        ) != "Y" ? true : false);

        if ($NoFilter && isset($GLOBALS["FORUM_CACHE"]["TOPIC"][$ID]) && is_array(
                $GLOBALS["FORUM_CACHE"]["TOPIC"][$ID]
            ) && is_set($GLOBALS["FORUM_CACHE"]["TOPIC"][$ID], "ID")) {
            return $GLOBALS["FORUM_CACHE"]["TOPIC"][$ID];
        } elseif (!$NoFilter && isset($GLOBALS["FORUM_CACHE"]["TOPIC_FILTER"][$ID]) && is_array(
                $GLOBALS["FORUM_CACHE"]["TOPIC_FILTER"][$ID]
            ) && is_set($GLOBALS["FORUM_CACHE"]["TOPIC_FILTER"][$ID], "ID")) {
    ...
    public static function GetByIDEx($ID, $arAddParams = array())
    {
        ...
        $strSql =
            "SELECT FT.*,\n" .
            "	FT.TITLE_SEO as TITLE_SEO_REAL, " . CForumNew::Concat(
                "-",
                array("FT.ID", "FT.TITLE_SEO")
            ) . " as TITLE_SEO, \n" .
            "	" . $DB->DateToCharFunction("FT.START_DATE", "FULL") . " as START_DATE, \n" .
            "	" . $DB->DateToCharFunction("FT.LAST_POST_DATE", "FULL") . " as LAST_POST_DATE, \n" .
    ...
    public static function GetSelectFields($arAddParams = array(), $fields = array())
    {
        ...
        $arAddParams["sTablePrefix"] = $DB->ForSql(
            empty($arAddParams["sTablePrefix"]) ? "FT." : $arAddParams["sTablePrefix"]
        );
        ...
        $fields = array_merge(
            array(
                ...
                "TITLE_SEO" => CForumNew::Concat(
                    "-",
                    array(
                        $arAddParams["sTablePrefix"] . "ID",
                        $arAddParams["sTablePrefix"] . "TITLE_SEO"
                    )
                ),
                "TAGS" => "TAGS",
        ...
        if (!$update) {
            ...
            if ($arTopic) {
                ...
                if (intval($LastVisit) > 0) {
                    $Fields["LAST_VISIT"] = $DB->CharToDateFunction(
                        $DB->ForSql(Date(CDatabase::DateFormatToPHP(CLang::GetDateFormat("FULL")), $LastVisit)),
                        "FULL"
                    );
                }
                ...
                if ($DB->type == "MYSQL") {
                    ...
                    $strSql = "INSERT INTO b_forum_user_topic (" . implode(", ", $ar1) . ") VALUES(" . implode(
                            ", ",
                            $ar2
                        ) . ") ON DUPLICATE KEY UPDATE " . implode(", ", $ar3);
            ...
            $result = $DB->Update(
                "b_forum_user_topic",
                $Fields,
                "WHERE (FORUM_ID=" . $ID . " AND USER_ID=" . intval($USER->GetID()) . ")",
                "File: " . __FILE__ . "<br>Line: " . __LINE__
            );
            $DB->StopUsingMasterOnly();
            return $result;
    ...
    public static function SetStat($ID = 0, $arParams = array())
    {
        ...
        if (!empty($arMessage)) {
            ...
            if (intval($res["ABS_FIRST_MESSAGE_ID"]) > 0 && intval($res["ABS_FIRST_MESSAGE_ID"]) != intval(
                    $res["FIRST_MESSAGE_ID"]
                )) {
                $GLOBALS["DB"]->Query(
                    "UPDATE b_forum_message SET NEW_TOPIC = (CASE WHEN ID=" . intval(
                        $res["ABS_FIRST_MESSAGE_ID"]
                    ) . " THEN 'Y' ELSE 'N' END) WHERE TOPIC_ID=" . $ID,
                    false,
                    "File: " . __FILE__ . "<br>Line: " . __LINE__
                );
            ...
            foreach (
                array(
                    ...
                    "ABS_LAST_POSTER_NAME" => "USER_START_NAME"
                ) as $key => $val
            ) {
            ...
            if ($this->noFilter !== true) {
                ...
                if (is_set($res, "FIRST_POST") || is_set($res, "LAST_POST")) {
                    ...
                    if (!empty($arSqlSearch)):
                        $strSql = "SELECT FM.ID, " . $DB->DateToCharFunction(
                                "FM.POST_DATE",
                                "FULL"
                            ) . " AS POST_DATE " .
