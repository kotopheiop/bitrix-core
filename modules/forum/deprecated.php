...
$GLOBALS["aSortTypes"] = array(
    "reference" => array(
        GetMessage("FDATE_LAST_MESSAGE"),
        GetMessage("FMESSAGE_TOPIC"),
        GetMessage("FNUM_ANSWERS"),
        GetMessage("FNUM_VIEWS"),
        GetMessage("FSTART_DATE"),
        GetMessage("FAUTHOR_TOPIC")
    ),
...
$GLOBALS["aForumPermissions"] = array(
    "reference" => array(
        GetMessage("FNO_ACCESS"),
        GetMessage("FREAD_ACCESS"),
        GetMessage("FANSWER_ACCESS"),
        GetMessage("FNEW_MESSAGE_ACCESS"),
        GetMessage("FMODERATE_ACCESS"),
        GetMessage("FEDIT_ACCESS"),
        GetMessage("FFULL_ACCESS")
    ),
...
function ForumInitParams()
{
    ...
    if ($GLOBALS["USER"]->IsAuthorized()) {
        if (!is_array($_SESSION["FORUM"]["USER"]) || $_SESSION["FORUM"]["USER"]["USER_ID"] != $GLOBALS["USER"]->GetID(
            ...
            if ($_SESSION["FORUM"]["USER"]):
                $_SESSION["FORUM"]["USER"]["LAST_VISIT_TIMESTAMP"] = MakeTimeStamp(
                    $_SESSION["FORUM"]["USER"]["LAST_VISIT"]
                );
            else:
        ...
        if (!is_array($_SESSION["FORUM"][$UserLogin]) || intVal($_SESSION["FORUM"][$UserLogin][0]) <= 0) {
            ...
            if (intVal($_SESSION["FORUM"][$UserLogin][0]) <= 0):
                ...
                $db_res = CForumUser::GetListUserForumLastVisit(
                    array(),
                    array(
    ...
    } else // If user is not authorized that get info from cookies only
    {
        ...
        if (!isset($_SESSION["FORUM"]["GUEST"]) || !is_array($_SESSION["FORUM"]["GUEST"]) || (intVal(
                    $_SESSION["FORUM"]["GUEST"][0]
                ) < 0)) {
...
function NewMessageTopic($FID, $TID, $LAST_POST_DATE, $LAST_VISIT)
{
    ...
    $LAST_VISIT = intVal(
        $GLOBALS["USER"]->IsAuthorized() ? MakeTimeStamp($LAST_VISIT) : $_SESSION["FORUM"]["GUEST_TID"][$TID]
    );
    $LAST_VISIT = max(
        $LAST_VISIT,
        $_SESSION["FORUM"]["LAST_VISIT_FORUM_0"],
        intVal($_SESSION["FORUM"]["LAST_VISIT_FORUM_" . $FID])
    );
...
function ForumGetFirstUnreadMessage($FID, $TID)
{
    ...
    if ($TID > 0) {
        ...
        $LastVisit = max(
            intVal($_SESSION["FORUM"]["LAST_VISIT_FORUM_0"]),
            intVal($_SESSION["FORUM"]["LAST_VISIT_FORUM_" . $FID])
        ); // client TZ
        ...
        if ($USER->IsAuthorized()) {
            $db_res = CForumMessage::GetListEx(
                ...
                array(
                    "TOPIC_ID" => $TID,
                    "USER_ID" => $USER->GetId(),
                    ">NEW_MESSAGE" => Date(CDatabase::DateFormatToPHP(CLang::GetDateFormat("FULL", LANG)), $LastVisit)
                ),
                0,
                1
            );
            ...
            $db_res = CForumMessage::GetList(
                ...
                array(
                    "TOPIC_ID" => $TID,
                    ">POST_DATE" => Date(CDatabase::DateFormatToPHP(CLang::GetDateFormat("FULL", LANG)), $LastVisit)
                ),
                0,
                1
            );
    ...
    <script>
        ...
        window.LHEDailogs['ForumVideo'] = function (pObj) {
            var str = '<table width="100%"><tr>' +
                '<td class="lhe-dialog-label lhe-label-imp"><label for="' + pObj.pLEditor.id + 'lhed_forum_video_path"><b><?= GetMessage(
                    'FR_VIDEO_P'
                )?>:</b></label></td>' +
                ...
                '</tr><tr>' +
                '<td></td>' +
                '<td style="padding: 0!important; font-size: 11px!important;"><?= GetMessageJS(
                    'FR_VIDEO_PATH_EXAMPLE'
                )?></td>' +
                '</tr><tr>' +
