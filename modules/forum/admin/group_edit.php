    ...
    <form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>" name="fform">
        ...
        foreach ($fields["LANG"] as $lid => $res) {
            ...
            <tr class="adm-detail-required-field">
                ...
                <td><input type="text" name="FORUM_GROUP[LANG][<?= htmlspecialcharsbx($lid) ?>][NAME]"
                           value="<?= htmlspecialcharsbx($res["NAME"]) ?>" size="40"/></td>
            </tr>
            <tr>
                <td><?= GetMessage("FORUM_DESCR") ?>:</td>
                <td><textarea name="FORUM_GROUP[LANG][<?= htmlspecialcharsbx($lid) ?>][DESCRIPTION]" rows="3"
                              cols="40"><?= htmlspecialcharsbx($res["DESCRIPTION"]) ?></textarea></td>
            </tr>
            <?
        }
