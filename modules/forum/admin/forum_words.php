...
$lAdmin->AddHeaders(
    [
        ...
        [
            "id" => "PATTERN_CREATE",
            "content" => GetMessage("FLT_PATTERN_CREATE"),
            "sort" => "PATTERN_CREATE",
            "default" => true
        ],
...
while ($res = $rsData->NavNext(true, "t_")) {
    ...
    $row->AddViewField(
        ...
        '<a title="' . GetMessage(
            "FLT_ACT_EDIT"
        ) . '" href="' . "forum_words_edit.php?DICTIONARY_ID=" . $res["DICTIONARY_ID"] . "&ID=" . $res["ID"] . "&amp;lang=" . LANG . '">' . $res["ID"] . '</a>'
    );
    ...
    $row->AddSelectField(
        "PATTERN_CREATE",
        array(
            "WORDS" => GetMessage("FLT_FLT_WORDS"),
            "TRNSL" => GetMessage("FLT_FLT_TRNSL"),
            "PTTRN" => GetMessage("FLT_FLT_PTTRN")
        )
    );
    $row->AddActions(
        [
            [
                ...
                "ACTION" => $lAdmin->ActionRedirect(
                    "forum_words_edit.php?DICTIONARY_ID=" . $res["DICTIONARY_ID"] . "&lang=" . LANG . "&ID=" . $res["ID"] . GetFilterParams(
                        "filter_",
                        false
                    ) . ""
                ),
            ...
            [
                ...
                "ACTION" => "if(confirm('" . GetMessage("FLT_ACT_DEL_CONFIRM") . "')) " . $lAdmin->ActionDoGroup(
                        $res["ID"],
                        "delete",
                        "DICTIONARY_ID=" . $res["DICTIONARY_ID"] . "&lang=" . LANG
                    )
            ]
        ]
    );
...
<? $oFilter->Begin(); ?>
    <tr>
        ...
        <td>
            <input type="text" size="47" name="find_pattern"
                   value="<?= htmlspecialcharsbx(isset($filter["find_pattern"]) ? $filter["find_pattern"] : "") ?>"
            ...
            echo SelectBoxFromArray(
                "find_type",
                $arr,
                (isset($filter["find_type"]) ? $filter["find_type"] : ""),
                "",
                ""
            ); ?>
        </td>
    ...
    <tr>
        ...
        <td><?= SelectBoxFromArray(
                ...
                [
                    "REFERENCE" => [
                        "",
                        GetMessage("FLT_FLT_WORDS"),
                        GetMessage("FLT_FLT_TRNSL"),
                        GetMessage("FLT_FLT_PTTRN")
                    ],
                    "REFERENCE_ID" => ["all", "WORDS", "TRNSL", "PTTRN"]
                ],
