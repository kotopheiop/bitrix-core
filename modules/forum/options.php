...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $FORUM_RIGHT == "W" && $_REQUEST["Update"] <> '' && check_bitrix_sessid()) {
    ...
    COption::SetOptionString(
        "forum",
        "parser_link_target",
        ($_REQUEST["parser_link_target"] == "_blank" ? "_blank" : "_self")
    );
...
$aTabs = array(
    ...
    array(
        "DIV" => "edit3",
        "TAB" => GetMessage("USE_FILTER"),
        "ICON" => "vote_settings",
        "TITLE" => GetMessage("USE_FILTER")
    ),
);
...
<form method="POST"
    ...
    <tr>
        ...
        <td width="60%">
            ...
            <input type="text" size="35" maxlength="255" value="<?= htmlspecialcharsbx($val) ?>"
                   name="FORUM_FROM_EMAIL"/></td>
    </tr>
    <tr>
        <td><label for="SHOW_VOTES"><?= GetMessage("FORUM_GG_SHOW_VOTE") ?></label></td>
        <td>
            <? $val = COption::GetOptionString("forum", "SHOW_VOTES", "Y"); ?>
            <input type="checkbox" value="Y" name="SHOW_VOTES" id="SHOW_VOTES" <? if ($val == "Y") {
                echo "checked";
            } ?>></td>
    </tr>
    ...
    <? if (($val = COption::GetOptionString("forum", "FORUM_GETHOSTBYADDR", "N")) == "Y"): ?>
        <tr>
            ...
            <td><input type="checkbox" value="Y" name="FORUM_GETHOSTBYADDR" id="FORUM_GETHOSTBYADDR" checked="checked"/>
            </td>
        </tr>
    <? endif; ?>
        ...
        <td>
            ...
            <div class="adm-list">
                <div class="adm-list-item">
                    <div class="adm-list-control"><input type="checkbox" name="LOGS" id="LOGS"
                                                         value="Y" <?= ($val > "A" ? "checked='checked'" : "") ?> <?
                                                         ?>onclick="BX('log-additional').style.display=(this.checked ? 'block' : 'none')">
                    </div>
                ...
                <div id="log-additional" class="adm-list-item"<?= ($val <= "A" ? " style='display:none;'" : "") ?>>
                    <div class="adm-list-control"><input type="checkbox" name="LOGS_ADDITIONAL" ID="LOGS_ADDITIONAL"
                                                         value="Y" <?= ($val > "Q" ? "checked='checked'" : "") ?>></div>
    ...
    <tr class="heading">
        <td colspan="2"><?= GetMessage("F_USER_SETTINGS") ?></td>
    </tr>
    <tr>
        ...
        <td>
            ...
            <select name="USER_EDIT_OWN_POST" id="USER_EDIT_OWN_POST">
                <option value="Y" <? if ($val == "Y") {
                    echo "selected";
                } ?>><?= GetMessage("FORUM_USER_EDIT_OWN_POST_Y") ?></option>
                <option value="N" <? if ($val != "Y") {
                    echo "selected";
                } ?>><?= GetMessage("FORUM_USER_EDIT_OWN_POST_N") ?></option>
            </select>
    </tr>
    <tr>
        ...
        <td>
            ...
            <input type="checkbox" value="Y" name="USER_SHOW_NAME" id="USER_SHOW_NAME" <? if ($val == "Y") {
                echo "checked";
            } ?>></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            ...
            <input type="checkbox" value="Y" name="parser_nofollow" id="parser_nofollow" <? if ($val == "Y") {
                echo "checked";
            } ?>></td>
    </tr>
    <tr>
        ...
        <td>
            ...
            <input type="checkbox" value="_blank" name="parser_link_target"
                   id="parser_link_target" <? if ($val == "_blank") {
                echo "checked";
            } ?>></td>
    </tr>
    <tr>
        ...
        <td>
            ...
            <input type="checkbox" value="Y" name="USE_AUTOSAVE" id="USE_AUTOSAVE" <? if ($val == "Y") {
                echo "checked";
            } ?>></td>
    </tr>
    <tr>
        ...
        <td>
            ...
            <input type="text" size="35" maxlength="255" value="<?= htmlspecialcharsbx($val) ?>"
                   name="avatar_max_size"/></td>
    </tr>
    <tr>
        <td><?= GetMessage("FORUM_GG_AVATAR_W") ?>:</td>
        <td>
            <? $val = COption::GetOptionString("forum", "avatar_max_width", 100); ?>
            <input type="text" size="14" maxlength="255" value="<?= htmlspecialcharsbx($val) ?>"
    ...
    <tr>
        ...
        <td>
            ...
            <input type="text" size="35" maxlength="255" value="<?= htmlspecialcharsbx($val) ?>" name="file_max_size">
        </td>
    </tr>
    <tr class="heading">
        <td colspan="2"><?= GetMessage("F_PM_SETTINGS") ?></td>
    </tr>
    <tr>
        ...
        <td>
            ...
            <select name="UsePMVersion" id="UsePMVersion" onclick="OnClickUsePMVersion(this)">
                <option value="none" <? if ($val != "1" && $val != "2") {
                    echo "selected";
                } ?>><?= GetMessage("FO_USEPMVERSION") ?></option>
    ...
    <tr class="heading">
        <td colspan="2"><?= GetMessage("F_FORUM_STATUSES") ?></td>
    </tr>
    <tr>
            ...
            <table border="0" class="internal" style="width:auto;">
                ...
                foreach ($arNameStatuses as $lid => $names):
                    ...
                    <tr>
                        ...
                        foreach ($names as $key => $val):
                            ?>
                            <td><input type="text" style="width:110px" name="STATUS_NAME[<?= $lid ?>][<?= $key ?>]"
                                       value="<?= htmlspecialcharsbx($val) ?>"/></td>
                        <?
                        endforeach;
                        ?>
                    </tr>
    ...
    <tr>
        ...
        <td width="60%">
            ...
            <input type="checkbox" value="Y" name="FILTER" id="FILTER" <? if ($val == "Y") {
                echo "checked";
            } ?> onclick="DisableAction(this)"></td>
    </tr>
    <tr>
        ...
        <td>
            <? echo SelectBoxFromArray(
                ...
                array(
                    ...
                    "REFERENCE_ID" => array("non", "del", "rpl")
                ),
                COption::GetOptionString("forum", "FILTER_ACTION", "rpl")
            ) ?>
        </td>
    </tr>
    <tr>
    ...
    <script language="JavaScript">
        function DisableAction(CheckB) {
            ...
            if (CheckB.checked) {
                ...
                Form.FILTER_ACTION.value = '<?=CUtil::JSEscape(
                    COption::GetOptionString("forum", "FILTER_ACTION", "rpl")
                )?>';
        ...
        <td colspan="2" align="center">
            <table border="0" cellspacing="6" class="internal" style="width:auto;">
                <tr class="heading">
                    ...
                    <td align="center"><span id="SECTION_NAME_TITLE"><?= GetMessage(
                ...
                while ($ar = $l->ExtractFields("l_")) {
                    ...
                    <td><?= SelectBoxFromArray(
                            "FILTER_DICT[" . $ar["LID"] . "][W]",
                            $Dict["W"],
                            COption::GetOptionString("forum", "FILTER_DICT_W", '', $ar["LID"])
                        ) ?></td>
                    <td><?= SelectBoxFromArray(
                            "FILTER_DICT[" . $ar["LID"] . "][T]",
                            $Dict["T"],
                            COption::GetOptionString("forum", "FILTER_DICT_T", '', $ar["LID"])
                        ) ?></td>
                    </tr><?
                }
                ?></table>
    ...
    <script language="JavaScript">
        function RestoreDefaults() {
            if (confirm('<?=CUtil::JSEscape(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>'))
                ...
                )?>?RestoreDefaults=Y&lang=<?echo LANG?>&mid=<?=urlencode($module_id)?>&<?=bitrix_sessid_get()?>";
        }
    ...
    <input <? if ($FORUM_RIGHT < "W") echo "disabled" ?> type="submit" class="adm-btn-green" name="Update"
                                                         value="<? echo GetMessage("PATH_SAVE") ?>"/>
    <input type="hidden" name="Update" value="Y"/>
    <input type="reset" name="reset" value="<? echo GetMessage("PATH_RESET") ?>"/>
    <input <? if ($FORUM_RIGHT < "W") echo "disabled" ?> type="button"
