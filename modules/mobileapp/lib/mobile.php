...
class Mobile
{
    ...
    private function __construct()
    {
        ...
        if (mb_strrpos(ToUpper($deviceDetectSource), "IPHONE") > 0 || mb_strrpos(
                ToUpper($deviceDetectSource),
                "IPAD"
            ) > 0) {
            self::$platform = "ios";
        } elseif (mb_strrpos(ToUpper($deviceDetectSource), "ANDROID") > 0 || mb_strrpos(
                ToUpper($deviceDetectSource),
                "ANDROID"
            ) === 0) {
...
JSCODE;
        ...
        if (mb_strpos($userAgent, "WKWebView/BitrixMobile") === false) {
            if (self::getInstance()->getBXScriptSupported()) {
                ...
                $APPLICATION->AddHeadString(
                    "<script type=\"text/javascript\" src=\"" . $pgJsFile . "\"></script>",
                    false,
                    true
                );
            } else {
                ...
                $APPLICATION->AddHeadString(
                    "<script type=\"text/javascript\" src=\"" . \CUtil::GetAdditionalFileURL(
                        $pgJsFile
                    ) . "\"></script>",
                    false,
                    true
        ...
        if (self::$platform == "android") {
            $APPLICATION->AddHeadString(
                "<script type=\"text/javascript\">app.bindloadPageBlank();</script>",
                false,
                false
            );
        }
