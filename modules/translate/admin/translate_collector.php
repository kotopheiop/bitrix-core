...
?>
    <form method="post" action="" name="form1">
        ...
        <tr class="adm-required-field">
            ...
            <td><input type="text" name="lang_date" size="10" maxlength="8"
                       value="<?= htmlspecialcharsbx($langDate) ?>"></td>
        </tr>
        <?
        if (!$isUtfMode && !$useTranslationRepository) {
            ...
            <tr>
                ...
                <td><input type="checkbox" name="convert_encoding"
                           value="Y" <?= ($convertEncoding ? 'checked="checked"' : '') ?>></td>
            </tr>
            <?
        } else {
            ?>
            <tr>
                <td><?
                    echo Loc::getMessage("TR_CONVERT_NATIONAL") ?>:
                </td>
                <td><input type="checkbox" name="convert_encoding"
                           value="Y" <?= ($convertEncoding ? 'checked="checked"' : '') ?> onClick="EncodeClicked()">
                </td>
            </tr>
            <tr>
                <td width="40%"><?
                    echo Loc::getMessage("TR_CONVERT_ENCODING") ?>:
                </td>
                <td width="60%">
                    <select name="encoding">
                        ...
                        foreach ($allowedEncodings as $enc) {
                            ...
                            if ($enc == $encoding) echo " selected"; ?>><?= $encTitle ?></option><?
                        }
                        ?>
                    </select>
...
?>
    <form method="post" action="" name="form2" enctype="multipart/form-data">
        ...
        <tr class="adm-required-field">
            <td width="40%"><?= Loc::getMessage("TR_SELECT_LANGUAGE") ?> <?= Loc::getMessage("TR_SELECT_LANGUAGE_DESCRIPTION") ?>
                :
            </td>
            <td width="60%">
                <select name="language_id">
        ...
        if (!$isUtfMode && !$useTranslationRepository) {
            ...
            <tr>
                ...
                <td><input type="checkbox" name="localize_encoding"
                           value="Y" <?= ($convertEncoding ? 'checked="checked"' : ''); ?>></td>
            </tr>
            <?
        } else {
            ...
            <tr>
                ...
                <td><input type="checkbox" id="localize_encoding" name="localize_encoding"
                           value="Y" <?= ($convertEncoding ? 'checked="checked"' : ''); ?>></td>
            </tr>
...
$tabControl->Buttons();
?>
    <input type="submit" id="tr_submit" class="adm-btn-green"
           value="<?= ($request->get('tabControl_active_tab') === 'download' ? Loc::getMessage("TR_DOWNLOAD_LANGUAGE") : Loc::getMessage("TR_COLLECT_LANGUAGE")) ?>">
<?
$tabControl->End();
?>
    <script type="text/javascript">
