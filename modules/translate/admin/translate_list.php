...
if (!$useSearch && !empty($arrChain)) {
    ...
    foreach ($arrChain as $row) {
        ...
        if ($row['PATH'] !== '') {
            $last_path = $row['PATH'];
            ?><a href="?lang=<?= LANGUAGE_ID ?>&path=<?= urlencode($last_path); ?>"
                 title="<?= Loc::getMessage('TR_FOLDER_TITLE'); ?>"><?= htmlspecialcharsbx($row['NAME']); ?></a><?
        } else {
    ...
    <form action="<?= $APPLICATION->GetCurPage() ?>">
        ...
        <table cellspacing="0">
            <tr>
                ...
                <td style="padding-left:5px;"><input class="form-text" type="text" name="path" id="path_to" size="50"
                                                     value="<?= htmlspecialcharsbx($path) ?>"></td>
                <td style="padding-left:3px; padding-right:3px;"><input type="submit"
                                                                        value="<?= Loc::getMessage("TRANS_GO") ?>"
                                                                        class="form-button"></td>
            </tr>
        </table>
    </form>
<?
...
if ($permissionRight == Translate\Permission::WRITE) {
    ...
    <form action="<?= $APPLICATION->GetCurPageParam('go_path=' . htmlspecialcharsbx($path), array('go_path')) ?>"
          name="form3" method="post" enctype="multipart/form-data">
        <?= bitrix_sessid_post() ?>
        <input type="hidden" name="lang" value="<?= LANGUAGE_ID; ?>">
        ...
        <tr>
            ...
            <td valign="top">
                <input id="F_ACTION_1" type="radio" name="rewrite_lang_files" value="N" checked><label
                        for="F_ACTION_1"><?= Loc::getMessage('TR_NO_REWRITE_LANG_FILES') ?></label><br>
                <input id="F_ACTION_3" type="radio" name="rewrite_lang_files" value="U"><label
                        for="F_ACTION_3"><?= Loc::getMessage('TR_UPDATE_LANG_FILES') ?></label><br>
                <input id="F_ACTION_2" type="radio" name="rewrite_lang_files" value="Y"><label
                        for="F_ACTION_2"><?= Loc::getMessage('TR_REWRITE_LANG_FILES') ?></label>
            </td>
        </tr>
        <?
                ...
                <td width="60%">
                    <select name="encoding">
                        ...
                        foreach ($allowedEncodings as $enc) {
                            ...
                            ?>
                            <option value="<?= htmlspecialcharsbx($enc); ?>"<?
                            if ($isSelected) echo " selected"; ?>><?= $encTitle ?></option><?
                        }
                        ?>
                    </select>
                </td>
    ...
    <form action="translate_csv_download.php" name="form4" method="post">
        ...
        <tr>
            ...
            <td valign="top">
                <input id="F_ACTION_2_1" type="radio" name="download_translate_lang" value="A" checked><label
                        for="F_ACTION_2_1"><?= Loc::getMessage('TR_DOWNLOAD_LANG') ?></label><br>
                <input id="F_ACTION_2_2" type="radio" name="download_translate_lang" value="N"><label
                        for="F_ACTION_2_2"><?= Loc::getMessage('TR_DOWNLOAD_NO_TRANSLATE') ?></label>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <select name="languages[]" multiple="multiple"
                        size="<?= (count($enabledLanguages) <= 7 ? count($enabledLanguages) : 7) ?>">
                    ...
                    while ($row = $iterator->fetch()) {
                        ...
                        ?>
                        <option value="<?= $row['ID'] ?>" <?= ($isSelected ? ' selected=""' : '') ?>><?= $row['NAME'] ?>
                        (<?= $row['ID'] ?>)</option><?
                    }
                    ?>
                </select>
