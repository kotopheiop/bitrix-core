...
?>
    ...
    <form name="form1" method="POST"
          action="<?= $APPLICATION->GetCurPage() ?>?show_error=<?= htmlspecialcharsbx($showOnlyUntranslated) ?>&file=<?= htmlspecialcharsbx($path) ?>&lang=<?= LANGUAGE_ID ?>">
        <?= bitrix_sessid_post() ?>
        <?
                ...
                <table border="0" cellspacing="3" cellpadding="3" width="100%">
                    ...
                    <tr>
                        <td valign="top" align="right" width="35%"
                            nowrap><? echo Loc::getMessage("TRANS_FILENAME") ?></td>
                        <td valign="top" align="left" width="65%" nowrap>
                            <b><?= htmlspecialcharsbx(basename($path)) ?></b></td>
                    </tr>
                    <tr>
                                    ...
                                    if (($showOnlyUntranslated == 'Y' && $red) || $showOnlyUntranslated == 'N') {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                if ($red) {
                                                    ?><span class="required">
                                                    <b><?= htmlspecialcharsbx($phraseId); ?></b></span><?
                                                } else {
                                            ...
                                            <td style="text-align:right;white-space:nowrap;">
                                                ...
                                                <input type="checkbox" name="DEL_<?= $phraseId ?>" value="Y" <?= $s ?>
                                                       id="DEL_<?= $key_del ?>" onclick="SelectOneDelete(this);">
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        foreach ($enabledLanguages as $langId) {
                                            ...
                                            ?>
                                            <tr>
                                                <td valign="top">[<?= $langId ?>
                                                    ]&nbsp;<?= $arTLanguages[$langId]["NAME"] ?>:&nbsp;
                                                </td>
                                                <td colspan="2">
                                                    <input type="hidden" name="<?= $inpKey ?>_PREV"
                                                           value="<?= (!empty($arMESS[$langId][$phraseId]) ? 'Y' : '') ?>">
                                                    <textarea cols="60" rows="<?= $rows ?>" name="<?= $inpKey ?>"
                                                              style="width:90%"><?= $valMsg ?></textarea>
                                                </td>
                                            </tr>
                                            <?
                                        }
                                        ...
                                        foreach ($enabledLanguages as $langId) {
                                            ...
                                            <input type="hidden" name="<?= $inpKey ?>_PREV"
                                                   value="<?= (!empty($arMESS[$langId][$phraseId]) ? 'Y' : '') ?>">
                                            <input type="hidden" name="<?= $inpKey ?>"
                                                   value="<?= $htmlSpecialChars($arMESS[$langId][$phraseId]) ?>">
                                            <?
                                        }
                                    }
                                }
                    ...
                    if ($permissionRight >= Translate\Permission::WRITE && $boolShowDeleteFromCur) {
                        ...
                        <tr>
                            <td valign="top" align="right" nowrap colspan="2">
                                ...
                                <input type="checkbox" name="del_current" id="del_current"
                                       onclick="SelectDeleteCurrent(this);">
                            </td>
                        </tr>
                        <?
                    }
                        ...
                        <tr>
                            <td valign="top" align="right" nowrap colspan="2">
                                ...
                                <input type="checkbox" name="all" id="all" value=""
                                       onclick="SelectAllDelete('<?= $key_del ?>');"<?
                                if ($permissionRight < Translate\Permission::WRITE) echo " disabled"; ?>>
                            </td>
                        </tr>
                        <?
                    }
...
if ($permissionRight == Translate\Permission::WRITE) {
    ...
    <form action="translate_csv_download.php" name="form4" method="post">
        ...
        <tr>
            ...
            <td valign="top">
                <label>
                    <input type="radio" name="download_translate_lang" value="A" <?
                    if (!$showOnlyUntranslated): ?>checked<?
                    endif ?>>
                    <?= Loc::getMessage('TRANS_GET_FULL_TRANSLATE_TITLE') ?>
                </label><br>
                <label>
                    <input type="radio" name="download_translate_lang" value="N" <?
                    if ($showOnlyUntranslated): ?>checked<?
                    endif ?>>
