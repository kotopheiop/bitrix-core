...
class CAllBlogComment
{
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
        ...
        if ((is_set($arFields, "BLOG_ID") || $ACTION == "ADD") && intval($arFields["BLOG_ID"]) <= 0) {
            ...
            if (!$arResult) {
                $APPLICATION->ThrowException(
                    str_replace("#ID#", $arFields["BLOG_ID"], GetMessage("BLG_GCM_ERROR_NO_BLOG")),
                    "ERROR_NO_BLOG"
                );
                return false;
            }
        ...
        if ((is_set($arFields, "POST_ID") || $ACTION == "ADD") && intval($arFields["POST_ID"]) <= 0) {
            ...
            if (!$arResult) {
                $APPLICATION->ThrowException(
                    str_replace("#ID#", $arFields["POST_ID"], GetMessage("BLG_GCM_ERROR_NO_POST")),
                    "ERROR_NO_POST"
                );
                return false;
            }
        ...
        if (is_set($arFields, "PARENT_ID") && $arFields["PARENT_ID"]) {
            ...
            if (!$arResult) {
                $APPLICATION->ThrowException(
                    str_replace("#ID#", $arFields["PARENT_ID"], GetMessage("BLG_GCM_ERROR_NO_COMMENT")),
                    "ERROR_NO_COMMENT"
                );
                return false;
            }
        ...
        if ((is_set($arFields, "DATE_CREATE") || $ACTION == "ADD") && (!$DB->IsDate(
    ...
    public static function Delete($ID)
    {
        ...
        if ($arResult) {
            $DB->Query(
                ...
                "	PARENT_ID = " . ((intval($arResult["PARENT_ID"]) > 0) ? intval(
                    $arResult["PARENT_ID"]
                ) : "null") . " " .
                "WHERE PARENT_ID = " . $ID . " " .
            ...
            $res = CBlogImage::GetList(
                array(),
                array(
                    "BLOG_ID" => $arResult["BLOG_ID"],
                    "POST_ID" => $arResult["POST_ID"],
                    "IS_COMMENT" => "Y",
                    "COMMENT_ID" => $ID
                )
            );
    ...
    public static function GetByID($ID)
    {
        ...
        if (isset($GLOBALS["BLOG_COMMENT"]["BLOG_COMMENT_CACHE_" . $ID]) && is_array(
                $GLOBALS["BLOG_COMMENT"]["BLOG_COMMENT_CACHE_" . $ID]
            ) && is_set($GLOBALS["BLOG_COMMENT"]["BLOG_COMMENT_CACHE_" . $ID], "ID")) {
                ...
                if (!empty($arPost) && $arPost["BLOG_ID"] == $arBlog["ID"] && $arPost["ENABLE_COMMENTS"] == "Y") {
                    ...
                    if ($arPathTemplate["PATH_TO_POST"] <> '') {
                        $url = "http://" . $serverName . htmlspecialcharsbx(
                                CComponentEngine::MakePathFromTemplate(
                                    $arPathTemplate["PATH_TO_POST"],
                                    array(
                                        "blog" => $arBlog["URL"],
                                        "post_id" => CBlogPost::GetPostID(
                                            $arPost["ID"],
                                            $arPost["CODE"],
                                            $arPathTemplate["ALLOW_POST_CODE"]
                                        ),
                                        "user_id" => $arBlog["OWNER_ID"],
                                        "group_id" => $arBlog["SOCNET_GROUP_ID"]
                                    )
                    ...
                    if ($arPathTemplate["USE_SOCNET"] == "Y") {
                        $blogName = GetMessage(
                            "BLG_GCM_RSS_TITLE_SOCNET",
                            Array(
                                "#AUTHOR_NAME#" => htmlspecialcharsEx(
                                    $arUser["NAME"] . " " . $arUser["LAST_NAME"]
                                ),
                                "#POST_TITLE#" => htmlspecialcharsEx($arPost["TITLE"])
                            )
                        );
                    } else {
                        $blogName = GetMessage(
                            "BLG_GCM_RSS_TITLE",
                            Array(
                                "#BLOG_NAME#" => htmlspecialcharsEx($arBlog["NAME"]),
                                "#POST_TITLE#" => htmlspecialcharsEx($arPost["TITLE"])
                            )
                        );
                    }
                    ...
                    if ($type == "rss.92") {
                        ...
                        if ($arPathTemplate["PATH_TO_USER"] <> '') {
                            $authorURLP = "http://" . $serverName . htmlspecialcharsbx(
                                    CComponentEngine::MakePathFromTemplate(
                                        $arPathTemplate["PATH_TO_USER"],
                        ...
                        while ($arComments = $dbComments->Fetch()) {
                            ...
                            if (intval($arComments["AUTHOR_ID"]) > 0) {
                                $author = CBlogUser::GetUserName(
                                    $arComments["BLOG_USER_ALIAS"],
                                    $arComments["USER_NAME"],
                                    $arComments["USER_LAST_NAME"],
                                    $arComments["USER_LOGIN"],
                                    $arComments["USER_SECOND_NAME"]
                                );
                                if ($arPathTemplate["PATH_TO_USER"] <> '') {
                                    $authorURL = "http://" . $serverName . htmlspecialcharsbx(
                                            CComponentEngine::MakePathFromTemplate(
                                                $arPathTemplate["PATH_TO_USER"],
                                                array("user_id" => $arComments["AUTHOR_ID"])
                                            )
                                        );
                                } else {
                                    $authorURL = "http://" . $serverName . htmlspecialcharsbx(
                                            CBlogUser::PreparePath($arComments["AUTHOR_ID"], $arGroup["SITE_ID"])
                                        );
                                }
                            } else {
                            ...
                            if ($arPathTemplate["NO_URL_IN_COMMENTS"] == "L" || (intval(
                                        $arComments["AUTHOR_ID"]
                                    ) <= 0 && $arPathTemplate["NO_URL_IN_COMMENTS"] == "A")) {
                                $arAllow["CUT_ANCHOR"] = "Y";
                            }

                            if ($arPathTemplate["NO_URL_IN_COMMENTS_AUTHORITY_CHECK"] == "Y" && $arAllow["CUT_ANCHOR"] != "Y" && intval(
                                    $arComments["AUTHOR_ID"]
                                ) > 0) {
                            ...
                            $title = GetMessage(
                                "BLG_GCM_COMMENT_TITLE",
                                Array(
                                    "#POST_TITLE#" => htmlspecialcharsEx($arPost["TITLE"]),
                                    "#COMMENT_AUTHOR#" => htmlspecialcharsEx($author)
                                )
                            );
                            /*$title = str_replace(
                                array("&", "<", ">", "\""),
    ...
    {
        if (intval($arParams["BLOG_ID"]) <= 0 || intval($arParams["POST_ID"]) <= 0 || !CModule::IncludeModule(
                "search"
            )) {
            return false;
        }
    ...
    public static function UpdateLog($commentID, $arBlogUser, $arUser, $arComment, $arPost, $arParams)
    {
        ...
        $text4message = $parserBlog->convert(
            $arComment["POST_TEXT"],
            false,
            $arParams["IMAGES"],
            $arAllow,
            array("isSonetLog" => true)
        );
    ...
    public static function GetSocNetUserPermsNew($postId = 0, $authorId = 0, $userId = 0, &$permsBySG)
    {
        ...
        $blogModulePermissions = $APPLICATION->GetGroupRight(
            "blog",
            ($bCurrent ? false : CUser::GetUserGroup($userId))
        );
    ...
    public static function addLiveComment($commentId = 0, $arParams = array())
    {
        ...
        if (
            ...
            $arParams["DATE_TIME_FORMAT"] = (isset($arParams["DATE_TIME_FORMAT"]) ? $arParams["DATE_TIME_FORMAT"] : $DB->DateFormatToPHP(
            ...
            if ($commentAuxProvider = \Bitrix\Socialnetwork\CommentAux\Base::findProvider($comment)) {
                ...
                $comment["TextFormated"] = $p->convert(
                    $comment["POST_TEXT"],
                    false,
                    array(),
                    $arAllow,
                ...
                $p->bMobile = true;
                $comment["TextFormatedMobile"] = $p->convert(
                    $comment["POST_TEXT"],
                    false,
                    array(),
                    $arAllow,
            ...
            $eventHandlerID = AddEventHandler(
                ...
                Array("CSocNetLogTools", "logUFfileShow")
            );
