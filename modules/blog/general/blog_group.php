...
class CAllBlogGroup
{
        ...
        if ((is_set($arFields, "SITE_ID") || $ACTION == "ADD") && $arFields["SITE_ID"] == '') {
            ...
            if (!$dbResult->Fetch()) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["SITE_ID"], GetMessage("BLG_GG_ERROR_NO_SITE")),
                    "ERROR_NO_SITE"
                );
                return false;
            }
    ...
    public static function Delete($ID)
    {
        ...
        if ($dbResult->Fetch()) {
            $GLOBALS["APPLICATION"]->ThrowException(
                str_replace("#ID#", $ID, GetMessage("BLG_GG_ERROR_NOT_EMPTY")),
                "ERROR_NOT_EMPTY"
            );
            return false;
        }
    ...
    public static function GetByID($ID)
    {
        ...
        if (isset($GLOBALS["BLOG_GROUP"]["BLOG_GROUP_CACHE_" . $ID]) && is_array(
                $GLOBALS["BLOG_GROUP"]["BLOG_GROUP_CACHE_" . $ID]
            ) && is_set($GLOBALS["BLOG_GROUP"]["BLOG_GROUP_CACHE_" . $ID], "ID")) {
