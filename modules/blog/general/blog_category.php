...
class CAllBlogCategory
{
        ...
        if ((is_set($arFields, "BLOG_ID") || $ACTION == "ADD") && intval($arFields["BLOG_ID"]) <= 0) {
            ...
            if (!$arResult) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["BLOG_ID"], GetMessage("BLG_GCT_ERROR_NO_BLOG")),
                    "ERROR_NO_BLOG"
                );
                return false;
            }
    ...
    public static function GetByID($ID)
    {
        ...
        if (isset($GLOBALS["BLOG_CATEGORY"]["BLOG_CATEGORY_CACHE_" . $ID]) && is_array(
                $GLOBALS["BLOG_CATEGORY"]["BLOG_CATEGORY_CACHE_" . $ID]
            ) && is_set($GLOBALS["BLOG_CATEGORY"]["BLOG_CATEGORY_CACHE_" . $ID], "ID")) {
