...
class CAllBlogUserGroup
{
        ...
        if ((is_set($arFields, "BLOG_ID") || $ACTION == "ADD") && intval($arFields["BLOG_ID"]) <= 0) {
            ...
            if (!$arResult) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $arFields["BLOG_ID"], GetMessage("BLG_GUG_ERROR_NO_BLOG")),
                    "ERROR_NO_BLOG"
                );
                return false;
            }
    ...
    public static function SetGroupPerms(
        $ID,
        $blogID,
        $postID = 0,
        $permission = BLOG_PERMS_DENY,
        ...
        if (!$arUserGroup) {
            $GLOBALS["APPLICATION"]->ThrowException(
                str_replace("#ID#", $ID, GetMessage("BLG_GUG_ERROR_NO_USER_GROUP")),
                "ERROR_NO_USER_GROUP"
            );
            $bSuccess = false;
        }
        ...
        if ($bSuccess) {
            ...
            if (!$arBlog) {
                $GLOBALS["APPLICATION"]->ThrowException(
                    str_replace("#ID#", $blogID, GetMessage("BLG_GUG_ERROR_NO_BLOG")),
                    "ERROR_NO_BLOG"
                );
                $bSuccess = false;
            }
        ...
        if ($bSuccess) {
            if ($postID > 0) {
                ...
                if (!$arPost) {
                    $GLOBALS["APPLICATION"]->ThrowException(
                        str_replace("#ID#", $postID, GetMessage("BLG_GUG_ERROR_NO_POST")),
                        "ERROR_NO_POST"
                    );
                    $bSuccess = false;
                }
            ...
            if ($currentPerms) {
                ...
                if ($postID > 0) {
                    $DB->Query(
                        ...
                        "VALUES (" . $blogID . ", " . $ID . ", '" . $DB->ForSql(
                            $permsType
                        ) . "', " . $postID . ", '" . $DB->ForSql($permission) . "') "
                    );
                } else {
                    $DB->Query(
                        "INSERT INTO b_blog_user_group_perms (BLOG_ID, USER_GROUP_ID, PERMS_TYPE, POST_ID, PERMS) " .
    ...
    public static function GetByID($ID)
    {
        ...
        if (isset($GLOBALS["BLOG_USER_GROUP"]["BLOG_USER_GROUP_CACHE_" . $ID]) && is_array(
                $GLOBALS["BLOG_USER_GROUP"]["BLOG_USER_GROUP_CACHE_" . $ID]
            ) && is_set($GLOBALS["BLOG_USER_GROUP"]["BLOG_USER_GROUP_CACHE_" . $ID], "ID")) {
    ...
    public static function GetGroupPerms($ID, $blogID, $postID = 0, $permsType = BLOG_PERMS_POST)
    {
        ...
        if (isset($GLOBALS["BLOG_USER_GROUP"][$varName . $ID]) && is_array(
                $GLOBALS["BLOG_USER_GROUP"][$varName . $ID]
            )) {
