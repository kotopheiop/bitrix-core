...
//						if original size bigger than limits - need resize
                        ...
                        if ($this->isSonetLog) {
                            ...
                            if ($type == "mail") {
                                ...
                                $res = '<img src="' . $strImage . '" title="" alt="' . htmlspecialcharsbx(
                                        $f["TITLE"]
                                    ) . '" border="0"' . $strPar . ' data-bx-image="' . $sourceImage . '" />';
                            }
                        }
                    }
                } else {
...
//					if original size bigger than limits - need resize
                    ...
                    if ($this->isSonetLog) {
                        ...
                        if ($type == "mail") {
                            ...
                            $res = '<img src="' . $strImage . '" title="" alt="' . htmlspecialcharsbx(
                                    $f["TITLE"]
                                ) . '" border="0" data-bx-image="' . $sourceImage . '"' . $strPar . ' />';
                            if (!empty($this->blogImageSizeEvents)) {
                                foreach ($this->blogImageSizeEvents as $arEvent) {
                                    ExecuteModuleEventEx(
                                        $arEvent,
                                        Array(&$res, $strImage, $db_img_arr, $f, $arDestinationSize)
                                    );
                                }
                            }
                        }
    ...
    function convert_open_tag($marker = "quote")
    {
        ...
        return "<div class='blog-post-" . $marker . "' title=\"" . GetMessage(
                "BLOG_" . ToUpper($marker)
            ) . "\"><table class='blog" . $marker . "'><tr><td>";
    }
    ...
    function render_user($fields)
    {
        ...
        $this->allow["USER_LINK"] == "N"
            ? $userName
            : '<a class="blog-p-user-name' . $classAdditional . '" id="bp_' . $anchorId . '" href="' . CComponentEngine::MakePathFromTemplate(
                $pathToUser,
...
class CBlogTools
{
    ...
    {
        $result = false;
        if (mb_strpos($arParams['arUserField']['FIELD_NAME'], CBlogPost::UF_NAME) === 0 || mb_strpos(
    ...
    {
        $result = false;
        if ($arParams['arUserField']['FIELD_NAME'] == CBlogPost::UF_NAME || mb_strpos(
                $arParams['arUserField']['FIELD_NAME'],
                'UF_BLOG_COMMENT_DOC'
            ) === 0) {
