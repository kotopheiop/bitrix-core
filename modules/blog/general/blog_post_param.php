...
class CBlogUserOptions
{
    ...
    public static function GetList($arOrder = array("ID" => "ASC"), $arFilter = array(), $arAddParams = array())
    {
        ...
        $arFields = array(
            ...
            "RANK" => ($arOrder["OWNER_ID"] > 0 ? array(
                ...
                "FROM" =>
                    ...
                    (isset($arFilter["POST_ID"]) && !is_array($arFilter["POST_ID"]) && intval(
                        $arFilter["POST_ID"]
                    ) > 0 ? " WHERE RV2.ENTITY_ID IN (SELECT BPP.USER_ID as USER_ID FROM b_blog_post_param BPP WHERE BPP.POST_ID = " . intval(
                            $arFilter["POST_ID"]
                        ) . ")\n\t\t" : "") .
                    ...
                    "WHERE RV1.USER_ID = " . $arOrder["OWNER_ID"] . (isset($arFilter["POST_ID"]) && !is_array(
                        $arFilter["POST_ID"]
                    ) && intval(
                        $arFilter["POST_ID"]
                    ) > 0 ? " AND RV1.OWNER_ID IN (SELECT BPP.USER_ID as USER_ID FROM b_blog_post_param BPP WHERE BPP.POST_ID = " . intval(
                            $arFilter["POST_ID"]
                        ) . ")" : "") . "\n\t\t" .
        ...
        $arSelect = (is_array($arAddParams["SELECT"]) && !empty($arAddParams["SELECT"]) ? array_intersect(
            $arAddParams["SELECT"],
            $arSelect
        ) : $arSelect);
        ...
        foreach ($arOrder as $by => $order) {
            ...
            if (array_key_exists($by, $arFields) && !array_key_exists($by, $arSqlOrder)) {
                ...
                if (isset($arFields[$by]["FROM"]) && !empty($arFields[$by]["FROM"]) && mb_strpos(
                        $arSql["FROM"],
                        $arFields[$by]["FROM"]
                    ) === false) {
                ...
                if ($by == "RANK") {
                    ...
                    $arSqlOrder[$by] = (IsModuleInstalled(
                        "intranet"
                    ) ? "RV.VOTE_WEIGHT " . $order . ", USER_RANK " . $order :
                        "USER_RANK " . $order . ", RV.VOTE_WEIGHT " . $order);
                } else {
                    $arSqlOrder[$by] = (array_key_exists(
                            "ORDER",
                            $arFields[$by]
    ...
    public static function DeleteUsersOptions($user_id = false)
    {
        ...
        $user_id = intval($user_id);
        if ($DB->Query(
            "DELETE FROM b_blog_post_param WHERE " . ($user_id <= 0 ? "USER_ID IS NULL" : "USER_ID=" . $user_id),
            false,
