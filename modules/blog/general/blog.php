...
//		if for user unset option "WRITE TO BLOG", they can only read (even if all user can write), or smaller rights, if group have smaller
    ...
    public static function CheckFields($ACTION, &$arFields, $ID = 0)
    {
		...
		*/
        ...
        if ((is_set($arFields, "URL") || $ACTION == "ADD") && $arFields["URL"] == '') {
            ...
            $dbResult = CBlog::GetList(
                array(),
                array("URL" => $arFields["URL"], "!ID" => $ID),
                false,
                false,
                array("ID")
            );
            ...
            if (in_array(mb_strtolower($arFields["URL"]), $GLOBALS["AR_BLOG_RESERVED_NAMES"])) {
                $APPLICATION->ThrowException(
                    str_replace("#NAME#", $arFields["URL"], GetMessage("BLG_GB_RESERVED_NAME")),
                    "RESERVED_NAME"
                );
                return false;
            }
        ...
        if (is_set($arFields, "LAST_POST_DATE") && (!$DB->IsDate(
                    $arFields["LAST_POST_DATE"],
                    false,
                    LANG,
                    "FULL"
                ) && $arFields["LAST_POST_DATE"] <> '')) {
        ...
        if (is_set($arFields, "OWNER_ID") && is_set($arFields, "SOCNET_GROUP_ID") && intval(
                $arFields["OWNER_ID"]
            ) <= 0 && intval($arFields["SOCNET_GROUP_ID"]) <= 0) {
            $APPLICATION->ThrowException(GetMessage("BLG_GB_EMPTY_OWNER_ID"), "EMPTY_OWNER_ID");
            return false;
        }
        ...
        if ((is_set(
                    $arFields,
                    "ENABLE_COMMENTS"
                ) || $ACTION == "ADD") && $arFields["ENABLE_COMMENTS"] != "Y" && $arFields["ENABLE_COMMENTS"] != "N") {
            $arFields["ENABLE_COMMENTS"] = "Y";
        }
        if ((is_set(
                    $arFields,
                    "ENABLE_IMG_VERIF"
                ) || $ACTION == "ADD") && $arFields["ENABLE_IMG_VERIF"] != "Y" && $arFields["ENABLE_IMG_VERIF"] != "N") {
            $arFields["ENABLE_IMG_VERIF"] = "N";
        }
        if ((is_set(
                    $arFields,
                    "ENABLE_RSS"
                ) || $ACTION == "ADD") && $arFields["ENABLE_RSS"] != "Y" && $arFields["ENABLE_RSS"] != "N") {
            $arFields["ENABLE_RSS"] = "N";
        }
        if ((is_set(
                    $arFields,
                    "ALLOW_HTML"
                ) || $ACTION == "ADD") && $arFields["ALLOW_HTML"] != "Y" && $arFields["ALLOW_HTML"] != "N") {
            $arFields["ALLOW_HTML"] = "N";
        }
        if ((is_set(
                    $arFields,
                    "USE_SOCNET"
                ) || $ACTION == "ADD") && $arFields["USE_SOCNET"] != "Y" && $arFields["USE_SOCNET"] != "N") {
    ...
    public static function PrepareSql(
        ...
        foreach ($arOrder as $by => $order) {
            ...
            if (array_key_exists($by, $arFields)) {
                $arSqlOrder[] = " " . (array_key_exists(
                        "ORDER",
                        $arFields[$by]
                    ) ? $arFields[$by]["ORDER"] : $arFields[$by]["FIELD"]) . " " . $order . " ";
    ...
    public static function GetByID($ID)
    {
        ...
        if (!isset($GLOBALS["BLOG"]["BLOG_CACHE_" . $ID]) || !is_array(
                $GLOBALS["BLOG"]["BLOG_CACHE_" . $ID]
            ) || !is_set($GLOBALS["BLOG"]["BLOG_CACHE_" . $ID], "ID")) {
            $strSql =
    ...
    public static function GetByOwnerID($ID, $arGroup = Array())
    {
        ...
        if (!isset($GLOBALS["BLOG"]["BLOG4OWNER_CACHE_" . $ID][$groups]) || !is_array(
                $GLOBALS["BLOG"]["BLOG4OWNER_CACHE_" . $ID][$groups]
            ) || !is_set($GLOBALS["BLOG"]["BLOG4OWNER_CACHE_" . $ID][$groups], "ID")) {
            $strSql =
                "SELECT B.ID, B.NAME, B.DESCRIPTION, B.ACTIVE, B.OWNER_ID, B.URL, B.GROUP_ID, " .
                "	B.ENABLE_COMMENTS, B.ENABLE_IMG_VERIF, B.EMAIL_NOTIFY, B.ENABLE_RSS, B.REAL_URL, " .
                "	B.LAST_POST_ID, B.AUTO_GROUPS, B.ALLOW_HTML, B.SEARCH_INDEX, B.SOCNET_GROUP_ID, B.USE_SOCNET, " .
    ...
    public static function GetByUrl($BLOG_URL, $arGroup = Array())
    {
        ...
        if (!isset($GLOBALS["BLOG"]["BLOGBYURL_CACHE_" . $BLOG_URL][$groups]) || !is_array(
                $GLOBALS["BLOG"]["BLOGBYURL_CACHE_" . $BLOG_URL][$groups]
            ) || !is_set($GLOBALS["BLOG"]["BLOGBYURL_CACHE_" . $BLOG_URL][$groups], "ID")) {
            $strSql =
                "SELECT B.ID, B.NAME, B.DESCRIPTION, B.ACTIVE, B.OWNER_ID, B.URL, B.GROUP_ID, " .
                "	B.ENABLE_COMMENTS, B.ENABLE_IMG_VERIF, B.EMAIL_NOTIFY, B.ENABLE_RSS, B.REAL_URL, " .
                "	B.LAST_POST_ID, B.AUTO_GROUPS, B.ALLOW_HTML, B.SEARCH_INDEX, B.SOCNET_GROUP_ID, B.USE_SOCNET, " .
    ...
    public static function GetBySocNetGroupID($ID, $arGroup = Array())
    {
        ...
        if (!isset($GLOBALS["BLOG"]["BLOG4OWNERGROUP_CACHE_" . $ID][$groups]) || !is_array(
                $GLOBALS["BLOG"]["BLOG4OWNERGROUP_CACHE_" . $ID][$groups]
            ) || !is_set($GLOBALS["BLOG"]["BLOG4OWNERGROUP_CACHE_" . $ID][$groups], "ID")) {
            $strSql =
                "SELECT B.ID, B.NAME, B.DESCRIPTION, B.ACTIVE, B.OWNER_ID, B.URL, B.GROUP_ID, " .
                "	B.ENABLE_COMMENTS, B.ENABLE_IMG_VERIF, B.EMAIL_NOTIFY, B.ENABLE_RSS, B.REAL_URL, " .
                "	B.LAST_POST_ID, B.AUTO_GROUPS, B.ALLOW_HTML, B.SEARCH_INDEX, B.SOCNET_GROUP_ID, B.USE_SOCNET, " .
            ...
            if ($arGroup && $arGroup["SITE_ID"] == SITE_ID || $arParams["USE_SOCNET"] == "Y") {
                    ...
                    if ($bSoNet) {
                        if (intval($arParams["USER_ID"]) > 0) {
                            ...
                            if ($arUser = $dbUser->Fetch()) {
                                $blogName = htmlspecialcharsbx(
                                    GetMessage(
                                        "BLG_RSS_NAME_SONET",
                                        Array(
                                            "#AUTHOR_NAME#" => CUser::FormatName(
                                ...
                                $blogURL = "http://" . $serverName . htmlspecialcharsbx(
                            ...
                            if ($arGroupSoNet = CSocNetGroup::GetByID($arParams["SOCNET_GROUP_ID"])) {
                                $blogName = htmlspecialcharsbx(
                                    GetMessage(
                                        "BLG_RSS_NAME_SONET_GROUP",
                                        Array("#GROUP_NAME#" => $arGroupSoNet["NAME"])
                                    )
                                );
                                $blogURL = "http://" . $serverName . htmlspecialcharsbx(
                                        CComponentEngine::MakePathFromTemplate(
                                            $blogTemplate,
                                            array("group_id" => $arParams["SOCNET_GROUP_ID"])
                                        )
                                    );
                            }
                        }
                        ...
                        if ($blogTemplate <> '') {
                            ...
                            $blogURL = "http://" . $serverName . htmlspecialcharsbx(
                                    CBlog::PreparePath($arBlog["URL"], $arGroup["SITE_ID"])
                                );
                        }
                ...
                if ($postPerm >= BLOG_PERMS_READ) {
                    ...
                    while ($arPost = $dbPosts->Fetch()) {
                        if (!$bSoNet) {
                            $perms = CBlogPost::GetBlogUserPostPerms(
                                $arPost["ID"],
                                ($GLOBALS["USER"]->IsAuthorized() ? $GLOBALS["USER"]->GetID() : 0)
                            );
                        ...
                        $arDate = ParseDateTime(
                            $arPost["DATE_PUBLISH"],
                            CSite::GetDateFormat("FULL", $arGroup["SITE_ID"])
                        );
                        $date = date(
                            "r",
                            mktime(
                        ...
                        if ($arPost["PATH"] <> '') {
                            $url = "http://" . $serverName . htmlspecialcharsbx(
                                    str_replace(
                                        "#post_id#",
                                        CBlogPost::GetPostID(
                                            $arPost["ID"],
                            ...
                            $url = "http://" . $serverName . htmlspecialcharsbx(
                                    CComponentEngine::MakePathFromTemplate(
                                        ...
                                        array(
                                            ...
                                            "user_id" => (intval(
                                                $arParams["USER_ID"]
                                            ) > 0 ? $arParams["USER_ID"] : $arBlog["OWNER_ID"]),
                                            "group_id" => (intval(
                                                $arParams["SOCNET_GROUP_ID"]
                                            ) > 0 ? $arParams["SOCNET_GROUP_ID"] : $arBlog["SOCNET_GROUP_ID"])
                                        )
                                    )
                                );
                        } else {
                            $url = "http://" . $serverName . htmlspecialcharsbx(
                                    CBlogPost::PreparePath($arBlog["URL"], $arPost["ID"], $arGroup["SITE_ID"])
                                );
                        }
                        ...
                        if ($userTemplate <> '') {
                            $authorURL = "http://" . $serverName . htmlspecialcharsbx(
                                    CComponentEngine::MakePathFromTemplate(
                                        $userTemplate,
                                        array("user_id" => $arPost["AUTHOR_ID"])
                                    )
                                );
                        } else {
                            $authorURL = "http://" . $serverName . htmlspecialcharsbx(
                                    CBlogUser::PreparePath($arPost["AUTHOR_ID"], $arGroup["SITE_ID"])
                                );
                        }

                        if ($arPost["DETAIL_TEXT_TYPE"] == "html") {
                            $text = $parser->convert_to_rss(
                                $arPost["DETAIL_TEXT"],
                                $arImages,
                                array(
                                    "HTML" => "Y",
                        ...
                        if ($type == "rss.92") {
                            ...
                            $dateISO = date("Y-m-d\\TH:i:s", $timeISO) . mb_substr(
                                    date("O", $timeISO),
                                    0,
                                    3
                                ) . ":" . mb_substr(date("O", $timeISO), -2, 2);
    ...
    public static function BuildRSSAll(
        $GroupId = 0,
        $type = "RSS .92",
        $numPosts = 10,
        $siteID = SITE_ID,
        ...
        if ($GroupId > 0) {
            $blogName = GetMessage("BLG_RSS_ALL_GROUP_TITLE") . " \"" . htmlspecialcharsbx(
                    $arGroup["NAME"]
                ) . "\" (" . $serverName . ")";
        } else {
            $blogName = GetMessage("BLG_RSS_ALL_TITLE") . " \"" . htmlspecialcharsbx(
                    $arSite["NAME"]
                ) . "\" (" . $serverName . ")";
        }
        ...
        while ($arPost = $dbPosts->Fetch()) {
            ...
            if ($arPost["PATH"] <> '') {
                $url = "http://" . $serverName . htmlspecialcharsbx(
                        str_replace(
                            "#post_id#",
                            CBlogPost::GetPostID($arPost["ID"], $arPost["CODE"], $arPathTemplates["ALLOW_POST_CODE"]),
                            $arPost["PATH"]
                        )
                    );
                ...
                if (intval($arPost["BLOG_SOCNET_GROUP_ID"]) > 0 && $arPathTemplates["GROUP_BLOG_POST"] <> '') {
                    $url = "http://" . $serverName . htmlspecialcharsbx(
                            CComponentEngine::MakePathFromTemplate(
                                $arPathTemplates["GROUP_BLOG_POST"],
                                array(
            ...
            if ($arPathTemplates["USER"] <> '') {
                ...
                $authorURL = "http://" . $serverName . htmlspecialcharsbx(
                        CBlogUser::PreparePath($arPost["AUTHOR_ID"], $arPost["BLOG_GROUP_SITE_ID"])
                    );
            }
    ...
    public static function SendPing($blogName, $blogUrl, $blogXml = "")
    {
        ...
        if ($urls = COption::GetOptionString(
            "blog",
        ...
        if (CModule::IncludeModule("socialnetwork")) {
            ...
            if ($type == "G") {
                $strSql = "SELECT b.ENTITY_ID as ID, bf.OPERATION_ID, bf.ROLE, bu.USER_ID, blg.NAME as BLOG_NAME, blg.ID as BLOG_ID, g.NAME
					...
					LEFT  JOIN b_blog_group blgg ON (blg.GROUP_ID = blgg.ID AND blgg.SITE_ID = '" . $DB->ForSql(
                        $site_id
                    ) . "')
