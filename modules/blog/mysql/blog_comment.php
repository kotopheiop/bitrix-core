...
class CBlogComment extends CAllBlogComment
{
    ...
    public static function Add($arFields, $bSearchIndex = true)
    {
        ...
        if (
            ...
            if (CBlogUserGroup::GetGroupPerms(
                    1,
                    $arComment["BLOG_ID"],
                    $arComment["POST_ID"],
                    BLOG_PERMS_POST
                ...
                if (
                    ...
                    if (
                        ...
                        CBlogSearch::fillSearchParamsWithMentionData(
                            CBlogComment::GetMentionedUserID($arComment),
                            $arSearchIndex
                        );
                    }
    ...
    public static function Update($ID, $arFields, $bSearchIndex = true)
    {
        ...
        if ($strUpdate <> '') {
            ...
            if ($bSearchIndex && CModule::IncludeModule("search")) {
                $newPostPerms = CBlogUserGroup::GetGroupPerms(
                    1,
                    $arComment["BLOG_ID"],
                    $arComment["POST_ID"],
                    BLOG_PERMS_POST
    ...
    public static function GetList(
        ...
        $arFields = array(
            ...
            "USER_LOGIN" => array(
                "FIELD" => "U.LOGIN",
                "TYPE" => "string",
                "FROM" => "LEFT JOIN b_user U ON (C.AUTHOR_ID = U.ID)"
            ),
            "USER_NAME" => array(
                "FIELD" => "U.NAME",
                "TYPE" => "string",
            ...
            "BLOG_USER_ALIAS" => array(
                "FIELD" => "BU.ALIAS",
                "TYPE" => "string",
                "FROM" => "LEFT JOIN b_blog_user BU ON (C.AUTHOR_ID = BU.USER_ID)"
            ),
            ...
            "BLOG_URL" => array(
                "FIELD" => "B.URL",
                "TYPE" => "string",
                "FROM" => "INNER JOIN b_blog B ON (C.BLOG_ID = B.ID)"
            ),
            "BLOG_OWNER_ID" => array(
                "FIELD" => "B.OWNER_ID",
                "TYPE" => "string",
            ...
            "SOCNET_BLOG_READ" => array(
                "FIELD" => "BSR.BLOG_ID",
                "TYPE" => "int",
                "FROM" => "INNER JOIN b_blog_socnet BSR ON (C.BLOG_ID = BSR.BLOG_ID)"
            ),

            "POST_CODE" => array(
                "FIELD" => "BP.CODE",
                "TYPE" => "string",
                "FROM" => "INNER JOIN b_blog_post BP ON (C.POST_ID = BP.ID)"
            ),
            "POST_TITLE" => array(
                "FIELD" => "BP.TITLE",
                "TYPE" => "string",
            ...
            "BLOG_POST_PUBLISH_STATUS" => array(
                "FIELD" => "BP.PUBLISH_STATUS",
                "TYPE" => "string",
                "FROM" => "INNER JOIN b_blog_post BP ON (C.POST_ID = BP.ID)"
            ),
        ...
        if (
            ...
            $arFields["RATING_TOTAL_VALUE"] = array(
                ...
                "TYPE" => "double",
                "FROM" => "LEFT JOIN b_rating_voting RV ON ( RV.ENTITY_TYPE_ID = 'BLOG_COMMENT' AND RV.ENTITY_ID = C.ID )"
            );
        ...
        if ($blogModulePermissions < "W") {
            ...
            $arFields["PERMS"] = array(
                "FIELD" => "UGP.PERMS",
                "TYPE" => "char",
                "FROM" => "INNER JOIN b_blog_user_group_perms UGP ON (C.POST_ID = UGP.POST_ID AND C.BLOG_ID = UGP.BLOG_ID AND UGP.USER_GROUP_ID IN (" . $strUserGroups . ") AND UGP.PERMS_TYPE = '" . BLOG_PERMS_COMMENT . "')"
            );
            $bNeedDistinct = true;
        } else {
