    ...
    <form name="find_form" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?">
        ...
        <tr>
            ...
            <td><input type="text" name="filter_author_email"
                       value="<? echo htmlspecialcharsbx($filter_author_email) ?>"
                       size="40"><?= ShowFilterLogicHelp() ?></td>
        </tr>
        <tr>
            ...
            <td>
                <select name="filter_blog_group_id[]" multiple size="5">
                    ...
                    while ($arGroup = $dbGroup->GetNext()) {
                        ...
                        if (in_array($arGroup["ID"], $filter_blog_group_id)) echo " selected" ?>>[<?= $arGroup["ID"] ?>
                        ] <?= $arGroup["NAME"] ?> (<?= $arGroup["SITE_ID"] ?>)</option><?
                    }
                    ?>
                </select>
            </td>
            ...
            <td>
                <select name="filter_blog_group_site_id">
                    ...
                    while ($arSitesList = $dbSitesList->GetNext()) {
                        ?>
                        <option value="<?= ($arSitesList["LID"]) ?>"<?
                        if ($arSitesList["LID"] == $filter_blog_group_site_id) echo " selected"; ?>>
