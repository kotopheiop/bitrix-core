...
?>
    ...
    <form method="POST" action="security_hosts.php?lang=<?= LANGUAGE_ID ?><?= $returnUrlQuery ?>"
        ...
        <tr>
            ...
            <td width="60%"><input type="checkbox" name="properties[active]"
                                   value="Y" <?= $properties['active'] ? 'checked' : '' ?> ></td>
        </tr>
        <tr>
            <td><?= Loc::getMessage('SECURITY_HOSTS_LOGGING') ?>:</td>
            <td><input type="checkbox" name="properties[logging]"
                       value="Y" <?= $properties['logging'] ? 'checked' : '' ?> ></td>
        </tr>
        <tr>
            <td><?= Loc::getMessage('SECURITY_HOSTS_REACTION') ?>:</td>
            <td>
                <label>
                    <input type="radio" name="properties[action]"
                           value="<?= $hosts::ACTION_STOP ?>" <?= $properties['action'] === $hosts::ACTION_STOP ? 'checked' : ''; ?>>
                    <?= Loc::getMessage('SECURITY_HOSTS_REACTION_STOP') ?>
                </label><br>
                <label>
                    <input type="radio" name="properties[action]"
                           value="<?= $hosts::ACTION_REDIRECT ?>" <?= $properties['action'] === $hosts::ACTION_REDIRECT ? 'checked' : ''; ?>>
