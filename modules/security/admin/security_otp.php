...
<form method="POST" action="security_otp.php?lang=<?= LANGUAGE_ID ?><?= htmlspecialcharsbx($returnUrl) ?>"
    ...
    <tr>
        <td colspan="2" align="left">
            <? if (CSecurityUser::isActive()): ?>
                ...
                <input type="submit" name="otp_siteb"
                       value="<? echo GetMessage("SEC_OTP_NEW_BUTTON_OFF") ?>"<? if (!$canWrite) echo " disabled" ?>>
            <? else: ?>
                <input type="hidden" name="otp_active" value="Y">
                <input type="submit" name="otp_siteb"
                       value="<? echo GetMessage("SEC_OTP_NEW_BUTTON_ON") ?>"<? if (!$canWrite) echo " disabled" ?>
                       class="adm-btn-save">
            <? endif ?>
        </td>
    </tr>
    ...
    <tr>
        ...
        <td width="60%">
            <input type="text" size="4" name="window_size"
                   value="<?= (int)COption::GetOptionInt("security", "hotp_user_window") ?>">
        </td>
    </tr>
    <tr>
        <td>
    ...
    <tr data-hide-by-mandatory="yes"
        style="<?= (COption::GetOptionString("security", "otp_mandatory_using") == "Y") ? "" : "display: none;"; ?>">
        <td>
            <?= GetMessage("SEC_OTP_MANDATORY_SKIP_DAYS") ?>:
        </td>
        <td>
            <input type="text" size="4" name="otp_mandatory_skip_days" id="otp_mandatory_skip_days"
                   value="<?= (int)COption::GetOptionInt("security", "otp_mandatory_skip_days") ?>">
        </td>
    </tr>
    <tr data-hide-by-mandatory="yes"
        style="<?= (COption::GetOptionString("security", "otp_mandatory_using") == "Y") ? "" : "display: none;"; ?>">
            ...
            <div id="bx_access_div">
                <? foreach ($targetRights as $code): ?>
                    ...
                    <div style="margin-bottom:4px">
                        ...
                        <?= htmlspecialcharsbx($value) ?>&nbsp;<a href="javascript:void(0);" data-role="delete-access"
