            ...
            <select name="<?= $FIELD_NAME ?>" id="<?= $FIELD_NAME ?>" <?= $strAttributes ?>>
                ...
                <? foreach ($arOptions as $key => $value):?>
                    <option value="<?= htmlspecialcharsbx(
                        $key
                    ) ?>"<?= $key == $PARAM1 ? " selected" : "" ?>><?= htmlspecialcharsbx($value) ?></option>
                <?endforeach; ?>
            </select>
            <?
            break;
        case "input":
            ?><input type="text" size="15" name="<?= $FIELD_NAME ?>" id="<?= $FIELD_NAME ?>"
                     value="<?= htmlspecialcharsbx($PARAM2) ?>"><?
            break;
        case "hidden":
...
$aTabs = array(
    array(
        "DIV" => "edit1",
        "TAB" => GetMessage("customrank_edit_rule"),
        "ICON" => "main_user_edit",
        "TITLE" => GetMessage("customrank_edit_rule_title")
    ),
);
...
if ($REQUEST_METHOD == "POST" && ($save != "" || $apply != "") && $POST_RIGHT == "W" && check_bitrix_sessid()) {
    ...
    if ($res) {
        if ($apply != "") {
            LocalRedirect(
                "search_customrank_edit.php?ID=" . $ID . "&lang=" . LANG . "&" . $tabControl->ActiveTabParam()
            );
        } else {
...
$APPLICATION->SetTitle(
    ($ID > 0 ? GetMessage("customrank_edit_edit_rule") . $ID : GetMessage("customrank_edit_add_rule"))
);
...
if ($ID > 0) {
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('" . AddSlashes(
                GetMessage("customrank_edit_delete_conf")
            ) . "'))window.location='search_customrank_admin.php?ID=" . $ID . "&action=delete&lang=" . LANG . "&" . bitrix_sessid_get(
            ) . "';",
        "ICON" => "btn_delete",
    );
}
...
<? CAdminMessage::ShowMessage($strError); ?>
    ...
    <script type="text/javascript">
        function ParamUpdate() {
            ...
            switch (module_id) {
                case 'main':
                    document.getElementById('param1_label').innerHTML = '<?=AddSlashes(
                        GetMessage("customrank_edit_param1")
                    )?>';
                    document.getElementById('param2_label').innerHTML = '<?=AddSlashes(
                        GetMessage("customrank_edit_param2")
                    )?>';
                    document.getElementById('item_id_label').innerHTML = '<?=AddSlashes(
                        GetMessage("customrank_edit_param3_main")
                    )?>';
                    document.getElementById('main_item_id').style.display = "block";
                    break;
                case 'iblock':
                    document.getElementById('param1_label').innerHTML = '<?=AddSlashes(
                        GetMessage("customrank_edit_param1_iblock")
                    )?>';
                    document.getElementById('param2_label').innerHTML = '<?=AddSlashes(
                        GetMessage("customrank_edit_param2_iblock")
                    )?>';
                    document.getElementById('item_id_label').innerHTML = '<?=AddSlashes(
                        GetMessage("customrank_edit_param3_iblock")
                    )?>';
                    document.getElementById('iblock_item_id').style.display = "block";
                    break;
                case 'forum':
                    document.getElementById('param1_label').innerHTML = '<?=AddSlashes(
                        GetMessage("customrank_edit_param1_forum")
                    )?>';
                    document.getElementById('param2_label').innerHTML = '<?=AddSlashes(
                        GetMessage("customrank_edit_param2_forum")
                    )?>';
                    document.getElementById('item_id_label').innerHTML = '<?=AddSlashes(
                        GetMessage("customrank_edit_param3_forum")
                    )?>';
                ...
                default:
                    ...
                    document.getElementById('item_id_label').innerHTML = '<?=AddSlashes(
                        GetMessage("customrank_edit_param3")
                    )?>';
                    document.getElementById('main_item_id').style.display = "block";
                    break;
            }
    ...
    <form method="POST" Action="<? echo $APPLICATION->GetCurPage() ?>" ENCTYPE="multipart/form-data" name="post_form"
        ...
        <tr>
            ...
            <td width="60%"><? echo CLang::SelectBox(
                    "SITE_ID",
                    $str_SITE_ID,
                    "",
                    "BoxUpdateNew('param1')",
        ...
        <tr>
            ...
            <td>
                <select name="MODULE_ID" id="MODULE_ID" OnChange="BoxUpdateNew('param1')">
                    <option value="main"<?= $str_MODULE_ID == "main" ? " selected" : "" ?>><?= GetMessage(
                            "customrank_edit_files"
                        ) ?></option>
                    <? foreach (CSearchParameters::GetModulesList() as $module_id => $module_name): ?>
                        <option value="<? echo $module_id ?>"<?= $str_MODULE_ID == $module_id ? " selected" : "" ?>><? echo htmlspecialcharsbx(
                                $module_name
                            ) ?></option>
                    <? endforeach; ?>
                </select>
                    ...
                    <? elseif ($str_MODULE_ID == "forum" && CModule::IncludeModule("forum")): ?>
                        <select name="PARAM1" id="PARAM1" OnChange="BoxUpdateNew('param1')">
                            ...
                            while ($arForum = $rsForum->Fetch()) {
                                ...
                                ?>
                                <option value="<?= $arForum["ID"] ?>"<?= $arForum["ID"] == $str_PARAM1 ? " selected" : "" ?>><?= "[" . $arForum["ID"] . "] " . htmlspecialcharsbx(
                                        $arForum["NAME"]
                                    ) ?></option>
                            <?
                            }
                    ...
                    <? if ($str_MODULE_ID == "iblock" && $str_PARAM1 <> "" && CModule::IncludeModule("iblock")): ?>
                        <select name="PARAM2" id="PARAM2" OnChange="BoxUpdateNew('param1')">
                            ...
                            $rsIBlock = CIBlock::GetList(
                                array("SORT" => "ASC"),
                                array("TYPE" => $str_PARAM1, "LID" => $str_SITE_ID)
                            );
                            while ($arIBlock = $rsIBlock->Fetch()) {
                                $arIBlock["ID"] = intval($arIBlock["ID"]);
                                ?>
                                <option value="<?= $arIBlock["ID"] ?>"<?= $arIBlock["ID"] == $str_PARAM2 ? " selected" : "" ?>><?= "[" . $arIBlock["ID"] . "] " . htmlspecialcharsbx(
                                        $arIBlock["NAME"]
                                    ) ?></option>
                            <?
                            }
        ...
        <tr>
            ...
            <td>
                <div id="iblock_item_id" style="display:<?= $str_MODULE_ID == "iblock" ? "block" : "none" ?>">
                    ...
                    <input type="button" value="..." id="ITEM_ID_CHOOSE"
                           onClick="jsUtils.OpenWindow('iblock_element_search.php?lang=<?= LANG ?>&amp;IBLOCK_ID='+document.getElementById('PARAM2').value+'&amp;n=ITEM_ID&amp;k=KEY', 600, 500);">
                    &nbsp;<span id="sp_<?= md5("ITEM_ID") ?>_<?= "KEY" ?>" class="tablebodytext">
		<?
        if ($str_MODULE_ID == "iblock" && $str_PARAM1 != "" && $str_PARAM2 != "" && $str_ITEM_ID != "") {
            echo CSearchCustomRank::__GetParam(
                $lang,
                $str_SITE_ID,
                $str_MODULE_ID,
                $str_PARAM1,
