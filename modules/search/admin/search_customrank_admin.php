...
if ($Rebuild <> '') {
        ...
        foreach ($FIELDS as $ID => $arFields) {
            ...
            if (($rsData = $cData->GetByID($ID)) && ($arData = $rsData->Fetch())) {
                ...
                $lAdmin->AddGroupError(
                    GetMessage("customrank_edit_error") . " " . GetMessage("customrank_no_rule"),
                    $ID
                );
                $searchDB->Rollback();
            }
    ...
    while ($arRes = $rsData->NavNext(true, "f_")):
        ...
        $row->AddEditField(
            ...
            CLang::SelectBox(
                ...
                $f_SITE_ID,
                "",
                "BoxUpdateNew('param1'," . $f_ID . ")"
            )
        );
        $row->AddViewField(
            "SITE_ID",
            "[" . $f_SITE_ID . "] " . htmlspecialcharsbx(
                CSearchCustomRank::__GetParam($lang, $f_SITE_ID)
            )
        );
        ...
        $row->AddSelectField(
            ...
            array("OnChange" => "BoxUpdateNew('param1'," . $f_ID . ")")
        );
        ...
        if ($f_MODULE_ID == "iblock" && CModule::IncludeModule("iblock")) {
            ...
            while ($ar = $rs->Fetch()) {
                if ($arIBType = CIBlockType::GetByIDLang($ar["ID"], LANG)) {
                    $strPARAM1 .= '<option value="' . htmlspecialcharsbx(
                            $ar["ID"]
                        ) . '" ' . ($ar["ID"] == $f_PARAM1 ? " selected" : "") . '>' . htmlspecialcharsbx(
                            "[" . $ar["ID"] . "] " . $arIBType["~NAME"]
                        ) . '</option>';
                }
            }
        ...
        $f_PARAM1_NAME = "[" . $f_PARAM1 . "] " . CSearchCustomRank::__GetParam(
                $lang,
                $f_SITE_ID,
                $f_MODULE_ID,
                $f_PARAM1
        ...
        if ($f_MODULE_ID == "iblock" && CModule::IncludeModule("iblock")) {
            ...
            while ($ar = $rs->Fetch()) {
                $strPARAM2 .= '<option value="' . htmlspecialcharsbx(
                        $ar["ID"]
                    ) . '" ' . ($ar["ID"] == $f_PARAM2 ? " selected" : "") . '>' . htmlspecialcharsbx(
                        "[" . $ar["ID"] . "] " . $ar["NAME"]
                    ) . '</option>';
            }
            $strPARAM2 .= '</select>';
        ...
        $f_PARAM2_NAME = "[" . $f_PARAM2 . "] " . CSearchCustomRank::__GetParam(
                $lang,
                $f_SITE_ID,
                $f_MODULE_ID,
                $f_PARAM1,
        ...
        $f_ITEM_ID_NAME = "[" . $f_ITEM_ID . "] " . CSearchCustomRank::__GetParam(
                $lang,
                $f_SITE_ID,
                $f_MODULE_ID,
                $f_PARAM1,
        ...
        if ($SEARCH_RIGHT >= "W") {
            $arActions[] = array(
                ...
                "ACTION" => "if(confirm('" . GetMessage('customrank_delete_confirm') . "')) " . $lAdmin->ActionDoGroup(
                        $f_ID,
                        "delete"
                    ),
            );
    ...
    <form name="form1" method="get" action="<? echo $APPLICATION->GetCurPage(); ?>">
        ...
        <tr>
            ...
            <td>
                <input type="text" size="25" name="find" value="<? echo htmlspecialcharsbx($find) ?>"
                       title="<?= GetMessage("customrank_find_title") ?>">
                <?
                $arr = array(
                    "reference" => array(
                        GetMessage("customrank_site"),
        ...
        <tr>
            ...
            <td><?= CSearchCustomRank::ModulesSelectBox(
                    "find_module_id",
                    $find_module_id,
                    GetMessage("customrank_all"),
                    "",
                    ""
                ) ?></td>
        </tr>
        <tr>
            ...
            <td><input type="text" name="find_param1" size="47" value="<? echo htmlspecialcharsbx($find_param1) ?>">
            </td>
        </tr>
        <?
    ...
    <p>
        <input type="button" id="start_button" value="<? echo GetMessage("customrank_update") ?>"
               OnClick="StartRebuild();">
        <input type="button" id="stop_button" value="<?= GetMessage("customrank_stop") ?>" OnClick="StopRebuild();"
               disabled>
        <input type="button" id="continue_button" value="<?= GetMessage("customrank_continue") ?>"
               OnClick="ContinueRebuild();" disabled>
    </p>
