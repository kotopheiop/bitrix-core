...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["Reindex"] == "Y") {
    ...
    echo htmlspecialcharsbx(LANG) ?>" name="fs1">
        ...
        <tr>
            <td width="40%"><?
                echo GetMessage("SEARCH_REINDEX_REINDEX_CHANGED") ?></td>
            <td width="60%"><input type="checkbox" name="Full" id="Full" value="N" checked
                                   OnClick="Full_OnClick(this.checked)"></td>
        </tr>
        <?
        ...
        <tr>
            ...
            <td><input type="text" name="max_execution_time" id="max_execution_time" size="3" value="<?
                ...
                echo GetMessage("SEARCH_REINDEX_STEP_sec") ?></td>
        </tr>
        <tr>
            <td><?= GetMessage("SEARCH_REINDEX_SITE") ?></td>
            <td><?
                echo CLang::SelectBox("LID", $str_LID, GetMessage("SEARCH_REINDEX_ALL"), "", "id=\"LID\""); ?></td>
        </tr>
        <tr>
            ...
            <td>
                <select name="MODULE_ID" id="MODULE_ID">
                    ...
                    foreach (CSearchParameters::GetModulesList() as $module_id => $module_name):?>
                        <option value="<?
                        echo $module_id ?>"><?
                            echo htmlspecialcharsbx($module_name) ?></option>
                    <?endforeach; ?>
                </select>
            </td>
        </tr>
        <tr>
            <td width="40%"><?
                echo GetMessage("SEARCH_REINDEX_CLEAR_SUGGEST") ?></td>
