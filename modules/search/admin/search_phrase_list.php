...
$arHeaders = array(
    array(
        "id" => "ID",
        "content" => GetMessage("SEARCH_PHL_ID"),
        "sort" => "ID",
        "default" => true,
        "align" => "right"
    ),
    ...
    array(
        "id" => "TIMESTAMP_X",
        "content" => GetMessage("SEARCH_PHL_TIMESTAMP_X"),
        "sort" => "TIMESTAMP_X",
        "default" => true
    ),
    ...
    array(
        "id" => "RESULT_COUNT",
        "content" => GetMessage("SEARCH_PHL_RESULT_COUNT"),
        "sort" => "RESULT_COUNT",
        "default" => true,
        "align" => "right"
    ),
    array(
        "id" => "PAGES",
        "content" => GetMessage("SEARCH_PHL_PAGES"),
        "title" => GetMessage("SEARCH_PHL_PAGES_ALT"),
        "sort" => "PAGES",
        "default" => true,
        "align" => "right"
    ),
...
if (IsModuleInstalled('statistic')) {
    $arHeaders[] = array(
        "id" => "STAT_SESS_ID",
        "content" => GetMessage("SEARCH_PHL_STAT_SESS_ID"),
        "sort" => "STAT_SESS_ID",
        "default" => true,
        "align" => "right"
    );
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    $row->AddViewField(
        "URL_TO",
        ($f_URL_TO_SITE_ID ? "[" . $f_URL_TO_SITE_ID . "]&nbsp;" : "") . "<a " . ($f_URL_TO_404 == "Y" ? 'style="color:red"' : '') . " title=\"" . GetMessage(
            "SEARCH_PHL_LINK_OPEN"
        ) . "\" href=\"$f_URL_TO\">" . TruncateText(
            InsertSpaces($f_URL_TO, 50, "<wbr>"),
            100
        ) . "</a>&nbsp;"
    );
    $row->AddViewField(
        ...
        "<a title=\"" . GetMessage(
            "SEARCH_PHL_SESSION"
        ) . "\" href=\"session_list.php?lang=" . LANGUAGE_ID . "&find_id=$f_STAT_SESS_ID&find_id_exact_match=Y&set_filter=Y\">$f_STAT_SESS_ID</a>&nbsp;"
    );
}
