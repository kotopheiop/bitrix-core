...
class CSearchCustomRank
{
    ...
    public static function Delete($ID)
    {
        ...
        return $DB->Query(
            "DELETE FROM b_search_custom_rank WHERE ID=" . $ID,
            false,
    ...
    public static function __GetParam(
        $lang,
        $site_id,
        $module_id = false,
        $param1 = false,
    ...
    public static function ModulesSelectBox(
        $sFieldName,
        $sValue,
        $sDefaultValue = "",
        $sFuncName = "",
        ...
        $s1 = '<option value="main"' . ($sValue == "main" ? ' selected' : '') . '>' . GetMessage(
                "customrank_files"
            ) . '</option>' . "\n";
        foreach (CSearchParameters::GetModulesList() as $module_id => $module_name) {
            $s1 .= '<option value="' . $module_id . '"' . ($sValue == $module_id ? ' selected' : '') . '>' . htmlspecialcharsEx(
                    $module_name
                ) . '</option>' . "\n";
        }
