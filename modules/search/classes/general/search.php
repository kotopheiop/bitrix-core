...
class CAllSearch extends CDBResult
{
    ...
    function Search($arParams, $aSort = array(), $aParamsEx = array(), $bTagsCloud = false)
    {
        ...
        $query = $this->Query->GetQueryString(
            (BX_SEARCH_VERSION > 1 ? "sct" : "sc") . ".SEARCHABLE_CONTENT",
            $strQuery,
            $bTagsSearch,
            $aParamsEx["STEMMING"],
            $this->_opt_ERROR_ON_EMPTY_STEM
        );
            ...
            if ($ar["DO_INSERT"]) {
                ...
                if ($rsUpdate->AffectedRowsCount() <= 0) {
                    $strSql = "
						...
						('" . $DB->ForSQL(
                            $ar["ID"]
                        ) . "', '" . $sql_lang_id . "', " . ($site_id <> '' ? "'" . $sql_site_id . "'" : "NULL") . ", " . $FREQ . ", " . number_format(
                            $ar["TF"],
                            2,
                            ".",
                            ""
        ...
        do {
            ...
            if ($strType == "STEM") {
                ...
                foreach ($a as $stem => $cnt) {
                    if ($stem == $strCondUpp) {
                        $strWh = mb_substr($strWh, 0, $pos) . "%^%" . mb_substr(
                                $strWh,
                                $pos,
                                $e - $pos + 1
                            ) . "%/^%" . mb_substr($strWh, $e + 1);
                        $strWhUpp = mb_substr($strWhUpp, 0, $pos) . "%^%" . str_repeat(
                                " ",
                                $e - $pos + 1
                ...
                $strWh = mb_substr($strWh, 0, $pos) . "%^%" . mb_substr($strWh, $pos, $l) . "%/^%" . mb_substr(
                        $strWh,
                        $pos + $l
                    );
                $strWhUpp = mb_substr($strWhUpp, 0, $pos) . "%^%" . str_repeat(" ", $l) . "%/^%" . mb_substr(
                        $strWhUpp,
                        $pos + $l
                    );
                $pos += 7 + $l;
    ...
    function PrepareSearchResult($str)
    {
        ...
        foreach ($words as $search => $type) {
            if ($type == "STEM") {
                $pregMask = "(?<!" . $pcreLettersClass . ")" . preg_quote(
                        $search,
                        "/"
                    ) . $pcreLettersClass . "*|" . $pregMask;
            } else {
        ...
        $matches = array();
        if (preg_match_all(
            "/(" . $pregMask . ")/i" . BX_UTF_PCRE_MODIFIER,
            $strUpp,
            $matches,
        ...
        if ($r) {
            ...
            if (count($this->url_add_params)) {
                ...
                if ($p2 === false) {
                    ...
                    $r["URL"] = mb_substr($r["URL"], 0, $p2) . $ch . implode("&", $this->url_add_params) . mb_substr(
                            $r["URL"],
                            $p2
                        );
                }
    ...
    public static function ReIndexAll($bFull = false, $max_execution_time = 0, $NS = Array(), $clear_suggest = false)
    {
        ...
        foreach (GetModuleEvents("search", "OnReindex", true) as $arEvent) {
            if ($NS["MODULE_ID"] != "" && $NS["MODULE_ID"] != $arEvent["TO_MODULE_ID"]) {
                continue;
            }
            if ($max_execution_time > 0 && $NS["MODULE"] <> '' && $NS["MODULE"] != "main" && $NS["MODULE"] != $arEvent["TO_MODULE_ID"]) {
                continue;
            }
    ...
    public static function GetIndex($MODULE_ID, $ITEM_ID)
    {
        $DB = CDatabase::GetModuleConnection('search');
        $rs = $DB->Query(
            "select * from b_search_content where MODULE_ID = '" . $DB->ForSql(
                $MODULE_ID
        ...
        $rs = $DB->Query(
            "select * from b_search_content_site where SEARCH_CONTENT_ID = " . $DB->ForSql($arFields["ID"])
        );
        while ($ar = $rs->Fetch()) {
        ...
        $rs = $DB->Query(
            "select * from b_search_content_right where SEARCH_CONTENT_ID = " . $DB->ForSql($arFields["ID"])
        );
        while ($ar = $rs->Fetch()) {
        ...
        $rs = $DB->Query(
            "select * from b_search_content_param where SEARCH_CONTENT_ID = " . $DB->ForSql($arFields["ID"])
        );
        while ($ar = $rs->Fetch()) {
    ...
    public static function Index($MODULE_ID, $ITEM_ID, $arFields, $bOverWrite = false, $SEARCH_SESS_ID = "")
    {
        ...
        if (array_key_exists("SITE_ID", $arFields)) {
            ...
            $strSql = "
				...
				" . (is_set(
                    $arFields,
                    "PARAM1"
                ) ? "AND (CR.PARAM1 IS NULL OR CR.PARAM1='' OR CR.PARAM1='" . $DB->ForSQL(
                        $arFields["PARAM1"]
                    ) . "')" : "") . "
				" . (is_set(
                    $arFields,
                    "PARAM2"
                ) ? "AND (CR.PARAM2 IS NULL OR CR.PARAM2='' OR CR.PARAM2='" . $DB->ForSQL(
                        $arFields["PARAM2"]
                    ) . "')" : "") . "
				" . ($ITEM_ID <> "" ? "AND (CR.ITEM_ID IS NULL OR CR.ITEM_ID='' OR CR.ITEM_ID='" . $DB->ForSQL(
                        $ITEM_ID
                    ) . "')" : "") . "
				ORDER BY
        ...
        if ($arResult = $r->Fetch()) {
            ...
            if ($bTitle && $bBody && $arFields["BODY"] == '' && $arFields["TITLE"] == '') {
                ...
                $DB->Query(
                    "DELETE FROM b_search_content_param WHERE SEARCH_CONTENT_ID = " . $ID,
                    false,
                    "File: " . __FILE__ . "<br>Line: " . __LINE__
                );
                $DB->Query(
                    "DELETE FROM b_search_content_right WHERE SEARCH_CONTENT_ID = " . $ID,
                    false,
                    "File: " . __FILE__ . "<br>Line: " . __LINE__
                );
                $DB->Query(
                    "DELETE FROM b_search_content_site WHERE SEARCH_CONTENT_ID = " . $ID,
                    false,
                    "File: " . __FILE__ . "<br>Line: " . __LINE__
                );
                $DB->Query(
                    "DELETE FROM b_search_content_title WHERE SEARCH_CONTENT_ID = " . $ID,
                    false,
                    "File: " . __FILE__ . "<br>Line: " . __LINE__
                );
                $DB->Query(
                    "DELETE FROM b_search_tags WHERE SEARCH_CONTENT_ID = " . $ID,
                    false,
                    "File: " . __FILE__ . "<br>Line: " . __LINE__
                );
            ...
            if (array_key_exists("LAST_MODIFIED", $arFields)) {
                ...
                $arFields["~DATE_CHANGE"] = $arFields["DATE_CHANGE"] = $DATE_CHANGE = $DB->FormatDate(
            ...
            if (!$bOverWrite && $DATE_CHANGE == $arResult["DATE_CHANGE"]) {
                if ($SEARCH_SESS_ID <> '') {
                    $DB->Query(
                        "UPDATE b_search_content SET UPD='" . $DB->ForSql($SEARCH_SESS_ID) . "' WHERE ID = " . $ID,
                        false,
                        "File: " . __FILE__ . "<br>Line: " . __LINE__
                    );
                }
            ...
            if ($ID === false) {
                ...
                $strSql = "SELECT ID FROM b_search_content WHERE MODULE_ID = '" . $DB->ForSQL(
                        $MODULE_ID
                    ) . "' AND ITEM_ID = '" . $DB->ForSQL($ITEM_ID) . "' ";
                $rs = $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
                $ar = $rs->Fetch();
    ...
    public static function ReindexFile($path, $SEARCH_SESS_ID = "")
    {
        ...
        if ($SEARCH_SESS_ID <> '') {
            $DATE_CHANGE = $DB->CharToDateFunction(
                FormatDate(
                    $DB->DateFormatToPHP(CLang::GetDateFormat("FULL")),
            ...
            $r = $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
            if ($arR = $r->Fetch()) {
                $strSql = "UPDATE b_search_content SET UPD='" . $DB->ForSQL(
                        $SEARCH_SESS_ID
                    ) . "' WHERE ID = " . $arR["ID"];
        ...
        if (!is_array($arrFile)) {
            ...
            if ($sHeadEndPos !== false) {
                ...
                $arMetaMatch = array();
                if (preg_match(
                    "/<(meta)\\s+([^>]*)(content)\\s*=\\s*(['\"]).*?(charset)\\s*=\\s*(.*?)(\\4)/is",
                    mb_substr($sFile, 0, $sHeadEndPos),
                    $arMetaMatch
                )) {
    ...
    public static function OnChangeFilePermissions(
        $path,
        $permission = array(),
    ...
    public static function DeleteOld($SESS_ID, $MODULE_ID = "", $SITE_ID = "")
    {
        ...
        while ($ar = $rs->Fetch()) {
            ...
            $DB->Query(
                "DELETE FROM b_search_content_param WHERE SEARCH_CONTENT_ID = " . $ar["ID"],
                false,
                "File: " . __FILE__ . "<br>Line: " . __LINE__
            );
            $DB->Query(
                "DELETE FROM b_search_content_right WHERE SEARCH_CONTENT_ID = " . $ar["ID"],
                false,
                "File: " . __FILE__ . "<br>Line: " . __LINE__
            );
            $DB->Query(
                "DELETE FROM b_search_content_site WHERE SEARCH_CONTENT_ID = " . $ar["ID"],
                false,
                "File: " . __FILE__ . "<br>Line: " . __LINE__
            );
            $DB->Query(
                "DELETE FROM b_search_content_title WHERE SEARCH_CONTENT_ID = " . $ar["ID"],
                false,
                "File: " . __FILE__ . "<br>Line: " . __LINE__
            );
            $DB->Query(
                "DELETE FROM b_search_tags WHERE SEARCH_CONTENT_ID = " . $ar["ID"],
                false,
                "File: " . __FILE__ . "<br>Line: " . __LINE__
            );
            CSearchFullText::getInstance()->deleteById($ar["ID"]);
            $DB->Query(
                "DELETE FROM b_search_content WHERE ID = " . $ar["ID"],
                false,
                "File: " . __FILE__ . "<br>Line: " . __LINE__
            );
        }
    ...
    public static function ChangePermission(
        $MODULE_ID,
        $arGroups,
        $ITEM_ID = false,
        $PARAM1 = false,
    ...
    function _CallbackPARAMS($field_name, $operation, $field_value)
    {
        ...
        if (is_array($field_value)) {
            foreach ($field_value as $key => $val) {
                ...
                $arSql[] = "EXISTS (SELECT * FROM b_search_content_param WHERE SEARCH_CONTENT_ID = " . $field_name . " AND PARAM_NAME = '" . $DB->ForSQL(
                        $key
...
class CAllSearchQuery
{
    ...
    function ParseStr($qwe)
    {
        ...
        if (!$this->no_bool_lang) {
            ...
            if ($this->rus_bool_lang == 'yes') {
                $qwe = preg_replace(
                    "/(\\s+|^|[|&~])" . GetMessage("SEARCH_TERM_OR") . "(\\s+|\$|[|&~])/is" . BX_UTF_PCRE_MODIFIER,
                    "\\1|\\2",
                    $qwe
                );
                $qwe = preg_replace(
                    "/(\\s+|^|[|&~])" . GetMessage("SEARCH_TERM_AND") . "(\\s+|\$|[|&~])/is" . BX_UTF_PCRE_MODIFIER,
                    "\\1&\\2",
                    $qwe
                );
                $qwe = preg_replace(
                    "/(\\s+|^|[|&~])" . GetMessage("SEARCH_TERM_NOT_1") . "(\\s+|\$|[|&~])/is" . BX_UTF_PCRE_MODIFIER,
                    "\\1~\\2",
                    $qwe
                );
                $qwe = preg_replace(
                    "/(\\s+|^|[|&~])" . GetMessage("SEARCH_TERM_NOT_2") . "(\\s+|\$|[|&~])/is" . BX_UTF_PCRE_MODIFIER,
                    "\\1~\\2",
                    $qwe
                );
            }
    ...
    function StemWord($w)
    {
        ...
        if (!$this->no_bool_lang) {
            if (preg_match("/^(OR|AND|NOT|WITHOUT)$/", $wu)) {
                ...
                if ($preg_ru === false) {
                    $preg_ru = "/^(" . ToUpper(
                            GetMessage("SEARCH_TERM_OR") . "|" . GetMessage("SEARCH_TERM_AND") . "|" . GetMessage(
                                "SEARCH_TERM_NOT_1"
                            ) . "|" . GetMessage("SEARCH_TERM_NOT_2")
    ...
    function StemQuery($q, $lang = "en")
    {
        ...
        return preg_replace_callback(
            "/([" . $arStemInfo["pcre_letters"] . "]+)/" . BX_UTF_PCRE_MODIFIER,
            array($this, "StemWord"),
            $q
        );
    }
