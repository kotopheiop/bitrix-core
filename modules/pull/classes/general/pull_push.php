...
class CPushManager
{
    ...
    public static function DeleteFromQueueByTag($userId, $tag, $appId = self::DEFAULT_APP_ID)
    {
        ...
        $strSql = "DELETE FROM b_pull_push_queue WHERE USER_ID = " . intval($userId) . " AND TAG = '" . $DB->ForSQL(
                $tag
            ) . "'";
    ...
    public function AddQueue($arParams)
    {
        ...
        $arFields['SUB_TAG'] = '';
        if (isset($arParams['SUB_TAG']) && trim($arParams['SUB_TAG']) <> '' && mb_strlen(
        ...
        foreach ($info as $userId => $params) {
            if (in_array($userId, $arFields['SKIP_USERS'])) {
                ...
                if ($params['mode'] == self::SEND_DEFERRED && isset($arParams['SEND_IMMEDIATELY']) && $arParams['SEND_IMMEDIATELY'] === 'Y') {
    ...
    public static function GetDeviceInfo($userId, $options = Array(), $appId = self::DEFAULT_APP_ID)
    {
        ...
        $query->registerRuntimeField(
            ...
            new \Bitrix\Main\Entity\ExpressionField(
                'IS_ONLINE_CUSTOM',
                'CASE WHEN LAST_ACTIVITY_DATE > ' . $sago . ' THEN \'Y\' ELSE \'N\' END'
            )
        );
        $query
            ->addSelect('ID')
        ...
        $query->registerRuntimeField(
            ...
            new \Bitrix\Main\Entity\ExpressionField(
                'HAS_MOBILE',
                'CASE WHEN main_user_push.USER_ID > 0 THEN \'Y\' ELSE \'N\' END'
            )
        );
        $query
    ...
    public function SendMessage($arMessages = [], $arDevices = [])
    {
        ...
        foreach ($arDevices as $arDevice) {
            $arDevice["APP_ID"] = \Bitrix\Main\Config\Option::get(
                "mobileapp",
                "app_id_replaced_" . $arDevice["APP_ID"],
                $arDevice["APP_ID"]
            );
    ...
    protected static function filterMessagesBeforeSend(array $messages, string $deviceType, string $deviceToken)
    {
        foreach ($messages as $k => $message) {
            if (isset($message['ADVANCED_PARAMS']['filterCallback']) && is_callable(
    ...
    public static function DeleteFromQueueBySubTag($userId, $tag, $appId = self::DEFAULT_APP_ID)
    {
        ...
        $strSql = "DELETE FROM b_pull_push_queue WHERE USER_ID = " . intval($userId) . " AND SUB_TAG = '" . $DB->ForSQL(
                $tag
            ) . "'";
        $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
    ...
    public static function SendAgent()
    {
        ...
        $strSql = $DB->TopSql(
            "SELECT ID, USER_ID, MESSAGE, PARAMS, ADVANCED_PARAMS, BADGE, APP_ID FROM b_pull_push_queue" . $sqlDate,
            280
        );
        ...
        while ($arRes = $dbRes->Fetch()) {
            ...
            try {
                $arRes['ADVANCED_PARAMS'] = $arRes['ADVANCED_PARAMS'] <> '' ? Bitrix\Main\Web\Json::decode(
