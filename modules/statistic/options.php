...
if ($STAT_RIGHT >= "R"):
    ...
    <form name="form_settings" method="POST"
        ...
        foreach ($arOPTIONS["TAB1"] as $key => $Option):
            if (!is_array($Option)):
                if ($key == "USE_AUTO_OPTIMIZE" && strtolower($statDB->type) == "mysql"):?>
                    <tr>
                        <td><label for="<?= $key ?>"><?
                                echo GetMessage("STAT_OPT_USE_AUTO_OPTIMIZE") ?></label></td>
                        <td nowrap><input type="checkbox" name="<?= $key ?>" id="<?= $key ?>" value="Y" <?
                            if (${$key} == "Y") echo "checked"; ?>></td>
                    </tr>
                <? elseif ($key == "BASE_CURRENCY" && CModule::IncludeModule("currency")):?>
                    <tr>
                        <td class="adm-detail-valign-top"><?
                            echo GetMessage("STAT_OPT_BASE_CURRENCY") ?></td>
                        <td><?
                            echo CCurrency::SelectBox("BASE_CURRENCY", $BASE_CURRENCY, " ", True, "") ?><br>
                            <input type="checkbox" name="recount_base_currency" id="recount_base_currency"
                                   value="Y"><label for="recount_base_currency"><?
                                echo GetMessage("STAT_OPT_DO_RECOUNT") ?></label></td>
                    </tr>
                <?endif;
            else:
                ...
                <tr>
                    <td <?
                    if ($type[0] == "textarea") echo 'class="adm-detail-valign-top"' ?>><label for="<?
                        echo htmlspecialcharsbx($Option[0]) ?>"><?
                            echo $Option[1] ?></label></td>
                    <td nowrap>
                        <?
                        if ($type[0] == "checkbox"):?>
        ...
        <? if (CModule::IncludeModule("fileman")):?>
            <tr>
                <td align="center" colspan=2>[ <a
                            href="/bitrix/admin/fileman_file_edit.php?lang=<?= LANGUAGE_ID ?>&amp;full_src=Y&amp;path=%2Fbitrix%2Factivity_limit.php"><? echo GetMessage("STAT_OPT_GRABBER_EDIT_503_TEMPLATE_LINK") ?></a>
                    ]
                </td>
            </tr>
        <? endif; ?>
        ...
        <tr>
            ...
            <td><input size="3" type="text" name="DEFENCE_DELAY" id="DEFENCE_DELAY"
                       value="<?= htmlspecialcharsbx($DEFENCE_DELAY) ?>">&nbsp;<? echo GetMessage("STAT_OPT_DEFENCE_DELAY_MEASURE_SEC") ?>
            </td>
        </tr>
        <tr>
            ...
            <td><input size="3" type="text" name="DEFENCE_STACK_TIME" id="DEFENCE_STACK_TIME"
                       value="<?= htmlspecialcharsbx($DEFENCE_STACK_TIME) ?>">&nbsp;<? echo GetMessage("STAT_OPT_DEFENCE_STACK_TIME_MEASURE_SEC") ?>
            </td>
        </tr>
        <tr>
            ...
            <td><input size="3" type="text" name="DEFENCE_MAX_STACK_HITS" id="DEFENCE_MAX_STACK_HITS"
                       value="<?= htmlspecialcharsbx($DEFENCE_MAX_STACK_HITS) ?>">&nbsp;<? echo GetMessage("STAT_OPT_DEFENCE_MAX_STACK_HITS_MEASURE") ?>
            </td>
        </tr>
        <tr>
            <td nowrap><label
                        for="DEFENCE_LOG"><? echo GetMessage("STAT_OPT_DEFENCE_LOG", array("#HREF#" => "/bitrix/admin/event_log.php?lang=" . LANGUAGE_ID . "&set_filter=Y&find_type=audit_type_id&find_audit_type[]=STAT_ACTIVITY_LIMIT")) ?></label>
            </td>
        ...
        foreach ($arOPTIONS["TAB5"] as $key => $Option):
            if (!is_array($Option)):
                if ($key == "ADV_EVENTS_DEFAULT"):?>
                    <tr>
                        <td width="40%"><?
                            echo GetMessage("STAT_OPT_ADV_EVENTS_DEFAULT") ?></td>
                        <td nowrap width="60%"><?
                            $arr = array(
                                "reference" => array(
        ...
        foreach ($arOPTIONS["TAB3"] as $key => $Option):
            if (!is_array($Option)):
                if ($key == "BROWSERS"):?>
                    <tr>
                        <td class="adm-detail-valign-top"
                            width="40%"><?= GetMessage("STAT_OPT_STORAGE_BROWSERS") ?></td>
                        <td nowrap width="60%"><textarea name="BROWSERS" rows="5"
                                                         cols="30"><?= htmlspecialcharsbx($BROWSERS) ?></textarea></td>
                    </tr>
                <? else:?>
                    <tr class="heading">
                        <td valign="top" colspan="2" align="center"><b><?= $Option ?></b></td>
                ...
                <tr>
                    <td <?
                    if ($type[0] == "textarea") echo 'class="adm-detail-valign-top"' ?> width="40%"><label for="<?
                        echo htmlspecialcharsbx($Option[0]) ?>"><?
                            echo $Option[1] ?></label></td>
                    <td nowrap width="60%">
                        <?
                    ...
                    <td nowrap width="60%">
                        ...
                        echo htmlspecialcharsbx($Option[0]) ?>"><?
                            if (strlen($Option[3]) > 0):?>
                                &nbsp;<label for="<?
                                echo htmlspecialcharsbx($Option[0]) ?>_clear"><?
                                    echo GetMessage("STAT_OPT_TIME_CLEAR") ?>:</label>&nbsp;<input type="checkbox"
                                                                                                   name="<?
                                                                                                   echo htmlspecialcharsbx($Option[0]) ?>_clear"
                                                                                                   id="<?
                                                                                                   echo htmlspecialcharsbx($Option[0]) ?>_clear"
                            ...
                            if (strlen($Option[4]) > 0):?>
                                &nbsp;&nbsp;(<?
                                echo GetMessage("STAT_OPT_TIME_RECORDS") ?>&nbsp;<?
                                echo $count ?>)
                            <?endif; ?>
                        <?endif ?>
                    </td>
            ...
            if (!is_array($Option)):
                if ($key == "SKIP_STATISTIC_WHAT"):?>
                    <tr>
                        <td class="adm-detail-valign-top" width="40%"><?
                            echo GetMessage("STAT_OPT_SKIP_RULES") ?>:
                        </td>
                        <td width="60%">
                            ...
                            <input type="radio" name="SKIP_STATISTIC_WHAT" id="SKIP_STATISTIC_WHAT_groups"
                                   ...
                                   OnClick="manageSkip('groups')"<?= $SKIP_STATISTIC_WHAT == "groups" ? " checked" : "" ?>><label
                                    for="SKIP_STATISTIC_WHAT_groups"><?= GetMessage("STAT_OPT_SKIP_GROUPS") ?></label><br>
                            <input type="radio" name="SKIP_STATISTIC_WHAT" id="SKIP_STATISTIC_WHAT_ranges"
                                   ...
                                   OnClick="manageSkip('ranges')"<?= $SKIP_STATISTIC_WHAT == "ranges" ? " checked" : "" ?>><label
                                    for="SKIP_STATISTIC_WHAT_ranges"><?= GetMessage("STAT_OPT_SKIP_RANGES") ?></label><br>
                ...
                elseif ($key == "SKIP_STATISTIC_GROUPS"):
                    ...
                    <tr>
                        ...
                        <td nowrap
                            width="60%"><?= SelectBoxMFromArray("arSKIP_STATISTIC_GROUPS[]", array("REFERENCE" => $ug, "REFERENCE_ID" => $ug_id), $arSKIP_STATISTIC_GROUPS, "", false, 10); ?></td>
                    </tr>
                <? elseif ($key == "SKIP_STATISTIC_IP_RANGES"):?>
                    <tr>
                        ...
                        <td nowrap width="60%"><textarea name="SKIP_STATISTIC_IP_RANGES" rows="5"
                                                         cols="30"><?= htmlspecialcharsbx($SKIP_STATISTIC_IP_RANGES) ?></textarea><br>
        ...
        <input <? if ($STAT_RIGHT < "W") echo "disabled" ?> type="submit" name="RestoreDefaults"
                                                            ...
                                                            OnClick="return confirm('<? echo GetMessageJS("MAIN_HINT_RESTORE_DEFAULTS_WARNING") ?>')"
    ...
    <form name="cleanupform" method="POST"
          action="<? echo $APPLICATION->GetCurPage() ?>?mid=<?= htmlspecialcharsbx($mid) ?>&amp;lang=<?= LANGUAGE_ID ?>">
    ...
    <? if (strtolower($statDB->type) == "mysql"):?>
    <form name="optimizeform" method="POST"
          action="<? echo $APPLICATION->GetCurPage() ?>?mid=<?= htmlspecialcharsbx($mid) ?>&amp;lang=<?= LANGUAGE_ID ?>">
        <? $tabControl2->BeginNextTab(); ?>
        <tr>
    ...
    <form name="optimizeform" method="POST"
        ...
        <tr>
            <td align="left" colspan="2">
                ...
                <span class="required"><? echo GetMessage("STAT_OPT_INDEX_ATTENTION") ?></span>
                - <? echo GetMessage("STAT_OPT_INDEX_ATTENTION_DETAIL") ?>
                <? echo EndNote(); ?>
            </td>
        </tr>
        <? $tabControl2->EndTab(); ?>
