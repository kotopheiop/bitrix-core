...
while ($arData = $dynamic->GetNext()) {
    ...
    $arrX[] = $date;
    if ($find_sessions == "Y") {
        $arrY_sessions[] = intval($arData["SESSIONS"]);
    }
    if ($find_sessions_back == "Y") {
        $arrY_sessions_back[] = intval($arData["SESSIONS_BACK"]);
    }
    if ($find_guests == "Y") {
        $arrY_guests[] = intval($arData["GUESTS"]);
    }
    if ($find_new_guests == "Y") {
        $arrY_new_guests[] = intval($arData["NEW_GUESTS"]);
    }
    if ($find_guests_back == "Y") {
        $arrY_guests_back[] = intval($arData["GUESTS_BACK"]);
    }
    if ($find_hosts == "Y") {
        $arrY_hosts[] = intval($arData["C_HOSTS"]);
    }
    if ($find_hosts_back == "Y") {
        $arrY_hosts_back[] = intval($arData["HOSTS_BACK"]);
    }
    if ($find_hits == "Y") {
        $arrY_hits[] = intval($arData["HITS"]);
    }
    if ($find_hits_back == "Y") {
        $arrY_hits_back[] = intval($arData["HITS_BACK"]);
    }
    $prev_date = $date;
}
...
$arrY = array();
if ($find_sessions == "Y") {
    $arrY = array_merge($arrY, $arrY_sessions);
}
if ($find_sessions_back == "Y") {
    $arrY = array_merge($arrY, $arrY_sessions_back);
}
if ($find_guests == "Y") {
    $arrY = array_merge($arrY, $arrY_guests);
}
if ($find_guests_back == "Y") {
    $arrY = array_merge($arrY, $arrY_guests_back);
}
if ($find_new_guests == "Y") {
    $arrY = array_merge($arrY, $arrY_new_guests);
}
if ($find_hosts == "Y") {
    $arrY = array_merge($arrY, $arrY_hosts);
}
if ($find_hosts_back == "Y") {
    $arrY = array_merge($arrY, $arrY_hosts_back);
}
if ($find_hits == "Y") {
    $arrY = array_merge($arrY, $arrY_hits);
}
if ($find_hits_back == "Y") {
    $arrY = array_merge($arrY, $arrY_hits_back);
}
