...
if ($grby == "S") {
    ...
    $headers = array(
        ...
        array(
            "id" => "SEARCHER_ID",
            "content" => GetMessage("STAT_SERVER"),
            "sort" => "s_searcher_id",
            "default" => true
        ),
        array(
            "id" => "DATE_HIT",
            "content" => GetMessage("STAT_DATE_INSERT"),
            "sort" => "s_date_hit",
            "default" => true
        ),
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    $row->AddViewField(
        "SEARCHER_ID",
        (($f_SEARCHER_ID > 1) ? "[<a title=\"" . GetMessage(
                "STAT_SEARCHER_INDEXING"
            ) . "\" href=\"searcher_list.php?lang=" . LANGUAGE_ID . "&find_id=$f_SEARCHER_ID&find_id_exact_match=Y&set_filter=Y\">$f_SEARCHER_ID</a>]" : "[$f_SEARCHER_ID]") .
        "&nbsp;<a title=\"" . GetMessage(
            "STAT_SEARCHER_PHRASES"
        ) . "\" href=\"?lang=" . LANGUAGE_ID . "&group_by=none" . (intval(
            $f_SEARCHER_ID
        ) > 1 ? "&menu_item_id=1" : "") . "&find_searcher_id=$f_SEARCHER_ID&find_searcher_id_exact_match=Y&set_filter=Y\">" . ((intval(
                $f_SEARCHER_ID
            ) > 1) ? $f_SEARCHER_NAME : "&lt;internal&gt;") . "</a>"
    );
    ...
    if ($grby == "P") {
        $row->AddViewField(
            ...
            "<a title=\"" . GetMessage(
                "STAT_PHRASE_SORT"
            ) . "\" href=\"?lang=" . LANGUAGE_ID . "&find_phrase=" . urlencode(
                htmlspecialcharsback($f_PHRASE)
            ) . "&find_phrase_exact_match=Y&set_filter=Y&group_by=none&menu_item_id=1\">$f_PHRASE</a>"
        );
    }
    ...
    $row->AddViewField(
        ...
        "<a title=\"" . GetMessage(
            "STAT_REFERRING_SITE"
        ) . "\" href=\"referer_list.php?lang=" . LANGUAGE_ID . "&find_id=$f_REFERER_ID&find_id_exact_match=Y&set_filter=Y\">$f_REFERER_ID</a>&nbsp;"
    );
    $row->AddViewField(
        "SESSION_ID",
        "<a title=\"" . GetMessage(
            "STAT_FIND_SESSION"
        ) . "\" href=\"session_list.php?lang=" . LANGUAGE_ID . "&find_id=$f_SESSION_ID&find_id_exact_match=Y&set_filter=Y\">$f_SESSION_ID</a>&nbsp;"
    );
}
...
?>
    ...
    <form name="form1" method="GET" action="<?= $APPLICATION->GetCurPage() ?>">
        ...
        <tr>
            ...
            <td><input type="text" name="find_phrase" size="47"
                       value="<? echo htmlspecialcharsbx($find_phrase) ?>"><?= ShowExactMatchCheckbox("find_phrase") ?>
        ...
        <tr valign=top>
            ...
            <td><?
                ...
                ?><br>
                ID: <input type="text" name="find_searcher_id_str" size="20"
                           value="<? echo htmlspecialcharsbx($find_searcher_id_str) ?>"> <?= GetMessage("STAT_NAME") ?>:
                <input type="text" name="find_searcher" size="40"
                       value="<? echo htmlspecialcharsbx($find_searcher) ?>"><?= ShowExactMatchCheckbox(
                    "find_searcher"
                ) ?> <?= ShowFilterLogicHelp() ?>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td><?
                ...
                ?>&nbsp;<input type="text" name="find_to" size="33"
                               value="<? echo htmlspecialcharsbx($find_to) ?>"><?= ShowExactMatchCheckbox("find_to") ?>
                &nbsp;<?= ShowFilterLogicHelp() ?></td>
        </tr>
        <tr>
            <td nowrap><? echo GetMessage("STAT_F_REFERER") ?></td>
            <td><input type="text" name="find_referer_id" size="47"
                       value="<? echo htmlspecialcharsbx($find_referer_id) ?>"><?= ShowExactMatchCheckbox(
                    "find_referer_id"
                ) ?>&nbsp;<?= ShowFilterLogicHelp() ?></td>
        </tr>
        <tr>
