...
?>
    ...
    <p><?= $show_site_id ?><?
        if ($public != "Y"):
            ?><a target="_blank" href="<?= htmlspecialcharsbx($section) ?>"
                 title="<?= GetMessage("STAT_GO_LINK") ?>"><?= htmlspecialcharsbx(TruncateText($section, 100)) ?></a><?
        else:
...
if ($days >= 2): ?>
    ...
    <form name="form1" method="POST" action="<?= $APPLICATION->GetCurPage() ?>">
        ...
        <p><?
            echo InputType("checkbox", "find_enter_points", "Y", $find_enter_points, false); ?>
            &nbsp;<?= GetMessage("STAT_ENTER_POINTS"); ?></p>
        <p><?
            echo InputType("checkbox", "find_exit_points", "Y", $find_exit_points, false); ?>
            &nbsp;<?= GetMessage("STAT_EXIT_POINTS"); ?></p>
        <p><?
            echo InputType("checkbox", "find_hits", "Y", $find_hits, false); ?>&nbsp;<?= GetMessage("STAT_HITS") ?></p>
        <input type="submit" name="set_filter" value="<?
        echo GetMessage("STAT_CREATE_GRAPH") ?>">
...
<?
    ...
    <form><input type="button" onClick="window.close()" value="<?
        echo GetMessage("STAT_CLOSE") ?>"></form>
<?endif ?>
<?
