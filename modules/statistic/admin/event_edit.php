...
$upload_dir = $_SERVER["DOCUMENT_ROOT"] . "/" . COption::GetOptionString(
        "main",
        "upload_dir",
        "/upload/"
    ) . "/statistic";
...
if ($Load != "" && $tabControl_active_tab == "load_manual_tab" && $STAT_RIGHT >= "W" && check_bitrix_sessid()) {
    if ($fp_out = fopen($OUTPUT_CSV_FILE, "wb")) {
        if (intval($nums) > 0) {
            $_SESSION["SESS_NUMS"] = intval($nums);
        }
        $total_rows = 0;
        foreach ($ARR as $pid) {
            $pid = intval($pid);
...
if (
    ...
    if (preg_match("/^(" . preg_quote($upload_dir . "/", "/") . ")[0-9a-fA-F]{32}_(in|out)\\.csv$/", $csvfile)) {
        ...
        if ($all_loaded == "N") {
            $success = "N";
            $link .= "&csvfile=" . urlencode($csvfile) . "&time_step=" . intval(
                    $time_step
                ) . "&next_line=" . $next_line . "&total_loaded=" . $total_loaded . "&total_rows=" . $total_rows . "&check_unique=" . $check_unique . "&total_duplicate=" . $total_duplicate . "&total_processed=" . $total_processed . "&next_pos=" . $next_pos . "&preview=" . $preview . "&" . $tab . "&" . bitrix_sessid_get(
                );
        } elseif ($all_loaded == "Y") {
            $success = "N";
        ...
        CAdminMessage::ShowMessage(
            array(
                "DETAILS" => GetMessage(
                    ...
                    ) . '<br><a href="/bitrix/admin/settings.php?lang=' . LANGUAGE_ID . '&amp;mid=statistic">' . GetMessage(
                ...
                <table cellspacing="0" cellpadding="0" border="0" class="internal">
                    ...
                    <? if ($check_unique == "Y"): ?>
                        <tr>
                            ...
                            <td align="right"><span
                                        class="<? echo $total_duplicate > 0 ? "required" : "" ?>"><?= $step_duplicate ?></span>
                            </td>
                            <td align="right"><span
                                        class="<? echo $total_duplicate > 0 ? "required" : "" ?>"><?= $total_duplicate ?></span>
                            </td>
                        </tr>
                    <? endif; ?>
                    <tr>
...
<? else://if(!$success=="N")?>
    ...
    <script language="JavaScript">
        ...
        function addNewRow(currentId) {
            ...
            oCell = oRow.insertCell(i++);
            oCell.noWrap = true;
            oCell.innerHTML = '<input type="hidden" name="ARR[]" value="' + id + '"><input type="text" size="14" name="EVENT_ID_' + id + '" value="" OnFocus="addNewRow(' + id + ')"><input type="button" onClick="SelectEvent(\'form1\',\'EVENT_ID_' + id + '\')" title="<?echo GetMessage(
                "STAT_SELECT_EVENT"
            )?>" value="..."></a>';
            ...
            oCell = oRow.insertCell(i++);
            oCell.innerHTML = '<input type="text" name="DATE_ENTER_' + id + '" size="18" value=""> <a href="javascript:void(0);" onClick="Calendar(\'name=DATE_ENTER_' + id + '&from=&to=&form=form1\', document.form1.DATE_ENTER_' + id + '.value);" title="<?echo GetMessage(
                "STAT_CALENDAR"
            )?>"><img src="/bitrix/images/icons/calendar.gif" alt="<?echo GetMessage(
                "STAT_CALENDAR"
            ...
            oCell = oRow.insertCell(i++);
            oCell.innerHTML = '<a href="javascript: Copy(' + id + ')"><img src="/bitrix/images/statistic/copy.gif" width="15" height="15" border=0 class="tb2" hspace="2" alt="<?echo GetMessage(
                "STAT_COPY"
            )?>"></a>';
            ...
            <td>
                <table cellspacing="0" cellpadding="0" border="0">
                    <tr>
                        ...
                        <td><span id="edit_link_span">[&nbsp;<a target="_blank" class="tablebodylink"
                                                                href="javascript:void(0)" id="edit_link"><?= GetMessage(
                                        "STAT_EDIT"
                                    ) ?></a>&nbsp;]</span></td>
                    </tr>
                </table>
        ...
        <tr>
            <td colspan="2">
                <table border="0" cellspacing="0" cellpadding="0" class="internal" id="table1">
                    <tr class="heading">
                        <td>#</td>
                        <td><input type="checkbox" name="selectall" value="Y" onclick="OnSelectAll(this.checked)"
                                   checked></td>
                    ...
                    while ($i <= $count) :
                        ...
                        <tr>
                            ...
                            <td nowrap>
                                ...
                                <input type="text" size="14" name="EVENT_ID_<? echo $i ?>"
                                       value="<?= htmlspecialcharsbx($event_id) ?>" OnFocus="addNewRow(<? echo $i ?>)">
                                <input type="button" onClick="SelectEvent('form1','EVENT_ID_<? echo $i ?>')"
                                       title="<? echo GetMessage("STAT_SELECT_EVENT") ?>" value="...">
                            </td>
                            <td><input type="text" size="20" name="EVENT3_<? echo $i ?>"
                                       value="<?= htmlspecialcharsbx($event3) ?>" OnFocus="addNewRow(<? echo $i ?>)">
                            </td>
                            <td nowrap><? echo CalendarDate(
                                    "DATE_ENTER_" . $i,
                                    htmlspecialcharsbx($date_enter),
                                    "form1",
                                    "18"
                                ) ?></td>
                            <td><input type="text" style="width:100%" name="PARAM_<? echo $i ?>"
                                       value="<?= htmlspecialcharsbx($param) ?>" OnFocus="addNewRow(<? echo $i ?>)">
                            </td>
                            <td><input type="text" size="5" name="MONEY_<? echo $i ?>"
                                       value="<?= htmlspecialcharsbx($money) ?>" OnFocus="addNewRow(<? echo $i ?>)">
                            </td>
                            <? if ($currency_module == "Y") :?>
                            ...
                            <td><a href="javascript: Copy(<?= $i ?>)"><img src="/bitrix/images/statistic/copy.gif"
                                                                           width="15" height="15" border=0 class="tb2"
                                                                           hspace="2"
                                                                           alt="<? echo GetMessage("STAT_COPY") ?>"></a>
                            </td>
                        </tr>
                        <?
                        $i++;
    ...
    <span class="required"><?= GetMessage("STAT_ATTENTION_1") . " !&nbsp;&nbsp;" ?></span><?= GetMessage(
        "STAT_CONVERT_TO_DEFAULT_CUR"
    ) ?>
    <? echo EndNote(); ?>
