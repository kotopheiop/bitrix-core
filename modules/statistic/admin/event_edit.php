                ...
                <table cellspacing="0" cellpadding="0" border="0">
                    <tr>
                        ...
                        <td><span id="edit_link_span">[&nbsp;<a target="_blank" class="tablebodylink"
                                                                ...
                                                                id="edit_link"><?= GetMessage("STAT_EDIT") ?></a>&nbsp;]</span>
                        </td>
                    </tr>
                </table>
            </td>
                ...
                <table border="0" cellspacing="0" cellpadding="0" class="internal" id="table1">
                    ...
                    while ($i <= $count) :
                        ...
                        <tr>
                            <td align="right"><?= $i ?></td>
                            <td><?
                                echo InputType("checkbox", "LOAD_" . $i, "Y", "Y", false); ?></td>
                            <td nowrap>
                                <input type="hidden" name="ARR[]" value="<?
                                echo $i ?>">
                                <input type="text" size="14" name="EVENT_ID_<?
                                echo $i ?>" value="<?= htmlspecialcharsbx($event_id) ?>" OnFocus="addNewRow(<?
                                echo $i ?>)">
                                <input type="button" onClick="SelectEvent('form1','EVENT_ID_<?
                                echo $i ?>')" title="<?
                                echo GetMessage("STAT_SELECT_EVENT") ?>" value="...">
                            </td>
                            <td><input type="text" size="20" name="EVENT3_<?
                                echo $i ?>" value="<?= htmlspecialcharsbx($event3) ?>" OnFocus="addNewRow(<?
                                echo $i ?>)"></td>
                            <td nowrap><?
                                echo CalendarDate("DATE_ENTER_" . $i, htmlspecialcharsbx($date_enter), "form1", "18") ?></td>
                            <td><input type="text" style="width:100%" name="PARAM_<?
                                echo $i ?>" value="<?= htmlspecialcharsbx($param) ?>" OnFocus="addNewRow(<?
                                echo $i ?>)"></td>
                            <td><input type="text" size="5" name="MONEY_<?
                                echo $i ?>" value="<?= htmlspecialcharsbx($money) ?>" OnFocus="addNewRow(<?
                                echo $i ?>)"></td>
                            <?
                            if ($currency_module == "Y") :?>
                                <td><?
                                    echo SelectBoxFromArray("CURRENCY_" . $i, $arrCurrency, $currency, " "); ?></td>
                            <?endif; ?>
                            <td><?
                                echo InputType("checkbox", "CHARGEBACK_" . $i, "Y", $chargeback, false); ?></td>
                            <td><a href="javascript: Copy(<?= $i ?>)"><img src="/bitrix/images/statistic/copy.gif"
                                                                           width="15" height="15" border=0 class="tb2"
                                                                           hspace="2" alt="<?
                                    echo GetMessage("STAT_COPY") ?>"></a></td>
                        </tr>
                        <?
                        $i++;
                    endwhile;
