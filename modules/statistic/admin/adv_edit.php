...
function CheckFields()
{
    ...
    if (trim($str_REFERER1) == '' && trim($str_REFERER2) == '') {
        ...
        if ($a->Fetch()) {
            $str .= GetMessage("STAT_WRONG_REFERER") . "<br>";
        }
    }
    $strError .= $str;
    if ($str <> '') {
...
$aTabs = array(
    ...
    array(
        "DIV" => "edit2",
        "TAB" => GetMessage("STAT_MORE"),
        "ICON" => "stat_adv_edit",
        "TITLE" => GetMessage("STAT_MORE_TITLE")
    ),
);
...
if ($base_currency <> '') {
    if (CModule::IncludeModule("currency")) {
        ...
        while ($arCur = $rsCur->Fetch()) {
            ...
            if ($base_currency == $arCur["CURRENCY"]) {
                $str .= " [" . GetMessage("STAT_BASE") . "]";
            }
            $arrRef[] = $str;
        ...
        if ($strError == '') {
            ...
            if ($strError == '') {
                if ($save <> '') {
                    ...
                    LocalRedirect(
                        $APPLICATION->GetCurPage() . "?lang=" . LANG . "&ID=" . $ID . "&" . $tabControl->ActiveTabParam(
                        )
                    );
                }
            }
...
if (intval($ID) > 0) {
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('" . GetMessageJS(
                "STAT_RESET_ADV_CONFIRM"
            ) . "'))window.location='adv_list.php?clear_id=" . $ID . "&lang=" . LANGUAGE_ID . "&" . bitrix_sessid_get(
            ) . "';",
    );
    ...
    $aMenu[] = array(
        ...
        "LINK" => "javascript:if(confirm('" . GetMessageJS(
                "STAT_DELETE_ADV_CONFIRM"
            ) . "'))window.location='adv_list.php?ID=" . $ID . "&action=delete&lang=" . LANGUAGE_ID . "&" . bitrix_sessid_get(
            ) . "';",
    );
}
...
?>
    <a name="tb"></a>
<? if ($base_currency == '') : ?>
    <p><?= GetMessage("STAT_BASE_CURRENCY_NOT_INSTALLED") . $base_currency ?>&nbsp;[&nbsp;<a
                href="/bitrix/admin/settings.php?lang=<?= LANGUAGE_ID ?>&mid=statistic"><?= GetMessage(
...
<?endif; ?>
    <form method="POST" action="<?= $APPLICATION->GetCurPage() ?>?ID=<?= $ID ?>&lang=<?= LANGUAGE_ID ?>">
        ...
        <tr>
            ...
            <td width=60%><input type="text" name="PRIORITY" size="5" value="<?= $str_PRIORITY ?>">&nbsp;<?= GetMessage(
                    "STAT_PRIORITY_ALT"
                ) ?></td>
        </tr>
        <tr class=heading>
        ...
        <tr valign="top">
            <td valign="top"><?= GetMessage("STAT_SEARCHERS") ?><br><IMG SRC="/bitrix/images/statistic/mouse.gif"
                                                                         WIDTH="44" HEIGHT="21" BORDER=0
                                                                         ALT=""><br><?= GetMessage(
                    "STAT_SELECT_WHAT_YOU_NEED"
                ) ?></td>
            <td><?
                $ref = $ref_id = array();
        ...
        <? if ($STAT_RIGHT > "M"): ?>
            ...
            <tr>
                ...
                <td width=60%><input type="text" name="COST" size="10"
                                     value="<? echo ($str_COST != 0) ? $str_COST : "" ?>"><? if ($currency_module == "Y") : ?>&nbsp;<? echo SelectBoxFromArray(
                        "CURRENCY_COST",
                        $arrCurrency,
                        $CURRENCY_COST
                    ); ?><?endif; ?></td>
            ...
            <tr>
                ...
                <td><input type="text" name="REVENUE" size="10"
                           value="<? echo ($str_REVENUE != 0) ? $str_REVENUE : "" ?>"><? if ($currency_module == "Y") : ?>&nbsp;<? echo SelectBoxFromArray(
                        "CURRENCY_REVENUE",
                        $arrCurrency,
                        $CURRENCY_REVENUE
                    ); ?><?endif; ?></td>
        ...
        <tr>
            ...
            <td><?
                $arr = array(
                    "reference" => array(
                        GetMessage("STAT_SHOW_LINK"),
                        GetMessage("STAT_SHOW_LIST"),
                        GetMessage("STAT_GROUP_BY_EVENT1"),
                        GetMessage("STAT_GROUP_BY_EVENT2")
                    ),
                    "reference_id" => array("link", "list", "event1", "event2")
                );
        ...
        $tabControl->EndTab();
        $tabControl->Buttons(
            array("disabled" => ($STAT_RIGHT < "W"), "back_url" => "adv_list.php?lang=" . LANGUAGE_ID)
        );
        $tabControl->End();
        ?>
    </form>
