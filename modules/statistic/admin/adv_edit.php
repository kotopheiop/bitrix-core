...
if (strlen($base_currency) <= 0) : ?>
    <p><?= GetMessage("STAT_BASE_CURRENCY_NOT_INSTALLED") . $base_currency ?>&nbsp;[&nbsp;<a
                href="/bitrix/admin/settings.php?lang=<?= LANGUAGE_ID ?>&mid=statistic"><?= GetMessage("STAT_CHOOSE_CURRENCY") ?></a>&nbsp;]
    </p>
<?endif; ?>
    <form method="POST" action="<?= $APPLICATION->GetCurPage() ?>?ID=<?= $ID ?>&lang=<?= LANGUAGE_ID ?>">
        ...
        <tr>
            ...
            <td width=60%><input type="text" name="PRIORITY" size="5"
                                 value="<?= $str_PRIORITY ?>">&nbsp;<?= GetMessage("STAT_PRIORITY_ALT") ?></td>
        </tr>
        <tr class=heading>
        ...
        <tr class=heading>
            <td colspan="2"><?
                echo GetMessage("STAT_ID_ADD"); ?><span class=required><sup>1</sup></span></td>
        </tr>
        <tr>
            <td valign=top><?
                echo GetMessage("STAT_PAGES_FROM") ?></td>
            <td><textarea name="PAGES_FROM" cols="50" rows="8"><?
        ...
        <tr valign="top">
            <td valign="top"><?= GetMessage("STAT_SEARCHERS") ?><br><IMG SRC="/bitrix/images/statistic/mouse.gif"
                                                                         ...
                                                                         ALT=""><br><?= GetMessage("STAT_SELECT_WHAT_YOU_NEED") ?>
            </td>
            <td><?
                $ref = $ref_id = array();
        ...
        <tr>
            <td valign=top><?
                echo GetMessage("STAT_PAGES_TO") ?></td>
            <td><textarea name="PAGES_TO" cols="50" rows="8"><?
        ...
        if ($STAT_RIGHT > "M"): ?>
            ...
            <tr>
                ...
                <td width=60%><input type="text" name="COST" size="10" value="<?
                    echo ($str_COST != 0) ? $str_COST : "" ?>"><?
                    if ($currency_module == "Y") : ?>&nbsp;<?
                        echo SelectBoxFromArray("CURRENCY_COST", $arrCurrency, $CURRENCY_COST); ?><?endif; ?></td>
            </tr>
            <tr>
                <td><?= GetMessage("STAT_REVENUE") ?></td>
                <td><input type="text" name="REVENUE" size="10" value="<?
                    echo ($str_REVENUE != 0) ? $str_REVENUE : "" ?>"><?
                    if ($currency_module == "Y") : ?>&nbsp;<?
                        echo SelectBoxFromArray("CURRENCY_REVENUE", $arrCurrency, $CURRENCY_REVENUE); ?><?endif; ?></td>
            </tr>
        <?endif; ?>
        <tr class=heading>
        ...
        <tr>
            ...
            <td><textarea name="DESCRIPTION" rows="8" cols="50"><?
                    echo $str_DESCRIPTION ?></textarea></td>
        </tr>
        <?
        $tabControl->EndTab();
...
echo GetMessage("STAT_ID_ADD_NOTE") ?>
<?
echo EndNote();
