    ...
    foreach ($arID as $ID) {
        ...
        switch ($_REQUEST['action']) {
            case "delete":
                ...
                $statDB->Query(
                    "DELETE FROM b_stat_searcher_params WHERE SEARCHER_ID = " . $ID,
                    false,
                    $err_mess . __LINE__
                );
                $statDB->Query(
                    "DELETE FROM b_stat_searcher_day WHERE SEARCHER_ID = " . $ID,
                    false,
                    $err_mess . __LINE__
                );
                $statDB->Commit();
...
$headers = array(
    ...
    array(
        "id" => "TODAY_HITS",
        "content" => GetMessage("STAT_TODAY_HITS") . " $today",
        "sort" => "s_today_hits",
        "default" => true,
        "align" => "right"
    ),
    array(
        "id" => "YESTERDAY_HITS",
        "content" => GetMessage("STAT_YESTERDAY_HITS") . " $yesterday",
        "sort" => "s_yesterday_hits",
        "default" => true,
        "align" => "right"
    ),
    array(
        "id" => "B_YESTERDAY_HITS",
        "content" => GetMessage("STAT_B_YESTERDAY_HITS") . " $b_yesterday",
        "sort" => "s_b_yesterday_hits",
        "default" => true,
        "align" => "right"
    ),
);
if ($arFilter["DATE1_PERIOD"] <> '') {
    $headers[] = array(
        "id" => "PERIOD_HITS",
        "content" => GetMessage("STAT_PERIOD_HITS") . " " . htmlspecialcharsEx(
                $arFilter["DATE1_PERIOD"]
            ) . " " . htmlspecialcharsEx($arFilter["DATE2_PERIOD"]),
        "sort" => "s_period_hits",
        "default" => true,
        "align" => "right"
    );
...
$headers[] = array(
    "id" => "TOTAL_HITS",
    "content" => GetMessage("STAT_TOTAL_HITS"),
    "sort" => "s_total_hits",
    "default" => true,
    "align" => "right"
);
$headers[] = array(
    "id" => "DATE_LAST",
    "content" => GetMessage("STAT_LAST_DATE"),
    "sort" => "s_date_last",
    "default" => true
);
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    if ($dynamic_days >= 2 && function_exists("ImageCreate")) {
        $txt = "<a title=\"" . GetMessage(
                "STAT_GRAPH_ALT"
            ) . "\" href=\"searcher_graph_list.php?lang=" . LANGUAGE_ID . "&find_searchers[]=$f_ID&set_filter=Y\">$f_NAME</a>";
    ...
    if ($f_TODAY_HITS > 0) {
        $txt = "<a title=\"" . GetMessage(
                "STAT_HITS_SHOW"
            ) . "\" href=\"hit_searcher_list.php?lang=" . LANGUAGE_ID . "&find_searcher_id=$f_ID&find_searcher_id_exact_match=Y&find_date1=$today&set_filter=Y\">$f_TODAY_HITS</a>";
    ...
    if ($f_YESTERDAY_HITS > 0) {
        $txt = "<a title=\"" . GetMessage(
                "STAT_HITS_SHOW"
            ) . "\" href=\"hit_searcher_list.php?lang=" . LANGUAGE_ID . "&find_searcher_id=$f_ID&find_searcher_id_exact_match=Y&find_date1=$yesterday&find_date2=$yesterday&set_filter=Y\">$f_YESTERDAY_HITS</a>";
    ...
    if ($f_B_YESTERDAY_HITS > 0) {
        $txt = "<a title=\"" . GetMessage(
                "STAT_HITS_SHOW"
            ) . "\" href=\"hit_searcher_list.php?lang=" . LANGUAGE_ID . "&find_searcher_id=$f_ID&find_searcher_id_exact_match=Y&find_date1=$b_yesterday&find_date2=$b_yesterday&set_filter=Y\">$f_B_YESTERDAY_HITS</a>";
        $row->AddViewField("B_YESTERDAY_HITS", $txt);
    }

    if ($f_PERIOD_HITS > 0) {
        $txt = "<a title=\"" . GetMessage("STAT_HITS_SHOW") . "\" href=\"" . htmlspecialcharsbx(
                "hit_searcher_list.php?lang=" . urlencode(LANGUAGE_ID) . "&find_searcher_id=" . urlencode(
                    $f_ID
                ) . "&find_searcher_id_exact_match=Y&find_date1=" . urlencode(
                    $arFilter["DATE1_PERIOD"]
                ) . "&find_date2=" . urlencode($arFilter["DATE2_PERIOD"]) . "&set_filter=Y"
            ) . "\">$f_PERIOD_HITS</a>";
    ...
    if ($f_TOTAL_HITS > 0) {
        $txt = "<a title=\"" . GetMessage(
                "STAT_HITS_SHOW"
            ) . "\" href=\"hit_searcher_list.php?lang=" . LANGUAGE_ID . "&find_searcher_id=$f_ID&find_searcher_id_exact_match=Y&set_filter=Y\">$f_TOTAL_HITS</a>";
    ...
    if (function_exists("ImageCreate")) {
        $arActions[] = array(
            "TITLE" => GetMessage("STAT_GRAPH_ALT"),
            "ACTION" => $lAdmin->ActionRedirect(
                "searcher_graph_list.php?lang=" . LANGUAGE_ID . "&find_searchers[]=$f_ID&set_filter=Y"
            ),
    ...
    $arActions[] = array(
        "TITLE" => GetMessage("STAT_DYNAMICS_ALT"),
        "ACTION" => $lAdmin->ActionRedirect(
            "searcher_dynamic_list.php?lang=" . LANGUAGE_ID . "&find_searcher_id=$f_ID&find_searcher_id_exact_match=Y&set_filter=Y"
        ),
    ...
    $arActions[] = array(
        "ICON" => "edit",
        "TITLE" => GetMessage("STAT_CHANGE_SEARCHER"),
        "ACTION" => $lAdmin->ActionRedirect("searcher_edit.php?lang=" . LANGUAGE_ID . "&ID=$f_ID"),
    ...
    if ($STAT_RIGHT >= "W") {
        $arActions[] = array(
            ...
            "ACTION" => "javascript:if(confirm('" . GetMessageJS(
                    "STAT_CONFIRM"
                ) . "')) window.location='?lang=" . LANGUAGE_ID . "&action=delete&ID=$f_ID&" . bitrix_sessid_get(
                ) . "'",
...
$APPLICATION->SetTitle(
    GetMessage("STAT_RECORDS_LIST", array("#STATISTIC_DAYS#" => COption::GetOptionString("statistic", "SEARCHER_DAYS")))
);
...
?>
    ...
    <form name="form1" method="GET" action="<?= $APPLICATION->GetCurPage() ?>?">
        ...
        <tr>
            ...
            <td width="0%" nowrap><input type="text" name="find_name" value="<? echo htmlspecialcharsbx($find_name) ?>"
                                         size="47"><?= ShowExactMatchCheckbox("find_name") ?>
        ...
        <tr>
            ...
            <td width="0%" nowrap><input type="text" maxlength="10" name="find_hits1"
                                         value="<? echo htmlspecialcharsbx($find_hits1) ?>"
                                         size="9"><? echo "&nbsp;" . GetMessage("STAT_TILL") . "&nbsp;" ?><input
                        type="text" maxlength="10" name="find_hits2" value="<? echo htmlspecialcharsbx($find_hits2) ?>"
                        size="9"></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td width="0%" nowrap><input type="text" name="find_user_agent"
                                         value="<? echo htmlspecialcharsbx($find_user_agent) ?>"
                                         size="47"><?= ShowExactMatchCheckbox("find_user_agent") ?>
                &nbsp;<?= ShowFilterLogicHelp() ?></td>
        </tr>
        <?
