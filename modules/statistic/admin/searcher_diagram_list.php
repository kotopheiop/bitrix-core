        ...
        <table cellpadding="0" cellspacing="0" border="0" class="graph" align="center">
            <tr>
                <td><?
                    ...
                    ?><img class="graph" src="<?
                    echo htmlspecialcharsbx($url) ?>" width="<?= $diameter ?>" height="<?= $diameter ?>">
                </td>
                <td>
                    <table border="0" cellspacing="2" cellpadding="0" class="legend">
                        ...
                        foreach ($arr as $key => $sector) {
                            ...
                            <tr>
                                ...
                                <td nowrap class="number"><?
                                    echo sprintf("%01.2f", $procent) . "%" ?></td>
                                <td nowrap class="number">(<a title="<?
                                    echo GetMessage("STAT_VIEW_SEARCHER_HITS") ?>" href="<?
                                    echo htmlspecialcharsbx("/bitrix/admin/hit_searcher_list.php?lang=" . urlencode(LANGUAGE_ID) . "&find_searcher_id=" . urlencode($id) . "&find_date1=" . urlencode($arFilter["DATE1_PERIOD"]) . "&find_date2=" . urlencode($arFilter["DATE2_PERIOD"]) . "&set_filter=Y") ?>"><?= $sector["COUNTER"] ?></a>)
                                </td>
                                <td nowrap>[<a title="<?= GetMessage("STAT_SEARCHER_LIST_OPEN") ?>"
                                               href="searcher_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $id ?>&set_filter=Y"><?= $id ?></a>]
                                    <a title="<?
                                    echo GetMessage("STAT_VIEW_SEARCHER_GRAPH") ?>"
...
?>
    <form name="form1" action="<?= $APPLICATION->GetCurPage() ?>?">
        ...
        if (is_array($arrSEARCHERS)):
            ...
            <tr valign="top">
                <td width="0%" nowrap><?
                    echo GetMessage("STAT_F_SEACHERS") ?><br><IMG SRC="/bitrix/images/statistic/mouse.gif" WIDTH="44"
                                                                  HEIGHT="21" BORDER=0 ALT=""></td>
                <td width="0%" nowrap><?
                    echo SelectBoxMFromArray("find_searchers[]", array("REFERENCE" => $ref, "REFERENCE_ID" => $ref_id), $find_searchers, "", false, "11", "' style=\"width:100%\""); ?></td>
            </tr>
        <?
        endif;
