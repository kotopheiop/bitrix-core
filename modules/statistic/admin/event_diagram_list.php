        ...
        <table cellspacing="0" cellpadding="0" class="graph" border="0" align="center">
            <tr>
                <td valign="top" class="graph"><?
                    ...
                    ?><img class="graph" src="<?
                    echo htmlspecialcharsbx("event_diagram.php?rand=" . urlencode(rand()) . "&lang=" . urlencode(LANGUAGE_ID) . $s . "&find_date1=" . urlencode($arFilter["DATE1_PERIOD"]) . "&find_date2=" . urlencode($arFilter["DATE2_PERIOD"])) ?>"
                        ...
                        foreach ($arr as $key => $sector):
                            ...
                            <tr>
                                ...
                                <td align="right" class="number"><?
                                    echo sprintf("%01.2f", $procent) . "%" ?></td>
                                <td align="right" class="number">(<a title="<?
                                    echo GetMessage("STAT_VIEW_EVENT_LIST") ?>" class="stat_link" href="<?
                                    echo htmlspecialcharsbx("event_list.php?lang=" . urlencode(LANGUAGE_ID) . "&find_event_id=" . urlencode($id) . "&find_date1=" . urlencode($arFilter["DATE1_PERIOD"]) . "&find_date2=" . urlencode($arFilter["DATE2_PERIOD"]) . "&set_filter=Y") ?>"><?= $counter ?></a>)
                                </td>
                                <td>[<a class="stat_link"
                                        href="event_type_list.php?lang=<?= LANG ?>&amp;find_id=<?= $id ?>&amp;set_filter=Y"><?= $id ?></a>]
                                    <a title="<?
                                    echo GetMessage("STAT_VIEW_GRAPH") ?>" class="stat_link"
                                       href="event_graph_list.php?lang=<?= LANG ?>&amp;find_events[]=<?= $id ?>&amp;set_filter=Y"><?
                                        echo htmlspecialcharsbx($name) ?></a></font></td>
                            </tr>
                        <?endforeach; ?>
                    </table>
                </td>
...
?>
    <script type="text/javascript">
        function applyFilter() {
            ...
            <?=$sFilterID?>.
            OnSet('<?=CUtil::JSEscape($sTableID)?>', '<?=CUtil::JSEscape($APPLICATION->GetCurPage() . "?lang=" . LANG . "&")?>');
            return false;
        }
        ...
        function deleteFilter() {
            ...
            <?=$sFilterID?>.
            OnClear('<?=CUtil::JSEscape($sTableID)?>', '<?=CUtil::JSEscape($APPLICATION->GetCurPage() . "?lang=" . LANG . "&")?>');
            return false;
        }
    </script>
    <form name="find_form" method="GET" action="<?= $APPLICATION->GetCurPage() ?>?">
        ...
        <tr>
            ...
            <td><?
                echo CalendarPeriod("find_date1", $find_date1, "find_date2", $find_date2, "find_form", "Y") ?></td>
        </tr>
        <tr valign="top">
            <td><?
                echo GetMessage("STAT_F_EVENTS") ?><br><img src="/bitrix/images/statistic/mouse.gif" width="44"
            ...
            <td><?
                ...
                <br>
                <input type="button" OnClick="selectEventType('find_form','find_events[]')"
                       value="<?= GetMessage("STAT_ADD") ?>...">
                <input type="button" OnClick="jsSelectUtils.deleteSelectedOptions('find_events[]');"
                       value="<?= GetMessage("STAT_DELETE") ?>">
            </td>
        </tr>
        <?
        $oFilter->Buttons() ?>
        <span class="adm-btn-wrap"><input type="submit" class="adm-btn" name="set_filter"
                                          ...
                                          onClick="return applyFilter();"></span>
        <span class="adm-btn-wrap"><input type="submit" class="adm-btn" name="del_filter"
                                          ...
                                          onClick="return deleteFilter();"></span>
        <?
        $oFilter->End();
        ?>
    </form>
