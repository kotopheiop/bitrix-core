        ...
        <table cellspacing="0" cellpadding="0" class="graph" border="0" align="center">
            <tr>
                <td valign="top" class="graph"><?
                    ...
                    $s = "";
                    foreach ($find_events as $sid) {
                        $s .= "&find_events[]=" . $sid;
                    }
                    ?><img class="graph" src="<? echo htmlspecialcharsbx(
                        "event_diagram.php?rand=" . urlencode(rand()) . "&lang=" . urlencode(
                            LANGUAGE_ID
                        ) . $s . "&find_date1=" . urlencode($arFilter["DATE1_PERIOD"]) . "&find_date2=" . urlencode(
                            ...
                            <tr>
                                ...
                                <td align="right" class="number">(<a
                                            ...
                                            href="<? echo htmlspecialcharsbx(
                                                "event_list.php?lang=" . urlencode(
                                                    LANGUAGE_ID
                                                ) . "&find_event_id=" . urlencode($id) . "&find_date1=" . urlencode(
                                ...
                                <td>[<a class="stat_link"
                                    ...
                                    <a title="<? echo GetMessage("STAT_VIEW_GRAPH") ?>" class="stat_link"
                                       href="event_graph_list.php?lang=<?= LANG ?>&amp;find_events[]=<?= $id ?>&amp;set_filter=Y"><? echo htmlspecialcharsbx(
                                            $name
                                        ) ?></a></font></td>
                            </tr>
                        <?endforeach; ?>
...
?>
    <script type="text/javascript">
        function applyFilter() {
            ...
            <?=$sFilterID?>.
            OnSet('<?=CUtil::JSEscape($sTableID)?>', '<?=CUtil::JSEscape(
        ...
        function deleteFilter() {
            ...
            <?=$sFilterID?>.
            OnClear('<?=CUtil::JSEscape($sTableID)?>', '<?=CUtil::JSEscape(
                $APPLICATION->GetCurPage() . "?lang=" . LANG . "&"
            )?>');
            return false;
        }
