...
class CAllTraffic
{
    public static function DynamicDays($date1 = "", $date2 = "", $site_id = "")
    {
        $arMaxMin = array();
        $is_filtered = false;
        $z = CTraffic::GetDailyList(
            '',
            '',
            $arMaxMin,
            array("DATE1" => $date1, "DATE2" => $date2, "SITE_ID" => $site_id)
        );
        $d = 0;
        while ($zr = $z->Fetch()) {
            $d++;
    ...
    public static function DecParam(
        $arParam,
        $arParamSite = false,
        $SITE_ID = false,
        $DATE = false,
    ...
    // updates traffic counters
    public static function IncParam(
        $arParam,
        $arParamSite = false,
        $SITE_ID = false,
        $DATE = false,
        ...
        if ($SITE_ID <> '' && is_array($arParamSite) && count($arParamSite) > 0) {
            ...
            if (count($arFields) > 0) {
                $rows = $DB->Update(
                    "b_stat_day_site",
                    $arFields,
                    $strWhere . " AND SITE_ID='" . $DB->ForSql($SITE_ID, 2) . "'"
                );
            }
        }
        return $rows;
